...
- Agents SHOULD require a delivery-target challenge before activating a subscription: POST a one-time challenge token to the registered URL, expect the subscriber to echo it back at a known path. The path is `/.well-known/adcp-catalog-webhook-challenge` — AdCP-defined, intentionally not registered with IANA, scoped to this protocol so subscribers implement it without colliding with general-purpose well-known URIs. Closes the "register a victim URL to weaponize webhook fire against a third party" path. The challenge POST itself MUST pass the URL constraints above (HTTPS, TLS floor, address allowlist, rebind defense) — the challenge is not a carve-out from the anti-SSRF rules.
