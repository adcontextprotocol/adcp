...
Product discovery should let a buyer provide the targeting it intends to use
when the media buy is created. Sellers should return products, prices, and
forecasts that already account for that targeting. The split tasks carry these
fields under `criteria`; `get_products` remains the compatibility facade.
...
Products returned by discovery are buyable offers. Non-custom listed
product IDs remain stable for the same logical offer within the seller and
declared cache scope. Request-specific products use `is_custom: true`; their
opaque IDs are stable only within the discovery/refinement lineage that issued
them. Using the product in a direct `create_media_buy`, or accepting a finalized
proposal that purchases it, accepts the targeting resolution disclosed with
that product.
...
requirements stated there. When a seller translates hard prose into structured
targeting that materially affects product eligibility, pricing, or forecasting,
it must confirm the machine-readable interpretation once in the discovery
response's `targeting_resolution.brief_targeting`. Confirmation remains
a best practice when the interpretation does not materially affect those offer
facts. Requirements without a structured AdCP representation remain visible
through the natural-language response.
...
the seller includes the resolved predicate once in response-level
`targeting_resolution.brief_targeting` whenever the interpretation materially
affects product eligibility, pricing, or forecasting; otherwise it remains a
best practice.
...
`product_id` identifies the thing the buyer can purchase. Buyers treat it as
opaque. Sellers reuse a non-custom wholesale ID for the same logical offer
within the seller and declared cache scope, including across reads and webhooks.
Feed and pricing versions communicate temporal catalog mutation; retirement or
replacement may end that identity. Binding commercial terms are immutable under
their `pricing_option_id`: changing a price, floor, currency, model, or priced
applicability requires a new option ID. A custom product ID is lineage-bound and
buyers do not assume stability across independent contexts.

Whenever two concurrent discovery results differ in effective targeting,
disclosed modifications, forecast assumptions, terms, or future overlay
support, their configured `product_id` values must distinguish those
configurations. Pricing alternatives within the same configuration are
distinguished by immutable `pricing_option_id` values. Otherwise a later
`create_media_buy` call carrying the two selected IDs would be ambiguous.
...
On `request_proposals`, the example's `filters`, `targeting_overlay`, and
`required_overlay_support` are nested under `criteria` and `filters` is named
`offer_filters`. `list_products` uses the same `criteria` object without a
brief. A `refine_proposals` entry can replace any present criteria field while
omitted fields inherit from the source proposal. The flattened form below is
the `get_products` compatibility shape.
...
The same outcome-versus-selectability model applies across inventory
dimensions, with one deliberately narrow fixed-restatement exception:

- A product whose observable inventory is already limited to the requested
  placements, properties, or collections may satisfy discovery inherently.
...
- A selected placement set exactly equal to a fixed product's complete
  ...
  package readback but are not independently selectable. This exact-equality
  rule applies at discovery, create, and update and does not require
  `overlay_support.placement_selection`; any partial selection still requires a
  selectable product.
...
- The exact-restatement exception is limited to the product's explicitly
  enumerated placement refs. Property and collection list references are
  opaque, externally mutable list handles, so equality cannot be inferred from
  matching IDs or a point-in-time dereference. Supplying either list on create
  or update therefore requires the corresponding `overlay_support` field even
  when the seller believes the current members resemble fixed product scope.
...
| Omit `placement_selection` to leave it unchanged | Omit either the dimension or the entire `targeting_overlay`; use `placement_selection: null` to clear it | All replaceable targeting dimensions use the same request-only three-state contract: omitted preserves, non-null replaces, and null clears. Effective readback remains strict and non-null. |
...
- requirement `true` matches product support `true` or any valid support object;
...
capability: empty, extension-only, and false-only objects are invalid. Numeric
limits such as `max_values_per_package` and `max_packages` are
...
conditions under which it can be used. Any returned forecast describes the
product's discovery/default scope, not all possible later metro selections. If
the buyer wants a forecast for known metros, it must rediscover with those
values in `targeting_overlay`.

Selectable support does not weaken the product's pricing contract. A returned
`fixed_price` or `floor_price` retains its normal binding semantics for later
selections within declared support; `price_guidance` remains non-binding. If a
supported selection has no current inventory, create returns
`PRODUCT_UNAVAILABLE` rather than silently substituting values or changing the
price. `REQUOTE_REQUIRED` remains an update-only response for a change outside
the original priced envelope.
...
brief and materially affects eligibility, pricing, or forecasting, the seller
must instead confirm its interpretation once in the response-level resolution.
...
Using a returned `product_id` in a direct `create_media_buy`, or accepting a
finalized proposal that purchases it, accepts every disclosed targeting
modification bound to that product.
...
A returned `product_id` identifies a buyable offer and buyers treat it as
opaque. For non-custom wholesale products, sellers reuse the ID for the same
logical offer within the seller and declared `cache_scope`, including across
reads and wholesale-feed webhooks. `wholesale_feed_version` and
`pricing_version` communicate mutation; removal or replacement may retire the
ID. This stability is scoped, not a promise of permanent identity.

For `is_custom: true`, sellers keep the ID stable throughout the same
discovery/refinement lineage so a later `refine` call can reference prior
results and the buyer can purchase the configuration before it expires. The
lineage is carried explicitly by opaque `product_id` and `proposal_id` values;
it is not a transport session and requires no implicit conversational state.
Buyers do not infer custom-ID stability across independent discovery requests
that reference none of those IDs.
...
After expiration, a seller that still recognizes the issued ID for the
authenticated account and referenced lineage rejects creation with
`PRODUCT_EXPIRED`. Once its expiry tombstone is no longer retained, or when the
ID belongs to another account or lineage, `PRODUCT_NOT_FOUND` applies. This
choice cannot be used as a cross-tenant existence oracle. The protocol does not
require sellers to retain tombstones indefinitely.
...
product's bound values remain in force. A refined result keeps the same product
ID only if effective targeting, disclosed resolution, forecast assumptions,
non-pricing terms, and overlay support are unchanged; otherwise the seller
returns a new configured ID and expiry. Pricing alternatives or changed
commercial terms within the same configuration receive distinct immutable
`pricing_option_id` values. This preserves stateless exchanges while allowing
...
product ID binds the targeting resolution, forecast, and non-pricing terms; the
...
availability guarantee. It guarantees selectability only, and any forecast is
for the product's discovery/default scope. A buyer needing a value-specific
forecast rediscovers with concrete values in `targeting_overlay`.
...
3. Define product IDs as opaque buyable identities: non-custom wholesale IDs
   are stable for the same logical offer within seller and cache scope, while
   custom configured IDs are stable only inside one discovery/refinement lineage.
...
5. Exact acceptance does not echo structured targeting; brief-derived hard
   targeting that materially affects the offer is confirmed once at response level.
...
9. `product_id` is opaque. Non-custom wholesale IDs are stable within seller and
   cache scope for the same logical offer; custom IDs are lineage-bound.
