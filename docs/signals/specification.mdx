...
- **Account**: The commercial relationship on whose behalf the request is made (e.g., Nike, Omnicom). See [Accounts Protocol](/docs/accounts/overview).

### Signal Agent Types

**Private Signal Agents** — owned by a single account with exclusive access:
- Signal agents MUST return `AGENT_NOT_FOUND` for unauthorized accounts
