...
**Data-provider signals** come from external data providers who publish definitions at `/.well-known/adagents.json`. Buyers can independently verify that a seller or signal source is authorized to resell them.
...
- **Signal IDs**: Authorize specific local `signals[].id` values in the data provider's published signal definitions — fine-grained control
- **Signal tags**: Authorize all signals with certain tags — scales as the signal set grows
