...
description: Compact reference for executing AdCP Signals Protocol tasks. Use this when interacting with signal agents via call_adcp_agent.
keywords: [adcp tasks, signals api, signal agent, get_signals, activate_signal, audience targeting, quick reference]
---

Use the `call_adcp_agent` tool to execute these tasks against any AdCP signal agent.
...
- `signals`: Array of matching signals with:
  ...
  - `pricing_options`: Array of pricing options (CPM, subscription, etc.) — pass the selected `pricing_option_id` in `report_usage`
...
- `action` (string, optional): `"activate"` (default) or `"deactivate"` for data governance compliance
- `destinations` (array, required): Target destination(s) with `type`, `platform`/`agent_url`, and optional `account`
