...
| `cache_scope` | string | `"public"` or `"account"`. **REQUIRED** when the request included `account`; structurally `"public"` when not. Declares whether `catalog_version` / `pricing_version` describe a universal layer or an account-specific overlay. See [Cache layering](#cache-layering). |
| `unchanged` | boolean | `true` ONLY when the request carried `if_catalog_version` (and/or `if_pricing_version`) matching the agent's current version for the caller's `cache_scope`, in which case `signals[]` MUST be omitted; `catalog_version`, `cache_scope`, and `pricing_version` (when used) MUST still be echoed. `unchanged: false` is permitted as an explicit no-op affirmation but MUST be accompanied by `signals[]`. Absent `unchanged` is equivalent to `false`. Callers receiving `unchanged: true` MUST NOT mutate their local catalog mirror. |
...
When a consumer (storefront, federated marketplace, registry) needs to mirror a signals agent's full priced catalog, set `discovery_mode: "wholesale"` and omit `signal_spec` / `signal_ids`. Wholesale enumeration is symmetric with [`get_products` `buying_mode: "wholesale"`](/docs/media-buy/task-reference/get_products): synchronous, paginated, firm-priced, with partial completion declared via `incomplete[]`.
