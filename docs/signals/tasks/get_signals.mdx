...
- **signals**: Array of matching signals
  ...
  - **pricing_options**: Array of pricing options (CPM, subscription, etc.)
    - **pricing_option_id**: Unique identifier for this pricing option
    - **pricing_model**: Pricing model type (e.g., "cpm", "time")
    - **currency**: Currency code
    - **fixed_price**: Price per unit (per 1,000 impressions for CPM, per time unit for time-based)

Select the pricing option that matches your billing model and pass its `pricing_option_id` in `report_usage` for billing verification. If a signal offers multiple models (e.g., CPM and monthly subscription), choose based on your expected delivery volume and campaign structure.
...
{
  "$schema": "/schemas/signals/get-signals-response.json",
  "message": "Found 1 segment matching your criteria. Two pricing options are available: CPM at $3.50 or a monthly subscription at $500.",
  ...
  "signals": [
    {
      ...
      "pricing_options": [
        ...
        },
        {
          ...
          "pricing": { "model": "percent_of_media", "percent": 15, "currency": "USD" }
        }
      ]
    }
  ]
...
- **signals** (array): Array of matching signals
  ...
  - **pricing_options** (array): Array of pricing options available for this signal. Select one and pass its `pricing_option_id` in `report_usage`.
    ...
    - **pricing_model** (string): Pricing model type (e.g., "cpm", "time")
    - **currency** (string): ISO 4217 currency code
    - **fixed_price** (number): Price per unit (per 1,000 impressions for CPM, per time unit for time-based)
