...
- `get_products` returns the subset selectable or seller-applied on a product in `products[].signal_targeting_options`, with product-scoped `pricing_options` when the signal has a product-specific price.
