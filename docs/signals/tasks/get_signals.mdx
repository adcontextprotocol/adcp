...
- `get_products` declares whether a product has package-level signal targeting and where to discover the selectable menu: inline `products[].signal_targeting_options` when present, or `get_signals` for wholesale products that set `signal_targeting_allowed: true` and omit inline options.
