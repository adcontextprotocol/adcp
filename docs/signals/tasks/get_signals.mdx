...
| `push_notification_config` | PushNotificationConfig | No | Webhook configuration for async status and completion notifications on `brief` semantic discovery. If omitted and the agent returns `submitted`, poll `get_task_status` (legacy `tasks/get`) with `task_id`. Ignored for `wholesale`; agents MUST NOT route wholesale reads through the Submitted arm because this field is present. |

### Async discovery

`discovery_mode: "brief"` MAY return `submitted` when semantic discovery depends on slow provider queries or review that cannot finish before the initial response. If `push_notification_config` is present, the agent sends status and completion notifications to that webhook. Otherwise, poll `get_task_status` (legacy `tasks/get`) with `task_id`. `discovery_mode: "wholesale"` remains a synchronous feed read and reports partial completion via `incomplete[]`, not `submitted`.
