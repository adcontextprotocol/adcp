...
- **signals**: Array of matching signals
  - **signal_ref**: Canonical signal reference. Use `scope: "data_provider"` for signals resolved through published `adagents.json` `signals[]`, `scope: "signal_source"` for source-native signals that are not published in `adagents.json` `signals[]`, and `scope: "product"` only in product-contextual responses.
...
A sales agent that owns or is authorized to apply targeting signals exposes buy-time product eligibility through `get_products`. It can also expose broader cross-product signal discovery through `get_signals` when buyers need discovery or activation before package selection:
