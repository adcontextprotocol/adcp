...
| `if_catalog_version` | string | No | Opaque `catalog_version` token from a prior `get_signals` response. When provided, the agent compares against its current catalog version and MAY return `unchanged: true` (with `signals` omitted) if nothing has changed. Versions are scoped to the (agent, account, filters, `discovery_mode`) tuple. See [Catalog versioning](#catalog-versioning). |
...
- A returned `catalog_version` is scoped to the request parameters that produced it. Callers MUST cache the version alongside the `(account, filters, discovery_mode, destinations, countries)` tuple used.
