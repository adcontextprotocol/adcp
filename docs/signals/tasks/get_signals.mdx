...
- **signals**: Array of matching signals
  ...
  - **pricing_options**: Array of pricing options (CPM, subscription, etc.)
    ...
    - **pricing_model**: Pricing model type (e.g., "cpm")
    - **currency**: Currency code
    - **fixed_price**: Price in the specified currency
...
- **signals** (array): Array of matching signals
  ...
  - **pricing_options** (array): Array of pricing options (CPM, subscription, etc.)
    - **pricing_option_id** (string): Unique identifier for this pricing option
    - **pricing_model** (string): Pricing model type (e.g., "cpm")
    - **currency** (string): Currency code
    - **fixed_price** (number): Price in the specified currency
...
def generate_signals_message(signals, request):
    ...
    if len(signals) == 1:
        ...
        price_commentary = f"The CPM is ${signal.pricing_options[0].fixed_price}, which {'is well within' if signal.pricing_options[0].fixed_price <= request.filters.max_cpm else 'exceeds'} your budget."
