...
- **signals**: Array of matching signals
  - **signal_ref**: Canonical signal reference. Use `scope: "data_provider"` for signals resolved through an adagents.json signal catalog, `scope: "signal_source"` for source-native signals that are not catalog-published, and `scope: "product"` only in product-contextual responses.
  - **signal_id**: Deprecated legacy SignalId object. New clients should read `signal_ref`.
...
- `get_products` declares whether a product has package-level signal targeting. Inline `products[].signal_targeting_options` can carry a product-specific menu, price, activation handle, default, grouping hint, or brief/refine-selected subset; wholesale products can omit inline options and use `get_signals` as the selectable signal feed.
