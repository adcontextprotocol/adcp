...
**Agent destinations** — The `activation_key` confirms the signal is live on the sales agent. The SA records the activation internally and can apply signal-based targeting when fulfilling media buys through `create_media_buy`. When the seller exposes selectable signals through `Product.signal_targeting_options`, the buyer selects those signals in `packages[].targeting_overlay.signal_targeting_groups`, carrying the selected `signal_ref`, `pricing_option_id`, and any separate seller execution handle the product option requires. The buyer does not need to know which DSP the SA uses — downstream platform coordination is the SA's responsibility.
