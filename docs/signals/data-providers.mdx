...
Data providers (Polk, Experian, Acxiom, etc.) own valuable audience and contextual data, but integrating with the growing ecosystem of AI-powered advertising agents presents challenges:

**Discovery is fragmented.** Each signals agent (LiveRamp, The Trade Desk, etc.) needs custom integrations to know what signals you offer. There's no standard way for an AI agent to ask "what automotive purchase intent signals does Polk have?"
...
```json
{
  ...
  "contact": {
    ...
    "email": "partnerships@acme-data.com",
...
| Field | Type | Description |
|-------|------|-------------|
| `description` | string | Detailed description of methodology and coverage |
| `category` | enum | High-level category (e.g., `purchase_intent`, `demographics`, `interests`) |
...
| `pricing_guidance` | object | CPM range guidance: `{ cpm_range: { min, max }, currency }` |
| `coverage_notes` | string | Geographic coverage, match rates, availability |
| `methodology_url` | string | URL to documentation on signal derivation |
...
**Targeting**: Target users with specific values (e.g., "users who own tesla or bmw").
  ...
  "authorized_agents": [
    {
      ...
      "signal_ids": ["likely_tesla_buyers", "luxury_auto_intenders"]
    }
  ]
}
```
...
The buyer fetches `https://acme-data.com/.well-known/adagents.json` and checks:
...
// Binary targeting
{
  "signal_id": { "source": "catalog", "data_provider_domain": "acme-data.com", "id": "likely_tesla_buyers" },
  "value_type": "binary",
  "value": true
}
...
{
  "signal_id": { "source": "catalog", "data_provider_domain": "acme-data.com", "id": "vehicle_ownership" },
  "value_type": "categorical",
  "values": ["tesla", "bmw"]
}
...
// Numeric targeting
{
  "signal_id": { "source": "catalog", "data_provider_domain": "acme-data.com", "id": "purchase_propensity" },
  "value_type": "numeric",
  "min_value": 0.7
}
```
...
- Signal comes from an external data provider (Polk, Experian, etc.)
  ...
  "signals": [
    {
      ...
      "pricing_guidance": {
        "cpm_range": { "min": 2.50, "max": 5.00 },
        "currency": "USD"
      },
      "coverage_notes": "US coverage, ~85% match rate on major DSPs",
      "methodology_url": "https://polk.com/methodology/purchase-intent"
    },
    {
      "id": "vehicle_ownership",
      "name": "Current Vehicle Ownership",
...
Include `description`, `coverage_notes`, and `methodology_url` so buyers understand what they're purchasing.
...
{
  "signals": {
    "data_provider_domains": ["polk.com", "experian.com", "acxiom.com"]
  }
}
```
...
- [get_signals Task](/docs/signals/tasks/get_signals) - Signal discovery API
- [activate_signal Task](/docs/signals/tasks/activate_signal) - Signal activation API
