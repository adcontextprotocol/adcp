...
A network declares its managed properties in its [`brand.json`](/docs/governance/brand/brand-json) using the `relationship` field on each property. This is the operator's public declaration of which publishers they work with — the AdCP equivalent of `sellers.json` in programmatic advertising.
...
```json
{
  ...
  "brands": [{
    ...
    "properties": [
      ...
      { "type": "website", "identifier": "assistant-alpha.example.com", "relationship": "managed" },
      { "type": "website", "identifier": "assistant-beta.example.com", "relationship": "managed" },
      { "type": "website", "identifier": "search-gamma.example.com", "relationship": "represented" }
    ],
    "agents": [{
      "type": "sales",
...
The `relationship` field has three values:

| Relationship | Meaning | Example |
|---|---|---|
| `owned` (default) | You own and operate this property | Your own website |
| `managed` | You manage monetization for this property | A publisher network managing ad tech for partner sites |
| `represented` | You sell inventory for this property as an exchange or SSP | An exchange representing a publisher's inventory |
...
|---|---|---|---|
| `brand.json` (operator) | The network/SSP | "I manage/represent these publishers" | `sellers.json` |
| `adagents.json` (publisher) | Each publisher | "This network's agents are authorized to sell my inventory" | `ads.txt` |

Both sides must agree. The network declares the relationship in `brand.json`, and each publisher confirms by authorizing the network's agents in their `adagents.json`. If only one side declares, the relationship is incomplete — the network health dashboard flags this as a missing pointer (network declared but publisher hasn't deployed) or an orphaned pointer (publisher authorized but network hasn't declared).

## adagents.json for networks

Each publisher in the network authorizes the network's agent in their [`adagents.json`](/docs/governance/property/adagents):
...
When an ad network sells [SI Chat Protocol](/docs/sponsored-intelligence/si-chat-protocol) sessions on behalf of brands, it acts as an intermediary in the session flow. The brand syncs offerings to the network via `sync_catalogs` with `type: "offering"`, and the network forwards them to underlying platforms. When a platform triggers a session, the network routes it to the correct brand agent.
...
<Note>
For the SI Chat Protocol session lifecycle and capability negotiation details, see the [SI Chat Protocol](/docs/sponsored-intelligence/si-chat-protocol) and [implementing SI hosts](/docs/sponsored-intelligence/implementing-si-hosts).
</Note>
