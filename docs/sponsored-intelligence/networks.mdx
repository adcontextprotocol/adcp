...
A network declares its managed properties in its [`brand.json`](/docs/brand-protocol/brand-json) using the `relationship` field on each property. This is the operator's public declaration of which publishers they work with — the AdCP equivalent of `sellers.json` in programmatic advertising.
