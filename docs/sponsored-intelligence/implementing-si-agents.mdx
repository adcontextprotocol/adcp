...
1. `si_check_availability` - Respond to anonymous availability checks
...
```json
{
  ...
  "endpoint": {
    "transport": "mcp",
    "url": "https://yourbrand.example/si-agent"
  },
  "capabilities": {
    "modalities": {
      "voice": false,
...
server.tool(
  ...
  "Start a conversational session with this brand agent",
  {
    ...
    offer_id: { type: "string", description: "Brand-specific offer reference (optional)" },
  },
  async ({ context, identity, media_buy_id, offer_id }) => {
...
> "I'm connecting you with Delta to help with your Boston flight. They can check availability and offers for you."
