...
description: "Sponsored Intelligence (SI) is the AdCP protocol for conversational brand experiences in AI assistants. Defines session lifecycle, capability negotiation, identity consent, UI components, and ACP commerce handoff."
...
On AI assistant surfaces, the [Trusted Match Protocol (TMP)](/docs/trusted-match) triggers SI sessions. TMP handles ad selection; SI handles engagement:
...
4. The winning offer's `brand`, `creative_manifest`, and `media_buy_id` flow into SI's `initiate` call

This separation means SI never needs to evaluate ad relevance or user targeting — TMP handles that upstream. SI receives a pre-selected package and focuses on the conversational experience.

### The Flow

1. **TMP selects** → Context Match + Identity Match determine the winning package (see [AI Assistants surface](/docs/trusted-match/surfaces/ai-assistants))
...
4. **Session initiation** → Host invokes brand agent with context + capabilities (`si_initiate_session`), passing `media_buy_id` and `package_id` from the TMP offer
