...
2. **Availability check** (optional) → Host verifies offer/product is available (`si_check_availability`)
...
The `context` is a conversation handoff - the host tells the brand agent what the user needs, and the brand agent responds naturally. The `offer_id` references a campaign promotion (like free upgrades on eligible flights) that the brand knows how to apply.
