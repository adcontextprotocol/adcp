...
|-------|------|----------|-------------|
| `checkout_url` | uri | No | URL for completing the purchase |
| `checkout_token` | string | No | Opaque token for the checkout flow |
| `product` | object | No | Product details relevant to the purchase |

Hosts SHOULD use `acp_handoff` data to initiate the appropriate commerce flow. The `checkout_token` is opaque to the host — it correlates the SI session with the subsequent transaction on the brand's side.
