...
**How these surface in delivery:** `viewed_seconds` is reported via `viewability.viewed_seconds` (the same `standard` threshold governs both `viewable_rate` and `viewed_seconds`). `attention_seconds` and `attention_score` are vendor-defined with no graduated standard — they flow through `vendor_metric_values[]` with `metric_id: "attention_seconds"` or `"attention_score"` and the vendor identified on the row. The optimization-side vendor-binding follow-up is tracked in [#4644](https://github.com/adcontextprotocol/adcp/issues/4644).
...
<Card title="Conversion tracking & optimization goals" icon="arrow-right" href="/docs/media-buy/conversion-tracking/index">
