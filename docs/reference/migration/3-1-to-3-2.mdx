...
<Warning>
**3.2 is in release candidate.** RC.2 is the current signed protocol
checkpoint. TypeScript `@adcp/sdk@14.0.0-rc.35`, Python `adcp==8.0.0b14`, and
Go `adcp/v3@v3.2.1` embed RC.1 exactly; retain that wire pin when using those
packages, or use the signed RC.2 artifacts directly for exact RC.2 testing.
Python and Go higher-level coverage remains role-dependent. Keep production
traffic pinned to `"3.1"` while validating an exact 3.2 checkpoint in staging.
</Warning>

<Warning>
**Trusted Match cache-namespace conformance is prospective until October 25,
2026.** Version 3.1.22 already contains emergency single-user privacy and
request-partitioning corrections. Affected Context Match routers must bypass
caching until both request partitioning and the trusted result-affecting
security/configuration namespace are implemented. See the
...
For prerelease artifacts and SDK timing, use the [3.2 prerelease program](/docs/reference/3-2-beta).
...
| 10 | Trusted Match routers | Preserve the v3.1.22 privacy and request-hash corrections; bypass affected Context Match caches until the prospective 3.2 trusted evaluation namespace is implemented. |
