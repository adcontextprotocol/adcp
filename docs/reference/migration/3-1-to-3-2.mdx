...
The 3.x `get_products` facade leaves the key optional only for a guaranteed
side-effect-free synchronous read, where a seller may ignore a supplied key. An
execution arm that allocates a task, finalizes a proposal, or otherwise changes
observable state requires a key and applies replay protection before that
effect. New callers should use the compact tasks so the read/mutation boundary
is explicit.
