...
| 2 | SDK and codegen maintainers | Generate from the signed `3.2.0-beta.11` protocol tarball, not `main` or `/schemas/latest/`. |
...
SHOULD omit new `aggregated_totals` output. Buyers MUST tolerate both fields
being absent and MUST NOT treat the legacy currency as an aggregation currency.
