...
embeds `3.2.0-rc.2`, while Python `adcp==8.0.0b14` and Go `adcp/v3@v3.2.1`
still embed `3.2.0-rc.1`; Python and Go higher-level coverage remains
role-dependent. Keep production traffic pinned to `"3.1"` while validating an
...
| 2 | SDK and codegen maintainers | Generate from the signed `3.2.0-rc.3` protocol tarball, not `main` or `/schemas/latest/`. |
...
| 8 | Compliance operators | Test RC.3 from its signed assets; no SDK pairing covers RC.3 yet. `@adcp/sdk@14.0.0-rc.36` tests RC.2, and Python `adcp==8.0.0b14` or Go `adcp/v3@v3.2.1` test RC.1. Retain the exact package and wire versions. |
