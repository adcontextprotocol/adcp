...
<Warning>
**3.2 is in beta.** Beta.10 is the current protocol checkpoint. TypeScript
`@adcp/sdk@14.0.0-beta.28` and Python `adcp==8.0.0b11` embed beta.10 exactly.
...
| 2 | SDK and codegen maintainers | Generate from the signed `3.2.0-beta.10` protocol tarball, not `main` or `/schemas/latest/`. |
...
| 8 | Compliance operators | Test beta.10 from its signed assets, or pair `@adcp/sdk@14.0.0-beta.28` or Python `adcp==8.0.0b11` with exact beta.10; retain the exact package and wire versions. |

## Required to claim 3.2 behavior

### Serve and echo the exact release

During beta, prerelease matching is exact. A seller serving beta.10 advertises
`"3.2-beta.10"`. Do not advertise stable `"3.2"` before GA, silently negotiate
