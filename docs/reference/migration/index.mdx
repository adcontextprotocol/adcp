...
| Capabilities | `compliance_testing` protocol in `supported_protocols` | New requirement | [Capability discovery](/docs/protocol/get_adcp_capabilities#compliance_testing) |

<Warning>
**Comply-blocking requirement:** `buying_mode` on `get_products` is checked by comply testing. Sellers must handle all three buying modes (`browse`, `brief`, `refine`). See the [v3 readiness checklist](/docs/reference/migration/v3-readiness) for details.
</Warning>
