...
<Info>
**Preparing for 3.2?** Start with the [3.1 to 3.2 migration guide](/docs/reference/migration/3-1-to-3-2). For the signed RC.1 artifact, exact wire pins, matched SDKs, and feedback guidance, read the [3.2 prerelease program](/docs/reference/3-2-beta).
</Info>

<Warning>
**Implementing Reliable Reporting?** Read the active
[consumer-status breaking-change notice](/docs/reference/migration/reliable-reporting-status-loop)
before adopting the opt-in extension or planning for the next eligible minor.
Published RC.1 remains unchanged.
</Warning>

<Warning>
**Implementing Trusted Match Context Match?** Read the
[privacy and context-cache breaking/security notice](/docs/reference/migration/trusted-match-context-cache).
Version 3.1.22 shipped emergency privacy and request-partitioning corrections;
the additional trusted cache namespace is prospective AdCP 3.2 conformance
effective October 25, 2026. Affected routers must bypass caching until both
...
| Task status | Legacy `task_status` and `response_status` fields MUST NOT be emitted alongside v3 `status` — remove both | Remove | [Task lifecycle](/docs/building/by-layer/L3/task-lifecycle) |
...
| Idempotency | `idempotency_key` required on all mutating requests (UUID v4) | New requirement | [Security § Idempotency](/docs/building/by-layer/L1/security) |
| Request signing | RFC 9421 Ed25519 signing profile (optional in 3.0, mandatory under AdCP Verified) | Additive | [Security § Request signing](/docs/building/by-layer/L1/security) |
| Webhook signing | Unified on RFC 9421 profile, baseline-required for sellers; HMAC fallback deprecated (removed in 4.0) | New requirement | [Webhooks](/docs/building/by-layer/L3/webhooks) |
| Webhook idempotency | `idempotency_key` required on every webhook payload | New requirement | [Webhooks § Reliability](/docs/building/by-layer/L3/webhooks) |
...
<CardGroup cols={2}>
  <Card title="3.1 to 3.2" icon="rocket" href="/docs/reference/migration/3-1-to-3-2">
    Role-based beta checklist, required behavior changes, capability adoption, and rollback
  </Card>
  <Card title="Targeting-aware discovery" icon="bullseye" href="/docs/reference/migration/targeting-aware-discovery">
  ...
  <Card title="Cross-role governance" icon="shield" href="/docs/reference/migration/cross-role-governance-enforcement">
    Adopt the experimental task-scoped authorization and outcome contract as one migration
  </Card>
  <Card title="Trusted Match cache security" icon="shield-check" href="/docs/reference/migration/trusted-match-context-cache">
    Apply the v3.1.22 privacy corrections and prepare for the prospective 3.2 trusted cache namespace
  </Card>
</CardGroup>
