...
<Warning>
`buying_mode` on `get_products` is checked by storyboard testing. Sellers must handle the `brief`, `wholesale`, `refine`, and `open` buying modes. See the [v3 readiness checklist](/docs/reference/migration/v3-readiness) for details.
</Warning>
