...
|---|---|---|---|
| `idempotency_key` on mutating requests | Optional | **Required** — UUID v4 in envelope | Generate fresh UUID v4 per logical operation. Persist keys across agent instances. Handle `IDEMPOTENCY_CONFLICT` and `IDEMPOTENCY_EXPIRED`. See [Security § Idempotency](/docs/building/implementation/security). |
| Webhook signing | HMAC-SHA256 with `push_notification_config.authentication` (required) | RFC 9421 profile (baseline-required for sellers); HMAC fallback available through 3.x | Publish webhook-signing key at `jwks_uri` on `brand.json` with `adcp_use: "webhook-signing"`. Drop `push_notification_config.authentication` from new configs. Receivers verify against the sender's JWKS. HMAC removed in 4.0. |
...
- **Webhook signing unified on RFC 9421** — Baseline-required for sellers emitting webhooks. Sellers publish a webhook-signing key at `jwks_uri` on their `brand.json` `agents[]` entry with `adcp_use: "webhook-signing"`. 14-step webhook verifier checklist with `webhook_signature_*` error codes. HMAC-SHA256 remains a legacy fallback through 3.x (removed in 4.0).
