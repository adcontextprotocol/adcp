...
| Capabilities | No `compliance_testing` protocol | Agents declare compliance testing via a `compliance_testing` capability block; `supported_protocols` never gained `compliance_testing` as a value. | Agents implementing `comply_test_controller` include a `compliance_testing: { scenarios: [...] }` block in `get_adcp_capabilities`. Storyboard runners check for the block before running deterministic steps. |
...
- **Compliance testing capability** — Agents include a `compliance_testing: { scenarios: [...] }` block in `get_adcp_capabilities` declaring which `comply_test_controller` scenarios they support. The block's presence is the signal; it is not a `supported_protocols` value. Storyboard runners use the block to determine whether deterministic testing steps can be validated.
