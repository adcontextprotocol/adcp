...
| Capabilities | No `compliance_testing` protocol | `compliance_testing` added to `supported_protocols` enum | Agents implementing `comply_test_controller` should declare `compliance_testing` in capabilities. Schema: `protocol/get-adcp-capabilities-response.json`. |
...
- **Specialisms + compliance catalog** — Storyboards ship in the protocol at `/compliance/{version}/` (universal + domains + specialisms + test-kits). New `specialisms` field on `get_adcp_capabilities` with 21 values across 6 domains. Four 3.1 archetypes (`sales-streaming-tv`, `sales-exchange`, `sales-retail-media`, `measurement-verification`) carry `status: preview`. Per-version protocol tarball at `/protocol/{version}.tgz`. See the [Compliance Catalog](/docs/building/compliance-catalog).
