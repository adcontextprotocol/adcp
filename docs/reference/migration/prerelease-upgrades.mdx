...
- **Governance list webhook body signatures retired** — Property-list and collection-list change payloads no longer require a body-level `signature` in 3.2. The field remains optional and deprecated so existing 3.x payloads still validate, but receivers ignore it for authentication; RFC 9421 `Signature`, `Signature-Input`, and `Content-Digest` headers are authoritative. New senders omit the field. It is removed in 4.0.
