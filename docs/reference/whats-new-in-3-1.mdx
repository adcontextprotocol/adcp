...
<Warning>
**Status: 3.1 release-candidate preparation.** The spec is feature-complete for 3.1; the published `@adcp/sdk` and runtime grader are still advisory against release-candidate surfaces until the RC is cut. Beta adopters can pin `adcp_version: "3.1-beta"` today, move to `"3.1-rc.N"` during RC validation, and migrate the pin to `"3.1"` on GA without code changes.
</Warning>
...
- **Creative formats have a canonical vocabulary.** 12 canonical `format_kind` values + a publisher catalog discovery surface + a projection-ref mechanism. Creative agents also advertise buildable canonical outputs through `creative.supported_formats[]` with stable `capability_id` values.
