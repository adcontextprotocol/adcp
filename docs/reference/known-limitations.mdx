...
- **No protocol-level breach-notification SLA.** AdCP does not specify a normative "seller MUST notify buyer within N hours of suspected key compromise" clock. Time-bound notification commitments continue to live in DPAs between parties, and a future major version may tighten the SLA itself. Two narrowings already land inside 3.x and partially close the gap: (a) the `identity.compromise_notification` webhook event (documented in [Webhooks](/docs/building/implementation/webhooks)) gives operators a machine-readable channel to push a compromise signal to subscribed counterparties so caches and tokens invalidate on receipt, and (b) operators SHOULD publish a public security contact per [RFC 9116](https://datatracker.ietf.org/doc/html/rfc9116) at `/.well-known/security.txt` including a security-contact email and a PGP key for encrypted disclosures — so an out-of-band path exists even when the in-band webhook channel is down.
- **No coordinated vulnerability disclosure baked into the protocol.** Individual agent operators SHOULD publish `/.well-known/security.txt` per [RFC 9116](https://datatracker.ietf.org/doc/html/rfc9116) with a security-contact address and PGP key (same file as the breach-notification bullet above); there is no normative AdCP requirement on the file's contents yet, and no protocol-level CVD policy — a future version is expected to promote the `security.txt` SHOULD to a MUST and publish an industry CVD policy at a known URL.
- **No cleartext PII on the wire, and hashed PII is pseudonymous — not anonymous.** The `hashed_email` and `hashed_phone` fields in `sync_audiences` require hashing on the buyer side (scheme negotiated via `privacy.audience_hash_schemes` — `hmac_sha256` recommended, `sha256_plain` legacy) — the schemas do not accept cleartext. These hashed values are pseudonymous identifiers, not anonymization: the email universe is small enough (~5B addresses) that `sha256_plain` is invertible by rainbow table, and any party holding the HMAC key can still recover membership for a known cleartext. See [Privacy Considerations §PII Hashing](/docs/reference/privacy-considerations#pii-hashing) for the normative scheme and its limits. If you need cleartext PII on the wire, AdCP is not the right carrier.
