...
| `creative.localization` request/readback | `verifier_constraints` (unique/disjoint locale identities, exact request→sync→list identity, exact source count, status/launch compatibility, archive atomicity, safe source upsert, aggregate status precedence) | Draft-07 `uniqueItems` cannot enforce uniqueness by property or cross-object status rules, and a response schema cannot compare itself with the initiating request or prior stored source. Creative verifiers reject duplicate or drifted locale identities, require complete accepted readback, enforce lifecycle pairs and whole-creative archive, reject ambiguous source changes when localization is omitted, and recompute the top-level status. |
