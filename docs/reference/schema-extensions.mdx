...
| `required_when` | array of conditions | Storyboard-enforced required-when rules (3.x). Each condition is one of: `{ "field": "...", "non_empty": true }`, `{ "field": "...", "equals": <value> }`, `{ "field": "...", "any_subfield_present": true }`. The array is OR-joined; wrap with `{ "any_of": [...] }` or `{ "all_of": [...] }` for explicit boolean shape. |
| `schema_required_when` | condition | When the rule promotes from storyboard-enforced to schema-required. Typically keyed on `adcp.supported_versions` containing a major version (e.g., 4 for the 4.0 cutover). |
...
This keyword landed in 3.x as the first structured-validation annotation. Other rule-bearing fields (`idempotency` discriminated union, `request_signing.warn_for`/`required_for` invariants, `webhook_signing.algorithms` allowlist) are candidates for migration in follow-up minors — tracked at [#TODO migrate prose-rules to x-adcp-validation](https://github.com/adcontextprotocol/adcp/issues).
