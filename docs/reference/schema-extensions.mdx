...
| `creative.localization` request/readback | `verifier_constraints` (unique/disjoint locale identities, exact source count, creative-scope review equality, aggregate status precedence) | Draft-07 `uniqueItems` cannot enforce uniqueness by property or cross-object status rules. Creative verifiers reject duplicate source/target locales or IDs, require exactly one readback source, preserve creative-wide review consistency, and recompute the top-level status. |
