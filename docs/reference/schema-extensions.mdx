...
| `creative.localization` request/readback | `verifier_constraints` (capability gate, canonical locale form, unique/disjoint identities, valid default and fallback references, strict RFC 4647 Lookup followed by explicit fallback rules, exact request→sync→list identity, source-asset equality, transactional replacement, and delivery attribution) | Draft-07 cannot compare a request with negotiated capabilities, fully enforce canonical BCP 47 form, express uniqueness/reference integrity by property, compare a response with the initiating request/prior stored source, or require delivery attribution only for localized parents. Creative conformance scenarios exercise those rules and require per-item `localization_unavailable` rather than partial readback. |
