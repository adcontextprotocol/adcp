...
| `identity.key_origins` | `verifier_constraints` (`purpose_anchoring`) | Every purpose listed MUST have a corresponding signing posture declared elsewhere on the response. Cross-field rule. See [security.mdx §Origin separation](/docs/building/by-layer/L1/security#origin-separation). |
...
| `creative.localization` request/readback | `verifier_constraints` (capability gate, canonical locale form, unique/disjoint locale identities, exact request→sync→list identity, exact source count, status/launch compatibility, archive atomicity, safe source upsert, aggregate status precedence) | Draft-07 cannot compare a request with negotiated capabilities, enforce canonical BCP 47 form, express uniqueness by property or cross-object status rules, or compare a response with the initiating request or prior stored source. Creative verifiers reject undeclared locale/mode/count requests and duplicate or drifted locale identities, require complete accepted readback, enforce lifecycle pairs and whole-creative archive, reject ambiguous source changes when localization is omitted, and recompute the top-level status. |
