...
| `3.2.0-beta.10` | Add managed reporting delivery, authenticated principals, audio VAST and DOOH support, programmed channel collections, committed collection selection, and expanded conformance coverage | Current protocol checkpoint with exact TypeScript support in `@adcp/sdk@14.0.0-beta.28` and exact Python schema/type support in `adcp==8.0.0b11` |
...
| An SDK maintainer | Generate from the current signed beta.10 tarball, publish an exact support statement, and route accepted protocol corrections into a later beta. |
| A compliance operator | Test beta.10 from its signed assets or with `@adcp/sdk@14.0.0-beta.28` or Python `adcp==8.0.0b11`; retain the exact package and wire versions and do not issue a 3.2 badge from a beta checkpoint. |
