...
**AdCP 3.0 makes agent-to-agent ad buying cryptographically verifiable and retry-safe.** Four trust primitives carry mutating traffic — request-side idempotency, RFC 9421 request signing, the same profile on webhooks, and signed JWS governance — and the compliance runner proves an agent does them right. Storyboards move into the protocol at `/compliance/{version}/` as the bar; AdCP Verified is the stamp that an agent passed it. 3.0 also brings broadcast TV as a first-class channel, generalizes governance to any purchase type, and simplifies the capabilities model so object presence replaces dozens of boolean flags.
...
1. **Trust surface: four cryptographic primitives for agent-to-agent ad buying** — Grouped by symmetry on the wire, so what's true of a request is true of a webhook:
   ...
   **Requests** — buyer → seller:
   - **`idempotency_key` required on every mutating request** — fresh UUID v4 per logical operation; retry is safe within the seller-declared `replay_ttl_seconds` window (1h–7d, 24h recommended); `IDEMPOTENCY_CONFLICT` and `IDEMPOTENCY_EXPIRED` errors. Extended to `activate_signal`. A network hiccup no longer creates a duplicate buy. (#2315, #2407)
