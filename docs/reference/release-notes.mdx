...
| `3.2.0-beta.11` | Add `get_principal` to the MCP media-buy role profile and align the public training agent with the latest beta bundle | Current protocol checkpoint with exact TypeScript support in `@adcp/sdk@14.0.0-beta.29`; Python support is pending |
...
| A compliance operator | Test beta.11 from its signed assets or with `@adcp/sdk@14.0.0-beta.29`; Python `adcp==8.0.0b11` remains paired with beta.10. Retain the exact package and wire versions and do not issue a 3.2 badge from a beta checkpoint. |
