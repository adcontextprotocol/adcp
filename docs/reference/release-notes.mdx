...
3. **Shows and Episodes** — Content dimension for products representing persistent programs (podcasts, TV series, YouTube channels). `show_ids` on products, `shows` array in `get_products` responses. Distribution identifiers for cross-seller matching, episode lifecycle states, break-based ad inventory, talent linking, show relationships, international content ratings.

4. **AI Media Channel** — `ai_media` added to the media channel taxonomy for AI platform advertising (AI assistants, AI search, generative AI experiences).
...
6. **Account Model Simplification** — Removed `account_resolution` capability. `require_operator_auth` now determines both auth model and account reference style.

7. **Certification Program** — Three-tier certification (Basics, Practitioner, Specialist) taught by Addie through interactive chat. Vibe coding build projects for non-technical participants.

8. **Documentation Overhaul** — Illustrated protocol walkthroughs, buy-side guides, FAQ expansion, schema-compliant examples, and collapsed sidebar navigation.
...
| Brand logo fields | `tags` only | Added `orientation`, `background`, `variant`, `usage` |
