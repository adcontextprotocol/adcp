...
| Signal delivery targeting | `deliver_to: { deployments: [...] }` | Top-level `destinations` array |
...
<Warning>
Requests using the old `deliver_to` wrapper will be silently ignored by the server — targeting will not be applied. Update clients to use the top-level fields.
</Warning>
