...
- **3.1.0** — sellers MUST accept reads that carry `idempotency_key`; SHOULD reject reads that omit it (MAY accept the omission during the 3.1.x maintenance window only for guaranteed side-effect-free synchronous reads). Calls that return `Submitted` or commit/finalize state are never covered by the omission grace.
...
| A buyer with hand-rolled MCP clients (curl, thin clients, raw OpenAPI codegen) | Add `idempotency_key` to polymorphic tools such as `get_products` now: calls that allocate tasks or commit/finalize state have no omission grace, and buyers cannot predict the execution arm. Add it to guaranteed side-effect-free synchronous reads (`list_creative_formats`, `list_accounts`, etc.) before the 3.2 cut; 3.1.x sellers MAY accept omission only on that narrower read path. |
