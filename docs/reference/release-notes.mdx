...
Late-RC security hardening on the way to 3.0.0 GA. The rc.4 entry below carries the governance changes; this rev adds two normative tightenings from the external 3.0 security review.

**1. Idempotency cache insert-rate limiting is now MUST.** Sellers MUST apply per-`(authenticated_agent, account)` rate limits on idempotency-cache inserts (separate from request rate limits) and MUST return `RATE_LIMITED` with `retry_after` when the configured ceiling is crossed. First-deployment ceiling: **60 inserts/sec sustained per agent (3,600/min), with burst to 300/sec over rolling 10-second windows** — sized against a realistic high-volume launch pattern (10 media buys/min × 10 packages × 10 creatives, with 3–5× headroom) and consistent with the existing 100k-per-keyid webhook replay cap and 1M-per-keyid request replay cap. Tunable per deployment. Closes a nonce-flood DoS amplification vector. See [security.mdx idempotency § bullet 8](/docs/building/implementation/security#idempotency) for normative detail.

**Implementer migration**: sellers rate-limiting at the request layer today MUST add a cache-insert-layer limit if they don't already have one. Buyers with burst-onboarding traffic above 60/sec sustained SHOULD coordinate with their seller to tune the ceiling before GA; well-behaved launch patterns are covered by the 3–5× headroom. Buyers MUST respect `retry_after`.

**2. Webhook-registration signing is now MUST for signing-capable sellers.** Sellers that support request signing MUST reject webhook-registration requests carrying `push_notification_config.authentication` over bearer-only (unsigned) transport, with `request_signature_required`. Structural defense against on-path mutators silently injecting or stripping the `authentication` block during onboarding — a 9421-signed registration cryptographically commits to the body. Sellers that do not support request signing at all fall back to the existing log-and-alarm posture (unchanged).

**Implementer migration — scoped breakage warning**: this flips registration behavior for sellers with a conditional signing posture (signs some operations but accepts bearer on webhook registration today). Any buyer that previously registered webhooks with `authentication` blocks against such a seller via bearer transport must switch to 9421-signed registration, or the seller must carve `authentication`-carrying registration requests out of its bearer-authenticated path. Fully unsigned-only sellers and fully 9421-required sellers are unaffected.

**3. Conformance coverage.** Negative test vector `027-webhook-registration-authentication-unsigned.json` lands with the tightening. Runtime storyboard grading for the idempotency insert-rate ceiling requires a burst-runner test-kit contract not yet defined — covered for 3.0 GA by spec-level MUST plus implementer attestation in `universal/idempotency.yaml`, with runtime grading tracked as a follow-up.

**4. Error-code vocabulary cleanup** (resolves #2704). The uniform-response MUST from rc.4 (#2691) forbids sellers from minting custom `*_NOT_FOUND` codes for typed parameters. The spec itself was out of compliance — 12 custom codes appeared in task-reference pages. This cleanup aligns the spec with the rule:

- **Promoted to standard vocabulary:** `PLAN_NOT_FOUND` (used across `report_plan_outcome`, `get_plan_audit_logs`, `check_governance`; recovery via `sync_plans`). Same uniform-response MUST as the other entity-specific codes.
- **Collapsed to `REFERENCE_NOT_FOUND`:** `CHECK_NOT_FOUND`, `CAMPAIGN_NOT_FOUND`, `BRAND_NOT_FOUND`, `STANDARDS_NOT_FOUND`, `FORMAT_NOT_FOUND`, `AGENT_NOT_FOUND`, `SIGNAL_AGENT_SEGMENT_NOT_FOUND`, `SEGMENT_NOT_FOUND`, `AUDIENCE_NOT_FOUND`, `CATALOG_NOT_FOUND`, `EVENT_SOURCE_NOT_FOUND`. Implementers name the typed parameter that failed to resolve in `error.field` (per the existing `REFERENCE_NOT_FOUND` normative clause in [`error-handling.mdx`](/docs/building/implementation/error-handling)).
- **Signals spec auth-uniformity tightened:** `docs/signals/specification.mdx` previously said "Private signal agents MUST return `AGENT_NOT_FOUND` for unauthorized accounts." That rule now explicitly routes through `REFERENCE_NOT_FOUND` — the same response regardless of whether the agent exists or the caller is unauthorized — preventing cross-tenant enumeration of private agents. No behavior change for implementers already following the uniform-response MUST.

**Implementer migration**: sellers returning any of the 11 collapsed codes today MUST switch to `REFERENCE_NOT_FOUND` with `error.field` identifying the failed parameter. Sellers returning `PLAN_NOT_FOUND` today continue to do so. None of the 12 codes appeared in JSON schemas, so no schema-enum migration is required; this is a prose-level cleanup.

**What did NOT change.** The admission-pressure threshold structural shape (short-window ratio / medium-window ratio / long-window ratio / proportional ceiling) is already normative; concrete starting numbers live in the non-normative [Webhook Verifier Tuning Guide](/docs/building/implementation/webhook-verifier-tuning). A separate restructure was already landed in PR #2548.
