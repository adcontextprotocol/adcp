...
Authoritative version-by-version release record for AdCP, with cumulative change detail, migration guidance, and per-version adoption notes. For a curated adopter overview of the 3.1 minor release, see [What's New in AdCP 3.1](/docs/reference/whats-new-in-3-1). For a focused upgrade checklist, see [Migrating from 3.0 to 3.1](/docs/reference/migration/3-0-to-3-1). For the at-a-glance status of every published version, see [Versions & Compatibility](/docs/reference/versions). For source-level changelogs, see [CHANGELOG.md](https://github.com/adcontextprotocol/adcp/blob/main/CHANGELOG.md). For version stability, schema-change scope, and the 3.x guarantees see [Versioning & Governance](/docs/reference/versioning). For v2 end-of-life see the [v2 sunset page](/docs/reference/v2-sunset).

---

## Version 3.1.0 (GA pending)

**Status:** Release-candidate validation — minor release. The current published release candidate is `v3.1.0-rc.15`; its wire negotiation value is `"3.1-rc.15"`. The published 3.0.x line remains stable; 3.1 is feature-complete and validating through RC artifacts. SDKs pin to exact release values advertised in `supported_versions` during RC validation and use `"3.1"` at GA.
...
|---|---|
| A 3.0-conformant production agent | Nothing. Stable schemas remain wire-compatible with 3.0.0. During RC validation, pin exact advertised values such as `"3.1-rc.15"`; at GA, move your SDK pin to `"3.1"` when you are ready to consume the new fields. |
...
**Pin your release.** Every SDK exposes a constructor option for the version pin. Through 3.x, the SDK SHOULD also emit `adcp_major_version` automatically so legacy 3.x sellers that only read the integer keep negotiating correctly. During RC validation, use the exact advertised prerelease value such as `"3.1-rc.15"`; after GA, use `"3.1"`. The wire shape is **release-precision only** (`"3.1"`, `"3.1-rc.15"`) — full semver like `"3.1.2"` or `"3.1.0-rc.15"` is invalid on the wire.
...
const client = new SingleAgentClient(
  ...
  { adcpVersion: "3.1-rc.15" }, // use "3.1" after GA
...
client = ADCPClient(
    ...
    adcp_version="3.1-rc.15",  # use "3.1" after GA
                              # adcp_major_version: 3 mirror auto-emitted
)
```
