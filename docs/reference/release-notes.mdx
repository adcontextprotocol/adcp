...
AdCP 3.2 adds `list_products`, `request_proposals`, `refine_proposals`, and `finalize_proposals` as compact, task-specific contracts and retains `get_products` throughout 3.x.
...
- `refine_proposals` creates immutable proposal revisions; `finalize_proposals` commits one or more revisions atomically.
