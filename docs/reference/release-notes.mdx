...
- **Inventory identity:** first-class FAST and virtual linear channel collections, host-property carriage mapping, and collection-scoped host attestation for owner-sold avails.
...
| A compliance operator | Test beta.6 from its signed assets or with `@adcp/sdk@14.0.0-beta.8`; pair Python `adcp==8.0.0b7` with beta.5; do not infer beta.6 support from another SDK or issue a 3.2 badge from a beta checkpoint. |
