...
| A compliance operator | Test beta.8 directly from its signed assets; use `@adcp/sdk@14.0.0-beta.12` only with exact beta.6 and Python `adcp==8.0.0b7` only with exact beta.5; do not infer beta.8 support from an older SDK or issue a 3.2 badge from a beta checkpoint. |
