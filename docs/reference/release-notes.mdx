...
Product signal targeting also clarifies the broad-feed workflow. Products may point to `get_signals` with `signal_targeting_options_source: "get_signals"` instead of duplicating a wholesale signal feed inline. Buyers use `get_signals` for candidate discovery, then confirm product eligibility and composability with `get_products.filters.signal_targeting` or a product-specific `get_products` response before sending `packages[].targeting_overlay.signal_targeting_groups`.
