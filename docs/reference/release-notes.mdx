...
**Per-version verification badge issuance.** Public default verification runs can now issue 3.1 badges while 3.0 badges remain active. Badges are version-scoped; buyers pinned to a specific AdCP release should read versioned badge data instead of flattening to a single current badge. See [Compliance catalog](/docs/building/verification/compliance-catalog) and [Versioning](/docs/reference/versioning).
