...
13. **Media Buy Status Lifecycle** — `pending_activation` splits into `pending_creatives` (approved, no creatives assigned) and `pending_start` (ready to serve, waiting for flight date). `compliance_testing` protocol capability supports storyboard-driven agent testing. (#2034)
