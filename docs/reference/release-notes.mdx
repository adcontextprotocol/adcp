...
**Status:** In development — minor release targeting the 3.2.0 milestone. Additive over 3.1 for wire shapes; this section records the creative-format **discovery** deprecation posture (#5779).
...
**Compliance:** storyboard steps that call `list_creative_formats` are gated with `requires_tool` so agents that have migrated to canonical discovery grade those steps `not_applicable` rather than failing. Agents that still implement the v1 tool continue to conform.
