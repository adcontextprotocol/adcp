...
| `3.2.0-beta.6` | Expand reporting negotiation and breakdowns; add CTV experience profiles and premium display canonicals; align A2A request-signing method names | Exact TypeScript support in `@adcp/sdk@14.0.0-beta.12`; direct artifact, raw-wire, and code-generation testing; Python remains on beta.5 pending refresh |
...
| A compliance operator | Test beta.7 directly from its signed assets; use `@adcp/sdk@14.0.0-beta.12` only with exact beta.6 and Python `adcp==8.0.0b7` only with exact beta.5; do not infer beta.7 support from an older SDK or issue a 3.2 badge from a beta checkpoint. |
