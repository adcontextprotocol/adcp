...
**Breaking-change notice for an experimental surface. The six-week clock begins when this notice merges.**

The proposed campaign-governance update adds task-scoped service enforcement, opaque plan binding, intent-only conditions negotiation, authenticated caller binding, phase-correct media-buy identifiers, and governance-authoritative outcome accounting. The implementation and migration guide will remain draft until the notice window has elapsed. See [#5944](https://github.com/adcontextprotocol/adcp/issues/5944) for the governing use case.
...
The v1 task remains **functional through 4.x**. Removal is aligned with the v1 named-format / `format_ids` deprecation calendar in the [canonical formats migration guide](/docs/creative/canonical-formats-migration) (2027-Q4 floor / 2029-Q1 ceiling; adoption-driven 5.0 cut sequence) — not an immediate hard cut in 3.2.
...
- Legacy named-format creatives and v1 `format_id` / `format_ids[]` remain supported through 4.x.
...
**Compliance:** storyboard steps that call `list_creative_formats` are gated with `requires_tool`, producing an explicit non-failing `missing_tool` skip when an agent has migrated to canonical discovery. The creative lifecycle requires either a non-empty `creative.supported_formats` catalog or a passing v1 discovery call. Agents that still implement the v1 tool continue to conform.
