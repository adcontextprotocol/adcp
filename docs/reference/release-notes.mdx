...
**Schema Infrastructure & Type Safety** - Version 2.5.0 focuses on improving developer experience through better type generation, schema validation, and API consistency. This release adds discriminator fields throughout the protocol for superior TypeScript/Python type inference, standardizes response patterns, and introduces build-time schema versioning.

**Batch Preview Creative** - Generate previews for up to 50 creatives in a single API call, reducing preview generation time by 5-10x. Supports direct HTML embedding for iframe-less rendering.

**Application Context Passthrough** - Task requests now accept an optional `context` object that gets echoed back in responses and webhooks, enabling better request correlation and debugging.

**Visual Cards for Products & Formats** - Publishers and creative agents can now include optional visual card definitions that reference card formats for rich UI displays.
