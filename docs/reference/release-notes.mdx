...
1. **Trust Surface: Idempotency, Request Signing, and Signed Governance** — Three primitives that make agent-to-agent transactions safe for real money:
   - **`idempotency_key` required on every mutating request** — fresh UUID v4 per logical operation; retry safe under the declared `replay_ttl_seconds` (default 24h); `IDEMPOTENCY_CONFLICT` and `IDEMPOTENCY_EXPIRED` errors. (#2315)
   ...
   - **Signed JWS `governance_context`** — governance decisions are cryptographically verifiable offline. Sellers bind the decision to a specific buyer, plan, phase, and time using the governance agent's JWKS (from `sync_governance`) without round-tripping. (#2316)
   - **Universal security storyboard** — every agent is compliance-tested on auth (unauth rejection, API key, OAuth discovery per RFC 9728, audience binding). Signed-requests runner harness validates end-to-end. Cross-instance state persistence required for any production agent. (#2304, #2350, #2352, #2363, #2381)
