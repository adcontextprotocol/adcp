...
**Status:** Release-candidate preparation — minor release. The published 3.0.x line remains stable; 3.1 is feature-complete and pending release-candidate validation. SDKs pin to release precision (`"3.1-beta"` during beta, `"3.1-rc.N"` during RC, `"3.1"` at GA) and pick up minors at their own cadence.
...
**Creative-agent canonical build capabilities.** Creative agents declare what they can produce via `creative.supported_formats[]` on `get_adcp_capabilities`. Each entry carries an agent-local stable `capability_id`; buyers use that ID as `target_format_id.id` in `build_creative`. This is intentionally separate from media-buy `format_option_id`, which selects a product-local or publisher-catalog format option. The training agent and storyboards verify advertised capability IDs, canonical `format_kind` manifests, unsupported target rejection, and 3.0 compatibility gating.

**Canonical format satisfaction at create time.** Media-buy create/update/read surfaces now preserve package-level canonical selectors, including direct `format_kind`/`params` requests. The conformance suite tests that under-specified selectors fail closed instead of matching products by loose overlap.

**Per-version verification badge issuance.** Public default verification runs can now issue 3.1 badges while 3.0 badges remain active. Badges are version-scoped; buyers pinned to a specific AdCP release should read versioned badge data instead of flattening to a single current badge.
...
**Package correlation across mixed seller versions.** 3.1 sellers echo request `packages[].product_id` on every response package that represents that requested package. Buyers that must interoperate with older sellers should also put opaque line-item correlation in `packages[].context` (for example `context.buyer_ref`) and treat it as a legacy-safe fallback when `product_id` is missing on create/read/webhook responses.

**Upstream traffic attestation mode selection.** The upstream-traffic storyboards now lock the normative choice between raw and digest attestation modes so sellers cannot pass by emitting the wrong proof shape for the negotiated mode.
