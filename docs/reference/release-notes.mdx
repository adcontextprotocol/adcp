...
**Per-version verification badge issuance.** Public verification runs now issue only the selected public AdCP line; the default remains 3.0 while 3.1 prerelease runs are diagnostic. Badges are version-scoped, so 3.0 badges remain active independently of future 3.1 badges. Buyers pinned to a specific AdCP release should read versioned badge data instead of flattening to a single current badge. See [Compliance catalog](/docs/building/verification/compliance-catalog) and [Versioning](/docs/reference/versioning).

**Discovery precision additions.** Late schema additions are optional and additive: `video_placement_types` on products, placements, and `get_products` filters; `ext` on discovery filters for vendor- or seller-specific criteria; time/scoped authorized operators on `brand.json`; and machine-readable brand guideline constraints. See [`get_products`](/docs/media-buy/task-reference/get_products) and [`brand.json`](/docs/brand-protocol/brand-json).
