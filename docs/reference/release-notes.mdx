...
New integrations SHOULD use the compact tools. Existing integrations do not need an immediate migration: the three compatibility facades retain their 3.x request shapes. On `get_products`, a seller may ignore an optional key for a guaranteed side-effect-free synchronous read, but an arm that allocates a task or changes observable state must require a key and be fenced before that effect.
