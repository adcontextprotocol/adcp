...
Authoritative version-by-version release record for AdCP, with cumulative change detail, migration guidance, and per-version adoption notes. For the 3.2 adopter overview, see [What's New in AdCP 3.2](/docs/reference/whats-new-in-3-2); for the upgrade checklist, see [Migrating from 3.1 to 3.2](/docs/reference/migration/3-1-to-3-2); and for prerelease artifact and SDK timing, see the [3.2 prerelease program](/docs/reference/3-2-beta). For the at-a-glance status of every published version, see [Versions & Compatibility](/docs/reference/versions). For source-level changelogs, see [CHANGELOG.md](https://github.com/adcontextprotocol/adcp/blob/main/CHANGELOG.md). For version stability, schema-change scope, and the 3.x guarantees see [Versioning & Governance](/docs/reference/versioning). For v2 end-of-life see the [v2 sunset page](/docs/reference/v2-sunset).

---

## Version 3.2.0

**Status:** Release-candidate cycle — minor release targeting the 3.2.0 milestone. RC.2 is the current signed protocol checkpoint. RC.1 completes the experimental Reliable Reporting 1.0 contract and hardens delivery, reach, and vendor-metric semantics; RC.2 adds the opt-in consumer-status loop and the other changes listed below. TypeScript `@adcp/sdk@14.0.0-rc.35`, Python `adcp==8.0.0b14`, and Go `adcp/v3@v3.2.1` embed RC.1 exactly; use signed RC.2 assets directly for exact RC.2 testing. Python and Go higher-level coverage remains role-dependent. Stable wire shapes remain backward compatible except for the ratified `media_buy_status` cleanup and request-signature encoding migration described below; explicitly experimental surfaces may carry noticed changes under the [experimental-status contract](/docs/reference/experimental-status).
...
See the [migration guide](/docs/reference/migration/reliable-reporting-status-loop).
</Warning>

<Warning>
**Trusted Match breaking/security notice — published September 13, 2026.**
Version 3.1.22 already introduced the #7396 single-user context privacy
restriction and #7397 request-aware Context Match cache partitioning as
emergency security corrections. Affected Context Match routers must bypass or
disable caching until request partitioning and the trusted,
result-affecting security/configuration namespace are both implemented. The
additional namespace conformance is prospective for AdCP 3.2, effective no
earlier than October 25, 2026; no immutable 3.2 RC or GA carrying its mandatory
enforcement may publish before then. Published RC.1 and RC.2 artifacts remain
immutable; neither contains the prospective namespace enforcement. See the
[migration notice](/docs/reference/migration/trusted-match-context-cache).
</Warning>
...
| `3.2.0-rc.1` | Complete the experimental Reliable Reporting 1.0 contract and harden delivery currency, aggregate reach, vendor-metric identity, and integration behavior | Previous signed checkpoint with exact TypeScript, Python, and Go SDK pairings |
| `3.2.0-rc.2` | Publish the opt-in Reliable Reporting consumer-status loop and the additional changes recorded in the source changelog | Current signed protocol checkpoint; use its signed assets for exact RC.2 testing |
...
A seller may voluntarily cache keyed reads, but buyers cannot rely on that behavior. Such sellers follow the full durability, rate-limit, and encryption contract and isolate read inserts from mutation replay capacity. See [security.mdx § Idempotency](/docs/building/by-layer/L1/security#idempotency) for the current contract.
