...
3.0 hardens AdCP's **trust surface**. Every mutating request carries an `idempotency_key` and can be cryptographically signed with RFC 9421 HTTP Message Signatures; webhooks are signed under the same profile with required idempotency keys on payloads; governance decisions return as signed JWS `governance_context` artifacts sellers verify offline. Storyboards move into the protocol at `/compliance/{version}/` with a universal security baseline, protocol baselines, and specialism storyboards. On top of this trust foundation, 3.0 also brings broadcast TV as a first-class channel, generalizes governance to any purchase type, simplifies the capabilities model, and introduces **specialisms and storyboard-driven compliance** as the foundation for AdCP agent verification.
...
1. **Trust Surface: Idempotency, Request Signing, Signed Governance, and Signed Webhooks** — Five primitives that make agent-to-agent transactions safe for real money:
   - **`idempotency_key` required on every mutating request** — fresh UUID v4 per logical operation; retry safe under the declared `replay_ttl_seconds` (default 24h); `IDEMPOTENCY_CONFLICT` and `IDEMPOTENCY_EXPIRED` errors. Extended to `activate_signal`. (#2315, #2407)
   - **RFC 9421 HTTP Message Signatures — optional in 3.0, mandatory under AdCP Verified** — Ed25519 over a canonicalized covered-component list (including `content-digest`). Published test vectors and a 15-step verification checklist with `keyid` cap-before-crypto. sf-binary and URL canonicalization pinned so independent implementations agree bit-for-bit. (#2323, #2341, #2342, #2343)
   - **Webhooks unified on the RFC 9421 profile — baseline-required in 3.0** — Sellers sign outbound webhook requests with a key published at their `jwks_uri` (discoverable via `brand.json` `agents[]`). No shared secret crosses the wire. 14-step webhook verifier checklist with `webhook_signature_*` error codes. HMAC-SHA256 remains a legacy fallback for 3.x; removed in 4.0. (#2423)
   - **Webhook payloads carry a required `idempotency_key`** — Every webhook is dedupable by a sender-generated UUID v4 using the same format as request-side keys. Replaces fragile `(task_id, status, timestamp)` dedup tuples across five webhook payload schemas. (#2416, #2417)
   - **Signed JWS `governance_context`** — governance decisions are cryptographically verifiable offline. Sellers bind the decision to a specific buyer, plan, phase, and time using the governance agent's JWKS (from `sync_governance`) without round-tripping. `check_governance` required on every spend-commit. (#2316, #2403, #2419)
   - **Universal security storyboard** — every agent is compliance-tested on auth (unauth rejection, API key, OAuth discovery per RFC 9728, audience binding). Signed-requests and signed-webhooks runner harnesses validate end-to-end. Cross-instance state persistence required for any production agent. (#2304, #2350, #2352, #2363, #2381)

2. **Specialisms and Storyboard-Driven Compliance** — Storyboards move into the protocol at `/compliance/{version}/` (universal + protocols + specialisms + test-kits). New `specialisms` field on `get_adcp_capabilities` lets agents claim narrow capability specialisms across 6 protocols (media-buy, creative, signals, governance, brand, sponsored_intelligence). `sponsored_intelligence` is promoted from specialism to full protocol. `broadcast-platform` → `sales-broadcast-tv`, `social-platform` → `sales-social`. `property-governance` + `collection-governance` merge into `inventory-lists`, which then renames to `property-lists` with `collection-lists` split out as a sibling. Four 3.1 archetypes (`sales-streaming-tv`, `sales-exchange`, `sales-retail-media`, `measurement-verification`) are marked `status: preview`. Compliance taxonomy renames `domains` → `protocols`; `audience-sync` reclassified from `governance` to `media-buy`. Per-version protocol tarball published at `/protocol/{version}.tgz`. See the [Compliance Catalog](/docs/building/compliance-catalog). (#2176, #2300, #2332, #2336)
...
5. **IO Approval at the Task Layer** — `MediaBuy.pending_approval` is removed. IO approvals are modeled as explicit approval tasks with their own lifecycle and audit trail — decoupled from media-buy status. Sellers implementing `sales-guaranteed` handle human-in-the-loop approval without overloading media-buy status semantics. (#2270, #2351)

6. **Capabilities Model Simplification** — Redundant boolean gates are removed throughout `get_adcp_capabilities`. If a `content_standards` object exists in the response, the agent supports content standards — no separate boolean needed. `reporting_capabilities` is now required on every product. Geo capability fields keep their typed shapes (`geo_countries`, `geo_regions`, `geo_metros`, `geo_postal_areas`). (#2143, #2157)

7. **Collection Lists** — Program-level brand safety for shows, series, and podcasts. Collection lists parallel property lists but target content programs using distribution identifiers (IMDb, Gracenote, EIDR) for cross-publisher matching. New `collection_list` and `collection_list_exclude` fields on the targeting overlay. Genre taxonomy enum normalizes classification. 16 new collection schemas. (#2005)

8. **Structured Measurement Terms** — Guaranteed buys gain a formal negotiation surface. `measurement_terms` covers billing vendor, IVT threshold, and viewability floor. Sellers declare defaults; buyers propose overrides at `create_media_buy`; sellers accept, reject, or adjust. `cancellation_policy` schema declares notice periods and penalties. `TERMS_REJECTED` error code. (#1962)

9. **Unified Vendor Pricing** — Pricing extends from signals to creative, governance, and property list agents. All vendor pricing now uses a shared schema covering CPM, percent-of-media, and flat-fee models. `pricing_options[]` on `list_creatives`, `build_creative`, `get_creative_features`, and property lists. (#1937)

10. **Per-Request Version Declaration** — `adcp_major_version` on all 56+ request schemas lets buyers declare which major version their payloads conform to. Sellers return `VERSION_UNSUPPORTED` on mismatch. When omitted, sellers default to their highest supported version. (#1959)

11. **Offline Reporting Delivery** — Sellers can push reports to buyer-provided cloud storage. `reporting_delivery_methods` on capabilities declares supported protocols (SFTP, S3, GCS, Azure Blob). `reporting_bucket` on accounts specifies the destination. Products declare `supports_offline_delivery` in `reporting_capabilities`. File formats: CSV, JSON, Parquet, Avro, and ORC. (#2198, #2205)

12. **GOVERNANCE_DENIED Error and Schema Consistency** — New `GOVERNANCE_DENIED` correctable error code for governance-rejected operations. `context` and `ext` fields added to all request and response schemas across governance, collection, property, SI, and content-standards protocols. `signal_id` is now required on `get_signals` response items. `comply_test_controller` schema flattened from a oneOf union to a flat object with a `scenario` discriminant. (#2194)
...
14. **Brand Schema Extensions** — Generic `agents` array on `brand.json` for declaring brand-associated agents. Visual tokens: `border_radius`, `elevation`, `spacing`, and extended color roles. Structured font definitions with `weight`, `style`, `stretch`, and `optical_size`. `data_subject_contestation` satisfies Art 22(3) discovery. (#1973, #1871, #1856, #2310)

15. **Media Buy Status Lifecycle** — `pending_activation` splits into `pending_creatives` (approved, no creatives assigned) and `pending_start` (ready to serve, waiting for flight date). `MediaBuy.pending_approval` removed — approvals are now task-layer objects (see #5 above). Top-level `compliance_testing: { scenarios: [...] }` capability block (not a `supported_protocols` value) declares `comply_test_controller` support for storyboard-driven agent testing. (#2034, #2270, #2351)

16. **Operating an Agent, Release Cadence, CHARTER** — New "Operating an Agent" guide for publishers without engineering teams (partner / self-host / build). Named release cadence policy: patch monthly, minor quarterly, major annual if needed. v2 EOL August 1, 2026. Formal `CHARTER.md` linked from README, IPR, and intro. AI disclosure page on the main site. (#2202, #2309, #2312, #2321, #2329, #2362)
...
- **Webhooks** — Migrate from HMAC-SHA256 to RFC 9421 signing. Publish your webhook-signing key at `jwks_uri` on your `brand.json` `agents[]` entry with `adcp_use: "webhook-signing"`. Drop `push_notification_config.authentication` from new configs. Receivers verify against the sender's JWKS. Every outbound webhook payload must carry a `idempotency_key` (UUID v4). Listeners must dedupe on `idempotency_key` with a 24h minimum TTL. HMAC fallback remains available through 3.x for buyers that opt in.
- **Idempotency** — Generate a fresh UUID v4 `idempotency_key` on every mutating request. Same key + identical payload on retry → `replayed: true`. Same key + different payload → `IDEMPOTENCY_CONFLICT`. Key older than declared TTL (default 24h) → `IDEMPOTENCY_EXPIRED`. Your agent must persist keys across instances (see [cross-instance state persistence](#trust-surface-idempotency-request-signing-and-signed-governance)).
...
**Status:** Release Candidate | [AdCP 3.0 overview](/docs/reference/whats-new-in-v3) | [Annex III obligations](/docs/governance/annex-iii-obligations)

### What's New

1. **GDPR Art 22 / EU AI Act Annex III as schema invariants** — Regulated verticals (credit, insurance pricing, recruitment, housing) now enforce mandatory human review at the protocol layer rather than in deployer policy documents. Resolves #2310.

2. **Two-axis autonomy model** — `budget.authority_level` is removed; two orthogonal fields replace it:
   - `budget.reallocation_threshold` (number ≥ 0) governs budget reallocation autonomy. Denominated in `budget.currency`. Mutually exclusive with `reallocation_unlimited: true` (explicit full-autonomy sentinel).
   - `plan.human_review_required` (boolean) governs per-decision human review under Art 22 / Annex III. Every plan action escalates regardless of spend when true.

3. **Automatic Art 22 cascade** — Governance agents MUST set `human_review_required: true` when any of: `policy_categories` contains `fair_housing | fair_lending | fair_employment | pharmaceutical_advertising`; any resolved policy has `requires_human_review: true`; or `policy_ids` includes `eu_ai_act_annex_iii`. Enforced at the JSON Schema layer via cross-field `if/then`. Buyers cannot opt out by omitting the flag.

4. **Append-only revisions with human override** — `GovernancePlanState.revisionHistory` retains prior plan configurations. Downgrading `human_review_required` from true requires a `human_override` artifact with ≥20-char reason, email-format approver, and optional 24h-fresh `approved_at`. Mode `advisory | audit` MUST NOT neuter `human_review_required`.

5. **`eu_ai_act_annex_iii` registry policy** — Seeded as a regulation (enforcement: must, EU jurisdictions, `requires_human_review: true`) with §1(b) recruitment, §5(b) credit, §5(c) insurance references. Guidance documents AdCP's Art 25 data-governance-provider role.

6. **`requires_human_review` on policies and categories** — New boolean on `policy-entry.json` and `policy-category-definition.json`. Effective immediately on resolution — NOT gated by `effective_date`. Art 22 GDPR predates AI Act effective dates. Seeded true on four categories (`fair_housing`, `fair_lending`, `fair_employment`, `pharmaceutical_advertising`).

7. **`data_subject_contestation` on brand and house** — New optional contact reference (URL / email / languages) on `brand.json` at both brand and house levels (resolution order: brand → house → missing). Also available as inline override on `brand-ref.json` for operators who do not control the brand's canonical brand.json. Satisfies Art 22(3) discovery — AdCP provides the pointer; the deployer operates the workflow.

8. **Expanded restricted attributes** — `age` and `familial_status` added to the `restricted-attribute` enum. `fair_housing` and `fair_employment` categories updated to include them (closes the HUD v. Facebook gap).

9. **Prompt-injection bounds** — `objectives` bounded at 2000 chars; `policy-entry.policy` at 5000; `policy-entry.description` at 500. Governance agents MUST pin registry-sourced policy text as system-level instructions; `custom_policies` evaluated as additional restrictions only — cannot relax or override registry.

10. **Annex III obligations doc** — New `/docs/governance/annex-iii-obligations` page covers deployer obligations (Art 14 oversight, Art 12 logging, Art 13 transparency, Art 10 data governance, Art 22(3) contestation), AdCP's Art 25 role, the jurisdictional-scoping limitations of `plan.countries`, and the `reallocation_threshold` vs. `human_review_required` distinction.

### Breaking Changes

| Change | rc.3 | rc.4 |
|--------|------|------|
| Budget autonomy | `authority_level: agent_full \| agent_limited \| human_required` enum on `budget` | Removed. Use `reallocation_threshold` (number) or `reallocation_unlimited: true` (sentinel) |
| Spend-authority enum file | `budget-authority-level.json` | Deleted |
| Human review | Implicit via enum | Explicit `plan.human_review_required` (boolean) |
| Annex III verticals | Policy-document concern | Schema invariant — `if/then` rejects `human_review_required: false` when `policy_categories` ∋ regulated set |
| Plan revisions | Overwritten | Append-only `revisionHistory` |
| Mode override | `advisory \| audit` downgrades all findings | Cannot downgrade when `human_review_required: true` |

### Migration Notes For rc.3 Adopters

**Budget rewrite (required):**
- `authority_level: 'agent_full'` → `reallocation_unlimited: true`
- `authority_level: 'agent_limited'` → `reallocation_threshold: <positive>` (typically your per-change escalation threshold)
- `authority_level: 'human_required'` → `human_review_required: true` at plan level (and optionally `reallocation_threshold: 0`)

**Regulated-vertical campaigns (required):**
- Declare `policy_categories` explicitly for credit / insurance / recruitment / housing campaigns
- Set `human_review_required: true` when those categories are declared (or omit the field and let your governance agent set it — governance agents MUST do so automatically)
- Populate `brand.data_subject_contestation` on `brand.json` with a URL or email contact point — or use the inline `brand-ref` override when you don't control brand.json

**Governance-agent implementors (required):**
- Auto-flip `human_review_required` based on resolved policy categories, policy IDs, and custom policies
- Emit a critical `data_subject_contestation` finding in `check_governance` when `human_review_required` is true and no contestation contact is discoverable
- Enforce `human_override` validation on downgrades (≥20-char reason, email approver, 24h freshness)
- Preserve revision history — do not silently replace plan state on re-sync
- Mode `advisory | audit` MUST NOT downgrade denied/escalated when `human_review_required` is true

**Signal declarations (recommended):**
- Signals that infer `age` or `familial_status` MUST declare those in `restricted_attributes` — or they will be filtered out of fair_housing / fair_employment / fair_lending campaigns automatically
