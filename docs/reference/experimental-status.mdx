...
|---|---|---|
| `brand.rights_lifecycle` | [`get_rights`](/docs/brand-protocol/tasks/get_rights), [`acquire_rights`](/docs/brand-protocol/tasks/acquire_rights), [`update_rights`](/docs/brand-protocol/tasks/update_rights); plus the `brand.rights`, `brand.right_types`, `brand.available_uses`, and `brand.generation_providers` capability fields on `get_adcp_capabilities` | Legal-construct surface added late in the 3.0 cycle. First enterprise deployments will expose edge cases in partial rights, sublicensing, revocation, and dispute resolution. |
