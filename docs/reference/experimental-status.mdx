...
|---|---|---|
| `protocol.agent_configuration` | [`sync_agent_configuration`](/docs/protocol/sync_agent_configuration), `adcp.agent_configuration` capability discovery, and the caller-scoped reusable reporting destination schemas | Introduces a durable authenticated-principal relationship layer between global seller capabilities and advertiser accounts. Stable-principal mapping, key rotation, optimistic concurrency, destination proof, and multi-account destination reuse need cross-party implementation experience before freezing. |
