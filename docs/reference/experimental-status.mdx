...
Experimental status is the escape valve that keeps the [3.x stability guarantees](/docs/reference/versioning#3x-stability-guarantees) credible. A surface is either stable — in which case it cannot break inside 3.x — or explicitly experimental, in which case it can. There is no undeclared middle ground. For a list of the protocol's non-goals and deferred items, see [Known Limitations](/docs/reference/known-limitations).
...
| `measurement.core` | The `measurement` capability block and seller `media_buy.performance_feedback` block on [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities), compact performance-feedback assertion and receipt fields, and the `measurement` value in `supported_protocols` | Measurement currently exposes vendor metric catalog discovery and task-based compact feedback production. Attribution, pricing/coverage, seller application disposition, and baseline compliance storyboards are not frozen, so the surface remains experimental in 3.1. |
| `measurement.gateway` | The `measurement_gateway` capability block on [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities#measurement_gateway) and its task-based provider interchange contract | The buyer-orchestrator gateway initially reuses `get_media_buy_delivery` and `provide_performance_feedback`. Authorization, identifier mapping, and cross-seller disclosure need implementation experience before the role can freeze; webhook and offline paths are explicitly deferred. |
| `trusted_match.core` | [TMP](/docs/trusted-match/) | Privacy architecture is thinly specified relative to what a regulator deep-dive will demand. Version 3.1.22 shipped emergency single-user privacy and request-aware cache-partitioning corrections. The [active breaking/security migration notice](/docs/reference/migration/trusted-match-context-cache) makes the additional trusted cache namespace prospective AdCP 3.2 conformance effective October 25, 2026 and requires affected routers to bypass caching until both partitions are implemented. Exposure tokens, country-partitioned identity, and Offer creative data are expected to change. |
| `trusted_match.verified_identity` | The `attestation` object on `identity-match-request` entries, the top-level `sealed_credentials[]` array, the `world_id_nullifier` value in the `uid-type` enum, the `attestation-claim` enum, and `brand.json` `identity_relying_parties[]` — see [Verified Identity Attestation](/docs/trusted-match/specification#verified-identity-attestation) | Verifiable proof-of-personhood / age forwarded into Identity Match so buyers verify rather than trust an assertion. Issuer-agnostic, but only the World ID scheme is specified so far; the `signal_binding` freshness policy, the issuer/scheme registry-vs-free-form decision, and the `brand.json` variant placement are WG-open. The widening of the otherwise-`additionalProperties:false` identity-match-request schema is contract-bearing and may be revised under experimental. |
| `sponsored_intelligence.core` | [`si_get_offering`](/docs/sponsored-intelligence/tasks/si_get_offering), [`si_initiate_session`](/docs/sponsored-intelligence/tasks/si_initiate_session), [`si_send_message`](/docs/sponsored-intelligence/tasks/si_send_message), [`si_terminate_session`](/docs/sponsored-intelligence/tasks/si_terminate_session); the `sponsored_intelligence` capability field on `get_adcp_capabilities`; the SI identity, capability negotiation, and UI component surfaces defined in the [SI specification](/docs/sponsored-intelligence/specification) | Conversational brand experiences are a new advertising model. Session lifecycle, UI components, identity/consent object shape, and capability negotiation are expected to evolve as first-party AI hosts and brand agents integrate. Planned changes track the [3.1.0 roadmap](https://github.com/adcontextprotocol/adcp/issues/2201). |
