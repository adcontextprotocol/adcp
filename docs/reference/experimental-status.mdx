...
| `media_buy.reporting_delivery` | [`get_reporting_status`](/docs/media-buy/task-reference/get_reporting_status), `sync_accounts.reporting_delivery_configs[]`, `reporting.delivery_ready`, and the `media_buy.reporting_delivery` capability block | New managed-reporting reliability surface (#6911). Live synthetic BigQuery and Databricks tests validate warehouse and dataset-share mechanics, but multi-party production SDK consumption, obligation derivation, provider-specific compatibility, and status-SLA behavior need implementation experience before freezing. |
