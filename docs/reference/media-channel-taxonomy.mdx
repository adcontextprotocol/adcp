...
<Note>
Some names like `dooh`, `podcast`, `radio`, and `streaming_audio` appear as both a channel and a property type. The channel describes the **buying context** (how budget is allocated); the property type describes the **technical surface** (where the ad renders). For example, an in-store retail display or audio placement can be `retail_media` channel / `dooh` property type, while a standalone digital billboard or place-based audio network is `dooh` channel / `dooh` property type.
</Note>
...
| `dooh` | Digital inventory delivered through devices installed in physical out-of-home venues |
...
Digital advertising delivered through devices installed in physical out-of-home venues. The canonical creative format determines whether a placement is visual, audio-only, or multimodal.
...
- Audio players installed in taverns, stores, gyms, and other shared venues

**Excludes**:
- Static billboards (use `ooh`)
- Cinema screens (use `cinema`)
- Audio streams delivered to a listener's own device (use `streaming_audio`)
...
| `dooh` | Digital out-of-home venue networks and installed digital surfaces | `dooh`, `retail_media` |
