...
**Action Source**
Where an event physically occurred: `website`, `app`, `in_store`, `phone_call`, `crm`, `server`, or `other`. Set on each event in `log_event` and declared at the product level in `conversion_tracking.action_sources`.
...
**Optimization Goal**
A package-level setting that tells the seller what to optimize delivery for. For conversion goals (purchases, leads, ROAS), the buyer specifies an event source and event type, with strategies `maximize_conversions` (default), `target_cpa`, or `target_roas`. Delivery goals (clicks, video completions) are implied by the pricing model — CPC optimizes for clicks, CPCV optimizes for completed views — and do not require an event source.
...
**UID (Universal ID)**
A privacy-safe user identifier used for cross-platform attribution matching. AdCP supports standard UID types: `rampid` (LiveRamp RampID), `id5` (ID5), `uid2` (Unified ID 2.0), `euid` (European Unified ID), and `pairid` (Publisher Advertiser Identity Reconciliation). Passed in `user_match.uids` on events logged via `log_event`.
...
**User Match**
The set of identifiers on an event used to attribute it to ad delivery. Includes universal IDs (`uids`), `click_id` (platform-specific click identifiers like fbclid, gclid), and optional `client_ip` + `client_user_agent` for probabilistic matching. No PII (email, phone) is supported.
