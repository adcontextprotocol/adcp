...
**Optimization Goal**
A single entry in a package's `optimization_goals` array. Two kinds: `kind: "event"` (optimize for an advertiser-tracked conversion event — purchase, lead, install, etc. — with optional `target_cpa` or `target_roas`) and `kind: "metric"` (optimize for a seller-native delivery metric — clicks, views, completed views — with optional `target_cpc`, `target_cpv`, or `target_cpcv`). Metric goals do not require an event source. When multiple goals are present, `priority` (1 = highest) controls which the seller treats as primary.
