...
**Audience Selector**
A discriminated union describing one targeting criterion — either a signal reference (pointing to a specific data provider signal) or a natural language description. Audience selectors appear in plan audience constraints, `governance_context.audience_targeting`, and `planned_delivery.audience_targeting`.
