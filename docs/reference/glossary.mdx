...
**Signal ID**
A structured identifier referencing a signal in a data provider's catalog. Contains `data_provider_domain` (where the catalog is hosted) and `id` (signal identifier within that catalog). Example: `{ "data_provider_domain": "polk.com", "id": "likely_tesla_buyers" }`.
