...
**Audience Selector**
A discriminated union describing one targeting criterion — either a signal reference (pointing to a specific `signal_id`) or a natural language description. Audience selectors appear in plan audience constraints and `planned_delivery.audience_targeting`.
