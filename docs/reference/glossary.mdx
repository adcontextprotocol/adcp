...
**Action Source**
Where a conversion event physically occurred: `website`, `app`, `in_store`, `phone_call`, `crm`, `server`, or `other`. Set on each conversion event in `log_event` and declared at the product level in `conversion_tracking.action_sources`.
...
**Conversion Event**
A marketing event (purchase, lead, registration, etc.) logged via `log_event` for attribution and optimization. Each event has an `event_id` (for deduplication), `event_type`, `event_time`, and optional `user_match` and `custom_data`.

**Conversion Tracking**
The system for logging conversion events and attributing them to ad delivery. Involves three steps: configure event sources (`sync_event_sources`), set optimization goals on packages (`optimization_goal`), and send events (`log_event`). Seller-level capabilities are declared in `get_adcp_capabilities`; product-level support is declared in `product.conversion_tracking`.
...
**Data Provider**
An organization that owns and publishes audience or contextual data (e.g., Polk, Experian, Acxiom). Data providers publish signal catalogs via `/.well-known/adagents.json` on their domain, enabling discovery and authorization verification. See [Data Provider Guide](/docs/signals/data-providers).
...
**Event Type**
A standardized classification for conversion or marketing events (e.g. `purchase`, `lead`, `add_to_cart`, `page_view`). Used to categorize events logged via `log_event` and to specify optimization goals on packages.
...
**Optimization Goal**
A package-level setting that tells the seller which event source and event type to optimize delivery against. Supports `maximize_conversions` (default), `target_cpa`, or `target_roas` strategies. Requires an event source configured via `sync_event_sources`.
...
**Signal Catalog**
A collection of signal definitions published by a data provider via `/.well-known/adagents.json`. The catalog includes signal metadata (id, name, value_type, tags) and authorization declarations for signals agents. See [Data Provider Guide](/docs/signals/data-providers).
...
**Signal Source**
The origin type for a signal identifier: `catalog` (from a data provider's published catalog, authorization verifiable via adagents.json) or `agent` (native to the signals agent, not externally verifiable). See [Data Provider Guide](/docs/signals/data-providers).
...
**UID (Universal ID)**
A privacy-safe user identifier used for cross-platform attribution matching. AdCP supports standard UID types: `rampid` (LiveRamp RampID), `id5` (ID5), `uid2` (Unified ID 2.0), `euid` (European Unified ID), and `pairid` (Publisher Advertiser Identity Reconciliation). Passed in `user_match.uids` on conversion events.
...
**User Match**
The set of identifiers on a conversion event used to attribute it to ad delivery. Includes universal IDs (`uids`), `click_id` (platform-specific click identifiers like fbclid, gclid), and optional `client_ip` + `client_user_agent` for probabilistic matching. No PII (email, phone) is supported.
