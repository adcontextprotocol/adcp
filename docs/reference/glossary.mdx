...
**UID (Universal ID)**
A user identifier used for cross-platform attribution matching. AdCP supports standard UID types: `rampid` (LiveRamp RampID), `id5` (ID5), `uid2` (Unified ID 2.0), `euid` (European Unified ID), `pairid` (Publisher Advertiser Identity Reconciliation), and `maid` (Mobile Advertising ID — IDFA/GAID). Passed in `user_match.uids` on events logged via `log_event`.
