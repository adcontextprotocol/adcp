...
**Optimization Goal**
A single entry in a package's `optimization_goals` array. Two kinds: `kind: "event"` (optimize for an advertiser-tracked conversion event — purchase, lead, install, etc. — with optional `cost_per` or `per_ad_spend` target) and `kind: "metric"` (optimize for a seller-native delivery metric — clicks, views, completed views — with optional `cost_per` or `rate` target). Event goals with `per_ad_spend` targets require a `value_field` identifying which event data field carries the monetary value. Metric goals do not require an event source. When multiple goals are present, `priority` (1 = highest) controls which the seller treats as primary.
