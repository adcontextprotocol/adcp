...
**Hashed Identifier**
A pseudonymous identifier derived from buyer-normalized personally identifiable information (PII). Used in audience uploads (`sync_audiences`) and event attribution (`log_event`). Normalization: emails to lowercase+trim, phone numbers to E.164 format. The hashing scheme is negotiated via the seller's `privacy.audience_hash_schemes` capability: `hmac_sha256` (HMAC-SHA-256 with a per-seller onboarding key) in hardened integrations and `sha256_plain` (plain SHA-256) for legacy 3.x compatibility. The seller matches by applying the same scheme and keying material to its own user data — neither party shares raw PII on the wire. A hashed identifier is NOT anonymous: the email universe is small enough (~5B addresses) that plain SHA-256 is trivially inverted by rainbow table, and any party holding the HMAC key can still recover membership for a known cleartext. Treat hashed identifiers as pseudonyms — useful for matching, not for anonymity — and see [Privacy Considerations §PII Hashing](/docs/reference/privacy-considerations#pii-hashing) for the full posture. Distinct from [UIDs](#uid-universal-id), which are already-resolved pseudonymous tokens from identity graphs.
