...
**`idempotency_key` is required on every mutating request.** Buyers generate a fresh UUID v4 per logical operation and reuse the same key only to retry a failed request with an identical payload. Sellers respond `replayed: true` on an exact replay, `IDEMPOTENCY_CONFLICT` when the same key accompanies a different payload, and `IDEMPOTENCY_EXPIRED` after the declared TTL (default 24h, declared via `adcp.idempotency.replay_ttl_seconds`). This is what makes automated retries safe at scale: a network hiccup never creates a duplicate buy.
...
**Webhooks are signed under the same RFC 9421 profile — baseline-required for sellers.** Webhook authentication unifies on the AdCP 9421 profile as a symmetric variant of request signing: the seller signs outbound webhook requests with a key published at its `jwks_uri` (discoverable via `brand.json` `agents[]` with `adcp_use: "webhook-signing"`). No shared secret crosses the wire. The buyer verifies the signature using the seller's JWKS. A 14-step webhook verifier checklist with `webhook_signature_*` error codes covers trust-anchor scoping, downgrade-and-injection resistance, and per-keyid replay dedup (100K per keyid, 10M aggregate). HMAC-SHA256 remains a legacy fallback through 3.x; removed in 4.0.
...
See the [Security implementation guide](/docs/building/implementation/security) for the full threat model, principal roles (brand / operator / agent), and step-by-step verification paths.

<Card title="Security implementation" icon="arrow-right" href="/docs/building/implementation/security">
...
- **`specialisms`** — 21 narrow capability claims across the 6 domains. Examples: `sales-guaranteed`, `sales-broadcast-tv`, `creative-generative`, `inventory-lists`, `signal-marketplace`, `brand-rights`. Each rolls up to one parent protocol.

Specialism `status: preview` flags four 3.1 archetypes (`sales-streaming-tv`, `sales-exchange`, `sales-retail-media`, `measurement-verification`) whose storyboards are scheduled but not yet authoritative — the runner warns instead of verifying.
...
<AccordionGroup>
  ...
  <Accordion title="Sponsored Intelligence agents (new in v3)">
    Implement conversational brand experiences. See [SI Chat Protocol](/docs/sponsored-intelligence/si-chat-protocol).
