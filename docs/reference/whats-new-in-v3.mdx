...
Format `requirements` for video assets now use schema-defined field names that match `video-asset-requirements.json`. Creative agents can mechanically validate assets against format requirements without interpreting freeform fields.

New fields on `video-asset-requirements.json`: `frame_rate_type`, `scan_type`, `gop_type`, `min_gop_interval_seconds`, `max_gop_interval_seconds`, `moov_atom_position`, `audio_codecs`, `audio_sample_rates`, `audio_channels`, `loudness_lufs`, `loudness_tolerance_db`, `true_peak_dbfs`.

### Third-party tracker transparency

Whether a format supports third-party measurement is determined by whether its assets include a tracker slot (`tracker_pixel` or `tracker_script`). Buyer agents should check format assets before proposing performance standards that require creative-level verification. Formats without tracker slots (broadcast, some DOOH) rely on measurement vendors declared in `billing_measurement`.

<Card title="Format definitions" icon="arrow-right" href="/docs/creative/formats#third-party-tracker-support">
  How to determine tracker support from format asset definitions.
</Card>

### Experimental surfaces

AdCP 3.0 ships a core of stable surfaces under the [3.x stability guarantees](/docs/reference/versioning#3x-stability-guarantees), plus three surfaces that are part of the core protocol but not yet frozen. Experimental surfaces carry `x-status: experimental` in their schemas, and sellers implementing them declare the feature id in `experimental_features` on `get_adcp_capabilities`. They may change between 3.x releases with at least 6 weeks' notice.
