...
checkpoint, and no SDK embeds it yet: TypeScript `@adcp/sdk@14.0.0-rc.36`
embeds RC.2, while Python `adcp==8.0.0b14` and Go `adcp/v3@v3.2.1` still embed
RC.1; Python and Go higher-level coverage remains role-dependent. Test RC.3
from its signed bundle, and use these exact packages for staging tests against
the checkpoints they embed. See the
...
the next eligible minor after that date. No SDK embeds RC.3 yet;
`@adcp/sdk@14.0.0-rc.36` adds `@adcp/sdk/reporting/source` and
`@adcp/sdk/reporting/ledger` against the RC.2 bundle. See
...
See [Targeting-aware product discovery](/docs/reference/migration/targeting-aware-discovery).
...
| A compliance operator | Test RC.3 from its signed assets; no SDK pairing covers RC.3 yet. `@adcp/sdk@14.0.0-rc.36` tests RC.2, and Python `adcp==8.0.0b14` or Go `adcp/v3@v3.2.1` test RC.1. Retain exact package and wire versions end to end, and do not issue a 3.2 badge from a prerelease. |

## Start here

1. Read the [3.2 prerelease program](/docs/reference/3-2-beta).
