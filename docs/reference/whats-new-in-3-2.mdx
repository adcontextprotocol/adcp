...
`@adcp/sdk@14.0.0-beta.29` embeds beta.11; Python `adcp==8.0.0b11` carries
beta.10. Python higher-level helpers remain partial, and Go exact support is
still pending. See the [3.2 beta program](/docs/reference/3-2-beta) before testing.
</Warning>
...
`overlay_support.daypart_targets`. See [Targeting-aware product discovery](/docs/reference/migration/targeting-aware-discovery).
...
| An SDK maintainer | Generate from the signed beta.10 tarball, name the exact supported bundle, and feed accepted protocol corrections into the RC. |
| A compliance operator | Test beta.10 from its signed assets or an SDK that explicitly lists beta.10, such as TypeScript SDK 14 beta.28 or Python SDK 8 beta.11; retain end-to-end integration evidence and do not issue a 3.2 badge from a beta. |
