...
| Area | Item |
|---|---|
| Security | [Mandatory request signing for mutating calls (RFC 9421)](https://github.com/adcontextprotocol/adcp/issues/2307) — the 3.0 optional profile in `security.mdx` becomes required for spend-committing operations |

v4.0 is the earliest version that may require behavior currently optional in 3.x. Breaking changes are gathered here so the ecosystem can plan a single migration window rather than chasing per-minor deprecations.
