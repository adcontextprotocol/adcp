...
TypeScript SDK `14.0.0-rc.35`, Python SDK `8.0.0b14`, and Go SDK
`adcp/v3@v3.2.1` all still embed `3.2.0-rc.1`:

<Info>
**RC.3 has no matched SDK yet.** Use its signed bundle for raw-wire validation
and code generation. The exact package pairings below are all still on RC.1,
so an SDK-backed run is RC.1 evidence — not RC.3 evidence. Do not infer
compatibility from a floating package tag or from an SDK generated against an
earlier checkpoint.
</Info>
...
| `3.2.0-rc.1` | Previous protocol checkpoint for Reliable Reporting 1.0 and reporting-semantics hardening | SDK-backed integration testing with an exact RC.1 package pairing: `@adcp/sdk@14.0.0-rc.35`, Python `adcp==8.0.0b14`, or Go `adcp/v3@v3.2.1` |
...
| Current SDK wave | `@adcp/sdk@14.0.0-rc.35`, Python `adcp==8.0.0b14`, and Go `adcp/v3@v3.2.1` all embed `3.2.0-rc.1` | Exact RC.1 SDK-backed testing; higher-level Python and Go coverage remains role-dependent. No row names RC.2 or RC.3 |
...
The TypeScript SDK has exact RC.1 support. The npm `latest` tag remains on SDK
...
lifecycle, the RC.1 schemas and compliance assets, version-aware request
...
The current `rc` npm tag of `@adcp/sdk` is ahead of this checkpoint: it embeds `3.2.0-rc.2` and adds the seller-side reporting modules (`@adcp/sdk/reporting/source` and `@adcp/sdk/reporting/ledger`). The repository checkpoint alignment follows once the Python and Go SDKs publish matching bundles.
...
All three rows name exact RC.1 support with different higher-layer coverage.
None implies support for another checkpoint, and none names RC.2 or RC.3. Treat generated 3.2 types and
...
TypeScript SDK RC.35, Python SDK beta.14, and Go SDK v3.2.1 all embed RC.1.
