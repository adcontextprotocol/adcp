...
current protocol checkpoint. The current TypeScript SDK beta consumes the exact
signed beta.6 bundle; the Python SDK beta remains on beta.5:
...
| `3.2.0-beta.6` | Current TypeScript SDK-backed protocol checkpoint for reporting, CTV, canonical display, and request-signing integration | Buyer and seller teams running end-to-end TypeScript SDK integrations, raw-wire adopters, and SDK maintainers |
...
SDK-backed TypeScript testing, pair SDK beta.11 with protocol beta.6. Python
testing remains paired with beta.5 until a Python SDK release explicitly names
beta.6. You can also test beta.6 from its signed artifact with raw-wire or
...
Protocol beta.1 through beta.6 are published. TypeScript SDK beta.11 embeds the
exact beta.6 bundle, while Python SDK beta.7 embeds the exact beta.5 bundle.
Before treating a test result as beta.6 integration evidence, confirm that:
