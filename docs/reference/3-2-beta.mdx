...
current protocol checkpoint. TypeScript SDK `14.0.0-beta.29` consumes the
signed beta.11 bundle; Python SDK `8.0.0b11` carries beta.10:
...
| SDK wave | `@adcp/sdk@14.0.0-beta.29` embeds `3.2.0-beta.11`; Python `adcp==8.0.0b11` carries beta.10; Go remains pending | TypeScript adopters, Python adopters pinned to beta.10, Go maintainers, and early integration teams |

Earlier betas remain useful as pinned implementation checkpoints. For
SDK-backed TypeScript testing, pair SDK beta.28 with protocol beta.10. Pair
Python beta.11 with protocol beta.10. You can also test beta.10 from its signed
artifact as a historical checkpoint; use beta.11 for current raw-wire or
code-generation workflows. Do not combine a newer wire pin with an
...
Python `adcp==8.0.0b11` carries beta.10 schemas and generated types. Its
...
The TypeScript row names exact beta.11 support; the Python row names beta.10
support with different higher-layer coverage. Neither implies support
...
Protocol beta.1 through beta.11 are published. TypeScript SDK beta.29 embeds
the exact beta.11 bundle; Python SDK beta.11 carries beta.10. Before treating
