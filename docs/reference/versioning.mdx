...
now the current signed protocol checkpoint, and no SDK embeds it yet:
TypeScript SDK RC.36 embeds `3.2.0-rc.2`, while Python SDK beta.14 and Go SDK
`adcp/v3@v3.2.1` still embed `3.2.0-rc.1`. See the
...
| **`ext.{namespace}`** on any request or response object | Anyone, by declaring a namespace in `extensions_supported` | Namespaced, out-of-band, does not affect baseline interop. Namespaces SHOULD be registered in the [extension registry](/docs/building/by-layer/L2/context-sessions#extensions). |
...
- **[Versions & Compatibility](/docs/reference/versions)** — At-a-glance status of every published version
- **[Specification lifecycle](/docs/reference/specification-lifecycle)** — Stage definitions (Draft → Proposed → Final → Deprecated → Sunset), entry criteria, and how to check the current stage of a spec section
