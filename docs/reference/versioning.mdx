...
Buyers and sellers negotiate at **release precision** (VERSION.RELEASE, e.g. `"3.0"`, `"3.1"`) on every request and response. Released-precision pinning lets buyer SDKs select which schemas they validate against, and lets sellers return responses shaped to the contract the buyer expects (Stripe model).
...
3. **Server resolves**:
   ...
   - Same major, server's max release < buyer's pin → downshift to the highest supported release ≤ the pin.
   - Same major, no server release ≤ buyer's pin (gap or sub-min) → return `VERSION_UNSUPPORTED` with `supported_versions` in `error.data`.
...
**Dual-emit during 3.x:** a buyer that emits `adcp_version` MUST also emit `adcp_major_version` (with the major component of its pin) on the same request. This lets legacy 3.x sellers that only read the integer keep negotiating correctly. When both fields are present and disagree at the major level, the server MUST treat the request as malformed and return `VERSION_UNSUPPORTED`.
...
| Phase | Buyer obligation | Seller obligation |
|---|---|---|
| **3.1 (additive ship)** | SHOULD emit `adcp_version` (with `adcp_major_version` mirror through 3.x). | SHOULD honor and echo `adcp_version` on responses. MUST emit `supported_versions` on capabilities. |
| **3.2** | MUST emit `adcp_version` (with `adcp_major_version` mirror through 3.x). | MUST honor and echo `adcp_version`. MUST emit `supported_versions` on capabilities. Compliance grader fails sellers that don't echo. |
| **4.0** | MUST emit `adcp_version`. `adcp_major_version` removed. | MUST honor and echo `adcp_version`. `adcp.major_versions` and `extensions.adcp.adcp_version` removed. |

In-house clients that haven't moved to the SDK yet can keep sending `adcp_major_version` only through all of 3.x — the server falls back to it. Adding `adcp_version` is opt-in at 3.1 and required at 3.2.
...
| **`ext.{namespace}`** on any request or response object | Anyone, by declaring a namespace in `extensions_supported` | Namespaced, out-of-band, does not affect baseline interop. Namespaces SHOULD be registered in the [extension registry](/docs/building/integration/context-sessions#extensions). |
...
2. **Experimental addition.** A surface the working group wants to ship for feedback but is not ready to freeze enters the protocol as experimental — see [experimental status](/docs/reference/experimental-status). Experimental surfaces graduate to stable or are removed; there is no permanent experimental state.
