...
1. **Seller advertises**: `adcp.supported_versions` on the `get_adcp_capabilities` response lists all release-precision versions the seller speaks (e.g. `["3.0", "3.1"]`). `adcp.build_version` (e.g. `"3.1.2"`) is optional advisory metadata for incident triage — not part of the wire contract.
2. **Buyer declares**: `adcp_version` (release-precision string) on every request tells the seller which release the buyer's payloads conform to. The field is optional on every AdCP request.
3. **Server resolves**:
   ...
   - Same major, server's min release > buyer's pin → serve the buyer's pin (server downshifts to the buyer's contract).
   - Different major → return `VERSION_UNSUPPORTED` with `supported_versions` in `error.data`.
   - Omitted → server uses its default release.
4. **Server echoes**: `adcp_version` on every response tells the buyer which release the seller actually served. Buyers SHOULD validate the response against that release's schema, not against their pin.
