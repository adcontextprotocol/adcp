...
|---|---|---|---|
| **Beta** | 3.2 (current prerelease: `beta.10`) | `"3.2-beta.10"` | Development and staging only. Beta.10 is the current protocol checkpoint; TypeScript SDK beta.28 and Python beta.11 embed it exactly. Follow the [3.2 beta program](/docs/reference/3-2-beta). |
...
| Version | Released | Notes |
|---|---|---|
| **3.2.0-beta.10** | 2026-09-01 | Current protocol checkpoint. Adds managed reporting delivery, authenticated principals, audio VAST and DOOH support, programmed channel collections, and committed collection selection. TypeScript SDK beta.28 and Python SDK beta.11 embed it exactly; use `"3.2-beta.10"` only with peers that explicitly advertise it. |
...
| Implementing or testing 3.2 without an SDK | Use the signed **3.2 beta.10** artifacts with raw-wire or code-generation workflows. Follow the [beta program](/docs/reference/3-2-beta). |
| Running an SDK-backed 3.2 integration test | Use `@adcp/sdk@14.0.0-beta.28` or Python `adcp==8.0.0b11` with exact wire pin `"3.2-beta.10"`; use Go only after its support matrix row names an exact checkpoint. |
