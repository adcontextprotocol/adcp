...
|---|---|---|---|
| **Release candidate** | 3.2 (externally exposed candidate: `rc.3`; latest tagged release: `rc.2`) | `"3.2-rc.2"` | Development and staging only. RC.3 artifacts are signed and publicly reachable but have no Git tag or GitHub Release; RC.2 remains the latest official release. Use only exact pins advertised by the peer. Follow the [3.2 prerelease program](/docs/reference/3-2-beta). |
...
| Version | Released | Notes |
|---|---|---|
| 3.2.0-rc.3 | 2026-09-14 (artifact exposure) | Signed candidate artifacts were exposed through R2, but the Release workflow was cancelled and no Git tag or GitHub Release exists. Do not treat RC.3 as an official release; use `"3.2-rc.3"` only for explicit incident validation with peers that advertise it. |
| **3.2.0-rc.2** | 2026-09-13 | Latest tagged and GitHub-released signed protocol checkpoint. Adds the opt-in Reliable Reporting `sync_reporting_status` loop and other additive RC hardening. Use `"3.2-rc.2"` only with peers that explicitly advertise it; the current SDK wave still embeds `3.2.0-rc.1`. |
| 3.2.0-rc.1 | 2026-09-05 | Previous SDK-backed protocol checkpoint. Completes the experimental Reliable Reporting 1.0 contract and hardens delivery, reach, and vendor-metric semantics. TypeScript `@adcp/sdk@14.0.0-rc.35`, Python `adcp==8.0.0b14`, and Go `adcp/v3@v3.2.1` embed it exactly; use `"3.2-rc.1"` only with peers that explicitly advertise it. |
...
### What's "current"?
For new production integrations, **3.1.20** is current; pin your integration to `"3.1"` (not `"3.1.20"`). Existing 3.0 integrations can remain pinned to `"3.0"` while they migrate. Use 3.2 only for prerelease testing with an exact advertised pin. See [Version negotiation](/docs/reference/versioning#version-negotiation).
...
| Implementing or testing 3.2 without an SDK | Use the official signed **3.2 RC.2** artifacts and wire pin `"3.2-rc.2"` with raw-wire or code-generation workflows. RC.3 artifacts are externally reachable but not tagged or GitHub-released; use `"3.2-rc.3"` only for explicit incident validation with peers that advertise it. Follow the [prerelease program](/docs/reference/3-2-beta). |
| Running an SDK-backed 3.2 integration test | The current SDK wave (`@adcp/sdk@14.0.0-rc.35`, Python `adcp==8.0.0b14`, and Go `adcp/v3@v3.2.1`) still embeds RC.1 and requires `"3.2-rc.1"`; use raw-wire or regenerated clients for exact RC.2 testing. |
...
| Trying a post-3.1 feature | Use the [3.2 prerelease program](/docs/reference/3-2-beta), [Release Notes](/docs/reference/release-notes), and [experimental status](/docs/reference/experimental-status). |
...
- **[What's new in AdCP 3.2](/docs/reference/whats-new-in-3-2)** — prerelease feature set and adopter actions
- **[3.2 prerelease program](/docs/reference/3-2-beta)** — current artifacts, exact SDK pairing, and prerelease testing gates
