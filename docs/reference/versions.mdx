...
| 3.2.0-rc.1 | 2026-09-05 | Previous protocol checkpoint. Completes the experimental Reliable Reporting 1.0 contract and hardens delivery, reach, and vendor-metric semantics. TypeScript `@adcp/sdk@14.0.0-rc.35`, Python `adcp==8.0.0b14`, and Go `adcp/v3@v3.2.1` embed it exactly. |
...
| Implementing or testing 3.2 without an SDK | Use the signed **3.2 RC.3** artifacts with raw-wire or code-generation workflows. Follow the [prerelease program](/docs/reference/3-2-beta). |
| Running an SDK-backed 3.2 integration test | Pair the exact RC.1 wire pin `"3.2-rc.1"` with `@adcp/sdk@14.0.0-rc.35`, Python `adcp==8.0.0b14`, or Go `adcp/v3@v3.2.1`. No SDK embeds RC.2 or RC.3 schemas or types yet — test those against their signed artifacts on the raw wire, or from types generated from them, until the next SDK wave ships. |
