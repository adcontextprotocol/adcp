...
|---|---|---|---|
| **Pending beta** | 3.2 (`beta.0` protocol-first; beta.1 converges protocol and SDKs) | Exact prerelease after publication, such as `"3.2-beta.0"` | Development and staging only. Follow the [3.2 beta program](/docs/reference/3-2-beta). |
...
|---|---|---|
| 3.2.0-beta.0 | Pending first cut | Protocol-first beta input for SDK work. Do not use the artifact or `"3.2-beta.0"` wire pin until the GitHub prerelease and signed bundle are published. Move this row into the published prerelease table with its release date in the beta.0 Version Packages PR. |
...
### What's "current"?
For new production integrations, **3.1.15** is current; pin your integration to `"3.1"` (not `"3.1.15"`). Existing 3.0 integrations can remain pinned to `"3.0"` while they migrate. Use 3.2 only for beta testing with an exact advertised prerelease pin. See [Version negotiation](/docs/reference/versioning#version-negotiation).
