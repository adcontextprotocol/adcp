...
|---|---|---|---|
| **Beta** | 3.2 (current prerelease: `beta.11`) | `"3.2-beta.11"` | Development and staging only. Beta.11 is the current protocol checkpoint; TypeScript SDK beta.29 and Python SDK beta.12 embed it exactly. Follow the [3.2 beta program](/docs/reference/3-2-beta). |
...
### What's "current"?
For new production integrations, **3.1.20** is current; pin your integration to `"3.1"` (not `"3.1.20"`). Existing 3.0 integrations can remain pinned to `"3.0"` while they migrate. Use 3.2 only for beta testing with an exact advertised prerelease pin. See [Version negotiation](/docs/reference/versioning#version-negotiation).
...
| Implementing or testing 3.2 without an SDK | Use the signed **3.2 beta.11** artifacts with raw-wire or code-generation workflows. Follow the [beta program](/docs/reference/3-2-beta). |
