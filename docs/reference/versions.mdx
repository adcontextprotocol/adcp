...
|---|---|---|---|
| **Beta** | 3.2 (current prerelease: `beta.11`) | `"3.2-beta.11"` | Development and staging only. Beta.11 is the current protocol checkpoint; TypeScript SDK beta.29 embeds it exactly. Follow the [3.2 beta program](/docs/reference/3-2-beta). |
...
| Running an SDK-backed 3.2 integration test | Use `@adcp/sdk@14.0.0-beta.29` with exact wire pin `"3.2-beta.11"`; use Go only after its support matrix row names an exact checkpoint. |
