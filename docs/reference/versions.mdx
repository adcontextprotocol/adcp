...
<Info>
**Choosing today:** stable production integrations use 3.0 with wire pin `"3.0"`. 3.1 validation uses the current RC, `v3.1.0-rc.15`, with wire pin `"3.1-rc.15"`. After GA, 3.1 integrations use wire pin `"3.1"`.
</Info>

| Status | Versions | Wire pin | What it means |
|---|---|---|---|
| **Active** | 3.0 (current stable: 3.0.19) | `"3.0"` | Stable default for production integrations. Receives feature and security patches. |
| **Validation** | 3.1 (current RC: `v3.1.0-rc.15`) | `"3.1-rc.15"` during validation; `"3.1"` at GA | Feature-complete minor release validating through RC artifacts. See [What's New in AdCP 3.1](/docs/reference/whats-new-in-3-1) for the final feature set. |
...
**Wire values are release-precision only.** Send `"3.0"` for the stable 3.0 line, exact pre-release pins such as `"3.1-rc.15"` while validating 3.1, and `"3.1"` only after GA. Do not send patch components like `"3.0.19"` or full semver pre-release values like `"3.1.0-rc.15"`. See [patches are not negotiated](/docs/reference/versioning#patches-are-not-negotiated).
...
| 3.1.0-rc.1 through **3.1.0-rc.15** | 2026-05-29 to 2026-06-16 | Feature-complete 3.1 validation train. Current published RC is `v3.1.0-rc.15`; use wire pin `"3.1-rc.15"` during validation and `"3.1"` only after GA. See [What's New in AdCP 3.1](/docs/reference/whats-new-in-3-1) for the final feature list. |
...
### What's "current"?
For stable production, **3.0.19** is current; pin your integration to `"3.0"` (not `"3.0.19"`). For 3.1 validation, the current published RC is **v3.1.0-rc.15**; pin to `"3.1-rc.15"` until GA, then use `"3.1"`. See [Version negotiation](/docs/reference/versioning#version-negotiation).
...
| Validating the final 3.1 feature set before GA | **3.1.0-rc.15**. Pin `"3.1-rc.15"` on the wire, read [What's New in AdCP 3.1](/docs/reference/whats-new-in-3-1) and the [3.0 to 3.1 migration guide](/docs/reference/migration/3-0-to-3-1), and move to `"3.1"` at GA. |
