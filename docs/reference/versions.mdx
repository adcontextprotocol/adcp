...
|---|---|---|---|
| **Release candidate** | 3.2 (current prerelease: `rc.3`; latest tagged release: `rc.2`) | `"3.2-rc.3"` | Development and staging only. RC.3 is the current signed protocol checkpoint: its artifacts are signed and served, and its Git tag and GitHub Release are pending a release-workflow re-run. No SDK embeds RC.3 yet — TypeScript SDK RC.36 embeds `3.2.0-rc.2`, and Python SDK beta.14 and Go SDK 3.2.1 still embed `3.2.0-rc.1`. Test RC.3 from its signed artifacts with raw-wire or code-generation workflows. Follow the [3.2 prerelease program](/docs/reference/3-2-beta). |
...
| Version | Released | Notes |
|---|---|---|
| **3.2.0-rc.3** | 2026-09-14 | Current protocol checkpoint: signed and served, with its Git tag and GitHub Release pending a release-workflow re-run. Hardens the experimental Reliable Reporting consumer-status loop (`content_mismatch` with a closed `mismatch_code`, a bounded grace window for a stale `received`, the buyer posting deadline and `obligation_counts.consumer_status_pending`, issue lifecycle fields, and the reserved `authoritative_party` value), adds a shared MediaBuy-level `frequency_cap` with structured product frequency-cap constraints, and adds request-only targeting inputs with `null`-clear semantics. No SDK embeds it yet; use `"3.2-rc.3"` only with peers that explicitly advertise it. |
| 3.2.0-rc.2 | 2026-09-13 | Previous protocol checkpoint, and the latest tagged and GitHub-released one. Adds the opt-in experimental Reliable Reporting `sync_reporting_status` consumer-status loop, buyer and orchestrator storyboards with a buyer-orchestrator compliance track, cursor pagination for truncated `get_media_buy_delivery` breakdowns, sub-country product coverage filters, and restored `canonical-forecast-point` constraints. TypeScript SDK RC.36 embeds it exactly. |
| 3.2.0-rc.1 | 2026-09-05 | Previous protocol checkpoint. Completes the experimental Reliable Reporting 1.0 contract and hardens delivery, reach, and vendor-metric semantics. Python `adcp==8.0.0b14` and Go `adcp/v3@v3.2.1` embed it exactly, and TypeScript `@adcp/sdk@14.0.0-rc.35` is its historical pairing. |
...
| Implementing or testing 3.2 without an SDK | Use the signed **3.2 RC.3** artifacts with raw-wire or code-generation workflows; RC.2 remains the latest tagged release if you need one. Follow the [prerelease program](/docs/reference/3-2-beta). |
| Running an SDK-backed 3.2 integration test | Pair the exact RC.2 wire pin `"3.2-rc.2"` with `@adcp/sdk@14.0.0-rc.36`, or the exact RC.1 wire pin `"3.2-rc.1"` with Python `adcp==8.0.0b14` or Go `adcp/v3@v3.2.1`. No SDK embeds RC.3 schemas or types yet — test RC.3 against its signed artifacts on the raw wire, or from types generated from them, until the RC.3 SDK wave ships. |
