...
| **Validation** | 3.1 (current RC: `v3.1.0-rc.9`) | `"3.1-rc.9"` during validation; `"3.1"` at GA | Feature-complete minor release validating through RC artifacts. See [What's New in AdCP 3.1](/docs/reference/whats-new-in-3-1) for the final feature set. |
...
| 3.1.0-rc.1 through **3.1.0-rc.9** | 2026-05-29 to 2026-06-05 | Feature-complete 3.1 validation train. Current published RC is `v3.1.0-rc.9`; use wire pin `"3.1-rc.9"` during validation and `"3.1"` only after GA. See [What's New in AdCP 3.1](/docs/reference/whats-new-in-3-1) for the final feature list. |
...
### What's "current"?
For stable production, **3.0.15** is current; pin your integration to `"3.0"` (not `"3.0.15"`). For 3.1 validation, the current published RC is **v3.1.0-rc.9**; pin to `"3.1-rc.9"` until GA, then use `"3.1"`. See [Version negotiation](/docs/reference/versioning#version-negotiation).
...
| Validating the final 3.1 feature set before GA | **3.1.0-rc.9**. Pin `"3.1-rc.9"` on the wire, read [What's New in AdCP 3.1](/docs/reference/whats-new-in-3-1), and move to `"3.1"` at GA. |
