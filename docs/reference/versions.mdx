...
| Version | Released | Notes |
|---|---|---|
| **3.2.0-beta.6** | 2026-08-23 | Current prerelease and TypeScript SDK-backed checkpoint. `@adcp/sdk@14.0.0-beta.11` embeds this exact bundle. Adds reporting negotiation and breakdown contracts, CTV experience profiles, premium display canonicals, and A2A request-signing method names. Use `"3.2-beta.6"` only when both peers explicitly advertise it. |
...
| Running an SDK-backed 3.2 integration test | Use `@adcp/sdk@14.0.0-beta.11` with exact wire pin `"3.2-beta.6"`, or Python `adcp==8.0.0b7` with exact wire pin `"3.2-beta.5"`; use Go only after its support matrix row names an exact checkpoint. |
