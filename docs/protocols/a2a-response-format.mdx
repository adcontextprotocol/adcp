...
Async operations (`status: "submitted"`) deliver push notifications following **A2A Protocol's push notification payload structure** (A2A Specification Section 4.3.3). The payload uses A2A's Task structure with a Message containing parts array, not the artifacts structure used in synchronous responses.
...
- Webhooks use `message.parts[]` (A2A Message structure), not `artifacts[]`
- Extract AdCP data by finding the DataPart in `message.parts` array (part with `data` property)
- Use the last DataPart as authoritative when multiple data parts exist
