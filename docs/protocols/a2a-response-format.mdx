...
POST /webhook-endpoint
{
  "statusUpdate": {
    ...
    "status": {
      ...
      "message": {
        "messageId": "msg_123_001",
        "role": "agent",
        "parts": [
          {"text": "Campaign requires approval"},
          {"data": {
...
**Final completion (`completed`) - using `task` for full state:**
```json
POST /webhook-endpoint
{
  "task": {
...
**Key differences from synchronous A2A responses:**
- Webhooks use StreamResponse wrapper containing `task`, `message`, `statusUpdate`, or `artifactUpdate` (not direct Task/Message)
...
- Extract AdCP data by finding the DataPart in `statusUpdate.status.message.parts` or `task.status.message.parts` array (part with `data` property)
- Use the last DataPart as authoritative when multiple data parts exist
...
**For webhook authentication, retry patterns, and security**, see [Core Concepts - Webhook Reliability](/docs/protocols/core-concepts#webhook-reliability).
