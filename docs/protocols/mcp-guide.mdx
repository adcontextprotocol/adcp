...
### Tool Call with Initiator Context

```javascript
// Pass opaque initiator context; agents must carry it back
...
```json
{
  ...
  "data": {                       // Task-specific structured data
    "context": { "ui": "buyer_dashboard" }, // Initiator context echoed back
    "products": [...],
...
**Note:** All protocol envelope fields are returned with every MCP tool response. The task’s domain payload (inside `data`) contains the echoed `context` object alongside task-specific fields (other protocols may use `payload`/`result`).
