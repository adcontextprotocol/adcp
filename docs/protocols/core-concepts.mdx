...
// Later: Webhook POST when approval is needed (A2A StreamResponse format)
...
function extractWebhookData(webhookPayload) {
  ...
  // Handle other StreamResponse variants (message, statusUpdate, artifactUpdate)
  // ...
}
```
...
- For task updates, use the `task` variant: `{task: {id, contextId, status: {state, message: {parts: [...]}, timestamp}, ...}}`
- Task ID is in `task.id` (not top-level `taskId`)
- Task status state is in `task.status.state`
- Task-specific data is in `task.status.message.parts[]` array - find the DataPart (part with `data` property)
