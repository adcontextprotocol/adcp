...
| Aspect | MCP | A2A |
|--------|-----|-----|
| **Spec Status** | AdCP provides this | Native protocol feature |
...
MCP doesn't define push notifications, so AdCP provides this capability using the same configuration structure as A2A.
...
Both validate against the same schema (e.g., `create-media-buy-async-response-completed.json`).
...
|--------|-------------|----------|
| `completed` | `[task]-async-response-completed.json` | Success payload (IDs, packages, etc.) |
| `failed` | `[task]-async-response-failed.json` | Error details (`errors` array) |
...
Each async operation has status-specific data schemas:

| Task | Completed Schema | Failed Schema |
|------|-----------------|---------------|
| `create_media_buy` | `create-media-buy-async-response-completed.json` | `create-media-buy-async-response-failed.json` |
| `update_media_buy` | `update-media-buy-async-response-completed.json` | `update-media-buy-async-response-failed.json` |
| `sync_creatives` | `sync-creatives-async-response-completed.json` | `sync-creatives-async-response-failed.json` |
...
**Key principle:** Push notifications are ONLY sent for `submitted` operations. The data payload uses the same schema regardless of transport (MCP webhook or A2A native message).
