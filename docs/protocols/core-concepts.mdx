...
// Later: Webhook POST when approval is needed (A2A StreamResponse format - statusUpdate for status change)
...
**`input-required` webhook (human needs to respond) - A2A StreamResponse format:**
```json
{
  "statusUpdate": {
    "taskId": "task_456",
    ...
    "status": {
      ...
      "message": {
        ...
        "parts": [
          {
            "text": "Campaign budget requires VP approval to proceed"
          },
          {
            "data": {
              "buyer_ref": "nike_q1_campaign_2024",
