...
// Later: Webhook POST when approval is needed (A2A format)
...
// Later: Webhook POST when approved and completed (A2A format)
POST /webhooks/adcp/create_media_buy/agent_123/op_456 HTTP/1.1
Content-Type: application/json
X-ADCP-Signature: sha256=...
...
**AdCP fully adopts A2A Protocol's push notification payload structure** (A2A Specification Section 4.3.3). The payload follows A2A's Task structure with a Message containing parts array. ADCP declares only the structure of `parts[].data` (DataPart data field) for task-specific payloads.
...
**`input-required` webhook (human needs to respond) - A2A format:**
```json
{
  "taskId": "task_456",
  "status": "input-required",
...
**`completed` webhook (operation finished - full create_media_buy response) - A2A format:**
```json
{
  "taskId": "task_456",
  "status": "completed",
...
**`failed` webhook (operation failed) - A2A format:**
```json
{
  "taskId": "task_456",
  "status": "failed",
...
function extractWebhookData(webhookPayload) {
  ...
  const dataPart = webhookPayload.message.parts.find(part => part.data !== undefined);
  
  if (dataPart) {
    return dataPart.data; // This is the AdCP task-specific payload
  }
  
  // For interim statuses (input-required, working), DataPart may be absent
  // Extract text message instead
  const textPart = webhookPayload.message.parts.find(part => part.text !== undefined);
  return {
...
- Task-specific data is in `message.parts[]` array - find the DataPart (part with `data` property)
