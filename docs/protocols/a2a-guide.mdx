...
await a2a.send({
  ...
  pushNotificationConfig: {
    ...
    authentication: {
      schemes: ["HMAC-SHA256"],
      credentials: generateUniqueSecret(operationId) // Unique secret per operation
    }
  }
});
```
...
If authentication is provided, AdCP enforces HMAC-SHA256. Server signs payload with `X-ADCP-Signature` and `X-ADCP-Timestamp` headers.
