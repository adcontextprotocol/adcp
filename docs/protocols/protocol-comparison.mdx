...
// Optional: Configure webhook (HMAC-SHA256 authentication)
const response = await session.call('create_media_buy', params, {
  push_notification_config: {
    url: "https://buyer.com/webhooks",
    authentication: {
      schemes: ["HMAC-SHA256"],
      credentials: generateUniqueSecret(operationId) // Unique per operation
    }
  }
});
```
...
// Native webhook support (HMAC-SHA256 authentication)
await a2a.send({
