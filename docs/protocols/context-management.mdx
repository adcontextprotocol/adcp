...
sequenceDiagram
    ...
    rect rgb(240, 248, 255)
        Note over B,S: Turn 1 - Establish Context
        B->>S: get_products(brief: "Video ads")
        S-->>B: products[prod_123, ...] + context_id: "ctx_abc"
    end

    rect rgb(240, 255, 240)
        Note over B,S: Turn 2 - Refine (same context)
        B->>S: get_products(context_id: "ctx_abc", brief: "Focus on premium")
        S-->>B: refined products[] + context_id: "ctx_abc"
    end

    rect rgb(255, 248, 240)
        Note over B,S: Turn 3 - Execute (same context)
        B->>S: create_media_buy(context_id: "ctx_abc", product_id: "prod_123")
        S-->>B: media_buy_id + task_id + context_id: "ctx_abc"
    end
```

The `context_id` persists across all turns, allowing the agent to remember previous search results and workflow state. The `product_id` in Turn 3 references a product discovered in Turn 1.
...
Webhook updates also include the same application-level `context` inside the webhook `result` payload, ensuring correlation tokens persist through asynchronous operations.
