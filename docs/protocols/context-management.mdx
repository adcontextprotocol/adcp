...
Task request payloads may include an optional `context` object that carries application-level metadata (session hints, correlation tokens, tracking identifiers, etc.). The agent must echo this object back unchanged inside the task payload, including webhook payloads (in `message.parts[].data` for A2A-compatible webhooks).
...
Webhook updates include the same application-level `context` inside the webhook payload's DataPart (`message.parts[].data` for A2A-compatible webhooks).
