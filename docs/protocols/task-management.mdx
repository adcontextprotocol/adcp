...
- `task`: A Task object with the current task state (most common for ADCP webhooks)
...
**Task object structure (when using `task` variant):**
...
**Message.parts structure (within task.status.message):**
...
function extractWebhookData(webhookPayload) {
  ...
  if (webhookPayload.task) {
    ...
    const taskId = task.id; // Task ID is in task.id, not top-level
    const status = task.status.state; // Status state is in task.status.state
    
    // Find the DataPart in task.status.message.parts array
  ...
  // Handle other StreamResponse variants...
  return null;
}
```
...
The webhook receives the **full A2A-compatible StreamResponse structure** for each status. Extract task data from `task.status.message.parts[]` array by finding the DataPart (part with `data` property). Task ID is in `task.id` and status state is in `task.status.state`. This means your webhook handler gets all the context and data needed to take appropriate action.
