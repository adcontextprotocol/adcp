...
**Top-level fields (A2A Task structure):**
- `taskId` (required) — A2A task identifier. Unique identifier for this task
- `status` (required) — Current task status (A2A TaskState enum)
- `timestamp` (optional) — ISO 8601 time when webhook was generated
- `contextId` (optional) — A2A context identifier. Conversation/session id
- `message` (required) — A2A Message structure containing parts array
...
**Example: `input-required` webhook (human approval needed) - A2A format:**
```http
...
**Example: `completed` webhook (after approval granted - full create_media_buy response) - A2A format:**
```http
POST /webhooks/adcp/create_media_buy/agent_123/op_456 HTTP/1.1
Host: buyer.example.com
Authorization: Bearer your-secret-token
...
The webhook receives the **full A2A-compatible response structure** for each status. Extract task data from `message.parts[]` array by finding the DataPart (part with `data` property). This means your webhook handler gets all the context and data needed to take appropriate action.
