...
**Task Type Identification**: The webhook payload does not include task type information. Task type identification depends on how you register your webhook. The recommended approach is to include the task type in the webhook URL path (e.g., `/webhooks/adcp/{task_name}/...`). See [Task Type Identification](/docs/protocols/core-concepts#task-type-identification) for details.
...
const response = await session.call('create_media_buy',
  { /* task params */ },
  {
    push_notification_config: {
      // Recommended: Include task type in URL path for easy identification
...
**Example: `input-required` webhook (additional input or fixes required) - A2A StreamResponse format:**
```http
...
app.post('/webhooks/adcp/:task_type/:agent_id/:operation_id', async (req, res) => {
  // Task type is extracted from URL path (recommended approach)
  // Alternative: Use taskId from payload and maintain a local mapping table
