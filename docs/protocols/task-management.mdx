...
|----------|-------------|---------------|----------|---------------|
| **MCP** | AdCP provides this | `pushNotificationConfig` | `mcp-webhook-payload.json` | `result` field |
| **A2A** | Native to protocol | `pushNotificationConfig` | `Task` or `TaskStatusUpdateEvent` | `status.message.parts[].data` |

Both protocols use **identical AdCP data schemas** (e.g., `create-media-buy-async-response-completed.json`).
...
**Webhooks (MCP) / Status Updates (A2A) are ONLY triggered when the initial response status is `submitted`** (long-running operations).
...
- Initial response is `working` → Will complete synchronously within ~120 seconds, client should wait

**When push notifications ARE sent (for `submitted` operations only):**
- Status changes to `input-required` → Notification sent (human input needed)
- Status changes to `completed` → Notification sent (final result available)
- Status changes to `failed` → Notification sent (error details provided)
- Status changes to `canceled` → Notification sent (cancellation confirmed)
...
- `result` — Task-specific payload validated against status-specific schema
...
| Status | Schema |
|--------|--------|
| `completed` | `[task]-async-response-completed.json` |
| `failed` | `[task]-async-response-failed.json` |
