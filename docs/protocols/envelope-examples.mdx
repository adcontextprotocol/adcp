...
### Task Status Webhook Payload (A2A Format)

AdCP fully adopts A2A Protocol's push notification payload structure (A2A Specification Section 4.3.3). The payload is a StreamResponse object (A2A Specification Section 3.2.3) containing exactly one of: `task`, `message`, `statusUpdate`, or `artifactUpdate`.
...
**Note:** ADCP declares only the structure of `parts[].data` (DataPart data field). Extract task data by finding the DataPart in `statusUpdate.status.message.parts` or `task.status.message.parts` array. Task ID is in `statusUpdate.taskId` (for statusUpdate) or `task.id` (for task variant). Status state is in `statusUpdate.status.state` or `task.status.state`.
