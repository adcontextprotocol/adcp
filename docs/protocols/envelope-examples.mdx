...
**Note:** ADCP declares only the structure of `parts[].data` (DataPart data field). Extract task data by finding the DataPart in `task.status.message.parts` array. Task ID is in `task.id` and status state is in `task.status.state`.
