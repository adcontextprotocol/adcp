...
AdCP fully adopts A2A Protocol's push notification payload structure (A2A Specification Section 4.3.3):
...
**Note:** ADCP declares only the structure of `parts[].data` (DataPart data field). Extract task data by finding the DataPart in `message.parts` array.
