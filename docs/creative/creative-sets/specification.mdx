...
<Info>
**AdCP 2.6 Proposal** - This specification is under development for AdCP 2.6. Feedback welcome via [GitHub Discussions](https://github.com/adcontextprotocol/adcp/discussions).
</Info>
...
The Creative Set Protocol provides:
...
The Creative Set Protocol involves three agent roles:

#### Creative Agent
The platform hosting creative sets and building creatives:
- **Responsibilities**: Store source assets, build creatives, serve ad tags
- **Capabilities**: Format building, creative adaptation, governance enforcement
...
- **Authentication**: Uses tokens from `creative_set_ref` to access creative sets

#### Orchestrator (Buyer Agent)
The platform coordinating media buying workflows:
- **Responsibilities**: Create creative sets, build initial creatives, create media buys
- **Capabilities**: Workflow orchestration, budget management, campaign planning
- **Authentication**: Has credentials with both creative and seller agents

### Creative Set

A creative set is a workspace containing:

| Component | Description | Required |
|-----------|-------------|----------|
| `creative_set_id` | Unique identifier | REQUIRED |
...
A `creative_set_ref` in a media buy provides seller agents with access to fetch creatives:
...
The seller agent MUST use these credentials when calling `get_creative_set`.
...
1. **Orchestrator** calls `create_creative_set` on creative agent
2. **Creative agent** creates workspace, returns `creative_set_id`
3. **Orchestrator** calls `build_creatives` for initial formats
4. **Creative agent** builds creatives, embeds trackers in ad tags

### Media Buy Phase

5. **Orchestrator** calls `create_media_buy` on seller agent with `creative_set_ref`
6. **Seller agent** sends `creative-set-attached` webhook to creative agent
7. **Creative agent** registers seller as subscriber for updates

### Fetch Phase

8. **Seller agent** calls `get_creative_set` to fetch creatives
...
14. **Seller agent** fetches new creative via `get_creative_set`

### Detachment Phase

15. **Seller agent** sends `creative-set-detached` webhook when media buy ends
...
Creates a new creative set workspace.

**Request**: See `/schemas/creative-set/create-creative-set-request.json`
...
**Response**: Returns `creative_set_id` and initial state.

### get_creative_set

Retrieves a creative set with its creatives.

**Request**: See `/schemas/creative-set/get-creative-set-request.json`

**Required Fields**:
- `creative_set_id`: Set to retrieve
...
**Response**: Returns creative set with filtered creatives. Each creative includes a `manifest` with the ad tag as an asset.
...
**Request**: See `/schemas/creative-set/list-creative-sets-request.json`
...
**Response**: Returns array of creative set summaries (metadata only, no ad tags).

### update_creative_set

Modifies a creative set.

**Request**: See `/schemas/creative-set/update-creative-set-request.json`

**Required Fields**:
- `creative_set_id`: Set to update
...
**Request**: See `/schemas/creative-set/delete-creative-set-request.json`

**Required Fields**:
- `creative_set_id`: Set to delete
...
**Request**: See `/schemas/creative-set/build-creatives-request.json`

**Required Fields**:
- `creative_set_id`: Set containing source assets
...
#### creative-set-attached

Sent when seller receives a media buy with `creative_set_ref`.
...
- MUST register seller as subscriber for this creative set
- SHOULD store `callback_url` for sending notifications
- MAY auto-build missing formats if `allow_format_requests` is true

#### creative-set-detached

Sent when media buy ends or creative set is replaced.
...
Sent when seller needs a format not in the creative set.
...
**Seller Behavior**:
- If `ready`, SHOULD fetch creative via `get_creative_set`
...
| `SET_ARCHIVED` | Cannot modify archived creative set |
...
Creative agents MUST authenticate seller access using the credentials in `creative_set_ref.authentication`. Supported schemes:
...
Tokens in `creative_set_ref` SHOULD be scoped to:
- The specific `creative_set_id`
...
- Use `creative_set_id` + `media_buy_id` + timestamp for deduplication

### Performance

- `get_creative_set` with `format_ids` filter SHOULD be optimized for seller polling
- Large creative sets SHOULD support pagination
