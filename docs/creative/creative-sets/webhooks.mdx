---
title: Webhooks
description: Webhook payloads for creative set lifecycle events
---
...
sequenceDiagram
    ...
    Seller->>Creative: creative-set-attached
    ...
    Seller->>Creative: creative-set-detached
```
...
Sent when a seller agent receives a media buy with a `creative_set_ref`. Registers the seller's interest and lists required formats.
...
|-------|----------|-------------|
| `creative_set_id` | Yes | The creative set being attached |
...
- Register the seller as a subscriber for this creative set
...
Sent when a media buy ends, is cancelled, or the creative set is replaced.
...
| Field | Required | Description |
|-------|----------|-------------|
| `creative_set_id` | Yes | The creative set being detached |
...
Sent when a seller needs a format that doesn't exist in the creative set. Only processed if `allow_format_requests` is true on the creative set.
...
| Field | Required | Description |
|-------|----------|-------------|
| `creative_set_id` | Yes | The creative set to build from |
...
| Field | Required | Description |
|-------|----------|-------------|
| `creative_set_id` | Yes | The creative set |
...
**Status values:**
- `ready` - Creative is built and available via `get_creative_set`
...
2. **Bearer token** - Use token from `creative_set_ref.authentication`
...
- Stop retrying after `creative-set-detached` received
