...
For traditional formats (display banners, video), use `portfolio_ref` to point to pre-built creative variants:
...
### Use `assets` for universal formats, `portfolio_ref` for traditional formats
...
These can coexist on the same offering.
