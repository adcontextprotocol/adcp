...
Meta Reels is a useful test of canonical-formats coverage: it's a platform-specific format from a vendor that hasn't adopted AdCP, with several rendering bells (CTA enum, primary text, headline limits, logo overlay, music) on top of a vertical video. Each Reels feature lands somewhere — canonical `params`, slot-level narrowing, `platform_extensions`, or `brand_kit_override` — and the canonical doesn't need to grow.
...
| Headline (40ch), Primary text (125ch) | `params.headline_max_chars`, `params.primary_text_max_chars` | Built-in slot narrowing on `video_hosted`. |
| CTA from a fixed enum | `params.cta_values[]` | Meta's CTA list as an enum narrowing on the slot. |
| Landing page URL | Built-in `video_hosted` slot | No special handling. |
| Logo overlay | `BrandRef.brand_kit_override.logo` on the manifest | Logo is brand context, not a per-format slot — Meta pulls from the brand kit. |
| Brand name | `BrandRef.display_name` (preferred) or `brand_name` slot | Meta auto-fills from the advertiser's Page. |
| Music overlay (Reels music library) | `platform_extensions/meta_placements_reels` | Not in current `video_hosted` canonical. Platform-specific. |
| Pixel / conversion tracking | `platform_extensions/meta_pixel` | Carries pixel_id, event mappings, deduplication keys. |
| Placement selection (Feed vs Reels vs Stories) | `platform_extensions/meta_placements_reels` | Sibling extensions exist for other Meta placements. |

### Community-registry hosting

Meta hasn't adopted AdCP, so its `adagents.json` lives at the AAO community-registry mirror, `https://mirror.adcontextprotocol.org/translated/meta/adagents.json`. The mirror file declares `formats[]` at the publisher catalog level — one declaration of Meta Reels, scoped to Instagram + Facebook (not WhatsApp), reused across every seller's products. When Meta later publishes its own `adagents.json` at `meta.com/.well-known/adagents.json`, the platform-hosted file takes precedence and the mirror entry deprecates.

```json test=false
// https://mirror.adcontextprotocol.org/translated/meta/adagents.json (excerpt)
{
  ...
  "formats": [
    {
      ...
      "params": {
        ...
        "platform_extensions": [
          { "uri": "https://mirror.adcontextprotocol.org/translated/meta/extensions/meta_pixel",            "digest": "sha256:a3f5..." },
          { "uri": "https://mirror.adcontextprotocol.org/translated/meta/extensions/meta_placements_reels", "digest": "sha256:b8e1..." }
        ]
      }
    }
  ],
...
A seller's `meta_reels_us` product references the publisher-catalog declaration by `capability_id`, narrowing only the parts specific to that product (geography, pricing). The format shape itself is inherited from `mirror.adcontextprotocol.org/translated/meta`:
...
The `capability_id: "meta_reels"` tag lets buyer agents recognize this as the same Meta Reels they read from the publisher catalog — the seller didn't reinvent the format, they're selling inventory against the canonical declaration. Buyer's manifest validates against canonical `video_hosted` first (does it satisfy the contract any seller speaking that canonical accepts?), then narrows against this product's specific parameters. Platform extensions (pixel, placements) are bundled in the `get_products` response under an `extensions` map keyed by `uri@digest` — buyers fetch them once and cache by digest.

### What's NOT a canonical change

Three Reels features that look like they want canonical-level support, but don't:

- **Logo overlay**: belongs on `BrandRef.brand_kit_override.logo`. A logo isn't a Reels-specific input; every brand-aware format consumes brand context the same way.
- **Music overlay**: lives in `platform_extensions/meta_placements_reels`. Genuinely platform-specific (Meta's music library, licensing flags, regional availability). Not worth promoting to canonical until 2+ platforms have substantively similar audio-overlay semantics.
- **Placement selection** (Feed vs Reels vs Stories vs Marketplace): also platform-specific extension. A sibling Stories declaration would use `meta_placements_stories`; same canonical, different extension.

This pattern — deeply-narrowed platform format expressed through `params` + slot narrowing + `platform_extensions` + `BrandRef` — is what keeps the 11 canonicals from accumulating per-platform fields.
...
The AAO catalog at `creative.adcontextprotocol.org` (the published v1 format library in `server/src/creative-agent/reference-formats.json`) has **41 of 57 entries** (72%) annotated with `canonical: <format_kind>` — direct v1→v2 projection, no SDK guesswork. The remaining 16 fall into deliberate gaps the spec is explicit about rather than coverage shortfalls:

- **2 entries** (`native_standard`, `native_content`) — deferred to 3.2 pending the native canonical decision (above).
- **3 entries** (`broadcast_spot_15s/30s/60s`) — broadcast TV's GRP/spot transaction model differs from web impressions; canonical decision deferred to 3.2.
- **4 entries** (`dooh_billboard_*`, `dooh_transit_screen`) — DOOH's location-keyed measurement (Geopath, COMMB) doesn't reduce to web pixels; canonical decision deferred.
- **7 entries** (`product_card_*`, `format_card_*`, `proposal_card_*`, `native_product_card`) — UI scaffolding for browsing catalogs in agent interfaces, not ad formats. To be moved out of `reference-formats.json` into a separate UI-scaffolding registry.

The SDK side's v1→v2 projection (validated in [adcp-client #1815](https://github.com/adcontextprotocol/adcp-client/pull/1815)) reports **41/57 clean projection + 16/57 honest fail-closed** (`catalog_lacks_canonical_annotation`). Same architecture symmetry on v2→v1 — clean projection on the annotated subset, honest fail-closed via `v1_translatable: false` / `canonical_formats_only: true` for canonicals the spec is explicit about not having v1 forms.

The honesty principle: SDKs don't shoehorn un-annotated catalog entries into wrong canonicals (`display_*_generative` → `image`, not `display_tag`; `native_mention` → `agent_placement` because the description literally matches; broadcast/DOOH stay un-annotated because the transaction model differs). Empirical coverage rises as the working group makes specific canonical decisions; it doesn't rise by guessing.
