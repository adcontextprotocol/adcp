...
Sellers offering this canonical MUST support `preview_creative` rendering every declared state × breakpoint from a candidate manifest, in every supply mode. This is what turns a 6–20 business-day proof-and-revise cycle into an API round-trip: the manifest is deterministically renderable, so there is no structural reason to hold a manual proof in reserve. Buyer approval of the returned previews is the acceptance step, replacing a separate physical proof round. (`preview_render.role` has no typed per-state binding — see below — so sellers label each state × breakpoint render descriptively through that field.)
