...
| **`creative.supported_formats`** | Capabilities-response field on creative agents declaring canonical build, validation, and preview capabilities. Each entry carries a stable `capability_id`, a full canonical `format` declaration, and required non-empty `operations`. |
...
2. **AAO community mirror**: on 404 or absence of `formats[]`, query the AAO registry for a community-maintained mirror. Mirror declarations are advisory and preserve their lower-authority provenance.
...
The resolver records whether the catalog came from the publisher origin or an AAO community mirror. Sales-agent product declarations retain seller provenance and are never reported as a publisher catalog.

### Community-registry hosting

When a platform has not adopted AdCP, AAO may maintain an advisory community `adagents.json` translation and expose it through the publisher registry lookup. The mirror can declare Meta Reels once, scoped to Instagram + Facebook (not WhatsApp), for compatibility checks across sellers. Buyers query the registry API first for provenance-aware summaries and follow `files.adagents_json.registry_url` only when they need fields omitted by the lossy projection. When the platform publishes its own origin file, that publisher-hosted declaration takes precedence and the community entry is no longer returned as authority.
