...
Every required ratio MUST also be accepted, and the array MUST contain exactly one valid rendition for each required ratio. Accepted ratios not listed as required are optional. All entries in a rendition set represent the same semantic image; only density-specific encoding properties such as URL, intrinsic dimensions, format, and file size may differ. SDKs MUST reject missing required coverage with `required_pixel_ratio_missing`, duplicate density entries with `pixel_ratio_duplicate`, and required ratios outside the accepted set with `required_pixel_ratio_not_accepted`. `min` and `max` SHOULD bound the expected rendition count, but density coverage—not array length alone—is authoritative.
...
**AdCP 3.1 reference-catalog bridge.** The 3.1 reference catalog publishes concrete 2x companions for its seven fixed image formats: `display_300x250_image_2x`, `display_728x90_image_2x`, `display_320x50_image_2x`, `display_160x600_image_2x`, `display_336x280_image_2x`, `display_300x600_image_2x`, and `display_970x250_image_2x`. Each v1 definition keeps the original logical `renders[].dimensions` and requires two ordinary legacy image assets: one at the logical 1x dimensions and one at exactly twice the width and height. A Retina-only submission is invalid. This is legacy-format support only: AdCP 3.1 does not define canonical pixel-density or rendition-set fields, and permissive JSON acceptance does not give unknown properties protocol meaning. The entries' density and coverage values under `canonical_parameters` are opaque forward-projection metadata that 3.2-aware SDKs may use when projecting the legacy ID into the 3.2 canonical model; they are not canonical 3.1 support. These aliases let 3.1 products use shared AAO format IDs instead of seller-custom formats.
...
SDKs project `width` and `height` as canonical logical dimensions and project a non-default `pixel_ratio` to `pixel_ratios: [pixel_ratio]`. Omitted `pixel_ratio` is 1x and MAY remain omitted canonically. A canonical declaration accepting multiple densities SHOULD carry one `v1_format_ref` per density, all pointing to the same `display_image` template with different parameter values. This is parameter fan-out, not format-definition proliferation. The concrete 3.1 `_image_2x` aliases remain valid compatibility identifiers but SHOULD NOT be extended to new sizes after the parameterized form ships.
