...
| **`creative.preview`** | Capabilities-response field declaring which `creative.supported_formats[].capability_id` routes accept `preview_creative` and whether their output is `authoritative` or `representative`. |
...
| **`presentation_ref`** | URI+digest reference on a publisher placement to publisher-specific chrome or layout metadata. It belongs to the placement, never the shared format. |
...
Dynamic preview support is declared in `get_adcp_capabilities.creative.preview`:
...
`authoritative` is reserved for a preview rendered by the serving platform's own preview machinery, including a seller proxying a walled garden's first-party preview API. `representative` means a seller- or creative-agent-rendered approximation. Buyers MUST NOT infer authority from `quality: "production"`: quality describes fidelity within the selected renderer, while `creative.preview.fidelity` describes whether that renderer represents the serving platform.

For automatic presentation, buyer tooling uses this authority order:

1. An `authoritative` seller `preview_creative` route.
2. The targeted publisher placement's `preview_provider` route.
3. The targeted publisher placement's `presentation_ref`, applied to the resolved canonical manifest.
4. The community format entry's `reference_renderer`.
5. No render; show the manifest and its assets.

Lower layers MUST NOT override or replace a higher layer. A `representative` `preview_creative` result may be shown as a clearly labeled approximation, but it is outside the authoritative chain and MUST NOT displace publisher presentation metadata. If more than one placement is targeted, tooling MUST NOT use one placement's presentation metadata as if it represented the others.

`preview_provider` lets a publisher designate a remote AdCP creative agent as authoritative for named format routes on one placement. Each route maps a same-file `format_option_id` to a provider-local `capability_id`. The buyer discovers the provider's current capabilities, verifies that the capability advertises `preview` and satisfies the resolved placement format, then calls `preview_creative`. A provider that advertises itself as `representative` becomes publisher-authoritative only inside the publisher's explicit delegation; it remains representative for every other placement and publisher. Consumers MUST use provider-specific authorization and MUST NOT forward seller or publisher credentials to the delegated endpoint.
...
`presentation_ref` is an immutable HTTPS `uri` plus SHA-256 `digest` on `adagents.json` `placements[]`. The referenced JSON describes publisher-specific chrome, layout, safe areas, or disclosure positions. It is data, not executable code; it remains in the publisher's namespace and MUST NOT be copied onto a shared format entry.

A community-registry `formats[]` entry may instead declare a `reference_renderer` with `runtime: "browser-esm"`, `package`, exact `version`, named `export`, and package-tarball `integrity`. The first renderer contract is deliberately browser-first: the export accepts canonical manifest data and returns an inert presentation without Node.js APIs, ambient credentials, delivery tracking, or undeclared network access. Non-JavaScript clients use a hosted `preview_creative` provider or show the manifest; renderer availability never gates protocol participation. Compatibility is bound between the named export and the enclosing format entry: the export's documented input/output contract MUST implement that entry's revision. Package semantic versioning only identifies the pinned distribution artifact, so one package version MAY expose separate exports for different formats or format revisions. When one format moves to a new major revision, its entry points to a compatible export without forcing unrelated exports or the shared package onto the same major. AgenticAdvertising.org reviews and references that package but never serves executable renderer code. Consumers MUST require npm provenance, verify the exact version and integrity before loading, and run the renderer as untrusted code in a sandbox. Its output MUST be labeled as a non-authoritative community reference presentation. VAST reference players and OpenRTB Native renderers are precedent for this executable half of an interoperable format specification; they are not precedent for treating a reference implementation as serving-platform truth.
...
AgenticAdvertising.org's hosted reference agent at `https://creative.adcontextprotocol.org/mcp` provides representative image, native-in-feed, and VAST preview routes corresponding to the browser implementations released from the JavaScript SDK repository as `@adcp/reference-renderers`. The current pinned prerelease is `1.0.0-beta.0`; it exports `renderImage`, `renderNativeInFeed`, and `renderVast`. Each `PreviewRender` includes versioned `renderer` audit metadata and `tracking_suppressed: true`. The VAST reference path renders a safe media file from inline XML but does not resolve remote wrappers, execute VPAID/SIMID code, expand macros, or fire impression, click, billing, conversion, or measurement resources. A publisher may designate these routes through `preview_provider`; without that publisher declaration, the hosted output remains representative.
...
Browser clients normally bundle the named export after their package manager verifies the pinned tarball. The export returns a complete inert HTML document for an isolated iframe; it does not mutate the host DOM. Clients that do not execute JavaScript call the hosted `preview_creative` route instead.
...
Uniform output shape does not imply equal authority. Buyers discover preview routes and their authority from `creative.preview`, then apply the [rendering authority and fallback order](#rendering-authority-and-fallback-order). A community `reference_renderer` is useful when no platform preview exists, but its output remains non-authoritative.
