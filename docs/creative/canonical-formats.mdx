...
- **8 entries** (`display_generative`, `display_300x250_generative`, `display_728x90_generative`, `display_320x50_generative`, `display_160x600_generative`, `display_336x280_generative`, `display_300x600_generative`, `display_970x250_generative`) — AI-generated display creatives. The v1 entries carry `generation_prompt: text` (no image asset), which cannot satisfy `format_kind: image`'s required `image_main: image` slot. A bare `canonical: image` annotation would project to a v2 declaration that wrongly demands image upload. Deferred to 3.2 pending a decision between (A) enriching the catalog `canonical` field shape to carry `asset_source` + `slots[]` override, or (B) adding an `image_generative` canonical (with parallel future `video_generative`, `audio_generative`). Tracked in [adcp#4689](https://github.com/adcontextprotocol/adcp/issues/4689).
...
The SDK side's v1→v2 projection (validated in [adcp-client #1815](https://github.com/adcontextprotocol/adcp-client/pull/1815)) reports **33/57 clean projection + 24/57 honest fail-closed** (`catalog_lacks_canonical_annotation`). Same architecture symmetry on v2→v1 — clean projection on the annotated subset, honest fail-closed via `v1_translatable: false` / `canonical_formats_only: true` for canonicals the spec is explicit about not having v1 forms.

**Generative-image deannotation note (3.1).** Earlier 3.1 drafts annotated the 8 `display_*_generative` catalog entries as `canonical: image`. SDK implementor review surfaced the contradiction: the bare annotation projects to a v2 declaration that requires image upload, which generative buyers cannot satisfy. **Sellers expressing "I don't do generative" is automatic** — the default `asset_source: buyer_uploaded` on `format_kind: image` already excludes generative submission. To opt INTO generative, a seller declares `asset_source: agent_synthesized` AND overrides `slots[]` to swap `image_main: image` for `creative_brief: brief` (or similar) — see the Veo worked example.

The honesty principle: SDKs don't shoehorn un-annotated catalog entries into wrong canonicals. `display_*_generative` cannot satisfy `image`'s required slot contract — leaving them un-annotated until the spec provides the right shape is more correct than a lossy projection. Empirical coverage rises as the working group makes specific canonical decisions; it doesn't rise by guessing.
