...
| Pixel / conversion tracking | NOT on the format. Conversion tracking is `sync_event_sources` / `event_log` territory — campaign-scoped, not creative-scoped. | The same pixel fires for every ad in the campaign regardless of creative. Putting it on the format declaration was a category error in earlier drafts. |
...
- **Campaign / event-log surfaces** — conversion tracking (Meta Pixel, GA4, server-side events). These are `sync_event_sources` / `event_log` concerns, NOT creative format concerns. Earlier drafts of this example put `meta_pixel` on the format declaration; that was a category error and is gone now.
...
**Responsive variant.** A modern responsive slot replaces `sizes[]` with min/max ranges — `min_width: 300, max_width: 970, min_height: 50, max_height: 250` — and accepts any dimensions within the box. Same multi-format pattern; different size declaration.

Below is a single-format/single-size narrowing for comparison (older fixed-MREC-only HTML5 product):
