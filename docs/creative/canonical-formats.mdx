...
> - **AdCP 3.2 is canonical-first end to end.** Exact plural `format_ids` fields are deprecated in 3.2 and removed in 4.0. Dual emission is limited to the 3.x migration window.
> - **Singular `format_id`, named-format definitions, and `list_creative_formats` are separate compatibility surfaces.** Their lifecycles are not implied by the 4.0 removal of exact plural `format_ids`.
...
A buyer reading a 3.x product on the named-format wire path sees `format_options` entries with `canonical_formats_only: true` absent from `format_ids`. This is intentional and not a producer error. Canonical-aware buyers reading `format_options` see them. Named-format-only buyers see fewer options on `format_ids` for the same product during the 3.x migration window; AdCP 4.0 removes exact plural `format_ids`.
...
**Canonical-first in 3.2.** New products author `format_options`. Sellers MAY dual-emit exact plural `format_ids` for 3.x compatibility, but MUST remove those fields for 4.0. Named-format definitions, singular `format_id`, and `list_creative_formats` follow separate schedules.

### Realistic 3.1 coverage

`v1-canonical-mapping.json` carries 7 structural fallbacks from 3.1 plus 22 non-wildcarded literals for observed legacy duration-, dimension-, VAST-token, and AAO-namespace static-display ids. The full v1 audit catalogued 86 formats across 12 platforms; ~76% of those (≈65 formats) fit existing canonicals structurally but **only project automatically when either (a) the seller adds an explicit `canonical` field to their v1 format file, or (b) someone files a registry PR adding the `format_id_glob` or structural match.** Through 3.x, buyers should dual-read while sellers move products to `format_options`; AdCP 4.0 consumers can rely on canonical product declarations because exact plural `format_ids` is no longer present.
