...
> - **9 of 14 canonical formats are non-experimental in 3.2** (`image`, `html5`, `display_tag`, `image_carousel`, `video_hosted`, `video_vast`, `audio_hosted`, `audio_daast`, `native_in_feed`). Five canonicals are experimental (`sponsored_placement`, `responsive_creative`, `agent_placement`, `multi_state_display`, `page_takeover`), and the `custom` escape-hatch is inherently experimental until a shape is promoted.
...
|---|---|
| **Canonical format** | One of 14 AdCP-defined format archetypes that products narrow (e.g., `image`, `video_vast`, `multi_state_display`, `page_takeover`). The buyer's stable validation target. |
...
| **`format_kind: "custom"`** | Adopter-defined shape that doesn't fit the 14 canonicals (branded content, AR lens, etc.). Requires `format_shape` (registry classifier) and `format_schema` (URI+digest reference to a fetchable schema). |
...
## The 14 canonical formats

Each canonical lives at `/schemas/formats/canonical/<name>.json`. Tracking model is **format-specific** (split by tracking model is why we have 14 instead of, say, 5).
...
| `multi_state_display` | Seller-rendered premium web unit with deterministic state × breakpoint canvases (expandable, adhesion, sticky, scroll-docking). No buyer tag. | Unit impression/click; sellers may additionally report by state, but 3.2 does not standardize state event names |
| `page_takeover` | Coordinated multi-placement ownership sold as one buy, with inline or sibling-referenced component formats and shared assets. | Component and takeover reporting are seller-defined in 3.2; no new event vocabulary is implied |

### Premium display in 3.2

Both premium-display canonicals are `experimental: true` in 3.2.

- Use `multi_state_display` only when the seller renders the unit and the buyer can know the exact state layouts before launch. The manifest carries exactly one `state_canvases` image for every declared state × breakpoint pair; each image binds with `state_id` and `breakpoint_id` and must match the declared dimensions.
- Use `responsive_creative` when the surface chooses or rearranges components per impression. The boundary is predictability: fixed templates are `multi_state_display`; algorithmic composition is `responsive_creative`.
...
- Use `page_takeover` when several placements are coordinated under one buy. Each component either declares one inline canonical or references a sibling `format_options[]` entry. `custom` and nested `page_takeover` components are forbidden. The manifest supplies component-specific slots under `component_assets.<component_id>` and supplies each declared shared slot once under top-level `assets`.

For Better Ads compatibility, fullscreen and overlay states SHOULD declare user-initiated triggers. The `trigger` field is the machine-readable answer to “does this auto-expand?” A seller can therefore reject or route an auto-expanding declaration without inferring behavior from prose.

The worked product declarations are [`streamhaus_expandable_sticky_leaderboard.json`](https://github.com/adcontextprotocol/adcp/blob/main/static/examples/products/canonical/streamhaus_expandable_sticky_leaderboard.json) and [`streamhaus_page_takeover.json`](https://github.com/adcontextprotocol/adcp/blob/main/static/examples/products/canonical/streamhaus_page_takeover.json). The first declares expanded 640/320×210 canvases that auto-collapse to 640/320×70 sticky canvases with an optional 15-second 16:9 video. The second references that leaderboard as a sibling component, adds a 2560×1440 skin with a safe area, and shares video/CTA assets where the consuming component accepts those slots.

A matching multi-state manifest binds every canvas explicitly:
...
For the takeover declaration, shared assets stay flat while component assets are namespaced:

```json test=false
{
  "format_kind": "page_takeover",
  "assets": {
    "video_main": { "asset_type": "video", "url": "https://cdn.streamhaus.example/spot.mp4", "duration_ms": 15000 },
  ...
  "component_assets": {
    ...
    "skin": {
      "image_main": { "asset_type": "image", "url": "https://cdn.streamhaus.example/skin.png", "width": 2560, "height": 1440 }
    }
  }
}
```
...
| `multi_state_display` | State/breakpoint canvas contract is new in 3.2 and needs cross-seller evidence | Two interoperable seller implementations plus manifest-validation evidence |
| `page_takeover` | Cross-placement composition, shared slots, and exclusivity need implementation evidence | Two interoperable sellers plus sibling-reference validation evidence |
...
`format_kind` names the creative contract shape — either one asset/asset pool or a bounded composition graph such as `page_takeover`. It describes what the buyer ships and what the surface accepts. It is not a delivery medium, measurement model, or targeting context. Conflating these is the most common architectural mistake a 3.2 contributor will be tempted to make. Four concrete examples:
...
1. **Creative contract** (image vs video vs html5 vs a bounded multi-component takeover) → `format_kind`.
...
Add a new `format_kind` only when the buyer-facing asset contract or bounded renderer assembly is structurally different. Broadcast TV, DOOH, and generative production stay on existing canonicals because channel and production source do not change that contract. `native_in_feed`, `multi_state_display`, and `page_takeover` clear the higher bar: each requires distinct assembly and validation logic that cannot be expressed as sibling refinement through `applies_to_channels`, `asset_source`, or `slots_override`. This is not precedent for every channel to request its own canonical.
...
## Custom formats — shapes the 14 canonicals don't cover

The 14 canonicals now include deterministic multi-state display and coordinated page takeovers. Custom remains the path for structures that have not been promoted, including branded content, cross-screen sponsorship, sponsorship lockup, newsletter sponsorship, AR lenses, playables, and live-event sponsorships.

These are real ad-industry product types with structures that have not yet converged enough for a shared canonical. AdCP handles them through a structured custom mechanism that buyer agents can reason about, not through free-form `ext`. Coordinated takeovers are no longer an example of this category in 3.2; they use `page_takeover`.
...
Same governance pattern that produced the original 12 canonicals from the v1 audit. The promotion queue lives at [adcp#3666](https://github.com/adcontextprotocol/adcp/issues/3666). AdCP 3.2 is the first promotion event: `multi_state_display` promotes its matching custom shape, while `page_takeover` promotes `multi_placement_takeover` and `roadblock`.
