...
| **`ProductFormatDeclaration`** | Inline format declaration: `format_kind` + `params` + optional `capability_id` + optional `applies_to_channels` + optional `experimental` + optional `v1_format_ref[]` (always array — see below). |
...
5. **Measurement / tracking / event model** (clicks vs impressions vs viewability vs conversion pixels) → `sync_event_sources` / `event_log` / `platform_extensions` (informational extension data).
6. **Targeting context** (audience vs geo vs daypart) → media-buy targeting overlay, not the format.

New `format_kind` only when the CREATIVE ASSET itself is structurally different (e.g., DAI's ad-stitched continuous audio stream is structurally different from `audio_hosted`'s file-per-impression). All 50 ad formats in the v1 catalog at GA project to canonicals via this rule; zero new canonicals were added for broadcast / DOOH / native / generative.
...
The Product on hand carries `format_options: [{ capability_id: "meta_reels" }]`. The buyer agent now knows the FULL declaration: `format_kind: video_hosted`, vertical 9:16, 3-90s duration, the full Meta CTA enum, plus the `v1_format_ref[]` array for v1-wire dual emission.
...
- **Generative-from-prompt** — same `format_kind` as the buyer-uploaded equivalent (image, video_hosted, audio_hosted) + `asset_source: agent_synthesized` + `slots_override` declaring the input shape (`generation_prompt: text`, `creative_brief: brief`). v2-aware buyers see "this format wants a text prompt, not image bytes."
- **Native** — `image` + `slots_override` covering native-specific slots (icon, disclosure, author, sponsored-by). `native_standard` and `native_content` catalog entries use this pattern. Richer native templates (TemplateCreative shape) deferred to 3.2 if they prove not to fit.
