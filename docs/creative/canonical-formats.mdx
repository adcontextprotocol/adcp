...
5. **Fail closed**: SDK MUST NOT synthesize a `format_options` entry. SDKs MUST surface the failure in a structured channel reachable by the calling agent — either a separate lint-output channel OR augmenting the response's `errors[]` array with `code: FORMAT_PROJECTION_FAILED`. NOT logger-only — a buyer that silently sees N fewer products has no remediation path. The advisory is non-fatal: the response stays 200/success, the product is still valid on the v1 path, only the v2 `format_options` projection is absent.
...
- SDKs SHOULD prefer `format_options` (canonical formats are the richer surface) and SHOULD surface the divergent product in a structured channel reachable by the calling agent (lint-output OR `errors[]` augmentation) with `code: FORMAT_DECLARATION_DIVERGENT`. NOT logger-only. Hard-failing the entire `get_products` response is discouraged — it punishes downstream buyers for a producer bug.
