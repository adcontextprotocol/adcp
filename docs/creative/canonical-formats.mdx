...
**AdCP 3.1 reference-catalog bridge.** The 3.1 reference catalog publishes concrete 2x companions for its seven fixed image formats: `display_300x250_image_2x`, `display_728x90_image_2x`, `display_320x50_image_2x`, `display_160x600_image_2x`, `display_336x280_image_2x`, `display_300x600_image_2x`, and `display_970x250_image_2x`. Each v1 definition keeps the original logical `renders[].dimensions`, requires an image exactly twice that width and height, and carries explicit `canonical_parameters` projecting to the logical canonical size plus `pixel_ratios: [2]`. SDKs supporting this bridge MUST honor `canonical_parameters` before inferring dimensions from the larger v1 asset. These aliases let 3.1 products use shared AAO format IDs instead of seller-custom formats.
