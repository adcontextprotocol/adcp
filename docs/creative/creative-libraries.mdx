...
|---|---|---|
| Account | Account (via [accounts protocol](/docs/accounts/overview)) | Advertiser in CM360, brand workspace in Celtra |
...
These lifecycles do not collapse into each other:

- A creative in the library has **zero or more** active assignments at any time.
- Rejecting or canceling a media buy releases its assignments. It does not change the creative's review state, remove the creative from the library, or affect the creative's use in other media buys.
- Re-reviewing a creative (seller pulls back an approval, seller approves a rejected creative) does not change existing assignments — sellers decide independently whether in-flight assignments continue serving based on the new state. See [creative review](/docs/creative/sales-agent-creative-capabilities#creative-review).
...
The account setup is the same whether the library is on a standalone creative agent or a sales agent. See [accounts protocol](/docs/accounts/overview) for details.

## Browsing creatives

Use [`list_creatives`](/docs/creative/task-reference/list_creatives) to browse the library. Filter by concept, format, status, tags, or date range:
...
The `status` field reflects the creative's current state in the library: `processing`, `pending_review`, `approved`, `rejected`, or `archived`. See [creative review](/docs/creative/sales-agent-creative-capabilities#creative-review) for how status transitions work.

## Uploading creatives

Use [`sync_creatives`](/docs/creative/task-reference/sync_creatives) to upload new creatives or update existing ones. The operation uses upsert semantics — if a `creative_id` already exists, it updates; otherwise it creates.
...
When you need a serving tag for a creative in the library, use [`build_creative`](/docs/creative/task-reference/build_creative) with `creative_id` instead of a manifest:
...
See [tag generation models](/docs/creative/implementing-creative-agents#tag-generation-models) for the full breakdown.
...
**Inline creatives follow the same library lifecycle as `sync_creatives` uploads.** The inline form is a convenience — "sync and assign in one call" — not a separate lifecycle. Once submitted, the creative enters the library with the same review flow, retention, and identifiers it would have under `sync_creatives`. If the media buy is rejected, canceled, or returns `pending_manual` and never activates, only the package assignments are released; the creatives remain in the library and may be referenced by `creative_id` in a subsequent `create_media_buy` call. Creative review proceeds independently of the media buy outcome — sellers MUST NOT short-circuit review based on a rejected buy, and a buy rejection does not imply rejection of the submitted creatives.
...
Call `sync_creatives` on each sales agent separately. Use the same `creative_id` and `concept_id` across sellers so you can correlate the same creative and campaign concept across your media buys. See [Multi-agent creative orchestration](/docs/creative/multi-agent-orchestration) for the full pattern.
...
**Library level**: The `status` field on each creative in [`list_creatives`](/docs/creative/task-reference/list_creatives) — `processing`, `pending_review`, `approved`, `rejected`, or `archived`.

**Package level**: The `approval_status` on each creative in [`get_media_buys`](/docs/media-buy/task-reference/get_media_buys) — `pending_review`, `approved`, or `rejected` with `rejection_reason`.
...
- [Generative creative](/docs/creative/generative-creative) — AI-powered creative generation and brief-in-media-buy workflows
...
- [Implementing creative agents](/docs/creative/implementing-creative-agents) — Building an AdCP creative agent around your platform
- [sync_creatives reference](/docs/creative/task-reference/sync_creatives) — Upload API details
- [list_creatives reference](/docs/creative/task-reference/list_creatives) — Query API details with full filtering options
