...
|---|---|---|---|---|
| **Retail-media deterministic, buyer-uploaded** | `deterministic` | `per_item` | `buyer_uploaded` | Amazon SP |
| **Retail-media network-composed** | `deterministic` | `per_item` | `seller_pre_rendered_from_brief` | Criteo SP, CitrusAd SP |
| **Collection-layout per impression** | `deterministic` (debatable) | `multi_item_in_creative` | `buyer_uploaded` | Pinterest Collection, Snap Collection |
| **Generative-per-SKU** | `deterministic` | `per_item` | `agent_synthesized` | Vendor-specific (Pencil, Persado, vertical retail-media platforms) |
