...
- v1 `list_creative_formats` tool — deprecated but functional through 4.x on its own compatibility schedule; its removal is not coupled to exact plural `format_ids`
...
Sellers MAY provide a server-side flatten wrapper that derives the v1 `list_creative_formats` shape from canonical product format declarations for callers that still use the task. The wrapper's lifecycle follows the task's separately documented schedule; it MUST NOT imply that exact plural `format_ids` remains valid in AdCP 4.0.
...
5. **Publish dual in 3.x**: add `format_options` while continuing to serve older buyers from the named-format model. Stop emitting exact plural `format_ids` before upgrading to AdCP 4.0.
...
7. **Deprecate timing**: exact plural `format_ids` is deprecated in 3.2 and removed in 4.0. Do not infer the lifecycle of singular `format_id`, named-format definitions, or `list_creative_formats` from that deadline.
...
| Drop `list_creative_formats` | Follow the task's separately documented compatibility schedule; this is not coupled to removal of exact plural `format_ids` in 4.0. |
...
Exact plural `format_ids` fields are deprecated in AdCP 3.2 and removed in AdCP 4.0. The `anyOf(format_ids, format_options)` shape on `Product` is a 3.x migration contract; 4.0 products use `format_options`, and requests use the corresponding canonical selectors and references.

This deadline applies only to fields named exactly `format_ids`. It does not remove singular `format_id`, named-format definitions, or `list_creative_formats`; those compatibility surfaces follow separately documented schedules.
