...
**Production source** — per-canonical `*_source` parameters. Describes **who renders the rendered asset, and when**:
- `audio_source` on `audio_hosted` — `buyer_uploaded | publisher_host_recorded | agent_synthesized`
- `image_source` on `image` — `buyer_uploaded | seller_pre_rendered_from_brief | seller_human_designed | agent_synthesized`
- `video_source` on `video_hosted` — same enum as `image_source`
