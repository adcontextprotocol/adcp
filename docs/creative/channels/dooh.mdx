...
description: "Canonical image, hosted-video, and audio contracts for digital inventory installed in physical venues."
"og:title": "AdCP — Digital Out-of-Home"
---

In AdCP, DOOH is digital advertising inventory delivered through a device installed in a physical out-of-home venue. The device can be a screen, an audio player, or another installed digital surface. The canonical format declares the creative medium: visual placements use `image` or `video_hosted`, while audio-only placements use `audio_hosted` or `audio_daast`.

Venue, installed-device identity, loop position, scheduling, and audience currency are inventory facts rather than creative format IDs. This delivery-context definition intentionally includes audio-only venue inventory even where another industry taxonomy uses DOOH only for screens or groups in-store audio under retail media. Streaming delivered to a listener's own device remains `streaming_audio`, and terrestrial broadcast remains `radio`.
...
## Audio-only DOOH in physical venues

An installed audio player in a tavern, grocery store, convenience store, gym, or similar venue is a DOOH property in AdCP. When sold as place-based out-of-home media, the product also uses the `dooh` channel to preserve that buying context; a retailer can instead sell the same property under `retail_media`. In both cases, `format_options` declares the accepted audio contract. Do not relabel the property as `streaming_audio` merely because the player receives its programming over an internet stream.
...
`audio_distribution_types` remains optional and describes the underlying audio feed when an existing value such as `music_streaming_service` or `web_radio` applies. It does not identify the installed venue context; `dooh` already does that. A sales agent representing several venue-network owners keeps each owner's `publisher_domain`, property catalog, and authorization distinct rather than inventing a network pseudo-publisher.
