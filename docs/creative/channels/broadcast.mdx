...
The :15 and :60 formats are identical except for `duration_seconds`. All three are available in the reference agent as `broadcast_spot_15s`, `broadcast_spot_30s`, and `broadcast_spot_60s`.
