...
- Agents SHOULD support filtering by `library_ids` and `format_ids` when the platform organizes creatives into libraries
- Agents MAY include dynamic content variable definitions when `include_variables=true`

**Account requirements:**
- Standalone creative agents (not also serving as sales agents) MUST implement the accounts protocol (`sync_accounts` / `list_accounts`) so buyers can establish and discover accounts before querying the library
- Sales agents that implement `list_creatives` already have accounts via the media-buy protocol
...
5. Support `creative_id` in `build_creative` when the agent hosts a creative library
6. Implement the accounts protocol (`sync_accounts` / `list_accounts`) when operating as a standalone creative agent (not also a sales agent)
