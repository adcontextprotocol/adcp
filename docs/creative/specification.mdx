...
- When the same relationship is visible through both reads, approval state, rejection reason, indicator types, scopes, detection times, evaluation coverage, and `indicators_as_of` MUST agree.
- Every indicator-capable seller accepts `indicators.changed` and may independently accept `creative.assignment_changed`, including when it is inline-only. These are prospective invalidations; receivers establish a complete all-status or known-ID baseline and repair through `get_media_buys` rather than treating payloads or the bounded reverse projection as complete relationship state.
