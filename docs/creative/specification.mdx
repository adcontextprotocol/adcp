...
`CreativeAsset.localization` makes every translation request explicit:
...
localized sync and every later `list_creatives` read preserve the buyer's
`locale_variant_id` and add the provider's `provider_variant_id`.
...
locale set; omission leaves localization unchanged; `localization: null`
removes it. `delete_missing` still operates only at creative scope: archiving a
missing creative archives all of its locale variants atomically, and it never
compares locale arrays between otherwise-present creatives.
