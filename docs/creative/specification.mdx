...
it only when `get_adcp_capabilities.creative.localization` is present. That
capability returns the complete BCP 47 locale set accepted for the current
caller, accepted translation modes, the target-variant ceiling, and whether
provider review is per variant or creative-wide. Because exact
`list_creatives` readback is part of the contract, advertising localization
also requires `creative.has_creative_library: true`.

`sync_creatives.creatives[].localization` makes every translation request
explicit. It is the only localization write surface. Inline creatives on
...
- The creative's top-level `assets` are the source/default variant.
- `localization.source` binds those assets to a canonical BCP 47 tag and a
  buyer-assigned `locale_variant_id`.
- Every `target_variants[]` entry has a unique locale and stable
  `locale_variant_id`.
- `translation_mode: buyer_supplied` carries locale-specific asset overrides;
  missing slots inherit source assets.
- `translation_mode: provider_generated` carries no override assets and is the
  only way to ask the provider to translate from source. A source asset alone
  never implies translation.
...
`zh-Hant-TW`). A receiver MUST reject duplicate, non-canonical, unsupported, or
lossily mapped locales before any provider or library mutation. After source
inheritance or provider generation, each fully resolved variant MUST satisfy
the selected creative format.
...
used for a provider's served execution in `get_creative_delivery`. A successful
localized sync and every later `list_creatives` read MUST preserve the exact
source ID/locale and exact target ID/locale/translation-mode set from the
request. Readback adds a `provider_variant_id` to each variant. The single
provider creative identity is the enclosing creative's top-level `platform_id`;
it is never duplicated inside `localization`.

Localized readback is atomic. The seller returns complete resolved assets,
provider creative/variant identities, review `status`, and `launch_status` for
the source and every target. If any element is unavailable or cannot be mapped
exactly, the seller MUST fail the creative read rather than omit the locale,
substitute the source language, or return a partial localization object.
`launch_status: ready` reports provider eligibility; it does not authorize
buyer spend. Lifecycle pairs are closed: `processing` and `pending_review` map
to `pending`, `approved` maps to `ready`, and `suspended`, `rejected`, and
`archived` map to `blocked`. A blocked variant includes at least one
machine-readable `launch_blockers` entry.

The creative-level `status` is deterministic. `archived` applies only when the
whole creative is archived, in which case every locale variant is archived.
Otherwise the seller selects the first state present in this precedence order:
`rejected`, `suspended`, `pending_review`, `processing`, `approved`. Therefore
the creative is `approved` only when every locale variant is approved. When
`review_scope: creative`, every locale MUST echo the same provider-backed
status and launch state.

On upsert, a non-null `localization` object replaces the creative's complete
locale set and `localization: null` removes it. Omission preserves existing
localization only when the top-level source assets exactly equal the prior
localized source assets. If source assets change, the buyer MUST send the full
replacement topology or explicit removal; the seller rejects an ambiguous
omission before mutation. `delete_missing` still operates only at creative
scope: archiving a missing creative archives all of its locale variants
atomically, and it never compares locale arrays between otherwise-present
creatives. Partial archive state is invalid.
