...
Locale tags MUST use the canonical representation from
[RFC 5646](https://www.rfc-editor.org/rfc/rfc5646): lower-case language and
variants, title-case script, and upper-case region (for example `en-US` and
`zh-Hant-TW`). A receiver MUST reject malformed, duplicate, non-canonical,
unsupported, or lossily mapped locales before mutating seller systems or the
creative library.
