...
A transformation or generation agent that charges for its services becomes stateful: it persists `creative_id` on build output, implements the Accounts Protocol, and returns pricing in `build_creative` responses. Free transformation agents remain stateless and unchanged.
