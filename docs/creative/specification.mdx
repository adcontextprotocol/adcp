...
- Sales agents declaring `list_creatives` in `media_buy.relationship_notifications.projection_tasks` MUST scope creative insights to `assignments.assigned_packages[].insights[]`; they MUST NOT place package- or publisher-specific fatigue on the creative as a global property.
