...
unsupported, or lossily mapped locales before any provider or library mutation.
...
preferences and the creative's available locale tags. Lookup progressively
truncates subtags until it finds a match. If one truncated range matches more
than one available tag, the seller chooses the first variant in source-then-
request-target order; sellers preserve target order on readback. When no match
exists, the seller MUST apply `unmatched_locale_action`; it MUST NOT silently
substitute the source.
Every delivered execution of a localized creative includes the selected
`locale_variant_id` in `get_creative_delivery`, including default fallback.
...
the request. Readback adds a `provider_variant_id` to each variant. The single
...
provider creative/variant identities, default policy, and matching mode for the
...
provider variants MUST NOT become visible. `localization: null` removes the
...
Capabilities govern new writes, not stored-data readability. If a seller later
withdraws localization support or stops accepting a locale/format pair, it
MUST keep existing localized creatives readable. A dependency loss that makes
an approved creative unsafe to serve moves the whole creative to `suspended`
and uses the normal lifecycle webhook. Sellers may reject new or replacement
localization, but MUST still allow explicit `localization: null` cleanup.
