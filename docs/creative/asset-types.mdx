...
| `impression_tracker`, `click_tracker`, `viewability_tracker`, `third_party_tracker` | `tracker_pixel` |

If neither `url_type` nor a format-side `role` is available, receivers MAY reject the manifest. Receivers MUST NOT silently pick a mechanism — firing a clickthrough URL as a pixel (or vice versa) is a measurement and user-experience hazard.
