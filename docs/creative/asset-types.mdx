...
- `tracking_events`: Array of supported tracking events. Valid values are defined by the [VAST Tracking Event](/schemas/v3/enums/vast-tracking-event.json) enum, aligned to the IAB VAST 4.2 specification:
...
- `tracking_events`: Array of supported tracking events. Valid values are defined by the [DAAST Tracking Event](/schemas/v3/enums/daast-tracking-event.json) enum:
  - **Playback**: `impression`, `start`, `firstQuartile`, `midpoint`, `thirdQuartile`, `complete`
