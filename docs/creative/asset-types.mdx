...
- `target`: Which VAST creative element scopes this tracker — `linear` (default), `non_linear`, or `companion`. VAST places `<TrackingEvents>` under three different parents (`<Linear>`, `<NonLinearAds>`, `<CompanionAds>/<Companion>`) with different valid event sets: `acceptInvitation` is non-linear, `creativeView` is the only companion event in the closed AdCP matrix, and `closeLinear` is linear. Sales agents read this to place the tracker correctly during VAST assembly.
