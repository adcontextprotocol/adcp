...
- `tracking_events`: Array of supported tracking events. Valid values are defined by the [VAST Tracking Event](https://adcontextprotocol.org/schemas/v3/enums/vast-tracking-event.json) enum, aligned to the IAB VAST 4.2 specification:
  - **Playback**: `impression`, `start`, `firstQuartile`, `midpoint`, `thirdQuartile`, `complete`
  - **Interaction**: `mute`, `unmute`, `pause`, `resume`, `rewind`, `skip`, `playerExpand`, `playerCollapse`, `fullscreen`, `exitFullscreen`
  ...
  - **Errors & verification**: `error`, `notViewable`, `viewable`, `measurableImpression`, `viewableImpression`
...
- `tracking_events`: Array of supported tracking events. Valid values are defined by the [DAAST Tracking Event](https://adcontextprotocol.org/schemas/v3/enums/daast-tracking-event.json) enum:
  - **Playback**: `impression`, `start`, `firstQuartile`, `midpoint`, `thirdQuartile`, `complete`
  - **Interaction**: `mute`, `unmute`, `pause`, `resume`, `skip`
  - **Progress**: `progress`
  - **Click & close**: `clickTracking`, `customClick`, `close`
  - **Errors & verification**: `error`, `measurableImpression`, `viewableImpression`
