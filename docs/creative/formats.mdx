...
Named `format_id` objects and `list_creative_formats` are deprecated compatibility surfaces. New implementations use the model below. Exact plural `format_ids` fields are removed in AdCP 4.0; the singular field and task follow their separately documented schedules.
