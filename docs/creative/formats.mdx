...
Because the capability entry includes `preview`, the enclosing capability response also lists `streamhaus_vertical_video_builder` in `creative.preview.supported_capability_ids` and declares `fidelity`. Capability IDs route operations; they do not make the preview authoritative.
...
- [Canonical formats specification](/docs/creative/canonical-formats)
