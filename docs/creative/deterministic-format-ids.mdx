...
**Fields:**
- `duration_ms` (number, minimum: 0) - Duration in milliseconds
...
### Generative Formats with Template Outputs

Generative formats can produce multiple deterministic outputs using `output_format_ids`:
...
```json
{
  ...
  "name": "AI-Generated Display Ads",
  "type": "generative",
  "output_format_ids": [
    {"agent_url": "...", "id": "display_static", "width": 300, "height": 250, "unit": "px"},
    {"agent_url": "...", "id": "display_static", "width": 728, "height": 90, "unit": "px"},
    {"agent_url": "...", "id": "display_static", "width": 160, "height": 600, "unit": "px"}
  ]
}
```

**Key principle**: Output format_ids are ALWAYS deterministic (include dimensions). The generative format itself may accept parameters, but outputs must specify exact dimensions for each variant produced.

## Discovery Pattern

### Creative Agent: Returns Templates

`list_creative_formats()` returns template format definitions:
...
### Sales Agent: Returns Concrete Instances

`get_products()` returns products with placement-specific format constraints:
...
**Flow**:
1. Buyer fetches formats from creative agent → learns `display_static` accepts dimensions
2. Buyer fetches products from sales agent → learns which dimensions are supported (300x250, 728x90)
3. Buyer creates creative with deterministic format_id matching a supported placement dimension
...
- When width/height are present, `unit` is REQUIRED (not optional)

**Numeric precision:**
- Numbers are compared as JSON numbers (300.0 equals 300)
- For pixels, use integers (300px not 300.5px)
- For physical units (inches, cm), decimals are allowed (0.5in, 7.62cm)
...
**Unit conversion:**
- Format IDs are compared WITHOUT unit conversion
- `300px` is NOT equal to `7.62cm` even if physically equivalent
- Unit field is part of identity, not just metadata
...
{"agent_url": "...", "id": "display_static", "width": 300}  // ❌ Missing height and unit
```
...
{"format_id": {"agent_url": "...", "id": "video_hosted", "duration_ms": 30000}}                 // ❌ Different format
```
