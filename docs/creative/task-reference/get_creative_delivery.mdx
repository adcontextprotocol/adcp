...
| `reach_window` | Window semantics for reach/frequency — object with `kind` (`cumulative` for rolling-since-campaign-start, `period` for snapshot-only) and optional `period` (Duration). Optional but strongly recommended whenever `reach` is present |
| `frequency` | Average frequency per `reach_unit`, measured over `reach_window` |
| `viewed_seconds` | Average seconds in view per impression — pairs with the `viewed_seconds` optimization goal |
| `attention_seconds` | Average vendor-attributed attention time per impression — pairs with the `attention_seconds` optimization goal |
| `attention_score` | Vendor-specific attention score per impression — pairs with the `attention_score` optimization goal |
