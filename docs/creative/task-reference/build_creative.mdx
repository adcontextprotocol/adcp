...
Compliance requirements differ by region: the US requires SEC-mandated disclosures while the UK requires FCA-mandated risk warnings. The third disclosure (no `regions`) applies globally. The `prohibited_claims` array tells the creative agent which claims to avoid in generated copy.
