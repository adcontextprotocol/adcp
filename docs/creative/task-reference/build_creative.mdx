...
- **variants[].eval**: Present only when the request supplied an advisory [`evaluator`](#request-parameters) and the agent advertises `creative.supports_evaluator`. The rank-side of the [`get_creative_features`](/docs/governance/creative/get_creative_features) feature oracle: `eval.features[]` carries the creative-feature values this leaf was measured on (same shape `get_creative_features` returns, e.g. a `creative_quality_score`), and `recommended`/`rank` are a preference over those values (`evaluator.rank_by`). It is a feature measurement, not a pass/fail verdict, and never gates which leaves are produced or billed; `eval.calls_used` / `eval.seconds_used` report external judge usage when `agent_url` was used.
