...
To refine, pass the previous response's `creative_manifest` back as input with a new `message`. Alternatively, update the `creative_brief` to change the creative direction — the brief is the buyer-owned source of truth for what the creative should be.
