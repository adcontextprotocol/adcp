...
For information about format IDs and how to reference formats, see [Creative Formats - Referencing Formats](/docs/creative/formats#referencing-formats).
...
<Note>
**Cross-agent workflow**: When the creative agent and sales agent are different systems, use `build_creative` on the creative agent to produce a manifest with tags, then [`sync_creatives`](/docs/creative/task-reference/sync_creatives) on the sales agent to upload it. For example: `list_creatives` on Flashtalking → `build_creative` on Flashtalking → `sync_creatives` on the DSP.
</Note>

### Multi-format generation

Generate creatives for multiple formats in a single call using `target_format_ids`. The creative agent produces one manifest per format from the same source assets and brief:
