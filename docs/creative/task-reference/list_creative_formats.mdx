...
| What formats can this sales agent deliver for a product? | [`get_products`](/docs/media-buy/task-reference/get_products) → `Product.format_options[]` |
| What can this creative agent build, validate, or preview? | [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities) → `creative.supported_formats[]` |
...
```json
{
  "creative": {
    ...
    "preview": {
      "supported_capability_ids": ["streamhaus_vertical_video"],
...
The buyer passes `streamhaus_vertical_video` as `target_capability_id` to [`build_creative`](/docs/creative/task-reference/build_creative). The returned manifest carries canonical `format_kind: "video_hosted"`; capability IDs never appear on products or manifests.
