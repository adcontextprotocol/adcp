...
Existing 3.x agents MAY continue serving the task for compatibility. New implementations MUST use the canonical surfaces below and SHOULD derive any legacy response mechanically from canonical declarations rather than maintaining a second catalog.

## Use the surface that owns the fact

| Question | Canonical source |
|---|---|
| What formats does a publisher declare? | The publisher's `/.well-known/adagents.json` `formats[]`, with AAO community-mirror fallback when the publisher has not adopted |
| What formats can this sales agent deliver for a product? | `get_products` → `Product.format_options[]` |
| What can this creative agent build, validate, or preview? | `get_adcp_capabilities` → `creative.supported_formats[]` |
...
The buyer passes `streamhaus_vertical_video` as `target_capability_id` to `build_creative`. The returned manifest carries canonical `format_kind: "video_hosted"`; capability IDs never appear on products or manifests.
...
- [`/schemas/v3/media-buy/list-creative-formats-request.json`](https://adcontextprotocol.org/schemas/v3/media-buy/list-creative-formats-request.json)
- [`/schemas/v3/media-buy/list-creative-formats-response.json`](https://adcontextprotocol.org/schemas/v3/media-buy/list-creative-formats-response.json)
...
See [Canonical formats](/docs/creative/canonical-formats) for the complete matching and ownership model.
