...
Formats can declare what catalog types they need via the `catalog_requirements` array. This tells buyers which catalogs to sync before submitting creatives for that format.
...
When `catalog_requirements` is present, buyers should sync the required catalogs via [`sync_catalogs`](/docs/media-buy/task-reference/sync_catalogs) before submitting creatives. See [Catalogs](/docs/creative/catalogs) for the full lifecycle.
