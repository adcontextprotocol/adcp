...
The task remains functional through 4.x as a named-format compatibility surface and follows its own removal schedule. Fields named exactly `format_ids` are deprecated in 3.2 and removed in 4.0; that deadline does not itself remove this task.

This deprecation does **not** affect [`list_creatives`](/docs/creative/task-reference/list_creatives), which remains the creative-library query task. Singular `format_id` and named-format definitions follow separate schedules from exact plural `format_ids`.
</Warning>
