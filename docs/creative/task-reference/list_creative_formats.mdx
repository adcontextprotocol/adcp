...
| `asset_types` | string[] | No | Filter to formats accepting these asset types: `image`, `video`, `audio`, `text`, `html`, `javascript`, `url`, `catalog`, `published_post`. Uses OR logic. **Recommended over `type` filter.** |
