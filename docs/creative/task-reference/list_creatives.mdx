...
request's exact source ID/locale, target ID/locale set, default ID, and unmatched
action. Each entry returns complete resolved assets and provider variant
identity. The top-level creative `status` is the single review lifecycle for
...
and equality between source readback assets and top-level creative assets.
...
At delivery, sellers apply RFC 4647 Lookup. When no variant matches, they follow
`unmatched_locale_action`: serve `default_locale_variant_id` or do not serve.
