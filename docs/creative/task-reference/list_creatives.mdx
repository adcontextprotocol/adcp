...
| `insight_types` | `InsightType[]` | Filter for creatives with at least one package assignment carrying any requested current insight type. See the [standard insight catalog](/docs/media-buy/media-buys/insights#standard-insight-types). * |
...
**To subscribe**, register a [`notification_configs[]`](/docs/accounts/tasks/sync_accounts#account-level-webhook-subscriptions) entry through `sync_accounts`. All insight-capable sellers accept `insights.changed` and may independently advertise `creative.assignment_changed`. Subscriptions are prospective, so establish a complete all-status or known-ID baseline through `get_media_buys` after activation. Dedupe later invalidations and reread `get_media_buys` rather than applying payload state; this bounded reverse projection is useful for discovery but cannot prove complete repair.
