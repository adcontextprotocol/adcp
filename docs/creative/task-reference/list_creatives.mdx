...
- Find creatives with seller insights such as package-scoped creative fatigue
...
| `assignment_projection` | string | No | `all` (default) returns active assignments; `matching` returns only assignments matching `filters.insight_types` and requires that filter. |
...
| `fields` | array | No | Specific fields to return (omit for all fields). Sparse responses may contain only the requested fields plus `creative_id`. Includes canonical format selectors, `"assignments"`, `"assets"`, `"pricing_options"`, and `"rights"` / `"rights_attestation_evaluations"`; selecting rights evaluations automatically includes `creative_id` and `rights`. |
...
| `insight_types` | [InsightType](https://adcontextprotocol.org/schemas/latest/enums/insight-type.json)[] | Filter for creatives with at least one package assignment carrying any requested current insight type * |
...
Creative-library sellers that advertise `list_creatives` in `media_buy.insight_notifications.repair_tasks` attach evaluated insight state to `assignments.assigned_packages[]` and include `media_buy_id` plus `approval_status` on every row, including unknown rows. These fields mirror `get_media_buys` for the same relationship. A mixed publisher/placement outcome uses `approval_status: "partially_approved"` plus complete `approval_scopes[]`; the assignment webhook remains a compact invalidation and buyers reread these scopes. Each evaluated row includes `insight_types_evaluated`, `insights_as_of`, and `insights[]`; omitted types remain unknown.
...
This request produces a genuinely sparse response: each creative may contain only `creative_id` and `assignments`. `assignment_count` remains the total active relationship count; `returned_assignment_count`, optional `matching_assignment_count`, and `assignments_truncated` describe the bounded nested projection. When truncated, use `get_media_buys` as the complete repair path.

The standard insight shape intentionally contains no insight ID, source envelope, lifecycle status, or sub-version. The responding seller is the source; AdCP defines the broad meaning of the type for the negotiated protocol release. Seller-specific scores, methodology, evaluation windows, and upstream attribution belong in `ext`.

Omitted `insights` means unknown. An empty array is evaluated-clear only for `insight_types_evaluated` and declared coverage. Do not infer clearing from filtered/paginated disappearance; directly reread without `insight_types`. See [Insights, Warnings, and Recommendations](/docs/media-buy/media-buys/insights).
...
Hard-purged creatives (`purge_kind: hard`, used for legal erasure under GDPR Article 17 / CCPA / equivalent) retain no tombstone; the [`creative.purged`](https://adcontextprotocol.org/schemas/v3/creative/creative-purged-webhook.json) webhook is the only signal. See [snapshot-and-log § Rule 4 carve-out](/docs/protocol/snapshot-and-log#account-level-adopters-3-1) for the rationale.

### Webhook activity

When `include_webhook_activity: true`, each creative carries recent `creative.status_changed`, `creative.purged`, `creative.assignment_changed`, and assignment-level `insights.changed` attempts. This is the buyer's debug surface for delivery and retry behavior. See [Webhook activity log pattern](/docs/protocol/snapshot-and-log#webhook-activity-log-pattern).

**To subscribe**, register a [`notification_configs[]`](/docs/accounts/tasks/sync_accounts#account-level-webhook-subscriptions) entry through `sync_accounts`. All insight-capable sellers accept `insights.changed`; sellers listing this task as a repair path additionally accept `creative.assignment_changed`. Subscriptions are prospective, so establish an unfiltered baseline after activation. Dedupe later invalidations and reread rather than applying payload state.
