...
For a localized creative, `localization` is the authoritative round trip. It
contains the stable provider creative ID, declares whether review is
per-variant or creative-wide, and lists exactly one source plus every target
variant. Each entry preserves the buyer's `locale_variant_id` and returns the
canonical locale, complete resolved assets, provider variant identity, review
status, and launch status.
...
creative is therefore approved only when every locale is approved.
