...
| `fields` | array | No | Specific fields to return (omit for all fields). Includes `"assets"` for local fallback filtering, `"pricing_options"` for sparse selection, and `"rights"` / `"rights_attestation_evaluations"` for rights readback. Selecting evaluations automatically includes `creative_id` and `rights`. |
...
**To subscribe** to these fires, register a [`notification_configs[]`](/docs/accounts/tasks/sync_accounts#account-level-webhook-subscriptions) entry on the account via [`sync_accounts`](/docs/accounts/tasks/sync_accounts). The seller fires per-subscriber against each entry whose `event_types[]` includes the type. `webhook_activity[]` on this read is the buyer-side debug log of what actually fired.
