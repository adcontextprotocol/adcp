...
| `fields` | array | No | Specific fields to return (omit for all fields). Sparse responses may contain only the requested fields plus `creative_id`; both `format_id` and `format_kind` are selectable. |
...
- [`list_creative_formats`](/docs/creative/task-reference/list_creative_formats) - Deprecated v1 format discovery (prefer `format_options[]` / `creative.supported_formats`; functional through 4.x)
