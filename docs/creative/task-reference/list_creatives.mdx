...
- Sort by creation date, update date, name, status, or performance metrics
- Cursor-based pagination for large libraries
- Optionally include assignments, performance data, sub-assets, and dynamic creative optimization (DCO) variables
...
| `variables` | array | Dynamic creative optimization (DCO) variable definitions (when `include_variables=true`) |
...
<Note>
Standalone creative agents (ad servers, creative management platforms that are not also sales agents) MUST implement the [accounts protocol](/docs/accounts/overview) (`sync_accounts` / `list_accounts`) so buyers can establish and discover accounts before querying the library. Sales agents that implement `list_creatives` already have accounts established via the media-buy protocol.
</Note>
