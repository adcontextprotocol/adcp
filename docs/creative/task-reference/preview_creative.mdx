...
| `requests` | array | Batch | Array of 1-50 preview requests. Each item accepts a canonical `creative_manifest`, inputs, quality, output format, item limit, and template ID. |
...
If the requested quality level is not supported, the agent renders at the best quality it can provide. The protocol does not require agents to support both levels. There is no response field echoing the actual quality used, so if quality accuracy matters, verify by inspection and consult the agent's `get_adcp_capabilities` declaration.
