...
`preview_creative` is synchronous by default. A buyer that can poll task results MAY set `allow_async: true`. Only then may an agent queue slow rendering and return:
...
The buyer polls `get_task_status` with `task_id`; the completed task result contains the normal single, batch, or variant preview response. `response_type: "submitted"` keeps this fourth arm in the same discriminator family as the three synchronous preview arms; `status: "submitted"` is the task-lifecycle signal. When `allow_async` is false or absent, agents MUST NOT return the submitted shape. `build_creative` already has a separate async response contract and is unchanged by this field.
