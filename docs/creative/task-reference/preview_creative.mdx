...
### Preview from build_creative

After calling `build_creative`, preview the output using `generated_creative_ref` instead of resending the manifest:
...
| `generated_creative_ref` | string | Conditional | Ephemeral reference from a `build_creative` response. Required unless `creative_manifest` is provided. |
| `creative_manifest` | object | Conditional | Complete creative manifest. Required unless `generated_creative_ref` is provided. |
...
| `GENERATED_CREATIVE_REF_EXPIRED` | The `generated_creative_ref` has expired. Re-submit with `creative_manifest` instead |
