...
The buyer polls `get_task_status` with `task_id`; the completed task result contains the normal single, batch, or variant preview response. When `allow_async` is false or absent, agents MUST NOT return the submitted shape. `build_creative` already has a separate async response contract and is unchanged by this field.
