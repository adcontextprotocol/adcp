...
Preview what a specific variant looked like when served. Use `variant_id` from [`get_creative_delivery`](/docs/creative/task-reference/get_creative_delivery) response:
...
| `variant_id` | string | Variant | Platform-assigned variant identifier from [`get_creative_delivery`](/docs/creative/task-reference/get_creative_delivery). |
...
Discover renderers from `get_adcp_capabilities.creative.supported_formats[]` by selecting entries whose `operations` contains `preview` and whose canonical `format` satisfies the manifest. New 3.2 producers MUST list those routes in `creative.preview.supported_capability_ids` and declare `creative.preview.fidelity` as `authoritative` or `representative`. The listed IDs equal the set of `supported_formats[]` capability IDs whose operations include `preview`, so every explicit route has one fidelity classification. `capability_id` is an agent-local renderer route; it never belongs in the portable manifest. If multiple renderers match, the caller must select one explicitly.

`authoritative` is reserved for the serving platform's own preview machinery, including a seller proxying a first-party platform preview API. `representative` identifies an approximation. Buyers MUST preserve this label and follow the [rendering authority and fallback order](/docs/creative/canonical-formats#rendering-authority-and-fallback-order).
...
After the campaign runs, use variant mode to see exactly what was served. Pass a `variant_id` from [`get_creative_delivery`](/docs/creative/task-reference/get_creative_delivery):
...
Quality and authority are independent. `quality_used: "production"` means full fidelity within the selected renderer; it does not turn `creative.preview.fidelity: "representative"` into serving-platform truth. Buyer tooling MUST retain both values when it records or presents a preview.

Each rendered piece may include `renderer` audit metadata: a stable `renderer_id`, exact semantic `version`, export name, renderer-declared `fidelity`, and whether delivery tracking was suppressed. This metadata makes a render reproducible but does not grant authority. Buyers still resolve authority from capability discovery and any publisher placement `preview_provider` delegation.
