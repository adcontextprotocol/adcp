...
| `localization`      | object \| null       | No          | Explicit source and target locale variants. Requires `get_adcp_capabilities.creative.localization`. An object replaces the complete locale set, omission leaves it unchanged, and null removes it. |
...
Accepted localized results include `localization` with the provider creative
identity and a complete source-plus-target variant list. Each variant preserves
`locale_variant_id` and returns complete resolved assets,
...
locale variant is archived atomically; it never treats omitted target locales
as missing creatives. A non-null `localization` object on an upsert replaces
the locale set, omission leaves it unchanged, and null removes it. The normal
active-delivery update protection applies to all three operations.
...
- `localization` - Exact provider-backed source/target locale state. Required on every accepted localized item, omitted on failed/deleted/unlocalized items. The creative-level status aggregates locale states in this precedence: `rejected`, `suspended`, `pending_review`, `processing`, `approved`.
