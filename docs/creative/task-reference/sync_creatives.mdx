...
When a truncated range matches multiple available tags, source then request
target order is the deterministic tie-break; readback preserves target order.
If lookup finds no available locale, it follows `unmatched_locale_action`:
serve `default_locale_variant_id` or make the creative ineligible. It never
silently assumes the source is the default.
...
variant list. The response preserves the request's exact source ID/locale,
target ID/locale set, default ID, and unmatched action. Each variant returns
complete resolved assets and `provider_variant_id`. The enclosing creative's
