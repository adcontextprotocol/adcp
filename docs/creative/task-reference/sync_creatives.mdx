...
| `dry_run`         | boolean    | No       | When true, rehearse this exact sync operation without applying it (default: false). Use this for seller trafficking acceptance checks; use `validate_input` for manifest-only preflight against canonical/product format targets. |
...
`get_adcp_capabilities.creative.localization` block names the exact BCP 47
locales and translation modes it currently accepts. A seller MUST reject a
locale outside that list, a duplicate target locale/ID, a target locale/ID that
reuses the source value, a missing buyer translation, or a lossy provider
mapping before provider or library mutation. JSON Schema `uniqueItems` compares
whole objects and is not sufficient for those property-level rules; verifiers
consume the schema's `x-adcp-validation` constraints.
        ...
        "target_variants": [
          {
            ...
            "assets": {
              "image": {
                "asset_type": "image",
                "url": "https://cdn.nova.example/summer-es.jpg",
                "width": 1080,
                "height": 1080
              },
              "headline": {
...
The source assets are the default. A buyer-supplied target contains only its
locale-specific overrides; missing slots inherit source. A
`provider_generated` target contains no `assets` and is the explicit request
for native provider translation. Source assets alone never request
translation.

Accepted localized results include a top-level `platform_id` as the single
provider creative identity and `localization` as a complete source-plus-target
variant list. The response preserves the request's exact source ID/locale and
exact target ID/locale/translation-mode set. Each variant returns complete resolved assets,
`provider_variant_id`, review `status`, and `launch_status`. If the provider
cannot read back any one of those values exactly, the item fails rather than
silently falling back to the source language.

Every accepted localized result (`created`, `updated`, or `unchanged`) MUST
carry top-level aggregate `status`, top-level `platform_id`, and complete
`localization`; `failed` and `deleted` results MUST omit localization. A later
`list_creatives` read preserves the same top-level `platform_id` and exact
source/target identities.

Variant lifecycle pairs are strict: `processing`/`pending_review` require
`launch_status: pending`, `approved` requires `ready`, and
`suspended`/`rejected`/`archived` require `blocked` plus at least one
machine-readable launch blocker.

`delete_missing` remains creative-scoped. If it archives a creative, every
locale variant is archived atomically; partial archive state is invalid, and
it never treats omitted target locales as missing creatives. A non-null
`localization` object on an upsert replaces the locale set and null removes it.
Omission preserves localization only when top-level source assets exactly equal
the prior localized source assets. A source asset change with localization
omitted is rejected before mutation; send the full replacement topology or
null. The normal active-delivery update protection applies to all three
operations.
...
- `localization` - Exact provider-backed source/target locale state. Required with top-level `status` and `platform_id` on every accepted localized item, omitted on failed/deleted/unlocalized items. The creative-level status aggregates locale states in this precedence: `rejected`, `suspended`, `pending_review`, `processing`, `approved`; `archived` is valid only when every variant is archived.
