...
reference before provider or library mutation. JSON Schema `uniqueItems`
...
creative ID assigned by the seller's execution platform (for example, its ad
server or decisioning platform) and `localization` as a complete
source-plus-target variant list. The response preserves the request's exact source ID/locale,
target ID/locale set, fallback-rule set, default ID, and unmatched action. Each
variant returns complete resolved assets and `provider_variant_id`. The enclosing creative's
single `status` governs the complete set. If the provider cannot read back any
one of those values exactly, the item fails rather than silently falling back
...
failure leaves the prior state unchanged and must not expose provider orphans.
...
- `localization` - Exact provider-backed materialized source/target locale state. Required with top-level `status` and `platform_id` on every accepted localized item, omitted on failed/deleted/unlocalized items. The top-level status is the single creative-wide review state.
...
| `CREATIVE_LOCALE_NOT_ACCEPTED` | Assigned creative has no materialized variant accepted by the package's effective format `locale_policy`, lacks locale topology, or uses an ineligible `serve_default` | Supply a matching source/target variant, choose a compatible format option, or change the default policy |
