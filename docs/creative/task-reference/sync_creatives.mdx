...
For published-post reference products, the asset role is usually `published_post` and the payload contains a post URL or platform post ID instead of uploaded media bytes. If the seller can resolve the post but lacks a required downstream platform connection, such as the publisher identity that owns the post, the terminal error is `AUTHORIZATION_REQUIRED`. New implementations should include `error.details.missing_connections[]` so the caller can send a human through the correct connections flow.
