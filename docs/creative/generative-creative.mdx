...
Once you've built and previewed your creative, attach it to a media buy. Pass the manifest from `build_creative` as a creative in `create_media_buy`:
...
- **User data in transcripts**: Variant manifests from `get_creative_delivery` may include user messages. Agents that declare `supports_compliance: true` apply data handling policies (anonymization, retention limits) to transcript data. For regulated industries, confirm the agent's data handling practices before launch.
