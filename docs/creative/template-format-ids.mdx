...
description: "Template format IDs in AdCP let a single format definition support many dimension variants without creating separate formats for each size."
"og:title": "AdCP — Template Format IDs"
---


Template formats allow a single format definition to support multiple dimension, pixel-density, or duration variants without creating separate format definitions for each variant. This eliminates format explosion when publishers support many similar variants.
...
A single template format definition (`display_image`) accepts logical dimension and pixel-density fields in the format_id object, allowing creatives to specify an exact 1x, 2x, or other density variant without a retina-specific format name.

The legacy format model did not standardize `format_id.pixel_ratio` or canonical pixel-density fields. The unversioned reference catalog therefore provides distinct concrete contracts for the seven standard fixed display sizes: the existing `_image` ID is 1x-only, `_image_2x` is 2x-only, and `_image_1x_2x` requires both assets for systems such as GAM. Additive IDs become discoverable to dynamically querying clients across 3.x when the catalog deploys; no 3.0.x or 3.1.x protocol backport is required. Their density and required-coverage values under `canonical_parameters` are opaque forward-projection metadata for 3.2-aware SDKs, not pre-3.2 canonical semantics. The parameterized examples below are the 3.2 replacement and avoid adding another concrete ID for every future size.
...
2. **Template formats** - Accept parameters in format_id
   ...
   - Example: `display_image` can be any logical dimensions and pixel density
...
3. **Parameterized format_id** - Template format with parameters
   - Example: `{id: "display_image", width: 300, height: 250, pixel_ratio: 2}`
...
**Key fields:**
- `accepts_parameters: ["dimensions", "pixel_ratio"]` - Format accepts logical width/height and optional intrinsic-pixel density in format_id
- `renders[].parameters_from_format_id: true` - Logical render dimensions come from format_id
- `requirements.parameters_from_format_id: true` - Intrinsic image dimensions must equal logical dimensions multiplied by `pixel_ratio` (default 1)
...
```json
{
  ...
  "assets": {
    "banner_image": {
      "asset_type": "image"
      "url": "https://cdn.example.com/banner-300x250@2x.png"
      "width": 600
      "height": 500
      "pixel_ratio": 2
    }
...
**This format_id is parameterized:** logical dimensions plus density identify the variant. The creative still renders at 300×250; 600×500 is the intrinsic asset size, not another placement size.
...
- `pixel_ratio` (number, exclusive minimum: 0) - Intrinsic pixels per logical pixel; requires width and height and defaults to 1 when omitted
