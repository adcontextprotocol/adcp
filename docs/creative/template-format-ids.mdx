...
AdCP 3.1 did not standardize `format_id.pixel_ratio`. Its reference catalog therefore provides concrete `_image_2x` compatibility IDs for the seven standard fixed display sizes. Those entries project losslessly through their explicit `canonical_parameters`. The parameterized examples below are the 3.2 replacement and avoid adding another concrete ID for every future size.
