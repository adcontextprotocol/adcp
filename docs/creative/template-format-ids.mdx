...
AdCP 3.1 did not standardize `format_id.pixel_ratio` or canonical pixel-density fields. Its reference catalog therefore provides distinct concrete contracts for the seven standard fixed display sizes: the existing `_image` ID is 1x-only, `_image_2x` is 2x-only, and `_image_1x_2x` requires both assets for systems such as GAM. Their density and required-coverage values under `canonical_parameters` are opaque forward-projection metadata for 3.2-aware SDKs, not canonical 3.1 semantics. The parameterized examples below are the 3.2 replacement and avoid adding another concrete ID for every future size.
