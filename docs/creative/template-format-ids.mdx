...
3. **Parameterized format_id** - Template format with parameters
   - Example: `{id: "display_static", width: 300, height: 250, unit: "px"}`
   - Used in creatives to specify exact dimensions
...
Sales agents return products with placement-specific format constraints:
...
**Discovery Flow**:
1. Buyer calls `list_creative_formats()` on creative or sales agent → learns `display_static` accepts dimensions
2. Buyer calls `get_products()` on sales agent → learns which dimensions are supported (300x250, 728x90)
3. Buyer creates creative with parameterized format_id matching a supported placement dimension
...
- Reference template formats in `format_ids` array for flexible support:
  ...
  {"agent_url": "...", "id": "display_static"}
  ```
- OR list specific parameterized formats to constrain dimensions:
  ```json
  [
    {"agent_url": "...", "id": "display_static", "width": 300, "height": 250, "unit": "px"},
    {"agent_url": "...", "id": "display_static", "width": 728, "height": 90, "unit": "px"}
...
- Use template format_ids (no parameters) for flexible placements
- Use parameterized format_ids (with dimensions) to constrain exact dimensions
...
- For parameterized matches, verify exact equality including dimensions
...
**Parameterized format in placement:**
```json
