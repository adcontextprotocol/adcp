...
AdCP 3.1 did not standardize `format_id.pixel_ratio` or canonical pixel-density fields. Its reference catalog therefore provides concrete `_image_2x` compatibility IDs for the seven standard fixed display sizes. Each requires both a logical-size 1x asset and an exact 2x asset through ordinary legacy asset requirements. Their density and required-coverage values under `canonical_parameters` are opaque forward-projection metadata for 3.2-aware SDKs, not canonical 3.1 semantics. The parameterized examples below are the 3.2 replacement and avoid adding another concrete ID for every future size.
