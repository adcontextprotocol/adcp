...
<Info>
The brand manifest is the **data** — it can come from anywhere. A [`brand.json` file](/docs/brand-protocol/brand-json) is one way brands can self-publish a manifest, but you can also pass manifests inline, host them on a CDN, or look them up from the [registry](/docs/registry/index). When using `brand_manifest` in tasks like `create_media_buy`, pass the manifest object (or a URL to one) — not the `brand.json` file itself.
</Info>
...
For discoverable brand identity, brands can host their manifest via a `brand.json` file at a well-known location:

```
https://yourdomain.com/.well-known/brand.json
```

The `brand.json` file provides:
- **Brand discovery**: Agents can find official brand information for any domain
- **Agent authorization**: Designate an official brand agent that speaks for the brand
- **Enterprise auth**: Control access to brand resources via OAuth or API keys

### Example brand.json

A brand with an agent that provides brand manifests via MCP:
...
```json
{
  ...
  "version": "1.0",
  "brand_agent": {
    "url": "https://agent.acmecorp.com/mcp",
    "id": "acmecorp_brand_agent"
  }
}
```

See [Brand Protocol](/docs/brand-protocol/index) for the complete brand.json specification.

## Related Documentation

- **[Brand Protocol](/docs/brand-protocol/index)** - Brand discovery via brand.json
