...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/latest/core/assets/display-tag-asset.json",
...
Sellers publish a seller-wide ceiling in `execution.creative_specs.macro_resolution_capabilities`. The selected product format option publishes the binding set in `params.macro_resolution_capabilities`. Each entry is one exact semantic/operation/actor contract; it does not create a cross-product from a semantic list and maximum depth.

Standalone creative agents use a separate route rule: the selected `creative.supported_formats[]` entry's `params.macro_resolution_capabilities` is authoritative for build, preview, and validation, and its build-time entries name `performed_by: creative_agent`. The sales seller∩product intersection is evaluated later if the resulting manifest enters a seller execution path.
...
A required `unsupported` or `ambiguous` token fails with `MACRO_RESOLUTION_FAILED`. Its `error.details` MUST conform to [`error-details/macro-resolution-failed.json`](https://adcontextprotocol.org/schemas/latest/error-details/macro-resolution-failed.json). Results include the asset pointer, declaration ID, requested encoding, structured reason, and advertised encoding candidates.
...
2. `build_creative.macro_values` and preview macro values supply concrete raw values. With declarations, a value binds only a verified-universal `resolve_value` occurrence performed_by `creative_agent` and advertised by the selected creative route, after delivery-variant selection, using its exact encoding. It never bypasses `translate_to_native` or the target capability chain. Without declarations, the legacy behavior remains unchanged for 3.x compatibility.
...
6. The proposed #6207 product tracking contract remains responsible for whether a seller honors and fires buyer tracker asset types. When it lands, its detailed substitution claims reuse these exact macro capability entries; it does not redefine dialect, ownership, or encoding.
