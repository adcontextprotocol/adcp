...
A required `unsupported` or `ambiguous` token fails with `MACRO_RESOLUTION_FAILED`. Results include the asset pointer, declaration ID, requested encoding, structured reason, and advertised encoding candidates.
