...
The `{AXEM}` macro is from the legacy AXE integration. In [TMP](/docs/trusted-match), contextual data is delivered through the creative manifest in the Context Match response rather than an encoded macro.
