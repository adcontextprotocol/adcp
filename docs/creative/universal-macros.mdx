...
When a buyer responds to an [Identity Match](/docs/trusted-match/specification#identitymatchresponse) request, the response can include a `tmpx` field — an encrypted exposure token for per-user frequency tracking.
...
3. **Sales agent substitutes macros into the creative.** When trafficking, the sales agent calls `build_creative` with `macro_values` populated from Offer macros and the TMPX token. Alternatively, the ad server substitutes macros natively at serve time.
