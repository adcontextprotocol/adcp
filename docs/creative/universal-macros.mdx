...
`universal_macro_translation` helper that applies a per-macro mapping to a tracking
URL's query parameters. Each macro maps to either a **native** ad-server token (left
raw, filled by the ad server at impression time) or a concrete **value** the agent
already knows (substituted now, percent-encoded per RFC 3986):

```typescript
import { universal_macro_translation } from '@adcp/sdk';
...
const { url, dropped_params, unmapped_macros } = universal_macro_translation(
