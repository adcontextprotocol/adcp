...
The `{AXEM}` macro is from the legacy AXE integration. In [TMP](/docs/trusted-match), this is replaced by three mechanisms:
- **Structured creative assets** move to the `creative_manifest` field on the [Offer](/docs/trusted-match/specification#offer).
- **Dynamic key-value pairs** (for rendering) pass through the Offer [`macros`](/docs/trusted-match/specification#offer) field.
...
4. **Substitutes TMP Offer macros** into the creative when present. Context Match [Offer](/docs/trusted-match/specification#offer) macros carry dynamic rendering values (e.g., sponsor labels, promo codes) that are known at trafficking time — the sales agent passes these via `build_creative`'s `macro_values` parameter. The `{TMPX}` exposure token from Identity Match is per-user and per-impression — it MUST be substituted at serve time by the ad server, not at trafficking time.

5. **Leaves VAST macros unchanged** (for video formats)
...
### TMP Offer Macros and Creative Rendering

When a buyer responds to a [Context Match](/docs/trusted-match/specification#contextmatchrequest) request, the [Offer](/docs/trusted-match/specification#offer) can include a `macros` field — key-value pairs for dynamic creative rendering (e.g., sponsor labels, promo codes, landing page paths).

When a buyer responds to an [Identity Match](/docs/trusted-match/specification#identitymatchresponse) request, the response can include a `macros` field with the `TMPX` key — an encrypted exposure token for per-user frequency tracking.

**End-to-end flow:**

1. **Buyer sets macros.** Context Match offers carry rendering macros. Identity Match responses carry the TMPX exposure token.
2. **Publisher joins responses.** The publisher intersects Context Match offers with Identity Match eligibility, collecting macros from both.
3. **Sales agent substitutes macros into the creative.** When trafficking, the sales agent calls `build_creative` with `macro_values` populated from both Offer and Identity Match macros. Alternatively, the ad server substitutes macros natively at serve time.
4. **Ad server renders.** At impression time, remaining macros (platform-specific, like `%%CACHEBUSTER%%` in GAM) are replaced with actual values.
5. **Buyer pixel fires.** The tracking URL carries both rendering values and the encrypted TMPX token. The buyer's impression pixel receives the TMPX, decrypts it, and logs the per-user exposure.

**Relationship to universal macros:** Offer macros and TMPX are buyer-defined values. Universal macros like `{MEDIA_BUY_ID}` are platform-defined and substituted by the ad server. `{TMPX}` is registered in the universal macro enum for discoverability via `list_creative_formats`. Both coexist in the same creative.
...
- [sync_creatives](/docs/creative/task-reference/sync_creatives) - Creative management API