...
Catalog-item macros are the one macro class where the value originates in **buyer-controlled data** (the catalog feed) and expands at impression time into **publisher-controlled contexts** (tracker URLs, landing URLs, VAST tags). That flow is attacker-adjacent: a catalog value containing `&`, `#`, `?`, or a stray URL fragment can break out of the URL context if substituted raw.
...
- **URL percent-encoding is required before substitution.** Sales agents MUST apply RFC 3986 percent-encoding of reserved characters (`: / ? # [ ] @ ! $ & ' ( ) * + , ; =`) to the catalog-item value before substituting it into a URL context (query string, path segment, or fragment). A raw `&` in a `{GTIN}` value that truncates the surrounding query string — whether introduced accidentally by a malformed feed or deliberately by a compromised catalog source — does not propagate to the emitted tracker URL.
- **Nested macro expansion is prohibited.** A catalog-item value that itself contains text matching `{MACRO_NAME}` MUST NOT be re-expanded. Sales agents perform macro substitution in one pass: source placeholders are replaced with literal values, and those literal values are not re-scanned. A `{JOB_ID}` value of `vacancy-{DEVICE_ID}-42` produces the literal string `vacancy-{DEVICE_ID}-42` in the emitted URL, not a second-round expansion.
- **The catalog-item macro substitution scope is URL contexts only.** If a publisher substitutes a catalog-item macro into an HTML attribute context (e.g., rendering a banner template server-side), the publisher — not AdCP — is responsible for HTML-attribute escaping in addition to percent-encoding. This rule exists to bound the spec's responsibility to the URL context that AdCP actually governs.

Non-catalog macros (`{MEDIA_BUY_ID}`, `{DEVICE_ID}`, `{GEO}`, etc.) are populated from publisher-controlled or seller-controlled state, not buyer-supplied feed data; their values are trusted within the substitution pass. This rule deliberately scopes the normative MUST to the buyer-controlled attacker-adjacent class — a narrower, verifiable contract than a universal canonicalization rule.
