...
| Creative library / workspace | `library_id` in `list_creatives` | Optional — platforms without library grouping scope by account |
...
**Creative-level tags** (e.g., Flashtalking, Celtra) — The platform can produce a tag for any creative in its library without placement context. `build_creative(creative_id)` maps directly: the agent resolves the creative and returns a manifest with the tag as an asset.

**Placement-level tags** (e.g., CM360) — The platform generates tags per placement, not per creative. A CM360 adapter has two options for `build_creative(creative_id)`:
...
|---|---|---|
| Ad server with creative library | `list_creative_formats`, `list_creatives`, `build_creative` | `preview_creative`, `get_creative_delivery` |
| Creative management platform | `list_creative_formats`, `list_creatives` | `build_creative`, `preview_creative` |
