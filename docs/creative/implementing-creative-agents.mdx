...
The 3.2 deprecation applies to using `list_creative_formats` as the preferred **format-discovery** surface. `creative.supported_formats` does not carry standardized account-scoped `pricing_options`, so charged transformation and generation agents that need buyers to see pricing before `build_creative` continue to expose `list_creative_formats` through 4.x.
...
- [ ] All `format_id` values use proper namespacing (`domain:id`) when using the v1 named-format path
- [ ] The domain in each v1 `format_id` matches your `agent_url` domain
- [ ] `list_creative_formats`, if retained, is treated as optional v1 compatibility through 4.x rather than the canonical discovery surface
