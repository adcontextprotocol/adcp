...
- Optionally include DCO variable definitions per creative
...
| Creative library / workspace / template folder | `library_id` in `list_creatives` | Optional — platforms without library grouping scope by account |
...
**Placement-level tags** (CM360) — The platform generates tags per placement, not per creative. A CM360 adapter has two options for `build_creative(creative_id)`:
1. Return the creative's `htmlCode` or `redirectUrl` directly as a manifest asset
2. Use internal trafficking to produce a tag scoped to the target format
...
| Platform type | Required tasks | Optional tasks |
|---|---|---|
| Ad server with creative library (CM360, Flashtalking) | `list_creative_formats`, `list_creatives`, `build_creative` | `preview_creative`, `get_creative_delivery` |
| Creative management platform (Celtra) | `list_creative_formats`, `list_creatives`, `build_creative` | `preview_creative` |
| Generative creative tool | `list_creative_formats`, `build_creative` | `preview_creative` |
