...
# 1. MCP initialize — establish session; capture Mcp-Session-Id if server is stateful
...
# 2. Send notifications/initialized (required by the MCP lifecycle before any tool call)
curl -s -o /dev/null -X POST "$AGENT_URL" "${HEADERS[@]}" \
  -d '{"jsonrpc":"2.0","method":"notifications/initialized"}'

# 3. Call get_adcp_capabilities
