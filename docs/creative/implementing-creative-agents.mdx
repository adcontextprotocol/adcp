...
> **Canonical formats in 3.2**: creative agents declare build, validation, and preview capabilities through `creative.supported_formats` on `get_adcp_capabilities`. `list_creative_formats` and named format IDs are deprecated compatibility surfaces. See [canonical formats](/docs/creative/canonical-formats).
...
When any entry explicitly includes `preview` in `operations`, the enclosing capability response MUST also publish `creative.preview`. Its `supported_capability_ids[]` equals the preview-operation routes, and `fidelity` declares whether they are `authoritative` serving-platform previews or `representative` approximations. A standalone creative agent normally declares `representative`; do not claim `authoritative` unless the serving platform's own preview machinery produced the output.
...
See [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities).
...
```json
{
  "creative": {
    ...
    "preview": {
      "supported_capability_ids": ["display_banner"],
...
- [Deprecated list_creative_formats task](/docs/creative/task-reference/list_creative_formats) - 3.x compatibility guidance
