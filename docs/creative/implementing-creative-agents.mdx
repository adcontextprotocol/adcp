...
A creative agent that charges for its services needs an account relationship and persistent creative identity. This means **if you charge, you're stateful** — even if you started as a stateless transformation engine.

The transition from free/stateless to paid/stateful requires:
1. **Implement the [Accounts Protocol](/docs/accounts/overview)** — buyers establish accounts with rate cards
2. **Persist `creative_id`** — every build output gets a durable identity so it can be referenced in `list_creatives` and `report_usage`
3. **Return `pricing_options` on `list_creatives`** — the buyer discovers your rates before building
4. **Return pricing in `build_creative` responses** — `pricing_option_id`, `vendor_cost`, `currency`, and `consumption`
5. **Accept `report_usage`** — orchestrators report what was served so you can track revenue

Free transformation agents remain stateless and unchanged. No account, no pricing, no persistence required.

#### Transformation agents that charge must implement `list_creatives`

If you are a stateless transformation agent today and want to start charging, you need to add `list_creatives` to your agent. This is how buyers discover your pricing. Without `list_creatives`, there is no surface for `pricing_options` — `list_creative_formats` does not carry pricing because formats describe capabilities, not commercial terms.

Your `list_creatives` response returns the creatives your agent has built for this account (persisted with `creative_id`), each with `pricing_options` from the account's rate card. This is the same task ad servers use to expose their libraries — the difference is that your library contains creatives your agent built, not creatives uploaded through a UI.

#### Pricing walkthrough

Here is the full round-trip for a transformation agent charging per format adapted.

**Step 1: Buyer discovers pricing via `list_creatives`**

The buyer calls `list_creatives` with `include_pricing: true` and their `account`. Your agent looks up the account's rate card and returns `pricing_options` on each creative:
...
Multiple options are common — here the buyer sees a standard rate and a volume rate.
