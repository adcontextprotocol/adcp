...
| **Specialism** | A narrow capability claim within a domain — e.g. `sales-guaranteed`, `creative-generative`, `signal-marketplace`. Declared via `specialisms`. |
...
An agent tells the network what it does via two fields on `get_adcp_capabilities`: `supported_protocols` (broad domain claims) and `specialisms` (narrow capability claims). Both are verified against storyboards the protocol ships at `/compliance/{version}/`. Universal storyboards run against every agent; domain and specialism storyboards run against matching claims. See the [Compliance Catalog](/docs/building/compliance-catalog) for the full taxonomy.
...
2. **Purchase** — `create_media_buy` with targeting and budget. Note the `confirmed_at` timestamp in the response — this is the seller's order confirmation.
