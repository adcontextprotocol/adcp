...
training seller supports the exact `3.2-rc.1` wire bundle. Drive it with
`@adcp/sdk@14.0.0-rc.36`, which embeds `3.2.0-rc.2` and therefore takes
`3.2-rc.1` as a wire pin rather than as its `adcpVersion` schema selection.
Verify those advertised capabilities before the lab; unknown-field acceptance
by an older seller is not evidence of support.
</Note>
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-rc.1/media-buy/request-proposals-request.json",
...
- Read `get_adcp_capabilities.adcp.supported_versions` and `media_buy.lifecycle_tools`, pin `adcp_version: "3.2-rc.1"` for this lab, and validate the echoed served release before using targeting-aware discovery.
