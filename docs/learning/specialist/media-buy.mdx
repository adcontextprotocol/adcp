...
- Interpret seller insights as current relationship-scoped state: explain why creative fatigue belongs to one package–creative assignment, why the same creative may be fatigued on one seller but not another, and why provider methodology belongs in `ext`
...
<CardGroup cols={2}>
  ...
  <Card title="Insights" icon="lightbulb" href="/docs/media-buy/media-buys/insights">
...
10. **Warnings, insights, and invalidations** — Analyze a success warning and the later resource snapshot; distinguish buy/package/assignment insight placement, exact type coverage, assignment approval, and webhook invalidation handling.

For the insight exercise, Addie supplies this compact creative-library fixture:
  ...
  "media_buys": [
    {
      ...
      "packages": [
        {
          "package_id": "pkg_social",
          "insight_types_evaluated": ["pacing_risk"],
          "insights_as_of": "2026-08-04T12:00:00Z",
          "insights": [{ "type": "pacing_risk" }]
        }
      ]
    }
  ],
...
Finally, after activating the subscriber and establishing a complete `get_media_buys` baseline (known IDs or every status, fully paginated, without `insight_types`), the learner receives these invalidations in either order:
...
For a generative assignment, the learner must explain that this insight describes the stable brief-or-creative × package aggregate. Fatigue for individual live renders belongs in `get_creative_delivery`. A material in-place brief revision invalidates the prior evaluation and fires `insights.changed`; the snapshot is unknown until reevaluated. After replacing the creative, a successful direct unfiltered read confirming the old assignment is absent retires its old insight keys; filtered disappearance does not.
