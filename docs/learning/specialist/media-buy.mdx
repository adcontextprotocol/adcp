...
- Interpret broadcast delivery reports: `get_media_buy_delivery` returns measurement window data (`live`, `c3`, `c7`) that progresses over days after broadcast — the C7 window won't close until seven days post-air, so expect incomplete data during that period; this is by design, not underdelivery
