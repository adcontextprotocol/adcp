...
5. **Campaign commitment and optimization** — Commit a direct offer with `buy_products`, assign creatives through `sync_creatives`, monitor through `get_media_buys`, and apply an in-envelope change with `control_media_buy`. Explain how the 3.x `create_media_buy` and `update_media_buy` facades map to those canonical operations.
6. **Proposal lifecycle walkthrough** — Use `request_proposals`, refine to a committed immutable proposal, and call `accept_proposal`. Trace the resulting buy through `pending_creatives` → `sync_creatives` → `pending_start` → `active`, reading `accepted_proposal`, `available_actions`, and transition state through `get_media_buys`.
