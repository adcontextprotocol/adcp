...
- Interpret seller advisories as current relationship-scoped state: explain why creative fatigue belongs to one package–creative assignment, why the same creative may be fatigued on one seller but not another, and why provider methodology belongs in `ext`
...
<CardGroup cols={2}>
  ...
  <Card title="Advisories" icon="lightbulb" href="/docs/media-buy/media-buys/advisories">
...
10. **Warnings, advisories, and invalidations** — Analyze a success warning and the later resource snapshot; distinguish buy/package/assignment advisory placement, exact type coverage, assignment approval, and webhook invalidation handling.

For the advisory exercise, Addie supplies this compact creative-library fixture:
...
The learner must identify the first relationship as evaluated for two creative types only on publisher A, with fatigue asserted only on its feed placement. The second is clear only for fatigue, not for every advisory type. The third is unknown but still has stable relationship identity and approval state. Clearing requires a direct, covered, strictly newer snapshot.

Addie then supplies this successful `update_media_buy` receipt:
...
The learner must explain that the update succeeded and the warning is only its immediate receipt. A subsequent unfiltered `get_media_buys` read supplies the durable conclusion:
  ...
  "media_buys": [
    {
      ...
      "packages": [
        {
          "package_id": "pkg_social",
          "advisory_types_evaluated": ["pacing_risk"],
          "advisories_as_of": "2026-08-04T12:00:00Z",
          "advisories": [{ "type": "pacing_risk" }]
        }
      ]
    }
  ],
...
Finally, after activating the subscriber and establishing a complete `get_media_buys` baseline (known IDs or every status, fully paginated, without `advisory_types`), the learner receives these invalidations in either order:
...
The learner describes the required signature-verification step, dedupes each fire, coalesces them into one unfiltered reread, and reconciles only from the returned snapshot. Cryptographic execution is out of scope for this module; implementers follow the [webhook callback verifier checklist](/docs/building/by-layer/L1/security#webhook-callbacks). A timestamp-only reevaluation must not fire. Assignment removal retires prior keys even when the two invalidations arrive in either order.

For a generative assignment, the learner must explain that this advisory describes the stable brief-or-creative × package aggregate. Fatigue for individual live renders belongs in `get_creative_delivery`. A material in-place brief revision invalidates the prior evaluation and fires `advisories.changed`; the snapshot is unknown until reevaluated. After replacing the creative, a successful direct unfiltered read confirming the old assignment is absent retires its old advisory keys; filtered disappearance does not.
