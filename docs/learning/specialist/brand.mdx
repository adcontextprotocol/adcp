...
<CardGroup cols={2}>
  <Card title="Brand Protocol overview" icon="fingerprint" href="/docs/brand-protocol">
  ...
  <Card title="Mutual-assertion trust model" icon="scale-balanced" href="/docs/brand-protocol/brand-json#mutual-assertion-trust-model">
    The two trust layers, the reciprocation table, and why consistency is not standing — the knowability framework.
  </Card>
  <Card title="Seller verification" icon="shield-check" href="/docs/verification/overview">
  ...
  <Card title="Brand identity → creative" icon="palette" href="/docs/brand-protocol/key-concepts#creative-generation">
  ...
  <Card title="Building a brand agent" icon="server" href="/docs/brand-protocol/building-a-brand-agent">
    Implementing `get_brand_identity`, `verify_brand_claim`, and the response-signing key.
  </Card>
  <Card title="Experimental status" icon="flask" href="/docs/reference/experimental-status">
...
- `…/fixtures/walkthrough/sportshaus-holdings/.well-known/brand.json` — the parent house's identity, with `brand_refs[]` **and** `authorized_operators[]` (a portfolio-wide agency plus a brand-scoped, US-only in-house operator)
  ...
  These are the multi-tier trust-chain fixtures from the [seller-verification walkthrough](/docs/verification/overview).
