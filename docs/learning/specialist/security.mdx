...
- Obtain a signed governance token from the sandbox governance agent, decode it, and walk the 15-step verification checklist (`alg` allowlist, `typ` match, SSRF-validated JWKS fetch, `aud`/`sub`/`phase` binding, `jti` replay dedup, revocation list check), explaining what each step closes and how revocation works — from the operator's own-issuance perspective
...
3. **Governance token verification** — Obtain a signed governance token from the sandbox governance agent (`sync_plans`, then an intent-phase `check_governance`), and decode its header (`alg`, `typ`, `kid`) and claims (`aud`, `sub`, `phase`, `jti`, `exp`). Walk the 15-step verification checklist and, for each step, explain the attack it closes — e.g. how `aud` byte-matching stops a token addressed to one seller from being accepted by another (confused-deputy / token redirection), while `jti` seen-before dedup is the separate step that stops replay of the same token, and why the revocation check (`kid`/`jti` in the signed revocation list) runs on every verification. The sandbox agent is a token *issuer*, so verifying against a tampered or revoked token is something you reason through for your own verifier implementation, not something the issuer rejects for you.
