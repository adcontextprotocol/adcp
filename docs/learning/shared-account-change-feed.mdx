...
4. let the training seller simulate an external change;
...
Create a sandbox creative, then call `comply_test_controller` with
`scenario: "force_creative_status"`. This controller action represents the
connected platform changing the creative; it is not an AdCP creative mutation
from your buyer workflow.

The training seller first commits the new creative status and account change
...
### 5. Prove tail and recovery behavior
...
You pass when you can demonstrate one externally initiated change end to end
