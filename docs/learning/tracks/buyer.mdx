...
- **Order lifecycle** — `confirmed_at` on creation, pause/resume, cancellation via `update_media_buy`, terminal states
- **Valid actions** — `get_media_buys` returns `valid_actions` so agents know what operations are permitted
...
Create a working buyer agent that discovers products and executes media buys. Use any AI coding assistant (Claude Code, Cursor, Copilot) with the [`@adcp/client`](/docs/building/schemas-and-sdks) SDK. The skill tested is orchestrating a buying workflow correctly.
...
Your buyer agent runs against the public test agent (`test-mcp`). Use `npx adcp` to execute tool calls and verify your agent handles the complete buying workflow:
