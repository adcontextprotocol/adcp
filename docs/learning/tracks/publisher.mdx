...
- `get_adcp_capabilities` that declares `adcp.major_versions` and validates `adcp_major_version` on incoming requests
...
- Proper error responses for invalid requests — including `VERSION_UNSUPPORTED`, `GOVERNANCE_DENIED`, and `TERMS_REJECTED` where applicable
