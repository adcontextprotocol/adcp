...
- **Seller-side governance** — when a buyer account has `governance_agents` (synced via [`sync_governance`](/docs/accounts/tasks/sync_governance)), call `check_governance` with `media_buy_id` + `planned_delivery` before confirming media buys
...
Create a working sales agent that responds to real buyer queries. Point your AI coding assistant (Claude Code, Cursor, Copilot) at the [`build-seller-agent`](/docs/building/build-an-agent) skill file — it generates a storyboard-validated agent from your specification. The skill tested is specifying correct AdCP behavior.
