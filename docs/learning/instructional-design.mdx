...
Modules B4, C4, and D4 use a five-phase approach built on the same tools developers use in practice — [skill files](/docs/building/build-an-agent) to generate agents and [storyboards](/docs/building/validate-your-agent) to validate them:
...
3. **Validate** (~10 min) — learner runs the matching storyboard from the CLI (`npx adcp storyboard run my-agent media_buy_seller`), shares results with Addie, and iterates on failures with their coding assistant
