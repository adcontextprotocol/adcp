...
async function evaluateAcquisition(request, talent) {
  ...
  return { status: "completed", rights_status: "acquired", /* ... */ };
}
```
...
How you identify callers depends on your authentication setup. The MCP transport provides session information; your auth middleware maps that to a linked account. See the [authentication guide](/docs/building/by-layer/L2/authentication) for patterns.
