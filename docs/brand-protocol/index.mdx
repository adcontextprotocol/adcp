...
A `brand.json` file may **include** brand identity data (logos, colors, tone) or point to a brand agent that provides it. But most AdCP operations work with the brand manifest directly. You don't need a `brand.json` file to use AdCP — you can pass brand manifests inline or reference the registry.

<Tip>
Think of `brand.json` as a signpost on a company's front door. It might provide brand information directly, redirect you to a house domain, or point you to a brand agent. The **brand manifest** is the information itself, regardless of how you obtained it.
</Tip>
...
|--------|-------------|-------------|
| [`resolve_brand`](#mcp-tools) | Fetches `/.well-known/brand.json`, extracts the manifest | Brand publishes a `brand.json` file |
| [Brand enrichment](/docs/registry/index#brand-resolution) | Fetches from Brandfetch, saves manifest to registry | No `brand.json` available, need enrichment |
| [Registry lookup](/docs/registry/index#brand-resolution) | Returns community or enriched manifest from the registry | Brand already registered |
