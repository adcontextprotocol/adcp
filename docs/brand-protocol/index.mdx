...
Returns brand identity data that's richer, more dynamic, or more access-controlled than static brand.json. Response depth depends on the caller's authorization — linked accounts (via `sync_accounts`) get full assets, voice configs, and rights availability.
