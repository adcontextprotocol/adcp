...
|---|---|
| `disputed` | The brand has spoken. Treat the underlying claim (leaf's `house_domain`, third party's `brand_refs[]`, etc.) as rejected. UI: "Brand X says this is not theirs." |
| `not_ours` | The brand affirms no relationship. Treat any countervailing claim as one-sided unverified metadata. UI surfaces the rejection. |

These responses override any leaf-side `house_domain` declaration. A leaf publisher whose claim is rejected has **no protocol-level recourse** beyond updating or removing its own `house_domain`.

### Assertion direction: mutual assertion still required

When the agent signs `owned`, `pending_review`, or `transferring`, the answer is informative but NOT trust-extending on its own. Consumers MUST still verify the leaf reciprocates via crawl (or via the leaf's own brand-agent, if one is published) before extending relationship trust. The agent's value here is:

- **Real-time confirmation** that a known mutual edge is still current (faster than crawl TTL).
- **Richer state surface** (`pending_review` and `transferring` express states that crawl can't).
- **Confirmation of agent identity** for the consumer (signed by the named house's JWK).

| Agent says | Crawl reciprocation (leaf's `house_domain` points back at this house) | Consumer treats as |
|---|---|---|
| `owned` | Yes (mutual) | **Trusted edge.** Governance propagation: yes. |
| `owned` | No (leaf silent or claims a different house) | **Unilateral assertion — not trusted.** UI: "Brand X claims this; the brand itself does not (yet) reciprocate." Equivalent to a crawled `brand_refs[]` entry with no leaf reciprocation. |
| `pending_review` | No (parent → leaf claim acknowledged; leaf may or may not reciprocate yet) | Trusted leaf identity (TLS); relationship trust withheld. When `expected_resolution_window_days` elapses without transition, treat as `unknown` and fall back to crawl. |
...
For `verify_property` and `verify_trademark`, the same direction-asymmetric rule applies. A brand-agent's `owned` claim over a property or trademark is informational unless backed by mutual assertion in the appropriate surface: for properties, the property's own brand.json (via reverse lookup of the registered domain) or `properties[]` listing on the claimed-owning brand's brand.json plus the property's own publishing; for trademarks, the public registry record. The agent surfaces nuanced licensing state (`licensed_in` / `licensed_out`) and rejection that crawls can't, but the underlying "does the brand actually own this" check is unchanged.

### The malicious-house scenario

Concretely: if Nike's brand-agent answers `owned` for `adidas.com`, the consumer:

1. Verifies Nike's signature.
2. Crawls `adidas.com/.well-known/brand.json` and reads its `house_domain` (or its own brand-agent's `verify_subsidiary_claim` response, with role-swap).
3. If Adidas reciprocates Nike → trusted edge. (This won't happen.)
4. If Adidas claims a different house, or no house, or rejects via its own agent → Nike's claim is unilateral, NOT trusted.

The same logic protects against the reverse: a third party can't unilaterally claim Nike as their parent without Nike's agent (or brand_refs[]) reciprocating.
...
- **Standing licensed relationships as a static brand.json surface.** `verify_trademark` returns `licensed_in` / `licensed_out` because the underlying relationships are real — Marriott franchisees use the MARRIOTT mark under license, music catalogs are licensed-out per territory, sub-brands license rights bidirectionally with their parents. The brand-agent can speak to these because they're in its internal records. **But `brand.json` itself doesn't yet have a publishing surface for standing licensed relationships** parallel to `brand_refs[]` for ownership. The existing rights protocol (`rights_agent`, `acquire_rights`) handles *transactional* licensing — negotiate a deal — not standing declarations like "I license-in the X mark from Y for Z use cases in CN" or "I've granted Acme Distribution rights to my catalog in CN." This gap is real and deserves its own RFC alongside the rights-protocol team. For this RFC: the verification surface exposes the licensed states because partners need to consume them, but the static-file substrate that backs them is a separate design. The `licensed_in` / `licensed_out` answers come from the brand-agent's internal records, not from a crawlable file. Tracked as a follow-up — the design likely needs `licensed_from[]` / `licensed_to[]` entries on the brand definition with jurisdictional scoping, mutual-assertion shape parallel to `brand_refs[]`, and integration with the rights protocol's `available_uses` / `right_types` vocabulary.
...
3. **Agent says `not_ours` while leaf publishes `house_domain` claim.** Normative: agent wins on the **rejection direction** — a brand has standing to refuse association unilaterally. On the **assertion direction**, the agent's `owned` does NOT override the absence of leaf reciprocation; mutual assertion remains required for positive trust extension. This asymmetry prevents a malicious or mistaken house from claiming subsidiaries it doesn't own. See [Trust model](#trust-model) for the full table. UI guidance for the rejected leaf's recourse is a follow-up issue.
