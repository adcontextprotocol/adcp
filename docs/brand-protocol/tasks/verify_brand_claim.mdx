...
A valid signature proves authorship, not brand authority. Because `brand_domain` and `agent_url` are fields inside the signed payload, a conformant agent for a domain the attacker controls can emit a cryptographically valid envelope naming any `brand_domain`. Before trusting a signed envelope as evidence about the asserted `brand_domain`, a verifier MUST resolve `{brand_domain}`'s `brand.json`, confirm an `agents[]` entry with `type: "brand"` whose `url` byte-equals the envelope's `agent_url`, and resolve the envelope's `kid` only within that entry's `jwks_uri` — the full normative step, including the reject reasons, is [Brand-authorization cross-check](/docs/building/by-layer/L1/security#designated-task-response-signing) in the security profile. Until it passes, `brand_domain` and `agent_url` are untrusted input, and this holds for both assertion and rejection directions.
