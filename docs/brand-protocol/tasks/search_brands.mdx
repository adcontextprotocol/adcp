...
`search_brands` is an experimental, read-only discovery task. It returns ranked public brand stubs from a brand agent's roster. Use the returned `brand_id` with [`get_brand_identity`](/docs/brand-protocol/tasks/get_brand_identity) for full identity data or [`get_rights`](/docs/brand-protocol/tasks/get_rights) for licensing terms.
