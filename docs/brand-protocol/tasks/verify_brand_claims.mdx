...
The single-target [Brand-authorization cross-check](/docs/building/by-layer/L1/security#designated-task-response-signing) applies unchanged: the one batch envelope's `agent_url` MUST byte-equal a `type: "brand"` `agents[]` entry in `{brand_domain}`'s `brand.json`, and the `kid` MUST resolve only within that entry's `jwks_uri`, before any result in `results[]` is trusted as evidence about `brand_domain`.
