...
The agent's response is signed under the brand's `adcp_use: "response-signing"` JWK. Consumers verify the signature, then apply **direction-asymmetric trust rules**:

- **Rejection direction** — agent's `disputed` / `not_ours` is authoritative. A brand can refuse association unilaterally; no mutual reciprocation required.
- **Assertion direction** — agent's `owned` / `pending_review` / `transferring` is informative but NOT trust-extending on its own. The leaf must still reciprocate (via its own `house_domain` claim, or its own brand-agent) before relationship trust extends. Otherwise a malicious or mistaken house could claim subsidiaries it doesn't own.

See [RFC § Trust model](/docs/brand-protocol/proposals/brand-verification-rfc#trust-model) for the full normative table.

| Agent says | Leaf reciprocation observed | Consumer treats as |
|---|---|---|
| `owned` | Yes (leaf's `house_domain` points back at this house) | **Trusted edge.** Governance propagation: yes. |
...
| `pending_review` | No (parent has acknowledged; leaf may not yet reciprocate) | Trusted leaf identity (TLS); relationship trust withheld. When `expected_resolution_window_days` elapses without transition, treat as `unknown` and fall back to crawl. UI: "Pending parent confirmation, expected within N days." |
...
| `disputed` | Any | The brand has spoken. Treat the underlying claim as rejected. UI: "Brand X says this is not theirs." |
| `not_ours` | Any | The brand affirms no relationship. Treat any countervailing claim as one-sided unverified metadata. |
| `unknown` | Any | Fall back to crawl-based mutual-assertion inference. |

The crawl path is required for `owned` and `pending_review` / `transferring` (to confirm reciprocation) and is the fallback when the agent is unreachable or returns `unknown`.
