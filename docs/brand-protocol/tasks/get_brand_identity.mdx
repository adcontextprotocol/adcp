...
```json Response (authorized)
{
  ...
  "fonts": {
    "primary": {
      "family": "Montserrat",
      "files": [
        { "url": "https://cdn.example.com/fonts/montserrat-400.woff2", "weight": 400, "style": "normal" },
        { "url": "https://cdn.example.com/fonts/montserrat-700.woff2", "weight": 700, "style": "normal" }
      ]
    },
    "secondary": "Open Sans"
  },
...
| `fonts` | object | No | Brand typography. Keys are role names (`primary`, `secondary`). Values are a CSS font-family string or an object with `family` and `files` (array of `{url, weight, style}`) |
