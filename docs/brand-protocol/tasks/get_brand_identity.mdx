...
| `voice_synthesis` | object | No | TTS voice synthesis configuration (`provider`, `voice_id`, `settings`, optional `rights_offering_id`). The offering ID is advisory provenance from `get_rights`, not a buyer-specific grant; resolve it through `acquire_rights` before synthesis. |
