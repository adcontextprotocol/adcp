...
Authorized callers (linked via [`sync_accounts`](/docs/accounts/tasks/sync_accounts)) get deeper data on top of the public baseline.
...
| `voice_synthesis` | object | No | TTS voice synthesis configuration (`provider`, `voice_id`, `settings`, optional `rights_offering_id`). The offering ID is advisory provenance from [`get_rights`](/docs/brand-protocol/tasks/get_rights), not a buyer-specific grant; resolve it through [`acquire_rights`](/docs/brand-protocol/tasks/acquire_rights) before synthesis. |
