...
| `fields` | string[] | No | Optional sections to include. Omit for all authorized sections. Core fields (`brand_id`, `house`, `names`) are always returned and do not need to be requested. |
...
Valid `fields` values: `description`, `industry`, `keller_type`, `logos`, `colors`, `fonts`, `tone`, `tagline`, `voice_synthesis`, `assets`, `rights`
