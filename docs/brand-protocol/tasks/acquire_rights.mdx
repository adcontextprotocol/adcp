...
All webhook signing follows the AdCP [push notification signing profile](/docs/building/implementation/webhooks#signature-verification) — RFC 9421 by default (publisher signs with its `adcp_use: "webhook-signing"` key), with the deprecated HMAC-SHA256 fallback available when the buyer populates `authentication.credentials`.
