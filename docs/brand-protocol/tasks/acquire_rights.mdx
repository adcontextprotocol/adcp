...
<Note>
**Experimental.** Brand rights lifecycle ([`get_rights`](/docs/brand-protocol/tasks/get_rights), `acquire_rights`, [`update_rights`](/docs/brand-protocol/tasks/update_rights)) is part of AdCP 3.0 as an experimental surface — it may change between 3.x releases with at least 6 weeks' notice. Sellers implementing any of these tasks MUST declare `brand.rights_lifecycle` in `experimental_features`. See [experimental status](/docs/reference/experimental-status) for the full contract.
</Note>

Binding contractual request to acquire rights from a brand agent. Parallels [`create_media_buy`](/docs/media-buy/task-reference/create_media_buy) — select a `pricing_option_id` from `get_rights` and provide campaign details. The agent clears against existing contracts and returns terms, generation credentials, and disclosure requirements.
...
|-------|------|----------|-------------|
| `rights_id` | string | Yes | Rights offering identifier from [`get_rights`](/docs/brand-protocol/tasks/get_rights) |
| `pricing_option_id` | string | Yes | Selected pricing option |
| `buyer` | brand-ref | Yes | Buyer's brand identity |
| `account` | account-ref | Conditional | Account context. Pass when the brand agent has a governance agent bound to this brand+operator via [`sync_governance`](/docs/accounts/tasks/sync_governance) — the brand agent uses it to resolve the bound governance agent and project commitment under the [CPM-priced rights validation](#cpm-priced-rights-under-a-governance-plan). Optional when the request is ungoverned or threads a `governance_context` token on the envelope instead. |
...
Two campaign-field validations are normative for `acquire_rights`. Both produce [`INVALID_REQUEST`](/docs/building/verification/compliance-catalog#error-code-invalid-request) with the offending `field` populated and `recovery: "correctable"` (the buyer can fix the request and retry).
...
Like [`create_media_buy`](/docs/media-buy/task-reference/create_media_buy), rights grants are not time-shiftable for elapsed windows: the contract attaches to the requested period, so reject-only is the correct contract for `acquire_rights`.
...
Ungoverned requests — neither `governance_context` token nor an account with a bound governance agent — are unaffected by this validation. `estimated_impressions` remains optional in that case (sellers MAY refuse to transact ungoverned plans as a matter of commercial policy, per [Spend-commit invocation](/docs/governance/campaign/specification#spend-commit-invocation), but that refusal is independent of this projection rule).
...
When `terms.impression_cap` is set, it is a **soft cap**. Delivery is not automatically halted at the cap — the buyer is responsible for monitoring usage via [`report_usage`](/docs/accounts/tasks/report_usage) and managing delivery accordingly. Impressions beyond the cap are billed at `terms.overage_cpm`. If the rights holder wants a hard cap (no delivery beyond the limit), they specify this in `restrictions`.

## Usage reporting

The `usage_reporting_url` in the acquired response is a convenience endpoint provided by the rights agent for HTTP-based impression reporting. It accepts the same payload as the [`report_usage`](/docs/accounts/tasks/report_usage) MCP task. Use whichever integration is simpler for your pipeline — the MCP tool for agent-to-agent workflows, or the URL for direct HTTP calls from ad servers.

## Renewal and updates

To extend a rights grant, adjust impression caps, change pricing, or pause/resume, use [`update_rights`](/docs/brand-protocol/tasks/update_rights). Extended grants receive re-issued generation credentials and an updated `rights_constraint` for re-embedding in manifests.

## Next steps

<CardGroup cols={2}>
  <Card title="update_rights" icon="rotate" href="/docs/brand-protocol/tasks/update_rights">
