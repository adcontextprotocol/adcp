...
If `acquire_rights` returns `pending_approval`, poll the rights agent by calling `get_rights` with the same `brand_id` to check when the status resolves. Set `approval_status: 'pending'` on any creative manifests built during this period.
