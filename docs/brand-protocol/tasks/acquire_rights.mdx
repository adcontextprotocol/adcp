...
<Note>
**Experimental.** `acquire_rights` is part of AdCP 3.0 as an experimental surface — it may change between 3.x releases with at least 6 weeks' notice. Sellers implementing it MUST declare `brand.rights_lifecycle` in `experimental_features`. See [experimental status](/docs/reference/experimental-status) for the full contract.
</Note>
...
All webhook authentication uses the AdCP [push notification signing convention](/docs/building/implementation/webhooks#hmac-sha256-recommended-for-production) — `X-ADCP-Signature` and `X-ADCP-Timestamp` headers with HMAC-SHA256.
