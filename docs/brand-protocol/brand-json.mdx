...
For every domain-derived well-known fetch in the resolution algorithm, resolvers MUST follow HTTP redirects only between the originally requested hostname and its exact `www` counterpart, on HTTPS at the original port. For example, `example.com` may redirect to `www.example.com` and back; `brand-assets.example.com` MUST be refused even though it shares the same registrable domain. Resolvers MUST re-apply [SSRF controls](/docs/building/by-layer/L1/security#webhook-url-validation-ssrf) on every hop and stop after 3 redirects. Any other hostname requires an explicit [`authoritative_location`](#1-authoritative-location-redirect). This permits ordinary apex ↔ `www` hosting normalization without trusting other delegated or dangling sibling subdomains. Operators that use this convention remain responsible for keeping the exact `www` name under the same administrative control as the original hostname.
...
For houses **with** a brand-agent advertising [`verify_brand_claim`](/docs/brand-protocol/tasks/verify_brand_claim), the consumer asks the agent directly instead — see [Agent-augmented verification](#agent-augmented-verification) below. The agent path surfaces richer states (`pending_review`, `transferring`, `disputed`, `licensed_in`) than the email loop can express, and when both house and leaf publish agents, mutual assertion completes via two signed agent calls without a static-file crawl.

### Agent-augmented verification

When the brand publishes a brand-agent ([variant 3](#3-brand-agent) or a `type: "brand"` entry in `agents[]` on variant 4/5), the agent can be asked verification questions via [`verify_brand_claim`](/docs/brand-protocol/tasks/verify_brand_claim) — a single tool that takes a `claim_type` (`subsidiary` / `parent` / `property` / `trademark`) and returns the brand's authoritative answer. The brand-agent is brand.json hidden behind a server: same data the static file carries, plus the richer states the static file can't capture (`pending_review`, `transferring`, `disputed`, `licensed_in`, `licensed_out`).
...
The rules below apply only after the [brand-authorization cross-check](/docs/building/by-layer/L1/security#designated-task-response-signing) returns verifier-local `trust: "trusted"`. A skipped or failed check is `untrusted` for relationship decisions and cannot extend or revoke trust.
...
Agent responses are signed under the brand's `adcp_use: "response-signing"` JWK. See the [task reference](/docs/brand-protocol/tasks/verify_brand_claim) for the full request/response shapes and per-claim-type details.
...
<Info>
**Licensed-in / licensed-out relationships** (Marriott franchisees using the MARRIOTT mark, music catalogs licensed per territory, etc.) are queryable via the brand-agent today through [`verify_brand_claim`](/docs/brand-protocol/tasks/verify_brand_claim) with `claim_type: "trademark"`. A static `brand.json` publishing surface for standing licensed relationships (parallel to `brand_refs[]` for ownership) is a future RFC alongside the rights-protocol team. The static `trademarks[]` array below covers ownership only; licensing posture comes from the agent.
</Info>
...
| `relationship` | enum | No | How this brand relates to the property: `owned` (default), `direct`, `delegated`, `ad_network`. Delegated/network values match `delegation_type` in adagents.json for bilateral verification; `owned` is a brand.json-only ownership claim. See [ad networks](/docs/sponsored-intelligence/networks). |
...
This is the AdCP equivalent of `sellers.json` — the operator's public declaration of which publishers they work with. For delegated or network paths, the publisher confirms by setting the matching `delegation_type` on the agent's authorization in their [adagents.json](/docs/governance/property/adagents). For first-party inventory, `relationship: "owned"` is the operator's inline ownership declaration. Sell-side implementations need the operator's `brand.json` claim and, when inventory is publisher-authorized or delegated, the publisher's matching `adagents.json` authorization. See [seller setup](/docs/brand-protocol/seller-setup) for the step-by-step sell-side pattern and [ad networks](/docs/sponsored-intelligence/networks) for network-specific guidance.
