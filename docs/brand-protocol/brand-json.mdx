...
The spec defines a self-healing loop:

1. The house publishes a reachable `contact.email` at the top level of its `brand.json` (already part of the House Portfolio schema).
...
This turns mutual assertion from static two-party coordination into a feedback loop. The publisher of the consumer-side notification mechanism (email today; a typed verification endpoint is a planned follow-up) is responsible for rate-limiting and de-duplication.
...
The single-house, single-hop trust model deliberately doesn't cover three shapes:
...
These cases belong in governance / corporate-structure specs, not brand.json. `brand.json` is the brand-identity surface; corporate legal structure is its own concern.
...
- **Agent-augmented verification (proposed — see [brand verification RFC](/docs/brand-protocol/proposals/brand-verification-rfc)).** When the named house publishes a brand-agent advertising `verify_subsidiary_claim` (or `verify_property` / `verify_trademark`) in its `supported_tasks`, consumers SHOULD call the agent in addition to performing crawl-based mutual-assertion inference. The agent's response is signed under the brand's `adcp_use: "response-signing"` JWK. The trust model is asymmetric by direction: (a) signed `disputed` or `not_ours` from the agent overrides any leaf-side `house_domain` claim — a brand has standing to refuse association unilaterally; (b) signed `owned` / `pending_review` / `transferring` from the agent does NOT replace mutual-assertion verification — the leaf must still reciprocate before relationship trust extends, because otherwise a malicious or mistaken house could unilaterally claim subsidiaries it doesn't own. The agent surfaces richer states (`pending_review`, `transferring`, `licensed_in`) and provides real-time confirmation that crawl can't, but the mutual-assertion floor remains. The crawl path is the fallback when the agent is unreachable or returns `unknown`. Not yet normative; the email notification SHOULD continues to apply for houses without a brand-agent.
