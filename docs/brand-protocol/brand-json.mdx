...
Optional fields:
- `auth`: Authentication requirements (see below)
- `contact`: Contact information

When a brand has an agent, the agent is the authoritative source for brand information. Agents should provide brand manifests and other data via MCP tools.
...
|-------|------|----------|-------------|
| `canonical_domain` | string | Yes | House's primary domain |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `canonical_domain` | string | Yes | Brand's primary domain (unique ID) |
| `names` | array | Yes | Localized names (see below) |
| `keller_type` | enum | No | `master`, `sub-brand`, `endorsed`, `independent` |
| `parent_brand` | string | No | Parent brand's canonical_domain |
| `properties` | array | No | Digital properties owned by brand |
| `brand_standards` | string | No | URL to brand-standards.json |
| `brand_manifest` | object/string | No | Inline manifest or URL |
...
| `sub-brand` | Carries parent brand name | Nike SB |
...
For enterprise brands requiring authentication:
...
```json
{
  "auth": {
    ...
    "scopes": ["brand:read"],
    "instructions_url": "https://developer.example.com/auth"
  }
}
```
...
|-------|------|-------------|
| `required` | boolean | Whether auth is required (default: false) |
| `method` | enum | `api_key`, `oauth2`, or `bearer_token` |
| `token_endpoint` | string | OAuth2 token endpoint URL |
| `scopes` | array | Required OAuth2 scopes |
| `instructions_url` | string | URL to credential documentation |
...
```json
{
  ...
  "version": "1.0",
  "house": {
    "canonical_domain": "bobsburgers.com",
...
```json
{
  ...
  "brand_agent": {
    ...
    "capabilities": ["creative_generation", "sponsored_intelligence", "product_inquiry"]
  },
  "auth": {
    "required": true,
    "method": "oauth2",
    "token_endpoint": "https://auth.enterprise.com/oauth/token",
    "scopes": ["brand:read", "creative:generate"]
  },
  "contact": {
