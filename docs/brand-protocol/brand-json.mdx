...
<Info>
`brand.json` is a discovery file that declares brand ownership and may include brand identity data. Most AdCP operations work with the [brand manifest](/docs/creative/brand-manifest) directly — you don't need a `brand.json` file to use AdCP. You can also pass manifests inline or look them up from the [registry](/docs/registry/index).
</Info>
