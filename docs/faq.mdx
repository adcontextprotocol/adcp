...
AdCP 3.0 is generally available and recommended for production. Stability guarantees:

- The 3.0 schemas, task definitions, and semantics are stable under semantic versioning
- Minor (3.x) releases add backward-compatible capabilities; patch releases are fixes only
- Breaking changes are reserved for the next major version

See the [release notes](/docs/reference/release-notes) for version history and the [versioning policy](/docs/reference/versioning) for support windows.
</Accordion>
