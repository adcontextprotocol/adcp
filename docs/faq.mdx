...
AAMP — IAB Tech Lab's Agentic Advertising Management Protocols framework — is an emerging suite of agentic-advertising initiatives (including an Agent Registry and Agentic Audiences work streams). AdCP and AAMP overlap in ambition (both target AI agents in advertising) and differ in several ways today:

| | AAMP | AdCP |
|---|---|---|
| **Maintainer** | IAB Tech Lab | AgenticAdvertising.org (open 501(c)(6), four equal voting classes) |
| **Maturity** | Emerging framework / work-in-progress | 3.0 release candidate, 3.0 GA targeted April 2026 |
...
| **Governance verification** | Under development | Signed governance context with 15-step verification (Layer 4 of the [security model](/docs/building/understanding/security-model)) |
| **Public schemas** | To be defined | [Published](https://adcontextprotocol.org/schemas/latest/), Apache 2.0 |

We do not yet publish a formal technical comparison because AAMP is still defining its normative surface. Implementers interested in both should watch both specifications as they stabilize. AdCP and AAMP are not mutually exclusive — a platform could implement both if scopes remain distinct.
</Accordion>
...
Put differently: MCP is the transport, AdCP is the protocol. You use MCP to *carry* AdCP tasks — the same way HTTP carries REST payloads. See [How AdCP relates to MCP and A2A](#how-does-adcp-relate-to-mcp-and-a2a) above, and [Industry landscape](/docs/building/understanding/industry-landscape) for the full picture.
</Accordion>
