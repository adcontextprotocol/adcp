...
```
GET https://{aao_directory}/api/v1/agents/{agent_url}/publishers
```

`{aao_directory}` is the directory origin, for example `https://agenticadvertising.org`; `/api/v1` is part of the endpoint path.
...
```
GET /api/v1/agents/https%3A%2F%2Fsales-agent.example.com%2F/publishers?status=authorized&status=revoked&limit=500
```

Equivalent in TypeScript:

```ts
const url = new URL(`${directory}/api/v1/agents/${encodeURIComponent(agentUrl)}/publishers`);
...
```python
requests.get(
    f"{directory}/api/v1/agents/{quote(agent_url, safe='')}/publishers",
...
```
GET /api/v1/agents/https%3A%2F%2Fsales-agent.example.com%2F/publishers?include=properties
```
...
| **Which publishers authorize my agent?** | **`GET /api/v1/agents/{agent_url}/publishers`** | **Pull, directory-supplied list** |
...
1. Call `GET /api/v1/agents/{agent_url}/publishers` to discover the publisher set.
