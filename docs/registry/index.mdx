...
- **Domain normalization (IDNA 2008 + confusable detection).** Save endpoints SHOULD apply IDNA 2008 to normalize internationalized domain names to ASCII before persistence, and SHOULD then run Unicode confusable-detection (for example, ICU `uspoof` or equivalent) against already-registered entries. Ambiguous submissions — mixed-script labels, homograph collisions with a registered brand, disallowed Unicode classes — SHOULD be rejected or flagged for human review rather than silently committed.
- **Ownership proof before commit.** Save endpoints SHOULD require evidence of domain control before a new brand or property entry commits. Accepted proofs SHOULD include either a DNS TXT record at `_adcp-owner.{domain}` matching a server-issued nonce, or an HTTP challenge hosted at `/.well-known/adcp-ownership.txt` on the domain. Revisions to an existing authoritative entry continue to require the `brand.json` / `adagents.json` authoritative path (see the 409 Conflict semantics in [Save brand](#save-brand) and [Save property](#save-property)); community-source saves are the case ownership proof covers.
