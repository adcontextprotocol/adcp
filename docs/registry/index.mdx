...
Payloads likewise carry `collections` — the collection constraints from `authorized_agents[*].collections`. When present, the grant does NOT authorize the property unqualified: consumers maintaining a local authorization index MUST scope it to those selectors (a selector without `collection_ids` is a bulk grant for all collections at that publisher domain), and a collection-constrained grant queried without collection scope fails closed. See [owner-sold channel carriage](/docs/governance/property/adagents#owner-sold-channel-carriage).

`collection.*` payloads annotate each `distribution[]` entry with a registry-computed `host_confirmed` boolean: whether the named host's own `adagents.json` affirmatively names the owning publisher in a `collections` selector that reaches the claimed properties. The flag is never publisher-authored (publisher-supplied values are stripped at projection) and is recomputed when the owner's file is re-crawled, so its freshness is bounded by crawl cadence. `false` covers both "the host does not corroborate this carriage" and "the registry has not crawled the host." It labels discovery data only — enforcement-grade verification still goes through the [supply-path verifier](/docs/governance/property/adagents#owner-sold-channel-carriage) or the authoritative files.
