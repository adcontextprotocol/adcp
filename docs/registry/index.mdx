...
Short-lived, user-scoped. Best for agent clients (MCP, AI assistants, custom apps) where a human is signing in or logging in to AAO. A single token works against both `/mcp` and the REST API.
