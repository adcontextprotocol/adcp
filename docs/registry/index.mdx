...
**Per-metric catalog discovery.** Each measurement agent publishes its full per-metric catalog at [`get_adcp_capabilities.measurement.metrics[]`](/docs/protocol/get_adcp_capabilities#measurement) — the canonical, vendor-controlled source of truth (parallels every other AdCP agent type's capability surface). For coarse pre-call filtering, agents MAY also declare `metric_categories[]` on their `brand.json` `agents[type='measurement']` entry — parallels rights agents' `available_uses[]`. Buyers query the AAO federated index for cross-vendor aggregation (`/api/registry/measurement-vendors?category=attention&metric_id=attention_units`); AAO crawls each measurement agent's `get_adcp_capabilities` on a TTL to keep the index fresh. The aggregating endpoint is tracked at [#3613](https://github.com/adcontextprotocol/adcp/issues/3613); buyer-agent direct-vs-index call patterns are documented at [#3614](https://github.com/adcontextprotocol/adcp/issues/3614).
