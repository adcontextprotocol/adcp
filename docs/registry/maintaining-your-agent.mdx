...
|---|---|
| **Legacy** | The current production grading and badge behavior. This remains authoritative during the read-only comparison rollout. |
| **Spec** | Strict v3 grading. Required evidence must be complete; missing or ambiguous evidence never counts as a pass. |
| **Sandbox** | Production-lifecycle grading that excludes only catalog-proven compliance-controller gaps. All ambiguous gaps and failures observable on the production endpoint still fail closed. |

This comparison is passive. Opening the dashboard does not contact your agent, enqueue another compliance run, or change its badge. The first rollout phase presents an **agent-wide preview** from one source run, including the concrete compliance bundle, evaluator policy, assessment time, and stale or incomplete evidence. It is not an exact badge-role grade and cannot be selected. Legacy continues to control every public registry and badge result while the role-specific data model and Sandbox exception catalog are reviewed.

Grading profiles are separate from badge `verification_modes`. Modes such as `spec` and `live` describe what kind of verification evidence a badge carries; `live` remains an independent signal. The Legacy grading profile is a compatibility policy, not a verification mode and not a replacement for `live`.
