...
AdCP's role is **Article 25 data-governance provider**: the protocol exposes structured fields that let the deployer discharge its Annex III obligations. AdCP does not itself perform conformity assessment, DPIA, or human-oversight workflow — those are the deployer's.
...
- [`eu_ai_act_annex_iii`](/docs/governance/policy-registry#seeded-policies) — the seeded registry policy
- [Policy registry](/docs/governance/policy-registry) — `requires_human_review` on policies and categories
- [Campaign governance specification](/docs/governance/campaign/specification) — `human_review_required` plan field
- [Embedded human judgment](/docs/governance/embedded-human-judgment) — the architectural principle behind mandatory oversight
