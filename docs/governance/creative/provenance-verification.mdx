...
Each entry on `embedded_provenance` and `watermarks` MAY include a `verify_agent` pointer at a governance agent that can verify the embedding via `get_creative_features`. This is an AdCP-native verification surface — not a vendor-specific webhook. The agent declares its features through `get_adcp_capabilities`; receivers invoke `get_creative_features` against the agent and read the returned feature value. Receivers MAY substitute a different governance agent that declares an equivalent feature.
...
Sellers express what they need on `creative_policy`, returned by `get_products` so buyers see requirements before submitting. Two layers:
...
| `provenance_requirements.{require_digital_source_type, require_disclosure_metadata, require_embedded_provenance}` | Field-level requirements. Submission missing the named field is rejected with the corresponding `PROVENANCE_*_MISSING` code. |
...
| `PROVENANCE_CLAIM_CONTRADICTED` | Independent verifier contradicts the claim (active refutation, not absence) | The provenance field whose claim was refuted; `error.details` SHOULD carry verifier `agent_url`, `feature_id`, and result |
...
| Nominate a verifier | Populate `verify_agent.agent_url` (and optionally `feature_id`) on `embedded_provenance[]` / `watermarks[]` entries so the seller can verify via `get_creative_features` against an AdCP governance agent |
| Read seller requirements upfront | Inspect `creative_policy.provenance_required` and `creative_policy.provenance_requirements` from `get_products` and pre-flight your creative shape before calling `sync_creatives` |
| Handle rejection codes | Treat `PROVENANCE_REQUIRED`, `PROVENANCE_*_MISSING`, and `PROVENANCE_CLAIM_CONTRADICTED` as machine-readable correctable errors — auto-retry without correction will not pass |
...
| Run AI detection | Call `get_creative_features` against the buyer's nominated `verify_agent` (or your own AdCP governance agent) to verify provenance claims |
| Compare claims vs detection | Implement logic to compare `digital_source_type` against `ai_generated` feature results |
| Reject contradictions | Use `PROVENANCE_CLAIM_CONTRADICTED` (with verifier identity in `error.details`) when an independent verification refutes the buyer's claim |
