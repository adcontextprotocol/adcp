...
Webhooks contain a summary only — recipients must call `get_collection_list` for the updated entries. Recipients MUST verify the delivery's RFC 9421 `Signature`, `Signature-Input`, and `Content-Digest` HTTP headers before processing, following the [webhook profile](/docs/building/by-layer/L1/security#webhook-callbacks). New senders omit the deprecated body-level `signature` field; receivers ignore it if an older 3.x sender includes it. Recipients MUST also dedupe by `idempotency_key` so retried deliveries of the same change event are ignored.
...
**Webhook signature algorithm.** The webhook signature MUST follow the RFC 9421 [webhook callbacks profile](/docs/building/by-layer/L1/security#webhook-callbacks): the governance agent signs with a webhook-valid key published at the `jwks_uri` of its `agents[]` entry in its own brand.json. New keys use `adcp_use: "request-signing"`; existing `webhook-signing` keys remain accepted during the compatibility window. The subscribing seller verifies covered components `@method`, `@target-uri`, `@authority`, `content-type`, `content-digest`, with `tag="adcp/webhook-signing/v1"`. Collection-list registration exposes only `webhook_url`, so there is no HMAC/Bearer mode selector and no header-sniffing fallback: these callbacks are RFC 9421-only. Any deprecated body `signature` member is an ignored compatibility value, not a copy that participates in authentication.
