...
**Webhook signature algorithm.** The webhook signature MUST follow the [standard webhook signing rules](/docs/building/implementation/security#webhook-security): the RFC 9421 webhook profile by default (governance agent signs with its `adcp_use: "webhook-signing"` key published in `adagents.json`; the subscribing seller verifies against the agent's JWKS), or the deprecated HMAC-SHA256 fallback when the subscribing seller populates `authentication.credentials` on the webhook registration. For the legacy HMAC path, any body `signature` field is a convenience copy — recipients MUST verify against the headers and MUST NOT trust the body value.
