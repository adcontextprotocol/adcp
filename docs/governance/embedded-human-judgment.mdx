...
|--------------|-----------------|-----------------|
| Humans define boundaries | `authority_level` and `reallocation_threshold` on plans | [`sync_plans`](/docs/governance/campaign/tasks/sync_plans) |
...
| Judgment cannot be delegated to software | `authority_level: human_required` forces async human review — `check_governance` goes async and resolves to `approved` or `denied` | [`sync_plans`](/docs/governance/campaign/tasks/sync_plans), [`check_governance`](/docs/governance/campaign/tasks/check_governance) |
