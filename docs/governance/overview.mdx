...
**Community governance agent** -- AgenticAdvertising.org operates a reference governance agent that implements the policy registry's seeded policies. It provides baseline compliance checks, basic budget guardrails, and property list validation. Brands can use it as a zero-setup entry point for governance or as a testing endpoint for agent developers integrating governance support.
