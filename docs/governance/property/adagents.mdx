...
In programmatic, `sellers.json` is hosted by the seller/exchange and declares which publishers they represent. AdCP handles this through `brand.json` instead of a separate file. An operator declares managed or represented properties in their [`brand.json`](/docs/brand-protocol/brand-json) using the `relationship` field on each property. This creates the same bilateral verification pattern:
