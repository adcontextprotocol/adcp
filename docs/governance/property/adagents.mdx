...
Publishers MUST populate `signing_keys` for any authorized agent whose delegated scopes include mutating operations (for example, sales agents able to create or modify media buys). Leaving `signing_keys` empty in that case reduces the trust chain to counterparty-controlled `jwks_uri` discovery and forfeits the publisher's pin as a cross-check.
