...
- **Explicit publisher scoping required**: a manager-hosted `adagents.json` MUST explicitly mention the source publisher domain. The publisher must appear on the *authorization path* — under `authorized_agents[].publisher_properties[].publisher_domain`, inside `authorized_agents[].publisher_properties[].publisher_domains[]` (the compact managed-network form), or under `authorized_agents[].collections[].publisher_domain`. A `publisher_domain` value that only appears inside the file's top-level `properties[]` array, or that is reached only by resolving a top-level `authorization_type: property_tags` selector through `properties[]`, does **not** satisfy this rule. If the source publisher is not explicitly scoped on an authorization path, fallback MUST fail closed.
