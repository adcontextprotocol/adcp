...
- MAY attach a pinned `reference_renderer` (`runtime: "browser-esm"`, `package`, exact `version`, named `export`, and SRI `integrity`) to a `formats[]` entry. The package is a browser-safe, community-maintained, non-authoritative reference presentation. The registry references but never serves executable code; consumers require npm provenance, verify integrity, sandbox execution, and label the result as community-rendered. Non-JavaScript clients use hosted `preview_creative` or show the manifest.
