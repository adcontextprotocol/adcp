...
1. Read `ads.txt` and parse `managerdomain` entries.
   - **IAB directive form only**: `MANAGERDOMAIN=example.com`. Comment-prefixed lines (`# managerdomain=…`) are not recognized — they are publisher notes, not live directives.
...
- **Ambiguity fails closed**: when multiple eligible managerdomain entries remain after `#noagents` filtering, clients MUST abort fallback rather than guessing which manager to trust. This is intentionally stricter than IAB ads.txt 1.1, which uses last-wins semantics for duplicate `MANAGERDOMAIN` lines. AdCP fails closed because a wrong manager selection silently authorizes an incorrect agent across the publisher's entire inventory.
- **No success by silence**: if the manager lookup fails, treat the publisher as missing `adagents.json` (same as no fallback).
- **Cache ads.txt fetches**: validators SHOULD cache `ads.txt` responses (4 hours for success, 1 hour for 404/error) and apply per-host backoff to avoid amplification on repeated publisher 404s.
