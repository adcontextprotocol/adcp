...
#### Owner-sold FAST channel carriage

Model a FAST or virtual linear channel as a `kind: "channel"` collection declared
by the channel owner. The host CTV app remains a property. When the owner sells
its allocated avails, the host attests that supply path by authorizing the
owner's sales agent for the host property and narrowing the authorization to the
owner's canonical collection:
...
`seller_inline` product placements. A host adds `placement_ids` or
`placement_tags` only when its authorization must be position-specific.
...
missing host authorization fails closed.
