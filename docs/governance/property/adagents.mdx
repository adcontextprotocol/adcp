...
- MAY attach a pinned `reference_renderer` (`runtime: "browser-esm"`, `package`, exact `version`, named `export`, and SRI `integrity`) to a `formats[]` entry. The package is a browser-safe, community-maintained, non-authoritative reference presentation. Compatibility binds the named export's documented contract to this format entry's revision; package major versions are independent because one artifact may expose exports for several formats or revisions. The registry references but never serves executable code; consumers require npm provenance, verify integrity, sandbox execution, and label the result as community-rendered. Non-JavaScript clients use hosted `preview_creative` or show the manifest. On a confirmed vulnerability, registry maintainers MUST rotate `version`, `integrity`, and `catalog_etag` through a reviewed update; consumers that identify the pin as unsafe MUST NOT execute it and instead continue the rendering fallback order.
