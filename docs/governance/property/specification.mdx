...
- `REFERENCE_NOT_FOUND`: Referenced property, policy, or property list does not exist or is not accessible by the caller. Returned uniformly for both "does not exist" and "exists but unauthorized" — see [Uniform response for inaccessible references](/docs/building/by-layer/L3/error-handling#standard-error-codes).
...
- See the [get_adcp_capabilities task reference](/docs/protocol/get_adcp_capabilities) for capability discovery
