...
**Unknown Records Are Valid**: For both property and authorization checks, unknown records are excluded from rate calculations. You cannot penalize for detection gaps or unreachable publisher files.

**Rate Calculations**:
```
compliance_rate = compliant_impressions / (total_impressions - unknown_impressions)
authorization_rate = authorized_impressions / (checked_impressions - unknown_impressions)
```
