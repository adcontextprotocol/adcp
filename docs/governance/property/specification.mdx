...
**Unknown Records Are Valid**: When a governance agent cannot resolve an identifier (e.g., client-side JavaScript detection didn't fire, or the identifier type is outside the agent's coverage), the record receives `unknown` status. Unknown records are excluded from compliance rate calculations - you cannot penalize for detection gaps.
