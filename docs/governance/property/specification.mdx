...
**Unknown Records Are Valid**: For both property and authorization checks, unknown records should be excluded when calculating rates. You cannot penalize for detection gaps or unreachable publisher files.

**Response Format**: The response returns raw counts (compliant, non_compliant, unknown impressions). Consumers calculate rates as needed, typically excluding unknown impressions from the denominator.
