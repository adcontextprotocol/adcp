...
- The network SHOULD retain each `revoked_publisher_domains[]` entry for at least 7 days after `revoked_at` so any cached copy of the previous file is overridden before the entry ages out.
