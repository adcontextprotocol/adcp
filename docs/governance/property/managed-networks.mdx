...
- Cap response size (recommended: reject > 20 MB) and enforce short connect/read timeouts (≤ 10s each). This is intentionally higher than the [general SSRF body cap](/docs/building/by-layer/L1/security#webhook-url-validation-ssrf) (5 MB) because a single authoritative file fans out across an entire publisher network and routinely needs to enumerate thousands of properties or publisher domains.
