...
- `countries_all`: Property must be compliant in ALL listed countries (different jurisdictions have different laws)
...
Use `get_property_list` to retrieve the list. You'll receive an `auth_token` that can be shared with sellers:
...
Response includes an auth token for sharing. Note that **raw scores are not returned** - only properties that pass the filter thresholds are included:
...
Pass the `auth_token` to sellers so they can fetch and cache the resolved list.
