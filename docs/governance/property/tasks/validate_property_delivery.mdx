...
| `unmodeled_records` / `unmodeled_impressions` | Identifier recognized but no data available |
...
| `unmodeled` | Identifier recognized but governance agent has no data for it |
| `unidentified` | Identifier type not resolvable by this agent |

## Understanding Unmodeled vs Unidentified

These two statuses distinguish different types of "unknown" scenarios:

**`unmodeled`** - The governance agent recognized the identifier (e.g., it's a valid domain) but doesn't have data for that specific property. This happens when:
- A property is too new to be in the agent's database
- The property exists but hasn't been scored/evaluated yet
...
unverifiable = summary.unmodeled_impressions + summary.unidentified_impressions
...
- Unmodeled + Unidentified impressions: 50 (excluded)
