...
Push campaign plans to the governance agent. A plan defines the authorized parameters for a campaign -- budget limits, channels, flight dates, and authorized markets -- and serves as the source of truth for all validation. Compliance policies are resolved from the brand's configuration, not carried in the plan.
...
The plan specifies campaign context -- budget, channels, flight dates, and authorized markets. Compliance policies are not carried in the plan; they are resolved from the brand's [compliance configuration](/docs/governance/campaign/specification#brand-compliance-configuration). This ensures that the buying team operates within policies defined by the brand's policy team, regardless of which orchestrator or agency executes the campaign.

`compliance.jurisdictions` serves two purposes:

1. **Geo authorization** -- The governance agent rejects media buys targeting outside the authorized markets. A plan with `jurisdictions: ["US-MA"]` blocks buys that don't explicitly target Massachusetts.
2. **Policy resolution** -- The agent finds all policies whose jurisdictions overlap with the plan's markets. A plan targeting `["US"]` is subject to all US federal and state-level policies. A plan targeting `["US-MA"]` is only subject to Massachusetts-specific and federal policies.

Subdivision codes (ISO 3166-2) allow fine-grained market authorization. A pharma campaign running nationally uses `["US"]`; a cannabis campaign limited to legal states uses `["US-CO", "US-CA", "US-MA"]`.
...
| `plans[].compliance.jurisdictions` | array | No | ISO 3166-1 alpha-2 country codes (or ISO 3166-2 subdivisions like `US-MA`) for the markets this campaign is authorized to run in. Serves two purposes: geo authorization (reject buys targeting outside these markets) and policy resolution (find policies whose jurisdictions overlap). |
