...
| `plans[].governed_actions[].purchase_type` | enum | `media_buy`, `rights_license`, `signal_activation`, `creative_services`, or `measurement_verification`. |
...
| `plans[].entries[].purchase_type` | enum | `media_buy`, `rights_license`, `signal_activation`, `creative_services`, or `measurement_verification`. Present when the entry is associated with a specific governed action. |
