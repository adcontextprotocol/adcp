...
| `plans[].entries[].categories_evaluated` | array | Governance categories evaluated (e.g., `budget_authority`, `regulatory_compliance`). A single account-bound governance agent composes internal specialist review (legal, brand safety, category-specialist reviewers) behind one endpoint — governance agents MAY surface which internal reviewers or workflows touched the check via `categories_evaluated` or `findings[].details`, preserving auditability of the internal decomposition without exposing it as separate wire-level agents. |
