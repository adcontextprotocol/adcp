...
| `PLAN_HASH_MISMATCH` | correctable | The SHA-256 of the JCS-canonicalized plan bytes referenced by the request does not match the `plan_hash` claim on the accompanying `governance_context` token. Re-run `check_governance` against the current plan revision to obtain a fresh token. |
| `UPDATE_REQUIRES_GOVERNANCE` | correctable | An `update_media_buy` changed a price-affecting field (`bid_price`, `budget`, `daily_cap`, flight-date extension, or targeting broadening) without carrying a fresh `modification`-phase `governance_context` whose `plan_hash` covers the post-update plan state. Obtain a new `check_governance` approval and retry. |
