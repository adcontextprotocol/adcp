...
The seller calls with `binding: "committed"` when processing a request on an account that has a `governance_agent` (set during [`sync_accounts`](/docs/accounts/tasks/sync_accounts)). This is a binding check -- budget will be committed if approved. The seller presents the authentication credentials from the `governance_agent` configuration.
...
| `findings` | array | Per-category issues found. Present when status is `denied`, `conditions`, or `escalated`. MAY also be present on `approved` for advisory findings. |
| `conditions` | array | Present when status is `conditions`. Adjustments the caller must make before re-calling. |
| `escalation` | object | Present when status is `escalated`. Contains `reason`, `severity`, and `requires_human`. |
