...
| `governance_context` | string | No | Governance context token from a prior `check_governance` response. Include on subsequent lifecycle checks so the governance agent can maintain continuity. On execution checks, the governance agent uses `governance_context` + `planned_delivery` to identify the check. This is the sole lifecycle correlator across all purchase types. In 3.0 the value SHOULD be a compact JWS per the [AdCP JWS profile](/docs/building/implementation/security#signed-governance-context); 3.1 will require it. |
...
| `governance_context` | string | Governance context token for this governed action. Present when status is `approved` or `conditions`. Attach to the protocol envelope and include on all subsequent governance calls. This is the sole lifecycle correlator for all purchase types. In 3.0 SHOULD be a compact JWS per the [AdCP JWS profile](/docs/building/implementation/security#signed-governance-context) (3.1: MUST). Sellers that implement verification MUST verify before acting on the token; sellers that do not MUST still persist and forward it verbatim. |
