...
Report the outcome of an action to the governance agent. This is the "after" half of the governance loop -- it tells the governance agent what actually happened so it can update its state and flag any issues.
...
If the seller rejected the request, report it so the governance agent can release the reserved budget:
...
| `action_id` | string | No | The `action_id` from `validate_plan_action`. Links the outcome to the validated request. Required for `completed` and `failed` outcomes. |
...
| `delivery.impressions` | number | No | Impressions delivered in the period. |
...
| `ACTION_NOT_FOUND` | correctable | No validated action with this `action_id`. |
...
- [`validate_plan_action`](./validate_plan_action) -- The "before" half: validate an action before executing it
