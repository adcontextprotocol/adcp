...
The plan owner may submit a new observation for the same seller statement and `reporting_period` with `period_closed: true`. If the values still disagree, the response makes the limited operational meaning explicit:
...
| `delivery.period_closed` | boolean | No | Defaults to `false`. The authenticated plan owner may close this operational governance period. An unresolved close is retained as `closed_unresolved`; closure is not billing settlement. |
