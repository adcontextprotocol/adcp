...
The orchestrator pushes campaign plans via [`sync_plans`](/docs/governance/campaign/tasks/sync_plans), then calls [`check_governance`](/docs/governance/campaign/tasks/check_governance) with `tool` + `payload` before every action it sends to a seller. The seller independently calls `check_governance` with `media_buy_id` + `planned_delivery` before executing. After receiving the seller's response, the orchestrator calls [`report_plan_outcome`](/docs/governance/campaign/tasks/report_plan_outcome) to close the loop. The governance agent tracks budget from confirmed outcomes -- not just attempted actions.
...
flowchart LR
    ...
    seller -->|"4. check_governance<br/>(media_buy_id + planned_delivery)"| gov
...
3. **Check**: If the account has `governance_agents`, the seller calls `check_governance` with `media_buy_id` and `planned_delivery` — what it will actually deliver. The governance agent approves or denies.
