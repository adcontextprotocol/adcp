...
3. **Check**: If the account has a `governance_agent`, the seller calls `check_governance` with `binding: "committed"` and its `planned_delivery` -- what it will actually run. The governance agent approves or denies.
...
## Community governance agent

AgenticAdvertising.org operates a community governance agent that implements the registry's seeded policies. Brands that do not need a custom governance agent can point their `governance_agent` URL to the community agent for baseline policy compliance checks and basic budget guardrails. This provides a zero-setup entry point for governance without requiring brands to build or procure a dedicated agent.

The community agent supports:
- Policy compliance checks against all registry policies
- Basic budget threshold guardrails (reject or escalate above configurable limits)
- Property list validation against the property registry
- Geo and vertical alignment checks

For full budget tracking, delivery monitoring, and custom validation logic, brands should use a dedicated governance agent.
