...
Campaign Governance fills this gap with three mechanisms: **plans** that define what's authorized (independent of the agent), **seller-side authorization webhooks** that verify purchases independently of the buyer, and a **policy registry** that standardizes compliance rules across governance agents.
...
flowchart LR
    ...
    seller -->|"4. verify_authorization<br/>(planned_delivery)"| gov
...
- The **seller** independently verifies purchases by calling the buyer's authorization agent with its planned delivery parameters (seller-side authorization verification).
- The **authorization agent** validates both the orchestrator's intended actions and the seller's planned delivery against the same campaign plan.

This creates a trust model where neither the buyer nor the seller can unilaterally misrepresent what was authorized. The orchestrator cannot skip governance (the seller verifies independently), and the seller cannot deliver something different from what was authorized (the authorization agent has a record of the planned delivery).
...
3. **Verify**: If the account has an `authorization_agent`, the seller calls it with its `planned_delivery` -- what it will actually run. The authorization agent confirms or denies.
...
1. **Authorization verification**: The seller sends `planned_delivery` to the buyer's authorization agent, which confirms it matches the campaign plan. This prevents the seller from delivering something the buyer didn't authorize.
...
Authorization webhooks solve this by telling the seller: "call this URL when things happen." The buyer registers an `authorization_agent` URL (with credentials) when syncing accounts. When the seller receives a `create_media_buy`, it POSTs to the authorization agent with what it plans to deliver. The authorization agent checks against the plan and returns `authorized`, `denied`, or `conditions`.

This also catches misinterpretation. If the brief says "Mexico" and the seller interprets it as "New Mexico," the authorization agent sees the geo mismatch against the plan and denies the buy -- before it goes live.

The webhook covers the full media buy lifecycle:

- **Purchase**: POST before confirming `create_media_buy` -- is this buy authorized?
...
The authorization agent maintains all state. The seller just posts what's happening -- no chaining, no conversation history, no state to track across servers.

Authorization is optional at every phase. Sellers can start with purchase-only (one POST per buy) and add modification and delivery checks incrementally. See the [specification](/docs/governance/campaign/specification#authorization-verification) for the full protocol.
