...
4. Intersect with the plan's `compliance.jurisdictions` -- only policies applicable to the authorized markets are active. Supports both ISO 3166-1 alpha-2 country codes (`US`, `GB`) and ISO 3166-2 subdivisions (`US-MA`, `US-CA`) for state/region-level targeting
5. Include all custom brand policies (these apply regardless of jurisdiction)

The `compliance.jurisdictions` field also serves as **geo authorization**: the governance agent MUST reject media buys targeting markets outside the plan's authorized jurisdictions. A plan with `jurisdictions: ["US-MA"]` rejects buys not explicitly targeting Massachusetts, even if they are otherwise compliant.
