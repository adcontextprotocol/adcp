...
When `governance_agent` is absent, the seller processes media buy requests normally. The buyer-side governance loop (`validate_plan_action` → execute → `report_plan_outcome`) still applies, but there is no seller-side verification.
...
If the governance agent is unreachable (timeout, network error), the seller SHOULD proceed with the media buy and log the failure. Governance checks should not block ad delivery when governance infrastructure is unavailable. The seller SHOULD retry verification on the next scheduled check.
...
| Concern | Buyer-side (`validate_plan_action`) | Seller-side (`check_governance`) |
|---------|--------------------------------------|---------------------------------------|
| **Who checks** | Buyer's governance agent | Buyer's governance agent, called by seller |
...
flowchart TD
    ...
    X --> Y["Seller calls check_governance (modification)"]
...
The governance agent is a synchronous call in the orchestrator's action loop. Governance checks happens on the seller side and is transparent to the orchestrator -- the orchestrator sends the same `create_media_buy` request regardless of whether governance checks is configured. Modification and delivery phase checks happen between the seller and governance agent, independent of the orchestrator's governance loop.
