...
### When authorization is not available

Authorization verification is optional at every phase. If the account has no `authorization_agent`, the seller processes media buy requests normally. The buyer-side governance loop (`validate_plan_action` → execute → `report_plan_outcome`) still applies.

Sellers SHOULD NOT require authorization verification -- it is a trust enhancement, not a prerequisite. A seller that refuses to process media buys without an `authorization_agent` would break interoperability with buyers who do not use Campaign Governance.

Similarly, the `delivery` phase is optional even when `purchase` authorization is used. A seller MAY support purchase authorization without ongoing delivery reporting. The authorization agent indicates whether it expects delivery reports through the presence of `next_check` in the purchase response.
