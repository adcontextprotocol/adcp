...
The `check_governance` schema reserves an `actions` array field for batch validation. An orchestrator building a multi-seller media plan can submit multiple proposed actions as a group, allowing the governance agent to evaluate aggregate constraints (total budget across all actions, channel mix across the set) in a single round-trip.

The response includes a corresponding `results` array with one result per action, in the same order. Each result follows the same structure as the single-action response. Until batch support is widely implemented, orchestrators call `check_governance` sequentially for each action.

## Property list governance

Campaign governance intersects with property governance when media buys reference property lists. A governance agent MAY validate that property lists referenced in media buy requests meet the plan's brand safety and compliance requirements. The community governance agent integrates with the property registry to provide this validation, ensuring that property lists align with the brand's compliance configuration and enforced policies.
