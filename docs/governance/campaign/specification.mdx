...
The buyer registers governance agents via [`register_governance`](/docs/accounts/tasks/register_governance), pairing each account with the governance agent endpoints to call. Each agent includes authentication credentials so the governance agent can verify the seller's identity:

```json
{
  "tool": "register_governance",
  "arguments": {
    "accounts": [
      {
        "account": {
...
The `governance_context` field is an opaque string issued by the governance agent in the `check_governance` response. It enables lifecycle continuity — the governance agent encodes whatever internal state it needs (plan reference, budget snapshot, check history) into this value.
...
Accounts MAY register multiple governance agents via [`register_governance`](/docs/accounts/tasks/register_governance), each responsible for different validation categories. For example, one agent handles budget authority and strategic alignment while another handles regulatory compliance and brand policy.
