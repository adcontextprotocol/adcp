...
<Note>
**Concurrency**: An orchestrator may send `create_media_buy` requests to multiple sellers simultaneously, each triggering a `committed` check. The governance agent SHOULD use optimistic concurrency control or budget reservation to prevent concurrent approvals that together exceed the plan budget.
</Note>
...
- **Low confidence (<0.6)**: The finding is speculative. The governance agent flags it for human review rather than acting on it autonomously.
...
The seller stores this endpoint and presents the credentials when calling `check_governance`. The governance agent MUST verify that the Bearer token matches a registered credential for the account associated with the `plan_id`, and MUST reject requests with unrecognized or mismatched credentials.

### Governance modes

The `governance_agent` object accepts an optional `mode` field that controls how the governance agent treats findings:
...
| `advisory` | Return findings with real statuses (`denied`, `conditions`, `escalated`), but the seller treats all responses as non-blocking. | Post-hoc human review. The governance agent expresses opinions; humans act on them. |
| `enforce` | Block on `denied` or `escalated`. Require resolution before proceeding. | Production governance. The default. |

Default is `enforce` for backward compatibility. Organizations can start in `audit` mode to build confidence in the governance agent's calibration before enabling enforcement. The mode can be changed per-account by re-syncing with `sync_accounts`.
...
In `advisory` mode, the governance agent returns the real status it would have returned under enforcement. The check response includes a `mode` field so sellers and auditors can distinguish "denied in advisory mode (action proceeded)" from "denied in enforce mode (action blocked)."
...
The seller calls the governance agent at the registered `governance_agent.url` using MCP over HTTP (Streamable HTTP transport). The request is an MCP `tools/call` invocation with tool name `check_governance` and the request arguments as the tool input. Authentication uses the Bearer token from `governance_agent.authentication.credentials` in the `Authorization` header.
...
When `governance_agents` is present, it takes precedence over the singular `governance_agent` field. The singular field remains as a convenience for accounts with a single governance agent.
...
The buyer-side and seller-side governance checks MAY be handled by the same agent or by separate agents. The protocol does not prescribe the relationship -- only that the seller can call the `governance_agent` URL registered on the account.
