...
| Concern | Intent checks (orchestrator, `tool` + `payload`) | Execution checks (seller, `media_buy_id` + `planned_delivery`) |
|---------|--------------------------------------|---------------------------------------|
...
flowchart TD
    ...
    X --> Y["Seller calls check_governance(media_buy_id, phase: modification)"]
