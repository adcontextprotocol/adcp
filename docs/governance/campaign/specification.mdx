...
When delegations are present, the governance agent first resolves the requesting agent URL from the authenticated transport credential. It rejects the check when that identity is unavailable or does not exactly match the request's `caller` assertion. Only then does it match the authenticated agent URL to a delegation's `agent_url`. A request body cannot establish or switch agent identity. An agent requesting a media buy in France must have a delegation that includes France in its `markets`. An agent with `execute_only` authority cannot reallocate budget between channels.

When delegations are absent, the governance agent does not restrict which agents can act on the plan. If an authenticated agent URL is available, it still MUST match `caller` so audit records and signed contexts cannot attribute the check to a different agent. The same authenticated-identity rule applies before evaluating `approved_sellers`; a non-null list is a restricted plan and fails closed when the agent URL cannot be resolved from the credential.
