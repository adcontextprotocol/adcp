...
An account binds to exactly one governance agent per [`sync_governance`](/docs/accounts/tasks/sync_governance). Registration is single-agent by schema (`governance_agents` is an array for forward compatibility but MUST contain exactly one entry today); the envelope carries a single `governance_context` token; all lifecycle calls route to that one agent.
