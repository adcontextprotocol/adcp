...
Both are produced from [`get_plan_audit_logs`](/docs/governance/campaign/tasks/get_plan_audit_logs). The split is a matter of filtering and field selection, not a separate protocol.

## Why split at all

Three reasons buyers maintain distinct views:

1. **Strategy leakage.** Total authorized budget, channel allocation, and remaining headroom across all sellers are competitive signals. A single seller does not need to know the buyer's full plan to verify their own buy.
...
The table below classifies every field in the [`get_plan_audit_logs`](/docs/governance/campaign/tasks/get_plan_audit_logs#response) response by who can safely see it.
...
| `plans[].summary.drift_metrics.*` | yes | no | on request |
...
`mode` is set on the plan via [`sync_plans`](/docs/governance/campaign/tasks/sync_plans) and is the operator's primary lever. The same caller, same payload, same finding produces three different outcomes depending on mode.
...
- **`plan_hash` is verifiable.** It is `base64url_no_pad(SHA-256(JCS(plan_payload)))` over the plan revision the check was evaluated against. Any party with the plan revision can recompute and byte-compare the digest. See [Plan binding and audit](/docs/governance/campaign/specification#plan-binding-and-audit).
...
- [`get_plan_audit_logs`](/docs/governance/campaign/tasks/get_plan_audit_logs) — request and response schema
- [Campaign governance specification](/docs/governance/campaign/specification) — plan binding, drift detection, governance context lifecycle
- [Policy Registry](/docs/governance/policy-registry) — policy versioning, `effective_date`, registry vs inline policies
