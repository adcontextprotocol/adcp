...
sequenceDiagram
    ...
    Seller->>Buyer: Delivery records (get_media_buy_delivery)
    Buyer->>Verifier: validate_content_delivery (sampled)
    Verifier-->>Buyer: Validation results
```

**Key insight**: Runtime decisioning happens locally at the seller (for scale). Delivery records flow through the buyer, who validates samples against the verification agent.
