...
6. Sales agent pushes artifacts via webhook (or orchestrator polls)
...
5. **Push artifacts** to the orchestrator for validation
...
If you can't - for example, the brand requires competitive separation but you don't support it, or they need adjacency data for a channel where it doesn't apply (like billboards) - reject the buy.
...
def validate_content_delivery(standards_id: str, records: list) -> dict:
    ...
    for record in records:
        features = []
        for feature in ["brand_safety", "brand_suitability", "competitive_separation"]:
