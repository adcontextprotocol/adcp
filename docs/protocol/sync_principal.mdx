...
2. **Read [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities).** Discover `adcp.principal`, its supported sections, event and delivery offerings, and limits.
3. **Call `sync_principal`.** Declare consumption facts (`declarations`), register webhook subscribers (including `principal.changed` and, with explicit scope, account-anchored events), and register reusable reporting destinations.
4. **Authorize advertiser accounts separately** through [`sync_accounts`](/docs/accounts/tasks/sync_accounts) and account-level tasks. Nothing in steps 1–3 grants account authority.
5. **Bind each account's feeds** to the reusable `destination_ref`s.
6. **Observe readiness.** [`get_principal`](/docs/protocol/get_principal) shows destination and declaration state; `principal.changed` keeps it fresh.
