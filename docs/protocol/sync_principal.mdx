...
| Tier | Entity | Surface |
|---|---|---|
| 1 | **Principal** — who you are, resolved from authenticated transport | `sync_principal` / [`get_principal`](/docs/protocol/get_principal) |
| 2 | The principal's standing configuration with this seller | carried on the principal record |
| 3 | **Accounts** — principal × advertiser authority, feeds, schedules | [`sync_accounts`](/docs/accounts/tasks/sync_accounts) and domain tasks |

[`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities) states what the seller can support; `sync_principal` states which of it this principal uses; account-level tasks authorize work for particular advertisers.
...
Every applied or current readback carries the seller-resolved `principal_kind` — `buyer_agent` or `operator` — resolved solely from the authorization system, never from request content, so per-party policy such as billing gates can rely on it. A request may carry `expected_principal_kind` as an assertion fence ("I believe I am authenticating as the agent"); on mismatch with the resolved kind the seller rejects with [`CONFLICT`](/docs/building/verification/compliance-catalog#error-code-conflict) before mutation. The assertion never influences resolution.
...
In the normal call sequence, a client reads capabilities first, calls `sync_principal` when advertised, and then configures or authorizes advertiser accounts. A destination can be registered before account setup, but it cannot receive account data until a later account-scoped feed binding succeeds.
...
- **Caller-anchored events** (`capabilities.changed`, `principal.changed`) fire principal-wide. `principal.changed` is the invalidation doorbell for seller-driven state transitions — a destination reaching `ready`, a setup action expiring, the accepted declarations intersection changing — and receivers repair by re-reading [`get_principal`](/docs/protocol/get_principal).
- **Account-anchored events** (`creative.status_changed`, `account.change_recorded`, `product.*`, …) may be listed on caller-level subscribers. Authorization is evaluated **at fire time**: each fire covers only accounts the caller is currently authorized for, so losing account authority silently stops that account's fires without touching the subscription. Registration itself requires no account authority and grants none.
- **Media-buy-anchored delivery reports** stay on each buy's `push_notification_config` — they carry per-buy cadence choices this layer deliberately does not own.

There is no wildcard. Callers enumerate `event_types` and may set `include_future_event_types: true` to opt into caller-eligible types added by later AdCP versions. Sellers advertise which types they can emit here in `principal.caller_event_types`; an out-of-offering type fails the request with [`UNSUPPORTED_FEATURE`](/docs/building/verification/compliance-catalog#error-code-unsupported-feature).
...
- Declarations are facts about consumption ability — never preferences or policy. [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities) stays objective seller facts; the intersection is the negotiated contract.
...
- An empty `webhook_signing_algorithms` intersection while any webhook subscriber is active fails the request with [`UNSUPPORTED_FEATURE`](/docs/building/verification/compliance-catalog#error-code-unsupported-feature) — fires would be unverifiable.
...
| [`UNSUPPORTED_FEATURE`](/docs/building/verification/compliance-catalog#error-code-unsupported-feature) | Section not in `supported_sections`, destination outside `reporting_destination_offerings`, subscriber event type outside `caller_event_types`, empty signing-algorithm intersection with an active subscriber, or `expected_configuration_version` sent to a seller advertising `optimistic_concurrency: false`. | Re-read capabilities and select from the advertised offering. |
| [`CONFLICT`](/docs/building/verification/compliance-catalog#error-code-conflict) | `expected_configuration_version` is stale. Nothing was mutated. | Read the current version with [`get_principal`](/docs/protocol/get_principal), reconcile, and resubmit under a new `idempotency_key`. |
...
The broader task does not turn [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities) into caller-specific state. Capabilities remain cacheable seller facts; principal configuration remains authenticated caller state.
