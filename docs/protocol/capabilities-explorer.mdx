...
- **`features`** — boolean feature flags. Currently `catalog_signals` (structured `signal_ref` references to signals published in `adagents.json` `signals[]`). **Additional signals capability flags belong here**, not in a new top-level key. (E.g., a `signal_enforcement_on_guaranteed` flag for direct-sold targeting belongs at `signals.features`, not under `media_buy.execution.trusted_match`.)
