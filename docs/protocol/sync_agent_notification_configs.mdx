...
This is the specialized compatibility surface. When the seller advertises [`sync_principal`](/docs/protocol/sync_principal), use its `notification_configs` section so webhooks and reusable connection resources share one versioned caller-scoped configuration. Sellers exposing both tasks maintain one underlying subscriber set.
...
When `registration_task` is [`sync_principal`](/docs/protocol/sync_principal), call that task instead.
