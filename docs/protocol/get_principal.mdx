---
title: get_principal
description: "Read the authenticated caller's principal configuration, version, and destination setup states."
"og:title": "AdCP — get_principal"
---

`get_principal` returns the authenticated caller's current durable principal configuration with one AdCP agent: the same caller-scoped state that [`sync_principal`](/docs/protocol/sync_principal) replaces, plus retained superseded and revoked destination generations.
...
| `result.kind` | Meaning |
|---|---|
| `current` | The caller has a persisted principal record. Returns `principal_id`, the seller-resolved `principal_kind` (`buyer_agent` or `operator`), `configuration_version`, and complete credential-free configuration state. |
| `unconfigured` | The authenticated caller has no persisted principal record with this agent. |
| `failed` | The read could not be served. Carries only structured errors, never identifiers or state. |

A caller with no principal record and a caller probing coordinates owned by someone else receive indistinguishable results: configuration state is visible only to its owning principal.
