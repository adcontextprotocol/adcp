...
```json
{
  ...
  "creative": {
    ...
    "delivery_variant_resolution": {
      "supported": true,
      "strategies": ["source_order", "highest_compatible_vast"]
    },
...
- **Delivery representation resolution**: Deterministic selection from a `CreativeSource`; absence of `delivery_variant_resolution` means callers must select before sending a seller-bound manifest
