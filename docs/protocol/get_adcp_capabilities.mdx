...
| `bidding_policy` | Structured canonical bidding support by `media_buy`/`package` scope, modes, strengths, and supported combinations |

`bidding_policy` is intentionally not a boolean. Each scope declares `modes` from `automatic`, `bid_amount`, `max_bid`, `cost_per`, and `roas`. A scope advertising `cost_per` also declares `cost_per_strengths`; `roas` similarly declares `roas_strengths`. Optional `supported_combinations` lists `max_bid_with_cost_per` and/or `max_bid_with_roas`. Presence of one scope or mode makes no claim about another.
