...
| `localization` | object | Presence advertises exact native localization on `sync_creatives`/`list_creatives` and therefore requires `has_creative_library: true`. `supported_locales[]` is the complete current BCP 47 set accepted for the authenticated caller; `translation_modes[]` declares `buyer_supplied` and/or `provider_generated`; `max_target_variants` narrows the protocol ceiling of 50; `review_scope` is `per_variant` or `creative`. Omit the object when unsupported. Advertised support commits the agent to exact locale/assets/identity/review/launch readback and fail-closed writes. |
