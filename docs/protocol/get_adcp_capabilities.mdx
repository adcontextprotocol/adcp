...
`media_buy.acceptance_policy_discovery` advertises structured preflight for what the seller will accept. `catalog_url` identifies an HTTPS `acceptance-policy-catalog.json` document, `catalog_digest` pins its bytes, `default_profile_ids[]` selects seller-wide profiles, and `acceptance_context: true` confirms the seller evaluates the buyer's structured category/facet, role, industry, advertiser-jurisdiction, and delivery-jurisdiction facts.
