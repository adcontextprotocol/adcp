...
Measurement agents publish a per-metric catalog so buyers know which metrics each vendor offers (attention scores, emissions, panel-based demographics, brand suitability, viewability, etc.). This is the canonical source of truth — AAO crawls it to populate the federated [measurement-vendor index](/docs/registry/index#measurement-vendor-discovery).
...
| `category` | enum | **Required.** Coarse classification: `attention`, `viewability`, `invalid_traffic`, `brand_safety`, `brand_lift`, `incrementality`, `audience`, `reach`, `creative_quality`, `emissions`, `outcomes`, `other`. Used by AAO and buyer agents for cross-vendor discovery. |
...
| `description` | string | Human-readable description of what the metric measures and any methodology notes. |
