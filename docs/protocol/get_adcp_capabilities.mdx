...
When `notifications.supported: true`, sellers MUST include `cache_ttl_seconds` and at least one revision marker (`capabilities_version` or `last_modified`). The webhook is the fast path; the TTL and revision marker are the recovery path when delivery is missed or delayed.

| Field | Type | Description |
|-------|------|-------------|
| `capabilities_version` | string | Opaque revision token for the full capability document. Sellers SHOULD change it whenever any material field in `get_adcp_capabilities` changes. Buyers compare for equality only. |
...
`capabilities.changed` webhooks are invalidation signals, not replacement documents. The payload identifies the agent, change time, reason, optional `capabilities_version`, and optional advisory `changed_paths[]`; receivers SHOULD re-run `get_adcp_capabilities` and replace their cached snapshot from the fresh response. The payload schema is [`/schemas/latest/core/capabilities-changed-webhook.json`](https://adcontextprotocol.org/schemas/latest/core/capabilities-changed-webhook.json).
...
Capabilities rarely change. Cache results for no longer than `adcp.capability_changes.cache_ttl_seconds` when present, compare `capabilities_version` or `last_modified` on refresh, and subscribe to `capabilities.changed` if the seller advertises notification support. Fall back to legacy `last_updated` only when `capability_changes` is absent.
