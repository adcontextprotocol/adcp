...
| `supported_formats` | object[] | Canonical creative operation catalog. Every entry has a stable `capability_id`, a full canonical `format` declaration, and required `operations` (`build`, `validate`, `preview`). Exact publisher support carries `{publisher_domain, format_option_id}` inside `format`; generic capabilities declare a satisfiable canonical parameter envelope. Replaces `list_creative_formats` in 3.2. |
