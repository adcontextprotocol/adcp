...
Declares support for authenticated caller-scoped connection configuration. This is not a second capability document: `get_adcp_capabilities` still describes seller facts, while [`sync_agent_configuration`](/docs/protocol/sync_agent_configuration) carries one caller's desired webhooks and reusable reporting destinations, and [`get_agent_configuration`](/docs/protocol/get_agent_configuration) reads them back.

| Field | Type | Description |
|---|---|---|
| `supported` | boolean | Always `true` when the block is present. |
| `sync_task` | string | Always `sync_agent_configuration`. |
| `read_task` | string | Always `get_agent_configuration`. Side-effect-free read of caller-scoped state and version. |
...
Agents exposing this block MUST list `protocol.agent_configuration` in `experimental_features` and expose both declared tasks. Account authorization remains separate; a ready reusable destination does not grant access to any advertiser account.
...
| `notifications`        | object    | Whether the seller supports `capabilities.changed` webhooks. When `supported: true`, register subscribers with the declared task: [`sync_agent_configuration`](/docs/protocol/sync_agent_configuration) when the broader connection surface is supported, or [`sync_agent_notification_configs`](/docs/protocol/sync_agent_notification_configs) for the specialized compatibility path. |
...
| `legacy_hmac_fallback` | boolean  | **Required when `supported: true`.** `true` iff the seller supports the legacy HMAC-SHA256 scheme when the buyer populates `push_notification_config.authentication.credentials`, `accounts[].notification_configs[].authentication.credentials`, `sync_agent_configuration.configuration.notification_configs[].authentication.credentials`, or `sync_agent_notification_configs.notification_configs[].authentication.credentials`. `false` is the recommended posture in 3.x — the HMAC scheme is removed in AdCP 4.0.                                                                                                                                                                          |
