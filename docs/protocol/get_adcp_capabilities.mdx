...
| Field | Type | Description |
|-------|------|-------------|
| `supported` | boolean | **Required.** Whether the seller deduplicates replays. When `false`, buyers MUST use natural-key checks (e.g., `get_media_buys` by `buyer_ref`) before retrying spend-committing operations. |
