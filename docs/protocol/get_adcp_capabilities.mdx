...
AdCP uses native MCP tool discovery and the [AdCP A2A Profile Extension v3](/docs/building/by-layer/L0/a2a-profile-extension) on A2A 1.0. **The presence of `get_adcp_capabilities` in an agent's tool or skill list indicates that runtime AdCP discovery is available.**
...
Declares whether this seller honors `idempotency_key` replay protection on mutations. AdCP requires keys on mutating requests whose published schema declares them required. In the compact AdCP 3.2 product lifecycle, [`request_proposals`](/docs/media-buy/task-reference/request_proposals), [`refine_proposals`](/docs/media-buy/task-reference/refine_proposals), and [`decline_proposals`](/docs/media-buy/task-reference/decline_proposals) require keys; [`list_products`](/docs/media-buy/task-reference/list_products) and the legacy 3.x [`get_products`](/docs/media-buy/task-reference/get_products) facade leave the key optional. Pure reads may ignore a supplied key, so this declaration does not promise read-response replay. See [security.mdx § Idempotency](/docs/building/by-layer/L1/security#idempotency). Clients MUST NOT assume a default; a seller without this block is non-compliant and should be treated as unsafe for retry-sensitive operations.
...
Declares support for authenticated caller-scoped connection configuration. This is not a second capability document: `get_adcp_capabilities` still describes seller facts, while [`sync_principal`](/docs/protocol/sync_principal) carries one caller's desired webhooks and reusable reporting destinations, and [`get_principal`](/docs/protocol/get_principal) reads them back.
...
| `notifications`        | object    | Whether the seller supports `capabilities.changed` webhooks. When `supported: true`, register subscribers with the declared task: [`sync_principal`](/docs/protocol/sync_principal) when the broader connection surface is supported, or [`sync_agent_notification_configs`](/docs/protocol/sync_agent_notification_configs) for the specialized compatibility path. |
...
`media_buy.anonymous_discovery` declares whether the seller accepts product-discovery calls without caller credentials. It covers [`list_products`](/docs/media-buy/task-reference/list_products) and [`get_products`](/docs/media-buy/task-reference/get_products) in `brief` or `wholesale` mode. It does not cover proposal refinement or finalization, purchasing, account-scoped reads, or mutations; those operations follow their normative authentication requirements.
...
- Absence means unspecified legacy behavior. Callers MAY probe anonymously and handle [`AUTH_MISSING`](/docs/building/verification/compliance-catalog#error-code-auth-missing); they MUST NOT infer either value.
...
`media_buy.availability_horizon: true` declares flexible-window availability discovery: the seller parses `offer_filters.availability_horizon` and answers with `time`-dimensioned forecast points carrying `availability_status` — half-open, non-overlapping windows that partition the requested horizon (gaps signalled via `incomplete[]`), with status computed from all booking eligibility constraints, not only competing holds. See [Flexible Availability Windows](/docs/media-buy/product-discovery/media-products#flexible-availability-windows). When `false` or absent, support is unknown: buyers SHOULD use exact `start_date`/`end_date` filtering instead, and sellers MAY ignore the field or reject it. Conformance storyboards gate flexible-window checks on this declaration.
...
|---|---|---|---|
| **Core** | `supported: true` | Obligations, revisions, immutable post-official adjustments, the five health states, and seller [`get_reporting_status`](/docs/media-buy/task-reference/get_reporting_status). Current source also defines opt-in buyer-to-seller [`sync_reporting_status`](/docs/media-buy/task-reference/sync_reporting_status) over ordinary task transport; it becomes required only in the next eligible minor after the notice window. Core offerings omit `method`, destinations, materializations, Managed canonicalization, billing receipts, and bespoke status webhooks. | **Yes** — the only required tier |
...
| **Reconciled billing** | `reconciled_billing: true` | `receipt_task` ([`sync_reporting_receipts`](/docs/media-buy/task-reference/sync_reporting_receipts)), `consumer_receipt` reconciliation, and the canonical-digest contract in offering profiles. | No |
...
those terms with [`get_products`](/docs/media-buy/task-reference/get_products), replays
the exact object on a package in
[`create_media_buy`](/docs/media-buy/task-reference/create_media_buy), and receives the
...
[`TERMS_REJECTED`](/docs/building/verification/compliance-catalog#error-code-terms-rejected)
...
| `catalog_item_availability_updates` | Supports buyer-pushed [`sync_catalogs` availability updates and queries](/docs/media-buy/task-reference/sync_catalogs#immediate-item-availability) for revision-guarded suppression, restoration, and current-state readback in buyer-managed catalogs |
...
| `supported_activation_methods`  | object[] |              | **Experimental** (`media_buy.audience_activation` — MUST be listed in `experimental_features` when present; see [Experimental status](/docs/reference/experimental-status)). Union of per-product [`audience_activation.methods`](/docs/media-buy/product-discovery/media-products#audience-activation) across the seller's catalog. Fast-fail discovery: read this once and skip the catalog walk when nothing overlaps your pipeline. Per-product declarations are the source of truth; sellers MUST keep this consistent with the catalog. Operational coordinates are account-scoped: this union may omit `consumer_identities` and `destination_ref` until bilateral setup establishes them, and must never expose another account's coordinates. Absence of this field with `media_buy.audience_activation` listed in `experimental_features` means walk the catalog — only a present, non-overlapping union is a fast-fail signal. Each entry is an [activation method](/docs/media-buy/product-discovery/media-products#activation-method-patterns). |
...
| Field | Type | Description |
|-------|------|-------------|
| `anonymous_discovery` | boolean | Whether [`get_signals`](/docs/signals/tasks/get_signals) accepts anonymous discovery in `brief` or `wholesale` mode. `true` permits a successful anonymous response but makes no completeness or cross-account equivalence claim; `false` requires an authenticated principal; absence is unspecified legacy behavior. Presented credentials are always validated and invalid credentials never downgrade to anonymous access. |
...
| `has_creative_library` | boolean | When `true`, the agent exposes reusable [`sync_creatives`](/docs/creative/task-reference/sync_creatives) and `list_creatives` library operations. |
...
| `representation_resolution` | object | Destination-sales-agent opt-in to accept a complete `creative_representation_set` on [`build_creative`](/docs/creative/task-reference/build_creative), verify its revision digest, and select one compatible `representation_id` against that seller's current product and execution contract using an advertised deterministic strategy. Standalone creative agents cannot advertise seller-bound resolution. Absence means the caller selects before sending a seller-bound manifest; the agent cannot guess. See [creative representation sets](/docs/creative/representation-sets). |
