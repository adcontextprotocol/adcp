...
| Field | Type | Description |
|-------|------|-------------|
| `data_provider_domains` | string[] | Data provider domains this signals agent is authorized to resell. Buyers should fetch each provider's `adagents.json` for published signal definitions and to verify authorization. |
...
| `features.catalog_signals` | boolean | Supports signals published in data provider `adagents.json` `signals[]` with structured `signal_ref` references. |
