...
:::note
The agent card extension (`adcp-extension.json`) is deprecated. Use tool-based discovery instead.
:::
...
| Field | Type | Description |
|-------|------|-------------|
| `feature_id` | string | **Required.** Unique identifier (e.g., `consent_quality`, `coppa_certified`) |
...
```json
{
  ...
  "governance": {
    "property_features": [
      { "feature_id": "consent_quality", "type": "quantitative", "range": { "min": 0, "max": 100 }, "description": "Quality score for consent collection practices" },
      { "feature_id": "coppa_certified", "type": "binary", "description": "COPPA compliance certification" },
      { "feature_id": "carbon_score", "type": "quantitative", "range": { "min": 0, "max": 100 }, "description": "Carbon footprint sustainability score" },
...
Capabilities tell you what you CAN specify in create_media_buy targeting. Use `required_targeting_systems` to filter products to sellers that support specific geo targeting:
...
// Filter products by metro area AND require ZIP targeting capability
const products = await client.getProducts({
  brief: "Premium video inventory in NYC metro area",
  filters: {
    channels: ['olv', 'ctv'],
    // Filter to NYC metro area products (coarse geography)
    metros: [{ system: 'nielsen_dma', code: '501' }],
    // Require seller supports ZIP targeting (capability filter - no actual ZIPs needed)
    required_targeting_systems: ['us_zip'],
...
**Key distinction:**
- **Product filters** (get_products): Use coarse geography to find products with coverage in an area
- **Targeting overlay** (create_media_buy): Use fine-grained geography to restrict where ads actually deliver
