...
`effective_timezone` is the seller's default shared calendar-day boundary and MUST equal `get_account_financials.timezone` when that task is available. A buyer override is media-buy-level and applies to every cap on that buy. DST transition dates remain one calendar day carrying the full cap.
