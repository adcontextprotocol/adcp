...
- **Protocol support** - Which domain protocols (media_buy, signals, governance, sponsored_intelligence, creative, compliance_testing)?
...
| `protocols` | string[] | Optional. Filter to specific protocols (`media_buy`, `signals`, `governance`, `sponsored_intelligence`, `creative`, `compliance_testing`). If omitted, returns all supported protocols. |
...
Array of RPC protocol surfaces this seller exposes. This is independent of the compliance classification below — `supported_protocols` answers *"which tools does this agent implement"*, while `domains` and `specialisms` answer *"which compliance storyboards does this agent commit to pass"*.
...
Compliance domains this agent claims. Each entry corresponds to a baseline storyboard bundle at `/compliance/{version}/domains/{domain}/` that the agent commits to pass. See the [Compliance Catalog](/docs/building/compliance-catalog) for every domain's scope and the [enum schema](https://adcontextprotocol.org/schemas/v3/enums/adcp-domain.json) for the full list.
...
Valid values: `media-buy`, `creative`, `signals`, `governance`, `brand`, `sponsored-intelligence`.

### specialisms

Optional specialization claims. Each entry corresponds to a storyboard at `/compliance/{version}/specialisms/{id}/`. Every specialism rolls up to one domain — claiming `sales-guaranteed` implies claiming `media-buy`. The runner rejects a specialism claim if the parent domain is missing.
...
Compliance testing capabilities. Only present if `compliance_testing` is in `supported_protocols`. Indicates this agent supports deterministic testing via [`comply_test_controller`](/docs/building/implementation/comply-test-controller).
...
Storyboard runners check for `compliance_testing` in `supported_protocols` before running deterministic testing steps. If the agent does not declare compliance testing support, controller-dependent storyboard steps cannot be validated.

Agents that implement `comply_test_controller` SHOULD declare `compliance_testing` in `supported_protocols` and list supported scenarios. Agents that only support a subset of scenarios (e.g., media buy status but not SI sessions) declare only those scenarios — the runner skips unsupported ones.
