...
The conditional-fetch tokens (`if_catalog_version` / `if_pricing_version`) on [`get_products`](/docs/media-buy/task-reference/get_products#catalog-versioning) and [`get_signals`](/docs/signals/tasks/get_signals#catalog-versioning) are always available regardless of `catalog_change_feed.supported`. The change feed gives a continuous event stream; the version tokens give a cheap "anything changed?" probe. Adopters MAY ship either, both, or neither.
