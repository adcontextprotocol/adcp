...
Account and authentication capabilities. Sellers that manage advertiser relationships — including walled gardens, retail media networks, and AI platforms — should declare this section. When present, buyers read it before calling `sync_accounts` or any authenticated task. Omitting it signals a simple publisher model with no account management.
