...
Reporting dimension capabilities. Declares which dimensional breakdowns the seller supports in `get_media_buy_delivery` responses when buyers include `reporting_dimensions` in the request.

| Field | Type | Description |
|-------|------|-------------|
| `supports_geo_breakdown` | object | Geographic breakdown support. Contains boolean fields for `country`, `region`, and nested objects for `metro` and `postal_area` with system-specific booleans (mirrors targeting geo structure). |
| `supports_device_type_breakdown` | boolean | Device type breakdown (`by_device_type` in `by_package`) |
| `supports_device_platform_breakdown` | boolean | Device platform breakdown (`by_device_platform` in `by_package`) |
| `supports_audience_breakdown` | boolean | Audience segment breakdown (`by_audience` in `by_package`) |
| `supports_placement_breakdown` | boolean | Placement breakdown (`by_placement` in `by_package`) |
...
```json
{
  ...
  "media_buy": {
    ...
    "conversion_tracking": {
      ...
      "attribution_windows": [
        { "click_through": ["7d", "28d"], "view_through": ["1d", "7d"] }
      ]
    },
    "portfolio": {
    ...
    "reporting": {
      "supports_geo_breakdown": {
        "country": true,
        "region": true,
        "metro": { "nielsen_dma": true }
      },
      "supports_device_type_breakdown": true,
      "supports_device_platform_breakdown": false,
