...
`media_buy.reporting_delivery` advertises managed durable reporting. Its `offerings[]`
are atomic—not a cross-product—and each binds one feed purpose, content-addressed
reporting profile/schema, schedule, supported finality, transport, orchestration,
provider/access mode/format, and destination modes. It also declares automated
recovery, ledger-metadata retention, exact-resource retention, and authorization
revocation bounds. Presence requires `media_buy.reporting_delivery` in top-level
`experimental_features` and a supported RFC 9421 `webhook_signing` profile. See
[`get_reporting_status`](/docs/media-buy/task-reference/get_reporting_status) and
