...
| `features.catalog_signals` | boolean | Legacy wire flag name for structured `signal_ref` references to provider-published signal definitions in adagents.json `signals[]`. |

<Note>
`catalog_signals` remains on the wire for 3.x compatibility. New documentation describes the same capability as structured `signal_ref` references to provider-published signal definitions.
</Note>
