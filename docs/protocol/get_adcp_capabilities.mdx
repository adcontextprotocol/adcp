...
Capabilities tell you what you CAN specify in create_media_buy targeting. Product filtering uses coarser geography (metros), while targeting uses the full precision you need:
...
// Filter products by metro area (coarse geography)
const products = await client.getProducts({
  brief: "Premium video inventory in NYC metro area",
  filters: {
    channels: ['olv', 'ctv'],
    // Filter to NYC metro area products
    metros: [{ system: 'nielsen_dma', code: '501' }],
    // Only include if seller supports this AXE
    ...(mediaBuy.execution?.axe_integrations?.includes('https://agentic.scope3.com') && {
