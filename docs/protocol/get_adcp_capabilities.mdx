...
| `required_for_products` | boolean | Default: `false`. When `true`, an active account is required before calling `get_products`. |
...
See [Accounts and Agents](/docs/building/integration/accounts-and-agents#trust-model) for full workflows and [seller billing patterns](/docs/building/integration/accounts-and-agents#seller-billing-patterns) for common combinations of auth model, billing support, and default account.
...
1. **Set up accounts**: Follow the auth model from `account.require_operator_auth` — see [Accounts and Agents](/docs/building/integration/accounts-and-agents#trust-model)
