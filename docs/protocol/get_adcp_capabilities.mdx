...
| Field | Type | Description |
|-------|------|-------------|
| `supported_billing` | string[] | **Required.** Billing models this seller supports: `brand`, `operator`, `agent` |
...
| `default_billing` | string | Billing model applied when agent omits `billing` from `sync_accounts`. Must be one of `supported_billing`. |
| `account_resolution` | string[] | How this seller resolves account references in requests. Values: `account_id` (seller-assigned identifier) and/or `natural_key` (resolved from house + brand_id + operator). Buyers use this to determine whether to pass `account.account_id` or `account.house` (with optional `brand_id`/`operator`) in task requests. |
...
See [Accounts and Agents](/docs/building/integration/accounts-and-agents#what-sellers-declare) for full workflows and [seller patterns](/docs/building/integration/accounts-and-agents#seller-patterns) for common combinations of auth model, billing support, and default account.
...
- **Billing**: Brand or operator billing; default is brand
