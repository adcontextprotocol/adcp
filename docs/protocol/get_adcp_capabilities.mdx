...
Experimental buyer-orchestrator role within the measurement protocol. Presence means the orchestrator exposes a controlled interchange boundary to providers without granting them seller credentials. Orchestrators implementing it include `measurement` in `supported_protocols` and `measurement.gateway` in `experimental_features`.

The orchestrator intersects its methods with the selected provider's `measurement.delivery_input_methods` and `measurement.feedback_output_methods`, then provisions the chosen paths on the provider's orchestrator account.

| Field                     | Type     | Description                                                                                                                                                                                                           |
| ------------------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `delivery_output_methods` | string[] | How the orchestrator supplies delivery: `pull` through the orchestrator's `get_media_buy_delivery`, `webhook` reporting callbacks, or `offline` cloud-bucket delivery.                                                |
| `feedback_input_methods`  | string[] | How the orchestrator receives compact assertions: `task` through `provide_performance_feedback`, `webhook` callbacks carrying the same assertion shape, or `offline` files through an account-provisioned cloud path. |
...
| `delivery_input_methods`        | string[] | Orchestrator delivery methods the provider can consume: `pull`, `webhook`, or `offline`.                                                                                                                      |
| `feedback_output_methods`       | string[] | Result paths the provider can use: `task`, `webhook`, or `offline`.                                                                                                                                           |
| `metrics`                       | object[] | **Required.** Per-metric catalog described below.                                                                                                                                                             |

When `produces_performance_feedback` is true, both method arrays are required; a provider cannot claim feedback production without declaring at least one usable input and output path.
...
- **Auth model**: Agent-trusted (`require_operator_auth: false`) — authenticate once and use natural-key accounts, normally declared via `sync_accounts`
...
When `supported_protocols` includes `"creative"`, inspect the `creative` capability block for the tasks and canonical formats this endpoint actually supports. `list_creative_formats` is deprecated in 3.2. See [Creative capabilities on sales agents](/docs/creative/sales-agent-creative-capabilities).
