...
Extension schemas are published in the [AdCP extension registry](/docs/reference/extensions-and-context). When an agent declares support for an extension, buyers know to look for and process `ext.{namespace}` data in responses.
...
- Creative policies may include `ext.garm` with GARM brand safety categorizations
