...
Measurement protocol capabilities. Only present if `measurement` is in `supported_protocols`. Measurement agents publish a per-metric catalog so buyers know which metrics each vendor offers (Adelaide attention, Scope3 emissions, Nielsen DAR demographics, etc.). This is the canonical source of truth — AAO crawls it to populate the federated [measurement-vendor index](/docs/registry/index#measurement-vendor-discovery).
...
| Field | Type | Description |
|-------|------|-------------|
| `metric_id` | string | **Required.** Vendor-scoped identifier (`attention_units`, `gco2e_per_impression`, etc.) — matches the `metric_id` populated in `vendor_metric_values` on delivery. The full identity of a metric is the tuple `(vendor.domain, vendor.brand_id, metric_id)`. |
| `category` | [MeasurementCategory](#) | **Required.** Coarse classification: `attention`, `brand_lift`, `incrementality`, `audience`, `reach`, `creative_quality`, `emissions`, `outcomes`, `other`. Used by AAO and buyer agents for cross-vendor discovery. |
| `standard_reference` | string (URI) | Optional URI pointing at the published standard this metric implements (IAB Attention Measurement Guidelines, MRC Viewable Impression Measurement, GARM emissions framework). Anchors buyer trust and supports cross-vendor comparability. |
| `unit` | string | Unit of the value when reported in `vendor_metric_values.value` (`score`, `seconds`, `persons`, `gCO2e`, `lift_percent`, etc.). Sellers populating `vendor_metric_values.unit` MUST match this declaration. |
| `description` | string | Human-readable description (max 1000 chars). |
| `documentation_url` | string (URI) | URL to vendor's full methodology documentation. |
    ...
    "metrics": [
      {
        ...
        "documentation_url": "https://attentionvendor.example/docs/attention-units"
      },
      {
...
**Discovery vs. settlement.** Buyers MAY hit a measurement agent's `get_adcp_capabilities` directly to see its current catalog (live, canonical, no staleness), or query AAO's federated index for cross-vendor discovery (`/api/registry/measurement-vendors?category=attention`) for cheap aggregation. The index trades a TTL refresh window for cross-vendor speed; live calls trade speed for currency. Both are valid — typically planning queries hit the index, settlement/audit queries hit the agent.

**Coarse-filter precedent on brand.json.** Measurement agents MAY also declare `metric_categories[]` on their `agents[]` entry in brand.json (parallels rights agents' `available_uses[]` / `right_types[]`) for AAO directory pre-filter without crawling the agent. The full per-metric catalog still lives at `get_adcp_capabilities.measurement.metrics[]`; brand.json carries only the category list.
