...
The `reporting.delivery_ready` doorbell (`readiness_notification`) is **optional in
every tier** — a polling-only seller that repairs through `get_reporting_status`
is fully Core-conformant, and a supported RFC 9421 `webhook_signing` profile is
required only when the seller actually emits reporting webhooks.
