...
| `preview` | object | Per-format [`preview_creative`](/docs/creative/task-reference/preview_creative) declaration. `supported_capability_ids[]` names `supported_formats[]` entries whose `operations` includes `preview`; `fidelity` is `authoritative` only when the serving platform's own preview machinery renders the output and otherwise is `representative`. Production quality does not upgrade representative output to authoritative. See the [rendering authority order](/docs/creative/canonical-formats#rendering-authority-and-fallback-order). |
...
```json
{
  ...
  "creative": {
    ...
    "preview": {
      "supported_capability_ids": ["display_image_generator"],
