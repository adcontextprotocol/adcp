...
| `optimistic_concurrency` | boolean | Whether the seller enforces `expected_configuration_version`. |

Agents exposing this block MUST list `protocol.agent_configuration` in `experimental_features`. Account authorization remains separate; a ready reusable destination does not grant access to any advertiser account.
