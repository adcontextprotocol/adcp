...
#### supported_insight_types and insight_notifications
...
Insight meaning follows the negotiated AdCP release rather than an independent version. This capability does not promise complete upstream coverage. Each present array names exact type coverage in `insight_types_evaluated`.

The two media-buy fields are paired: a seller declaring `supported_insight_types` also declares `insight_notifications`, and vice versa. Every declaration contains `insights.changed` and `get_media_buys`; this is sufficient for inline-only sellers and for buy/package-only insight catalogs. Sellers with a creative-library reverse projection additionally include the paired `creative.assignment_changed` event and `list_creatives` repair task. Those sellers mirror relationship identity, scoped approval, and evaluated insight state on every `list_creatives` assignment row. Because the capability advertises outbound invalidations, `webhook_signing.supported` MUST be `true`.

Subscriptions are prospective: activation does not replay current conditions, so buyers establish a baseline with an unfiltered declared repair read. Sellers then fire the applicable invalidation after observing a semantic change. See [Insights, Warnings, and Recommendations](/docs/media-buy/media-buys/insights).

An inline-only declaration is therefore valid with `event_types: ["insights.changed"]` and `repair_tasks: ["get_media_buys"]`. The creative event and repair task are all-or-nothing; declaring either without the other is invalid.
...
- **Auth model**: Agent-trusted (`require_operator_auth: false`) — authenticate once, declare brands via [`sync_accounts`](/docs/accounts/tasks/sync_accounts)
