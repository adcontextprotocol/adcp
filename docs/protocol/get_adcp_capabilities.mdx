...
Conditional-fetch token capabilities for [`get_products`](/docs/media-buy/task-reference/get_products#catalog-versioning) and [`get_signals`](/docs/signals/tasks/get_signals#catalog-versioning). Independent of the change feed — an agent MAY support cheap version probes without implementing the full feed (and vice versa).
