...
Measurement capability block. **Presence indicates the agent supports measurement-vendor discovery; absence means it does not.** Modeled as a capability block (like `compliance_testing` and `webhook_signing`), not as a `supported_protocols` value, because measurement agents have one surface — this catalog — not a tool-set with mandatory tasks.
