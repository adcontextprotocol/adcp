...
Complementary to (and independent of) [`catalog_versioning`](#catalog_versioning) — the change feed gives a continuous event stream; the version tokens give a cheap "anything changed?" probe. Adopters MAY ship either, both, or neither.
