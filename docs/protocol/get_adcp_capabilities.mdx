...
Declares a seller's webhook-signing posture. Any seller that emits webhooks MUST include this block — buyers read it at onboarding to determine whether the seller will sign outbound webhooks per the [AdCP webhook-signing profile](/docs/building/implementation/security#webhook-callbacks) and which algorithms to expect. A buyer integrating with a seller that omits the block or advertises `supported: false` MUST fail onboarding with a user-actionable error; silent integration with a non-signing seller is unsafe for any mutating-webhook use case.

| Field | Type | Description |
|-------|------|-------------|
| `supported` | boolean | **Required.** `true` iff the seller signs outbound webhooks. `false` means webhooks are unsigned; buyers MUST fail onboarding. |
...
| `algorithms` | string[] | **Required when `supported: true`.** Subset of `["ed25519", "ecdsa-p256-sha256", "rsa-pss-sha512"]` — the algorithms this seller will sign webhooks with. Buyers MUST be prepared to verify any algorithm listed. |
...
| Field | Type | Description |
|-------|------|-------------|
| `experimental_features` | string[] | Experimental feature ids (e.g., `["brand.rights_lifecycle", "governance.campaign", "trusted_match.core"]`) |
