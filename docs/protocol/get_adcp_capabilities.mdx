...
An indicator-capable seller declares `supported_indicator_types` together with `relationship_notifications` containing `indicators.changed`. The reverse is intentionally not required: a seller without an indicator catalog may declare `creative.assignment_changed` alone for assignment and approval changes, including when it is inline-only. Every declaration names `get_media_buys` as the complete authoritative repair read. A creative-library seller may independently include [`list_creatives`](/docs/creative/task-reference/list_creatives) as a bounded reverse projection; it mirrors relationship identity, scoped approval, and evaluated indicator state on every returned assignment row but cannot prove completeness when truncated. Because the capability advertises outbound invalidations, `webhook_signing` MUST include `supported: true`, `profile`, `algorithms`, and `legacy_hmac_fallback`.

Subscriptions are prospective: activation does not replay current conditions. Buyers establish a complete `get_media_buys` baseline by enumerating known IDs or requesting all seven statuses and following pagination to exhaustion, without `indicator_types`. Sellers then fire the applicable invalidation after observing a semantic change. See [Indicators, Warnings, and Recommendations](/docs/media-buy/media-buys/indicators).
