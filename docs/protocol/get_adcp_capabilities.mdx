...
| `product_selection` | `product_changes[]` | Check `include`/`omit` against `commercial_terms.purchases[].product_id`. |
| `alternatives` | `alternatives.count` | Count the draft proposals returned for the source proposal. |

`max_alternatives` may accompany `alternatives` to publish a pre-flight count ceiling. An explicit `supported_dimensions` list is authoritative: buyers SHOULD NOT send omitted dimensions, and sellers SHOULD reject them with [`UNSUPPORTED_FEATURE`](/docs/building/verification/compliance-catalog#error-code-unsupported-feature). When `proposal_refinement` is absent, support is unknown; sellers report unsupported dimensions through `partial` or `unable` results instead.
