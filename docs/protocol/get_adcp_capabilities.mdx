...
Declaring support for a geographic targeting level implies support for both inclusion and exclusion at that level. For example, `geo_metros.nielsen_dma: true` means the seller supports both `geo_metros` and `geo_metros_exclude` with Nielsen DMA codes. See [Targeting Overlays](/docs/media-buy/advanced-topics/targeting) for exclusion semantics.
...
- `media_buy.execution.targeting.geo_postal_areas.us_zip: true` → Buyer can send US ZIP codes (inclusion and exclusion), seller MUST honor them
- `media_buy.execution.targeting.geo_metros.nielsen_dma: true` → Buyer can send DMA codes (inclusion and exclusion), seller MUST honor them
