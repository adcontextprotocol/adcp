...
`operator_unit_name` covers display-name-only changes. `operator_unit` covers adding, removing, or changing the stable unit ID and also implies name changes. `operator` covers changing the operator domain and may require seller approval. Sellers declaring support MUST return account `revision` from `sync_accounts` and `list_accounts`, atomically enforce a submitted revision, preserve account identity and account-scoped resources during rekeying, and expose any pending or rejected transition as `identity_change`. When the capability is absent or `supported: false`, buyers MUST NOT submit `operator_identity`; sellers reject it with `UNSUPPORTED_FEATURE`.
