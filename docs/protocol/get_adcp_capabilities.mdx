...
AdCP uses native MCP tool discovery and the [AdCP A2A Profile Extension v3.2](/docs/building/by-layer/L0/a2a-profile-extension) on A2A 1.0. **The presence of `get_adcp_capabilities` in an agent's tool or skill list indicates that runtime AdCP discovery is available.**
