...
| `sync_agent_notification_configs` | Conditional | Required when `get_adcp_capabilities.adcp.capability_changes.notifications.supported` is `true`; registers agent-level `capabilities.changed` webhooks. |
