...
AdCP uses native MCP/A2A tool discovery. **The presence of `get_capabilities` in an agent's tool list indicates AdCP support.**

```
Discovery Flow:
1. Browse agent's tool list (MCP) or skills (A2A)
2. See 'get_capabilities' tool → Agent supports AdCP
3. Call get_capabilities → Get version, protocols, features, capabilities
