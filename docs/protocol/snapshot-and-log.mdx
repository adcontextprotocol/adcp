...
- **Creative lifecycle, assignment, and insight webhooks** — `list_creatives.creatives[].webhook_activity[]` adopts this pattern for `creative.status_changed`, `creative.purged`, `creative.assignment_changed`, and assignment-level `insights.changed`. Assignment and insight events are invalidations repaired completely through `get_media_buys`; `list_creatives` is an optional bounded reverse projection, and neither its rows nor the webhook payload replaces the authoritative snapshot. The notification channel is the account's `notification_configs[]` set registered through `sync_accounts`.
