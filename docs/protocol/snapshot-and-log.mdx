...
This works for any state surface — impairments, statuses, delivery aggregates — because rule 2 guarantees the snapshot reflects every push. It does not work for purely point-in-time events that drain from the log (e.g., individual delivery report fires after retention expires). For those, the [webhook delivery visibility surface](/docs/media-buy/task-reference/get_media_buys) (`webhook_activity[]`) provides a bounded recent-fire window — that's the closest AdCP gets to a replay primitive.
