...
- **Creative lifecycle, assignment, and insight webhooks** — `list_creatives.creatives[].webhook_activity[]` adopts this pattern for `creative.status_changed`, `creative.purged`, `creative.assignment_changed`, and assignment-level `insights.changed`. Assignment and insight events are invalidations repaired through `list_creatives` or `get_media_buys`; their payload never replaces the snapshot. The notification channel is the account's `notification_configs[]` set registered through `sync_accounts`.
