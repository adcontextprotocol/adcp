...
#### Blocked on track 3 (3.2.0+)

Resources that outlive a single media buy cannot adopt this pattern in 3.1 because item 1 is unsatisfied; they are blocked on the per-account notification channel:

1. **#4582 track 3 (3.2.0)** introduces the per-account subscription model — the prerequisite notification channel for resources that outlive a buy. No new fields land on this read pattern as part of track 3 itself; track 3 supplies the *registration* primitive that unlocks the pattern for resources that need it.
