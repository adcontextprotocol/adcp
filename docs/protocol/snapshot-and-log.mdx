...
### 4. Replay = re-read the snapshot (for state events)

For **state-shaped events** (impairment, lifecycle, status changes), if a buyer's webhook receiver was offline and missed a fire, the recovery path is to call the relevant `get_*` task and read current state. The snapshot fully reconstructs.

For **data-bearing events** (delivery report fires, individual log events), this rule is **weaker today**. The snapshot reflects the cumulative integral (`get_media_buy_delivery` aggregates) but may not reconstruct the per-fire differential. Buyers SHOULD treat data-bearing webhook channels as primary delivery and SHOULD NOT assume snapshot recovery is lossless for them. The bounded event-history surface (`webhook_activity[]`, proposed in [#4278](https://github.com/adcontextprotocol/adcp/issues/4278)) is the path to making this rule hold universally — when it lands, the snapshot subsumes the log for data events too.

This honesty matters. Pretending Rule 4 holds for every push channel today would invite buyers to build recovery paths that silently lose data. The rule is the **destination**, not a uniform present-day claim.
