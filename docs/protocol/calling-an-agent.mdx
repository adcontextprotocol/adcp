...
Every mutating tool requires an `idempotency_key` (UUID). The published 3.x [`get_products`](/docs/media-buy/task-reference/get_products) compatibility facade keeps its key optional on every arm. Pure-read wrappers must tolerate an optional key, but may ignore it; callers cannot rely on read-response replay. Buyers SHOULD include a key whenever `get_products` may allocate a task, finalize a proposal, or otherwise change observable state.
