...
3. **[`sync_agent_configuration`](/docs/protocol/sync_agent_configuration)** *(when advertised)*: declares this authenticated caller's durable connection resources, such as agent-level webhooks and reusable reporting destinations. This is caller state, not capability discovery and not advertiser-account authorization.
