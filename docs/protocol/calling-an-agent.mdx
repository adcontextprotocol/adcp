...
// variant 1: by natural key (brand + operator, optional market/operator_region/sandbox)
...
| Response carries `status: 'submitted'` and `task_id` | Async — work is queued, NOT done | Poll with legacy `tasks/get`, or [`get_task_status`](https://adcontextprotocol.org/schemas/v3/protocol/get-task-status-request.json) when the seller advertises the alias. |
