...
Every mutating tool requires an `idempotency_key` (UUID). Pure-read wrappers must tolerate an optional key, but the seller may ignore it and execute a fresh read; callers cannot rely on read-response replay unless the seller documents that stronger behavior.
