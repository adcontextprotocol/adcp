...
3. Establish account
   sync_accounts(brand: {domain, brand_id}, operator, billing) → account_id
...
The `brand.domain` field in `sync_accounts` is not an arbitrary identifier — it is the brand's domain, resolvable to a `brand.json` file that declares the brand's canonical identity, sub-brands, authorized operators, and properties.
...
When an orchestrator calls `sync_accounts`, the `brand.domain` field is the brand's domain. Vendor agents can fetch `brand.domain/.well-known/brand.json` to verify:
