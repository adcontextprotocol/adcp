...
Not every caller with access to an account has the same grant. A seller may issue one agent a full media-buy scope and another agent a narrow read-plus-webhook-attach scope (e.g., an AAO compliance engine for Tier-2 Production Verified conformance). Authentication confirms the caller is who they claim to be; authorization answers "what is this caller allowed to do on this account?"

**Schema**: [`/schemas/v3/core/account-authorization.json`](https://adcontextprotocol.org/schemas/v3/core/account-authorization.json)

Sellers that support scope introspection attach an `authorization` object to each per-account entry in [`sync_accounts`](/docs/accounts/tasks/sync_accounts) and [`list_accounts`](/docs/accounts/tasks/list_accounts) responses:
...
| `field_scopes` | Optional per-task allowlist of request fields the caller may set. Keys are task names; values are field paths. When a task appears here, any field outside the allowlist returns `FIELD_NOT_PERMITTED`. Implicit framing fields (`account`, `media_buy_id`, `package_id`, `creative_id`, `revision`, `idempotency_key`, `buyer_ref`, `po_number`, `dry_run`, `context`, `ext`, `adcp_major_version`, `push_notification_config`) are always permitted and do not need to appear in the allowlist. |
| `scope_name` | Optional named scope identifier. Only `attestation_verifier` is standardized; seller-defined names MUST use the `seller:` prefix so typos of the standard value fail schema validation rather than pass through. |
...
- **Present**: the seller asserts the shape reflects its enforcement for this caller on this account at this moment. Stale-by-seconds is fine; systematically-divergent is non-conformant.
- **Absent on a single account**: the seller is not telling the caller their scope for that account. Caller falls back to error-driven discovery (try the task, handle the RBAC error codes).
- **Absent on all accounts**: the seller does not implement scope introspection. Callers MUST NOT infer access from absence — the seller may still enforce scope locally and return `SCOPE_INSUFFICIENT` / `READ_ONLY_SCOPE` / `FIELD_NOT_PERMITTED` on any task call.
- **`authorization` is optional for 3.x**. Sellers that want to avoid a breaking change can defer populating it. Populating is strictly additive — it lets callers preempt errors they would otherwise discover by trying.

Sellers claiming the `attestation_verifier` standard scope **MUST** populate `authorization` — the Tier-2 attestation flow depends on verifying the advertised scope matches enforcement.

### Identity binding, refresh cadence, and consistency

The `authorization` object is **implicitly scoped to the `(caller identity, account_id)` tuple** at read time. The same account returned to a different authenticated caller MAY return a different `authorization` object — that's the point of the RBAC model. Sellers MUST resolve caller identity from the authenticated request (not from any client-supplied field) and MUST bind the returned `authorization` to that resolved identity.
...
- Sellers MUST reflect operator-initiated scope changes in `sync_accounts` / `list_accounts` responses **within 300 seconds** of the change being made. A compliant caller polling every 300s sees operator changes within at most one refresh cycle.
- Sellers MAY cache the `authorization` object briefly but MUST NOT cache past 300 seconds without revalidation.
- The 300s figure is a floor, not a target — sellers whose scope changes more frequently SHOULD surface them faster, and callers running Tier-2 attestation SHOULD probe at the check-7 cadence (at least once per rolling window, plus on every observed scope change).

**Consistency.**

For a given `(caller identity, account_id)` tuple, sequential reads within the refresh window MUST return identical `authorization` objects, modulo operator-initiated scope changes. Flicker from load-balanced or eventually-consistent backends — two reads 10 seconds apart returning different `allowed_tasks` because they hit different replicas — is non-conformant. Compliance engines and coding agents rely on scope stability for state tracking; a seller that cannot guarantee it MUST omit `authorization` rather than populate it inconsistently.
...
Sellers advertising Tier-2 Production Verified conformance readiness (tracked in [#2965](https://github.com/adcontextprotocol/adcp/issues/2965)) MUST support a named scope identified by `scope_name: "attestation_verifier"` with the following minimum shape:

- `allowed_tasks` (at minimum — sellers MAY include additional read-only tasks):
  - `get_adcp_capabilities`
  - `get_products`
  - `get_media_buys`
  - `get_media_buy_delivery`
...
`get_products` and `list_creatives` are included because Tier-2 observability requires sanity-reading the seller's declared inventory and the creative pipeline state on active buys.

Other scope names are **not** standardized — taxonomies vary too much across sellers. Sellers MAY define custom scopes using the `seller:` prefix (e.g., `seller:governance_viewer`); buyers MUST NOT assume any semantics from a seller-prefixed scope name.
