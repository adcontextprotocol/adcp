...
- **Measurement agent on a buyer-orchestrator gateway account**: `custom:measurement_feedback` — `allowed_tasks: [get_media_buy_delivery, provide_performance_feedback]`, `read_only: false` when pull and task return are selected. Include only the tasks required by the negotiated paths; webhook and offline paths use separately provisioned signing or cloud credentials. Measurement providers do not receive seller-account grants. The exact task list is normative, so no standard named scope is needed.
