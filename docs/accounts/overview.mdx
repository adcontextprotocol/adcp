...
The account reference may be a seller-assigned `account_id` or a natural key — `brand` + `operator` + optional `sandbox`. Both forms are valid regardless of account model. See [Account references](/docs/building/integration/accounts-and-agents#account-references) for details.
