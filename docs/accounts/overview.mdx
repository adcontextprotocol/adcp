...
The account reference may be a seller-assigned `account_id` (explicit accounts, `require_operator_auth: true`) or a natural key — `brand` + `operator` (implicit accounts, `require_operator_auth: false`). For sandbox operations, use the natural key with `sandbox: true` regardless of account model. See [Account references](/docs/building/integration/accounts-and-agents#account-references) for details.
