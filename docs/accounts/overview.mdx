...
The account reference may be a seller-assigned `account_id` (explicit accounts, `require_operator_auth: true`) or a natural key — `brand` + `operator` (implicit accounts, `require_operator_auth: false`). For sandbox operations, pass the natural key with `sandbox: true` in the [account reference](/docs/building/integration/accounts-and-agents#account-references) — no account provisioning needed.
