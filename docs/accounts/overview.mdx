...
| How does the operator authenticate? | Seller capabilities | `require_operator_auth` determines the auth model and reference shape: `true` means operator credentials are required and account-scoped calls use `account_id`; `false` means the agent is trusted and declares accounts via `sync_accounts` |
