...
The Accounts Protocol applies across all vendor protocols. An orchestrator syncs accounts once per vendor agent and reuses the same `account_id` across all interactions with that agent:
...
| Task | Purpose |
|------|---------|
| [`sync_accounts`](/docs/accounts/tasks/sync_accounts) | Declare advertiser portfolio; receive `account_id` per brand |
