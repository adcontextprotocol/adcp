...
| `pricing_option_id` | string | Vendor: Yes | Pricing option from the vendor's discovery response ([`get_signals`](/docs/signals/tasks/get_signals), [`list_creatives`](/docs/creative/task-reference/list_creatives), [`list_content_standards`](/docs/governance/content-standards/tasks/list_content_standards), [`list_property_lists`](/docs/governance/property/tasks/property_lists#list_property_lists)) or execution response ([`build_creative`](/docs/creative/task-reference/build_creative)). The vendor uses this to verify the correct rate was applied. |
...
|------------|-------------|------------|
| [`ACCOUNT_NOT_FOUND`](/docs/building/verification/compliance-catalog#error-code-account-not-found) | Account reference in a usage record not found or not accessible | Verify via [`list_accounts`](/docs/accounts/tasks/list_accounts); re-run [`sync_accounts`](/docs/accounts/tasks/sync_accounts) if needed |
| [`INVALID_USAGE_DATA`](/docs/building/verification/compliance-catalog#error-code-invalid-usage-data) | A usage record has missing or invalid fields | Check required fields for your vendor type |
| [`INVALID_PRICING_OPTION`](/docs/building/verification/compliance-catalog#error-code-invalid-pricing-option) | `pricing_option_id` not found on this account | Verify `pricing_option_id` from the vendor's discovery response |
