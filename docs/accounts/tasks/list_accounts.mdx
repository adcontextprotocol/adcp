...
| `account_governance` | array | No | Per-account governance agent registration. Each entry pairs an [account reference](/docs/building/integration/accounts-and-agents) with governance agents to register. See [governance agents](#governance-agents). |
...
Pass `account_governance` in the request to register governance endpoints against specific accounts. Each entry pairs an account reference with the governance agents to register. Account references support both explicit accounts (`account_id`) and implicit accounts (`brand` + `operator`).

This uses **replace semantics** — each call replaces any previously registered agents on the specified accounts. Accounts not included in `account_governance` keep their existing registrations. Omitting `account_governance` entirely leaves all registrations unchanged.

The seller MUST verify that the authenticated agent has authority over each referenced account before persisting governance agents. Requests referencing accounts the agent does not own MUST return an error for those entries.

The seller calls the registered agents for approval during media buy lifecycle events (via `check_governance`). Each governance agent entry requires a `url` (HTTPS only) and `authentication`, and can optionally be scoped to specific `categories`. A successful response confirms the governance agents were registered — the returned accounts will include the `governance_agents` field reflecting the persisted configuration.
...
// Discover accounts, then register governance agents per account
...
const result = await testAgent.listAccounts({
  account_governance: [
    {
      account: { account_id: accounts.accounts[0].account_id },
      governance_agents: [
        {
    ...
    {
      account: { account_id: accounts.accounts[1].account_id },
      governance_agents: [
        {
...
async def main():
    ...
    result = await test_agent.simple.list_accounts(
        account_governance=[
            ...
            {
                "account": {"account_id": account_ids[1]},
                "governance_agents": [
                    {
                        "url": "https://governance.pinnacle-media.com/compliance",
                        "authentication": {
...
```json Explicit accounts (by account_id)
{
...
```json Implicit accounts (by brand + operator)
{
  "$schema": "https://adcontextprotocol.org/schemas/latest/account/list-accounts-request.json",
  "account_governance": [
    {
