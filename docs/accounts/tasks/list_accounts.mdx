...
| `governance_agents` | array | No | Governance agent endpoints to register against the returned accounts. The seller persists these and calls them during media buy lifecycle events. See [governance agents](#governance-agents). |
...
On explicit-account sellers (`require_operator_auth: true`), accounts are pre-created on the seller's platform. The buyer discovers them via `list_accounts` but cannot provision them via `sync_accounts`. Pass `governance_agents` in the request to register governance endpoints against the returned accounts.

When `governance_agents` is included, the call has a write side-effect: the seller persists the provided governance configuration against the returned accounts. This uses **replace semantics** — each call with `governance_agents` replaces any previously registered agents on those accounts. Calls without `governance_agents` leave existing registrations unchanged.
...
const result = await testAgent.listAccounts({
  governance_agents: [
    {
      url: "https://governance.pinnacle-media.com/mcp",
      authentication: {
        schemes: ["Bearer"],
        credentials: "gov-token-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
      },
      categories: ["budget_authority", "geo_compliance"]
    }
  ]
});
