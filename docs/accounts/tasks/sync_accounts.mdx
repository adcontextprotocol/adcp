...
Sync advertiser accounts with a seller for one or more advertiser/operator relationships, or update settings on existing accounts when the seller exposes that mode. A buyer-declared account's natural key describes the advertiser object the seller should provision: a brand, the countries where that advertiser identity applies, the operator, an optional operator-owned unit, an optional fixed currency, and whether the account is a sandbox.
...
| `revision` | integer | No | Expected current account revision in settings-update mode. Sellers reject stale revisions atomically with `CONFLICT`. Obtain it from `list_accounts` or the latest `sync_accounts` result. |
...
- Change `operator` to request an operator-domain transition. The seller MUST revalidate the new domain against the brand's `authorized_operators` policy and MAY place the transition into approval.
...
Identity reconciliation changes the same advertiser account; it is never provisioning or merging. The seller MUST resolve exactly one existing account, reject a target-key collision, preserve `account_id`, media buys, reporting, approval and billing state, grants, and every other account-scoped resource, and apply no identity change if it cannot preserve that continuity. `currency`, `sandbox`, and brand identity remain immutable through this field.

Every persisted account mutation increments its integer `revision`. Buyers SHOULD pass the latest observed `revision`; when present, the seller MUST compare it atomically with the write and return `CONFLICT` without side effects on mismatch. Dry runs, validation failures, reads, and exact idempotency replays do not increment it.

An immediately applied update returns the new canonical `operator` and `operator_unit`. If review is required, those fields remain canonical and the result carries `identity_change.status: "pending_approval"` with the requested identity. `list_accounts` exposes the same state. Approval atomically updates the canonical fields and clears `identity_change`; rejection retains the canonical fields and exposes `identity_change.status: "rejected"` with a reason. Each disposition increments `revision`.

After a rekey, the seller MUST tombstone the former natural key while the account or any account-scoped historical resource is retained. An authorized caller using that former key receives `ACCOUNT_MOVED` with `details.current_account`; the seller MUST NOT provision a second account from it. Callers without access to the current account receive `ACCOUNT_NOT_FOUND` so the redirect does not become a cross-tenant existence oracle.
...
| Field | Description |
|-------|-------------|
| `brand` | Echoed from request. Object with `domain`, optional `brand_id`, and optional `countries[]`. |
...
| `pending_approval` | Seller reviewing | Human may need to visit `setup.url` to complete credit or legal process. Poll [`list_accounts`](/docs/accounts/tasks/list_accounts) for updates. |
...
The payload is an invalidation signal. It intentionally omits the full account document and `setup.url`; setup URLs are often single-use or sensitive and should not be fanned out to every subscriber. Receivers SHOULD re-run [`list_accounts`](/docs/accounts/tasks/list_accounts) for `account_id` and reconcile from the fresh account snapshot.
...
|------------|-------------|------------|
| [`ACCOUNT_NOT_FOUND`](/docs/building/verification/compliance-catalog#error-code-account-not-found) | Referenced account does not exist or is not accessible | Check `account_id` or re-sync |
| `ACCOUNT_MOVED` | A former natural key was used after the account was rekeyed | Replace the stale reference with `error.details.current_account`, refresh through `list_accounts`, and retry with a fresh idempotency key |
| [`CONFLICT`](/docs/building/verification/compliance-catalog#error-code-conflict) | Submitted `revision` is stale or the desired natural key already belongs to another account | Re-read `list_accounts`; reconcile and retry a stale write, but do not retry a key collision without changing the desired identity |
| [`UNSUPPORTED_FEATURE`](/docs/building/verification/compliance-catalog#error-code-unsupported-feature) | Seller does not advertise the requested identity change in `account.identity_updates.supported_changes` | Leave identity unchanged or use a seller-supported transition |
| [`BILLING_NOT_SUPPORTED`](/docs/building/verification/compliance-catalog#error-code-billing-not-supported) | Seller-wide capability gate (`supported_billing` does not include the value) or per-account-relationship gate; see [Billing and Account Setup](/docs/building/by-layer/L3/error-handling#billing-and-account-setup) | Check [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities) for `supported_billing`, adjust or omit `billing`; inspect `error.details.scope` to disambiguate capability vs account scope |
| [`BILLING_NOT_PERMITTED_FOR_AGENT`](/docs/building/verification/compliance-catalog#error-code-billing-not-permitted-for-agent) | Seller-wide capability accepts the value, but the calling buyer agent's commercial relationship does not (e.g., passthrough-only — no payments relationship); see [Buyer-agent identity](/docs/building/by-layer/L2/accounts-and-agents#buyer-agent-identity) | Retry with `error.details.suggested_billing` (typically `operator`) when present; when absent, surface to a human — the agent cannot extend its own commercial relationship |
| [`PAYMENT_TERMS_NOT_SUPPORTED`](/docs/building/verification/compliance-catalog#error-code-payment-terms-not-supported) | Seller does not accept the requested payment terms | Omit `payment_terms` to accept the seller's default, or negotiate offline |
| [`ACCOUNT_PAYMENT_REQUIRED`](/docs/building/verification/compliance-catalog#error-code-account-payment-required) | Account has an outstanding balance requiring payment | Resolve outstanding balance or route to another account |
| [`ACCOUNT_SUSPENDED`](/docs/building/verification/compliance-catalog#error-code-account-suspended) | Account is suspended | Contact seller to resolve |
| [`BRAND_REQUIRED`](/docs/building/verification/compliance-catalog#error-code-brand-required) | Billable operation attempted without brand reference | Include `brand` in the request |
