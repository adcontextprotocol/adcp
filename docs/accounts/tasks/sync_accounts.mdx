...
Sync advertiser accounts with a seller for one or more brand/operator pairs. The seller provisions or links accounts, returning their status and any setup instructions. Brands are identified by a `brand` object containing `domain` + optional `brand_id`, resolved via `/.well-known/brand.json`.
...
console.log(`${validated.brand.domain}: ${validated.status}`);
...
async def main():
    ...
    print(f"{result.brand['domain']}: {result.status}")
