...
| `notification_configs` | array | No | Account-level webhook subscribers for events that outlive any single media buy: creative lifecycle notifications and wholesale feed change webhooks. Omit to leave existing subscribers unchanged; send `[]` to remove all subscribers; send a full array to replace. The stable match key is `subscriber_id` — an existing `subscriber_id` is upserted; IDs absent from the sent array are removed. |
...
Permitted in **both** provisioning and settings-update modes. Declarative semantics: omit to leave existing subscribers unchanged; send `[]` to remove all subscribers; send a full array to replace. The stable match key is `subscriber_id` — an entry with an existing `subscriber_id` upserts (replaces) that subscriber's active config. The seller MUST NOT merge the new array with persisted state: entries in persisted state that do not appear in the sent array are removed. Paused entries (`active: false`) are subject to the same semantics — to preserve a paused subscription, re-include the entry with `active: false` in the sent array.
...
Verify applied state via [`list_accounts`](/docs/accounts/tasks/list_accounts) — the response carries `notification_configs[]` per account with credentials redacted.
