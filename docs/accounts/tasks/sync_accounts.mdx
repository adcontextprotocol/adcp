...
| `authorization_agent` | object | No | Buyer's authorization agent endpoint with authentication. When present, the seller MUST call this agent to [verify media buy authorization](/docs/governance/campaign/tasks/verify_authorization) before confirming purchases. Contains `url` (agent endpoint) and `authentication` (credentials with optional `expires_at` for rotation). To rotate credentials, call `sync_accounts` again with updated authentication. |
...
| `authorization_agent` | Authorization agent endpoint (url + authentication), echoed from request when provided. |
