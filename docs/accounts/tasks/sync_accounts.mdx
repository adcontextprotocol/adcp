...
| Field | Description |
|-------|-------------|
| `account_id` | Vendor agent's identifier for this account. Pass this to protocol tasks (`create_media_buy`, `get_signals`, `activate_signal`, `report_usage`, etc.). May be absent when `status: "rejected"` or `action: "failed"`. Present for all other statuses including `pending_approval`. |
