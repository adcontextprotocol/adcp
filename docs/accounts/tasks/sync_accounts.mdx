...
| `operator` | string | Yes | Domain of the entity operating the seat (e.g. `pinnacle-media.com`). When the brand operates its own seat, set to the brand's domain. Verified against the brand's `authorized_operators` in brand.json. |
| `billing` | string | No | Who should be invoiced: `operator` or `agent`. Omit to accept the seller's default. The seller must either accept this billing model or reject the request. |
...
| Field | Description |
|-------|-------------|
| `account_id` | Seller-assigned account identifier. Pass this to protocol tasks (`create_media_buy`, `get_signals`, `activate_signal`, `report_usage`, etc.). May be absent when `status: "rejected"` or `action: "failed"`. |
...
| `billing` | Billing model applied. Matches the requested value, or the seller's default when omitted. |
...
for (const account of validated.accounts) {
  if (account.status === "active") {
    console.log(`Ready: ${account.brand.domain}/${account.brand.brand_id} → ${account.account_id}`);
...
async def main():
    ...
    for account in result.accounts:
        if account.status == 'active':
            print(f"Ready: {account.brand['domain']}/{account.brand.get('brand_id')} → {account.account_id}")
...
When a seller declines a request, the account entry has `status: "rejected"` and `account_id` is absent:
