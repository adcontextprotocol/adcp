...
`sync_accounts` is used across all seller protocols: media buy agents, signals agents, governance agents, and creative agents. It declares the buyer's intent — the seller provisions or links accounts internally. For implicit accounts (`require_operator_auth: false`), use natural keys (`brand` + `operator`) on subsequent requests. For explicit accounts (`require_operator_auth: true`), discover seller-assigned account IDs via [`list_accounts`](/docs/accounts/tasks/list_accounts). For sandbox operations, use the natural key with `sandbox: true` regardless of account model.
...
| `governance_agent` | object | No | Buyer's governance agent endpoint with authentication. When present, the seller MUST call this agent to [check governance](/docs/governance/campaign/tasks/check_governance) before confirming media buy requests. Contains `url` (agent endpoint), `authentication` (credentials the seller presents when calling), and optional `mode` (`audit`, `advisory`, or `enforce` -- default `enforce`). To rotate credentials, call `sync_accounts` again with updated authentication. |

**Natural key**: The tuple `(brand, operator, sandbox)` uniquely identifies an account relationship. `{brand: {domain: "acme-corp.com"}, operator: "acme-corp.com"}` (direct) is a different account from `{brand: {domain: "acme-corp.com"}, operator: "pinnacle-media.com"}` (via agency). Adding `sandbox: true` references the sandbox account for the same brand/operator pair.
...
| `governance_agent` | Governance agent endpoint (url + authentication), echoed from request when provided. |
