...
| `authorization` | Optional. The calling agent's scope grant for this account — `allowed_tasks`, `field_scopes`, `scope_name`, `read_only`. Present on `created`, `updated`, and `unchanged` results; omitted on `failed` results. Sellers that support scope introspection SHOULD populate this; sellers claiming the `attestation_verifier` standard scope MUST populate it. Absence means the seller does not advertise introspectable scope; callers MUST NOT infer access from absence. See [Caller authorization](/docs/accounts/overview#caller-authorization) for the full shape. |
