...
| `authorization_agent` | object | No | Buyer's authorization agent endpoint with authentication. When set, the seller calls this agent to [verify media buy authorization](/docs/governance/campaign/tasks/verify_authorization) before confirming purchases. Contains `url` (agent endpoint) and `authentication` (credentials the seller presents when calling). Follows the same authentication pattern as `push_notification_config`. |
