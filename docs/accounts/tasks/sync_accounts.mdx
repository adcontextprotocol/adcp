...
`notification_configs[]` carries account-level webhook subscribers for creative lifecycle and assignment changes, `insights.changed`, account status, wholesale feed changes, and future account-anchored events.

Insight and assignment subscriptions are prospective: activation or reactivation does not replay conditions that were already current. After activation, buyers perform an unfiltered baseline read using the repair tasks declared in `media_buy.insight_notifications` before relying on later invalidations.
...
- `event_types[]` — account-anchored types include `creative.status_changed`, `creative.assignment_changed`, `insights.changed`, `creative.purged`, `account.status_changed`, product/signal changes, and `wholesale_feed.bulk_change`. Media-buy delivery types and agent-level `capabilities.changed` are invalid here.
