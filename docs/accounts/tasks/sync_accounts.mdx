...
`sync_accounts` is used across all seller protocols: media buy agents, signals agents, governance agents, and creative agents. It declares the buyer's intent — the seller provisions or links accounts internally. On subsequent requests, pass either a natural key (`brand` + `operator` + optional `sandbox`) or a seller-assigned `account_id` from [`list_accounts`](/docs/accounts/tasks/list_accounts). Both forms are valid regardless of account model.
