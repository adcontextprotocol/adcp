...
<Note>
**Experimental.** The Trusted Match Protocol is part of AdCP 3.0 as an experimental surface — it may change between 3.x releases with at least 6 weeks' notice. TMPX exposure tokens, country-partitioned identity, and Offer macros are expected to evolve; see the [3.1.0 roadmap](https://github.com/adcontextprotocol/adcp/issues/2201) for planned changes. Sellers implementing TMP MUST declare `trusted_match` in `experimental_features`. See [experimental status](/docs/reference/experimental-status) for the full contract.
</Note>
