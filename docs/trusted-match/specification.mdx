...
| `macros` | Map\<string, string\> | No | Key-value pairs for dynamic creative rendering. In the GAM case, these flow as macro values at serve time. Distinct from the Identity Match `tmpx` field which carries an encrypted exposure token for frequency tracking. |
