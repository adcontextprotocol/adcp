...
| `tmpx_values` | List\<string\> | No | Ordered opaque TMPX chunks minted by this identity agent. Each entry is a URL-safe wire string. Position is the chunk's identity — the agent does NOT emit publisher-local names. Capped at 2 chunks in v1; the cap MAY rise without a shape change. Omitted when the agent mints no TMPX for this request. |
...
| `tmpx_providers` | Map\<string, \{ values: List\<string\> \}\> | No | Router-populated. Ordered TMPX value chunks grouped by the originating identity provider's `provider_id`. Each entry's `values[]` is a copy of the provider's emitted `tmpx_values` in the same order. Values are opaque URL-safe wire strings — the publisher's deployment configuration (see [Publisher TMPX macro mapping](#publisher-tmpx-macro-mapping)) resolves each `(provider_id, index)` pair to a local destination on this surface. Required by router conformance when any identity provider emitted TMPX in this request; collapsing per-provider tokens into a single string loses attribution and breaks per-provider impression accounting. Map keys MUST be valid provider_ids registered for this fan-out. |
...
**TMPX destinations are publisher-owned.** Naming an ad-server macro, targeting key, VAST substitution, or DOOH play-log field is a deployment concern owned by the publisher, not a protocol identifier. Providers emit ordered opaque values; the router preserves ordering under the emitting `provider_id`; the publisher's [Publisher TMPX macro mapping](#publisher-tmpx-macro-mapping) resolves each `(provider_id, ordinal)` to the local destination on that surface. Position within `tmpx_values` / `tmpx_providers[provider_id].values` is the chunk's identity — no slot IDs, no wire-side names. Ordered multi-chunk support lets a single TMPX exceed one macro slot (capped at 2 chunks per provider in v1 and MAY be raised without a shape change). This keeps the same identity provider mappable to different destinations across surfaces (GAM key-values vs. VAST macros vs. DOOH play-log fields) via publisher-side configuration, and closes the cross-provider name-hijack surface that existed while destination names travelled on the wire — the router never accepts a destination name from an untrusted provider.

`tmpx_providers` keys ordered value lists by `provider_id` so the router preserves attribution when its fan-out reaches multiple identity providers. The legacy `tmpx` field remains supported through 3.x for consumers that haven't migrated; when both fields are present, `tmpx_providers` is authoritative and the singular field SHOULD reflect one provider's first-slot value solely as a transitional convenience.
...
Where an identity provider's ordered TMPX values land in the ad server is a deployment concern owned by the publisher. Providers emit ordered opaque values on the wire (`tmpx_values` on the provider→router hop; `tmpx_providers[provider_id].values` on the router→publisher hop). The publisher maps each `(provider_id, ordinal)` to the ad-server macro name, GAM key-value, VAST URL macro, DOOH play-log field, or equivalent local destination for that surface. The mapping never travels between provider and router, so the router never accepts a destination name from an untrusted provider.
...
```yaml
tmpx_macro_mapping:
  provider_a: [PIN_TMPX_1, PIN_TMPX_2]
  provider_b: [NOVA_TMPX_1]
```

Names are provider-namespaced by convention so distinct providers do not collide on the same slot. The same `provider_id` may map to different destinations across surfaces (GAM key-values on web, VAST URL macros on CTV, play-log fields on DOOH) by carrying one instance of this configuration per surface. When a response carries more values for a provider than the mapping has slots for that provider, the publisher's adapter MUST fail closed for that provider on that impression — none of that provider's values are fired into the ad-serving path, and the adapter logs a configuration error. Other providers on the same response are unaffected. Fewer values than slots (partial fill) leave the trailing slots empty and the impression proceeds normally. A `provider_id` absent from the mapping — for example, a newly onboarded provider not yet trafficked on this surface — MUST cause the adapter to drop that provider's values on this surface rather than firing them into an unconfigured destination.

Because destinations are resolved from `(provider_id, ordinal)` on the publisher side, ordered position is sufficient for chunk identity: no slot IDs or protocol-level names are required. Position within the array is the chunk's identity across the fan-out; the mapping is the operator's contract with their own ad server.
...
1. For each property the package may serve, fetch `/.well-known/adagents.json` on the property domain. If the file contains an `authoritative_location` pointer, follow it at most one hop to a same-scheme HTTPS URL; do not chain further. Both the initial fetch and the `authoritative_location` hop MUST apply the [Webhook URL validation (SSRF)](/docs/building/by-layer/L1/security#webhook-url-validation-ssrf) rules — HTTPS-only, reserved IPv4 and IPv6 ranges rejected (including `::ffff:0:0/96` IPv4-mapped bypasses and the `169.254.169.254` / `fd00:ec2::254` cloud metadata addresses), no transparent redirects, and the TCP connection pinned to the validated IP. The inbound-fetch rules referenced for [Provider registration security](#provider-registration-security) apply equally to this outbound fetch.
...
For each terminal surface the publisher trafficks the destinations named in its [`tmpx_macro_mapping`](#publisher-tmpx-macro-mapping) — those names (macros, GAM key-values, VAST URL macros, DOOH play-log fields) are the operational contract the ad-server line items target. At serve time, the publisher walks `tmpx_providers[provider_id].values[]` from the response and substitutes each entry into the destination named by `tmpx_macro_mapping[provider_id][index]` for the ordinal. The legacy `{TMPX}` macro (from the deprecated singular `tmpx` field) remains supported for consumers that haven't migrated.

- **Web, mobile, CTV (SSAI), audio (DAI):** Standard impression pixel — substitute each `tmpx_providers[provider_id].values[index]` into the destination the publisher's mapping resolves for that `(provider_id, index)` pair.
- **CTV (client-side VAST):** Publisher performs the same per-ordinal substitution before the VAST document reaches the player.
- **DOOH:** Play-log-based — TMPX values are logged by the DOOH player into the play-log fields the publisher's mapping resolves for that `(provider_id, index)` rather than pixel URLs. Play log records MUST carry `provider_id` so the buyer can route to the correct decryption master.
