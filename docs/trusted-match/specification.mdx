...
| `ttl_sec` | integer | Yes | How long the router should cache this response, in seconds. During this window, the router serves at most one ad per eligible package without re-querying the buyer. |
...
The `ttl_sec` field is a caching contract. The buyer is saying: "Cache this for N seconds. During that window, serve at most one ad per eligible package." This eliminates the need for pod-specific or batch-specific protocol semantics — the router has cached eligibility and allocates across whatever placements exist during the TTL window (a CTV ad pod, a web page with 20 slots, a single pre-roll). The buyer doesn't need to know the allocation details.
...
Providers MAY support `context_match` without `identity_match` (e.g., pure enrichment providers). Providers MUST NOT support `identity_match` without `context_match` — the join requires offers from Context Match to intersect with eligibility from Identity Match. Without offers, eligibility has nothing to activate.
...
- Identity Match responses are cached per the `ttl_sec` value in the response. The router serves at most one ad per eligible package within the TTL window. Cache key: `{uid, provider_id}`.
