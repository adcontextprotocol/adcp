...
| `context_signals` | ContextSignals | No | Pre-computed classifier outputs for the content environment. Use when content is ephemeral (conversation turns, search queries) or to supplement artifact-based matching. Distinguish ephemeral content that many users encounter (a trending query, a syndicated segment) from ephemeral content authored by one user in one session: `embedding` is appropriate for the first, and for the second populate `topics`, `keywords`, `sentiment`, `language` and `summary` instead. Can replace `artifact_refs` entirely. Raw content MUST NOT be included — only classified outputs. The publisher is the classifier boundary. |
...
| `embedding` | string | No | Content embedding as base64-encoded int8 vector. Captures semantic content beyond topics and keywords. Publishers MUST NOT populate `embedding` from content authored by a single user within a single session (conversation turns, search queries, prompts). Unlike the vocabulary-bounded fields above, a dense vector cannot be inspected by the publisher before it is sent, so "only classified outputs" does not bound what it discloses. On `ai_assistant` and comparable property types, use `topics` and a publisher-authored `summary` instead. |
