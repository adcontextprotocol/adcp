...
| Field | Type | Required | Description |
|---|---|---|---|
| `topics` | List\<string\> | No | Topic taxonomy identifiers (e.g., `cooking.pasta`, `fitness.running`). |
...
| `brand_safety_tier` | enum | No | Publisher's brand safety classification: `safe`, `sensitive`, `unsafe`. |
