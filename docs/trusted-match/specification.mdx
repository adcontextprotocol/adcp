...
| `package_ids` | List\<string\> | No | Restrict evaluation to specific packages. When omitted, the provider evaluates all eligible packages for this placement (the common case). Package metadata (formats, catalogs, activation keys) is synced at media buy time — not sent per request. |
...
A buyer's response for a single package. Must include either `package_id` or `activation_key` — the seller controls which via `use_activation_keys` on the product's `trusted_match` configuration. When `use_activation_keys` is true, the router MUST reject offers that include `package_id` but not `activation_key`. The publisher resolves `activation_key` back to `package_id` for the identity match join using its synced package metadata.

| Field | Type | Required | Description |
|---|---|---|---|
| `package_id` | string | When `use_activation_keys` is false | Package identifier from the media buy. |
| `activation_key` | string | When `use_activation_keys` is true | Short key for ad server targeting (max 32 chars, alphanumeric/dash/underscore). The publisher uses this as the GAM key-value. |
...
| `use_activation_keys` | bool | No | Whether offers must use `activation_key` instead of `package_id` for targeting. When true, every synced package must have an `activation_key`, and the router rejects offers missing it. Default: false. |
| `providers` | List\<ProviderEntry\> | No | TMP providers integrated with this product's inventory. Each entry identifies a provider by `agent_url` (from the registry) and declares what match types it supports. Enables buyer discovery. |
