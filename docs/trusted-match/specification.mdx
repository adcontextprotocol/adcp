...
| **Artifact** | A typed content reference associated with a publisher property (article URL, episode EIDR, show Gracenote ID, conversation turn). Each artifact has a `type` and `value`. Referenced in context match requests. |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `type` | enum | Yes | One of: `url`, `url_hash`, `eidr`, `gracenote`, `rss_guid`, `isbn`, `custom`. |
| `value` | string | Yes | The identifier value. Format depends on `type`: URL for `url`, base64-encoded Blake3 hash for `url_hash`, EIDR ID for `eidr`, Gracenote ID for `gracenote`, RSS GUID for `rss_guid`, ISBN for `isbn`, publisher-defined string for `custom`. |
