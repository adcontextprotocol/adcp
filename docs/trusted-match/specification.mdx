...
| `tmpx_providers` | Map\<string, string\> | No | HPKE-encrypted exposure tokens keyed by the originating identity provider's `provider_id`. The router collects one TMPX token per identity provider that emitted one for this request and exposes them to the publisher as a per-provider map. Required by router conformance when more than one identity provider emitted a TMPX in this request; routers SHOULD also populate it for the single-provider case. The publisher trafficks `{TMPX_<provider_id>}` in their ad server (GAM key-value, VAST URL macro, DOOH play-log field) — `provider_id` is charset-constrained in provider-registration.json so the macro name is always a legal ad-server token without further encoding. Each value's wire format matches `tmpx`: `kid.base64url_nopad(ciphertext)`. Publishers MUST treat each value as opaque pass-through data. |
...
The response includes eligible package IDs, a serve-window throttle, and the optional `tmpx_providers` map (and its deprecated single-token counterpart `tmpx`). Each TMPX value is an HPKE-encrypted exposure token that flows through creative tracking URLs to the buyer's impression pixel, enabling real-time per-user frequency state updates without exposing user identity to the publisher. The buyer computes eligibility from whatever identity signals they have (frequency caps, audience membership, purchase history) and returns only the packages that pass. The publisher does not need to know why a package was excluded — just which packages are eligible.

`tmpx_providers` keys the tokens by `provider_id` so the router preserves attribution when its fan-out reaches multiple identity providers. Collapsing per-provider tokens to a single field loses that attribution and breaks per-provider impression accounting — a publisher that needs to fire each provider's token through a distinct macro cannot recover the mapping after the fact. The macro name is the publisher's `provider_id` verbatim wrapped in `{TMPX_…}` (e.g. `provider_id: "pinnacle_id"` → publisher trafficks `{TMPX_pinnacle_id}`); `provider_id`'s charset constraint at the provider-registration boundary keeps that token legal in every supported ad server. The legacy `tmpx` field remains supported through 3.x for consumers that haven't migrated; when both fields are present, the map is authoritative and the singular field SHOULD reflect one provider's token solely as a transitional convenience.

Single TMPX values that exceed one macro slot are out of scope for this version — the 255-char GAM budget with priority-ordered truncation (see § TMPX tokens, "Size budget") remains the answer. Provider-declared macro names plus multi-chunk encoding are tracked as a follow-up gated on routers actually hitting size-budget truncation in production.
...
|---|---|---|---|
| `provider_id` | string | Yes | Stable identifier for this provider registration. Used in logs, metrics, cache keys, and as the trailing token in the per-provider TMPX macro name (`{TMPX_<provider_id>}`) the publisher trafficks in their ad server. Charset is constrained to the macro-safe intersection across GAM, VAST URL parameters, and DOOH play-log fields (`^[A-Za-z0-9_]+$`), max length 64, so the synthesized macro name is always a legal ad-server token. |
...
For each registered identity provider the publisher trafficks `{TMPX_<provider_id>}` in their ad server (GAM key-value, VAST URL macro, DOOH play-log field). At serve time, the publisher reads each `tmpx_providers[provider_id]` value and substitutes it verbatim into that provider's macro slot. The legacy `{TMPX}` macro (from the deprecated singular `tmpx` field) remains supported for consumers that haven't migrated.

- **Web, mobile, CTV (SSAI), audio (DAI):** Standard impression pixel — substitute each `tmpx_providers[provider_id]` value into the matching `{TMPX_<provider_id>}` slot.
- **CTV (client-side VAST):** Publisher performs the same per-provider substitution before the VAST document reaches the player.
- **DOOH:** Play-log-based — TMPX values are logged by the DOOH player and included in play log records rather than pixel URLs. The play log MUST carry the `provider_id` alongside each TMPX value so the buyer can route to the correct decryption master.
