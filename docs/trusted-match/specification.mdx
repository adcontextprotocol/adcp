...
1. For each property the package may serve, fetch `/.well-known/adagents.json` on the property domain. The initial fetch MUST follow HTTP redirects only within the originally requested domain's registrable domain (eTLD+1), HTTPS-preserving and capped at 3 hops; it MUST anchor the comparison on the original domain and re-apply SSRF validation on every hop. If the file contains an `authoritative_location` pointer, fetch that exact same-scheme HTTPS URL with zero HTTP redirects and do not follow another document pointer. Both fetch stages MUST apply the [Webhook URL validation (SSRF)](/docs/building/by-layer/L1/security#webhook-url-validation-ssrf) rules — reserved IPv4 and IPv6 ranges rejected (including `::ffff:0:0/96` IPv4-mapped bypasses and the `169.254.169.254` / `fd00:ec2::254` cloud metadata addresses), and the TCP connection pinned to the validated IP. The inbound-fetch rules referenced for [Provider registration security](#provider-registration-security) apply equally to this outbound fetch.
...
<Warning>
The `cache_namespace` conformance requirements in this section are prospective
AdCP 3.2 requirements effective October 25, 2026. Version 3.1.22 already
introduced request partitioning as an emergency security correction. Until a
router implements both request partitioning and the trusted, result-affecting
security/configuration namespace below, it MUST bypass cache lookup, in-flight
coalescing, and insertion for affected Context Match traffic. See the
[breaking/security migration notice](/docs/reference/migration/trusted-match-context-cache).
</Warning>

Context Match responses are cacheable only when the result-affecting context is the same. Placement alone is not sufficient: artifacts, classified signals, geo, an explicit package subset, seller, or protocol version can change a provider's response. For AdCP 3.2 conformance on and after October 25, 2026, routers MUST partition each provider's Context Match response cache by `{provider_id, cache_namespace, context_hash}`. Equality of `provider_id` and the forwarded request body alone is insufficient when the response-affecting security or provider configuration context differs.
...
`provider_id` is the stable provider registration identity and MUST NOT be overloaded with rotating configuration or deployment generations. For each request, the router MUST select `cache_namespace` only from trusted authentication, authorization, deployment, and provider-configuration state; it MUST NOT accept the namespace or any of its inputs from the caller or request body. The namespace MUST NOT include or be derived from viewer identity, user tokens, or Identity Match state. Before consulting the cache and before returning a warm hit, the router MUST authenticate the current request and authorize the current publisher principal for the requested property under current policy. A prior authorization decision or possession of a cache key MUST NOT grant access.

`cache_namespace` is an opaque, non-secret value with equality-only semantics for the exact provider-side evaluation context of the outbound request. It MUST differ across provider-side authenticated principals or tenants whenever their authorization, entitlements, or tenant data can affect the response. It MUST also change whenever an authorization or entitlement revision, active-package data generation, provider endpoint replacement, provider configuration, deployed model, or targeting/rules generation can affect the response. A router MAY reuse a namespace across contexts only when it can guarantee that the difference cannot affect output. Credential rotation for the same principal does not require a new namespace unless it changes that evaluation context.

Implementations MAY assign a random namespace or construct a keyed pseudonym from internal, non-credential principal or tenant references plus immutable result-affecting generation identifiers. `cache_namespace` MUST NOT contain raw or directly encoded principal or tenant identifiers and MUST NOT be derived from credentials. Raw credentials, bearer tokens, session secrets, private keys, and other secret key material MUST NOT appear in a cache key. The namespace and its derivation metadata are non-secret but sensitive: they MUST NOT appear in logs, metric labels, or traces, and routers MUST apply the same access controls and retention boundary to them as to Context Match request data. The cache key MUST be represented as a native tuple or encoded with unambiguous length framing or canonical structured encoding; delimiter-only concatenation is insufficient.

The router MUST capture one immutable evaluation context and bind cache lookup, in-flight request coalescing, outbound endpoint and authentication selection, provider evaluation, and cache insertion to that same context. When the response arrives, the router MUST discard it for caching if the captured context is no longer current, and MUST NOT relabel it into a newer namespace. Namespace generation values MUST NOT be reused while an entry or in-flight request from the prior use can survive. A changed context MUST NOT be able to reach entries under its prior namespace; an unchanged, concurrently supported context MAY continue to use its own namespace. If the router cannot establish the applicable context or generation, or cannot coordinate generation change or invalidation with the provider, it MUST bypass cache lookup, coalescing, and insertion. A future response with `cache_ttl: 0` cannot invalidate an already-served warm hit.

On a cache hit, the router MUST set the returned response's `request_id` to the current request's `request_id`; it MUST NOT replay the cached request's correlation value. Raw request content MUST NOT appear directly in cache keys, logs, metric labels, or traces. `context_hash` and any retained hash preimage MUST NOT appear in logs, metric labels, or traces. The hash is a partitioning mechanism, not anonymization: routers MUST apply the same access controls and retention boundary to cached responses, `context_hash`, hash preimages, namespace mappings, and namespace derivation metadata as to Context Match request data.

Routers MAY prefix `context_hash` with `property_rid` for cache-store sharding, but that prefix does not replace any part of `{provider_id, cache_namespace, context_hash}` or change the partitioning guarantee. Requests carrying viewer-variable fields such as geo or per-session context signals can produce many distinct hashes and therefore a low hit rate; `cache_ttl: 0` is appropriate for those placements. The 5-minute default is sized for context-invariant placements.
