...
| Field | Type | Required | Description |
|---|---|---|---|
| `issuer` | string | Yes | Identity issuer, e.g. `world_id`. Free-form (no central scheme registry yet); selects the verifier. |
| `scheme` | string | Yes | Proof scheme and version, e.g. `world_id_v4`. |
| `rp_id` | string | No | Relying-party id the proof was minted for. Checked against the rp_id owner's published `identity_relying_parties[]` in `brand.json` (provenance). |
...
1. **Verify before trust.** Verify the proof for every `scheme` it accepts. An attestation that fails verification — or `signal_binding`, or `rp_id` provenance, or `expires_at` — MUST be treated as **absent** (no attestation), never as an asserted-true claim.
2. **No silent upgrade.** Never treat an unverified or unverifiable attestation as `true`.
3. **rp_id provenance.** Confirm the attestation's `rp_id` is published in the owning entity's `brand.json` `identity_relying_parties[]` before trusting it, so a genuine proof cannot be replayed under a different owner.
...
The router MUST: forward each `sealed_credentials[]` entry only to the provider that owns its `audience_kid` (route-by-audience, not broadcast); fold `sealed_credentials` into the per-provider re-signature canonical bytes so an injected or swapped blob breaks the request signature; and include a `sealed_credentials_hash` in its dedup cache key so a network-credential-driven eligibility change invalidates a cached response.
