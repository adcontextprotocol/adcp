...
| `tmpx_providers` | Map\<string, string\> | No | HPKE-encrypted exposure tokens keyed by the originating identity provider's `provider_id`. The router collects one TMPX token per identity provider that emitted one for this request, so each provider's token can flow through its own creative macro (e.g. `{TMPX_<provider_id>}`). Required by router conformance when more than one identity provider emitted a TMPX in this request; routers SHOULD also populate it for the single-provider case. Each value's wire format matches `tmpx`: `kid.base64url_nopad(ciphertext)`. Publishers MUST treat each value as opaque pass-through data. |
...
- **Web, mobile, CTV (SSAI), audio (DAI):** Standard impression pixel — `{TMPX}` substituted at serve time. When the response carries `tmpx_providers` with more than one entry, the publisher substitutes `{TMPX_<provider_id>}` per provider; `{TMPX}` (when also populated for back-compat) carries one provider's token for legacy consumers.
- **CTV (client-side VAST):** Publisher substitutes `{TMPX}` (or `{TMPX_<provider_id>}` per provider) before the VAST document reaches the player.
- **DOOH:** Play-log-based — TMPX tokens are logged by the DOOH player and included in play log records rather than pixel URLs. Per-provider tokens (when present) MUST be logged with their `provider_id` so the buyer can route each token to the correct decryption master.
