...
- Baseline: JSON over HTTP/2 POST. Implementations MUST support this.
...
- HTTP/3 (QUIC) is a recommended upgrade path — 0-RTT connection establishment and multiplexing without head-of-line blocking improve fan-out performance.
- Binary serialization (protobuf, Cap'n Proto) MAY be used for high-throughput deployments by mutual agreement between router and provider.
