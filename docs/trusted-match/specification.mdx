...
| `macros` | Map\<string, string\> | No | Key-value pairs for dynamic creative rendering. In the GAM case, these flow as macro values at serve time. Distinct from Identity Match `macros` which carry encrypted exposure tokens (TMPX) for frequency tracking. |
...
| `macros` | Map\<string, string\> | No | Key-value pairs for impression-time macro substitution. The `TMPX` macro carries an HPKE-encrypted exposure token containing resolved user identity tokens. The publisher substitutes this into creative tracking URLs as `{TMPX}`. The buyer's impression pixel receives the token, enabling real-time per-user frequency state updates. Publishers MUST treat these values as opaque pass-through data. |

The response includes eligible package IDs, a TTL, and optional macros. The `macros` field carries encrypted exposure tokens (TMPX) that flow through creative tracking URLs to the buyer's impression pixel, enabling real-time per-user frequency state updates without exposing user identity to the publisher. The buyer computes eligibility from whatever identity signals they have (frequency caps, audience membership, purchase history) and returns only the packages that pass. The publisher does not need to know why a package was excluded — just which packages are eligible.
...
TMPX uses HPKE (RFC 9180) with `mode_auth`:

| Parameter | Value |
|---|---|
| Mode | `mode_auth` — sender (read replica) authenticates with its own keypair |
| KEM | DHKEM(X25519, HKDF-SHA256) |
| KDF | HKDF-SHA256 |
| AEAD | ChaCha20-Poly1305 |

The buyer's cluster master holds the recipient private key. Each read replica holds its own sender keypair for authentication. The master verifies the sender's public key during decryption, rejecting tokens not produced by an authorized replica.
...
The TMPX macro value uses the format `<kid>.<base64url_ciphertext>`. The `kid` (key identifier, max 8 characters) is opaque — it MUST NOT encode geographic or deployment information. It maps to a cluster master private key internally.
...
Two keypair types per cluster:

1. **Master keypair** — the cluster master holds the private key for decryption. The public key is published in `encryption_keys` on agent authorization entries in `adagents.json`.

2. **Replica keypairs** — each read replica generates its own X25519 keypair for HPKE `mode_auth` sender authentication. The master maintains a registry of authorized replica public keys. Replica private keys never leave the replica.
...
The TMPX token is generated once per Identity Match evaluation and cached alongside the eligibility response for `ttl_sec` seconds. All impressions within the TTL window share the same TMPX value. The buyer's master MUST NOT deduplicate by TMPX value — each pixel fire is one impression.
