...
2. For each matching provider, it checks the provider's Context Match response cache using the `{provider_id, cache_namespace, context_hash}` key defined in [Caching](/docs/trusted-match/specification#caching). This key is a prospective AdCP 3.2 conformance requirement effective October 25, 2026. Until both request and trusted evaluation-context partitioning are implemented, the router bypasses caching for affected traffic. A placement-only key is unsafe because two requests on the same placement can carry different artifacts, signals, geo, sellers, or package subsets; a body-only key is still unsafe when the authenticated principal, tenant, provider configuration, deployed model, or targeting/rules generation can change the result. On a hit, it replaces the cached response's `request_id` with the current request's value.
