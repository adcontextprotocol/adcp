...
The TMP Router is a single Go binary. It reads a configuration file listing providers and their capabilities, then exposes a single HTTP/2 endpoint. The request `type` field distinguishes Context Match from Identity Match — the router uses type-based dispatch, not URL paths.
