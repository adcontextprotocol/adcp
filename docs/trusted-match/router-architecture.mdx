...
2. For each matching provider, it checks the provider's Context Match response cache using the `{provider_id, context_hash}` key defined in [Caching](/docs/trusted-match/specification#caching). A placement-only key is unsafe because two requests on the same placement can carry different artifacts, signals, geo, sellers, or package subsets. On a hit, it replaces the cached response's `request_id` with the current request's value.
