...
3. It further filters to providers whose `uid_types` list includes the request's `uid_type`.
4. It **strips the `country` field** before forwarding the request to the buyer agent.
5. It fans out to all matching providers in parallel, merges eligibility results, and returns a unified response.
