...
│  │  • Pre-computed signals    │  │  • ALL active package IDs  ││
│  │  • Available packages      │  │                            ││
│  │                            │  │  Outputs:                  ││
...
| `wire_formats` | list | Supported formats: `capnp`, `json` |
...
The router negotiates wire format with each provider based on their declared capabilities. Cap'n Proto is preferred when both formats are available — it eliminates serialization overhead and keeps payloads small (~100-300 bytes for a typical context match request). JSON is supported as a development and debugging fallback.
...
| Request format | Full OpenRTB BidRequest (~2-10KB JSON) | TMP ContextMatchRequest (~100-300 bytes Cap'n Proto) |
