...
If two providers return eligibility for the same package (uncommon), the router applies conservative merging: the package is only eligible if it appears in `eligible_package_ids` from both providers. The router uses the minimum `ttl_sec` across providers.
