...
| `axex` (exclude segment) | Identity Match `eligible: false` | User fails frequency cap, suppression list, or audience rule |
...
5. **Frequency state** is separate during parallel operation — AXE tracks its own counters, TMP tracks its own

<Warning>
During parallel operation, campaigns on AXE and campaigns on TMP maintain independent frequency counters. A user exposed to an AXE campaign will not have that exposure reflected in TMP's Identity Match, and vice versa. To avoid over-frequency during migration, avoid running the same campaign on both systems simultaneously. Migrate campaigns one at a time: move a campaign to TMP, let its AXE frequency state expire, then move the next.
</Warning>
...
| `axe_exclude_segment` | Identity Match — buyer checks suppression/frequency |
