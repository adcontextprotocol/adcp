...
The two paths are structurally separate: no shared memory, no shared state, no communication channel. The router cannot associate a user token with a page URL because no single code path ever holds both. See [Privacy Architecture](/docs/trusted-match/privacy-architecture) for the enforcement mechanism.
...
[AXE](/docs/media-buy/advanced-topics/agentic-execution-engine), TMP's predecessor, had a weaker data protection posture:
...
| Profile construction risk | Operator could theoretically build browsing profiles | Architecturally constrained for the router (no code path holds both signals); buyer-side correlation impeded by [decorrelation mechanisms](/docs/trusted-match/privacy-architecture) but depends on publisher compliance with SHOULD-level requirements |
...
Context Match caches can also contain sensitive request-derived decisions. Version 3.1.22 introduced request partitioning as an emergency security correction; the additional trusted evaluation-context namespace is a prospective AdCP 3.2 conformance requirement effective October 25, 2026. Until both are implemented, affected routers must bypass or disable caching. Routers that cache must partition provider responses by both the canonical hash of the complete result-affecting forwarded request and a trusted namespace for the provider-side authentication, tenant, authorization, active-package, endpoint, configuration, model, and rules context. Placement or body equality alone is insufficient; otherwise one session's artifact, signals, geo, package selection, or provider evaluation context can determine the response served to another. Namespace selection comes from trusted router state, never caller input, viewer identity, Identity Match state, raw principal or tenant identifiers, or credentials. Warm hits still require current authentication, authorization, and property access. Request content, context hashes, namespaces, and namespace derivation metadata must not appear in observability labels. Hashes and pseudonymous namespaces prevent direct disclosure in the key but are not anonymization, so cached responses and all key derivation metadata require the same access controls and retention boundary as Context Match data. See the [breaking/security migration notice](/docs/reference/migration/trusted-match-context-cache).
