...
The publisher sends a Context Match request to each buyer's agent. The request contains the page context. It contains no user identity and no package list — the buyer agent uses its synced package set for this placement.
...
Separately — and with a random time delay — the publisher sends an Identity Match request to each buyer's agent. The request contains an opaque user token and the list of packages to evaluate. It contains no page context.
