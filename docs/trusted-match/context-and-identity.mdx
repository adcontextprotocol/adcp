...
```json
{
  ...
  "context_signals": {
    "topics": ["632"],
    "keywords": ["coffee products", "non-alcoholic beverages", "product discovery"],
    "sentiment": "positive",
...
Resolve the provider-attributed targeting tuples through the publisher's [`targeting_kv_mapping`](/docs/trusted-match/specification#publisher-targeting-kv-mapping), then set the mapped ad-server keys. Drop any tuple without a mapping.
```
