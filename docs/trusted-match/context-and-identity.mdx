...
The `ttl_sec: 60` tells the router: "Cache this for 60 seconds. During that window, serve at most one ad per eligible package." The router uses this cached eligibility to fill whatever placements exist — a single slot, a CTV ad pod, or a page with multiple ad units — without re-querying the buyer.
