...
- `ttl_sec` tells the router how long to cache this response. During that window, the router serves at most one ad per eligible package — across all placements on the page — without re-querying the buyer.
