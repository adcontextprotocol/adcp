...
```json
{
  "request_id": "ctx-7f2a-oakwood-91b3",
  "offers": [
    { "package_id": "pkg-display-0041", "activation_key": "d41" },
    { "package_id": "pkg-native-0078", "activation_key": "n78" }
  ],
  "signals": {
    ...
    "targeting_kvs": [
      ...
      { "key": "adcp_pkg", "value": "d41" },
      { "key": "adcp_pkg", "value": "n78" }
    ]
  }
}
```
...
- For web/GAM activation, offers are simple -- just `package_id` and `activation_key`. Richer fields (`brand`, `price`, `summary`, `creative_manifest`, `macros`) are available for integrations that need them but are not required.
- `activation_key` is a short, ad-server-safe string assigned at media buy creation. GAM key-values use `activation_key` instead of the full `package_id` to avoid exposing internal IDs in page markup and to respect ad server length limits. When absent, `package_id` is used directly.
...
GAM line items are pre-configured to target on `adcp_pkg` values using the `activation_key`. When the ad request arrives with `adcp_pkg=d41`, GAM matches it to the corresponding line item and serves the creative.
