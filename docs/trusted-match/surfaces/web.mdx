...
Separately, the TMP Prebid module sends an Identity Match request with the user's opaque token and the publisher's `seller_agent_url`. This request carries no page context. The buyer resolves its active package set from `seller_agent_url`; when the module sends `package_ids` explicitly (as below), the list MUST include ALL active packages for the buyer — not just the packages on this page.
