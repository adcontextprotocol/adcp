...
The TMP SDK sends an Identity Match request with a publisher-scoped device token and the publisher's `seller_agent_url`. This request is structurally separate from Context Match — it carries no content signals and is sent with temporal decorrelation (a random delay of 100-2000ms).

The buyer resolves its active package set from `seller_agent_url`. When the SDK sends `package_ids` explicitly (as below), the list MUST include all active packages for the buyer, not just the packages from the current placement — sending only the placement-specific subset would let the buyer correlate Identity Match with Context Match by comparing package sets.
