...
Priya deploys the TMP Router — the piece that sits between her ad server and buyer agents, with structurally separate paths for context and identity. She configures Sam's buyer agent (Pinnacle) as a TMP provider, alongside other buyers.
...
A viewer opens a StreamHaus article about hiking gear. StreamHaus's properties are registered in the [property catalog](/docs/building/property-catalog) with stable `property_rid` identifiers, so the buyer knows exactly which property this request came from. StreamHaus sends a **Context Match** request: the article URL, and all available packages for the placement.
...
The buyer agent already knows which packages are active for this placement — it set them up via `create_media_buy`. No package list needs to travel on the wire.
...
The `package_ids` list includes ALL of Sam's active packages — not just those relevant to the current page. Sending only the page-specific subset would let the buyer match package sets across Context Match and Identity Match, breaking the structural separation.
...
<CardGroup cols={2}>
  <Card title="AI assistant mediation" icon="robot" href="/docs/trusted-match/ai-mediation">
    The first mediation protocol for AI assistants — how demand finds conversational AI when the context can't be broadcast.
  </Card>
