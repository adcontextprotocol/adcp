...
The example sends `package_ids` explicitly, but the publisher MAY omit it — Sam's identity-match service resolves the active package set from `seller_agent_url`. When `package_ids` IS sent, it MUST include ALL of Sam's active packages, not just those relevant to the current page; a page-specific subset would let the buyer correlate package sets across Context Match and Identity Match, breaking the structural separation.
