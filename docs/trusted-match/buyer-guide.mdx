...
| Identity Match | Opaque user token, all active package IDs | Eligible package IDs + TTL |
...
Return only the package IDs that pass your eligibility checks. Packages not in the list are treated as ineligible. The `ttl_sec` tells the router how long to cache this response — during that window, the router serves at most one ad per eligible package without re-querying you. Set the TTL based on how quickly your eligibility state changes (frequency caps, audience updates, etc.).
...
Context Match is required. Identity Match is optional — you can support Context Match only (contextual targeting without frequency capping) or both. Providers MUST NOT support Identity Match without Context Match — the publisher joins offers from Context Match with eligibility from Identity Match. Without offers, there's nothing to activate.
...
- The router caches the response for `ttl_sec` seconds, keyed by `{uid, provider_id}`
- During that window, the router serves **at most one ad per eligible package** without re-querying the buyer
- This works for any allocation scenario — a single pre-roll, a CTV ad pod, or a web page with multiple ad units
- The buyer doesn't need to know how many placements exist or how the router allocates
