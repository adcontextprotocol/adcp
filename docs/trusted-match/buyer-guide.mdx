...
A buyer agent exposes two HTTP endpoints:

| Endpoint | Receives | Returns |
|---|---|---|
| Context Match | Page/content signals, placement, geo | Offers with creative manifests |
| Identity Match | Opaque user token, all active package IDs | Eligibility per package |

Both endpoints must respond in under 50ms. The router enforces this budget and will skip slow providers.
...
// Your response
{
  "request_id": "id-9c4e",
  "eligibility": [
    { "package_id": "acme-outdoor-q2", "eligible": true, "intent_score": 0.82 },
    { "package_id": "acme-winter-clearance", "eligible": false },
    { "package_id": "acme-loyalty-retarget", "eligible": true, "intent_score": 0.45 }
  ]
}
```
...
1. Intersects your Context Match offers with your Identity Match eligibility
...
- **Return `eligible: false`** when a cap is hit
...
Register your endpoints with each publisher's router. The publisher configures your provider entry in their router's configuration file (see [router deployment](/docs/trusted-match/router-architecture#deployment)). This is an operational relationship — the publisher adds your endpoint after you establish a media buy via `create_media_buy`.
...
Context Match is required. Identity Match is optional — you can support Context Match only (contextual targeting without frequency capping) or both. Providers MUST NOT support Identity Match without Context Match.
...
The `intent_score` field on Identity Match eligibility is a 0.0–1.0 signal indicating how strongly this user matches the package's target audience. Publishers may use it for ranking when multiple packages are eligible.

- **0.0–0.3**: Weak match (user is in a broad audience segment)
- **0.4–0.7**: Moderate match (user shows relevant behavioral signals)
- **0.8–1.0**: Strong match (user is in a high-intent segment, e.g., recent site visitor, cart abandoner)

Intent score is optional. If omitted, the publisher treats all eligible packages equally.

## Performance Requirements

| Metric | Target |
|---|---|
| Context Match response | < 50ms p99 |
| Identity Match response | < 50ms p99 |
| Availability | 99.9% |
| Error rate | < 0.1% |

These are p99 targets, not hard cutoffs. The router tracks your latency percentiles and adaptively adjusts your timeout allocation. Consistently slow responses (p95 exceeding the budget) result in the router reducing your allocation or skipping your provider.
...
| **You return** | Bid price | Offer (creative manifest) or eligibility (boolean) |
