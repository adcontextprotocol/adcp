...
1. Resolve the tokens against your identity graph on whichever `uid_type`s you support. Entry order in the request is not semantically significant — apply your own preference order (e.g., prefer deterministic graphs over probabilistic ones). All entries identify the same user, so the first successful resolution is sufficient.
...
- The router caches the response for `ttl_sec` seconds, keyed by `{identities_hash, provider_id, hash(package_ids), consent_hash}` (see spec for canonical bytes). `identities_hash` is computed over the per-provider filtered subset you received — your cache partition is scoped to the identity types you resolve.
