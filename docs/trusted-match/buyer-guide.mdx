...
| `identity_match_request` | Seller agent URL, opaque user token, optional package ID list | Eligible package IDs + TTL |
...
The router sends you the seller's `seller_agent_url` and one or more opaque identity tokens. Use `seller_agent_url` to look up the active package set you have registered for that seller, resolve identities on whichever tokens you support, then check each package's eligibility rules against the resolved user. The publisher MAY also send `package_ids` to scope evaluation explicitly; when present, treat it as the candidate set for this request.
