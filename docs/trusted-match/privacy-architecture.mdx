...
The Identity Match response can include a `macros` field carrying a `TMPX` token — an HPKE-encrypted blob containing the user's resolved identity tokens. This token flows through creative tracking URLs to the buyer's impression pixel for per-user exposure tracking.
...
- The router never sees the decrypted contents — it passes the opaque `macros` field through.
- The publisher substitutes the value into tracking URLs without interpreting it.
- Only the buyer's cluster master can decrypt the token (HPKE `mode_auth` with the master's private key).
