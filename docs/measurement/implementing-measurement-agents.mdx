...
```json
{
  ...
  "measurement": {
    "campaign_measurement": {
      "methodologies": ["incrementality_test", "brand_lift_study", "geo_experiment"],
      "metric_types": [
  ...
  "media_buys": [
    {
      ...
      "trackers": [
        ...
        },
        {
          ...
          "description": "Viewability tracker"
        }
      ]
    }
  ]
...
Trackers reuse the shared `url-asset.json` schema from the creative domain. Use `url_type` to distinguish between pixel trackers (`tracker_pixel`) and script-based measurement SDKs (`tracker_script`).
...
- [ ] `methodologies` lists all methodology types your agent supports
