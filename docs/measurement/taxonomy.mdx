...
For the full capability → commitment → optimization → delivery picture across both standard and vendor metric flows, see [The metric lifecycle](/docs/media-buy/media-buys/optimization-reporting#the-metric-lifecycle---from-capability-to-reporting). The same `(vendor, metric_id)` key flows through every surface for vendor-attested metrics — discovery, optimization capability, reporting capability, package commitment, optimization goal, performance standard, and delivery value.
