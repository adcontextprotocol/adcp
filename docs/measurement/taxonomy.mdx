...
- **AI-content provenance** is a _verification_ concern (was this ad what the brand approved?), not an attribution concern. It belongs alongside other verification capabilities — see [Provenance Verification](/docs/governance/creative/provenance-verification) — not bolted onto outcome reporting.
...
The data path remains intentionally flexible, but the buyer orchestrator is the hub. It exposes buyer-approved delivery to the provider through the same patterns a seller would use—`get_media_buy_delivery`, reporting webhooks, or a cloud bucket—or through an existing SDK, clean room, or offline integration. The common AdCP output is the compact [`provide_performance_feedback`](/docs/media-buy/task-reference/provide_performance_feedback) assertion returned to that gateway.

Buyer orchestrators declare the experimental `measurement_gateway` capability when they expose this boundary. Gateway and provider capability lists negotiate pull, webhook, or offline delivery and task, webhook, or offline feedback. Task paths use narrowly scoped `authorization.allowed_tasks`; webhook and offline paths use provisioned signing or cloud credentials. The provider never receives seller credentials. The orchestrator authenticates and normalizes the provider result, maps cross-seller identifiers, and submits appropriate feedback to each seller under the buyer's identity.

This resolves the immediate protocol-home question with a narrow principle: **first-class measurement-agent identity and output through an orchestrator-controlled gateway, not direct seller integration or mandatory AdCP ingestion**. A future provider task surface should be added only for a concrete workflow that cannot fit the orchestrator's existing delivery reads, webhooks, buckets, provider-hosted evidence, and feedback submission.
