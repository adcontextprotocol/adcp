...
**Schema**: [`core/exposure-event.json`](/schemas/core/exposure-event.json)
...
**Schema**: [`core/readiness.json`](/schemas/core/readiness.json)
...
**Schema**: [`core/measurement-capabilities.json`](/schemas/core/measurement-capabilities.json)
...
| `exposure_event_version` | string | Accepted exposure event schema version |

### Methodology profiles

Each scope contains a `methodologies` array of structured profiles designed for AI assistants to recommend approaches:

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `type` | string | Yes | Methodology identifier |
| `name` | string | Yes | Display name |
| `description` | string | Yes | Plain-English explanation |
| `best_for` | string[] | No | Scenarios where this methodology excels |
| `limitations` | string[] | No | Known limitations |
| `minimum_requirements` | object | No | Minimum data requirements |
| `time_to_insight_days` | integer | No | Expected days to first results |
| `methodology_url` | string (uri) | No | Link to detailed documentation |

### Methodology types

| Type | Description |
|------|-------------|
| `mmm` | Marketing mix modeling -- regression-based channel-level analysis |
...
| `mroi` | Marginal ROI at current spend levels |
| `cpik` | Cost per incremental KPI |
| `channel_contribution` | Channel's share of total measured outcome (0-1) |
    ...
    "methodologies": [
      {
        ...
        "description": "Geographic holdout test comparing treatment and control regions to measure causal lift from advertising",
        "best_for": [
        ...
        "limitations": [
          ...
          "Higher cost due to withheld impressions in control regions"
        ],
        ...
        "time_to_insight_days": 42
      },
      {
        "type": "incrementality_test",
        "name": "Randomized incrementality",
        "description": "Audience-level randomized experiment measuring the causal effect of ad exposure on conversions",
        "best_for": [
          "Measuring lift for digital channels with audience targeting",
          "Creative-level incrementality"
        ],
        "limitations": [
          "Requires audience-level signal integration",
          "Not suitable for channels without user-level targeting"
        ],
        "minimum_requirements": {
          "history_weeks": 4,
    ...
    "methodologies": [
      {
        ...
        "description": "Regression-based analysis of spend vs outcomes across channels over time, providing budget allocation guidance",
        "best_for": [
          "Long-term budget allocation across channels",
          "Understanding channel saturation and diminishing returns"
        ],
        "limitations": [
          "Requires 52+ weeks of historical data",
          "Cannot attribute at user or creative level"
        ],
        "minimum_requirements": {
          "history_weeks": 52,
...
**Schema**: [`measurement/register-media-buys-request.json`](/schemas/measurement/register-media-buys-request.json) / [`measurement/register-media-buys-response.json`](/schemas/measurement/register-media-buys-response.json)
...
- Measurement agents MAY return `trackers` (impression, click, or viewability URLs) for the orchestrator to attach to ad serving
...
**Schema**: [`measurement/get-measurement-results-request.json`](/schemas/measurement/get-measurement-results-request.json) / [`measurement/get-measurement-results-response.json`](/schemas/measurement/get-measurement-results-response.json)
...
3. Provide methodology profiles in capabilities
4. Include interpretation hints for AI assistants
...
| Schema | Description |
|--------|-------------|
| [`measurement/register-media-buys-request.json`](/schemas/measurement/register-media-buys-request.json) | register_media_buys request |
| [`measurement/register-media-buys-response.json`](/schemas/measurement/register-media-buys-response.json) | register_media_buys response |
| [`measurement/get-measurement-results-request.json`](/schemas/measurement/get-measurement-results-request.json) | get_measurement_results request |
| [`measurement/get-measurement-results-response.json`](/schemas/measurement/get-measurement-results-response.json) | get_measurement_results response |
| [`core/exposure-event.json`](/schemas/core/exposure-event.json) | Exposure event |
| [`core/measurement-capabilities.json`](/schemas/core/measurement-capabilities.json) | Measurement capabilities |
| [`core/readiness.json`](/schemas/core/readiness.json) | Readiness assessment |
| [`core/measurement-metric.json`](/schemas/core/measurement-metric.json) | Measurement metric |
| [`core/data-transport.json`](/schemas/core/data-transport.json) | Data transport configuration |
| [`core/channel-reallocation.json`](/schemas/core/channel-reallocation.json) | Channel reallocation recommendation |
| [`core/tracker.json`](/schemas/core/tracker.json) | Tracking URL |
