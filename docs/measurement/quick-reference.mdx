...
### URL asset type (trackers)

Measurement trackers reuse the shared URL asset type enum from the creative domain.

| Value | Description |
|-------|-------------|
| `tracker_pixel` | Fires GET request on event, expects pixel/204 response |
| `tracker_script` | Measurement SDK loaded as `<script>` tag (OMID verification, native event trackers) |
| `clickthrough` | User click destination (landing page) |
