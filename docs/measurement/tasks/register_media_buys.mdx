...
Trackers reuse the shared `url-asset.json` schema from the creative domain.

| Field | Type | Description |
|-------|------|-------------|
| `url` | string (URI) | Tracking URL to fire on the corresponding event |
| `url_type` | string | URL asset type: `tracker_pixel` (fires GET, expects pixel/204) or `tracker_script` (measurement SDK loaded as `<script>`) |
| `description` | string | What this tracker measures (e.g., "Impression tracker") |
