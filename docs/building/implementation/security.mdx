...
**Known gap — body well-formedness.** The request-signing checklist does NOT currently include the duplicate-object-keys body-well-formedness check that the [webhook-signing profile](#webhook-callbacks) applies at step 14. Request bodies carry state-change and spend-committing payloads (`create_media_buy`, `update_media_buy_delivery`, etc.) whose parser-differential blast radius is larger than webhooks' status-flip blast radius (cf. CVE-2017-12635), so extending the rule to this surface is a security goal, not a style preference. The extension is deferred to [#2523](https://github.com/adcontextprotocol/adcp/issues/2523) pending a one-pass audit of the error taxonomy (`request_body_malformed` vs. reusing `webhook_body_malformed`, ordering relative to the cap-state invariant at step 13, coverage of `idempotency_key` collisions in the same payload) — not because request signing is safe without the check. Implementers SHOULD adopt the same body-well-formedness posture (strict-parse, structured malformed-body error, reject after replay-cache insert) on the request-signing path now; a future spec edit will formalize it. SDK authors reading this checklist as exhaustive would silently ship a verifier that accepts parser-differential request bodies — this paragraph exists specifically so that does not happen.
...
**Verifier checklist for webhooks.** Apply these 15 checks in order, short-circuiting on the first failure. The steps below are the [request verifier checklist](#verifier-checklist-requests) with two substitutions — the `tag` value (`adcp/webhook-signing/v1` instead of `adcp/request-signing/v1`) and the direction-of-trust resolution (seller's brand.json `agents[]` entry instead of the buyer's) — plus one added step (14, body well-formedness) not present on the request-signing path. Implementations SHOULD share verifier code between the two profiles and branch on those points. Error codes are prefixed `webhook_*` — most carry the `webhook_signature_*` infix, plus structural codes without it (currently `webhook_target_uri_malformed`, `webhook_mode_mismatch`, `webhook_body_malformed`) — so caller-side error handling distinguishes the two profiles.
...
14. **Body well-formedness.** Verifiers MUST reject bodies containing duplicate object keys (`webhook_body_malformed`). Per RFC 8259 §4, duplicate-key parse behavior is unpredictable — the signature is valid over the bytes on the wire, but two parsers can disagree on the parsed value, which is a parser-differential attack class (cf. CVE-2017-12635). This check closes the gap between the signature verifier's view of the payload and the downstream consumer's view. The check MUST use a parser that exposes duplicate keys — a last-wins/first-wins default that silently discards them does not satisfy this requirement. Per-language escape hatches, non-exhaustive: Python stdlib `json` accepts `object_pairs_hook=` (detect duplicate keys in the hook and raise); Node has no strict mode in `JSON.parse`, use `secure-json-parse` or an equivalent streaming parser that emits duplicate-key events; Go `encoding/json` has no strict mode at all, use `json.Decoder` token-walk with a manual unique-key guard or a third-party strict parser (`goccy/go-json`, `tidwall/gjson`); Java Jackson enables `DeserializationFeature.FAIL_ON_READING_DUP_TREE_KEY`; Ruby stdlib `JSON.parse` has no detection hook, use `Oj.load` with `strict_mode`. Signers SHOULD ALSO reject duplicate-key input before serialization so a signer whose upstream caller sent a duplicate-key payload emits a clean error rather than a silently-collapsed signed frame. A verifier that crashes rather than returning a structured `webhook_body_malformed` error is conformant-but-suboptimal — senders receive no actionable error code. The conformance fixture for this check is the `duplicate-keys-conflicting-values` vector in `static/test-vectors/webhook-hmac-sha256.json` — the 9421 profile MUST apply the same body-well-formedness rule after signature verification succeeds. `webhook_body_malformed` is distinct from `webhook_signature_digest_mismatch`: the signature IS valid; the body parses to ambiguous state.
