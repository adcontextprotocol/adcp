...
Operators SHOULD go further and serve each signing purpose from a distinct subdomain (up to four origins). Defense-in-depth: governance keys SHOULD be on offline-rotation (HSM/KMS with manual rotation and human approval), while transport and webhook keys MAY use automated rotation. Operators advertise their separation scheme by publishing an `identity.key_origins` map in `get_adcp_capabilities` (doc-only for 3.x — no schema change in this release; implementers SHOULD include the field and verifiers SHOULD honor it when present) so buyers can verify origin separation at onboarding rather than discovering co-tenancy after a compromise.
...
- `supported`: MUST be `true` for any seller that emits webhooks. Buyers reject onboarding on `false` or missing.
- `profile`: MUST be exactly `adcp/webhook-signing/v1` for this profile version. Future profile versions bump the string.
- `algorithms`: subset of `["ed25519", "ecdsa-p256-sha256", "rsa-pss-sha512"]` — the algorithm set this seller will sign with.
