...
`idempotency_key` is **required** on every mutating request (`create_media_buy`, `update_media_buy`, `sync_creatives`, `activate_signal`, `acquire_rights`, `creative_approval`, `update_rights`, `build_creative`, `calibrate_content`, `create_content_standards`, `update_content_standards`, `create_property_list`, `update_property_list`, `delete_property_list`, `create_collection_list`, `update_collection_list`, `delete_collection_list`, `log_event`, `provide_performance_feedback`, `report_usage`, `report_plan_outcome`, `si_initiate_session`). Sellers MUST reject any mutating request that omits it with `INVALID_REQUEST`.

#### Normative seller behavior

1. **First call is canonical.** The seller processes the request once and stores the canonical response keyed by `(authenticated_principal, idempotency_key)` along with a hash of the canonical request payload.
2. **Replay returns the cached response.** A subsequent request with the same `idempotency_key` AND an equivalent payload (see "Payload equivalence" below) MUST return the stored response verbatim without re-executing side effects (no duplicate buys, charges, or webhooks). The replayed response MUST set `replayed: true` at the top level so the buyer can distinguish a cached response from a fresh execution.
3. **Key reuse with a different payload is a conflict.** A request with the same `idempotency_key` but a payload whose canonical hash differs MUST be rejected with `IDEMPOTENCY_CONFLICT`. Sellers MUST NOT silently apply the second request.
4. **Replay window.** Sellers SHOULD declare their replay-window TTL in `capabilities.idempotency.replay_ttl_seconds` (minimum 24 hours / 86400 seconds recommended). When the capability is omitted, buyers MUST assume the 24-hour minimum. After the TTL elapses, the seller MAY discard the stored response. Keys reused after expiry are treated as new requests.
5. **Scope.** Keys are scoped to the authenticated principal on the seller. Cross-principal replay is never permitted — a buyer's idempotency_key on seller A has no meaning on seller B, and keys from principal X are not visible to principal Y on the same seller.

#### Payload equivalence

"Equivalent" payload means **identical canonical JSON form**, not field-by-field semantic comparison. Sellers MUST determine equivalence by hashing the request payload in canonical form and comparing hashes. The canonical form is [RFC 8785 JSON Canonicalization Scheme (JCS)](https://www.rfc-editor.org/rfc/rfc8785):

- Object keys sorted lexicographically
- No insignificant whitespace
- Numbers serialized per ECMAScript 2019 (`JSON.stringify` / Python's `float.__repr__`)
- Strings in NFC Unicode, with minimum required escaping
- The full request object EXCEPT `idempotency_key` and `context` are included in the hash. `context` is opaque echo data and MUST NOT affect equivalence — buyers change trace IDs on retry.

Sellers MUST NOT treat "missing optional field" and "field explicitly set to null" as equivalent — JCS preserves the distinction, and so do buyers.

Recommended hash: `SHA-256(JCS(payload_without_idempotency_key_and_context))`. Sellers using another stable canonical form MUST document it in their agent capabilities. Pinning to JCS removes the biggest source of false `IDEMPOTENCY_CONFLICT` errors: buyers and sellers disagreeing on what "same payload" means across JSON serializer implementations.

#### Response-level replay indicator

Every mutating response envelope carries a top-level `replayed` boolean:
...
On a fresh execution, `replayed` is `false` (or omitted — buyers MUST treat omission as `false`). On a cached replay, `replayed` is `true` and the rest of the response is byte-for-byte the original. Buyers use this for:

- **Billing reconciliation** — "we processed N buys this month" counts `replayed: false` only.
- **Logging** — distinguishing "retry succeeded by returning cache" from "retry triggered a new execution" (the latter is usually a bug in the replay window or key management).
- **Side-effect invariants** — any downstream system that expects exactly-once should read `replayed` before treating the response as a new event.

#### IDEMPOTENCY_CONFLICT response shape

The conflict response uses the standard AdCP error envelope. The error body:

- MUST include `code: "IDEMPOTENCY_CONFLICT"` and a human-readable `message`
- MAY include `field: "<json-pointer>"` identifying the first path where the canonical forms diverged (best-effort diagnostic)
- MUST NOT include the cached response, the original payload, or any fingerprint derived from them. Leaking cached state turns key-reuse into a read oracle — an attacker who can guess a victim's key could probe it for a payload match.
  ...
  "errors": [
    {
      ...
      "message": "idempotency_key was used with a different payload within the 24h replay window",
...
function payloadHash(request) {
  // Exclude idempotency_key and context from the equivalence hash
  const { idempotency_key, context, ...rest } = request;
...
async function createMediaBuy(request) {
  ...
  // Look up by (principal, key) — key scope is per-principal
  ...
  if (existing) {
    if (existing.request_hash !== requestHash) {
      throw new IdempotencyConflictError({
        ...
        // field is optional best-effort diagnostic; never include the cached payload
      });
    }
    // Return cached response with replayed: true
  ...
  return db.transaction(async (tx) => {
    ...
    await tx.idempotencyKeys.insert({
      ...
      response,
      expires_at: new Date(Date.now() + 86400 * 1000), // 24h TTL
    });
