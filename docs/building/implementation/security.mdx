...
2. Lowercase the host; convert IDN labels to Punycode A-labels per RFC 5890.
3. Strip userinfo (`user:pass@host` → `host`).
4. Strip default ports (`:443` for https, `:80` for http).
5. Apply `remove_dot_segments` (RFC 3986 §5.2.4) to the path; collapse consecutive slashes; ensure an empty path becomes `/` when an authority is present.
6. Normalize percent-encoding: uppercase hex digits (`%2f` → `%2F`); decode unreserved characters; leave reserved characters percent-encoded.
7. **Preserve the query string byte-for-byte** as received. Do not reorder parameters, do not re-encode, do not interpret `+` as space.
8. Strip the fragment (fragments are not sent on the wire).
