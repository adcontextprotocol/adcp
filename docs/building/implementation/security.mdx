...
**No symmetric response-signing profile.** This profile signs the *request*; there is no paired profile for signing the synchronous response body. Sellers MUST NOT sign synchronous AdCP response bodies under any `adcp_use` value, and buyers MUST NOT rely on a signature on the synchronous reply. Integrity of the immediate response rests on TLS within the authenticated session that carried the request; durable at-rest attestation for artifacts that need to survive past the session is the job of [signed webhooks](#webhook-callbacks) (`adcp_use: "webhook-signing"`). The split is deliberate — see [Security Model: What gets signed](/docs/building/understanding/security-model#what-gets-signed--and-what-doesnt) for the full rationale and the request-the-webhook pattern for tools whose canonical artifact needs to be attestable.
