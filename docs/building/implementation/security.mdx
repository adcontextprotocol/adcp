...
- **Side-effect invariants** — downstream systems that expect exactly-once should read `replayed` before treating the response as a new event.
  ...
  "errors": [
    {
      ...
      "message": "idempotency_key was used with a different payload within the replay window. Generate a fresh UUID v4 for the new request.",
