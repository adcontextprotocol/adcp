...
4. Configure your AdCP client with the private key and agent URL. Your SDK signs requests automatically for any operation listed in a seller's `supported_for` or `required_for` capability.
5. Validate end-to-end with the conformance vectors at `static/test-vectors/request-signing/` — if your client produces signatures that match the positive vectors' `expected_signature_base`, you're done.
...
**Minimum viable verifier (3.0 shadow mode):** steps 1–10 of the checklist, in-memory replay cache, one-minute revocation polling. Steps 11–13 (digest, replay insert, full revocation grace semantics) are recommended for production but can be added incrementally during the pilot. Do not skip ahead of step 1 — malformed signatures always reject, never fall back.
...
| `nonce` | ≥ 128 bits of entropy, base64url-encoded. Used for replay dedup. |
...
| `tag` | MUST be exactly `adcp/request-signing/v1` (byte-for-byte). The tag namespace is how the profile versions; future versions bump the tag rather than mutating parameter semantics. |

All six parameters are REQUIRED. Verifiers MUST reject (`request_signature_params_incomplete`) if any is absent. Algorithm allowlist aligns with the JWS profile (`EdDSA` / `ES256`) using RFC 9421 parameter naming. Edge-runtime rationale from the governance profile applies.
...
9. Check the [Transport revocation](#transport-revocation) list. Reject if `keyid` ∈ `revoked_kids` (`request_signature_key_revoked`). Reject with `request_signature_revocation_stale` if the verifier has not refreshed the revocation list within grace. **Revocation runs before cryptographic verify** so that replaying a revoked-key signature is an O(1) rejection rather than an O(signature verify) rejection — a cheap-amplification-resistance property. (Step 7's `keyid` resolution already reveals the key's existence, so moving revocation here does not leak more information than the profile already does.)
...
- `supported_for`: operations for which signatures are verified when present but not required. Signers SHOULD sign these. Typically a superset of `required_for`.

In 3.0, verifiers ship with `required_for: []` and populate it selectively (per-counterparty pilot, per-operation rollout). In 4.0 the protocol normatively requires `required_for` to include all spend-committing operations the verifier supports, and `covers_content_digest: "required"` is recommended for those operations.
...
TMP signing keys SHOULD be published at the TMP provider's `agents[]` entry `jwks_uri` with distinct `kid`, `key_ops: ["verify"]`, and a TMP-specific `adcp_use` value (or no `adcp_use` if the TMP profile does not yet define one — this profile rejects such keys for request signing, which is the desired behavior). One publication pattern, three signing systems (governance JWS, request-signing 9421, TMP envelope), each with its own `kid`-scoped key entry and distinct `adcp_use` value. This keeps operator key management uniform without enabling cross-purpose key reuse.
...
Illustrative only. The `verify9421` and `parseSignatureInput` callbacks encapsulate protocol-specific canonicalization and signature verification; implementations should pin a specific RFC 9421 library that has been validated against the AdCP conformance test vectors at `static/test-vectors/request-signing/`.
