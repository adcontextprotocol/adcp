...
**`@target-uri` canonicalization** applies RFC 3986 §6.2.2 (syntax-based normalization) and §6.2.3 (scheme-based normalization), in this order. The same algorithm is the canonical AdCP URL canonicalization — see [URL Canonicalization](/docs/reference/url-canonicalization) for non-signing callers (TMP seller authorization, `adagents.json` lookups, `format-id` comparison).
