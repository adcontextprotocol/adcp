...
3. Fetch that entry's `jwks_uri` (or default to `/.well-known/jwks.json` at the origin of `A`) with SSRF validation. JWKS cache TTL bounded above by the revocation-list polling interval.
4. Resolve `keyid` on the incoming `Signature-Input` to a JWK in the fetched set. On `kid` miss, refetch once (subject to the 30-second cooldown between refetches) before rejecting with `webhook_signature_key_unknown`.
...
**Verifier checklist for webhooks.** Apply these 14 checks in order, short-circuiting on the first failure. They parallel the [request verifier checklist](#verifier-checklist-requests) — the mechanical loop is identical; the differences below are where the webhook profile diverges.
