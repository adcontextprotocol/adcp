...
- **Duplicate object keys**: Signers MUST NOT emit duplicate object keys. Per RFC 8259 §4, the names within a JSON object "SHOULD be unique" and the behavior of software that receives an object with non-unique names is unpredictable — so two verifiers parsing the same HMAC-valid bytes can disagree on the parsed value. This is a parser-differential attack class (cf. CVE-2017-12635 where one CouchDB parser read `roles=[]` and another read `roles=["_admin"]` from the same signed body). Verifiers MAY reject bodies containing duplicate keys, and SHOULD do so for bodies carrying state-change or spend-committing payloads (webhook callbacks, governance context, media-buy status transitions). Two verifier outcomes are conformant for the fixture `duplicate-keys-conflicting-values` in `static/test-vectors/webhook-hmac-sha256.json`: (a) `accept` — the HMAC is mathematically valid and the verifier has no duty to parse; deterministic last-wins or first-wins downstream JSON parsing counts as `accept` — or (b) `reject-malformed` — the verifier returns a structured malformed-body error (distinct from a signature-mismatch error, because the signature IS valid). Crash / fail-closed is conformant-but-suboptimal; verifiers SHOULD return a structured malformed-body error instead. The non-conformant failure mode is silent accept where the signature verifier's parse diverges from the downstream business-logic parse — `reject-malformed` at the verifier is the only posture that closes this gap, which is why it is RECOMMENDED for any body that drives state change. A future AdCP release will tighten this from MAY to MUST for state-change bodies (tracked in [#2523](https://github.com/adcontextprotocol/adcp/issues/2523)); implementers SHOULD adopt `reject-malformed` now.
