...
**Key publication.** Sellers publish webhook-signing JWKs at the `jwks_uri` on their `adagents.json` `agents[]` entry — the same publication pattern as request-signing keys, with a distinct discriminator:
...
**Covered components** are identical to request signing: `@method`, `@target-uri`, `@authority`, `content-type`, and `content-digest`. `content-digest` is REQUIRED on webhook callbacks — the body carries the event, and a signature that doesn't commit to the body is not protecting the attack surface that matters. There is no `covers_content_digest: "forbidden"` opt-out for webhooks; transports that cannot preserve webhook body bytes MUST be fixed.
...
**JWKS discovery.** The buyer knows the seller's agent URL from the original AdCP call (it's the agent the buyer is integrated with). Buyer resolves:

1. Seller agent URL → seller's `adagents.json` `agents[]` entry (already cached from integration onboarding).
2. Fetch the entry's `jwks_uri` (or default to `/.well-known/jwks.json` at the agent URL's origin) with SSRF validation per [Webhook URL validation](#webhook-url-validation-ssrf).
3. Resolve `keyid` on the incoming `Signature-Input` to a JWK in the fetched set.

Buyers MUST NOT derive signer identity from webhook payload fields (`task_id`, `operation_id`, etc.). Identity is established solely via the signature → JWKS → seller agent chain.

**Verifier checklist for webhooks.** Apply the same 14 checks as the [request verifier checklist](#verifier-checklist-requests), with these webhook-specific substitutions:

- Error codes use the `webhook_signature_*` prefix instead of `request_signature_*` (see [Webhook error taxonomy](#webhook-error-taxonomy) below).
- Step 3: `tag` MUST be `adcp/webhook-signing/v1`.
- Step 6: `content-digest` is REQUIRED (not policy-advertised); omit the `covers_content_digest` branch.
- Step 7: resolve `keyid` via the seller's `adagents.json` entry (buyer's side) rather than the buyer's brand.json.
- Step 8: `adcp_use` MUST equal `"webhook-signing"`.
- There is no subsequent brand-operator authorization step — the seller's signed identity is sufficient to accept the webhook. The application-layer dedup on `idempotency_key` runs after signature verification.

**Revocation and replay dedup** reuse the same infrastructure as request signing: the seller's operator SHOULD serve a single combined revocation list covering request-signing and webhook-signing keys, and replay dedup is keyed on `(keyid, nonce)` with the same TTL semantics.
