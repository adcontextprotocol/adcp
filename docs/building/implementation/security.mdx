...
5. Validate end-to-end with the conformance vectors at [`/test-vectors/request-signing/`](https://adcontextprotocol.org/test-vectors/request-signing/) (also in the repo at `static/test-vectors/request-signing/`) — if your client produces signatures that match the positive vectors' `expected_signature_base`, you're done.
...
Illustrative only. The `verify9421` and `parseSignatureInput` callbacks encapsulate protocol-specific canonicalization and signature verification; implementations should pin a specific RFC 9421 library that has been validated against the AdCP conformance test vectors at [`/test-vectors/request-signing/`](https://adcontextprotocol.org/test-vectors/request-signing/).
