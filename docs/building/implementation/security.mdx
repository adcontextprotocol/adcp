...
|------|------|---------------|-------------|
| `request_signature_brand_json_url_missing` | Capabilities did not carry `identity.brand_json_url` and a signed request was received, or carried a non-HTTPS value | `agent_url` | Operator: add HTTPS `identity.brand_json_url` to capabilities. Verifier: surface to operations; do not retry. |
...
| `request_signature_brand_json_unreachable` | brand.json fetch failed (same conditions) | `brand_json_url`, `http_status`, `dns_error`, `last_attempt_at` | Same as above. |
...
1. **Fetch capabilities** for the signing agent's URL `A`. Use SSRF-safe HTTP per [Webhook URL validation](#webhook-url-validation-ssrf): HTTPS only, address-family + private-IP filtering, no redirects.
...
3. **eTLD+1 origin binding.** Compute `eTLD+1(A)` and `eTLD+1(brand_json_url)` using a pinned PSL snapshot. If they match, proceed. Otherwise fetch `brand.json` and check `authorized_operators[]` — if `eTLD+1(A)` is delegated, proceed. Else reject `request_signature_brand_origin_mismatch`.
4. **Fetch `brand.json`** with the same SSRF rules + no redirects, body cap 256 KiB, connect 5 s, total 10 s. Cache successful responses up to (but no longer than) the JWKS revocation polling interval; cache failures for at most 60 s.
...
7. **Consistency check.** For every purpose declared under capabilities `identity.key_origins`, verify the resolved `jwks_uri` host equals the declared origin (skip only the specific (agent, purpose, role) tuple sourced from a publisher pin). Reject `request_signature_key_origin_mismatch` / `_missing` as appropriate.
8. **Hand off to step 7+ of the [verifier checklist](#verifier-checklist-requests)** — fetch the JWKS, find the `kid`, verify per RFC 9421.

Pseudocode (TypeScript-flavored; SDK helpers below collapse this to a single call):

```ts
async function resolveAgent(agentUrl: string): Promise<{ jwksUri: string; jwks: JWKSet }> {
  const caps = await safeFetch(`${agentUrl}`, { ssrf: true });               // step 1
  ...
  const agentEtld1 = etldPlusOne(new URL(agentUrl).hostname);                 // step 3
  const brandEtld1 = etldPlusOne(new URL(brandJsonUrl).hostname);
  const brandJson = await safeFetch(brandJsonUrl, { ssrf: true, maxRedirects: 0, body: 262144 }); // step 4
  ...
  for (const [purpose, declared] of Object.entries(caps.identity?.key_origins ?? {})) { // step 7
    if (origin(jwksUri) !== declared) throw new Err('key_origin_mismatch', { purpose });
  }
  const jwks = await safeFetch(jwksUri, { ssrf: true, body: 65536 });          // step 8 setup
  return { jwksUri, jwks };
}
```
...
The 8-step algorithm ships in three SDKs — pick the one matching your runtime:

- **TypeScript** ([`@adcp/client`](https://github.com/adcontextprotocol/adcp-client)): `resolveAgent(url)` returns `{ agents[] entry, jwks_uri, jwks, trace, freshness }`. `getAgentJwks(url)` is the JWKS-only fast path. `createAgentJwksSet(url, opts)` returns a `JWTVerifyGetKey` for handing to `jose`'s `jwtVerify`.
- **Python** ([`adcp`](https://github.com/adcontextprotocol/adcp-client-python)): `resolve_agent(url)` returns an `AgentResolution` dataclass with the same shape. `verify_request_signature(request, *, agent_url, allowed_algs)` is the one-shot helper that runs the discovery chain and the [verifier checklist](#verifier-checklist-requests) in one call.
- **Go** ([`adcp-go`](https://github.com/adcontextprotocol/adcp-go)): `ResolveAgent(ctx, agentURL) (*AgentResolution, error)` and `VerifyRequestSignature(ctx, req, opts) (*VerifiedIdentity, error)` mirror the TS/Python signatures.

Each SDK ships a CLI for dev-loop debugging — `npx @adcp/client resolve <url>`, `python -m adcp resolve <url>`, `adcp-go resolve <url>` — printing the trace with per-step `fetched_at`/`age_seconds`/`ok` so an operator triaging a `request_signature_brand_*` failure can see exactly which step rejected and why.
