...
1. **First call is canonical.** The seller processes the request once and stores the canonical response keyed by `(authenticated_principal, idempotency_key)`.
2. **Replay returns the cached response.** A subsequent request with the same `idempotency_key` AND a semantically equivalent payload MUST return the stored response verbatim without re-executing side effects (no duplicate buys, charges, or webhooks).
3. **Key reuse with a different payload is a conflict.** A request with the same `idempotency_key` but a materially different payload MUST be rejected with `IDEMPOTENCY_CONFLICT`. Sellers MUST NOT silently apply the second request.
...
async function createMediaBuy(request) {
  ...
  if (existing) {
    if (!payloadsEquivalent(existing.request, request)) {
