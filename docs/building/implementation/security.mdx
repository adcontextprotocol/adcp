...
The `plan_hash` claim is a byte-equality invariant: two honest parties computing the hash over the same plan revision MUST arrive at identical bytes. Ambiguity about *which bytes* is the failure mode — without a normative scope definition, two implementations disagree on the hash and every verification fails. The rules below mirror the webhook HMAC byte-equality treatment in [#2478 / #2493](#webhook-hmac-sha256-legacy-scheme).

**Input document.** `plan_hash` is computed over the JSON value of the `plans[i]` array element in the `sync_plans` request that the governance agent accepted — the same bytes the governance agent evaluated against registry and bespoke policies at approval time. Not the envelope (`adcp_major_version`, `idempotency_key` are stripped), not the seller's post-resolution view, and not the governance agent's internal decision log.

**Fields excluded from the hash** (strip before canonicalization; their mutation does not invalidate an outstanding token):

- `updated_at`, `created_at`, or any governance-agent-assigned timestamp not present in the buyer's submitted body.
- Server-assigned correlation fields the governance agent attaches during approval (e.g., `check_id`, `decision_id`) — these are emitted alongside the plan, not into it.
- Seller-side annotations on the plan (a seller-local copy augmented with proposal IDs, pricing notes, or ingest metadata) — the hash is scoped to the buyer-signed plan body, not sellers' decorated views.
- Packages marked `soft_deleted: true` — a buyer soft-deleting a cancelled package MUST NOT invalidate outstanding tokens for other packages. The soft-deleted entry is stripped before canonicalization.

**Fields included in the hash.** Every other field on the `plans[i]` object as submitted by the buyer: `plan_id`, `brand`, `objectives`, `budget` (including `total`, `currency`, `allocations`, `reallocation_threshold`/`reallocation_unlimited`), `channels`, `flight`, `markets`, `policy_ids`, `custom_policies`, `human_review_required`, `packages[]`, and any profile extensions present at approval time. Any field the governance agent evaluates against policy MUST be hashed; any field stripped from the hash MUST NOT influence a policy outcome.

**Canonicalization.** RFC 8785 JSON Canonicalization Scheme (JCS): sorted object keys (lexicographic by code-point), compact separators (no whitespace), RFC 8259 number formatting, unicode characters in their canonical escape form. Implementations MUST NOT substitute a hand-rolled "compact JSON" serializer for JCS — JCS's number rules and unicode handling close the drift modes (`1.0` vs `1`, `\u00e9` vs literal `é`) that language-level serializers leave divergent.

**Byte-equality invariant.** `plan_hash` is computed over the JCS output bytes, not over a parsed JSON value. Governance agents and sellers MUST use the same JCS implementation contract (RFC 8785 conformance), and MUST NOT re-parse and re-serialize with a non-JCS library before hashing. A canonical test fixture (plan → JCS bytes → hex SHA-256) lives at `/static/test-vectors/governance-plan-hash.json`; conformant implementations MUST reproduce the recorded hash byte-for-byte.

**Pin-and-verify (TOCTOU).** A concurrent `sync_plans` between a seller's hash check and its execution writes a window where the seller would execute against a body that was never governance-approved. Sellers MUST pin the plan revision at the moment of hash verification (checklist step 13) and execute against the pinned revision — either by reading `plans[plan_id]` once into a transaction snapshot that covers both the hash check and the execution, or by re-verifying `plan_hash` against the live plan body immediately before execution and rejecting with `PLAN_HASH_MISMATCH` if the pin has moved. A seller that re-reads the plan after hash verification without re-checking MUST be treated as non-conformant. Governance agents SHOULD invalidate outstanding tokens for a plan whenever `sync_plans` rewrites the plan body (implementations may key revocation entries on plan_hash mismatches surfaced by sellers).

**Modification-phase hash.** For `modification`-phase tokens, `plan_hash` covers the *post-update plan state* — the bytes the governance agent would approve if the proposed update were applied. The buyer computes this by applying the update patch to its local copy of the plan, canonicalizing with JCS, and passing the resulting plan document on `check_governance` with `phase: "modification"`. The governance agent evaluates the post-update document, hashes the same bytes, and binds that hash into the issued token. Sellers recompute the hash over the plan document *as it will exist after the update is applied* and match against the token's claim. This is symmetric with intent-phase hashing — the only difference is which plan revision is canonicalized.

**Error-code decision tree** (sellers dispatching `check_governance` / `update_media_buy` rejections):

- `PERMISSION_DENIED` — token absent, signature invalid, `aud` mismatch, `exp` past, `jti` replay, `iss` not in brand.json, or any verification-checklist failure *other than* the plan-hash check.
- `PLAN_HASH_MISMATCH` — token is present and passes checklist steps 1-12 and 14-16, but step 13 (plan_hash recompute) does not byte-equal the token's claim. Buyer-mutated plan after approval, or buyer and governance agent disagree on canonicalization.
- `UPDATE_REQUIRES_GOVERNANCE` — `update_media_buy` on a price-affecting field with no `modification`-phase token attached (or with a `purchase`/`intent`-phase token). The check runs *before* the verification checklist because the token required for checklist entry is not present.
...
13. Pin the plan revision, compute SHA-256 over its JCS-canonicalized bytes, and reject with `PLAN_HASH_MISMATCH` if it does not byte-for-byte equal the token's `plan_hash` claim. Full scope rules for "which bytes are hashed" and the pin-and-verify TOCTOU requirement are in [Plan-hash computation](#plan-hash-computation); sellers MUST execute against the same pinned revision this step verified (or re-verify immediately before execution). For `update_media_buy`, the plan bytes used for this hash are the post-update plan state — see [`update_media_buy`](/docs/media-buy/task-reference/update_media_buy#governance-re-check-on-price-affecting-updates). This closes the plan-mutation race: a buyer who approves plan P1, mutates via `sync_plans` to P2, and then presents the P1-bound token against P2 fails this check.
