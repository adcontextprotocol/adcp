...
- **Sellers calling agent-side webhooks** sign their callbacks symmetrically; receiving agents verify against the seller's keys published under the seller's `adagents.json` agent entries.

**Dependencies:**
- Shares JWKS discovery, SSRF rules, alg allowlist, revocation semantics, and key rotation with the [AdCP JWS profile](#adcp-jws-profile) above. Cross-purpose key reuse is forbidden — a key used for governance signing (`typ: adcp-gov+jws`) MUST NOT also be presented as a request-signing key on the same JWKS. Per-purpose `kid` separation is the mechanism; verifiers enforce it.
...
|---|---|
| `@method` | Uppercase. |
| `@target-uri` | Canonicalized: scheme lowercased, host lowercased, default ports stripped, path RFC 3986 normalized (no `..`/`.`, no double slashes), query parameters in original order. |
| `@authority` | Lowercased host[:port], default ports stripped. |
| `content-type` | Required on requests with bodies. |
| `content-digest` | Required on requests with bodies if the verifier's capability advertisement sets `covers_content_digest: true`; prohibited otherwise. See [Content-digest and proxy compatibility](#content-digest-and-proxy-compatibility). |
...
**Signature parameters (`Signature-Input` parameters):**

| Parameter | Required | Notes |
|---|---|---|
| `created` | Yes | Unix seconds. Reject if more than 60 s in the future. |
| `expires` | Yes | Unix seconds. MUST satisfy `expires − created ≤ 300` (5-minute max validity). Reject if past, with ±60 s skew tolerance. |
| `nonce` | Yes | ≥ 128 bits of entropy, base64url-encoded. Used for replay dedup. |
| `keyid` | Yes | Matches a `kid` in the signer's published JWKS. |
| `alg` | Yes | MUST be `ed25519` or `ecdsa-p256-sha256`. Verifiers MUST enforce the allowlist independently of library defaults. |
| `tag` | Yes | MUST be exactly `adcp/request-signing/v1` (byte-for-byte). The tag namespace is how the protocol versions; future versions bump the tag rather than mutating parameter semantics. |

Algorithm allowlist aligns with the JWS profile (`EdDSA` / `ES256`) using RFC 9421 parameter naming. Edge-runtime rationale from the governance profile applies.
...
- `use`: `"sig"`
- `key_ops`: `["verify"]` in the verifier-visible JWKS; publishers hold the corresponding private key locally with `["sign"]` per JWK spec.
- `kid`: distinct from any key on the same JWKS that serves a different signing purpose.

Verifiers MUST reject signatures whose `keyid` resolves to a key without `key_ops` containing `verify`, and MUST reject a request-signing `keyid` that resolves to a key also registered as a governance-signing key (`typ: adcp-gov+jws`) on the same JWKS. Cross-purpose key reuse is forbidden.
...
2. Fetch the agent's `jwks_uri` (or default to `/.well-known/jwks.json` at the origin of the agent's `url`) with SSRF validation per [Webhook URL validation](#webhook-url-validation-ssrf). Cache TTL bounded above by the revocation-list polling interval.
3. Refetch once on `kid` miss with a 30-second cooldown before rejecting, matching the governance profile's key rotation behavior.
...
Verifiers MUST NOT derive signer identity from request body fields (e.g., a `principal_id` in the payload). The signature → JWKS → agent entry chain is the only authoritative identity path.

brand.json discovery follows one redirect (`authoritative_location`) and stops.

#### Verifier checklist (requests)

Before treating a request as authenticated for any operation listed in the verifier's `required_for` capability, verifiers MUST perform these checks in order, short-circuiting on the first failure. This checklist establishes agent identity only — brand-operator authorization is a separate, subsequent check governed by the target house's brand.json.

1. Parse `Signature-Input` and `Signature` headers per RFC 9421 §4. Reject if malformed or missing.
2. Reject if `tag` is not exactly `adcp/request-signing/v1`.
3. Reject if `alg` is not in the allowlist (`ed25519`, `ecdsa-p256-sha256`). Library defaults MUST NOT be relied upon.
4. Reject if `created` > now + 60 s, `expires` < now − 60 s, or `expires − created > 300`.
5. Reject if covered components do not include all of: `@method`, `@target-uri`, `@authority`. If a body is present, reject if `content-type` is not covered, and if the verifier's capability sets `covers_content_digest: true`, reject if `content-digest` is not covered.
6. Resolve `keyid` to a JWK via [Agent key publication](#agent-key-publication). On cache miss, refetch the JWKS once with a 30 s cooldown before rejecting. Reject if `keyid` cannot be resolved to a specific `agents[]` entry.
7. Verify the JWK's `use` is `"sig"` and `key_ops` includes `"verify"`. Reject keys marked for any other purpose (including governance-signing keys).
8. Compute the canonical signature base per RFC 9421 §2.5 using the covered components. Verify the signature against the JWK.
9. If `content-digest` is covered, recompute the digest from the received body bytes and compare. Reject on mismatch.
10. Check the nonce against the replay cache (see [Transport replay dedup](#transport-replay-dedup)). Reject if `(keyid, nonce)` has been seen within the (created, expires) window.
11. Check the [Transport revocation](#transport-revocation) list. Reject if `keyid` ∈ `revoked_kids`.
12. Insert `(keyid, nonce)` into the replay cache with TTL = `expires − now`.

Only after all 12 checks pass does the verifier treat the request as cryptographically authenticated. Verifiers SHOULD record `verified_signer: { keyid, agent_url, verified_at }` on the request context so downstream code — including the subsequent brand-operator authorization check — can log and audit by signed agent identity.

#### Content-digest and proxy compatibility

Covering `content-digest` binds the request body to the signature. This protects against body tampering by intermediaries. It also breaks easily.

**What invalidates a `content-digest`-covered signature in transit:**
- CDNs that recompress request bodies (some configurations of Cloudflare, Fastly, CloudFront on POST).
- WAFs that "sanitize" JSON (whitespace normalization, unknown field stripping, key reordering).
- Reverse proxies that re-serialize JSON for logging or transformation.
- HTTP/2 → HTTP/1.1 bridges that change framing in ways some implementations canonicalize differently.

**3.0 default:** `covers_content_digest: false` in capability advertisement. Verifiers who require body-binding signatures opt in by setting it `true` and accepting the deployment cost (CDN/proxy configuration, end-to-end interop testing).

**Recommended for 4.0:** `covers_content_digest: true` for spend-committing operations — body integrity matters most where the body specifies money.

**Signer guidance:** if signing for a verifier that advertises `covers_content_digest: true`, validate with a `POST` echo against a test endpoint before sending real traffic. Mismatches between sent and received body bytes are the single biggest source of 9421 interop bugs in production.

#### Transport replay dedup

Step 10 of the [verifier checklist](#verifier-checklist-requests) requires per-`(keyid, nonce)` deduplication. Unbounded sets are a memory and DoS risk.

- TTL on each entry = `expires − now` (typically ≤ 300 s).
- In-memory LRU keyed on `(keyid, nonce)` with TTL eviction, sized to expected request rate × max signature validity.
- Above ~10K req/s per signer: Redis `SETNX` with `EX = remaining_validity_seconds`.
- Distributed verifiers (multi-region): per-region replay cache is acceptable. The only attack this enables is a single replay within (expires − now) across regions, bounded by 5 min and only effective if the attacker controls intermediate routing.
...
- `covers_content_digest`: when true, signers MUST cover `content-digest`; when false, they MUST NOT cover it.
- `required_for`: operations for which unsigned requests are rejected. Empty in 3.0 by default. Signers MUST sign any listed operation.
- `supported_for`: operations for which signatures are verified when present but not required. Signers SHOULD sign these.

In 3.0, verifiers ship with `required_for: []` and populate it selectively (per-counterparty pilot, per-operation rollout). In 4.0 the protocol normatively requires `required_for` to include all spend-committing operations the verifier supports.
...
| Failure | Retry? | Code |
|---|---|---|
| `Signature` or `Signature-Input` header malformed or missing | No | `request_signature_header_malformed` |
...
| Signature window invalid (skew, expired, > 5 min validity) | No | `request_signature_window_invalid` |
...
| JWK `key_ops` lacks `verify`, or `kid` is scoped to another signing purpose | No | `request_signature_key_purpose_invalid` |
| Cryptographic verification failed | No | `request_signature_invalid` |
| `content-digest` mismatch | No | `request_signature_digest_mismatch` |
...
Trusted Match Protocol signs match-time requests with its own Ed25519 envelope. TMP's per-request budget (sample-verify at ~5%) is too tight for full RFC 9421 verification on every call. **TMP signing is out of scope for this section.**

TMP signing keys SHOULD be published at the TMP provider's `agents[]` entry `jwks_uri` with distinct `kid` and `key_ops: ["verify"]`. One publication pattern, three signing systems (governance JWS, request-signing 9421, TMP envelope), each with its own `kid`-scoped key entry. This keeps operator key management uniform without forcing cross-purpose key reuse.

#### Transport migration timeline

| Phase | Status | Behavior |
|---|---|---|
| 3.0 GA | Optional, capability-advertised | Verifiers MAY validate; `required_for: []` by default. Signers MAY sign. |
| 3.x | Reference SDKs ship | Conformance test vectors at `static/test-vectors/request-signing/`. Early adopters opt in via per-counterparty `required_for` pilots. |
| 4.0 | Required for spend-committing operations | `required_for` MUST include `create_media_buy`, `acquire_*`, and any spend-committing operation the verifier supports. Signers MUST sign. |
...
#### Request verifier reference (TypeScript, ~40 lines)
...
export async function verifyAdcpRequestSignature(req: Request, ctx: {
  resolveJwks: (keyid: string) => Promise<{ jwks: ReturnType<typeof createRemoteJWKSet>, agentUrl: string }>;
  isReplayed: (keyid: string, nonce: string, ttlSeconds: number) => Promise<boolean>;
  isKeyRevoked: (keyid: string) => Promise<boolean>;
  coversContentDigest: boolean;
  verify9421: (req: Request, jwk: unknown, covered: string[]) => Promise<void>;
}) {
  const sigInput = req.headers.get("signature-input");
  if (!sigInput) throw new RequestSignatureError("request_signature_header_malformed");

  const parsed = parseSignatureInput(sigInput); // { keyid, alg, created, expires, nonce, tag, components }
  ...
  if (parsed.created > now + 60 || parsed.expires < now - 60 || parsed.expires - parsed.created > 300) {
  ...
  if (req.body && ctx.coversContentDigest && !parsed.components.includes("content-digest")) {
    throw new RequestSignatureError("request_signature_components_incomplete");
  }

  const { jwks } = await ctx.resolveJwks(parsed.keyid); // throws _key_unknown on miss after refetch

  if (await ctx.isKeyRevoked(parsed.keyid)) throw new RequestSignatureError("request_signature_key_revoked");

  await ctx.verify9421(req, await jwks(parsed), parsed.components); // throws on signature or digest failure

  if (await ctx.isReplayed(parsed.keyid, parsed.nonce, parsed.expires - now)) {
    throw new RequestSignatureError("request_signature_replayed");
  }
}
```

The 9421 library call and `parseSignatureInput` implementation are illustrative; implementations should pin a specific RFC 9421 library version that has been validated against the AdCP conformance test vectors at `static/test-vectors/request-signing/`.
