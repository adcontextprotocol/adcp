...
- If the operation is in the verifier's `required_for` capability AND no `Signature-Input` header is present AND the caller presents no other valid credential accepted by this verifier for this operation (bearer, API key, mTLS), reject with `request_signature_required`. Unsigned requests that fall into this branch never enter the checklist. See [Composition with fallback authenticators](#composition-with-fallback-authenticators) for the rule governing unsigned-but-otherwise-authenticated callers.
...
`required_for` governs the signature requirement **relative to a caller's credential path**, not absolutely. A verifier typically accepts more than one authenticator (bearer, API key, mTLS, 9421) and `required_for` is one lever within that auth chain, not an override that trumps the others. The normative rule is:

- An **unauthenticated** request to a `required_for` operation MUST be rejected with `request_signature_required`. "Unauthenticated" means the caller presents neither a valid signature nor any other credential the verifier accepts for this operation.
...
- An **unrecognized alternative credential** (bearer or API key the verifier does not accept) is not a "valid alternative credential" for this rule — the caller is unauthenticated and falls into the first branch.

**Sellers that intend signing to be unconditional for an operation MUST configure their fallback authenticators to reject all other credential types for that operation.** Advertising `required_for: [create_media_buy]` while continuing to accept bearer tokens for `create_media_buy` does not prevent unsigned spend — the verifier advertised bearer as valid, and callers are entitled to use it. Sellers that want the strict reading (unsigned-always-rejected) MUST either stop accepting bearer for the operation, or gate the bearer authenticator on a per-caller flag that forces signing for counterparties who have completed 9421 onboarding.

Buyers reading `required_for` on a counterparty's capability surface learn **"unsigned unauthenticated callers will be rejected on this operation"** — not "all unsigned callers will be rejected." A buyer that wants its own unsigned bearer calls to fail closed on a `required_for` operation MUST negotiate with the seller to revoke bearer credentials for that operation rather than infer the behavior from the capability block.
