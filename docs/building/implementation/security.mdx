...
| `plan_hash` | Yes | `base64url_no_pad(SHA-256(JCS(plan_payload)))` — binds the attestation to the exact plan state the governance agent evaluated. Verified by the governance agent (on each `check_governance` re-evaluation) and by auditors; opaque to sellers, who cannot access the plan payload and do not verify this claim. See [Plan-state binding](#plan-state-binding). |
...
**Who verifies `plan_hash`**: the governance agent itself and auditors. Not sellers.

Sellers cannot verify `plan_hash` because the plan payload is a commercially sensitive buyer document — it carries cross-seller allocations, per-seller caps, objectives, `approved_sellers` lists, custom policies, and `ext` data that buyers do not share with any single seller. No plan-retrieval mechanism is specified in 3.x and none is planned; trying to share partial-plan views with sellers creates a whole new privacy surface without solving the binding problem. Sellers persist and forward the token verbatim per the existing forwarding rule and treat `plan_hash` as an opaque correlation value embedded in the token.

The verifying parties are:

- **The governance agent**: on every `check_governance` call (required on every spend-commit per [#2403](https://github.com/adcontextprotocol/adcp/issues/2403)), the GA re-evaluates the current plan state and re-hashes. Tampering with the GA's persisted plan between calls is detected by hash mismatch against the retained revision record. This is the primary 3.x enforcement path.
- **Auditors**: given access to the plan state via [`get_plan_audit_logs`](/docs/governance/campaign/tasks/get_plan_audit_logs) and the GA's retained per-revision `plan_hash` records, an auditor can recompute and verify any historical token against the plan state it attested to. This is the post-hoc accountability path — the property the signed format exists to deliver for regulators.
- **Buyer-side compliance systems**: a buyer's own compliance tooling can verify that its GA is producing tokens that match the plan the buyer actually pushed, catching a compromised or misbehaving governance vendor.

**`plan_hash` is never listed in `crit`**: `crit` means wire verifiers MUST reject tokens whose listed claims they can't process (RFC 7515 §4.1.11). The only parties who can process `plan_hash` fetch plan state from their own side of the boundary (GA re-reads its own store; auditors fetch via audit logs; buyer compliance reads its own plan); they are not gated by `crit`. Listing `plan_hash` in `crit` would force sellers to reject tokens they have no basis to verify, with no corresponding benefit. Governance agents MUST emit the claim and MUST NOT list it in `crit`.

**Enforcement bounds**:

- **Spend-commit paths**: bounded by the seller's next `check_governance` per [#2403](https://github.com/adcontextprotocol/adcp/issues/2403) — the GA re-evaluates current plan state and catches mutations at purchase time. Sub-second in the happy path; bounded by intent-token `exp` (≤15 minutes) in the worst case.
- **Modification and delivery phases**: bounded by the next seller-initiated `check_governance` call. A plan mutation between execution-phase tokens is undetected until the next check; the practical bound is the seller's check cadence, worst case the token `exp` (≤30 days).
- **Audit-time**: unbounded, cryptographically. Every token ever issued is forever-bindable to the plan state it attested to via the retained `plan_hash`. This is the property regulators and forensic audits depend on.

**Reference test vectors**: ten vectors under [`static/compliance/source/test-vectors/plan-hash/`](https://github.com/adcontextprotocol/adcp/tree/main/static/compliance/source/test-vectors/plan-hash) pin the canonicalization bit-exactly. They cover a minimal plan, a plan exercising every optional field, a bookkeeping-stripped case (GA-internal fields present on the stored plan but stripped before hashing, yielding the same hash as the bookkeeping-absent equivalent), paired vectors proving that omitted-vs-explicit-null, array order in `policy_categories`, and rotating `ext.trace_id` all produce distinct hashes, and a Unicode case confirming JCS does not normalize per RFC 8785 §3.2.5. Each vector records the preimage, the canonical JCS bytes, the SHA-256 hex digest, and the final `plan_hash` claim value. Governance agents and auditor verifiers MUST reproduce these hashes bit-exactly.
...
- **3.0**: governance agents MUST emit compact JWS per this profile, including a required `plan_hash` claim bound to the evaluated plan state. Sellers MAY verify the 15-check JWS checklist; sellers that do not verify MUST persist and forward the token unchanged. `plan_hash` is opaque to sellers (they cannot access the plan payload and are not expected to) and MUST NOT appear in `crit` — verification is performed by the governance agent itself and by auditors, not by sellers. Values that are not JWS are deprecated and SHOULD only appear from pre-3.0 governance agents during the transition; governance agents that emit non-JWS values in 3.0 MUST declare this in their capabilities so sellers can detect unverifiable deployments.
- **3.1**: all sellers MUST verify per the 15-check checklist above. Governance agents MUST emit JWS. Non-JWS values will be rejected end-to-end. `plan_hash` semantics do not change — it remains an audit-layer binding verified by the governance agent and auditors, not by sellers.
