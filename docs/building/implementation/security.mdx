...
- **New-keyid admission pressure** (MUST track, SHOULD alert). Verifiers MUST track the rate of cache entries admitted from previously-unseen `keyid`s per unit time (e.g., a 5-minute rolling count of distinct `keyid`s inserting their first entry). A sudden spike in new-keyid admission rate is the signature of a **distributed-compromise attack**: an attacker holding N compromised signer keys can drive N entries per TTL window each, every key staying well within its per-keyid cap (step 9a), while collectively saturating the aggregate cache. Each key's traffic individually looks like a low-volume legitimate signer; the aggregate shape is the signal. Operators SHOULD alert when new-keyid admission exceeds **any** of four thresholds, whichever triggers first: **(a)** `3×` the 24-hour moving average, **(b)** `2×` the 30-day P95, **(c)** `1.5×` the 90-day P99, or **(d)** a **proportional ceiling of `max(20 distinct new keyids, 10% of the 30-day unique-keyid count) per 5-minute window`**. The quadruple-threshold shape is load-bearing against four different attacker patterns: (a) sudden spikes against a stable baseline; (b) patient attackers who ramp up over weeks (30-day P95 is dominated by baseline-traffic tail over that horizon); (c) very-patient attackers who ramp over 60–90 days — the 90-day P99 is robust against anything shorter than a multi-quarter staged compromise; (d) sparse-traffic verifiers whose moving averages and P95/P99 values are near zero, AND auto-scaling for operators of any size (a small verifier with 20 active keyids and a 10k-keyid enterprise both get a proportionally useful floor). Operators MAY raise the fixed lower bound in (d) for documented high-volume onboarding windows but SHOULD document the raise and floor to baseline afterward. The threshold values above are spec **defaults** that operators SHOULD tune to their own traffic shape — operators MUST NOT use the published numbers as the final tuned values, which would hand attackers an oracle into the detection posture. The alarm payload MUST name which clause (a, b, c, or d) tripped so operator triage can respond to the right threat shape without replaying the full rule evaluation. Alarming here catches the slow-burn distributed-compromise pattern *before* the aggregate cap triggers — once `webhook_signature_rate_abuse` fires on the aggregate cap, the cache is already full and every legitimate signer is being rejected. Alarms SHOULD route to incident response, not to automatic revocation: the distinguishing signal between "attack" and "onboarding a batch of new sellers" is operator context, not machine-derivable.
