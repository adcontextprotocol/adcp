...
Sellers building AdCP into their production stack should consider **[AAO Verified](/docs/building/aao-verified)** once they're serving real campaigns. AAO Verified adds continuous observability of live delivery data on a designated compliance account and produces a mark distinct from AdCP Conformant (which comes from storyboards). Opt-in, no new infrastructure on the seller side — grant the [`attestation_verifier`](/docs/accounts/overview#standard-named-scope-attestation_verifier) scope on one account with live campaigns.
