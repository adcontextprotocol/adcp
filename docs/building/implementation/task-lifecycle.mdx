...
- **MCP**: Use [MCP Tasks](https://modelcontextprotocol.io/specification/2025-11-25/basic/utilities/tasks) — the client polls via `tasks/get` and retrieves results via `tasks/result` at the protocol level. See [MCP Guide](/docs/building/integration/mcp-guide#async-operations-via-mcp-tasks).
- **A2A**: Use native A2A task lifecycle with SSE streaming. See [A2A Guide](/docs/building/integration/a2a-guide).
- **REST**: Use AdCP's `task_id` with polling or [push notifications](/docs/building/implementation/webhooks).
:::
...
- **MCP**: Use [MCP Tasks](/docs/building/integration/mcp-guide#async-operations-via-mcp-tasks) or poll via `tasks/get`
- **A2A**: Subscribe to SSE stream for real-time updates
- **REST**: Use [push notifications](/docs/building/implementation/webhooks) (recommended) or poll with `task_id`
...
- **Async Operations**: See [Async Operations](/docs/building/implementation/async-operations) for handling different operation types
- **Webhooks**: See [Webhooks](/docs/building/implementation/webhooks) for push notification patterns
- **Error Handling**: See [Error Handling](/docs/building/implementation/error-handling) for error categories and recovery
