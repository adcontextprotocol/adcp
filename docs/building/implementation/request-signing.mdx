...
`requireSignatureWhenPresent` prevents bypass attacks where an invalid signature falls through to weaker authentication. When signature headers are present, only signature auth runs. When absent, the fallback (bearer/API key) runs as normal:
...
The spec ships test vectors at `compliance/cache/3.0.0/test-vectors/request-signing/` (source at `static/compliance/source/test-vectors/request-signing/`):
