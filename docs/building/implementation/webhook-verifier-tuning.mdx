...
- **Why this is a known limitation**: the admission-pressure rule closes volume-spike attacks, not low-rate long-duration attacks smoothed across long windows. This gap is covered by the **per-keyid cap** (step 9a of the verifier checklist), which bounds per-key traffic, and the **aggregate cache cap**, which bounds total cache size. The admission-pressure rule is the third layer; it is not a universal attack detector. Operators who want detection for slow-drip attacks SHOULD layer at the application layer (e.g., signer-reputation scoring, per-seller traffic-anomaly detection over business-meaningful windows like "signals delivered per billing period").
...
- [Webhook Security — Webhook replay dedup sizing](/docs/building/implementation/security#webhook-security) — normative spec for the rule this guide tunes.
- [Webhook verifier checklist](/docs/building/implementation/security#webhook-callbacks) — the full 15-check verifier flow, including step 14b (logging discipline) whose sanitization rules apply to the diagnostic information this guide assumes alarms carry.
