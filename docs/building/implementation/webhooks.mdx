...
description: "AdCP push notifications: how sellers deliver async task status updates to your webhook endpoint via HMAC-authenticated POST requests. Setup, URL templates, and idempotency."
...
4. The publisher POSTs webhook notifications to your URL as the task status changes, signing each POST with its `adcp_use: "webhook-signing"` key published in `adagents.json`
5. You verify the signature against the publisher's published JWKS and dedupe by `idempotency_key`
...
create_media_buy request
  └── push_notification_config
        └── url: "https://you.com/adcp/webhook/create_media_buy/agent_123/cd51e063-2b79-4a6d-afac-ed7789c3a443"
              // No shared secret — the publisher signs with its own key, you verify against
...
If you're using the `@adcp/client` library, this entire flow is handled automatically — configure `webhookUrlTemplate` and your agent URL once on the client, publish a webhook-signing JWK on your own `adagents.json` entry, and `push_notification_config` is injected into every outgoing task call. Incoming webhooks are verified against the publisher's JWKS automatically.

:::note Legacy HMAC fallback
Buyers integrating with receivers that have not yet adopted the RFC 9421 webhook profile MAY opt into the legacy HMAC-SHA256 scheme by populating `push_notification_config.authentication.credentials`. That path is deprecated and removed in AdCP 4.0 — see [Legacy HMAC-SHA256 fallback](#legacy-hmac-sha256-fallback-deprecated) below.
:::
...
`authentication` is omitted in the default case — the publisher signs with its own `adcp_use: "webhook-signing"` key. Include `authentication.credentials` only if you need the legacy HMAC-SHA256 fallback.
...
Every AdCP 3.0 webhook is signed under the [RFC 9421 webhook profile](/docs/building/implementation/security#webhook-callbacks). The publisher signs with its `adcp_use: "webhook-signing"` key published in `adagents.json`; you verify against the publisher's published JWKS. No shared secret crosses the wire.
...
The A2A `authentication.schemes: ["Bearer"]` scheme is also supported for compatibility and removed in AdCP 4.0. Bearer provides no tamper protection on the body — the 9421 profile is strictly stronger. Sellers SHOULD refuse Bearer for any mutating callback.
...
5. **Acknowledge immediately** — return `200` before doing any heavy processing to avoid publisher timeouts and unnecessary retries
