...
- **Mismatch forensics (MUST).** Silently deduping does NOT mean silently discarding the signal. When a receiver observes an incoming body whose hash differs from the cached body hash under the same `(sender, idempotency_key)`, the receiver MUST emit a structured log event of type `WEBHOOK_PAYLOAD_MISMATCH` (log event, not a returnable wire error code — the receiver still answers `2xx`; do not place it on the response) carrying at minimum: the sender identity, the `idempotency_key`, the SHA-256 of the cached body, the SHA-256 of the incoming body, and the arrival timestamp. The delivery response is still `2xx` (the dedup semantics above are unchanged), but the mismatch MUST be forensically recoverable. Receivers SHOULD alert operators when this event fires at non-trivial rates from a given sender, because in steady state it indicates either a sender bug or an attacker attempting to substitute content under a previously-accepted key.
