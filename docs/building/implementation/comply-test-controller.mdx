...
This is the design pattern Stripe uses for `/v1/test_helpers/*` (test clocks, fund balances, forced refund expirations, forced authorization captures, treasury-transfer outcome forcing), Plaid uses for sandbox, Twilio uses for test credentials, and Auth0 uses for test tenants. Same architecture in each case: one binary, runtime gate on the API-key namespace (`sk_test_` vs `sk_live_` in the Stripe case), production code paths run against test-namespaced data. None of those vendors maintain a separate test-mode binary. The pattern has been industry-standard for state-forcing test primitives in financial APIs for over a decade and is the security model AdCP follows.
...
5. **No real PII.** Sandbox accounts MUST NOT contain or operate on production audience data, production creative assets, or production reporting data. Hashed identifiers and 1P signals scoped to sandbox accounts MUST be synthetic.
