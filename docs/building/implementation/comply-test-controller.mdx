...
| `UNKNOWN_SCENARIO` | Scenario not implemented |
...
Sellers that do not implement `list_scenarios` SHOULD return an error with `UNKNOWN_SCENARIO`. Comply treats this as "all declared enum values are supported" — this fallback exists so early implementers who skip `list_scenarios` are not penalized.
...
5. get_account_financials() → verify total_spend reflects 95% of budget
...
2. **Tests are self-contained.** Each test SHOULD create dedicated entities (media buys, creatives, accounts) rather than reusing existing ones. This ensures additive simulation calls (`simulate_delivery`) start from known-zero state without needing a reset mechanism. No `reset` scenario is needed.
