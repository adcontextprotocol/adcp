...
- **Bundled-tree caveat.** Tools served from the pre-resolved bundled tree (`/schemas/{version}/bundled/...`) inline sub-schemas with their `$id` preserved (3.1+, see #3868). Consumers reading bundles produced before that fix see the response-root `$id` instead, which still names a valid published schema but doesn't pinpoint the rejecting variant. Detect by checking whether the `$id` ends in `/bundled/<tool>-response.json` — if so, fall back to walking the bundled schema by `pointer`.
- **Validator strict-mode.** The bundled tree contains the same `$id` multiple times when a sub-schema is referenced from multiple co-locations. Configure the validator in **non-strict mode** (Ajv: `strict: false`; `santhosh-tekuri/jsonschema` Go: `Compiler.Decoder = …` with strict disabled; Python `jsonschema`: last-wins is the default). Strict mode treats duplicates as an error.
