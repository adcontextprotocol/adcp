...
**Form consistency during the deprecation window.** A seller MUST choose one form (dedicated `AGENT_SUSPENDED` / `AGENT_BLOCKED` codes OR `PERMISSION_DENIED + details.status`) per agent state and emit it consistently across all surfaces, tasks, and replicas for the duration of that state. Mixing forms across the same seller — `AGENT_SUSPENDED` on `sync_accounts` but `PERMISSION_DENIED + status:"suspended"` on `create_media_buy`, or different forms across rollout cohorts — lets callers fingerprint partial-rollout boundaries; when rollouts track tenant cohorts, that fingerprint correlates to a tenant-cohort oracle. The choice is per-seller-deployment, not per-request. Sellers SHOULD migrate fully to the dedicated codes during the deprecation window; any consumer-facing inconsistency is a defect.
...
async function dispatchAuthzError(error) {
  ...
  // Preferred path; sellers SHOULD emit these directly once the dedicated codes ship.
  // Dedicated code wins: if both this and a deprecated `details.status` arrive
  // (transitioning seller), branch here and ignore status.
