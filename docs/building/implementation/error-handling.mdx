...
|------|----------|-------------|------------|
| `AUTH_MISSING` | correctable | No credentials were presented | Provide credentials via auth header |
| `AUTH_INVALID` | terminal | Credentials were presented but rejected (expired / revoked / malformed) | Human credential rotation required; do not auto-retry |
| `AUTH_REQUIRED` | correctable | **Deprecated** — use `AUTH_MISSING` or `AUTH_INVALID` | Retained for backward compatibility |
...
async function handleAdcpError(error) {
  ...
  switch (error.code) {
    case 'AUTH_MISSING':
    case 'AUTH_REQUIRED': // deprecated alias for AUTH_MISSING
      await refreshCredentials();
      return retry();

    case 'AUTH_INVALID':
      // Terminal — credentials were rejected server-side; human rotation required
      console.error('Credential rejected (expired/revoked):', error.message);
      throw error;
...
| `AUTH_MISSING` | correctable | Governance agent requires authentication |
