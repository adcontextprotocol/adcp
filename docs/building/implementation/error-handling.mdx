...
`PERMISSION_DENIED` covers two distinct rejection shapes that recover differently. When the gate that fired is the calling buyer agent's commercial relationship with the seller (the agent is `suspended`, `blocked`, or provisioned for sandbox traffic only), `error.details` SHOULD conform to [`error-details/agent-permission-denied.json`](https://adcontextprotocol.org/schemas/v3/error-details/agent-permission-denied.json) so callers can dispatch on `details.scope` + `details.status` / `details.reason` without parsing prose. The shape is closed (`additionalProperties: false`) and clamped:

| `details.scope` | `details.status` | `details.reason` | Meaning |
|---|---|---|---|
| `"agent"` | `"suspended"` | — | Agent is onboarded but the relationship is temporarily paused. Re-onboarding may resolve. |
| `"agent"` | `"blocked"` | — | Agent's relationship is permanently denied. No autonomous recovery. |
| `"agent"` | — | `"sandbox_only"` | Agent is provisioned for sandbox traffic only; the request was against a non-sandbox account. |

Exactly one of `status` or `reason` MUST be present alongside `scope: "agent"` — the schema's `oneOf` rejects both-present and neither-present envelopes. The `scope` value is a registered subset of the shared discriminator vocabulary in [`enums/error-scope.json`](https://adcontextprotocol.org/schemas/v3/enums/error-scope.json); per-agent scope is reserved for this shape and for [`BILLING_NOT_PERMITTED_FOR_AGENT`](#billing-and-account-setup) (which carries no explicit `scope` field).

Normative requirements:

- **Uniform response without established agent identity.** The per-agent scope is meaningful only when buyer-agent identity has been established via signed-request derivation per [Agent identity](/docs/building/implementation/security#agent-identity) or via a credential-to-agent mapping in the seller's onboarding record. Sellers MUST emit `details.scope: "agent"` ONLY on that path; in all other cases — including bearer credentials not mapped to a specific agent record — sellers MUST return `PERMISSION_DENIED` and MUST omit `error.details.scope`. Returning the per-agent scope without established identity lets unauthenticated probes use the discriminator as a cross-tenant onboarding oracle, the same shape the [`*_NOT_FOUND` uniform-response rule](#standard-error-codes) and [`BILLING_NOT_PERMITTED_FOR_AGENT`](#billing-and-account-setup) close.
...
- **Details shape is clamped.** `error.details` MUST conform to [`error-details/agent-permission-denied.json`](https://adcontextprotocol.org/schemas/v3/error-details/agent-permission-denied.json): `scope` plus exactly one of `status` or `reason`, with `additionalProperties: false` and a `oneOf` enforcing exclusivity. The shape MUST NOT carry the agent's full permitted-billing subset, rate cards, payment terms, credit limit, billing entity, custom reason strings, or any other per-agent commercial state — full-subset disclosure in a single probe is exactly the oracle the clamp prevents. The schema's `additionalProperties: false` is a schema-validation guard; sellers MUST treat unrecognized or extension keys received from a seller-side composer as a defect and drop them before transmission, not rely on validators alone. New `reason` values MUST be registered in the schema's enum so cross-language SDKs can dispatch without parsing prose; future per-agent state additions belong on new dedicated codes (each with its own clamped details shape), NOT by relaxing this shape.
- **No autonomous retry on the per-agent gate.** When `details.status` or `details.reason` is present, the rejection is terminal-pending-onboarding. The agent MUST NOT auto-retry against the same credential — the agent cannot unilaterally lift a suspension/block or extend its sandbox-only provisioning, and re-attempts only reinforce the gate. Agents MUST surface to a human at the buyer to resolve the underlying commercial state with the seller offline.

**Recovery dispatch — example.** Branch on `details.scope` to distinguish the per-agent gate from generic credential-shaped failures:

```javascript
async function dispatchPermissionDenied(error) {
  ...
  // Per-agent commercial gate — terminal-pending-onboarding.
  if (error.details.scope === 'agent') {
    const { status, reason } = error.details;
    // status: 'suspended' | 'blocked'  |  reason: 'sandbox_only'
    return surfaceToHuman({ code: error.code, status, reason });
  }
...
**Example envelope** for `PERMISSION_DENIED` with the per-agent gate firing on a suspended agent:
...
The wire-level `recovery: "correctable"` value is the registered classification on `PERMISSION_DENIED` per [`error-code.json`](https://adcontextprotocol.org/schemas/v3/enums/error-code.json) `enumMetadata` and matches the dual-surface contract — SDKs MUST NOT switch the registered value based on `details.scope`. Buyer agents MUST instead branch on `details.scope` itself: when `details.scope === "agent"`, treat the rejection as terminal-pending-onboarding regardless of the wire-level `recovery` field, surface to a human, and do not auto-retry. A future major version may either split `suspended` / `blocked` / `sandbox_only` into dedicated codes (each with its own registered `recovery` classification) or extend `enumMetadata` with a conditional-recovery shape; until then, `details.scope` is the load-bearing branch.
