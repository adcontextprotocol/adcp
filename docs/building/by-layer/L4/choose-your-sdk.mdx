...
Pre-3.0 callers should work through the [3.0 migration guide](/docs/reference/migration/index) before upgrading.
...
npx @adcp/sdk@latest mcp https://sales.example.com/mcp get_products '{"brief":"CTV campaign"}'
```

The CLI also drives storyboards (`adcp storyboard run`), the public test agent (`adcp test-mcp`), and registry diagnostics. See `--help` for the full surface.

### Python CLI

```bash
uvx adcp --help
uvx adcp https://sales.example.com/mcp get_products '{"brief":"CTV campaign"}'
```

Same positional shape as the JS CLI: `adcp <agent> [tool] [payload]`. Save aliases with `--save-auth` to avoid retyping URLs.
