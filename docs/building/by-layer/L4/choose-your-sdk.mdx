...
|---|---|---|:-:|:-:|:-:|:-:|
| **`@adcp/sdk`** (TypeScript) | `14.0.0-rc.36` | Exact `3.2.0-rc.2` support | ✅ | ✅ | ✅ | ✅ |
...
<Warning>
**No SDK embeds RC.3, the current protocol checkpoint.** TypeScript embeds
`3.2.0-rc.2`; Python and Go still embed `3.2.0-rc.1`. Pin the exact package
and wire versions shown here for development and staging, and test RC.3 from
its [signed bundle](/docs/reference/3-2-beta#use-the-rc-3-artifact-directly).
This is not a GA signal: production integrations should remain on 3.1 until
3.2 is released. See the [3.2 prerelease program](/docs/reference/3-2-beta).
</Warning>
...
```bash
npx @adcp/sdk@14.0.0-rc.36 --help
npx @adcp/sdk@14.0.0-rc.36 --save-auth my-agent https://sales.example.com/mcp
npx @adcp/sdk@14.0.0-rc.36 my-agent list_products '{"adcp_version":"3.2-rc.2","account":{"account_id":"account_123"}}'
# or against the built-in public test agent, which still serves "3.2-rc.1":
npx @adcp/sdk@14.0.0-rc.36 test-mcp list_products '{"adcp_version":"3.2-rc.1"}'
```
...
validation still uses the SDK's embedded `3.2.0-rc.2` bundle.
