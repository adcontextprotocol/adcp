...
const client = new ADCPClient({
  agentUrl: 'https://sales.example.com',
  authToken: process.env.ADCP_API_KEY,
});
```
</Tab>
...
The [Async responses section of Calling an agent](/docs/protocol/calling-an-agent#async-responses-status-submitted-means-queued) has the full polling contract. If you configure webhooks, the same `result` payload arrives in the push notification — see [Receive webhooks](#receive-webhooks) below.
...
For async tasks, you can either poll or register a webhook. The webhook delivers the same `result` payload as a `tasks/get` with `include_result: true`. The SDK's `webhook-receiver` helper handles RFC 9421 signature verification (resolving keys via the agent's brand.json by default), replay-window enforcement, and idempotent dedup of redelivered notifications:
...
For custom key resolution (private keystore, KMS), pass `resolveKey` and other [verifier options](/docs/building/by-layer/L1/webhook-verifier-tuning) — the L1 page covers clock-skew handling, key-rotation transitions, and signature failure diagnostics.
