...
Examples below use **A2A 1.0** wire format: Parts carry no `kind` discriminator (content type is implied by which field is set — `text`, `data`, `url`, or `raw`), roles are `ROLE_USER` / `ROLE_AGENT`, and task states are `TASK_STATE_*` (ProtoJSON canonical). See the [A2A Guide](/docs/building/by-layer/L0/a2a-guide#a2a-protocol-versions) for a side-by-side with v0.3.

There are two distinct status layers. A2A `task.status.state` describes transport execution; the AdCP payload decoded from the DataPart's JSON text has the task-specific top-level `status`. They usually align, but they are not aliases. For example, a structured `get_products` business rejection uses A2A `TASK_STATE_COMPLETED` while the decoded payload has `status: "rejected"`: the invocation completed and produced a deliberate commercial-refusal result.

The [AdCP A2A Profile Extension v3.2](/docs/building/by-layer/L0/a2a-profile-extension) also pins an AdCP `status: "submitted"` response inside an A2A `TASK_STATE_COMPLETED` Task. Native A2A `TASK_STATE_SUBMITTED` is an interim transport state before the AdCP handler returns; it is not the mapping for queued AdCP work.

The v3.2 profile encodes the complete AdCP payload once as JSON text in
string-valued `data` with `mediaType: "application/json"`. The legacy
`/extensions/adcp/v3` profile and v0.3 compatibility paths use object-valued
`data`; they are separate negotiated profiles, not fallback representations for
v3.2.
...
- Include at least one DataPart whose string-valued `data` field contains the task response as JSON text and whose `mediaType` is `application/json`
...
- **DataPart** (Part with string-valued `data` and `mediaType: "application/json"`): JSON-text AdCP response payload — **required**
  ...
  "status": {
    ...
    "message": {
      ...
      "parts": [
        ...
        {
          "data": "{\"percentage\":45,\"current_step\":\"analyzing_inventory\"}",
          "mediaType": "application/json"
        }
      ]
    }
...
**CRITICAL**: After decoding the DataPart JSON text once, its content MUST be the direct AdCP response payload, not wrapped in framework-specific objects.

```json
// ❌ WRONG decoded payload - wrapped in custom object
{"response": {"products": []}}

// ✅ CORRECT decoded payload - direct AdCP response
{"status": "completed", "cache_scope": "account", "products": []}
```
...
```javascript
// The negotiated profile selects decoding; never infer it from the value type.
const isDataPart = (p) => p.data != null;
const isTextPart = (p) => typeof p.text === 'string';

function decodeV3_2DataPart(part) {
  if (typeof part.data !== 'string' || part.mediaType !== 'application/json') {
    throw new Error('Invalid AdCP v3.2 DataPart');
  }
  const payload = JSON.parse(part.data);
  if (payload == null || typeof payload !== 'object' || Array.isArray(payload)) {
    throw new Error('Invalid AdCP v3.2 payload: expected an object');
  }
  return payload;
}
...
function extractDataPartFromArtifacts(response) {
  ...
  const payload = decodeV3_2DataPart(lastDataPart);
...
function extractDataPartFromMessage(response) {
  ...
  return dataPart ? decodeV3_2DataPart(dataPart) : null;
}
...
Field-presence detection works for both A2A wire versions. The decoder above is
specific to the negotiated v3.2 profile; a legacy `/v3` or v0.3 adapter instead
requires object-valued `data`. Implementations select that decoder from the
active profile and MUST NOT retry with it after v3.2 decoding fails.
...
async function executeTask(taskName, params) {
  const response = await a2aClient.send({
    message: {
      ...
      role: 'ROLE_USER',
      parts: [{
        data: JSON.stringify({ skill: taskName, input: params }),
        mediaType: 'application/json'
      }]
    }
  });
  ...
  "task": {
    ...
    "artifacts": [{
      ...
      "parts": [
        ...
        {"data": "{\"status\":\"completed\",\"cache_scope\":\"account\",\"products\":[],\"total\":12}", "mediaType": "application/json"}
      ]
    }]
  }
}
  ...
  status: {
    ...
    message: {
      ...
      parts: [
        ...
        { data: '{"percentage":50,"current_step":"analyzing"}', mediaType: 'application/json' }
      ]
    }
  }
};
...
const completedResponse = {
  ...
  artifacts: [{
    ...
    parts: [
      ...
      { data: '{"status":"completed","cache_scope":"account","products":[],"total":3}', mediaType: 'application/json' }
    ]
  }]
};
...
const wrappedResponse = {
  ...
  artifacts: [{
    artifactId: 'product-result',
    parts: [
      { data: '{"response":{"products":[]}}', mediaType: 'application/json' }
    ]
  }]
};
...
```json
{
  "task": {
    ...
    "artifacts": [{
      ...
      "parts": [{
        "data": "{\"status\":\"completed\",\"signals\":[],\"errors\":[{\"code\":\"NO_DATA_IN_REGION\",\"message\":\"No signal data available for Australia\"}]}",
        "mediaType": "application/json"
      }]
    }]
  }
...
```json
{
  "task": {
    ...
    "artifacts": [{
      ...
      "parts": [{
        "data": "{\"adcp_error\":{\"code\":\"AUTHENTICATION_FAILED\",\"message\":\"The API token is invalid or expired\"}}",
        "mediaType": "application/json"
      }]
    }]
  }
  ...
  "task": {
    ...
    "artifacts": [{
      ...
      "parts": [
        ...
        { "data": "{\"status\":\"completed\",\"media_buy_id\":\"mb_456\",\"confirmed_at\":\"2026-01-22T10:30:00.000Z\",\"revision\":1,\"packages\":[]}", "mediaType": "application/json" }
      ]
    }]
  }
}
    ...
    "status": {
      ...
      "message": {
        ...
        "parts": [
          ...
          { "data": "{\"reason\":\"START_TIME_REQUIRED\"}", "mediaType": "application/json" }
        ]
      }
    }
  }
...
```json
{
  "task": {
    ...
    "artifacts": [{
      ...
      "parts": [
        { "data": "{\"status\":\"completed\",\"creative_id\":\"cr_789\"}", "mediaType": "application/json" },
...
- [ ] **Use `.artifacts` array with at least one DataPart** containing JSON-text AdCP response payload
...
- [ ] **DataPart MUST be string-valued JSON with `mediaType: application/json`**, decoded exactly once at the adapter boundary
- [ ] **Decoded DataPart content MUST match AdCP schemas** (validate against `[task]-response.json`)
