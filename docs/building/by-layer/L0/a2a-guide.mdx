...
Deterministic AdCP 3.2 invocation on A2A 1.0 uses the [AdCP A2A Profile Extension v3.2](/docs/building/by-layer/L0/a2a-profile-extension), identified by `https://adcontextprotocol.org/extensions/adcp/v3.2`. Clients activate it on every request with `A2A-Extensions`. The older `/v3` identity remains the frozen object-valued profile for existing 3.0/3.1 integrations.

### 1. Initialize an A2A 1.0 client

```javascript
const endpoint = 'https://seller.example/a2a/jsonrpc';
const profileUri = 'https://adcontextprotocol.org/extensions/adcp/v3.2';
...
const adcpProfile = agentCard.capabilities.extensions?.find(
  ext => ext.uri === 'https://adcontextprotocol.org/extensions/adcp/v3.2'
);
...
A profile invocation contains exactly one authoritative DataPart whose `data`
is JSON text encoding `{ skill, input }` and whose `mediaType` is
`application/json`. Keeping the complete envelope in the protobuf string arm
prevents integer widening. The Message may also contain advisory TextParts;
text never overrides the decoded input:
...
AdCP 3.2 responses over A2A **MUST** include at least one DataPart whose
string-valued `data` field contains the task response as JSON text and whose
`mediaType` is `application/json`. A TextPart for human-readable messages is
...
if (artifact) {
  ...
  const dataPart = artifact.parts?.filter(isData).at(-1);
  if (typeof dataPart?.data !== 'string' || dataPart.mediaType !== 'application/json') {
  ...
  "task": {
    ...
    "artifacts": [{
      ...
      "parts": [
        ...
        {
          "data": "{\"status\":\"completed\",\"media_buy_id\":\"mb_12345\",\"confirmed_at\":\"2026-01-22T10:30:00.000Z\",\"revision\":1,\"creative_deadline\":\"2026-01-30T23:59:59.000Z\",\"packages\":[{\"package_id\":\"pkg_001\",\"context\":{\"line_item\":\"li_ctv_sports\"}}]}",
          "mediaType": "application/json"
        }
      ]
    }]
    ...
    "status": {
      ...
      "message": {
        ...
        "parts": [
          ...
          {
            "data": "{\"reason\":\"BUDGET_EXCEEDS_LIMIT\"}",
            "mediaType": "application/json"
          }
        ]
      },
...
**Do not conflate the two `submitted` values.** Native A2A `TASK_STATE_SUBMITTED` is an interim transport state and may appear in a `TaskStatusUpdateEvent` before an AdCP handler returns. An AdCP response whose decoded JSON-text DataPart contains `status: "submitted"` is instead pinned inside an A2A `TASK_STATE_COMPLETED` Task by the AdCP v3.2 profile. Observe that durable AdCP operation with `get_task_status`.
...
app.post('/webhooks/a2a/:taskType/:operationId', async (req, res) => {
  ...
  if (FINAL.includes(normalizedStatus)) {
    ...
    adcpData = dataPart ? decodeDataPart(dataPart) : undefined;
...
Generic A2A messages can combine text, data, and files. The activated AdCP v3.2 A2A profile deliberately narrows invocation messages to one JSON-text DataPart plus optional advisory TextParts. Put resource references in the selected AdCP task's typed `input`; do not add a FilePart that the task schema cannot validate.
...
If a task schema does not define a field for the file or resource, that input is not supported by the AdCP v3.2 A2A profile. Use a separate generic A2A interface or first transform the resource into schema-valid AdCP fields.
...
<Note>
Use [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities) for runtime capability discovery. The Agent Card extension declaration identifies the A2A wire profile only.
</Note>

Include the versioned AdCP profile under `capabilities.extensions[]` and activate it on every invocation with `A2A-Extensions: https://adcontextprotocol.org/extensions/adcp/v3.2`.

The A2A protocol's `AgentExtension` has:
- **`uri`**: Extension identifier (`https://adcontextprotocol.org/extensions/adcp/v3.2`)
...
if (adcpExt) {
  ...
  const capabilities = await a2a.send({
    message: {
      ...
      role: 'ROLE_USER',
      parts: [{
        data: JSON.stringify({ skill: 'get_adcp_capabilities', input: {} }),
        mediaType: 'application/json'
      }]
    }
  });
...
async function handleA2aResponse(response, invocation) {
  switch (response.status) {
    case 'input-required':
      ...
      return a2a.send({
        message: {
          ...
          role: "ROLE_USER",
          parts: [{
            data: JSON.stringify({ skill: invocation.skill, input: refinedInput }),
            mediaType: 'application/json'
          }]
        }
      });
    ...
    case 'completed':
      ...
      return JSON.parse(dataPart.data);
...
For uploading creative assets and handling validation errors, use the `sync_creatives` task. See [sync_creatives Task Reference](/docs/creative/task-reference/sync_creatives) for complete testable examples.
