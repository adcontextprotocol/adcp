...
Deterministic AdCP invocation on A2A 1.0 uses the [AdCP A2A Profile Extension v3](/docs/building/by-layer/L0/a2a-profile-extension), identified by `https://adcontextprotocol.org/extensions/adcp/v3`. Clients activate it on every request with `A2A-Extensions`.
