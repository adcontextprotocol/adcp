...
> **Deprecated:** `adcp_use: "webhook-signing"` is deprecated and will be removed in AdCP 4.0. Verifiers MUST still accept it for backward compatibility (a webhook signed under a `"webhook-signing"` key verifies cleanly), but new signers SHOULD publish and sign with `"request-signing"` keys only.
