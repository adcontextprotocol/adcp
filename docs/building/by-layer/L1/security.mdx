...
**Product-discovery migration in 3.2.** AdCP 3.2 adds five compact tools alongside the stable `get_products` facade:
...
- `request_proposals`, `refine_proposals`, `finalize_proposals`, and `decline_proposals` require a key because they can allocate tasks or change proposal state.
