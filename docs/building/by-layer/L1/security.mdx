...
**Product-discovery migration in 3.2.** AdCP 3.2 adds four explicit tools around the stable `get_products` operation family:

- `list_products` is a side-effect-free synchronous read; its key is optional.
- `recommend_products`, `refine_proposal`, and `finalize_proposals` require a key because they can allocate tasks or change proposal state.
...
Aliases MUST normalize to the same canonical `get_products` payload before authorization, idempotency hashing, task recovery, persistence, and webhook emission. A retry may therefore switch between the legacy and split name without duplicating work; the same key with a different canonical payload returns `IDEMPOTENCY_CONFLICT`.
