...
| `caller` | Yes | URL attributed to the party that requested the governance check. When the transport credential maps to an agent URL, the governance agent MUST require the request's `caller` assertion to match and MUST sign the resolved URL. A plan with delegations or non-null `approved_sellers` MUST reject when no authenticated agent URL can be resolved. On an unrestricted legacy plan only, the unresolved assertion MAY be retained for audit and signed-context continuity, but it is unauthenticated attribution and MUST NOT grant authorization. In intent phase, the caller is the orchestrator/buyer; in execution phases, it is typically the seller itself. |
