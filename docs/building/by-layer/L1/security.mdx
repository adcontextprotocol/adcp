...
5. **Cap response size and timeouts.** Recommended: 5 MB body cap, 10 s connect, 10 s read. Managed-network authoritative `adagents.json` files are the exception — they fan out across a publisher network and use a higher cap (recommended 20 MB); see [managed networks security](/docs/governance/property/managed-networks#security-considerations).
