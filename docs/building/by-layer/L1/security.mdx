...
2. Locate exactly one applicable `agents[]` entry whose `type` is `brand` and whose `url` matches the envelope's `agent_url` after applying the shared [URL canonicalization](/docs/reference/url-canonicalization) algorithm. For a Brand Agent or Brand Canonical Document, search the top-level `agents[]`. For a House Portfolio, search `house.agents[]` (which applies across the portfolio) and every inline `brands[].agents[]`; do not search `brand_refs[]`, because those entries require resolving the child's canonical document. No match is `agent_not_authorized`; multiple canonical matches across the applicable collections are `agent_authorization_ambiguous`.
