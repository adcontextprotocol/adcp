...
- `get_products` remains valid throughout 3.x as a deprecated compatibility facade. Its key is optional only when the selected execution arm is a guaranteed side-effect-free synchronous read, in which case the seller MAY ignore a supplied key. An arm that allocates a task, finalizes a proposal, or otherwise changes observable state MUST require a key and apply the replay contract before that effect.
