...
**One JWK per `adcp_use` — publication shape.** The single-purpose rule applies to key material **and** to JWKS publication, with one explicit exception: an operator MAY reuse its `"request-signing"` key to sign webhooks (see [Webhook callbacks](#webhook-callbacks), step 8), in which case it publishes a single `"request-signing"` entry and no separate webhook-signing entry. An operator that instead wants a dedicated webhook-signing key (RECOMMENDED for blast-radius isolation) needs distinct key material and publishes two entries with the same JWK shape, distinct `x`, distinct `kid`, and distinct `adcp_use`. The `adcp_use` value is always a **string**, not an array — publishing `"adcp_use": ["request-signing","webhook-signing"]` on a single entry is a schema error that receivers will reject:
