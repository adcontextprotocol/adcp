...
**Brand-authorization cross-check (verifier).** A valid signature proves only that *some* agent holding a `response-signing` key signed the payload — not that the key is authorized to answer for `brand_domain`. `brand_domain` and `agent_url` are attacker-choosable fields *inside* the payload the same key signs, so a verifier that resolves the envelope's `kid` at the `jwks_uri` the envelope's own `agent_url` advertises has confirmed authorship, not brand authority: an attacker running a conformant brand-agent for a domain it controls can emit a cryptographically valid `owned` envelope for `brand_domain: "victim.example"`. This is the response-side analogue of the governance token whose `iss` is untrusted until [step 13 of the seller verification checklist](#seller-verification-checklist), and of the request-signing agent-URL match in the [`brand_json_url` verifier](#quickstart-implement-a-brand_json_url-based-verifier). Before treating a signed `verify_brand_claim` / `verify_brand_claims` envelope as evidence about the asserted `brand_domain`, an online verifier MUST:

1. Resolve `https://{brand_domain}/.well-known/brand.json` following the SSRF-safe [brand.json resolution](#buyer-identity-resolution) rules (one redirect, no chains).
2. Locate the `agents[]` entry whose `type` is `brand` and whose `url` **byte-equals** the envelope's `agent_url` (no canonicalization at this step — same rule as the governance `iss` and request-signing agent-URL matches; the most common failure mode is a trailing-slash or scheme mismatch). Reject with `response_signature_agent_not_in_brand_json` if none matches, or `response_signature_brand_json_ambiguous` if more than one does (operator misconfig — `agents[]` URLs MUST be unique).
3. Resolve the envelope's `kid` to a JWK **only** within that entry's `jwks_uri` — never a `jwks_uri` the envelope, its origin, or the `agent_url` host advertises for itself.

Until this cross-check passes, `brand_domain` and `agent_url` are untrusted input regardless of a valid signature. The requirement is symmetric across the direction-asymmetric trust model: a verifier MUST bind the key to `brand_domain`'s brand.json before believing either an assertion (`owned` / `licensed_*` / …) or a rejection (`disputed` / `not_ours`), since an unbound signer could forge either. Audit verifiers MAY retain a cross-check failure as unauthenticated historical evidence but MUST NOT treat it as an authorization signal. Conformance vectors: [`/compliance/latest/test-vectors/brand-response-signing/brand-authorization-cross-check`](https://adcontextprotocol.org/compliance/latest/test-vectors/brand-response-signing/brand-authorization-cross-check.json).
