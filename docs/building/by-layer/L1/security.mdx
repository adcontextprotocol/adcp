...
2. Locate exactly one applicable `agents[]` entry whose `type` is `brand` and whose `url` matches the envelope's `agent_url` after applying the shared [URL canonicalization](/docs/reference/url-canonicalization) algorithm. No match is `agent_not_authorized`; multiple canonical matches are `agent_authorization_ambiguous`.
