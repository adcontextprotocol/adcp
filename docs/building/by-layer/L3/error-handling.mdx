...
| `retry_after` | integer | No | Whole seconds to wait before retrying (transient errors), from 1 through 3600 |
...
- Callers **MUST** honor integer `retry_after` when present and **MUST NOT** retry the same request sooner than the indicated number of seconds. During migration, a caller that accepts a legacy finite fractional value **SHOULD** round it up before clamping to `[1, 3600]`.
