...
| Account-level `sync_accounts.accounts[].notification_configs[]` | Event-specific payloads such as `creative-status-changed-webhook.json`, `creative-assignment-changed-webhook.json`, `insights-changed-webhook.json`, `account-status-changed-webhook.json`, or `wholesale-feed-webhook.json` |
