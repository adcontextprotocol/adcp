...
Property-list and collection-list change payloads may carry a deprecated body-level `signature` field from older 3.x implementations. It is optional in 3.2 and receivers MUST ignore it for authentication. New senders omit it and authenticate only through the HTTP signature headers above. Unlike `push_notification_config` and account/agent notification registrations, list webhook registration exposes no legacy authentication selector: these two delivery surfaces are RFC 9421-only. The compatibility field is removed in 4.0.
