...
contract](/docs/building/by-layer/L3/async-identity-and-convergence). Delivery
...
app.post('/webhooks/adcp', async (req, res) => {
  ...
  // publication. The claim result is one of: claimed, processing, completed,
  // or conflict. This helper is durable across every receiver instance.
  const claim = await db.claimWebhook({
    sender,
    idempotencyKey: idempotency_key,
    payloadFingerprint: canonicalFingerprint(payload)
  });

  if (claim === 'conflict') return res.status(409).end();
  if (claim === 'processing') return res.status(503).end();
  if (claim === 'completed') return res.status(200).end();
  ...
  if (convergence === 'terminal_conflict') {
    await db.rejectWebhookClaim(sender, idempotency_key, claim.owner);
    return res.status(409).end();
  }
  if (convergence === 'publication_in_progress') {
    await db.releaseWebhookClaim(sender, idempotency_key, claim.owner);
    return res.status(503).end();
  }
  if (convergence === 'exact_duplicate_or_stale') {
    await db.acknowledgeWebhook(sender, idempotency_key, claim.owner);
...
Agent-level webhooks outlive any account or media buy. They are registered with [`sync_agent_notification_configs`](/docs/protocol/sync_agent_notification_configs), not with account-level `notification_configs[]` or per-task `push_notification_config`.

The initial agent-level event type is `capabilities.changed`. It invalidates cached [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities) responses. The webhook payload is deliberately small and does not contain the full capability document; receivers re-read `get_adcp_capabilities` and replace their cached snapshot from the fresh response. Sellers MUST publish the new capability snapshot before firing so the webhook's `capabilities_version` is observable on read. Receivers SHOULD retry the read with normal transient-error backoff if the webhook revision is not yet observable.
