...
| Agent-level `sync_agent_configuration.configuration.notification_configs[]` or `sync_agent_notification_configs.notification_configs[]` | Event-specific AdCP payload schemas such as `capabilities-changed-webhook.json` |
...
Task webhooks fire once per logical task and stop when the task settles. **Persistent webhooks** — `reporting_webhook`, `push_notification_config` on a media buy, account-level `notification_configs[]`, and agent-level subscribers managed by `sync_agent_configuration` or `sync_agent_notification_configs` — outlive any single operation and fire repeatedly for the life of the resource or agent. The contract below applies to persistent channels.
