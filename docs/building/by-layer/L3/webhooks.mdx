...
app.post('/webhooks/adcp', async (req, res) => {
  ...
  const { idempotency_key, notification_id, task_id, status, timestamp, result } = payload;
...
- **At-least-once delivery.** Sellers MAY re-fire the same logical event under retry. Receivers MUST dedupe by `idempotency_key` for transport-layer dedup. For state-shaped events (currently: `impairment`), the envelope also carries `notification_id` — receivers MUST use it for application-layer state correlation: the same `notification_id` under a distinct `idempotency_key` signals a re-emission of the same state change, not a new event. Point-in-time data events (delivery report fires: `scheduled`, `final`, `delayed`, `adjusted`) do not carry `notification_id`.
...
- Missed delivery report fire → call `get_media_buy_delivery` for the window in question. Once [#4590](https://github.com/adcontextprotocol/adcp/issues/4590) lands, the pull returns the same per-window slices the webhook delivered; until then, the pull returns date-range aggregates and daily breakdowns but cannot reconstruct hourly fires.
