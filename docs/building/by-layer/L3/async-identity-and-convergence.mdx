...
| Different delivery keys for the same logical terminal observation | The first authoritative terminal observation owns publication | Correlate by stable notification identity when present, otherwise by the bound operation and canonical terminal payload; acknowledge duplicates without republishing |
