...
| Blocked on external dependency | `submitted` | Truly async — receive completion through `push_notification_config` when configured, or poll by `task_id`. Result may take hours or days |
...
**`submitted` is async.** The operation is blocked on something outside the server's control — publisher approval, human review, third-party processing. The caller may configure a webhook and move on, or poll the AdCP task status surface with `task_id`.
