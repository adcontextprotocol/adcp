...
| **SI sessions** | Initiate → message → terminate | Force `terminated` with timeout reason → verify [`SESSION_NOT_FOUND`](/docs/building/verification/compliance-catalog#error-code-session-not-found) on next call |
| **Reporting** | Call [`get_media_buy_delivery`](/docs/media-buy/task-reference/get_media_buy_delivery) → hope data exists | Simulate delivery → verify rollups |
...
- Reflect forced state changes in subsequent reads ([`list_creatives`](/docs/creative/task-reference/list_creatives), [`get_media_buys`](/docs/media-buy/task-reference/get_media_buys), etc.)
...
they expose
[`list_products`](/docs/media-buy/task-reference/list_products), [`request_proposals`](/docs/media-buy/task-reference/request_proposals), [`refine_proposals`](/docs/media-buy/task-reference/refine_proposals), [`decline_proposals`](/docs/media-buy/task-reference/decline_proposals),
[`accept_proposal`](/docs/media-buy/task-reference/accept_proposal), [`control_media_buy`](/docs/media-buy/task-reference/control_media_buy), and [`get_media_buys`](/docs/media-buy/task-reference/get_media_buys); the 3.x [`create_media_buy`](/docs/media-buy/task-reference/create_media_buy) and [`update_media_buy`](/docs/media-buy/task-reference/update_media_buy) compatibility facades remain optional.
...
or cross-principal IDs return [`NOT_FOUND`](/docs/building/by-layer/L3/comply-test-controller#error-codes). A proposal that was never an eligible
committed hold, or whose hold ended through another transition, returns
[`INVALID_STATE`](/docs/building/by-layer/L3/comply-test-controller#error-codes).
...
[`list_products`](/docs/media-buy/task-reference/list_products),
[`buy_products`](/docs/media-buy/task-reference/buy_products),
[`control_media_buy`](/docs/media-buy/task-reference/control_media_buy), and
[`get_media_buys`](/docs/media-buy/task-reference/get_media_buys). Deprecated
[`get_products`](/docs/media-buy/task-reference/get_products), `create_media_buy`, and [`update_media_buy`](/docs/media-buy/task-reference/update_media_buy) facades are not part
of this scenario.
...
[`reporting.core`](/docs/media-buy/reporting-core-implementation-guide)
...
[stale-received grace projection](/docs/media-buy/task-reference/get_reporting_status#consumer-status-mismatch)
...
Read the waiting obligation through [`get_reporting_status`](/docs/media-buy/task-reference/get_reporting_status):
...
`force_get_products_arm` applies only to [`get_products`](/docs/media-buy/task-reference/get_products) with
...
`force_get_signals_arm` applies only to [`get_signals`](/docs/signals/tasks/get_signals) with
...
on task completion through [`get_task_status`](https://adcontextprotocol.org/schemas/v3/protocol/get-task-status-request.json) (legacy
...
Injects synthetic delivery data for a media buy. Subsequent calls to [`get_media_buy_delivery`](/docs/media-buy/task-reference/get_media_buy_delivery) MUST reflect this data. Delivery simulation is additive — each call adds to existing delivery totals.
...
After calling `simulate_budget_spend`, the seller MUST reflect the simulated consumption in [`get_account_financials`](/docs/accounts/tasks/get_account_financials). Specifically:
...
Creates a creative fixture at a specific lifecycle status. Lets governance and delivery storyboards reference a pre-approved creative without round-tripping [`sync_creatives`](/docs/creative/task-reference/sync_creatives) first.
...
[Fixture handles and catalog resolution](/docs/building/verification/conformance#fixture-handles-and-catalog-resolution).
</Note>
...
| Error code | When |
|---|---|
| [`INVALID_TRANSITION`](/docs/building/by-layer/L3/comply-test-controller#error-codes) | Requested state-machine transition is not valid (e.g., `archived → processing`, `canceled → paused`) |
| [`INVALID_STATE`](/docs/building/by-layer/L3/comply-test-controller#error-codes) | Operation is not permitted for the resource's current status (e.g., re-seeding a fixture that already exists with a diverging shape) |
| [`NOT_FOUND`](/docs/building/by-layer/L3/comply-test-controller#error-codes) | Entity does not exist or caller does not have access (multi-tenant sandboxes SHOULD treat "not yours" as "not found") |
| [`UNKNOWN_SCENARIO`](/docs/building/by-layer/L3/comply-test-controller#error-codes) | Scenario not implemented by this seller |
| [`INVALID_PARAMS`](/docs/building/by-layer/L3/comply-test-controller#error-codes) | Missing or malformed params, or precondition not met (e.g., `simulate_budget_spend` on an entity with no budget configured) |
| [`FORBIDDEN`](/docs/building/by-layer/L3/comply-test-controller#error-codes) | Production account referenced from a sandbox connection |
| [`JCS_NON_FINITE_NUMBER`](/docs/building/by-layer/L3/comply-test-controller#error-codes) | Digest-mode `query_upstream_traffic` cannot canonicalize a parsed JSON-like value tree containing `NaN`, `+Infinity`, or `-Infinity`; controllers must not coerce these values, and runners grade the affected validation `not_applicable` |
| [`INTERNAL_ERROR`](/docs/building/by-layer/L3/comply-test-controller#error-codes) | Transient seller-side failure (e.g., sandbox database unavailable). The runner SHOULD retry once before treating as a failure. |
...
Sellers that do not implement `list_scenarios` SHOULD return an error with [`UNKNOWN_SCENARIO`](/docs/building/by-layer/L3/comply-test-controller#error-codes). When this happens, the runner tries each scenario individually and treats `UNKNOWN_SCENARIO` responses as coverage gaps (not failures). This means early implementers who skip `list_scenarios` are not penalized — the runner discovers supported scenarios through trial.
...
- Tests operation gates (e.g., [`create_media_buy`](/docs/media-buy/task-reference/create_media_buy) blocked when account is `suspended`)
...
- **Transition correctly rejected** — controller returned [`INVALID_TRANSITION`](/docs/building/by-layer/L3/comply-test-controller#error-codes) for an invalid state change. This is a pass.
...
6. Test the controller's transition validation itself — invalid transitions should return [`INVALID_TRANSITION`](/docs/building/by-layer/L3/comply-test-controller#error-codes), not silently succeed
