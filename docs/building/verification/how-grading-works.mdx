...
When your agent declares a specialism in [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities), the runner:
...
**Runner behavior:** all eight `requires_scenarios` run, including `proposal_finalize`. Priya's platform is graded on the full proposal lifecycle — brief with proposals, refine, finalize, and execute via [`create_media_buy`](/docs/media-buy/task-reference/create_media_buy).
...
In a raw `StoryboardResult`, `overall_passed` combines executed-step results with [storyboard invariants](/docs/building/verification/compliance-catalog#cross-resource-invariants). A storyboard-scoped assertion can set `overall_passed: false` even when `failed_count` — and the aggregate `summary.steps_failed` count — is zero. Inspect `assertions[]` for its `assertion_id`, `scope`, optional `step_id`, and `error` or structured `hint`. Step-scoped assertion failures also appear in that step's `validations[]` with `check: "assertion"`.

The `@adcp/sdk@13.0.0-rc.15` compliance aggregate neither preserves storyboard-scoped assertion records nor reflects an `onEnd`-only assertion failure in its track status ([adcp-client#2520](https://github.com/adcontextprotocol/adcp-client/issues/2520)). Until that runner gap is fixed, programmatic consumers that need an authoritative verdict must inspect the raw `StoryboardResult.overall_passed` and `assertions[]`; do not infer a full pass from `summary.steps_failed` alone. The raw object is available to direct `runStoryboard()` callers and, for in-process agents, under `results[]` from `storyboard run --local-agent ... --json`; standard remote `storyboard run ... --json` does not expose it. Rerunning one step cannot reproduce an end-of-storyboard assertion.

Skipped steps (`skip` block present) and not-selected items (`summary.not_selected[]` entries) do not contribute to `summary.steps_failed`, but they mean different things. `summary.steps_not_selected` says the runner deliberately excluded those scenarios from this run. `summary.steps_skipped` says the runner selected those scenarios but could not execute them; use `summary.skipped_by_reason` and `skip.detail` to distinguish optional capability choices from missing required surfaces. For ordinary step validation failures, see [Storyboard troubleshooting](/docs/building/operating/storyboard-troubleshooting).
...
- [Validate Your Agent](/docs/building/verification/validate-your-agent) — running the suite locally with `@adcp/sdk`
