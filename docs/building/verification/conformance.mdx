...
|---|---|
| Test controller availability ([`comply_test_controller`](/docs/building/by-layer/L3/comply-test-controller) in `tools/list`) | "Has the seller exposed deterministic-mode forces?" |
...
|------------|------------------|
| [`capability_discovery`](/compliance/latest/universal/capability-discovery.yaml) | [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities) shape, protocol/specialism declarations, version advertising |
| [`comply_controller_mode_gate`](/compliance/latest/universal/comply-controller-mode-gate.yaml) | Sandbox/live isolation for sellers that expose [`comply_test_controller`](/docs/building/by-layer/L3/comply-test-controller) on a shared endpoint — live-mode principals must be refused before scenario dispatch |
...
| [`schema_validation_signals`](/compliance/latest/universal/schema-validation-signals.yaml) | Response schema compliance for signals — required fields on every signal; gated on [`get_signals`](/docs/signals/tasks/get_signals) |
...
| [`read_tool_idempotency`](/compliance/latest/universal/read-tool-idempotency.yaml) | Read-only task wrappers and the 3.x [`get_products`](/docs/media-buy/task-reference/get_products) compatibility facade tolerate an optional `idempotency_key` without implying read replay; verifies the keyless capabilities bootstrap |
| [`reporting_core_declaration`](/compliance/latest/universal/reporting-core-declaration.yaml) | The proper-name Reliable Reporting 1.0 declaration and required Core boundary for [`get_reporting_status`](/docs/media-buy/task-reference/get_reporting_status), graded without requiring an optional test controller |
...
| [`agent_notification_configs`](/compliance/latest/universal/agent-notification-configs.yaml) | Agent-level `capabilities.changed` registration: revision-fenced capability declaration, NO_AUTH rejection, and dry-run clear semantics for [`sync_agent_notification_configs`](/docs/protocol/sync_agent_notification_configs) |
| [`principal`](/compliance/latest/universal/principal.yaml) | Stable-principal connection configuration for [`sync_principal`](/docs/protocol/sync_principal) and [`get_principal`](/docs/protocol/get_principal): experimental capability declaration, NO_AUTH rejection, dry-run validation, applied lifecycle with readback, idempotent replay, and section clearing |
...
| [`account_identity_reconciliation`](/compliance/latest/universal/account-identity-reconciliation.yaml) | Capability-gated account identity lifecycle: mandatory revision fences, stale-write atomicity, in-place operator-unit rekeying, collision rollback, preserved account IDs, and authorized former-key [`ACCOUNT_MOVED`](/docs/building/verification/compliance-catalog#error-code-account-moved) recovery |
| [`notification_config_lifecycle`](/compliance/latest/universal/notification-config-lifecycle.yaml) | Account-level `notification_configs[]` lifecycle on [`sync_accounts`](/docs/accounts/tasks/sync_accounts): paused registration, durable [`list_accounts`](/docs/accounts/tasks/list_accounts) echo, subscriber-keyed replacement, and clear-all semantics |
...
| [`pagination_integrity`](/compliance/latest/universal/pagination-integrity.yaml) | `cursor` ↔ `has_more` invariant on paginated [`list_creatives`](/docs/creative/task-reference/list_creatives) responses, walked from a continuation page through to a terminal page |
| [`get_products_pagination_integrity`](/compliance/latest/universal/get-products-pagination-integrity.yaml) | [`get_products`](/docs/media-buy/task-reference/get_products) wholesale pagination semantics: walks a seeded product feed from continuation to terminal while preserving brief/refine as capped curated/refined answers, not feed enumeration |
...
| [`pagination_integrity_creative_formats`](/compliance/latest/universal/pagination-integrity-creative-formats.yaml) | Deprecated named-format compatibility check: `cursor` ↔ `has_more` invariant when an agent still exposes [`list_creative_formats`](/docs/creative/task-reference/list_creative_formats) |
| [`get_media_buys_pagination_integrity`](/compliance/latest/universal/get-media-buys-pagination-integrity.yaml) | `cursor` ↔ `has_more` invariant on paginated [`get_media_buys`](/docs/media-buy/task-reference/get_media_buys) responses; storyboard seeds three media buys via `seed_media_buy` and walks from a continuation page to a terminal page |
| [`pagination_integrity_content_standards`](/compliance/latest/universal/content-standards-pagination-integrity.yaml) | `cursor` ↔ `has_more` invariant on paginated [`list_content_standards`](/docs/governance/content-standards/tasks/list_content_standards) responses; storyboard bootstraps three content standards configurations via [`create_content_standards`](/docs/governance/content-standards/tasks/create_content_standards) and walks from a continuation page to a terminal page |
| [`pagination_integrity_collection_lists`](/compliance/latest/universal/collection-lists-pagination-integrity.yaml) | `cursor` ↔ `has_more` invariant on paginated [`list_collection_lists`](/docs/governance/collection/tasks/collection_lists#list_collection_lists) responses; storyboard bootstraps three collection lists via [`create_collection_list`](/docs/governance/collection/tasks/collection_lists#create_collection_list) and walks from a continuation page to a terminal page |
| [`pagination_integrity_property_lists`](/compliance/latest/universal/property-lists-pagination-integrity.yaml) | `cursor` ↔ `has_more` invariant on paginated [`list_property_lists`](/docs/governance/property/tasks/property_lists#list_property_lists) responses; storyboard bootstraps three property lists via [`create_property_list`](/docs/governance/property/tasks/property_lists#create_property_list) and walks from a continuation page to a terminal page |
...
| [`billing_gate_dispatch`](/compliance/latest/universal/billing-gate-dispatch.yaml) | Two-gate dispatch on `sync_accounts.billing` rejection: seller-wide capability gate ([`BILLING_NOT_SUPPORTED`](/docs/building/verification/compliance-catalog#error-code-billing-not-supported) with `error.details.scope`) vs per-buyer-agent commercial-relationship gate ([`BILLING_NOT_PERMITTED_FOR_AGENT`](/docs/building/verification/compliance-catalog#error-code-billing-not-permitted-for-agent) with the clamped `rejected_billing` + optional `suggested_billing` shape). Capability phase skipped when the seller supports all three `billing` values; per-agent phases skipped when the test kit does not declare `commercial_relationship: passthrough_only`. |
...
- **Incident response** — AdCP emits the signals worth watching ([`IDEMPOTENCY_CONFLICT`](/docs/building/verification/compliance-catalog#error-code-idempotency-conflict) spikes, failed governance verifications, SSRF rejections); detection, alert routing, and response are operator concerns.
...
2. External certification does not imply AdCP conformance. SOC 2 Type II says nothing about whether [`create_media_buy`](/docs/media-buy/task-reference/create_media_buy) responses validate.

## How to claim conformance

1. Declare `supported_protocols` and `specialisms` in [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities).
...
- **Duplicate schemas and storyboards as per-task vectors.** The [Reference Test Vectors index](/docs/reference/test-vectors) catalogs byte-sensitive and library-level fixtures. Per-task shapes remain owned by JSON Schema, while task behavior remains owned by storyboards; a parallel frozen request/response corpus would create a third authority. SDK authors can derive local fixtures from the versioned storyboard when they need isolated unit tests.
