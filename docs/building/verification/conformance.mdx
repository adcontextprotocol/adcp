...
| [`premium_display_canonical_validation`](https://adcontextprotocol.org/compliance/latest/universal/premium-display-canonical-validation) | AdCP 3.2 premium-display conformance: exact multi-state canvas coverage plus page-takeover component resolution, namespaced asset coverage, and shared-slot references. |
...
| [`oauth_setup`](https://adcontextprotocol.org/compliance/latest/universal/oauth-setup) | Complete RFC 9728/RFC 8414 discovery graph, issuer/resource consistency, and passive endpoint reachability — skipped unless `oauth.supported: true` |
...
| [`agent_notification_configs`](https://adcontextprotocol.org/compliance/latest/universal/agent-notification-configs) | Agent-level `capabilities.changed` registration: revision-fenced capability declaration, NO_AUTH rejection, and dry-run clear semantics for `sync_agent_notification_configs` |
...
| [`pagination_integrity_creative_formats`](https://adcontextprotocol.org/compliance/latest/universal/pagination-integrity-creative-formats) | Deprecated named-format compatibility check: `cursor` ↔ `has_more` invariant when an agent still exposes `list_creative_formats` |
