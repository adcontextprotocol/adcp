...
<Note>
**Coming soon.** `sales-streaming-tv` (CTV / streaming) and `sales-exchange` (programmatic SSP / exchange) are scheduled for a future release. Sellers in those categories should claim `sales-guaranteed` or `sales-non-guaranteed` until the category-specific storyboards ship.
</Note>
