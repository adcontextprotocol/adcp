...
| `impairment.coherence` | Cross-resource — verifies that `media_buy.impairments[]` stays in sync with referenced resources. Forward: every entry references a currently-offline resource. Inverse: any offline resource referenced by a non-terminal buy MUST appear in `impairments[]`, and `health == "impaired"` iff non-empty. Out of scope: terminal-status buys; materiality (schema-enforced via `package_ids: minItems: 1`). | Buy-side specialisms today: `audience-sync`, `sales-catalog-driven`. Creative-track wiring (`creative-ad-server`, `creative-template`, `creative-generative`) is deferred until the training-agent runner registers the assertion id (same rollout as `status.monotonic` in [#2664](https://github.com/adcontextprotocol/adcp/issues/2664)). Grades `not_applicable` until the cross-resource exercise lands in [#2860](https://github.com/adcontextprotocol/adcp/issues/2860). |
