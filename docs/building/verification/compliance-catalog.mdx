...
Every AdCP agent declares its `supported_protocols` and `specialisms` in [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities). Each declaration maps to a compliance bundle at `/compliance/{version}/` that the storyboard runner executes to verify the claim.

<Note>
**`supported_protocols` is not exhaustive.** The `accounts` surface ([`sync_accounts`](/docs/accounts/tasks/sync_accounts), [`list_accounts`](/docs/accounts/tasks/list_accounts), [`sync_governance`](/docs/accounts/tasks/sync_governance)) is a foundation implicit in every `media_buy`, `creative`, and `signals` agent and is intentionally not a `supported_protocols` value. See [Accounts tasks](/docs/accounts/tasks/sync_accounts) for the full account surface.
</Note>
...
<Note>
Support for the [compliance test controller](/docs/building/by-layer/L3/comply-test-controller) is declared via the `capabilities.compliance_testing` block on [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities), not via `supported_protocols`. Compliance testing is an RPC surface for the test harness, not a functional protocol.
</Note>
...
<Note>
**Coming soon.** `sales-exchange` (programmatic SSP / exchange) is scheduled for a future release. Sellers in that category should claim `sales-guaranteed` or `sales-non-guaranteed` until the category-specific storyboard ships.
</Note>
...
| Specialism | Status | Purpose |
|-----------|--------|---------|
| `signal-owned` | stable | Owned signal agent exposing first-party segments through [`get_signals`](/docs/signals/tasks/get_signals); activation is not required for this claim |
| `signal-marketplace` | stable | Marketplace signal agent reselling third-party data; requires `get_signals` and [`activate_signal`](/docs/signals/tasks/activate_signal) |
...
<Warning>
**`sales-proposal-mode` is deprecated in 3.1.** Do not claim it for new agents. Existing agents that declare it must drop it entirely and replace it with `sales-guaranteed` + `media_buy.supports_proposals: true` in [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities). See [#3823](https://github.com/adcontextprotocol/adcp/issues/3823).
</Warning>
...
Declare your protocols and specialisms in [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities):
...
|--------|-------|---------|------------------|
| `snake_case` | Wire enums (`supported_protocols`, `delivery_type`, channel IDs, `signal_type`) | `media_buy`, `non_guaranteed`, `ctv`, `custom` | [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities) response, JSON payloads, generated schemas |
...
| `governance-spend-authority` | [`check_governance`](/docs/governance/campaign/tasks/check_governance), [`sync_plans`](/docs/governance/campaign/tasks/sync_plans) | [`governance_spend_authority`](/compliance/latest/specialisms/governance-spend-authority/index.yaml) | [`governance_spend_authority/denied`](/compliance/latest/specialisms/governance-spend-authority/denied.yaml) |
| `creative-generative` | [`build_creative`](/docs/creative/task-reference/build_creative) | [`creative_generative`](/compliance/latest/specialisms/creative-generative/index.yaml) | [`creative_generative/seller`](/compliance/latest/specialisms/creative-generative/generative-seller.yaml) |
| `brand-rights` | [`get_brand_identity`](/docs/brand-protocol/tasks/get_brand_identity), [`acquire_rights`](/docs/brand-protocol/tasks/acquire_rights) | [`brand_rights`](/compliance/latest/specialisms/brand-rights/index.yaml) | [`brand_rights/governance_denied`](/compliance/latest/specialisms/brand-rights/scenarios/governance_denied.yaml) |
...
- `property-lists` (specialism ID, kebab-case) maps to the `property_list` tool family ([`create_property_list`](/docs/governance/property/tasks/property_lists#create_property_list), [`validate_property_delivery`](/docs/governance/property/tasks/validate_property_delivery)) and storyboard category `property_lists`.
