...
| **[`comply_test_controller`](/docs/building/by-layer/L3/comply-test-controller) scenarios** | Protocol-level tool operations a seller exposes (`force_*`, `simulate_*`, `seed_*`) so storyboards can drive deterministic state. | The `scenario` parameter of the seller's `comply_test_controller` MCP tool. | ✅ Yes |
...
- The [`comply_test_controller`](/docs/building/by-layer/L3/comply-test-controller) operations the runner uses to seed deterministic state

[`conformance.mdx`](/docs/building/verification/conformance) calls these *"the truth"* — and means it. These YAML files are the basis for **AgenticAdvertising.org Verified (Spec)**; AgenticAdvertising.org issues that qualifier when its compliance heartbeat runs the applicable storyboards and every assertion passes.
...
A seller responding to [`comply_test_controller`](/docs/building/by-layer/L3/comply-test-controller) with a `scenario` parameter | A protocol-level test-control operation         | ✅ Yes (the contract is normative; the seller MAY refuse with [`UNKNOWN_SCENARIO`](/docs/building/by-layer/L3/comply-test-controller#error-codes))
