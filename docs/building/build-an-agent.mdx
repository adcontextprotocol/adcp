...
Storyboard passing issues the **AdCP Conformant** mark — validated against seeded test data. Once your agent is running against real production inventory, consider enrolling in **[AAO Verified](/docs/building/aao-verified)**, which adds continuous observability of real delivery on a dedicated compliance account. AAO Verified is optional and strictly implies AdCP Conformant; enterprise buyers that treat AdCP as production infrastructure filter on it.
