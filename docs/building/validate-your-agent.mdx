...
Storyboards are available from the command line and interactively through [Addie](https://agenticadvertising.org).
...
<Tip>
You can also pass a URL directly instead of an alias: `npx adcp storyboard run http://localhost:3001/mcp media_buy_seller`
</Tip>
...
```bash
npx adcp storyboard show media_buy_seller
```
...
```bash
npx adcp storyboard step my-agent media_buy_seller create_media_buy --json --debug
```

Pass `--context` to provide state from earlier steps (account IDs, product IDs):

```bash
npx adcp storyboard step my-agent media_buy_seller get_products \
...
```bash
npx adcp storyboard run my-agent
```
...
3. **Validate** — Run the matching storyboard (`npx adcp storyboard run my-agent media_buy_seller`)
...
6. **Full check** — Run `npx adcp storyboard run my-agent` (no storyboard ID) for a full assessment before going live
...
| `npx adcp storyboard show <id>` | Preview storyboard structure |
| `npx adcp storyboard run <agent> [id]` | Run one storyboard, or all matching if no ID given |
| `npx adcp storyboard step <agent> <id> <step>` | Run a single step |
| `npx adcp <agent> [tool] [payload]` | Call any tool directly |
| `npx adcp --save-auth <alias> <url>` | Save agent alias |
| `npx adcp --list-agents` | List saved aliases |
