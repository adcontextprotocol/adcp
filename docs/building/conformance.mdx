...
- **Verified** — AAO has tested the agent recently and issued a signed attestation. Gated on active membership and a live heartbeat. The [AAO Verified badge](/docs/building/aao-verified) carries one of two qualifiers: **(Spec)** for storyboard-conformance against a test-mode endpoint, **(Live)** for AAO-observed real production traffic via canonical campaigns. An agent can earn either or both.
...
The rest of this page describes **AdCP Conformant** — the agent's wire behavior matches the spec, verified against the storyboard suite with seeded test data. Storyboard passing issues AdCP Conformant; storyboard regression revokes it.

AdCP 3.x also defines **AAO Verified** (optional), a stricter mark for agents whose live ad-server integration AAO has directly observed. AAO Verified adds continuous observability of real delivery data against the seller's live endpoint, performed by the AAO compliance engine over a 7–14 day rolling window. It closes the gap storyboards cannot: a seller can pass every storyboard with a broken ad-server integration, because `simulate_delivery` is a parallel code path from the production reporting path. AAO Verified requires that the AdCP surface actually reaches real impressions on real inventory.

AAO Verified is opt-in, requires no new infrastructure (the AAO compliance engine already runs), and produces an auto-expiring mark distinct from AdCP Conformant. See [AAO Verified](/docs/building/aao-verified) for the full spec.
...
- **[Storyboard troubleshooting](/docs/building/implementation/storyboard-troubleshooting)** — Error pattern → root cause → fix for the most common storyboard failures
- **[Known spec ambiguities](/docs/building/implementation/known-ambiguities)** — Open spec gaps with workarounds and issue links; entries are removed as underlying issues close

## Further reading

- **[AAO Verified](/docs/building/aao-verified)** — Continuous-observability verification of the seller's live ad-server integration
