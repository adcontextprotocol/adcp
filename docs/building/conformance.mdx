...
The two qualifiers share one brand mark — **AAO Verified** — and an agent can earn either or both. (Spec) is a prerequisite for (Live) because a broken protocol implementation makes live observation unmeasurable. See [AAO Verified](/docs/building/aao-verified) for the full qualifier model and the (Live) spec; the rest of this page indexes the storyboards that back (Spec).
