...
Most chat-based MCP clients (Claude Desktop, Cursor) do not yet support MCP Tasks. If your client doesn't support task-augmented tool calls, use **webhooks** or **polling via `tasks/get`** instead — these work with any MCP client. See [Async Operations](/docs/building/implementation/async-operations) and [Push Notifications](/docs/building/implementation/webhooks) for transport-independent patterns.
...
See [Push Notifications](/docs/building/implementation/webhooks) for payload formats and authentication.
...
:::note
The `adcp_version` string in server card metadata is a static discovery hint. For runtime version negotiation, use the `adcp_major_version` integer field on [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities#version-negotiation) and subsequent requests. The major version component of `adcp_version` (e.g., `2` from `"2.6.0"`) corresponds to `adcp_major_version`.
:::
