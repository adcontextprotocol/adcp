...
| Field | Description |
|-------|-------------|
| `account_id` | Vendor agent's identifier for this account. Pass this to protocol tasks (`create_media_buy`, `get_signals`, `activate_signal`, `report_usage`, etc.). |
...
- [list_accounts](/docs/building/integration/list_accounts) — Poll for status changes on pending accounts
