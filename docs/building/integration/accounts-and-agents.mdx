...
When `true`: each operator must authenticate with the seller directly (via OAuth or API key). The agent obtains a credential per operator.
...
Seller checks `unilever.com/.well-known/brand.json`, finds Mindshare in `authorized_operators`, and fast-tracks provisioning:
...
**sync_accounts example — agent requests brand billing, seller maps to agent:**
...
Seller doesn't support direct billing for this brand and maps to agent billing:
