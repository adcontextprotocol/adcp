...
No `account_id` yet — the seller has acknowledged the relationship `(house: "acme-corp.com", billing: "brand")` but hasn't provisioned an account. A human at Acme Corp completes the setup at the URL. To check progress, the agent either:
- Re-calls `sync_accounts` with the same natural key — the seller returns the updated status (and `account_id` once provisioned)
- Receives a webhook notification if `push_notification_config` was provided in the request

Once provisioned, the seller assigns an `account_id` and the account appears in `list_accounts`. Some sellers may assign an `account_id` immediately — `account_id` is optional in `sync_accounts` responses but required in `list_accounts`.
