...
Once provisioned, the seller assigns an `account_id` and the account appears in `list_accounts`. `account_id` is present in all `sync_accounts` responses except when `status` is `rejected` or `action` is `failed`. It is always present in `list_accounts`.
