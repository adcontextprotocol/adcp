...
**Account** — A billing relationship between a buyer and seller. Determines rate card, payment terms, credit limit, and who receives invoices. Every billable operation requires an `account_id`.
...
Seller responds — registration required before provisioning:
...
No `account_id` yet — the seller hasn't provisioned the account. A human at Acme Corp completes the setup at the URL. The agent either:
- Polls `list_accounts` until the account appears with status `active` and an `account_id`, or
- Receives a webhook notification if `push_notification_config` was provided in the request

Once approved, the seller assigns the `account_id` and the account becomes usable.

<Note>
`account_id` is optional in `sync_accounts` responses — sellers may not assign one until setup is complete. However, `account_id` is **required** in `list_accounts` responses (per the [Account schema](/docs/accounts/tasks/list_accounts)). This means an account without an `account_id` won't appear in `list_accounts` until the seller provisions it. Use `push_notification_config` in the sync request to get notified when that happens.
</Note>
