...
**Account** — A billing relationship between a buyer and seller. Determines rate card, payment terms, credit limit, and who receives invoices. Every billable operation requires an account reference — either a seller-assigned `account_id` or a natural key (`brand`, `operator`, optional `sandbox`).
...
When `false` (default) — **implicit accounts**: the seller trusts the agent. The agent authenticates once and declares accounts via `sync_accounts`.

When `true` — **explicit accounts**: each operator must authenticate with the seller directly. The agent obtains a credential per operator — via OAuth using the seller's `authorization_endpoint`, or via API key out-of-band. The buyer discovers accounts via `list_accounts`.

Regardless of account model, subsequent requests can use either a natural key (`brand` + `operator` + optional `sandbox`) or a seller-assigned `account_id`. The seller resolves the reference internally.
...
3. For subsequent requests, pass the account reference:
   - Pass the natural key: `{ "brand": { "domain": "acme-corp.com" }, "operator": "pinnacle-media.com" }`
   - Or a seller-assigned `account_id`: `{ "account_id": "acc_acme_001" }` (discover via `list_accounts`)
...
`sync_accounts` does not return `account_id` — the seller manages account identifiers internally. On subsequent requests, pass either a natural key (`brand` + `operator` + optional `sandbox`) or a seller-assigned `account_id` from `list_accounts`. Both forms work regardless of account model.
