...
Seller provisions the account but requires setup before activation:
...
The account exists with an `account_id` but can't be used for buys yet. A human at Acme Corp completes the setup at the URL. The agent polls `list_accounts` (filtering by `status: "pending_approval"` or `status: "active"`) until the account transitions to `active`.

**Key point:** `pending_approval` is the normal path. Every buyer needs a direct relationship with the seller. The seller always assigns an `account_id` on creation — even pending accounts are trackable via `list_accounts`.
