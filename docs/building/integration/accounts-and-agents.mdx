...
When a sub-account is created with `billing: "agent"`, the agent receives a single consolidated invoice from the seller and is responsible for re-billing individual brands and operators on its side.
