...
For sandbox operations, use the natural key with `sandbox: true` regardless of account model — no account provisioning or discovery needed.
...
3. For subsequent requests, pass the account reference:
   ...
   - **Sandbox** (either model): pass the natural key with `sandbox: true`
...
`sync_accounts` does not return `account_id` — the seller manages account identifiers internally. For explicit accounts (`require_operator_auth: true`), discover account IDs via `list_accounts`. For implicit accounts (`require_operator_auth: false`), use natural keys (`brand` + `operator`) on subsequent requests. For sandbox operations, use the natural key with `sandbox: true` regardless of account model.
