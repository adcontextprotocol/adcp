...
When `default_billing: "agent"`, the seller maintains a pre-provisioned **agent account** — a standing account with no brand or operator association. It acts as the fallback: brands or operators without a direct account on the seller route through it automatically. Agents don't need to call `sync_accounts` to use it.
