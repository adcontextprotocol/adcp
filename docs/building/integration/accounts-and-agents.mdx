...
<Note>
`account_id` is optional in `pending_approval` responses. Some sellers provision the account immediately and return an `account_id` while approval is pending — others wait until setup is complete. Agents should handle both cases.
</Note>
