...
| Field | Description |
|-------|-------------|
| `setup_modes` | How accounts can be established: `sync_accounts`, `oauth`, `auto_provision` |
...
**`sync_accounts`** — Agent provisions accounts programmatically. The agent declares which brands it represents and who should be invoiced. The seller provisions accounts accordingly. This is the primary mechanism for establishing accounts at scale.

**`oauth`** — Buyer connects an existing account. For sellers where the buyer already has a direct account (social platforms, self-serve platforms), the agent connects via OAuth. After OAuth, `list_accounts` returns the buyer's existing accounts.

**`auto_provision`** — Seller creates account on first buy. Some sellers create accounts automatically on the first billable operation, using standard terms. The agent doesn't need to set up accounts in advance.

### Agent Workflow

```
1. get_adcp_capabilities → learn setup_modes, supported_billing, required_for_products
2. If sync_accounts in setup_modes → sync_accounts (declare brands, operators, billing)
   If oauth in setup_modes → OAuth flow, then list_accounts
3. Wait for account status = active (poll list_accounts if pending)
4. get_products with account_id (required if required_for_products = true)
...
When an agent syncs an account with `operator: "mindshare.com"` for brand `house: "unilever.com"`, `brand_id: "dove"`, the seller can:

1. Resolve `unilever.com/.well-known/brand.json`
2. Check `authorized_operators` for `domain: "mindshare.com"` with `"dove"` in `brands`
3. If found → proceed (account may still need credit approval)
4. If not found → return `pending_authorization` with a setup URL for Unilever to approve

If `authorized_operators` is not present in brand.json, the seller falls back to `pending_authorization` for any third-party operator.
...
Billable operations require `account_id` when accounts are established via `sync_accounts` or `oauth`. The agent gets the `account_id` from the sync response (or `list_accounts` for OAuth-connected accounts):
...
For sellers that support `auto_provision`, the agent may omit `account_id` on the first operation — the seller creates an account automatically and returns the `account_id` in the response.
...
| Account capabilities | `account` section in capabilities | `setup_modes`, `supported_billing`, `required_for_products` |
