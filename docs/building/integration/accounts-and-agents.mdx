...
An account may require multiple steps before becoming active (e.g., operator authorization *and* credit approval). The `setup.url` points to the seller's workflow, which handles multi-step processes internally. The account status reflects the current blocker — it may transition from `pending_authorization` to `pending_approval` to `active` as each step completes. The agent polls `list_accounts` (or uses push notifications) to track progress through the seller's setup flow.
