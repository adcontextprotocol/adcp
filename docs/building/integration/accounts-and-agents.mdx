...
|-----------|-------|-------------|
| `account_id` | Explicit account IDs | Accounts are managed out-of-band (advertiser portal, sales rep). Buyer discovers them via `list_accounts` and passes a seller-assigned `account_id`. |
| `natural_key` | Implicit account IDs | Buyer declares intent via `sync_accounts` — who's advertising, who's paying — and the seller provisions accounts. On subsequent requests, the buyer passes brand identity (`house`, `brand_id`, `operator`) and the seller resolves internally. |

Default: `["account_id"]`. See [Account references](#account-references) for detailed workflows.
