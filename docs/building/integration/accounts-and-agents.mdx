...
**Brand** — The advertiser whose products or services are promoted. Identified by house domain + brand_id, resolved via `/.well-known/brand.json`. Single-brand houses use the domain alone (no `brand_id`).

**Account** — A billing relationship between a buyer and seller. Determines rate card, payment terms, credit limit, and who receives invoices. Every billable operation requires an account reference — either an `account_id` or a natural key (`house`, `brand_id`, `operator`).
...
|-----------|-------------|
| `account_id` | Buyer passes a seller-assigned `account_id` (from `sync_accounts` or `list_accounts`) |
| `natural_key` | Seller resolves from the natural key (`house`, `brand_id`, `operator`) — `sync_accounts` may be optional |

Default: `["account_id"]`. When `natural_key` is supported, buyers can skip `sync_accounts` and pass brand identity directly in every request.
...
{
  "account": {
    "account_resolution": ["account_id", "natural_key"],
...
|---------|------------------------|--------------------|--------------------|
| Operator-gated | `true` | `["brand", "operator"]` | — |
| Direct-only | `false` | `["brand", "operator"]` | — |
| Agent-consolidated | `false` | `["agent"]` | `"agent"` |
| Hybrid | `false` | `["brand", "operator", "agent"]` | `"agent"` |
...
The seller has assigned an `account_id` and acknowledged the relationship `(house: "acme-corp.com", billing: "brand")`, but the account is pending review before it becomes active. A human at Acme Corp completes the setup at the URL. To check progress, the agent either:
...
Every account-scoped operation accepts an `account` object instead of a flat `account_id` string. The seller's `account_resolution` capability declares which mode it supports — and the mode determines the buyer's entire integration path.

### By account ID

The seller already has accounts provisioned. The buyer discovers them via `list_accounts`, picks the right `account_id`, and passes it on every request.

**Workflow:**

1. Call `list_accounts` to discover existing accounts
2. Pick the `account_id` that matches the brand/operator you're buying for
3. Pass `{ "account_id": "acc_acme_001" }` on every request (`get_products`, `create_media_buy`, etc.)

**When to expect this:** Sellers where accounts are pre-provisioned — through a portal, a sales rep, or an earlier `sync_accounts` call. The buyer already has a relationship; they just need to look up the identifier.

### By natural key

The buyer doesn't need to know account IDs. Instead, they declare intent — who they are, what they want — and the seller figures out the account.
...
|------|-------------------------|
| `house` + `brand_id` | Which brand is advertising |
| `operator` | Who operates the seat (agency, trading desk) |
| `billing` | Who gets the invoice — `brand`, `operator`, or `agent` |
...
1. Call `sync_accounts` with one or more declarations
2. Seller provisions or links accounts for each, returns status and `account_id` values
3. For subsequent requests, pass either:
   - `{ "account_id": "acc_acme_001" }` — using the ID the seller returned
   - `{ "house": "acme-corp.com", "brand_id": "bolt", "operator": "pinnacle-media.com" }` — if the seller supports `natural_key` resolution
4. When anything changes (billing model, new brand, new operator), call `sync_accounts` again

**When to expect this:** Most integrations. The buyer declares what they need; the seller decides how to provision it. Sellers that support `natural_key` in `account_resolution` also let buyers skip `sync_accounts` entirely and pass brand identity directly on every request — the seller resolves the account internally.

### Natural key semantics

The tuple `(house, brand_id, operator)` uniquely identifies an account relationship. Missing fields are distinct from present fields — `{house: "acme-corp.com"}` (brand buying direct) is a different account from `{house: "acme-corp.com", operator: "pinnacle-media.com"}` (brand via agency).
...
The agent requests accounts by natural key — `(house, brand_id, operator)`. The seller decides what granularity to assign. The `account_scope` field in the response tells the agent how the seller resolved the request:
...
The agent does not choose the scope — the seller assigns it based on its own account policy. An agent requesting `(house: "acme-corp.com", operator: "pinnacle-media.com")` might receive an operator-scoped account, a brand-scoped account, or a dedicated operator_brand account depending on the seller.
...
When an agent declares `(house: "acme-corp.com", operator: "pinnacle-media.com")`, the seller looks up or creates records in its own system — CRM, OMS, ad server, or billing platform. The `account_id` returned is the seller's internal identifier for that relationship.
