...
For sandbox operations, pass `sandbox: true` in the account reference — no account provisioning or discovery needed.
...
Every combination of flags that might require the seller to do something different — bill a different entity, set up a different rate card — is a distinct declaration.
...
3. For subsequent requests, pass the account reference:
   ...
   - **Sandbox** (either model): pass the natural key with `sandbox: true` in the account reference — no `sync_accounts` needed
...
`sync_accounts` does not return `account_id` — the seller manages account identifiers internally. For explicit accounts (`require_operator_auth: true`), discover account IDs via `list_accounts`. For implicit accounts (`require_operator_auth: false`), use natural keys (`brand` + `operator`) on subsequent requests. For sandbox operations, pass `sandbox: true` in the account reference — no sync needed.
