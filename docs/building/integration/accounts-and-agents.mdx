...
| Field | Description |
|-------|-------------|
| `setup_modes` | How accounts can be established: `sync_accounts`, `oauth` |
| `authorization_endpoint` | OAuth URL for obtaining operator credentials. Required when `oauth` is in `setup_modes`. |
...
The two setup modes represent different trust models — specifically, who authenticates to the seller.

**`sync_accounts`** (agent-trusted) — The seller trusts the agent's identity claims. The agent authenticates once with its own bearer token, then calls `sync_accounts` to declare which brands and operators it represents. The seller provisions accounts based on the agent's claims, optionally verifying operators against `brand.json`. All subsequent calls use the agent's single credential.

**`oauth`** (operator-scoped) — The seller requires operator-level credentials. Each operator must authenticate independently with the seller via OAuth (using `authorization_endpoint` from capabilities). The agent obtains a bearer token per operator, opens a separate session per operator, and calls `sync_accounts` within each session to declare brands. The operator's token — not the agent's — authorizes all calls in that session.

A seller can support both modes. For example, a publisher might trust established agent platforms (`sync_accounts`) while requiring OAuth for operators it hasn't vetted.

### Agent Workflow

**For `sync_accounts` (agent-trusted):**
```
1. get_adcp_capabilities → learn setup_modes, supported_billing, required_for_products
...
**For `oauth` (operator-scoped):**
```
1. get_adcp_capabilities → learn setup_modes, authorization_endpoint
2. For each operator:
   a. Direct operator to authorization_endpoint to authenticate with seller
   b. Obtain operator's bearer token
   c. Open a new session with the operator's token
   d. sync_accounts → declare brands for this operator
   e. list_accounts → discover existing accounts
...
All AdCP sessions use OAuth 2.0 bearer tokens. Whose token it is depends on the seller's setup mode:

**Agent-trusted (`sync_accounts` mode):** The bearer token identifies the agent. The agent holds a single credential and operates on behalf of all its brands and operators. Account access is scoped by `account_id` on each request.

**Operator-scoped (`oauth` mode):** The bearer token identifies the operator. The agent obtains a separate token per operator via the seller's `authorization_endpoint`, then opens a dedicated session per operator. All calls within that session are authorized by the operator's credential.

```
Authorization: Bearer <token>
```

In both models, the protocol does not mandate a specific OAuth flow. Sellers implement authentication according to their existing systems. The `authorization_endpoint` in capabilities provides the entry point for the `oauth` setup mode.
...
| Account capabilities | `account` section in capabilities | `setup_modes`, `authorization_endpoint`, `supported_billing`, `required_for_products` |
