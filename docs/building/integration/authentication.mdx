...
For schema definitions, see [`account.json`](https://adcontextprotocol.org/schemas/v3/core/account.json) and [`agent.json`](https://adcontextprotocol.org/schemas/v3/core/agent.json).
...
See [Testing & Development Guide](/docs/media-buy/advanced-topics/testing) for complete testing capabilities including dry run mode and time simulation.