...
- **MCP**: Requires manual context_id management (buyers) or session ID fallback (agents)

## Agent-Side Context Handling

When implementing an AdCP agent, you may receive requests without an explicit `context_id`. This is common with simple MCP clients like ChatGPT and Claude that don't manage context themselves.

### Using Transport Session ID as Fallback

AdCP agents should use the transport's session ID as a fallback when clients don't pass an explicit `context_id`. This enables automatic session-based persistence for simple clients while preserving explicit control for advanced use cases.

**Why this matters**: When `get_products` generates customized products with targeting baked in, those products need to be stored so `create_media_buy` can look them up later. Without a `context_id`, the agent can't associate related operations.

### Implementation Pattern

The MCP SDK provides `sessionId` via the `extra` parameter in tool handlers:
...
| Source | Use Case | Behavior |
|--------|----------|----------|
| **Explicit `context_id`** | Advanced clients, resuming sessions | Buyer controls context; can span multiple sessions |
| **MCP `sessionId`** | Simple clients (ChatGPT, Claude) | Automatic; zero configuration; resets on reconnect |

### Benefits

1. **Zero-friction for simple clients**: ChatGPT and Claude get automatic session persistence without any configuration
2. **Full control for advanced buyers**: Buyers can explicitly manage context across sessions when needed
3. **Resumable workflows**: Buyers can return later with the same `context_id` to resume a conversation

### Implementation Notes

- MCP SDK provides `sessionId` via `extra` parameter in tool handlers
- Consider a 30-day TTL cleanup for stored contexts to prevent unbounded growth
- For a working implementation of this pattern, see the [Snap AdCP Agent](https://github.com/scope3data/snap-adcp)
...
### For MCP (Agents)
- Use transport session ID as fallback when `context_id` is not provided
- Accept explicit `context_id` to enable advanced use cases like resumable sessions
- Store context-specific state (products, filters) keyed by context ID
- Implement TTL cleanup for stored contexts
