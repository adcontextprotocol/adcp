...
Declare the domains and specialisms your agent supports in `get_adcp_capabilities` — the compliance runner executes the matching bundles to verify. See the full [Compliance Catalog](/docs/building/compliance-catalog) for every domain and specialism an agent can claim.
