...
adcp-{version}/
  ...
  compliance/             # domains/, specialisms/, universal/, test-kits/, index.json
...
| `/compliance/{version}/domains/{domain}/` | Baseline required to claim a domain (`media-buy`, `creative`, `signals`, `governance`, `brand`, `sponsored-intelligence`) |
| `/compliance/{version}/specialisms/{id}/` | Optional specialization claims (e.g. `sales-guaranteed`, `sales-broadcast-tv`) |
| `/compliance/{version}/index.json` | Enumerates available domains, specialisms, and universal storyboards |

Declare `supported_protocols` (for domain baselines) and `specialisms` (for narrow capability claims) in `get_adcp_capabilities` — the compliance runner executes the matching bundles to verify. See the full [Compliance Catalog](/docs/building/compliance-catalog) for every protocol and specialism an agent can claim.
