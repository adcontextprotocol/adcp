...
- **Shared-governance-agent supply chain.** A governance agent often signs for many brands from a single origin. Its compromise is a multi-tenant breach. The JWKS / revocation-list requirements in the [governance profile](/docs/building/implementation/security#signed-governance-context) limit the blast radius and make rotation observable, but the buyer's due-diligence posture toward its governance agent is a real-world security dependency — treat the governance agent as a processor with multi-customer blast radius and assess it accordingly. Any operator hosting agents on behalf of more than one principal — a governance agent serving multiple brands, a buyer agent serving multiple advertisers, a sales agent serving multiple publishers — MUST scope signing keys per principal rather than reuse one key across the fleet. Concretely, each `keyid` MUST bind to a single principal using a convention such as `{operator}:{principal}:{key_version}` (or equivalent tuple that makes the owning principal unambiguous from the `keyid` alone), so a single compromised key reduces to a single-principal breach and revocation is granular. Operators SHOULD advertise this isolation in their capability surface as `identity.per_principal_key_isolation: true` so counterparties can verify the property without reading out the JWKS by hand.
...
- **Non-repudiation.** Because the governance attestation is signed and public-key-verifiable, a buyer cannot later claim a plan was never approved, and a seller cannot later claim an approval was never received.
...
The [AgenticAdvertising.org registry](/docs/registry/index) already resolves brand identity, property authorization, and agent discovery, and can cache the `signing_keys[]` declared in a publisher's `adagents.json`. But it does not today operate as a **key-transparency anchor**: there is no enrollment ceremony binding a domain to a root verification key, no append-only rotation log, and no cryptographic commitment that every verifier sees the same key history. A silent key swap at the counterparty origin still propagates into the registry's cache.
...
The controls above assume the verifier already knows *some* honest key material for each counterparty; they defend against silent swaps thereafter. That assumption begs a separate question: where does the first trustworthy key come from? In 3.x the answer is TOFU-with-continuity — the verifier caches whatever the counterparty serves at first contact and enforces rotation discipline against that cached baseline. An attacker who swaps keys before any verifier has cached anything wins silently; that is the honest 3.x ceiling and the reason this section sits inside the key-discovery gap rather than outside it.

The 4.0 mechanism closes the bootstrap gap by shipping a registry-anchored trust root and verifying it out-of-band before the first counterparty contact:

1. **Pinned root-key certificate.** Official SDK releases carry a pinned registry-operator root-key certificate compiled into the SDK. The root key is long-lived, rotated via new SDK releases, and independent of any per-counterparty infrastructure. Shipping it with the SDK means the verifier never has to bootstrap trust over the same network path it is trying to protect.
2. **Signed trust manifest at a well-known URL.** The registry publishes a signed trust manifest at `https://registry.agenticadvertising.org/.well-known/adcp-trust-root.jws` listing the current top-level trust anchors (registry signing keys, rotation history, delegation records). The manifest is a JWS signed by the pinned root key in step 1, so any response tampering is detectable without trusting the serving origin.
3. **SDK-first-run verification and cache.** On first run, the SDK MUST verify the signed manifest against its pinned root key, persist the result in a local cache with an explicit expiry, and refuse to proceed on signature failure. Subsequent runs refresh the cache on a bounded cadence and treat cache-expiry-with-no-refresh as fail-closed rather than silently continuing on stale data.

This is a 4.0 deliverable, not a 3.x requirement. It is described here so implementers building onto the 3.x attestation surfaces — `brand.json` `agents[].signing_keys[]`, signed governance JWS, pinned `signing_keys` in `adagents.json` — can shape their key-declaration conventions so a later registry-anchored bootstrap can verify them without protocol breakage. Until 4.0 lands, 3.x verifiers MUST rely on TOFU-with-continuity plus the multi-source controls above; the bootstrap gap is the named residual risk, not a hidden one.

### What AdCP 4.0 needs: key-transparency anchoring in the registry

The permanent fix extends the existing [AgenticAdvertising.org registry](/docs/registry/index) with a key-transparency layer analogous in spirit to Certificate Transparency for TLS or `sellers.json` for the ad-tech identity layer. The registry already indexes the entities whose keys need anchoring — publishers, governance agents, sales agents. What it does not yet do is commit to a cryptographic history of each party's keys. The minimal protocol-relevant properties of that extension:
...
4. **Governance-neutral operation.** The registry continues to be operated by an industry body with published governance, adds documented key-ceremony transparency for the registry's own signing keys, and has a succession plan independent of any single vendor.
...
This is **explicitly not a 3.x requirement.** It is logged as a 4.0 track so implementers who build the in-protocol attestation surfaces today — `brand.json` `agents[].signing_keys[]`, `authoritative_location`, signed governance JWS — can shape their data so a later registry-anchored lookup can verify it without protocol breakage. Specifically, implementers SHOULD keep key declarations at stable single-purpose URIs, SHOULD carry key fingerprints alongside full key material (the registry can only anchor what it can unambiguously identify), and SHOULD NOT conflate signing keys with transport keys.

Until the registry adds key-transparency anchoring, the multi-source controls above are the 3.x normative baseline. They are the difference between "an attacker who compromises one counterparty origin gets silent authority" and "the compromise produces a detectable signal within a bounded window." 3.x promises the second; it does not promise the first.
