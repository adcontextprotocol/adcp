...
- **Governance tokens** use `policy_decision_hash` instead of inline `policy_decisions` when the buyer's compliance posture is sensitive — the full decision log remains available to auditors via an access-controlled pointer.
- **Audience members** in `sync_audiences` are SHA-256 hashed on the buyer side; the protocol never carries cleartext PII.
...
- [ ] What is the rotation cadence? (≤24h for write-capable tokens, ≤15 min for financial tokens)
...
Knowing what a protocol doesn't do is part of evaluating it. These are explicit non-goals or deferred items in 3.0 — tracked as follow-ups, not hidden:
...
- **No coordinated vulnerability disclosure policy baked into the protocol.** Individual agent operators should publish `/.well-known/security.txt`; there is no normative AdCP requirement yet.
- **No protocol-level PII transport.** Audience members are hashed client-side by design; if you need cleartext PII on the wire, AdCP is not the right carrier.
- **No in-protocol payment or settlement.** `report_usage` is consumption reporting for billing, not settlement. Money moves out-of-band through the buyer/seller commercial relationship.
