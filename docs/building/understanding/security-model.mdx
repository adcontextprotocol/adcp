...
**Synchronous AdCP responses are NOT signed at the body level.** Buyers MUST NOT rely on response-body signatures; integrity guarantees for synchronous replies are delivered by TLS. Artifacts requiring at-rest attestation MUST be delivered via signed webhooks.

### Why the split is deliberate

This is two surfaces with two purposes, not one surface with a coverage gap.

- **TLS-scoped synchronous + signed-webhook async is the design.** The synchronous reply is consumed inside the authenticated session that carried the request — the buyer already holds TLS-bound proof that the seller it authenticated to answered. A signature on that body adds no information the session doesn't already carry. At-rest integrity, by contrast, is what webhooks are for: the artifact survives past the original transport and verifies long after the session has closed, against keys that long-outlive the TCP connection.
...
- **Cross-agent forwarding** (e.g., sales-intelligence relay, brand-rights handoff). Covered by the relevant specialism's own signing surface, which has its own `adcp_use` purpose and is not a generic response-signing primitive. Don't conflate the two — folding cross-agent forwarding into a generic "signed responses" surface widens the trust model in ways the specialism-scoped key purpose intentionally narrows.
