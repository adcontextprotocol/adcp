...
```json
{
  ...
  "artifacts": [{
    ...
    "parts": [
      ...
      {
        "data": "{\"adcp_error\":{\"code\":\"RATE_LIMITED\",\"message\":\"Request rate exceeded\",\"retry_after\":5,\"recovery\":\"transient\"}}",
...
Under the AdCP v3.2 profile, error DataParts use the same required
`mediaType: "application/json"` and string-valued JSON text as every other AdCP
DataPart. A custom error media type or object-valued fallback is not permitted.
After profile decoding, the `adcp_error` key is the authoritative signal.
...
2. **`decode(artifacts[].parts[].data).adcp_error`** — A2A v3.2 task-level error (artifacts)
3. **`decode(status.message.parts[].data).adcp_error`** — A2A v3.2 task-level error (status message)
  ...
  if (task.artifacts) {
    for (const artifact of task.artifacts) {
      ...
      for (const part of dataParts) {
        const payload = decode(part);
        if (payload?.adcp_error) {
          return validate(payload.adcp_error);
        }
      }
    }
  }
