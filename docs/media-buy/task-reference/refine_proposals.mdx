...
| `alternatives.count` | Number of commercially distinct drafts requested, from 2 through the protocol maximum of 10 and no higher than a seller's advertised `max_alternatives`. Returned alternatives have unique `terms_digest` values; diversity strategy remains free text. |
