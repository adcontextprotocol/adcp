...
**Request schema:** [`/schemas/3.2.0-beta.0/media-buy/refine-proposals-request.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.0/media-buy/refine-proposals-request.json)
...
**Response schema:** [`/schemas/3.2.0-beta.0/media-buy/refine-proposals-response.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.0/media-buy/refine-proposals-response.json)
...
A request containing `finalize` must contain only finalize entries. Sellers validate the complete batch before creating any holds; multi-proposal finalization is atomic. When any hold cannot be created, no result is `finalized`: the failing entry returns `unable` with `reason_code: "hold_unavailable"`, and otherwise-eligible siblings return `unable` with `reason_code: "batch_aborted"`. Exact retries use the same idempotency key.

Not every proposal-capable seller holds inventory. A seller declaring `hold_semantics: "terms_only"` under `media_buy.proposal_refinement` in [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities) commits **commercial terms only** at finalize: the committed proposal is returned with the same atomic all-or-none batch semantics, but no inventory hold is placed, and the seller MUST NOT return `hold_unavailable` or a hold-driven `batch_aborted`. Reservation is owned by the requesting side — the shape used in delegated composition, where the requester owns the sellable catalog and performs its own availability control at acceptance. Absent the declaration, `inventory_hold` semantics (everything above) apply. A new key does not extend or recreate an existing hold: while a committed successor's hold is unexpired, finalizing the same draft again MUST fail at task level with [`INVALID_STATE`](/docs/building/verification/compliance-catalog#error-code-invalid-state) rather than minting a second hold; after expiry a new finalize MAY create a new one.
