...
| `demographic_targeting_resolution` | Lossless demographic readback containing `requested`, `applied`, derived `equivalent`, and a discriminated `execution` object for continuous bounds, interval IDs, or signal refs. Required whenever demographics were requested or applied. Non-equivalent packages also require `difference_reason` and `buyer_approved: true`. |
