...
For per-buy events, sellers whose declared `propagation_surfaces` excludes `webhook` omit those records. That does not suppress account-level insight/assignment records when `media_buy.relationship_notifications.supports_webhook_activity` is true.
