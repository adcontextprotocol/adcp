...
| `bidding` | Package-authored bidding override. Omitted when the package inherits media-buy `bidding`. |
...
| `targeting_overlay` | Targeting currently applied to the package. When demographic targeting is present, `targeting_overlay.demographics` equals the applied predicate in `demographic_targeting_resolution`. |
| `demographic_targeting_resolution` | Lossless demographic readback containing `requested`, `applied`, exact-only `equivalent: true`, and a discriminated `execution` object for continuous bounds, interval IDs, or signal refs. Required whenever demographics were requested or applied. Sellers reject non-equivalent compilations; a buyer choosing another supported predicate submits it as a new request. |
