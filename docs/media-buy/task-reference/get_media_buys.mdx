...
| `insight_types` | string[] | No | Return buys with at least one requested current insight on the buy, a package, or a package–creative assignment. Request only advertised types. |
...
| `insights` | Current buy-level seller insights. In 3.2, `budget_constrained` is valid here. Sibling evaluation fields define exact type coverage and freshness. |
...
| `webhook_activity` | Recent delivery, health, `insights.changed`, and `creative.assignment_changed` attempts relevant to this buy. Only present when requested and supported. |
...
| `insights` | Current package-level seller insights such as diversity, audience saturation, inventory forecast, pacing, or budget constraint |
...
| `insights` | Current seller insights scoped to this package–creative relationship |
| `insight_types_evaluated` | Exact insight types covered by this snapshot; omitted types remain unknown |
| `insights_as_of` | When the seller completed the evaluation represented by `insights` |
| `insights_evaluated_scope` | Optional publisher or placement coverage; omission means the whole assignment was evaluated |

Insights are deliberately attached here rather than to the creative globally. A creative may be fatigued in one package or publisher context while remaining effective elsewhere.
...
The media buy itself may also carry `budget_constrained`; packages may carry `creative_diversity_low`, `audience_saturation`, `inventory_shortfall_forecast`, `pacing_risk`, or `budget_constrained`. Assignment types remain nested under `creative_approvals[]`. Omitted `insights` is unknown; empty is evaluated-clear only for `insight_types_evaluated` and declared coverage. See [Insights, Warnings, and Recommendations](/docs/media-buy/media-buys/insights).
...
Per-buy events (`scheduled`, `final`, `delayed`, `adjusted`, `impairment`) use the buy's `push_notification_config`; their `subscriber_id` may be absent in a single-subscriber setup. Insight and assignment invalidations use account `notification_configs[]`; their `subscriber_id` is required. Support for the latter is declared by `media_buy.insight_notifications`, including whether this activity log is available.
...
| `subscriber_id` | Identifies which registered subscriber received this fire. Required for account-level insight/assignment invalidations; may be absent for a single per-buy subscriber. |
...
| `notification_type` | Verbatim from the webhook payload, including `scheduled`, `final`, `delayed`, `adjusted`, `impairment`, `insights.changed`, and `creative.assignment_changed`. |
...
| State | Meaning |
|-------|---------|
| Field **omitted** | Seller does not surface an applicable activity log for this buy. Neither a declared per-buy webhook surface nor `media_buy.insight_notifications.supports_webhook_activity` provides one, no relevant subscriber is registered, or the seller does not persist fire history. |
...
For per-buy events, sellers whose declared `propagation_surfaces` excludes `webhook` omit those records. That does not suppress account-level insight/assignment records when `media_buy.insight_notifications.supports_webhook_activity` is true.

**Diagnosing an unexpected omission.** For a per-buy event, check the buy's `push_notification_config` and `media_buy.propagation_surfaces`. For an insight or assignment event, check the account's `notification_configs[]` and `media_buy.insight_notifications.supports_webhook_activity`. If the relevant registration and capability both check out, omission is a seller-side persistence gap.
