...
| `status_filter` | string \| string[] | No | Status filter: `"pending_activation"`, `"active"`, `"paused"`, `"completed"`, `"rejected"`, `"canceled"`. Defaults to `["active"]` only when both `media_buy_ids` and `buyer_refs` are omitted. |
...
| `status` | Current status (`pending_activation`, `active`, `paused`, `completed`, `rejected`, `canceled`) |
...
| `canceled_at` | ISO 8601 timestamp when canceled (present only when `status` is `canceled`) |
| `canceled_by` | `"buyer"` or `"seller"` — who initiated cancellation |
| `cancellation_reason` | Reason for cancellation (present only when `status` is `canceled`) |
...
| `canceled_at` | ISO 8601 timestamp when canceled (present only when `canceled` is true) |
| `canceled_by` | `"buyer"` or `"seller"` — who initiated cancellation |
| `cancellation_reason` | Reason for cancellation (present only when `canceled` is true) |
