...
| `detail` | object | Seller-specific scoring (optional). Contains `score`, `max_score`, and `label`. Supplementary — don't use for cross-seller logic. |
...
| `issues` | array | Actionable problems with severity (`error`, `warning`, `info`) and message |
