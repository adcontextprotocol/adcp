...
A receipt is evidence agreement, not a separate commercial-settlement mutation.
For billing, the immutable official `reporting_revision_id` is the invoice anchor.
If the source is corrected later, the seller publishes a separate reporting
adjustment into an open accounting period; it does not replace the official
revision or alter its receipt. The consumer recomputes the adjustment's
...
**Request schema:** [`sync-reporting-receipts-request.json`](https://adcontextprotocol.org/schemas/v3/media-buy/sync-reporting-receipts-request.json)

**Response schema:** [`sync-reporting-receipts-response.json`](https://adcontextprotocol.org/schemas/v3/media-buy/sync-reporting-receipts-response.json)
...
through [`get_reporting_status`](/docs/media-buy/task-reference/get_reporting_status), allowing the consumer to read back that agreement.
