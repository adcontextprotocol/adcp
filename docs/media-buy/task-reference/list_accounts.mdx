...
Returns all accounts the authenticated agent can operate on this seller. Use this to discover existing accounts, check status changes on pending accounts, and retrieve `account_id` values for use in media buy operations.

**Response Time**: ~1s.

**Request Schema**: [`/schemas/account/list-accounts-request.json`](https://adcontextprotocol.org/schemas/account/list-accounts-request.json)
**Response Schema**: [`/schemas/account/list-accounts-response.json`](https://adcontextprotocol.org/schemas/account/list-accounts-response.json)
...
|-----------|------|----------|-------------|
| `status` | string | No | Filter by account status: `active`, `pending_approval`, `payment_required`, `suspended`, or `closed`. |
...
| Field | Description |
|-------|-------------|
| `account_id` | Seller's identifier. Use this in `create_media_buy`, `get_products`, and other operations. |
| `name` | Seller's display name for the account |
| `house` | Brand's house domain |
| `brand_id` | Brand ID within the house (when applicable) |
| `operator` | Operator domain (when applicable) |
| `status` | Current account state: `active`, `pending_approval`, `payment_required`, `suspended`, or `closed` |
| `billing` | Billing model in effect: `brand`, `operator`, or `agent` |
| `parent_account_id` | Parent account when this is a sub-account |
...
- [sync_accounts](/docs/media-buy/task-reference/sync_accounts) — Declare new accounts or update existing ones
- [Accounts and Agents](/docs/building/integration/accounts-and-agents) — Billing models, trust models, and authorized operators
- [create_media_buy](/docs/media-buy/task-reference/create_media_buy) — Use `account_id` from list_accounts in media buy operations
