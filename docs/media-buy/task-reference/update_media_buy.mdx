...
*Either `media_buy_id` OR `media_buy_id` is required (not both)
...
*Either `package_id` OR `media_buy_id` is required for each package update
...
const pkg = validated.affected_packages?.find(p => p.=== 'ctv_package');
if (pkg) {
...
async def increase_budget():
    result = await test_agent.update_media_buy(
        UpdateMediaBuyRequest(
            media_buy_id='mb_12345',
            packages=[
                {'package_id': result.packages[0]['package_id'], 'budget': 50000}
            ]
        )
    )
    ...
    pkg = next((p for p in result.affected_packages if p.== 'ctv_package'), None)
    if pkg:
