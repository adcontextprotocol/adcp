...
**Request Schema**: [`https://adcontextprotocol.org/schemas/v1/media-buy/update-media-buy-request.json`](https://adcontextprotocol.org/schemas/v1/media-buy/update-media-buy-request.json)
**Response Schema**: [`https://adcontextprotocol.org/schemas/v1/media-buy/update-media-buy-response.json`](https://adcontextprotocol.org/schemas/v1/media-buy/update-media-buy-response.json)
...
| `creative_ids` | string[] | No | Simple creative assignments (references existing library creatives) - use for basic creative list updates |
| `creatives` | CreativeAsset[] | No | Full creative objects to upload and assign to this package (alternative to creative_ids - creatives will be added to library). Supports both static and generative creatives. Max 100 creatives. |
| `creative_assignments` | CreativeAssignment[] | No | Full assignment objects with weights and placement targeting (alternative to creative_ids - provides granular control). Each assignment includes creative_id, optional weight (0-100), and optional placement_ids array. |

*Either `package_id` or `buyer_ref` must be provided

**Creative Assignment Fields**: You can provide ONE of: `creative_ids` (simple), `creatives` (upload new), or `creative_assignments` (granular control). All three use replacement semantics.
...
- **Array fields use replacement semantics** - When you provide an array field (like `creative_ids`, `creatives`, or `creative_assignments`), it completely replaces the existing array
  ...
  - To replace all: Provide the new complete list
  - Omitting the field entirely leaves existing assignments unchanged
...
### Example 4: Add Creative to Existing Rotation

To add a creative while keeping existing ones, include all existing creative IDs plus the new one. Due to replacement semantics, you must specify the complete desired list:
  ...
  "packages": [
    {
      "buyer_ref": "purina_ctv_package",
      "creative_ids": [
        "purina_hero_30s",        // Existing creative
        "purina_product_focus_30s", // Existing creative
        "purina_winter_promo_30s"  // New creative to add
      ]
    }
  ]
}
...
#### Response
**Message**: "CTV package updated with 3 creatives. Added 1 new creative to the rotation. Changes take effect immediately for future impressions."
...
### Example 5: Remove Creative from Rotation

To remove a creative, specify only the creatives you want to keep (omit the one to remove):
  ...
  "packages": [
    {
      ...
      "creative_ids": [
        "purina_hero_30s",
        "purina_winter_promo_30s"
        // purina_product_focus_30s is NOT included, so it will be removed
      ]
    }
  ]
}
...
#### Response
**Message**: "CTV package updated with 2 creatives. Removed 1 creative from rotation (purina_product_focus_30s). Remaining creatives will continue serving. Changes take effect immediately for future impressions."
...
### Example 6: Update Creative Weights and Placement Targeting

Use `creative_assignments` to update weights or placement targeting. You can also remove creatives by omitting them from the list:
  ...
  "packages": [
    {
      ...
      "creative_assignments": [
        {
          "creative_id": "purina_hero_30s",
          "weight": 70,
          "placement_ids": ["living_room_smart_tv"]
        },
        {
          "creative_id": "purina_product_focus_30s",
          "weight": 30,
          "placement_ids": ["bedroom_streaming_device"]
        }
      ]
    }
  ]
...
#### Response
**Message**: "CTV package creative rotation updated. Hero creative now weighted 70% (previously 50%) and assigned to living room placement. Product focus creative now weighted 30% and assigned to bedroom placement. Changes take effect immediately for future impressions."
...
### Example 7: Upload New Creative with Weight and Placement

You can upload and assign new creative assets with weight and placement targeting in one step:
  ...
  "packages": [
    {
      ...
      "creatives": [
        {
          "creative_id": "purina_winter_promo_30s",
          "format_id": {
            "agent_url": "https://creative.adcontextprotocol.org",
            "id": "video_standard_30s"
          },
          "name": "Purina Winter Sale 30s Video",
          "assets": {
            "video": {
              "asset_kind": "media",
              "delivery_type": "url",
              "url": "https://cdn.purina.com/ads/winter-sale-30s.mp4"
            }
          },
          "weight": 100,
          "placement_ids": ["living_room_smart_tv", "bedroom_streaming_device"]
        }
      ]
    }
  ]
...
**Note**: Using `creatives` replaces all existing creative assignments with just the newly uploaded creative. The `weight` and `placement_ids` fields are optional - they configure how the creative is assigned to this package but are not stored in the creative library. To keep existing creatives while uploading new ones, first upload using `sync_creatives` (without weight/placement), then update using `creative_assignments` with the complete list including weights/placements.

#### Response
**Message**: "CTV package updated with 1 creative. Previous creative assignments have been replaced. The new creative will begin serving immediately."
...
- **Creative assignment uses replacement semantics**:
  - Providing `creative_ids` replaces ALL existing assignments with the new list (simple, no weights/placements)
  - Providing `creatives` uploads new creatives AND replaces all assignments (upload + assign)
  - Providing `creative_assignments` replaces ALL existing assignments with granular control (weights + placements)
  - Omitting all three fields leaves creative assignments unchanged
  - To add one creative: specify all existing assignments plus the new one
  - To remove one creative: specify all assignments except the one to remove
  - To update weights/placements only: use `creative_assignments` with same creative_ids but different weights/placement_ids
- Choose the right field for your use case:
  - `creative_ids`: Simple list changes (add/remove creatives, no weights/placements needed)
  - `creatives`: Upload brand new creative assets with optional weight/placement configuration
  - `creative_assignments`: Update weights or placement targeting for existing creatives
- New creatives uploaded via the `creatives` field are automatically added to the creative library
- **Weight and placement in creative uploads**: When using `creatives`, you can optionally include `weight` and `placement_ids` in each creative object to configure assignment at upload time. These fields affect only the assignment to this package - they are not stored in the creative library.
