...
| `creative_ids` | string[] | No | Update creative assignments (references existing library creatives) |
...
- **Array fields use replacement semantics** - When you provide an array field (like `creative_ids` or `creatives`), it completely replaces the existing array
  - To add a creative: Include all existing creative IDs plus the new one
  - To remove a creative: Include all creative IDs except the one to remove
  - To replace all creatives: Provide the new complete list
...
- **Creative assignment uses replacement semantics**:
  - Providing `creative_ids` replaces ALL existing assignments with the new list
  - Providing `creatives` uploads new creatives AND replaces all assignments
  - Omitting both fields leaves creative assignments unchanged
  - To add one creative: specify all existing IDs plus the new one
  - To remove one creative: specify all IDs except the one to remove
