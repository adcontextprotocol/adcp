...
[`control_media_buy`](/docs/media-buy/task-reference/control_media_buy) does not accept assignments, so a control followed by
[`sync_creatives`](/docs/creative/task-reference/sync_creatives) is a non-atomic saga and cannot substitute for this shape.
...
mapping: [`refine_proposals`](/docs/media-buy/task-reference/refine_proposals) has no typed billing-entity refinement, and its
...
**Assertion path** (`packages` or `new_packages` present): the seller applies the explicit package mutations and requires `total_budget.amount` to equal the resulting fixed-mode package sum. If the total is inconsistent, the seller MUST return [`VALIDATION_ERROR`](/docs/building/verification/compliance-catalog#error-code-validation-error) and apply no changes.
...
[`sync_creatives`](/docs/creative/task-reference/sync_creatives) task does not increment MediaBuy revision and does not share
...
specific to the compatibility facade; compact [`control_media_buy`](/docs/media-buy/task-reference/control_media_buy) makes root
...
| [`ACTION_NOT_ALLOWED`](/docs/building/verification/compliance-catalog#error-code-action-not-allowed) | Field cannot be changed | Branch on `error.details.reason`; for `not_supported_on_buy`, use optional `decline_reason` to inform renegotiation |
...
| [`CREATIVE_REJECTED`](/docs/building/verification/compliance-catalog#error-code-creative-rejected) | Creative failed content-policy, brand-safety, or accessibility review | Revise according to the applicable policy or validated accessibility criteria |
