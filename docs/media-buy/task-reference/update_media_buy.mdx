...
| `bidding` | BiddingPolicy \| null | Replace the complete package-authored override, or clear it with `null` so the package inherits media-buy bidding. |
| `bid_price` | number | **Deprecated in 3.2.** Legacy package bid representation; cannot be combined with `bidding`. |
