...
| `increase_budget` | `total_budget`, `packages[].budget` (raise) |  |
...
| `update_budget_allocation` | `budget_allocation`, `packages[].min_spend_target` | Changes fixed/shared mode, cross-package goals, or soft minimum targets. Automatic allocation inside an accepted seller-optimized buy is not an update action. |
...
| `total_budget` | TotalBudget | No | Updated hard aggregate lifetime budget. Already-spent amounts continue to count against the new total. |
...
| `bidding` | BiddingPolicy \| null | No | Replace the complete media-buy bidding default, or clear it with `null`. Package-authored overrides remain explicit. |
...
| `budget` | number | Updated hard package spend cap. In seller-optimized mode this is not a current allocation. |
| `min_spend_target` | number | Updated soft package minimum-spend target for seller-optimized allocation. |
...
| `bidding` | BiddingPolicy \| null | Replace the complete package-authored override, or clear it with `null` so the package inherits media-buy bidding. During legacy migration, `null` may accompany `bid_price` or a monetary optimization-goal target; only a non-null canonical block conflicts with those representations. |
