...
| `increase_budget` | `total_budget`, `packages[].budget` (raise or `null`) | `packages[].budget: null` removes a seller-optimized package cap and therefore relaxes, rather than lowers, its ceiling. |
| `decrease_budget` | `total_budget`, `packages[].budget` (lower) | Sellers typically bound by already-spent |
...
| `update_pacing` | `pacing`, `packages[].pacing` | Media-buy pacing controls aggregate spend; package pacing is subordinate. |
...
The coarse legacy actions (`update_budget`, `update_dates`, `update_packages`, [`sync_creatives`](/docs/creative/task-reference/sync_creatives)) cover the finer-grained vocabulary in a single value for sellers still emitting the 3.0 enum surface. Sellers SHOULD migrate to the finer set; the legacy values are removed in 4.0.
...
| `canceled` | boolean | No | Cancel the entire media buy (irreversible). Must be `true` when present. Seller may reject with [`NOT_CANCELLABLE`](/docs/building/verification/compliance-catalog#error-code-not-cancellable). |
...
| `invoice_recipient` | [BusinessEntity](/docs/building/by-layer/L2/accounts-and-agents#billing-entity-and-invoice-recipient) | No | Override who receives the invoice for this buy. The seller MUST validate authorization and include in [`check_governance`](/docs/governance/campaign/tasks/check_governance) when governance agents are configured. |
| `new_packages` | PackageRequest[] | No | New packages to add to this media buy. Same shape as [`create_media_buy`](/docs/media-buy/task-reference/create_media_buy) packages. When `budget_allocation` is omitted or fixed, each new package requires `budget` and cannot use `min_spend_target`. To add an uncapped package to an existing seller-optimized buy, include the resulting seller-optimized `budget_allocation` block so the allocation context is explicit. Only supported by sellers that advertise `add_packages` in `valid_actions`. |
| `total_budget` | TotalBudget | No | Updated hard aggregate lifetime budget. Mutually exclusive with `packages` and `new_packages`. In fixed mode, the seller atomically scales active package budgets in proportion to their current committed budgets; in seller-optimized mode, it updates the shared pool without treating package caps as allocations. Already-spent amounts continue to count against the new total. `currency` must equal the existing media-buy currency. |
...
For budget/pacing interaction and bidding inheritance, see [Budget & Pacing Controls](/docs/media-buy/task-reference/create_media_buy#budget--pacing-controls) and [BiddingPolicy Object](/docs/media-buy/task-reference/create_media_buy#biddingpolicy-object) on the `create_media_buy` reference.
...
**Note**: Responses use discriminated unions. Terminal errors never carry `warnings`; warnings occur only on successful updates. See [Indicators and Warnings](/docs/media-buy/media-buys/indicators).
...
**[`INVALID_STATE`](/docs/building/verification/compliance-catalog#error-code-invalid-state) error response** (e.g., trying to update a completed media buy):
```json
{
  "errors": [{
    "code": "INVALID_STATE",
...
**[`REQUOTE_REQUIRED`](/docs/building/verification/compliance-catalog#error-code-requote-required) error response** (update changes the parameter envelope the quote was priced against):
```json
{
  "errors": [{
...
| [`PACKAGE_NOT_FOUND`](/docs/building/verification/compliance-catalog#error-code-package-not-found) | Package doesn't exist | Verify `package_id`; for legacy package correlation use `get_media_buys` + package `context.buyer_ref` |
| [`ACTION_NOT_ALLOWED`](/docs/building/verification/compliance-catalog#error-code-action-not-allowed) | Field cannot be changed | See "What Can Be Updated" above |
| [`BUDGET_TOO_LOW`](/docs/building/verification/compliance-catalog#error-code-budget-too-low) | New budget below minimum | Increase budget amount |
| [`POLICY_VIOLATION`](/docs/building/verification/compliance-catalog#error-code-policy-violation) | Update violates content policy | Review policy requirements |
| [`INVALID_STATE`](/docs/building/verification/compliance-catalog#error-code-invalid-state) | Operation not allowed in current state (e.g., updating completed/canceled media buy) | Check campaign status via `get_media_buys` |
| [`NOT_CANCELLABLE`](/docs/building/verification/compliance-catalog#error-code-not-cancellable) | Media buy or package cannot be canceled | Check seller's cancellation policy or contact seller |
| [`CREATIVE_REJECTED`](/docs/building/verification/compliance-catalog#error-code-creative-rejected) | Creative failed content policy review | Revise the creative per the seller's advertising policies |
| [`CREATIVE_LOCALE_NOT_ACCEPTED`](/docs/building/verification/compliance-catalog#error-code-creative-locale-not-accepted) | Creative has no materialized variant accepted by the package's effective format locale policy, or its serving default is ineligible | Supply a matching locale variant, assign a different creative, or change the creative's default behavior |
| [`CREATIVE_DEADLINE_EXCEEDED`](/docs/building/verification/compliance-catalog#error-code-creative-deadline-exceeded) | Creative change submitted past the package's `creative_deadline` | Check package `creative_deadline` before submitting creative changes |
| [`BUDGET_EXCEEDED`](/docs/building/verification/compliance-catalog#error-code-budget-exceeded) | Operation would exceed allocated budget | Reduce the amount or increase media buy total budget |
| [`CONFLICT`](/docs/building/verification/compliance-catalog#error-code-conflict) | Revision mismatch — another update was applied since you last read | Re-read via `get_media_buys` and retry with current `revision` |
| [`REQUOTE_REQUIRED`](/docs/building/verification/compliance-catalog#error-code-requote-required) | Requested change (budget, dates, volume, targeting, or signal targeting price) falls outside the envelope the original quote was priced against | Adjust the update to fit the current quote, rediscover products/terms, add packages when `add_packages` is available, or create a separate media buy. 3.1 does not define an amendment-quote artifact for `update_media_buy`. Seller's `error.details.envelope_field` names which fields breached. |
| [`VALIDATION_ERROR`](/docs/building/verification/compliance-catalog#error-code-validation-error) | Request field values, including a duplicate submitted `creative_id`, violate seller/product business rules | Follow `error.field` and `error.message`; for a duplicate inline creative, use a different package-scoped `creative_id` |
...
See the [seller integration guide](/docs/building/operating/seller-integration#execution-checks) for the full execution check workflow and code example.
</Note>
