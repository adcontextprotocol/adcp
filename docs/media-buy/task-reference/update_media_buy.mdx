...
| `creatives` | CreativeAsset[] | No | Upload and assign new creative assets inline (`creative_id` must not already exist in library) |
...
- **Inline creatives**: The `creatives` array creates NEW creatives only. To update existing creatives, use [`sync_creatives`](/docs/media-buy/task-reference/sync_creatives). To assign existing creatives, use `creative_ids` in the Package Update object.
