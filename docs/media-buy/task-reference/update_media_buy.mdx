...
The response includes `status: "canceled"` to confirm the transition. Cancellation is irreversible. If the seller cannot cancel (e.g., contractual obligations), the response will contain an error with code `NOT_CANCELLABLE`.
...
| `UNSUPPORTED_FEATURE` | Field cannot be changed | See "What Can Be Updated" above |
| `BUDGET_TOO_LOW` | New budget below minimum | Increase budget amount |
...
| `CREATIVE_REJECTED` | Creative change rejected (may be past `creative_deadline`) | Check package `creative_deadline` before submitting creative changes |
