...
| `UPDATE_REQUIRES_GOVERNANCE` | Price-affecting update (`bid_price`, `budget`, `daily_cap`, flight extension, targeting broadening) without a fresh `modification`-phase `governance_context` whose `plan_hash` covers the post-update plan state | Call `check_governance` with `phase: "modification"` against the proposed post-update plan, then retry with the new token |
...
**Price-affecting fields (MUST trigger re-check when present in the update)**:

- `bid_price` (any change)
- `budget` (any change — media-buy-level or package-level)
- `daily_cap`
- `flight_dates` / `start_time` / `end_time` when the change **extends** flight duration
- `targeting` / `targeting_overlay` when the change **broadens** the addressable audience (adding geos, removing exclusions, adding inventory, widening demographic filters)
...
**Buyer workflow**: before submitting a price-affecting update, the buyer calls `check_governance` with `phase: "modification"`, the proposed post-update plan state, and the existing `governance_context`. The governance agent returns a new token bound to the post-update `plan_hash`. The buyer attaches the new token to `update_media_buy` on the protocol envelope.
