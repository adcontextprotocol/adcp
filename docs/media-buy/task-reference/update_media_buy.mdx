...
| `daily_budget_cap` | number | Updated hard per-calendar-day spend ceiling, in the media buy's currency. PATCH semantics — applied from the next cap-day boundary forward, not retroactively. Same enforcement contract as create: requires `media_buy.budget_capping` support; unenforceable caps are rejected with `UNSUPPORTED_FEATURE`. |
| `budget_cap_timezone` | string | Updated IANA timezone override for the cap day boundary. Valid only when the seller advertises `media_buy.budget_capping.buyer_timezone_override: true`. |
