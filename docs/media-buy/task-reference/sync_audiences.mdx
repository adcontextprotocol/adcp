...
**Polling guidance**: Matching typically completes within 1–48 hours. Poll no more frequently than every 15 minutes. Use discovery-only calls (omit `audiences`) to check status without modifying data.
