...
**Async completion**: When the seller returns `status: "processing"`, the protocol assigns a `task_id`. Configure `push_notification_config` at the protocol level to receive a webhook when matching completes — the payload will include the full `sync_audiences` result. See the protocol task tracking documentation for details.

**Polling fallback**: Poll no more frequently than every 15 minutes using discovery-only calls (omit `audiences`). Matching typically completes within 1–48 hours.

**Agent workflow**: Upload returns `status: "processing"`. The agent must externalize the `audience_id`, `account_id`, and `task_id` before the session ends. Resume in a future session — either via the webhook notification or by polling `sync_audiences` with no `audiences` array and filtering the response by `audience_id`. When `status: "ready"`, proceed to `create_media_buy`.
