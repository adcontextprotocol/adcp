...
**Identifier support varies by seller**: Check `get_adcp_capabilities` → `targeting.supported_identifier_types` and `targeting.supported_uid_types` before sending. MAID support is not universal (LinkedIn does not accept MAIDs; iOS IDFA requires App Tracking Transparency consent). The `matching_latency_hours` range and `minimum_audience_size` in capabilities are also seller-specific.
