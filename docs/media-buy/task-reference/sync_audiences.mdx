...
**Upload confirmation**: The upload is accepted synchronously (1–2s). Configure `push_notification_config` at the protocol level to receive a webhook confirming the upload was accepted and the `task_id` assigned. This fires at upload time, not when matching completes.

**Audience readiness**: Platform matching is independent of task completion and takes 1–48 hours. Check `get_adcp_capabilities` → `audience_targeting.matching_latency_hours` to know when to start polling. Poll no more frequently than every 15 minutes using discovery-only calls (omit `audiences`).

**Agent workflow**: Upload, capture the `audience_id`, `account_id`, and initial status before the session ends. Resume in a future session: call `sync_audiences` with no `audiences` array, filter by `audience_id`, and check status. When `status: "ready"`, proceed to `create_media_buy`.
