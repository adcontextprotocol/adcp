...
**Request Schema**: [`/schemas/v1/media-buy/list-creative-formats-request.json`](https://adcontextprotocol.org/schemas/v1/media-buy/list-creative-formats-request.json)
**Response Schema**: [`/schemas/v1/media-buy/list-creative-formats-response.json`](https://adcontextprotocol.org/schemas/v1/media-buy/list-creative-formats-response.json)
...
See [Format schema](https://adcontextprotocol.org/schemas/v1/core/format.json) for complete format object structure.
...
```python Python
from adcp.test_helpers import test_agent
...
for format in result['formats']:
    print(f"{format['name']}: {len(format['assets_required'])} assets required")
```
...
for format in result['formats']:
    print(f"\n{format['name']}:")
    for asset in format['assets_required']:
...
console.log(`Found ${result.formats.length} third-party tag formats ≤ 970×250`);
...
for format in result['formats']:
    renders = format.get('renders', [])
    if renders:
        dims = renders[0]['dimensions']
        print(f"{format['name']}: {dims['width']}×{dims['height']}")
```
...
console.log(`Found ${result.formats.length} video formats`);
...
for format in result['formats']:
    duration = format.get('requirements', {}).get('duration', 'unknown')
    by_duration[duration].append(format['name'])
...
console.log(`Found ${result.formats.length} mobile formats`);
...
for format in result['formats']:
    print(f"- {format['name']} ({format['type']})")
```
...
console.log(`Found ${result.formats.length} responsive display formats`);
...
for format in result['formats']:
    print(f"{format['name']}: Adapts to container")
```
...
- **`third_party_tag`** - External ad tag
...
- [Format Schema](https://adcontextprotocol.org/schemas/v1/core/format.json) - Complete format structure
- [Asset Requirements](/docs/creative/asset-requirements) - Asset specification details
- [Standard Formats](/docs/creative/standard-formats) - IAB-compatible reference formats
