...
Measurement agents do not receive seller credentials. In the first experimental gateway tier, the provider reads buyer-approved cross-seller delivery from the orchestrator's `get_media_buy_delivery` task and returns compact assertions through its `provide_performance_feedback` task.
...
The producer does not need to disclose the raw baseline value. For ratio-compatible metrics, higher-is-better measures SHOULD use `observed / baseline`; lower-is-better measures such as CPA SHOULD use `baseline / observed`. In both cases values above `1.0` mean better performance.
...
2. The provider calls the orchestrator's `get_media_buy_delivery` task for buyer-approved, cross-seller delivery.
