...
| `vendor` | [BrandRef](/docs/reference/glossary#b) | Conditional | Vendor that produced this feedback. Required when `feedback_source` is `third_party_measurement` or `verification_partner`; optional otherwise. The vendor's `brand.json` `agents[type='measurement']` is the discovery anchor; metric definitions live on the agent's `get_adcp_capabilities.measurement.metrics[]` block. |
