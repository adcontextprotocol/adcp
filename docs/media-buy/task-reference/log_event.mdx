...
| Field | Type | Description |
|-------|------|-------------|
| `value` | number | Monetary value of the event; requires `currency` |
| `currency` | string | ISO 4217 code declared in the event source's `value_currencies` (e.g. `USD`, `EUR`, `GBP`) |
...
5. **Include contracted value and currency** - Monetary events require both `custom_data.value` and `custom_data.currency`; the currency must appear in the source's `value_currencies` declared through `sync_event_sources`. Each ROAS buy consumes only records matching its media-buy currency.
