...
|-----------|------|----------|-------------|
| `event_source_id` | string | Yes | Event source configured on the account via [`sync_event_sources`](/docs/media-buy/task-reference/sync_event_sources) |
...
|------------|-------------|------------|
| [`REFERENCE_NOT_FOUND`](/docs/building/verification/compliance-catalog#error-code-reference-not-found) | Event source not configured or not accessible (`error.field` = `event_source_id`) | Run [`sync_event_sources`](/docs/media-buy/task-reference/sync_event_sources) first |
| [`INVALID_REQUEST`](/docs/building/verification/compliance-catalog#error-code-invalid-request) | An event field violates its schema, or the batch contains more than 10,000 events | Correct the field identified by `error.field` or split the request |
| [`VALIDATION_ERROR`](/docs/building/verification/compliance-catalog#error-code-validation-error) | The event type is disallowed for the configured source, the timestamp is outside the attribution window, or no usable user match is present | Check the event source configuration and correct the event values |
| [`RATE_LIMITED`](/docs/building/verification/compliance-catalog#error-code-rate-limited) | Too many requests | Wait and retry with exponential backoff |

## Best Practices

1. **Configure sources first** - Always run [`sync_event_sources`](/docs/media-buy/task-reference/sync_event_sources) before sending events. Events sent to unconfigured sources are rejected.
