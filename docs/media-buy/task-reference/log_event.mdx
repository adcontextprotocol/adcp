...
| `events` | ConversionEvent[] | Yes | Events to log (min 1, max 10,000) |
...
At least one of `uids`, `click_id`, or `client_ip` + `client_user_agent` is required:

| Field | Type | Description |
|-------|------|-------------|
| `uids` | UID[] | Universal ID values (`rampid`, `id5`, `uid2`, `euid`, `pairid`) |
...
**No PII:** AdCP does not support hashed email or phone for user matching. Use universal IDs and click IDs only.
...
| `MISSING_USER_MATCH` | No user identifiers provided | Include at least one of: uids, click_id, or client_ip + client_user_agent |
...
2. **Include user_match** - Events without user identifiers cannot be attributed. Provide the strongest identifiers available: UIDs > click IDs > IP/UA.
...
- [create_media_buy](/docs/media-buy/task-reference/create_media_buy) - Set optimization goals on packages
