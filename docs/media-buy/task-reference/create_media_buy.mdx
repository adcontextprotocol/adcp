...
`daily_budget_cap` requires seller support: sellers advertise `media_buy.budget_capping` in [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities) (declaring `enforcement` strength and the `effective_timezone`) and MUST reject a cap they cannot enforce with `UNSUPPORTED_FEATURE` rather than silently dropping it. Sellers echo the accepted cap on read surfaces so buyers can verify it took effect.
