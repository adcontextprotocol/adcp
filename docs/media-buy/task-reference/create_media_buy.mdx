...
The compact lifecycle deliberately has a different boundary. [`buy_products`](/docs/media-buy/task-reference/buy_products) or
[`accept_proposal`](/docs/media-buy/task-reference/accept_proposal) commits first and returns the seller-assigned package IDs;
[`sync_creatives`](/docs/creative/task-reference/sync_creatives) then uploads and assigns library creatives. That sequence is a
...
| `proposal_id` | string | No* | ID of the exact committed proposal snapshot produced by [`refine_proposals`](/docs/media-buy/task-reference/refine_proposals) with `action: "finalize"`, or by the legacy [`get_products`](/docs/media-buy/task-reference/get_products) finalize action. Drafts fail with [`PROPOSAL_NOT_COMMITTED`](/docs/building/verification/compliance-catalog#error-code-proposal-not-committed); declined or already executed proposals fail with [`INVALID_STATE`](/docs/building/verification/compliance-catalog#error-code-invalid-state). Exact retries reuse the original idempotency key and replay success. |
...
An unexpired `committed` proposal carries an inventory hold and cannot be rejected merely because the seller sold that reserved inventory elsewhere. After `expires_at`, the hold lapses and the seller returns [`PROPOSAL_EXPIRED`](/docs/building/verification/compliance-catalog#error-code-proposal-expired).
...
| `catalogs` | Catalog[] | No | Catalog bodies, references, or filtered selections promoted by this package. Compact purchase accepts previously synchronized catalog IDs only, so translating an inline body requires a separate, non-atomic [`sync_catalogs`](/docs/media-buy/task-reference/sync_catalogs) mutation. |
...
Execute a committed proposal finalized through [`refine_proposals`](/docs/media-buy/task-reference/refine_proposals) without manually constructing packages:
...
`daily_budget_cap` requires seller support: sellers advertise `media_buy.budget_capping` in [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities), including supported scopes and the default timezone basis. A cap is always hard. Sellers MUST reject an undeclared scope with [`UNSUPPORTED_FEATURE`](/docs/building/verification/compliance-catalog#error-code-unsupported-feature) rather than silently dropping or softening it, and echo accepted caps plus the resolved shared timezone on read surfaces.
  ...
  "task": {
    ...
    "artifacts": [{
      ...
      "parts": [
        ...
        {
          "data": "{\"status\":\"completed\",\"media_buy_id\":\"mb_12345\",\"confirmed_at\":\"2025-06-01T10:00:00Z\",\"creative_deadline\":\"2025-06-15T23:59:59Z\",\"revision\":1,\"packages\":[{\"package_id\":\"pkg_001\"}]}",
          "mediaType": "application/json"
        }
      ]
    }]
...
```json
{
  "task": {
    ...
    "artifacts": [{
      ...
      "parts": [{
        "data": "{\"status\":\"submitted\",\"task_id\":\"adcp-task-abc\",\"message\":\"Awaiting budget approval\"}",
        "mediaType": "application/json"
      }]
    }]
  }
    ...
    "status": {
      ...
      "message": {
        ...
        "parts": [
          ...
          { "data": "{\"reason\":\"APPROVAL_REQUIRED\"}", "mediaType": "application/json" }
        ]
      }
    }
  }
  ...
  "task": {
    ...
    "artifacts": [{
      ...
      "parts": [
        ...
        {
          "data": "{\"adcp_error\":{\"code\":\"INSUFFICIENT_INVENTORY\",\"message\":\"Requested targeting yields no available impressions\",\"suggestion\":\"Expand geographic targeting\"}}",
          "mediaType": "application/json"
        }
      ]
    }]
