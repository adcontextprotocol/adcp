...
Each package SHOULD specify the formats it will use, via either `format_ids[]` (v1 path — structured `{agent_url, id}` references) or `format_option_refs[]` (v2 path — references into the product's `format_options[]`). Omitting all format selectors defaults to all formats supported by the product. Specifying lets the system:
...
Use `format_option_refs[]` when the buyer's codebase is V2-native (reads `Product.format_options[]`); use `format_ids[]` when integrating with v1-only sellers or libraries. Dual-emission is allowed and recommended for buyer SDKs targeting a mixed seller population — see the `format_ids` row above.

V2-path example (buyer authoring against a publisher-scoped `Product.format_options[]` entry):
...
Invalid format option example (v2 path):
