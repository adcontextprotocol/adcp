...
| `proposal_id` | string | No* | ID of the exact committed proposal snapshot from [`finalize_proposals`](/docs/media-buy/task-reference/finalize_proposals). For IDs from the 3.2 split lifecycle, draft proposals fail with [`PROPOSAL_NOT_COMMITTED`](/docs/building/verification/compliance-catalog#error-code-proposal-not-committed), while declined or already executed proposals fail with [`INVALID_STATE`](/docs/building/verification/compliance-catalog#error-code-invalid-state). Exact retries reuse the original idempotency key and replay success. Legacy [`get_products`](/docs/media-buy/task-reference/get_products) proposals retain 3.x behavior. |
