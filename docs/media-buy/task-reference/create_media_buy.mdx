...
| `measurement_terms` | [MeasurementTerms](/docs/media-buy/advanced-topics/pricing-models#measurement-terms) | No | Buyer's proposed measurement and verification terms for this package (billing measurement vendor, IVT threshold, viewability floor). Overrides the product's defaults. The seller accepts (echoed on confirmed package), rejects with `TERMS_REJECTED`, or adjusts and returns actual terms. When omitted, the product's `measurement_terms` apply. |
