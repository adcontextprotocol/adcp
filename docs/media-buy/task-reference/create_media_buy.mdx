...
| `UNSUPPORTED_FEATURE` | v1 path: `format_ids[]` entry not supported by product. v2 path: `capability_ids[]` entry doesn't resolve against the target product's `format_options[]` — or the product is v1-only (no `format_options[]` at all) — or the product's `format_options[]` entries don't publish `capability_id` values | v1 path: check the product's supported `format_ids`; v2 path: re-author against `format_ids[]`, or pick a `capability_id` from the product's published `format_options[]` |
