...
| `optimization_goals` | [OptimizationGoal[]](/docs/media-buy/conversion-tracking/#optimization-goals) | No | Optimization targets for this package. Each goal is either `kind: "event"` (conversion event with optional `cost_per` or `per_ad_spend` target) or `kind: "metric"` (seller-native metric with optional `cost_per` or `rate` target). Product must declare support in `conversion_tracking.supported_optimization_strategies`. |
...
Set a per_ad_spend target for conversion-optimized delivery. The product must declare support in `conversion_tracking.supported_optimization_strategies`, and you must have an event source configured via `sync_event_sources`:
