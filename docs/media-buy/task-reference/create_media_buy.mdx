...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `product_id` | string | Yes | Opaque configured-product ID from `get_products`. It is stable within the discovery/refinement context that issued it, including subsequent `refine` calls, but is not guaranteed to remain valid in an independent discovery context. Selecting it accepts any `targeting_resolution` disclosed on that product. Sellers MUST echo it on every response package object representing the request. |
...
| `targeting_overlay` | TargetingOverlay | No | Additional constraints intersected with the configured targeting represented by `product_id` (see [Targeting](/docs/media-buy/advanced-topics/targeting)). The resulting package echoes the complete effective overlay. Direct create-time additions must compile exactly or be rejected. For `demographics`, inspect the selected product's `demographic_targeting` declaration first; sellers never silently broaden or narrow the original. Use `placement_selection` here to select targetable placements; creative assignment placement refs only route creatives. |
...
| `packages` | Array of created packages with complete state. Packages SHOULD echo canonical selectors supplied on create so read surfaces are lossless. When additional creative coverage is required, each package carries `formats_to_provide[]`: full canonical ProductFormatDeclaration snapshots normalized from the selected product contract. |
...
| `PRODUCT_EXPIRED` | Configured product passed `expires_at` | Re-run `get_products` and select a current configuration |
