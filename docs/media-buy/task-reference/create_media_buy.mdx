...
- **packages**: Array of created packages with complete state information (full Package objects)
  ...
  - **paused**: Whether the package is paused (false = active, true = paused)
...
Provide a full brand manifest with guidelines, assets, colors, fonts, and product catalog. See [Brand Manifest](/docs/creative/brand-manifest) for complete documentation.
...
The brand and promoted products are validated during media buy creation. If a policy violation is detected, the API will return an error:
...
- The brand and promoted products align with the selected packages
- Any uploaded creatives match the declared brand and products
