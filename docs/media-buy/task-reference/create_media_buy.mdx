...
- The fields MUST NOT change post-creation. `update_media_buy` cannot modify them; renegotiating the metric contract requires a new buy.
- Optional in v1 (sellers without per-package snapshot infrastructure can adopt incrementally; absence is conformant). Expected to become required at the next major.
