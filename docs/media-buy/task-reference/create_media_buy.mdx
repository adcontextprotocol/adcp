...
| `proposal_id` | string | No* | ID of a committed proposal from [`get_products`](/docs/media-buy/task-reference/get_products) to execute. Alternative to providing packages. Draft proposals must be finalized first; sellers reject draft proposal execution with [`PROPOSAL_NOT_COMMITTED`](/docs/building/verification/compliance-catalog#error-code-proposal-not-committed). |
...
Execute a proposal from [`get_products`](/docs/media-buy/task-reference/get_products) without manually constructing packages:
