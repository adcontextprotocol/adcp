...
| `audience_evidence_selected` | AudienceEvidence[] | No | Exact compact product evidence rows the buyer relied on during manual product selection. Copy matching rows from `Product.audience_evidence`; the seller validates them against the current product and echoes accepted rows as `audience_evidence_used`. Seller-authored proposals persist their selected rows without requiring this field. |
