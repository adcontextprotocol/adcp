...
| `optimization_goals` | [OptimizationGoal[]](/docs/media-buy/conversion-tracking/#optimization-goals) | No | Optimization targets for this package. Each goal is either `kind: "event"` (conversion events with `event_sources` array, optional `cost_per` or `per_ad_spend` target) or `kind: "metric"` (seller-native metric with optional `cost_per` or `threshold_rate` target). Product must declare support in `conversion_tracking.supported_optimization_strategies`. |
