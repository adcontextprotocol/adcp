...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `product_id` | string | Yes | Opaque configured-product ID from `get_products`. It is stable within the discovery/refinement context that issued it, including subsequent `refine` calls, but is not a cross-session catalog identity. Selecting it accepts any `targeting_resolution` disclosed on that product. Sellers MUST echo it on every response package object representing the request. |
