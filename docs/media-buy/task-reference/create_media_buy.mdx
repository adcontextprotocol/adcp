...
`daily_budget_cap` requires seller support: sellers advertise `media_buy.budget_capping` in [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities), including supported scopes and the effective timezone. A cap is always hard. Sellers MUST reject an undeclared scope with `UNSUPPORTED_FEATURE` rather than silently dropping or softening it, and echo accepted caps plus the shared timezone on read surfaces.
