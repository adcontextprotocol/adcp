...
| `optimization_goals` | [OptimizationGoal[]](/docs/media-buy/conversion-tracking/#optimization-goals) | No | Optimization targets for this package. Each goal is either `kind: "event"` (conversion event with optional CPA/ROAS target) or `kind: "metric"` (seller-native metric with optional CPC/CPV/CPCV target). Product must declare support in `conversion_tracking.supported_optimization_strategies`. |
