...
| `total_budget` | TotalBudget | No* | Hard aggregate lifetime budget. Required for proposals and seller-optimized explicit packages. Optional in fixed explicit-package mode; when supplied there, it must equal the sum of package budgets. |
...
| `bidding` | BiddingPolicy | No | Complete media-buy bidding default inherited by packages that omit `packages[].bidding`. In shared-budget mode it is authoritative for provider-native campaign strategy. |
...
| `invoice_recipient` | [BusinessEntity](/docs/building/by-layer/L2/accounts-and-agents#billing-entity-and-invoice-recipient) | No | Override the account's default billing entity for this buy. The seller MUST validate the recipient is authorized and include it in `check_governance` when governance agents are configured. |
...
| `budget` | number | Conditional | Hard lifetime package spend cap. Required in fixed mode. Optional in seller-optimized mode; it remains a ceiling, not a current or reserved allocation. |
| `min_spend_target` | number | No | Soft lifetime package spend target for seller-optimized allocation. The seller should attempt to reach it, but it is not a billing or delivery guarantee. |
...
| `bidding` | BiddingPolicy | No | Complete package-authored bidding override. Omit to inherit media-buy `bidding`. This is replacement inheritance, not a field-by-field merge. |
...
`optimization_goals` select the objective; `bidding` constrains how the seller executes against it. Currency is inherited from the media buy and is not repeated in the block.

| Field | Meaning |
|-------|---------|
| `bid_amount` | Manual auction bid in the selected pricing option's auction unit. It is not a promise that the clearing price equals the bid. |
| `max_bid` | Hard per-auction ceiling in the selected pricing option's auction unit. It is the only canonical hard auction ceiling. |
| `cost_per: { amount, strength }` | Average cost per primary-goal result. `strength` is `cap` or `target`; neither is a per-result guarantee. |
| `roas: { value, strength }` | Dimensionless return per unit of ad spend. `strength` is `floor` or `target`; value-bearing event sources are required. |

`bid_amount`, `cost_per`, and `roas` are mutually exclusive primary modes. `max_bid` can stand alone or supplement `cost_per`/`roas` only where the provider supports it; `bid_amount` and `max_bid` cannot coexist. `cost_per` resolves against the package's primary optimization goal. `bid_amount` and `max_bid` resolve against the selected pricing option's auction unit, which permits combinations such as a conversion-cost target plus a max-CPC ceiling.
...
Keep the purchase event as the objective, then express target ROAS and the max-CPC auction ceiling separately in `bidding`. You must have an event source configured via `sync_event_sources`:
...
5. Media-buy `bidding` governs campaign-level execution policy; package overrides are allowed only when the provider can preserve their authored semantics under the shared strategy.

All packages in a seller-optimized buy MUST use the `total_budget.currency`. Every participating product MUST support the primary cross-package optimization goal. Sellers reject incompatible product combinations, currencies, pricing terms, or delivery constraints with `TERMS_REJECTED` or `UNSUPPORTED_FEATURE` and identify the incompatible package.
