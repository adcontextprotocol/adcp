...
| `brand_manifest` | BrandManifestRef | Yes | Brand information manifest serving as the namespace and identity for this media buy. Required as the primary identifier for campaigns, but **minimal manifests are acceptable** for sales agents focused purely on media buying. Provides brand context for policy checks, creative generation, and optimization when needed. Can be provided as an inline object or URL reference to a hosted manifest. Can be cached and reused across multiple requests. See [Brand Manifest](/docs/creative/brand-manifest) for details and minimal usage patterns. |
...
The `brand_manifest` field is **required** as it serves as the primary namespace and identifier for all campaigns. However, **sales agents focused purely on media buying can provide minimal manifests**.

### When You Need Policy Checks, Creative Generation, or Optimization

Provide a full brand manifest with guidelines, assets, and product catalog. See [Brand Manifest](/docs/creative/brand-manifest) for complete documentation.

### When You're Only Creating Media Buys

**Minimal manifests are perfectly acceptable.** You just need to identify the brand - AdCP uses this as the campaign namespace:
...
- **Campaign grouping**: All media buys for the same brand are linked by the manifest
- **Basic identity**: Name and URL for reporting and tracking
- **Future extensibility**: Can be expanded later if you need creative generation or policy checks

**What you don't need to provide:**
- Brand colors, fonts, or logos (only needed for creative generation)
- Product catalogs (only needed if using `promoted_products` field)
- Tone or guidelines (only needed for AI-powered creative generation)

The brand manifest is required by design to ensure proper campaign organization and enable future capabilities, but AdCP doesn't enforce that you provide extensive brand details unless your use case requires them.
