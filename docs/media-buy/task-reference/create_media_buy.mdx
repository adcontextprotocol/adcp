...
| `frequency_cap` | MediaBuyFrequencyCap | No | Aggregate max-impression cap using one counter across packages. Requires seller `aggregate_frequency_capping` and compatible `media_buy_support` on every product; sellers reject an unsupported cap or product mix with [`UNSUPPORTED_FEATURE`](/docs/building/verification/compliance-catalog#error-code-unsupported-feature) before any mutation. Omit when executing a proposal because accepted terms are authoritative. |
