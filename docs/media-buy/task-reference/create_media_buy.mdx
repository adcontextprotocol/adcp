...
| `optimization_goal` | [OptimizationGoal](/docs/media-buy/conversion-tracking/#optimization-goals) | No | Conversion optimization goal - event source, event type, and optional target ROAS or CPA. Product must declare support in `conversion_tracking.supported_optimization_strategies`. |
...
Include the `catalog` field in a package to make it catalog-driven. The referenced catalog must already be synced via [`sync_catalogs`](/docs/media-buy/task-reference/sync_catalogs).
...
- **Inline creatives**: The `creatives` array creates NEW creatives only. To update existing creatives, use [`sync_creatives`](/docs/media-buy/task-reference/sync_creatives). To assign existing library creatives, use `creative_assignments` instead.
