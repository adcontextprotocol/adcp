...
| `format_ids` | FormatID[] | No† | v1 path: format IDs that will be used — must be supported by the product. Omit to activate all formats, or use `capability_id` (v2 path) instead. |
| `capability_id` | string | No† | v2 path: stable identifier matching a `format_options[]` entry on the product (from `ProductFormatDeclaration.capability_id`). Symmetric with the v2 path on creative manifests. See [v2 format selection](#v2-format-selection-capability_id) below. |
...
† `format_ids` and `capability_id` are mutually optional but complementary. When neither is present, all formats supported by the product are active.

### v2 format selection — `capability_id`

Products that publish `format_options[]` (the v2 path) give each format declaration an optional `capability_id` — a stable, publisher-scoped string like `"streamhaus_homepage_takeover_premium"` or `"pinnacle_reels_vertical"`. `PackageRequest.capability_id` is the write-path mirror: it lets a buyer who discovered a product via `get_products` and read a `capability_id` off a `format_options` entry use that same value at buy time, without having to translate back through `v1_format_ref[]`.

**Resolution rules:**

| Request state | Seller behavior |
|---|---|
| `capability_id` only | Look up `format_options[]` by `capability_id`; use the matching entry's `v1_format_ref[]` as the effective format set. Echo `capability_id` on the confirmed package. |
| Both `capability_id` + `format_ids` | `capability_id` wins for routing. `format_ids` is a v1-compat hint. SHOULD validate each `format_id` is in the resolved entry's `v1_format_ref[]`; MUST reject with `UNSUPPORTED_FEATURE` (`packages[N].format_ids`) on a contradicting format_id. Echo `capability_id` on the confirmed package. |
| `format_ids` only | Existing v1 behavior; unchanged. |
| Neither | All formats for the product are active; unchanged. |

**Error on miss:** If `capability_id` is not present in the product's `format_options[]`, the seller MUST reject with `UNSUPPORTED_FEATURE` and set `error.field` to `packages[N].capability_id`.

**Example — v2-native package:**

```json
{
  "product_id": "prod_homepage_takeover",
  "capability_id": "streamhaus_homepage_takeover_premium",
  "pricing_option_id": "fixed_cpm_usd",
  "budget": 50000
}
```

The seller looks up `format_options[]` for `streamhaus_homepage_takeover_premium`, resolves the format set, and echoes `capability_id: "streamhaus_homepage_takeover_premium"` on the confirmed package. The buyer's creative manifest targets the same `capability_id` at `sync_creatives` time — no v1 vocabulary needed anywhere in the flow.
...
Format specification (`format_ids` or `capability_id`) tells the publisher which creative formats the package will run, enabling them to:
...
4. create_media_buy → Specify formats (format_ids or capability_id)
