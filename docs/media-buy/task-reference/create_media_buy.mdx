...
| `daily_budget_cap` | number | No | Hard ceiling on spend per calendar day, in the media buy's currency. Orthogonal to `pacing` (the cap bounds the daily maximum; pacing governs within-day distribution). Day boundary is the seller's billing timezone (`get_account_financials.timezone`). Requires seller support: the seller declares `media_buy.budget_capping` in `get_adcp_capabilities` and rejects unenforceable caps with `UNSUPPORTED_FEATURE`. |
| `budget_cap_timezone` | string | No | IANA timezone override for the `daily_budget_cap` day boundary. Valid only when the seller advertises `media_buy.budget_capping.buyer_timezone_override: true`; otherwise rejected with `UNSUPPORTED_FEATURE`. |
...
| `daily_budget_cap` | Package | number | Optional hard ceiling on spend per calendar day (seller billing timezone). Orthogonal to `pacing`. Requires `media_buy.budget_capping` support; see [Daily budget cap](#daily-budget-cap). |
...
An optional per-package `daily_budget_cap` sets a hard ceiling on spend per calendar day. It is **orthogonal to `pacing`** — the cap bounds the daily maximum while `pacing` governs within-day distribution, so `pacing: "asap"` plus a cap is valid (spend fast, but never past the ceiling on any single day). The day boundary is the seller's billing timezone (`get_account_financials.timezone`), so cap resets align with invoice periods.
