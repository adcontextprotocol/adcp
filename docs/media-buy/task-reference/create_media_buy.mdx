...
| `proposal_id` | string | No* | ID of a committed proposal from [`finalize_proposals`](/docs/media-buy/task-reference/finalize_proposals) to execute. Alternative to providing packages. Draft proposals must be finalized first; sellers reject draft proposal execution with [`PROPOSAL_NOT_COMMITTED`](/docs/building/verification/compliance-catalog#error-code-proposal-not-committed). The legacy [`get_products`](/docs/media-buy/task-reference/get_products) finalize form remains supported throughout 3.x. |
