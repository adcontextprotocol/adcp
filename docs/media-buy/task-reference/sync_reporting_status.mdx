...
<Note>
`3.2.0-rc.2` publishes the task itself; `3.2.0-rc.3` publishes the hardening
...
`operations_contact` / `consumer_mismatch_escalation_seconds` capability
fields. Validate against the `3.2.0-rc.3` pin above; an RC.2 seller exposes the
task without those additions.
</Note>
