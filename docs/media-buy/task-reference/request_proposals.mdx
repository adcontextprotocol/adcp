...
## Binding the product universe with product_ids_exclusive

By default `criteria.product_ids` is a candidate hint: the seller may compose over other products it deems fit. Setting `criteria.product_ids_exclusive: true` makes the listed products the **complete composable universe** for the request — every purchase in every returned proposal, and every product echoed alongside the proposals, MUST reference a listed `product_id`. A brief the seller cannot satisfy within the universe returns the `rejected` outcome (or `constraint_unsatisfiable` on a later [`refine_proposals`](/docs/media-buy/task-reference/refine_proposals) revision); the seller never widens the universe on its own.

This is the contract for **delegated composition**: a requesting platform that owns the sellable catalog asks an external agent to supply only the composition judgment, and grounding becomes enforceable by construction — the requester verifies returned products are a subset of what it sent.

Sellers declaring `media_buy.supports_exclusive_product_ids` in [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities) accept the flag. Sellers that do not declare it MUST NOT silently ignore it — they reject the request at task level with [`UNSUPPORTED_FEATURE`](/docs/building/verification/compliance-catalog#error-code-unsupported-feature), so a buyer never mistakes unconstrained composition for a bounded one.
...
exception for older peers: a valid 2.5, 3.0, or 3.1 [`get_products`](/docs/media-buy/task-reference/get_products) brief may
...
- `listed_purchase` means the coordinator successfully re-read the exact
  ...
  [`buy_products`](/docs/media-buy/task-reference/buy_products) with those
...
- `legacy_create` means no truthful fence is available. The coordinator can
  ...
  [`create_media_buy`](/docs/media-buy/task-reference/create_media_buy), but
