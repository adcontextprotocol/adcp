...
By default `criteria.product_ids` is a candidate hint: the seller may compose over other products it deems fit. Setting `criteria.product_ids_exclusive: true` makes the listed products the **complete composable universe** for the request — every purchase in every returned proposal, and every product echoed alongside the proposals, MUST reference a listed `product_id`. A brief the seller cannot satisfy within the universe returns the `rejected` outcome (or `constraint_unsatisfiable` on a later `refine_proposals` revision); the seller never widens the universe on its own.
