...
if (result.success && result.data) {
  console.log(`Agent represents ${result.data.publisher_domains.length} publishers`);
  console.log(`Primary channels: ${result.data.primary_channels?.join(', ')}`);
  console.log(`Countries: ${result.data.primary_countries?.join(', ')}`);
  ...
  const expectedResponse = {
    ...
    primary_channels: ["ctv", "display", "video"],
    primary_countries: ["US", "CA", "GB"],
    portfolio_description: "Premium publisher network focused on sports and news content with engaged audiences across CTV and digital platforms.",
...
async def main():
    ...
    if hasattr(auth_result, 'errors') and auth_result.errors:
        print('Failed to get authorized properties:', auth_result.errors)
    else:
        # Step 2: Fetch property definitions from each publisher
