...
:::warning Deprecated
`list_authorized_properties` is deprecated. Use [`get_capabilities`](/docs/media-buy/task-reference/get_capabilities) instead, which provides expanded capability declaration including protocol versions, execution features, and portfolio information.

The portfolio fields from this task are available in the `portfolio` section of `get_capabilities` response.
:::
