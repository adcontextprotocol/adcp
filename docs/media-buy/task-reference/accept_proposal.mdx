...
An optional `name` sets the same human-readable trafficking-UI display label [`create_media_buy`](/docs/media-buy/task-reference/create_media_buy) accepts — it is not a covered component of `proposal_terms_digest`. When supplied, the request value wins and the seller MUST persist it and echo it unchanged on the response and subsequent [`get_media_buys`](/docs/media-buy/task-reference/get_media_buys) reads. When omitted, sellers MAY seed the MediaBuy name from the accepted proposal's own `name` (bounded to 255 characters) and MUST then report it on read surfaces like any persisted name.
