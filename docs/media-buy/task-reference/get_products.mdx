...
Use `product_selectors` with a brand to discover advertising products where your catalog items have matches at specific retailers:
...
uvx adcp \
  https://test-agent.adcontextprotocol.org/mcp \
  get_products \
  '{"brand":{"domain":"acmecorp.com"},"product_selectors":{"manifest_tags":["ketchup","organic"],"manifest_category":"food/condiments"},"filters":{"channels":["retail_media"]}}' \
  --auth 1v8tAhASaUYYp4odoQ1PnMpdqNaMiTrCRqYo9OJp6IQ
```
