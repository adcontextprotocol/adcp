...
| `omit` | boolean | No | Remove this proposal from the response. When `true`, `notes` is ignored. Default: `false` |
| `notes` | string | No | Adjustment instructions (e.g., `"shift more budget toward video"`, `"reduce total by 10%"`) |
...
| `PROPOSAL_EXPIRED` | A proposal ID in `refine.proposals` has passed its `expires_at` timestamp | Re-discover with a new `brief` or `wholesale` request |
