...
| `if_pricing_version` | string | No | Opaque `pricing_version` token from a prior response. Narrower scope than `if_catalog_version`: structure AND pricing must be unchanged. Optional — sellers that don't track pricing separately ignore this and fall back to `if_catalog_version` semantics. |
