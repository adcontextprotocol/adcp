...
| `submitted` | Custom curation queued | Wait for webhook notification if `push_notification_config` was provided, or poll `get_task_status` (legacy `tasks/get`) with `task_id` |
