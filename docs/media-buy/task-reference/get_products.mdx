...
| `fields` | string[] | No | Specific product fields to include in the response for lightweight discovery. When requesting signal targeting metadata, buyers SHOULD request `signal_targeting_allowed`, `signal_targeting_options`, and `signal_targeting_rules` so storefronts can discover inline product-specific options when present, render optional/required/fixed controls, and enforce selection limits. |
