...
| `targeting_overlay` | TargetingOverlay | No | Concrete delivery constraints the buyer expects to carry into `create_media_buy`. Prefer this over equivalent brief prose: it is compact, deterministic, and avoids lossy extraction. Returned products, pricing, and forecasts MUST account for the effective targeting. A product that cannot honor the request exactly is omitted or returned with sparse Product `targeting_resolution.modifications`; absence of Product resolution confirms exact acceptance of the structured overlay for that product. |
| `required_overlay_support` | TargetingOverlayRequirements | No | Buyer minimums for targeting dimensions whose values will be supplied on packages later. This requests selectable capability, not current values and not one product per value. The request cannot contain seller limit fields. Every returned product MUST declare `overlay_support` covering all requested fields and system-array subsets; missing or unknown requirements exclude the product. |
| `fields` | string[] | No | Specific product fields to include for lightweight discovery. `product_id` and `name` are always included. Projection never suppresses safety-critical request-specific fields: modified products include `targeting_resolution` and `expires_at`, a request with `required_overlay_support` receives `overlay_support`, and evidence-driven inclusion or ranking receives `audience_evidence_selections`. Request `audience_evidence` for the full evidence payload. When requesting signal metadata, buyers SHOULD request `included_signals` for non-selectable bundled/planned signals and the signal-targeting fields for selectable package-level signals. |
...
explicit hard targeting requirement stated only in a brief. Because the buyer
did not provide a machine-readable value to compare against, the seller SHOULD
confirm its shared interpretation once on the response:
...
the request with `INVALID_REQUEST`. This rule applies to every deprecated
...
the booked package. Because `placement_selection` represents the complete purchased
set, a product with any included placement does not declare selectable
placement overlay support; sellers expose a separate selectable product when
needed.
...
Returns an array of `products` and optionally `proposals`. When the seller
inferred hard targeting from the brief, it SHOULD include one response-level
...
| Field | Type | Description |
|-------|------|-------------|
| `product_id` | string | Opaque identifier for this configured product response. It remains stable within the same discovery/refinement context so `refine` can reference it, but MUST NOT be treated as a cross-session catalog identity. Selecting it accepts any disclosed `targeting_resolution`. |
...
| `overlay_support` | TargetingOverlaySupport | Product-scoped targeting dimensions that may be selected later. Required when the discovery request includes `required_overlay_support`. |
...
| `expires_at` | string | Expiration of this request-scoped configured product or quote. Buyers must rediscover after it expires. |
...
- **Product IDs identify opaque buyable configurations.** They remain stable only within the discovery/refinement lineage that issued them. Request-specific products set `is_custom: true`, require `expires_at`, and return `PRODUCT_EXPIRED` after that timestamp; buyers must rediscover rather than treating the ID as permanent catalog identity.
...
| Seller could not produce a usable answer because a dependency timed out or was unavailable | `SERVICE_UNAVAILABLE`, normally with `recovery: "transient"` and optional `retry_after` |
...
| `PRODUCT_NOT_FOUND` | One or more referenced product IDs are unknown | Remove invalid IDs and retry, or re-discover with a `brief` request |
| `PRODUCT_EXPIRED` | One or more referenced configured products passed `expires_at` | Re-discover with a new `brief` or `wholesale` request |
...
| `POLICY_VIOLATION` | The request itself violates an applicable policy and must be corrected | Review the structured policy details and revise the request. A seller making a deliberate business decision to decline an otherwise well-formed brief uses `status: "rejected"` instead. |
