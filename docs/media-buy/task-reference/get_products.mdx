...
|-----------|------|----------|-------------|
| `buying_mode` | string | Yes | `"brief"`, `"wholesale"`, or `"refine"`. `"brief"`: publisher curates products from the brief. `"wholesale"`: raw inventory for buyer-directed targeting, `brief` must not be provided. `"refine"`: iterate on products and proposals from a previous response using the `refine` object. |
...
| `refine` | [Refine](#refine-object) | Conditional | Refinement payload with per-product actions and per-proposal adjustment instructions. Required when `buying_mode` is `"refine"`. Must not be provided when `buying_mode` is `"brief"` or `"wholesale"`. See [Refine Object](#refine-object) below. |
...
The `refine` object consolidates all refinement intent into a single payload. At least one field is required. Each product declares an explicit action: `include` (keep with optional adjustments), `omit` (exclude), or `more_like_this` (find similar). The seller considers all fields together when composing the response.

| Field | Type | Description |
|-------|------|-------------|
| `overall` | string | Direction for the overall selection (e.g., `"more video options"`, `"increase emphasis on premium inventory"`). Guides the seller to adjust the set composition beyond specific product instructions. |
| `products` | RefineProduct[] | Per-product refinement instructions (see below) |
| `proposals` | RefineProposal[] | Per-proposal refinement instructions (see below) |
...
| `action` | string | Yes | `"include"`: return this product with updated pricing and data. `"omit"`: exclude this product from the response. `"more_like_this"`: find additional products similar to this one (the original is also returned). |
| `notes` | string | No | Adjustment instructions. For `"include"`: specific changes to request (e.g., `"add 16:9 format"`). For `"more_like_this"`: what "similar" means (e.g., `"same audience but video format"`). Ignored when `action` is `"omit"`. |
...
| `action` | string | Yes | `"include"`: return this proposal with updated allocations and pricing. `"omit"`: exclude this proposal from the response. |
| `notes` | string | No | Adjustment instructions (e.g., `"shift more budget toward video"`, `"reduce total by 10%"`). Ignored when `action` is `"omit"`. |
...
After initial discovery, use `buying_mode: "refine"` to iterate on specific products and proposals. The `refine` object declares what to do with each product: keep it, drop it, or find more like it. The seller returns updated products with revised pricing and configurations. See [Refinement](/docs/media-buy/product-discovery/refinement) for the full guide.

### Refine products

Each product in `refine.products` declares an explicit action. Use `overall` for set-level direction. Filters contain absolute values (not deltas), so `budget_range` is the new target budget:
...
```json test=false
{
  ...
  "refine": {
    ...
    "products": [
      { "product_id": "prod_premium_video", "action": "include", "notes": "add 16:9 format option" },
      { "product_id": "prod_display_run_of_site", "action": "omit" },
      { "product_id": "prod_native_feed", "action": "more_like_this", "notes": "same audience but video format" }
    ]
  },
  "filters": {
...
Reference proposals by ID in `refine.proposals` to request adjustments:
...
```json test=false
{
  ...
  "refine": {
    "products": [
      { "product_id": "prod_premium_video", "action": "include" },
      { "product_id": "prod_display_run_of_site", "action": "include" }
    ],
    "proposals": [
      { "proposal_id": "prop_awareness_q2", "action": "include", "notes": "shift more budget toward video, reduce display allocation" }
    ]
  },
  "filters": {
    "start_date": "2026-04-01",
...
```json test=false
{
  ...
  "refine": {
    "products": [
      { "product_id": "prod_premium_video", "action": "more_like_this", "notes": "same premium audience but different formats" }
    ]
  }
}
```
...
The response uses the same format as discovery. Each product reflects the updated parameters — revised pricing options and any configuration changes the seller recommends based on the `refine` object and filters. Call refine as many times as needed before committing to a `create_media_buy`.
...
- **Valid actions**: Each product entry requires `product_id` and `action` (`include`, `omit`, or `more_like_this`). Each proposal entry requires `proposal_id` and `action` (`include` or `omit`).
