...
| `audience_evidence_requirements` | AudienceEvidenceRequirement[] | No | Required or preferred rules for compact product audience evidence. A required rule fails closed when no matching evidence exists; a preferred rule affects ranking without excluding products. Successful responses acknowledge evaluation with `audience_evidence_requirements_applied: true`. |
...
Buyers can request products with a minimum age-composition estimate and acceptable evidence bases:

<!-- Using latest because audience evidence is not yet released in a versioned schema. Update after the next release. -->
```json
{
  ...
  "audience_evidence_requirements": [
    {
      ...
      "minimum_share": 0.75,
      "accepted_bases": ["verified", "declared", "population_estimate"],
...
All `required` rules use AND semantics: every rule must be satisfied by at least one evidence row. A row matches when its canonical age range is exactly equal, its `estimated_share` is greater than or equal to `minimum_share` when supplied, and its basis is accepted. For verified evidence, every `verification_methods` value on the row must appear in the buyer's `accepted_verification_methods` when that constraint is supplied; a mere intersection is insufficient. Products without matching evidence do not satisfy a required rule, and absence is never treated as proof. `preferred` rules inform seller ranking but do not remove products. Sellers that cannot evaluate the field reject the request with `UNSUPPORTED_FEATURE` rather than silently ignoring it. A completed response to a request containing evidence requirements MUST include `audience_evidence_requirements_applied: true`.

When the buyer manually purchases a selected product, it copies the matching compact row into `packages[].audience_evidence_selected`. The seller validates that row against the current product and echoes it immutably as `audience_evidence_used`. Seller-authored proposals persist the rows they used and provide the same readback without requiring the buyer to reconstruct the selection state.
...
| `audience_evidence` | AudienceEvidence[] | (Optional) Compact age-composition evidence: age range, estimated share, and basis (`verified`, `declared`, `inferred`, or `population_estimate`). Evidence supports planning and ranking and never implies user-level targeting capability. |
...
| `audience_evidence_requirements_applied` | boolean | Present and `true` when the request supplied `audience_evidence_requirements` and the seller evaluated every rule. It is never emitted as `false`. |
