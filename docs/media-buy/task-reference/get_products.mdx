...
|-----------|------|----------|-------------|
| `buying_mode` | string | Yes | `"brief"`, `"wholesale"`, or `"refine"`. `"brief"`: publisher curates products from the brief. `"wholesale"`: raw inventory for buyer-directed targeting, `brief` must not be provided. `"refine"`: iterate on known products or proposals — requires at least one of `product_ids` or `proposal_id`. Use `feedback` to provide per-item and overall adjustment instructions. |
...
| `feedback` | [Feedback](#feedback-object) | No | Structured refinement feedback with per-product and per-proposal adjustment instructions. Only valid when `buying_mode` is `"refine"`. See [Feedback Object](#feedback-object) below. |
...
Structured refinement feedback for `refine` mode. All fields are optional — provide at least one. Use `overall` for set-level composition changes (e.g., "more video options"). Use per-item entries for targeted adjustments to specific products or proposals. The seller considers both.

Only include items you want to omit or adjust — unlisted items are returned as-is with updated forecasts. Product IDs in `feedback.products` should reference products from `product_ids` or from the referenced proposal. Proposal IDs in `feedback.proposals` should reference the top-level `proposal_id`.

| Field | Type | Description |
|-------|------|-------------|
| `overall` | string | Feedback about the overall selection (e.g., `"good selection but I want more video options"`). Guides the seller to adjust the set composition. |
| `products` | ProductFeedback[] | Per-product feedback (see below) |
| `proposals` | ProposalFeedback[] | Per-proposal feedback (see below) |

#### ProductFeedback

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `product_id` | string | Yes | Product ID from the previous `get_products` response |
| `omit` | boolean | No | Remove this product from the response. Default: `false` |
| `notes` | string | No | Adjustment instructions (e.g., `"increase budget allocation"`, `"add 16:9 format"`). Ignored when `omit` is `true`. |

#### ProposalFeedback

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `proposal_id` | string | Yes | Proposal ID from the previous `get_products` response |
| `omit` | boolean | No | Remove this proposal from the response. Default: `false` |
| `notes` | string | No | Adjustment instructions (e.g., `"shift more budget toward video"`, `"reduce total by 10%"`). Ignored when `omit` is `true`. |
...
### Refine products with feedback

Use `feedback` to provide structured per-item and overall adjustment instructions. Filters contain absolute values (not deltas), so `budget_range` is the new target budget:

```json test=false
{
  "buying_mode": "refine",
  "product_ids": ["prod_premium_video", "prod_display_run_of_site", "prod_native_feed"],
  "feedback": {
    "overall": "good selection but I want more video options",
    "products": [
      { "product_id": "prod_native_feed", "omit": true },
      { "product_id": "prod_premium_video", "notes": "increase budget allocation to 60%" }
    ]
  },
  "filters": {
...
```json test=false
{
  ...
  "feedback": {
    "proposals": [
      { "proposal_id": "prop_awareness_q2", "notes": "shift more budget toward video, reduce display allocation" }
    ]
  },
  "filters": {
    "start_date": "2026-04-01",
...
Sellers MAY return proposals alongside refined products, even when the buyer only provided `product_ids` (without a `proposal_id`). For example, a buyer who refines three products may receive those products back with updated forecasts *and* a proposal suggesting how to combine them.
...
- **Signal interest via `feedback.overall`.** Include a message like `"I like these products — suggest how to combine them"` to indicate you'd welcome a proposal.
...
- **`product_ids`, `proposal_id`, and `feedback` are only valid in `refine` mode.** Requests that include these fields in `brief` or `wholesale` mode are rejected with `INVALID_REQUEST`.
...
- **Omit consistency**: Products omitted via `feedback.products[].omit: true` should still be included in `product_ids` — the seller needs to know which product to exclude. Alternatively, simply remove the product from `product_ids` for the same effect.
...
The `@adcp/client` library validates responses against Zod schemas automatically. For request-side validation, use `GetProductsRequestSchema` from `@adcp/client` to catch structural errors before sending.
...
Use `buying_mode: "refine"` with `product_ids` and reference specific previews by `generated_creative_ref` in `feedback`:
...
```json test=false
{
  ...
  "feedback": {
    "overall": "gen_sr_eng_sf is good. gen_staff_eng_la needs shorter headline and more prominent salary range. Use the dark logo variant on all creatives."
  }
}
```
