...
| `unchanged` | boolean | `true` ONLY when the request carried `if_catalog_version` (and/or `if_pricing_version`) matching the seller's current version, in which case `products[]` MUST be omitted; `catalog_version` (echoed) MUST still be present. Buyers receiving `unchanged: true` MUST NOT mutate their local catalog mirror. |
...
Response (catalog changed — full payload returned):
...
- **Pagination interaction.** `catalog_version` describes the catalog as a whole, not individual pages. When the catalog mutates between pages, sellers SHOULD return the new `catalog_version` on each page (RECOMMENDED) or snapshot the catalog at the start of pagination. Buyers receiving a `catalog_version` change mid-pagination SHOULD restart pagination from `cursor: null`.
