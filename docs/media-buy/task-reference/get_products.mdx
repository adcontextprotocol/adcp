...
| `unchanged` | boolean | `true` ONLY when the request carried `if_catalog_version` (and/or `if_pricing_version`) matching the seller's current version for the buyer's `cache_scope`, in which case `products[]` MUST be omitted; `catalog_version`, `cache_scope`, and `pricing_version` (when used) MUST still be echoed. `unchanged: false` is permitted as an explicit no-op affirmation but MUST be accompanied by `products[]`. Absent `unchanged` is equivalent to `false`. Buyers receiving `unchanged: true` MUST NOT mutate their local catalog mirror. |
