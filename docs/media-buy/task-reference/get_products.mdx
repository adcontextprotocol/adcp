...
| `fields` | string[] | No | Specific product fields to include in the response for lightweight discovery. When requesting signal targeting metadata, buyers SHOULD request `signal_targeting_allowed`, `signal_targeting_options_source`, `signal_targeting_options`, and `signal_targeting_rules` so storefronts can discover the menu source, render optional/required/fixed controls, and enforce selection limits. |
...
| `signal_targeting_options_source` | string | (Optional) Where buyers discover selectable signals: `"product"` for inline `signal_targeting_options`, `"get_signals"` for the seller's `get_signals` feed, or `"product_and_get_signals"` for product-specific overrides plus the broader feed. |
