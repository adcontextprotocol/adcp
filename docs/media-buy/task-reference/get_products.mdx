...
The `brief` in refine mode describes what changed — the seller does not need the original brief. Filters contain absolute values (not deltas), so `budget_range` is the new target budget:
