...
| `fields` | string[] | No | Specific product fields to include for lightweight discovery. `product_id` and `name` are always included. Projection never suppresses safety-critical request-specific fields: modified products include `targeting_resolution` and `expires_at`, and a request with `required_overlay_support` receives `overlay_support`. |
...
| `places` | object[] | Deprecated. Use `targeting_overlay.geo_places` for known catalog-backed place IDs or `required_overlay_support.geo_places` for future selection. |
...
| `required_geo_targeting` | object[] | Deprecated. Use `required_overlay_support`. Legacy place entries use `{ level: "place", country, system, place_type, system_version? }`. |
...
does not guarantee value-specific inventory before the IDs are provided.
