...
| `sponsored_placement_types` | string[] | _WG review draft (#5270)._ Filter catalog-driven retail-media products by acceptable declared sponsored-placement types: `sponsored_search`, `sponsored_display`, or `sponsored_native`. Sellers should return only products they can satisfy with at least one requested type, and should exclude mixed, non-targetable bundles unless delivery can be constrained to the requested type. |
| `social_placement_surfaces` | string[] | _WG review draft (#5278)._ Filter social products by acceptable declared social-placement surfaces: `feed`, `stories`, `reels`, `shorts`, `spotlight`, `explore`, `search`, or `marketplace`. Sellers should return only products they can satisfy with at least one requested surface, and should exclude mixed, non-targetable bundles unless delivery can be constrained to the requested surface. |
...
| `sponsored_placement_types` | _WG review draft (#5270)._ Declared sponsored-placement types for catalog-driven retail-media inventory. Concrete placements usually declare one value; aggregate placements may declare multiple. |
| `social_placement_surfaces` | _WG review draft (#5278)._ Declared social-placement surfaces for social inventory. Concrete placements usually declare one value; aggregate placements may declare multiple. |
