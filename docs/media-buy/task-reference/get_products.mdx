...
| `required_vendor_metrics` | object[] | Filter to products whose `reporting_capabilities.vendor_metrics` covers vendor-defined metrics (Adelaide attention, Scope3 emissions, Nielsen DAR demographics, etc.). Each entry pins a `vendor` (BrandRef), `metric_name`, or `metric_category` — at least one of the three. Use `metric_category` (e.g., `"attention"`, `"emissions"`) for cross-vendor discovery, since vendors rarely converge on `metric_name` strings before standardization. Same filter-not-fail semantics as `required_metrics`. |
