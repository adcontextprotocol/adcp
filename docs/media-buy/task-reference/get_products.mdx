...
once on the response, not on each product. Every request-specific configured product still carries
`is_custom: true` and `expires_at`, including exact products without a
resolution echo. If a product offers a different executable structured
...
| `is_custom` | boolean | `true` for a request-specific configured offer. Custom products always include `expires_at`, including exact targeting configurations with no Product resolution echo. |
