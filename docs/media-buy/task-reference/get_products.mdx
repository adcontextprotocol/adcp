...
|-----------|------|----------|-------------|
| `buying_mode` | string | Yes* | `"brief"` or `"wholesale"`. When `"brief"`, `brief` is required. When `"wholesale"`, publisher returns raw inventory for buyer-directed targeting, `brief` must not be provided, and proposals are omitted. Not required when `product_ids` is provided. |
| `brief` | string | No | Natural language description of campaign requirements. Required when `buying_mode` is `"brief"`. Must not be provided when `buying_mode` is `"wholesale"`. When combined with `product_ids`, provides context for forecast refinement (e.g., "Update forecast for $100k budget targeting California"). |
| `product_ids` | string[] | No* | Specific product IDs from a previous `get_products` response. Skips discovery and returns only these products with updated forecasts. Combine with `brief` and/or `filters` to specify different budget, dates, or targeting for forecast generation. |
...
*Either `buying_mode` or `product_ids` must be provided. When `product_ids` is present, `buying_mode` is not required.
...
uvx adcp \
  https://test-agent.adcontextprotocol.org/mcp \
  get_products \
  '{"brand":{"domain":"acmecorp.com"},"catalog":{"type":"product","tags":["ketchup","organic"],"category":"food/condiments"},"filters":{"channels":["retail_media"]}}' \
  --auth 1v8tAhASaUYYp4odoQ1PnMpdqNaMiTrCRqYo9OJp6IQ
```
...
After initial discovery, use `product_ids` to refresh forecasts for known products with different parameters. This skips the discovery step and goes straight to forecast generation.

```json test=false
{
  "product_ids": ["prod_premium_video", "prod_display_run_of_site"],
  "brief": "Update forecast for $100k budget, April 1-30, targeting California and New York",
  "filters": {
    "start_date": "2026-04-01",
    "end_date": "2026-04-30",
    "budget_range": { "min": 100000, "max": 100000, "currency": "USD" }
  }
}
```

The response uses the same product format as discovery — each product includes an updated `forecast` with delivery points, method, and currency.
