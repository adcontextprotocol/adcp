...
or an incompatible product mix atomically with [`UNSUPPORTED_FEATURE`](/docs/building/verification/compliance-catalog#error-code-unsupported-feature)
...
The media-buy `daily_budget_cap` is a hard aggregate ceiling and does not allocate its value among purchases. A purchase-level budget cap is an optional subordinate ceiling, not a reservation. Every budget cap shares the accepted media-buy `budget_cap_timezone`; sellers reject undeclared budget-cap scopes with [`UNSUPPORTED_FEATURE`](/docs/building/verification/compliance-catalog#error-code-unsupported-feature).

An optional `name` sets the same human-readable trafficking-UI display label [`create_media_buy`](/docs/media-buy/task-reference/create_media_buy) accepts. When supplied, the seller MUST persist it and echo it unchanged on the response and subsequent [`get_media_buys`](/docs/media-buy/task-reference/get_media_buys) reads. It is not an identifier or financial reference.
