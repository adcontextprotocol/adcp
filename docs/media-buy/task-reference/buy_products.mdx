...
An optional `name` sets the same human-readable trafficking-UI display label [`create_media_buy`](/docs/media-buy/task-reference/create_media_buy) accepts. When supplied, the seller MUST persist it and echo it unchanged on the response and subsequent `get_media_buys` reads. It is not an identifier or financial reference.
