...
{/* Using latest because these 3.2 delivery-reporting changes are not yet released.
    Update to the v3 alias after the next release. */}
...
| `aggregated_totals` | **Deprecated in 3.2; removed in 4.0.** Legacy cross-buy aggregate. Sellers should omit it and buyers should calculate only aggregates whose row currencies and metric semantics are compatible. |
...
See [schema](https://adcontextprotocol.org/schemas/latest/media-buy/get-media-buy-delivery-response.json) for the complete field list.
...
The response-wide `currency` and `aggregated_totals` fields are deprecated in AdCP 3.2 and removed in 4.0. One response can contain media buys in different currencies, and AdCP does not define exchange rates, conversion timestamps, or an FX authority. The response-wide currency therefore MUST NOT be interpreted as an aggregation currency or as evidence that the seller converted package spend.
