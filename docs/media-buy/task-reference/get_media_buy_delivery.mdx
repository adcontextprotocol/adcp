...
| **Reach Window** | Window semantics for reported reach/frequency: `cumulative` (rolling since campaign start, do not sum) or `period` (uniques within reporting window only). Optional but strongly recommended when reach is present. |
| **Frequency** | Average ad exposures per reach unit, measured over `reach_window` |
| **Viewed Seconds** | Average seconds in view per impression — reporting-side counterpart to the `viewed_seconds` optimization goal |
| **Attention Seconds** | Average vendor-attributed attention time per impression — reporting-side counterpart to the `attention_seconds` optimization goal |
| **Attention Score** | Vendor-specific attention score per impression — reporting-side counterpart to the `attention_score` optimization goal |
