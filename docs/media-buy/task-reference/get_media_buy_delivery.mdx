...
**Qualifier vocabulary** is closed today on both `committed_metrics` and `metric_aggregates` (`additionalProperties: false`). Two keys exist: `viewability_standard` (MRC vs GroupM viewability disambiguation) and `completion_source` (seller-attested vs vendor-attested `completion_rate` disambiguation). The delivery vocabulary is **expected to diverge from contract** in future minors as transparency disclosures buyers don't commit to ship delivery-only (e.g., `tracker_firing` pending #3832). New qualifier keys ship explicitly in subsequent minors on either surface.
