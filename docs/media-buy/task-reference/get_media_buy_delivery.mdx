...
**Unqualified metrics stay top-level.** `impressions`, `spend`, `media_buy_count`, and other metrics with no qualifier remain at the top of `aggregated_totals`. `metric_aggregates` is only used for metrics with non-empty qualifier sets — avoids duplicate sources of truth and the trivial-case walk.

**Qualifier vocabulary** is closed at the contract layer (`viewability_standard` only today) and a deliberate **superset** at the delivery layer (reserves space for transparency disclosures buyers don't commit to, e.g., `tracker_firing` pending #3832). New qualifier keys ship explicitly in subsequent minors.
