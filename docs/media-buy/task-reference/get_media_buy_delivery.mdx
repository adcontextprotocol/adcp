...
*Either `media_buy_ids` can be provided. If neither provided, returns all media buys in current session context.
...
- If neither `media_buy_ids` nor `media_buy_ids` provided, returns all media buys from current session context
...
| `CONTEXT_REQUIRED` | No media buys in context | Provide media_buy_idss explicitly |
