...
**Broadcast TV**: Data freshness differs significantly for linear TV. Measurement data matures through defined windows (Live, C3, C7) as DVR playback accumulates. Final C7 numbers arrive ~15 days after broadcast. Products with `reporting_capabilities.measurement_windows` declare these timelines. Buyers reconcile against the `measurement_window` specified in `billing_measurement` on the agreed terms. See [Broadcast TV creative formats](/docs/creative/channels/broadcast) for full details.
