...
`supports_geo_breakdown` and `supports_spot_breakdown` are structured capability objects; the other declarations in this table are boolean flags. `supports_spot_breakdown.available_metrics` lists metrics available at spot grain. An empty array means the product supplies as-run airing evidence but no per-spot delivery metrics.
