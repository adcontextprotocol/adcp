...
| `reporting_dimensions` | object | No | Request dimensional breakdowns within `by_package`. Keys: `geo`, `device_type`, `device_platform`, `audience`, `placement`. Each accepts optional `limit` and `sort_by` (default: `spend` descending). Geo also requires `geo_level` and optionally `system`. Unsupported dimensions are silently omitted; malformed dimension requests (e.g., geo without `geo_level`) return a validation error. |
...
Each dimension accepts optional `limit` (max rows) and `sort_by` (metric to sort by, default `spend` descending). Geo also requires `geo_level` (`country`, `region`, `metro`, `postal_area`) and `system` for metro/postal levels. Unsupported dimensions are silently omitted from the response, but malformed requests (e.g., geo without `geo_level`) return a validation error. Breakdowns are per-dimension only — cross-dimensional intersections (e.g., device_type × geo) are not supported.
...
Each breakdown array has a sibling boolean flag (e.g., `by_geo_truncated`). When `true`, additional rows exist beyond the returned set. Rows are sorted by the requested `sort_by` metric descending.
