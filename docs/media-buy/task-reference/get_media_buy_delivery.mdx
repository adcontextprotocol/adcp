...
| `reporting_dimensions` | object | No | Request dimensional breakdowns within `by_package`. Keys: `geo`, `device_type`, `device_platform`, `audience`, `placement`. Each accepts optional `limit` and `sort_by` (default: `spend` descending). Geo also requires `geo_level` and optionally `system`. Unsupported dimensions are silently omitted. |
...
Each dimension accepts optional `limit` (max rows) and `sort_by` (metric to sort by, default `spend` descending). Geo also requires `geo_level` (`country`, `region`, `metro`, `postal_area`) and `system` for metro/postal levels.
