...
|-----------|------|----------|-------------|
| `account` | [account-ref](/docs/building/by-layer/L2/accounts-and-agents#account-references) | No | Account reference. Pass `{ "account_id": "..." }` or `{ "brand": {...}, "operator": "..." }` if the seller supports implicit resolution. Only returns media buys belonging to this account. When omitted, returns data across all accessible accounts. |
...
<Note>
**3.1 vocabulary note.** `get_media_buy_delivery` returns the lifecycle state on a nested `media_buy_deliveries[].status` field (no envelope collision — nested at depth 1). [`create_media_buy`](/docs/media-buy/task-reference/create_media_buy) and [`update_media_buy`](/docs/media-buy/task-reference/update_media_buy) success responses return the same lifecycle state on a top-level **`media_buy_status`** field (added in 3.1 to avoid colliding with the envelope task-status `status`). Same enum, two field names in 3.1 — the cascade unifies in 4.0 ([#4905](https://github.com/adcontextprotocol/adcp/issues/4905)). See [Migration › `media_buy_status`](/docs/reference/migration/media-buy-status) for the full picture.
</Note>
...
Whose number invoices the buy is a **contract term**, declared in the buy's [`measurement_terms.billing_measurement`](/docs/media-buy/advanced-topics/billing-authority):
...
- **Vendor-attested** (third-party measurement vendor named in the buy — e.g., Nielsen, IAS, DV, MOAT): invoice off the named vendor's authoritative numbers. Operationally this is most often the seller pulling from the vendor and publishing on `get_media_buy_delivery` with `is_final: true`; when the buyer holds the vendor relationship instead, the buyer pushes via [`report_usage`](/docs/accounts/tasks/report_usage) with `final: true` and `finalized_at` set.
...
- Context established by previous operations (e.g., [`create_media_buy`](/docs/media-buy/task-reference/create_media_buy))
...
- Products with `date_range_support: "lifetime_only"` reject requests that include `start_date`/`end_date` with an [`UNSUPPORTED_FEATURE`](/docs/building/verification/compliance-catalog#error-code-unsupported-feature) error
...
| [`AUTH_INVALID`](/docs/building/verification/compliance-catalog#error-code-auth-invalid) | Credentials rejected (expired / revoked) | Human credential rotation required; do not auto-retry |
| [`MEDIA_BUY_NOT_FOUND`](/docs/building/verification/compliance-catalog#error-code-media-buy-not-found) | Media buy doesn't exist | Verify `media_buy_id`; for legacy correlation use [`get_media_buys`](/docs/media-buy/task-reference/get_media_buys) + `context.internal_campaign_id` |
| [`INVALID_REQUEST`](/docs/building/verification/compliance-catalog#error-code-invalid-request) | A date string or filter violates its schema constraints | Use YYYY-MM-DD dates and a documented status value |
| [`VALIDATION_ERROR`](/docs/building/verification/compliance-catalog#error-code-validation-error) | Only one date is present, or `end_date` is not after `start_date` | Supply both dates with `start_date < end_date`, or omit both for lifetime delivery |
| [`UNSUPPORTED_FEATURE`](/docs/building/verification/compliance-catalog#error-code-unsupported-feature) | Product only supports lifetime reporting | Omit `start_date` and `end_date`. Check `reporting_capabilities.date_range_support` on the product. |
| [`UNSUPPORTED_GRANULARITY`](/docs/building/verification/compliance-catalog#error-code-unsupported-granularity) | Requested `time_granularity` is not in the product's `windowed_pull_granularities` | Re-issue with a granularity from `error.details.supported_granularities`, or omit `time_granularity` to fall back to cumulative date-range pulls. See [Windowed pull recovery](#windowed-pull-recovery). |
...
- **Capability-scoped MUST** — sellers MUST honor `time_granularity` requests for any value in `windowed_pull_granularities`. Pulls outside the declared set return [`UNSUPPORTED_GRANULARITY`](/docs/building/verification/compliance-catalog#error-code-unsupported-granularity).
...
This surface anchors [snapshot-and-log](/docs/protocol/snapshot-and-log) Rule 4 (either path is complete) for data-bearing events. See that page for the broader contract.
...
Geo, device, format, audience, demographic, placement, property, collection, installment, collection_property, placement_property, creative, keyword, and catalog_item dimensions accept optional `limit`, `sort_by` (any value from the `sort-metric` enum; defaults to `spend`), and `sort_direction` (`asc` or `desc`; defaults to `desc`, largest first). The seller echoes the sort it actually applied in the corresponding `by_*_sorted_by` and `by_*_sort_direction` fields. For every requested dimension, sellers MUST return both echo fields and the `by_*_truncated` flag whenever its breakdown array is present. Geo requires `geo_level`; demographic accepts optional canonical `age_ranges`; spot accepts only `limit` and is ordered by `aired_at`. Most arrays are independent one-dimensional marginals. `collection_property` and `placement_property` are deliberate intersections that prove which property carried the collection or placement.
...
| Placement × property | `by_placement_property` | `placement_id`, `publisher_domain`, `identifier`, `impressions`, `spend` | `placement_identity`, `property_ref`, convenience names | `supports_placement_property_breakdown` |
...
Placement identity names its authority. `placement_identity.kind: "publisher_ref"` carries the publisher `adagents.json` domain; `kind: "seller_inline"` carries the defining `seller_agent`. The flat `placement_id` remains required for 3.1 consumers. New 3.2 sellers MUST add `placement_identity`; legacy rows without it remain valid but are only package-scoped. `placement_name` is convenience metadata.

Every property row includes `publisher_domain` and an operational `identifier` such as a domain, app bundle, or channel ID. Their tuple is the stable operational key even for unregistered surfaces. When the surface resolves to the publisher catalog, optional `property_ref` adds `{publisher_domain, property_id}` and MUST use the same domain. Collection identity is canonical through `collection_ref`; `installment_ref` nests it. Independent marginals do not establish correlation. `by_collection_property` and `by_placement_property` are affirmative delivery evidence for returned intersections; catalog declarations establish only eligibility or carriage. When an intersection array is truncated or suppressed, the absence of a row does not prove non-delivery.

Sellers with a privacy or measurement floor declare `reporting_capabilities.property_breakdown_min_impressions`; absence means no floor. The floor applies to `by_property`, `by_collection_property`, and `by_placement_property`. Each returned array has a `by_*_suppressed` flag, separate from `by_*_truncated`, so buyers can distinguish threshold omissions from top-N limiting.
...
|--------|-------------|-------------|
| `synced` | Buyer's first-party data via [`sync_audiences`](/docs/media-buy/task-reference/sync_audiences) | Yes — use `audience_include`/`audience_exclude` |
...
2. Report outcomes to the governance agent via [`report_plan_outcome`](/docs/governance/campaign/tasks/report_plan_outcome)
3. The governance agent compares actual delivery against planned parameters (drift detection)
4. Validate property delivery via [`validate_property_delivery`](/docs/governance/property/tasks/validate_property_delivery) to catch unauthorized supply paths

| Governance task | Purpose |
|----------------|---------|
| [`report_plan_outcome`](/docs/governance/campaign/tasks/report_plan_outcome) | Feed delivery data to the governance agent for budget tracking and drift detection |
| [`validate_property_delivery`](/docs/governance/property/tasks/validate_property_delivery) | Validate delivery records against property lists — catches ads running on unauthorized properties |
| [`validate_content_delivery`](/docs/governance/content-standards/tasks/validate_content_delivery) | Validate content artifacts against brand suitability standards |
| [`get_plan_audit_logs`](/docs/governance/campaign/tasks/get_plan_audit_logs) | View the full plan state and audit trail |
...
- [Async Operations](/docs/building/by-layer/L3/async-operations) - Async patterns and status handling
