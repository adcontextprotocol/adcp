...
- **`vendor_metric_values`**: Reported values for vendor-defined metrics that the product's `reporting_capabilities.vendor_metrics` declared (Adelaide attention, Scope3 emissions, Nielsen DAR demographics, etc.). Each entry carries `{ vendor, metric_name, value, unit?, measurable_impressions?, breakdown? }`. The `measurable_impressions` field is the coverage denominator — vendor measurement is rarely 100% (Adelaide only scores impressions where their SDK fires, Nielsen DAR only matches panel-resolved impressions). Buyers compute coverage as `measurable_impressions / impressions`. When a declared vendor metric is omitted entirely from this array, infer no measurement happened (zero coverage).
