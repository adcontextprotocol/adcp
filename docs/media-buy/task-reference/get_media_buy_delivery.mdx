...
| **Viewability** | Object with `viewable_rate`, `viewed_seconds` (average in-view duration per measurable impression — pairs with the `viewed_seconds` optimization goal), `vendor`, `standard`, and `measurable_impressions` denominator |
