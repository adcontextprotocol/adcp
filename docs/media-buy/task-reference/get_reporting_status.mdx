...
The push doorbell is **optional in every tier**: a polling-only seller that
answers `get_reporting_status` truthfully is fully Core-conformant, and buyers
MUST treat polling as the authoritative recovery path regardless of whether
the doorbell is offered.
