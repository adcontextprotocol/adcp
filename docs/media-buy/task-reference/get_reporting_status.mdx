...
Current source adds opt-in consumer-status sync and readback. Sellers
implementing that extension advertise `consumer_status_task`; it becomes
required Core only in the next eligible minor after the notice window. Sellers declare
...
Current unreleased source adds `consumer_statuses[]` for sellers advertising
`consumer_status_task` and includes those immutable records in flat pagination
and `changes_after` checkpoints.
</Note>
...
| `action_required` | An SLA or retry boundary was crossed, a Reconciled Billing receipt is missing/rejected, **or** the authenticated buyer reports that seller-healthy reporting is omitted, missing, or unreadable; `issues[]` supplies the human action. |
...
- A **consumer status** is the authenticated buyer's immutable statement that an
  ...
  revision, or unreadable. It remains separately attributed and can identify a
...
conflicts, this caller/account view becomes `action_required` with a stable
`CONSUMER_STATUS_MISMATCH` issue.
The issue points at `reporting_status_id` and names the diagnosed
`responsible_party`. Neither claim overwrites the other.

A conflict includes `obligation_missing`, `revision_missing`, or `unreadable`,
and also `received` naming an older revision after the seller publishes a newer
required restatement. This is how the seller can see that reporting changed
after the buyer already read it.
