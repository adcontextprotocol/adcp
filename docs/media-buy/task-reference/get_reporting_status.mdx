...
{/* Using latest because Reliable Reporting 1.0 is not yet in a released schema bundle. */}
...
| `periods` | Enumerate a flat cursor-paginated ledger of obligations, retained revisions, post-official adjustments, materialization attempts, and consumer receipts. Use `changes_after` for incremental repair. |
...
The periods view keeps five facts distinct:
...
- A **revision** is one immutable logical publication. It is destination-independent,
  ...
  lineage. An official revision is the immutable, invoice-addressable close.
- An **adjustment** is one immutable post-official correction. It leaves the
  original invoice and revision intact and carries signed control-total deltas
  into a later open accounting period.
...
only relevant record, and must never rewrite an invoice bound to an official
revision when applying an adjustment.
...
`revisions[]`, `adjustments[]`, `materializations[]`, and `receipts[]`, so one
...
are present, and—when configured—the authenticated consumer receipt for that revision
is accepted. The invoice stores that exact `reporting_revision_id`; later corrections
are adjustments in a later accounting period.
