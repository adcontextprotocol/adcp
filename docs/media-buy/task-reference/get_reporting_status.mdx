...
`3.2.0-rc.2` adds `consumer_statuses[]` for sellers advertising
`consumer_status_task` and includes those immutable records in flat pagination
and `changes_after` checkpoints. Still unreleased in current source:
...
`operations_contact` capability fields.
</Note>
