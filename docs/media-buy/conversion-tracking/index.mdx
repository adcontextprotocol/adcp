...
**Three preconditions for goal acceptance.** Sellers MUST reject `vendor_metric` goals that fail any of:

1. **Discovery** — `metric_id` is in the vendor's published `measurement.metrics[]` catalog (queried from the vendor's `brand.json` measurement agent).
...
**Full lifecycle reference.** See [The metric lifecycle](/docs/media-buy/media-buys/optimization-reporting#the-metric-lifecycle---from-capability-to-reporting) for the complete capability → commitment → optimization → delivery picture across both standard and vendor metric flows — same `(vendor, metric_id)` key on every surface.
...
| Target emissions (vendor-bound) | Minimize carbon-per-impression from a specific provider | `kind: "vendor_metric"`, `vendor: { domain: "scope3.com" }`, `metric_id: "gco2e_per_impression"`, no target (maximize-inverse) or `target: { kind: "threshold_rate", value: 0.5 }` for a ceiling-style floor |
