...
description: Configure event sources, send conversion events, match users for attribution, and optimize delivery toward cost, threshold rate, and return-on-spend goals.
testable: true
keywords: [conversion tracking, event sources, attribution, optimization goals, user matching, event logging, cost_per, per_ad_spend, threshold_rate]
---
...
Optimize for advertiser-tracked conversion events. The `event_sources` array defines which source-type pairs feed this goal. The seller deduplicates by `event_id` across all entries — the same business event reported by multiple sources (e.g., web analytics and an MMP) counts once.
...
| `event_sources` | array | Yes | Source-type pairs feeding this goal. Seller deduplicates by `event_id` across entries. |
...
| `event_sources[].value_field` | string | When target is `per_ad_spend` | Field on `custom_data` carrying the monetary value. Must be numeric, in buy currency. |
| `event_sources[].value_factor` | number | No | Multiplier for value_field (default 1). Use -1 for refunds, 0.01 for cents. |
...
Optimize for a seller-tracked delivery metric. No event source needed — the seller tracks these natively.
...
|---|---|---|---|
| `clicks` | count/impression | 0.001 (0.1% CTR) | Click-throughs |
| `views` | count/impression | 0.70 (70% viewability) | Viewable impressions |
| `completed_views` | count/impression | 0.85 (85% VCR) | Video completions |
...
| `per_ad_spend` | — | ROAS, profit per spend, etc. | `sum(value_field * value_factor) / spend` |
