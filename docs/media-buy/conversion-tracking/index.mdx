...
description: Configure event sources, send conversion events, match users for attribution, and optimize delivery toward cost, rate, and return-on-spend goals.
testable: true
keywords: [conversion tracking, event sources, attribution, optimization goals, user matching, event logging, cost_per, per_ad_spend, rate]
---
...
Optimization goals tell the seller what to optimize delivery toward. Set them on a package in [`create_media_buy`](/docs/media-buy/task-reference/create_media_buy#campaign-with-conversion-optimization). A package accepts an array of goals — each with an optional `priority` (1 = highest).
...
Optimize for an advertiser-tracked conversion event. Requires an event source registered via `sync_event_sources`.
...
**Return on ad spend** (requires `value_field`):
...
The `value_field` tells the seller which field on each event's `custom_data` carries the monetary value. For `per_ad_spend` targets, the seller calculates `sum(value_field) / spend` and optimizes toward the target ratio.
...
| `event_type` | [EventType](#event-types) | Yes | Event type to optimize for (e.g., `purchase`, `lead`, `app_install`) |
| `value_field` | string | When target is `per_ad_spend` | Field on `custom_data` carrying the monetary value (e.g., `value`, `order_total`, `profit_margin`) |
| `target.kind` | `"cost_per"` \| `"per_ad_spend"` | No | Target type. When omitted, the seller maximizes conversions within budget. |
...
Optimize for a seller-tracked delivery metric. No event source needed — the seller tracks these natively. Use when buying on CPM but wanting to optimize toward a click or view cost target, or a target rate like CTR.
...
| `metric` | `"clicks"` \| `"views"` \| `"completed_views"` | Yes | Seller-native metric to optimize for |
| `target.kind` | `"cost_per"` \| `"rate"` | No | Target type. When omitted, the seller maximizes metric volume within budget. |
| `target.value` | number | Yes (if target set) | Cost per metric unit in buy currency, or rate as a decimal (e.g., 0.001 for 0.1% CTR) |
| `priority` | integer | No | 1 = highest priority. When omitted, sellers use array position. |
...
|---|---|---|---|
| `cost_per` | Cost per click/view/completed view | Cost per conversion event | `spend / count` |
| `rate` | CTR, VCR, etc. | — | `count / impressions` |
| `per_ad_spend` | — | ROAS, profit per spend, etc. | `sum(value_field) / spend` |

### Choosing a strategy

| Goal | When to use | What you set |
|---|---|---|
| Max conversions | As many conversions as possible within budget | `kind: "event"` + event source, no target |
| Target cost per conversion | Specific cost per event | `kind: "event"` + `target: { kind: "cost_per", value: 25.0 }` |
| Target return on ad spend | Specific return ratio on a value field | `kind: "event"` + `value_field` + `target: { kind: "per_ad_spend", value: 4.0 }` |
...
| Target CTR | Specific click-through rate | `kind: "metric"`, `metric: "clicks"` + `target: { kind: "rate", value: 0.001 }` |
| Max views | Maximize views within budget | `kind: "metric"`, `metric: "views"`, no target |
| Target cost per view | Specific cost per view | `kind: "metric"`, `metric: "views"` + `target: { kind: "cost_per", value: 0.02 }` |
| Target VCR | Specific video completion rate | `kind: "metric"`, `metric: "completed_views"` + `target: { kind: "rate", value: 0.85 }` |
...
The seller focuses on the `priority: 1` goal (installs at $10 cost per) and uses clicks as a proxy signal until install data accumulates.

### Prerequisites

Before setting event optimization goals:
