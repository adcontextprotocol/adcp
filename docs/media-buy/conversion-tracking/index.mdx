...
Event-specific data for attribution and reporting. A monetary `value` always requires `currency`, and that currency must appear in the event source's `value_currencies` contract.
