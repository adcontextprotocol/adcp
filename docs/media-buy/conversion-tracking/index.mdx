...
| Status | Meaning |
|--------|---------|
| `insufficient` | Not firing, volume too low, or setup incomplete — optimization cannot run |
...
When sellers compute health from reporting data, the `evaluated_at` timestamp tells the buyer how fresh the assessment is. The `detail` object is absent for these sellers — there is no native score to relay.
