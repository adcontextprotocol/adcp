...
| Status | Meaning |
|--------|---------|
| `insufficient` | Source is not firing or event volume is too low for optimization |
| `minimum` | Source works but data quality limits optimization effectiveness |
| `good` | Source meets quality thresholds for most optimization goals |
| `excellent` | Source exceeds quality thresholds across all dimensions |
...
| `detail` | object | Seller-specific `score`, `max_score`, `label`. Supplementary. |
| `match_rate` | number | Fraction of events matched to ad interactions (0.0-1.0). Low rates indicate weak user_match identifiers. |
...
| `issues` | array | Actionable problems with `severity` and `message`. Sellers should limit to the top 3-5 most actionable items. |
...
| Field | Type | Description |
|-------|------|-------------|
| `ready` | boolean | Whether the buyer's setup meets minimum requirements |
...
A buyer agent talking to multiple sellers writes one set of rules that works everywhere. The standardized `status` field means no per-seller integration code:
...
for (const seller of sellers) {
  ...
  // Surface issues from any seller
