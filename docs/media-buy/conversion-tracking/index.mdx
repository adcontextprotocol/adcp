...
| `target.kind` | `"cpa"` \| `"roas"` | No | Target type. When omitted, the seller maximizes conversions within budget. |
| `target.value` | number | Yes (if target set) | CPA in buy currency, or ROAS multiplier (e.g., 4.0) |
...
Optimize for a seller-tracked delivery metric. No event source needed — the seller tracks these natively. Use when buying on CPM but wanting to optimize toward a click or view cost target.
...
| `target.kind` | `"cpc"` \| `"cpv"` \| `"cpcv"` | No | Target type. When omitted, the seller maximizes metric volume within budget. |
| `target.value` | number | Yes (if target set) | Target cost per metric unit in buy currency |
| `priority` | integer | No | 1 = highest priority. When omitted, sellers use array position. |

**Valid metric/target pairings** — sellers should reject mismatched combinations:

| `metric` | Valid `target.kind` |
|---|---|
| `clicks` | `cpc` |
| `views` | `cpv` |
...
| Target ROAS | Specific return on ad spend | `kind: "event"` + `target: { kind: "roas", value: 4.0 }` |
| Target CPA | Specific cost per conversion | `kind: "event"` + `target: { kind: "cpa", value: 25.0 }` |
| Max clicks | Maximize clicks within budget | `kind: "metric"`, `metric: "clicks"`, no target |
| Target CPC | Specific cost per click | `kind: "metric"`, `metric: "clicks"` + `target: { kind: "cpc", value: 2.0 }` |
| Max views | Maximize views within budget | `kind: "metric"`, `metric: "views"`, no target |
| Target CPV | Specific cost per view | `kind: "metric"`, `metric: "views"` + `target: { kind: "cpv", value: 0.02 }` |
| Max completed views | Maximize completed views within budget | `kind: "metric"`, `metric: "completed_views"`, no target |
| Target CPCV | Specific cost per completed view | `kind: "metric"`, `metric: "completed_views"` + `target: { kind: "cpcv", value: 0.05 }` |
...
"optimization_goals": [
  ...
  },
  {
    ...
    "target": { "kind": "cpa", "value": 10.00 },
    "priority": 1
  }
]
```

The seller focuses on the `priority: 1` goal (installs at $10 CPA) and uses clicks as a proxy signal until install data accumulates.
