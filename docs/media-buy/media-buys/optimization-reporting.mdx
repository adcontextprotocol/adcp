...
Delivery data for products with measurement windows includes two fields on each package:
...
When a buyer receives `is_final: false` with `measurement_window: "c3"`, it means: "these are C3 numbers and they will be superseded by C7 numbers later." When `is_final: true` with `measurement_window: "c7"`, this is the data to reconcile against.
