...
**JSON Lines (JSONL)** - Recommended for most use cases

One JSON object per line (newline-delimited JSON). Preserves full nested structure from webhook payloads. Easy to parse line-by-line for streaming processing.

**Example JSONL file:**
```jsonl
{"notification_type": "scheduled", "sequence_number": 1, "reporting_period": {"start": "2024-02-05T00:00:00Z", "end": "2024-02-05T23:59:59Z"}, "currency": "USD", "media_buy_deliveries": [{"media_buy_id": "mb_001", "buyer_ref": "campaign_a", "status": "active", "totals": {"impressions": 50000, "spend": 1750.00, "clicks": 100, "ctr": 0.002}, "by_package": [{"package_id": "pkg_001", "impressions": 30000, "spend": 1050.00, "pacing_index": 0.95, "pricing_model": "cpm", "rate": 0.035, "currency": "USD"}, {"package_id": "pkg_002", "impressions": 20000, "spend": 700.00, "pacing_index": 0.98, "pricing_model": "cpm", "rate": 0.035, "currency": "USD"}]}]}
{"notification_type": "scheduled", "sequence_number": 2, "reporting_period": {"start": "2024-02-06T00:00:00Z", "end": "2024-02-06T23:59:59Z"}, "currency": "USD", "media_buy_deliveries": [{"media_buy_id": "mb_001", "buyer_ref": "campaign_a", "status": "active", "totals": {"impressions": 52000, "spend": 1820.00, "clicks": 104, "ctr": 0.002}, "by_package": [{"package_id": "pkg_001", "impressions": 31000, "spend": 1085.00, "pacing_index": 0.97, "pricing_model": "cpm", "rate": 0.035, "currency": "USD"}, {"package_id": "pkg_002", "impressions": 21000, "spend": 735.00, "pacing_index": 0.99, "pricing_model": "cpm", "rate": 0.035, "currency": "USD"}]}]}
```
...
Flattened structure suitable for spreadsheet tools. May require multiple files for nested data (one file per dimension). Best for simple aggregations without dimensional breakdowns.

**Example CSV structure:**
```csv
notification_type,sequence_number,reporting_period_start,reporting_period_end,currency,media_buy_id,buyer_ref,status,impressions,spend,clicks,ctr
scheduled,1,2024-02-05T00:00:00Z,2024-02-05T23:59:59Z,USD,mb_001,campaign_a,active,50000,1750.00,100,0.002
scheduled,2,2024-02-06T00:00:00Z,2024-02-06T23:59:59Z,USD,mb_001,campaign_a,active,52000,1820.00,104,0.002
```

**File naming:**
```
s3://buyer-reports/publisher_name/2024/02/05/media_buy_delivery.csv.gz
```

</Tab>
<Tab title="Parquet">

**Parquet** - For high-volume analytics
...
```json
{
  ...
  "fields": [
    ...
    {"name": "media_buy_deliveries", "type": {
      "type": "array",
      "items": {
        "type": "record",
        "name": "MediaBuyDeliveryItem",
        "fields": [
...
**File naming:**
```
s3://buyer-reports/publisher_name/2024/02/05/media_buy_delivery.parquet.gz
```
...
**File Structure:**
Each file contains the same structure as webhook payloads, with one payload per line (JSONL) or row (CSV/Parquet). Files may contain:
- Single media buy delivery data
- Aggregated delivery data for multiple media buys
- Multiple reporting periods (one per line/row)
...
Webhook delivery status is tracked through **AdCP's global task management system** (see [Task Management](/docs/protocols/task-management)).
...
See [Core Concepts: Webhook Reliability](/docs/protocols/core-concepts.mdx#webhook-reliability) for detailed implementation guidance.
