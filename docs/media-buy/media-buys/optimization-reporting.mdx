...
`optimization_goals` define the objective function: what the seller should optimize, its units, and priority. They do not define 3.2 bidding policy. Currency-bearing execution controls live in `bidding` at media-buy or package scope:

- `cost_per` in `bidding` resolves against the package's primary goal;
- `bid_amount` and `max_bid` resolve against the selected pricing option's auction unit; and
- `roas` is dimensionless and requires value-bearing event sources.

The separation matters when the units differ—for example, a seller can optimize toward conversions using a target CPA while also enforcing a max-CPC auction ceiling. Legacy goal targets `cost_per` and `per_ad_spend` remain accepted during 3.2 migration but cannot be combined with canonical `bidding` on the same effective package.
