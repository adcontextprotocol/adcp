...
For offline file delivery, publishers can provide reporting data in JSON Lines or Parquet format. Both formats preserve the nested JSON structure from webhook payloads, making them ideal for batch processing.

**File Naming Convention:**
Files should follow the naming pattern: `s3://buyer-reports/publisher_name/2024/02/05/media_buy_delivery.{jsonl|parquet}.gz`

**Compression:**
Files are typically compressed with gzip (`.jsonl.gz` or `.parquet.gz`) for efficient storage and transfer.

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

<Tabs>
<TabItem value="jsonl" label="JSON Lines (JSONL)" default>

---

JSON Lines (JSONL) preserves the exact webhook payload structure, with each line containing a complete JSON object. This format is ideal for:
...
Each line in the JSONL file contains a complete webhook payload object. Multiple media buys can be included by having multiple lines in the file.
...
**Formatted Example (for readability):**
...
Parquet is a columnar storage format optimized for analytics and batch processing. This format is ideal for:
