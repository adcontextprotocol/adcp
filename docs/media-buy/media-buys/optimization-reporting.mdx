...
| Who produced the number, relative to the seller? | Product capability | `reporting_capabilities.vendor_metrics[].measurement_source` (`first_party` / `affiliated` / `third_party`) |
...
- See [Security](/docs/building/by-layer/L1/security) for implementation details
...
Buyers can optionally request a subset via `requested_metrics` to reduce payload size and focus on relevant KPIs. The same field exists on the [`get_media_buy_delivery`](/docs/media-buy/task-reference/get_media_buy_delivery#requesting-a-metric-subset) request with identical narrowing semantics, giving the GET path the same control the webhook already has.
...
- **Declaration** (`reporting_capabilities.vendor_metrics`): each entry is a pointer (`{ vendor: BrandRef, metric_id }`) into the vendor's metric catalog. Sellers say "I support this vendor's metric"; everything else (category, methodology, standard alignment, human-readable documentation) lives at the vendor. Identifiers are namespaced by the vendor — the same `metric_id` may mean different things in different vendors' vocabularies. The one thing the vendor catalog cannot tell a buyer is the seller's relationship to the vendor, so the declaration carries it: `measurement_source` is `first_party` (the seller or its own measurement agent produced the number), `affiliated` (shared parent, subsidiary, or material commercial dependency), or `third_party` (independent vendor). Sellers SHOULD populate it whenever the vendor is not an unrelated third party; buyers MUST NOT infer independence from omission when the vendor's domain matches or is affiliated with the seller.
...
Webhook delivery status is tracked through **AdCP's global task management system** (see [Task Lifecycle](/docs/building/by-layer/L3/task-lifecycle)).
...
Offline files sit at rest for `file_retention_days`, so a misconfigured IAM policy leaks historical reporting across tenants. The [general security controls](/docs/building/by-layer/L1/security) apply; the offline-specific requirements:
...
See [Webhooks](/docs/building/by-layer/L3/webhooks) for detailed implementation guidance.
