...
Format breakdowns use the AdCP 3.1 canonical `format_kind` vocabulary and are GET-only. They compare creative shapes such as `video_hosted`, `native_in_feed`, and `image`; duration, dimensions, and other format-option parameters are outside this grain. Sellers combine adopter-defined shapes into one `custom` row. Buyers reconcile `by_format` and `by_creative` independently against package totals rather than against each other.
