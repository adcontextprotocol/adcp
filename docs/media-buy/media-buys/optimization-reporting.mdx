...
```json
{
  ...
  "reporting_webhook": {
    ...
    "authentication": {
      "schemes": ["HMAC-SHA256"],
      "credentials": generateUniqueSecret('campaign_2024_reporting')
    },
...
If provided, AdCP enforces HMAC-SHA256 signature-based authentication. Server signs payload with `X-ADCP-Signature` and `X-ADCP-Timestamp` headers.
