...
Each window's numbers supersede the previous one. C7 is typically the guarantee basis — the number both sides reconcile against.

Broadcast sellers set `expected_delay_minutes` to reflect the initial data availability (e.g., 21600 for 15 days). When using webhooks, the seller sends delivery data tagged with the applicable `measurement_window`. Buyers should expect the most recent 8 days of a flight to lack final C7 numbers.
