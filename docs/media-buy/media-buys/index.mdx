...
- **`canceled`**: Terminated before natural completion. Check `canceled_by` to determine whether the buyer or seller initiated.
...
```json
{
  "media_buys": [{
    ...
    "status": "active",
    "revision": 3,
    "valid_actions": ["pause", "cancel", "update_budget", "update_dates", "update_packages", "sync_creatives"],
    "packages": [...]
  }]
}
```
...
- A canceled package does not affect other packages in the same media buy. When all packages are canceled, the buyer SHOULD explicitly cancel the media buy.
- Sellers MAY cancel media buys or packages (e.g., policy violation, inventory withdrawal). Seller-initiated cancellations set `canceled_by: "seller"` and MUST trigger a webhook notification to the orchestrator.
