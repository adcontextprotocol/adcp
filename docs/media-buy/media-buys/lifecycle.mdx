...
The `name` display label is not in this lossy list: `buy_products` and
`accept_proposal` both accept and persist it on the same terms as
`create_media_buy`, and it is always readable afterward through
`get_media_buys`.
