...
**Snapshot is the default propagation surface, but not the only one.** Sellers declare their propagation posture via [`capabilities.media_buy.impairment_propagation`](https://adcontextprotocol.org/schemas/v3/protocol/get-adcp-capabilities-response.json) on `get_adcp_capabilities`. The default `snapshot` (this section's contract) populates `health` + `impairments[]` on `get_media_buys` reads. `webhook_only` sellers fire `notification-type: impairment` webhooks without mirroring on the snapshot — buyers reconcile from the push channel alone. `out_of_band` sellers propagate outside the AdCP surface entirely (email, dashboard, partner-specific feeds). Both alternative postures grade the `impairment.coherence` storyboard scenarios `not_applicable`; their compliance bar is the webhook contract or the offline agreement, not snapshot coherence. The field is about where the *buy snapshot* mirrors the impairment; `notification-type: impairment` webhooks may still fire under any of the three postures (the webhook channel is orthogonal and declared separately via `push_notification_config`).
