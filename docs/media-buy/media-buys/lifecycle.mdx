...
**Out of scope.** Buys in terminal status (`completed`, `canceled`, `rejected`) MAY remain unreported even when a referenced resource is offline; the buy is no longer serving and the synchronisation cost is wasted. The `impairments[]` ↔ `health` biconditional likewise relaxes on terminal buys — sellers MAY leave stale `health: "impaired"` paired with an empty `impairments[]` (or the reverse) once a buy reaches a terminal state. The three rules are all evaluated only against non-terminal buys. Materiality (the requirement that `package_ids` be non-empty) is enforced at the schema layer by `package_ids: minItems: 1` on `impairment.json` — `impairment.coherence` does not re-check it.
