...
The `impairment.coherence` assertion verifies that:

- Every entry in `impairments[]` references a resource currently in an offline state.
- Every resource in an offline state referenced by a non-terminal media buy appears in that buy's `impairments[]`.
