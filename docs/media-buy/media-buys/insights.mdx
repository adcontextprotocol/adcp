...
When `media_buy.insight_notifications.repair_tasks` includes `list_creatives`, the two assignment projections MUST agree on `media_buy_id`, `package_id`, `creative_id`, approval state, rejection reason, scoped approval outcomes, and evaluated insight state. The seller includes `media_buy_id` and `approval_status` on every creative assignment row, even when insights are unknown. Inline-only sellers use `get_media_buys` without implementing this reverse projection.
...
Sellers advertise this contract with the paired `media_buy.supported_insight_types` and `media_buy.insight_notifications` capabilities. Every seller declares `insights.changed` and `get_media_buys`. A creative-library seller additionally pairs `creative.assignment_changed` with `list_creatives`. The capability also declares whether activity logs are available:
...
3. reread `get_media_buys` or `list_creatives` without an insight filter; and
