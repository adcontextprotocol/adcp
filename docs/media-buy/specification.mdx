...
- Seller-initiated cancellation (e.g., policy violation, inventory withdrawal) transitions the media buy to `canceled` with `canceled_by: "seller"`. Sellers MUST notify orchestrators via webhook when performing seller-initiated cancellation.
...
- Sales agents MUST set `canceled_by` and `canceled_at` when transitioning a media buy or package to `canceled`
...
- Seller-initiated cancellation notifications MUST use the `push_notification_config` webhook provided by the orchestrator during `create_media_buy` or `update_media_buy`. The webhook payload MUST include `media_buy_id`, `status: "canceled"`, `canceled_by: "seller"`, and `cancellation_reason`.
...
- Packages MAY have a `creative_deadline` — after this deadline, creative changes to the package are rejected with `CREATIVE_REJECTED`. When absent, the media buy's `creative_deadline` applies.
...
- When all packages in a media buy are canceled, the media buy itself remains in its current status (`active` or `paused`). Sales agents MUST NOT auto-transition the media buy to `canceled`. Since `update_media_buy` does not currently support adding new packages, the buyer should cancel the media buy explicitly. Sales agents SHOULD notify orchestrators (via `context.notes` in the update response) when the last active package is canceled.
...
- Sales agents MUST reject creative changes to packages past their `creative_deadline` with error code `CREATIVE_REJECTED`
...
| `active` | `pause`, `cancel`, `update_budget`, `update_dates`, `update_packages`, `sync_creatives` |
| `paused` | `resume`, `cancel`, `update_budget`, `update_dates`, `update_packages`, `sync_creatives` |
| `completed` / `rejected` / `canceled` | *(empty array)* |

Sellers MAY omit actions based on business rules (e.g., omit `cancel` when contractual obligations prevent it).
...
- `CREATIVE_REJECTED`: Creative failed validation, approval, or was submitted past the package's `creative_deadline`
