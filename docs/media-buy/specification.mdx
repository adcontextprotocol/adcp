...
- Sales agents MUST reject updates to media buys in terminal states (`completed`, `rejected`, `canceled`) with error code `INVALID_STATE`, except that a re-cancel attempt (`canceled: true` on a buy already in `canceled`) MUST be rejected with `NOT_CANCELLABLE` — the cancellation-specific code takes precedence over the generic terminal-state rule
