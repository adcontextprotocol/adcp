...
- Sales agents SHOULD consider `refine.overall` when composing the response — this MAY result in additional products beyond those in `refine.products`. Per-product actions take precedence over `overall` direction.
