...
| Buyer | Keep a package cap editable after creation | `required_overlay_support.frequency_cap_support.mutable_fields` at discovery; whether a live buy can change now is `MediaBuy.available_actions` |
...
A cap nested in a package `targeting_overlay` owns an independent counter for
that package. Repeating the same package cap on every package does not create
a shared counter across the buy; one counter across the buy is a
MediaBuy-level control specified separately. `FrequencyCap` defines no `scope`
discriminator, and unknown extension fields cannot change the scope
established by field location.
...
`REQUOTE_REQUIRED`. Failure preserves state. Changing a cap never resets its
counter: prior qualifying exposures in the resulting active window continue to
count. Successful creates, updates, and reads echo the applied package cap.
...
`overlay_support.frequency_cap` or `overlay_support.frequency_cap_support`. A
seller that cannot enforce a requested cap
