...
Every state-mutating AdCP task request MUST carry an `idempotency_key`. In AdCP 4.0 that includes every `get_products` call because the tool is polymorphic. Sales agents MUST apply the replay contract:
