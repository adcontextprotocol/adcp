...
The compact tasks use the same product-discovery authorization policy as the legacy facade, but each stateful task has a distinct idempotency identity. A retry MUST use the same tool name and payload; changing names is a different logical request. Sellers retain `get_products` through 3.x for compatibility. On that facade, a guaranteed side-effect-free synchronous read may omit the key, and the seller may ignore one when supplied. An execution arm that allocates a task or changes observable state MUST require a key and apply replay protection before that effect. See [Request safety — idempotency](/docs/building/by-layer/L1/security#idempotency) for the normative cache, replay, and concurrency rules.
