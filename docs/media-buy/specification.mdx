...
create_media_buy ──▶ rejected (terminal)
...
- When all packages in a media buy are canceled, the media buy itself remains in its current status (`active` or `paused`). Sales agents MUST NOT auto-transition the media buy to `canceled` — the buyer decides whether to cancel the media buy or add new packages.
