...
`REQUOTE_REQUIRED` and goes through proposal refinement. As with package caps,
a root-cap change on a buy whose `available_actions` omits
`update_media_buy_frequency_cap` returns `ACTION_NOT_ALLOWED`, and that
authorization check precedes the `UNSUPPORTED_FEATURE` capability check.
