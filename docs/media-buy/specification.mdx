...
During 3.x, split and legacy calls are aliases in one logical operation family. Sellers normalize them before authorization, idempotency, task recovery, persistence, and webhook emission, so a retry may safely switch names. New buyers SHOULD use the split tools; sellers MUST retain `get_products` through 3.x. See [Request safety — idempotency](/docs/building/by-layer/L1/security#idempotency) for the normative cache, replay, and concurrency rules.
