...
- **`media_buy_id`**: Client-provided reference for idempotency. Orchestrators SHOULD include this to enable retry safety.
...
**Requirements:**
- Orchestrators MUST include either `media_buy_id`
...
**Requirements:**
- Orchestrators MAY filter by `account_id`, `media_buy_ids`, `media_buy_ids`, and `status_filter`
...
- Default `status_filter: ["active"]` applies only when `media_buy_ids` are both omitted
...
Sales agents SHOULD support idempotent operations using `media_buy_id`:

- If a `media_buy_id` has been seen before, sales agents SHOULD return the existing resource
