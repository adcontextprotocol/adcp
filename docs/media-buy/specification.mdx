...
- **`product_id`**: Opaque identifier for a configured product returned by `get_products`. It remains stable and resolvable throughout the same discovery/refinement context so buyers can reference it from `refine` and purchase it before expiration. Buyers MUST NOT assume the same ID will identify an equivalent product in an independent discovery context. Creating a package with the ID accepts any sparse `targeting_resolution` disclosed in that response.
...
even when the buyer did not duplicate it in a structured field. For hard
targeting inferred from prose, sellers SHOULD confirm the resulting predicate
once in `GetProductsResponse.targeting_resolution.brief_targeting`. Absence of
