...
- Orchestrators MUST provide `product_id` and `action` for each entry in `refine.products`
...
Each `get_products` request with `buying_mode: "refine"` is self-contained — sales agents MUST NOT depend on transport-level session state. The `refine` object and `filters` on each request fully specify the refinement intent. This enables stateless implementations and safe retries.

- Sales agents MUST omit products with `action: "omit"` from the response
- Sales agents MUST omit proposals marked with `refine.proposals[].omit: true` from the response
- Sales agents MUST return products with `action: "include"`, with updated pricing
- Sales agents SHOULD apply adjustment instructions from `notes` on products with `action: "include"`
- Sales agents SHOULD return additional products similar to those with `action: "more_like_this"`, plus the original product
- Sales agents SHOULD consider `refine.overall` when composing the response — this MAY result in additional products beyond those in `refine.products`
- Sales agents SHOULD apply adjustment instructions from `refine.proposals[].notes`
- Sales agents MAY return proposals alongside products in refine mode, even when the orchestrator did not include `refine.proposals`
