...
The Media Buy Protocol defines thirteen tasks. See task reference pages for complete request/response schemas and examples.
...
**Schema**: [`media-buy/get-forecast-request.json`](https://adcontextprotocol.org/schemas/v2/media-buy/get-forecast-request.json) / [`media-buy/get-forecast-response.json`](https://adcontextprotocol.org/schemas/v2/media-buy/get-forecast-response.json)

**Reference**: [`get_forecast` task](/docs/media-buy/task-reference/get_forecast)

Return a delivery forecast for a specific product configuration without a full `get_products` call.

**Requirements:**
- Orchestrators MUST include `product_id`, `budget`, `start_date`, and `end_date`
- Orchestrators MAY include `targeting_overlay` to narrow the forecast
- Orchestrators MAY include `optimization_goal` for goal-specific forecasts
- Sales agents MUST return a `forecast` object with delivery points, method, and currency
- Sales agents SHOULD include `valid_until` to indicate forecast freshness
...
| [`media-buy/get-forecast-request.json`](https://adcontextprotocol.org/schemas/v2/media-buy/get-forecast-request.json) | get_forecast request |
| [`media-buy/get-forecast-response.json`](https://adcontextprotocol.org/schemas/v2/media-buy/get-forecast-response.json) | get_forecast response |
