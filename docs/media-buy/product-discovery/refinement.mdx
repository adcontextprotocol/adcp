...
**Precedence**: Per-product actions take precedence over `overall` direction. If `overall` says "less display" but a specific product has `action: "include"`, that product is returned regardless.
...
```json
{
  ...
  "refine": {
    ...
    "proposals": [
      { "proposal_id": "prop_balanced_v1", "action": "include", "notes": "shift 20% from display to video" }
    ]
  }
}
```
...
|-------|------|-------------|
| `proposal_id` | string | Proposal ID from a previous response |
| `action` | string | `"include"`: return with updated allocations and pricing. `"omit"`: exclude from the response. |
| `notes` | string | Adjustment instructions for this proposal. Ignored when `action` is `"omit"`. |
...
```json
{
  ...
  "refine": {
    ...
    "proposals": [
      { "proposal_id": "prop_awareness_q2", "action": "include", "notes": "reallocate native budget to video products" }
    ]
  },
  "filters": {
...
```json
{
  ...
  "refine": {
    ...
    "proposals": [
      { "proposal_id": "prop_balanced_v1", "action": "include", "notes": "rebalance for just these two products" }
    ]
  }
}
```
...
Each `get_products` request with `buying_mode: "refine"` is self-contained. The `refine` object and `filters` on each request fully specify the refinement intent. Sales agents MUST NOT depend on transport-level session state (e.g., remembering what was sent in a previous request).
...
- SHOULD consider `refine.overall` when composing the response — this MAY result in additional products beyond those in `refine.products`. Per-product actions take precedence over `overall` direction.
