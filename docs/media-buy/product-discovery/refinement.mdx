...
**Multi-finalize is atomic.** When multiple finalize entries target different proposals in one call, sellers MUST either commit all of them or none of them — partial commit is non-conformant. On failure, `refinement_applied[]` and `errors[]` carry per-position outcomes. Sellers that cannot guarantee atomic multi-proposal commit MUST reject multi-finalize arrays with `MULTI_FINALIZE_UNSUPPORTED` (preferred — explicitly signals seller-side capability gap rather than client-side mistake) or `INVALID_REQUEST` (acceptable fallback for sellers on a pre-3.1 error catalog), and buyers SHOULD then sequence single-finalize calls. There is no capability flag for multi-finalize support — the failure response is the discovery surface, so buyers MUST NOT assume support without a successful first attempt.
