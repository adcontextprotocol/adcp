...
<Note>
  This is the deprecated `get_products` compatibility grammar. New compact-lifecycle integrations use [`refine_proposals`](/docs/media-buy/task-reference/refine_proposals). The surfaces share `ask`, product `include`/`omit` verbs, and the `budget_range` shape, but their response envelopes are intentionally different.
</Note>
