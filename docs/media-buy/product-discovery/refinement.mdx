...
All three channels are optional, but the `feedback` object requires at least one field when provided. The seller considers all channels together when composing the response. (Refinement without `feedback` is also valid — see [Refresh forecasts](#refresh-forecasts).)
...
You can achieve the same result by removing the product from `product_ids` entirely. The difference: `omit: true` explicitly signals removal, which may prompt the seller to suggest a replacement. Dropping from `product_ids` is silent. If you use `omit: true`, keep the product in `product_ids` so the seller knows which product to exclude.
...
For sellers that generate creatives from buyer catalogs (declared via `creative_generation` in capabilities), products may include `sample_creatives` — representative previews with a `generated_creative_ref` identifier. Reference these in `feedback.overall` to iterate on creative direction:
...
The seller returns updated `sample_creatives` reflecting the feedback. See [Seller-generated creatives](/docs/media-buy/task-reference/get_products#seller-generated-creatives) for the full discovery-to-execution workflow.
...
- **Omit consistency** — When using `feedback.products[].omit: true`, keep the product in `product_ids` so the seller knows which product to exclude. Alternatively, simply remove the product from `product_ids` instead.
...
| `INVALID_REQUEST` | `feedback` provided in `brief` or `wholesale` mode, empty `feedback` object (`{}`), or missing required fields in `refine` mode | Check `buying_mode` and required fields |
...
**Sales agents:**
- MUST omit products marked with `feedback.products[].omit: true` from the response
- MUST omit proposals marked with `feedback.proposals[].omit: true` from the response
- MUST return all products from `product_ids` that are not omitted via `feedback`, with updated forecasts and pricing
- MUST NOT remove products from `product_ids` that are not explicitly omitted via `feedback` — the orchestrator's selection is authoritative
- SHOULD apply adjustment instructions from `feedback.products[].notes` and `feedback.proposals[].notes`
- SHOULD consider `feedback.overall` when composing the response — this MAY result in additional products beyond those in `product_ids`
