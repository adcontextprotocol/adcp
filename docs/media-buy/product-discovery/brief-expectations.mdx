...
| `refine_proposals` | Not repeated | Revise a proposal with structured `criteria`, semantic `instructions`, or both; or finalize a draft without changing terms |
...
| refinement `instructions` | Semantic changes without a structured field | Favor premium adventure contexts |
...
interpretation came from refinement instructions. In other cases the
...
product/proposal IDs plus structured `criteria`, semantic `instructions`, or
both. `action: "finalize"` changes no commercial or targeting terms and cannot
carry either field. See [Refining products and proposals](/docs/media-buy/product-discovery/refinement).
