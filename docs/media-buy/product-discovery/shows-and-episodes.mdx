...
Publishers can declare shows in their [adagents.json](/docs/governance/property/adagents) file alongside properties. This makes the show catalog discoverable for buyers that crawl adagents.json files. Products in `get_products` responses reference these same show IDs.
