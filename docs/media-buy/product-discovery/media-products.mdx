...
- **`vendor_metric_values`** mirrors delivery reporting for vendor-defined metrics, but `value` and the coverage denominators (`measurable_impressions`, or `measurable_plays` / `measurable_screen_seconds` for play-based channels) use ForecastRange objects. Sellers SHOULD only forecast vendor metrics that the product declares in `reporting_capabilities.vendor_metrics`.
