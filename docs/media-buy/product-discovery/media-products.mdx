...
- `measurement` (Measurement, optional): Business outcome measurement included with the product — incremental sales lift, brand lift studies, etc. Common for retail media products.
...
Proposals can be refined using `buying_mode: "refine"` with a `proposal_id` from the initial discovery response. The seller returns an updated proposal with revised allocations, forecasts, and pricing reflecting the new brief and filters:
