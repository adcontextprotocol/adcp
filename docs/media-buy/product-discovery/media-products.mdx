...
- `sample_creatives` (list[CreativePreview], optional): Representative creative previews the seller generated for this product. Only present when the seller declares `creative_generation` in capabilities, the buyer provides a catalog, and the product's `catalog_types` overlap. See [Seller-generated creatives](#seller-generated-creatives).
- `total_creatives_count` (integer, optional): Total creatives the seller would generate for the full catalog on this product at execution time. Gives buyers a sense of scale beyond the `sample_creatives`.
...
Proposals can be refined using `buying_mode: "refine"` with a `proposal_id` from the initial discovery response. Use `feedback` to provide structured per-item adjustment instructions — the seller returns an updated proposal with revised allocations, forecasts, and pricing:
...
// Refine the proposal with structured feedback
get_products({
  buying_mode: "refine",
  proposal_id: "swiss_balanced_v1",
  feedback: {
    proposals: [
      { proposal_id: "swiss_balanced_v1", notes: "focus more on German speakers - try 60/40 instead of 40/60" }
    ]
  }
})
...
Proposals are optional — publishers may return only products if allocation guidance isn't applicable. In `refine` mode, sellers MAY return proposals alongside refined products even when the buyer only provided `product_ids`. Proposals are a seller suggestion — allocation and campaign optimization are primarily orchestrator (buyer-side agent) responsibilities.

### Seller-generated creatives

Some sellers generate creatives from the buyer's catalog and brand assets rather than requiring the buyer to provide creatives via `sync_creatives`. This is common for catalog-driven campaigns — dynamic job ads, sponsored product cards, dynamic travel ads.

When a seller supports creative generation (declared via `media_buy.creative_generation` in [`get_adcp_capabilities`](/docs/protocol/get_adcp_capabilities)), products returned from `get_products` may include `sample_creatives` — representative previews showing how ads would render using the buyer's catalog and brand.

Key characteristics:
- **Product-scoped**: Each product shows its own previews matching its `format_ids`
- **Addressable**: Each preview has a `generated_creative_ref` that buyers can reference in refinement feedback
- **Sampled**: Previews are a small representative set (3-5). `total_creatives_count` indicates how many would be generated for the full catalog
- **Refinable**: Use `buying_mode: "refine"` with `product_ids` and reference specific `generated_creative_ref` values in `requested_changes` to iterate on creative direction
...
Buyers do not need to call `sync_creatives` or use a creative agent for these campaigns. The seller handles creative production at execution time when the buyer commits via `create_media_buy`. See [`get_products` seller-generated creatives](/docs/media-buy/task-reference/get_products#seller-generated-creatives) for the full workflow.
