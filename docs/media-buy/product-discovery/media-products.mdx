...
- `placements` (list[Placement], optional): Specific public ad placements within this product. Placements can be `targetable` or `included`; seller-private delivery objects are not exposed here. See [Placements](#placements).
...
Products can optionally declare specific public ad placements within their inventory. Placement IDs are publisher-scoped. When a placement exists in a publisher's `adagents.json` placement catalog, the product placement should reference that catalog entry with `publisher_domain` and `placement_id`. When no public catalog entry exists, the sales agent may define an inline placement with buyer-facing fields such as `name`, `description`, formats, and tags; its `placement_id` is still interpreted in the publisher namespace, or in the seller agent's own publisher namespace when `publisher_domain` is omitted.
...
For legacy products where `placements[]` entries omit `mode`, buyers may treat the placement as targetable for creative assignment. New sellers SHOULD emit `mode` explicitly.
