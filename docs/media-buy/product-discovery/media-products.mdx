...
- `installments` (list[Installment], optional): Specific installments available within this product. See [Collections and installments](/docs/media-buy/product-discovery/collections-and-installments).
...
- `identity` (object, optional, experimental): Product-scoped persistent-identifier and custom-reach-methodology facts. A product declaring `persistent_identifier: false` omits `overlay_support.frequency_cap`; `devices`, `accounts`, and `cookies` are not valid delivery reach units for it, while modeled `individuals` and `households` remain valid. Its frequency-only delivery rows carry `reach_unit`; a custom unit reported with either reach or frequency requires `identity.reach_methodology`. Sellers using this surface list `media_buy.product_identity` in `experimental_features`; see [Product identity absence and reach reporting](/docs/media-buy/specification#product-identity-absence-and-reach-reporting-experimental).
...
Each entry declares a `pattern` plus pattern-specific fields. Vendor identity is a [BrandRef](/docs/brand-protocol/brand-json) domain — new platforms declare a domain, no enum change required.

| Pattern | Required fields | Meaning |
|---------|----------------|---------|
| `sync_audiences` | — | Inline audience member pushes via the AdCP [`sync_audiences`](/docs/media-buy/task-reference/sync_audiences) task. Any AdCP-speaking buyer can use it. |
...
| `platform_distribution` | `vendor` | The vendor's rails deliver segments to the seller's destination. The buyer initiates distribution in the vendor's system, then binds the arriving segment with a [`platform_segment` source reference](/docs/media-buy/task-reference/sync_audiences#external-audience-sources-experimental) on `sync_audiences`. Optional `destination_ref` is an opaque account-scoped destination or seat reference; omit it until bilateral setup establishes the destination. Optional `bind_expiry_days` declares when an unfulfilled bind may expire. |
...
Buyers filter discovery with [`filters.audience_activation_methods`](/docs/media-buy/task-reference/get_products): OR across requested entries, AND within an entry, omitted optional fields are wildcards, `directions` matches on non-empty intersection (a method that omits `directions` matches any requested directions), and `vendor` matches on `domain` plus `brand_id` when specified.

**The runtime leg.** These declarations are the discovery half. The runtime step — telling the seller which share or arriving segment maps to which targetable audience — is an [external source reference on `sync_audiences`](/docs/media-buy/task-reference/sync_audiences#external-audience-sources-experimental) (`dataset` for `dataset_query` grants, `platform_segment` for `platform_distribution` deliveries), under the same experimental feature id. Inline push works on any seller declaring `sync_audiences`; source references work on sellers declaring the corresponding grant/distribution patterns.
