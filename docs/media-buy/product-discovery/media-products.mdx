...
Forecast points can also carry `dimensions` when a seller needs to expose availability by country, region, placement, device, audience, or an intersection such as placement x country. `dimensions` is an array; each item declares one `kind` (`geo`, `placement`, `device_type`, `device_platform`, or `audience`) and uses the same canonical identifiers as targeting and delivery reporting, such as `geo_level`/`geo_code` for geography and `placement_ref` for placements. When multiple dimension items appear on one point, the point represents the intersection of those constraints. This keeps dimensional availability inside one product or proposal instead of forcing sellers to create separate products for every country or placement. Dimension rows are independent of `pricing_options`; the product's pricing options still describe how the product is bought.
...
Sellers can expose availability by country, placement, placement x country, or other dimensional intersections as forecast points on the same product. A point with multiple `dimensions` items represents the intersection of all listed constraints. Rows are comparable when they use the same dimension grain, such as placement x country rows for several countries. Buyers MUST NOT sum rows unless the seller explicitly documents that the rows form a complete, non-overlapping partition.
