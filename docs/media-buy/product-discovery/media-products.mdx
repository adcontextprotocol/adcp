...
- `identity` (object, optional, experimental): Product-scoped persistent-identifier and custom-reach-methodology facts. A product declaring `persistent_identifier: false` omits `overlay_support.frequency_cap` and `overlay_support.frequency_cap_support`; `devices`, `accounts`, and `cookies` are not valid delivery reach units for it, while modeled `individuals` and `households` remain valid. Its frequency-only delivery rows carry `reach_unit`; a custom unit reported with either reach or frequency requires `identity.reach_methodology`. Sellers using this surface list `media_buy.product_identity` in `experimental_features`; see [Product identity absence and reach reporting](/docs/media-buy/specification#product-identity-absence-and-reach-reporting-experimental).
