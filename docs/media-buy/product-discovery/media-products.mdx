...
Forecasts include metric ranges (low/mid/high) for standard delivery metrics like impressions, reach, clicks, and spend. Each forecast declares a confidence level:

- **`estimate`** — rough approximation, no delivery commitment
- **`modeled`** — derived from predictive models or historical data
- **`guaranteed`** — contractually committed delivery levels
