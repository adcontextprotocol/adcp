...
- `product_id` (string, required): Opaque configured-product identifier. It remains stable within the same discovery/refinement context, including `refine`, but buyers must not treat it as cross-session catalog identity.
...
- `expires_at` (datetime, conditionally required): Required when `is_custom` is true; the configured product is no longer valid after this time.
- `overlay_support` (TargetingOverlaySupport, optional): Product-scoped targeting dimensions whose values may be selected later. Required when requested through `required_overlay_support`.
- `targeting_resolution` (ProductTargetingResolution, optional): Sparse differences from a structured overlay. Absence confirms exact acceptance of that product's structured overlay, but not how prose was interpreted; response-level `targeting_resolution.brief_targeting` confirms hard brief interpretation once for the result set. Presence requires `is_custom: true` and `expires_at`.
...
- **`mode: "included"`** - The public placement is fixed product inventory and cannot be cherry-picked. A selected request exactly equal to the product's complete included set is an inherent match and may be echoed on the package.
...
Publishers can attach delivery forecasts to proposals and individual allocations to help buyers evaluate expected performance before committing budget. For targeting-aware discovery, every forecast and availability statement reflects the effective requested targeting after any disclosed `targeting_resolution` modifications.
