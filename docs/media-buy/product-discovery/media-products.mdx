...
The split 3.2 [`request_proposals`](/docs/media-buy/task-reference/request_proposals) and [`refine_proposals`](/docs/media-buy/task-reference/refine_proposals) tasks return executable immutable snapshots with firm terms and any inventory hold. `create_media_buy(proposal_id)` is the acceptance/execution step. Legacy `get_products` may still return drafts; sellers reject attempts to execute one with `PROPOSAL_NOT_COMMITTED`, and its finalize form remains supported throughout 3.x.
