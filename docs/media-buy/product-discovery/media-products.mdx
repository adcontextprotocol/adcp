...
- `signal_targeting_options_source` (string, optional): Where buyers discover selectable signals: inline `signal_targeting_options`, `get_signals`, or both.
...
Products use two different signal fields with different meanings:

- **`data_provider_signals`** is deprecated legacy/non-selectable metadata for data-provider catalog signals already bundled into or associated with the product. It has no per-product price and no seller activation handle. New implementations should describe bundled signal metadata in product terms instead of adding this field.
- **`signal_targeting_options_source`** tells buyers where to discover the selectable signal menu. Use `"product"` when the product returns inline `signal_targeting_options`, `"get_signals"` when buyers should call `get_signals` for the full menu, and `"product_and_get_signals"` when the product includes product-specific overrides or a curated subset while `get_signals` remains the broader feed.
- **`signal_targeting_options`** is the inline selectable/composable surface. Use it when a seller-offered signal can appear on a package and the product needs to expose a product-specific menu, price, activation handle, default/fixed choice, or grouping hint.

`Product.signal_targeting_allowed`, `signal_targeting_options_source`, and `signal_targeting_rules` are the canonical product contract for signal composition. `get_signals` is the canonical broad signal discovery surface; products can point to it instead of duplicating a wholesale feed inline. Sellers do not need to expose product-local options through `get_signals` unless they also want those signals to be discoverable outside this product. Sellers MUST set `signal_targeting_allowed: true` whenever they return `signal_targeting_options`, `signal_targeting_options_source`, or `signal_targeting_rules`, and MUST omit `signal_targeting_options` for signals that are bundled but not represented as package signal groups.

Signals are named targetable dimensions, similar to feature values. A signal definition declares `value_type` (`binary`, `categorical`, or `numeric`) and may carry richer metadata over time. Product targeting uses `signal_ref` with an explicit buy-time scope: `{ "scope": "product", "signal_id": "..." }` refers to a product-local signal option, while `{ "scope": "data_provider", "data_provider_domain": "...", "signal_id": "..." }` refers to a signal defined by a data provider's published adagents.json signal catalog. `signal_ref.scope` is the buy-time resolution path, not provenance, and authoritative enrichment lives on the seller or data-provider signal definition. For product-local signals exposed on both `get_signals` and `get_products`, `signal_ref.signal_id` MUST match the seller's `get_signals.signals[].signal_ref.signal_id` for the same signal. Detailed methodology and quality claims belong in linked documentation such as `methodology_url`, not in self-attested protocol fields; the protocol carries the reference, product availability, pricing, and operational targeting rules.

Signal targeting composition is declared on each product, not in seller-wide `get_adcp_capabilities`, because one seller can sell products backed by different ad servers or platforms with different include/exclude and grouping limits. Buyers should read `signal_targeting_options_source`, any inline `signal_targeting_options`, and `signal_targeting_rules` from the specific product they are composing.
...
- **`signal_targeting_allowed: true`**: The product has a package-level signal targeting surface. The buyer may select signals discovered from inline `signal_targeting_options`, from `get_signals`, or both, according to `signal_targeting_options_source` and `signal_targeting_rules`.
  ...
  "signal_targeting_options": [
    {
      ...
      "name": "Auto intenders",
      "description": "Households showing recent auto purchase intent",
      "methodology_url": "https://pinnacle-data.example/signals/auto-intenders",
...
In that shape, the buyer calls `get_signals` to discover candidate signals, then passes selected `signal_ref` entries in `packages[].targeting_overlay.signal_targeting_groups`. The seller validates that the selected signals are available for the product and account.
