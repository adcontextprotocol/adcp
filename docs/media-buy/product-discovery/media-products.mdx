...
Publishers may also include a `budget_curve` array showing how delivery metrics scale with different budget levels, and a `demographic` field for GRP-based forecasts (linear TV, radio).
