...
- **`audio_feed_types`** - Declared audio feed types for radio, streaming-audio, podcast, gaming, and other audio placements. Concrete placements usually declare one value; aggregate placements may declare multiple.
...
Audio products can declare `audio_feed_types` at the product level, placement level, or both. The vocabulary follows the IAB Tech Lab/OpenRTB 2.6 `Audio.feed` definitions, but AdCP uses readable field and value names rather than the OpenRTB numeric codes:
...
This field describes execution and distribution details inside the buyer-facing `radio`, `streaming_audio`, `podcast`, or `gaming` channel. It does not rename channels and it does not change `adagents.json` `property_type` semantics, which remain tied to authorizable inventory surfaces. The field is an array because a sellable product can aggregate multiple audio feed types. When both product and placement declarations are present, the product-level array should be the union of the audio feed types the seller may deliver under that product.

This is a discovery signal, not a verification claim. Buyers can filter for products that can satisfy a requested type with `get_products.filters.audio_feed_types`, but sellers should not return mixed, non-targetable bundles unless they can constrain delivery to the requested audio feed type during planning or purchase.
