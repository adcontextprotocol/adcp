...
Each forecast contains a `forecasts` array of one or more points, each pairing a budget level with metric ranges (low/mid/high). A single point is a standard forecast; multiple points ordered by ascending budget show how metrics scale with spend. Metric names come from the `forecastable-metric` enum (impressions, reach, clicks, spend, views, conversions, leads, grps, etc.). Platforms can include additional metrics beyond the standard set. Each forecast declares its method:
