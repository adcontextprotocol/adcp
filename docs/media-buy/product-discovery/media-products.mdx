...
- `audience_evidence` (list[AudienceEvidence], optional): Compact product audience-composition claims. Age evidence states an age range, its estimated audience share, and whether that estimate is based on verified, declared, inferred, or aggregate population data. This supports planning and ranking only; it never implies user-level targeting or legal age verification.
...
Age evidence answers two buyer questions without exposing the seller's full methodology: what share of this product's audience is in the age range, and how was that age established?
...
`estimated_share` uses the full product audience as its denominator, including people whose age is unknown. Unknown-age users are outside the numeric range numerator, so audience evidence always carries `include_unknown: false`.

The four evidence bases are:

- `verified`: underlying user ages were established using the listed `verification_methods`.
- `declared`: users supplied an age, date of birth, or threshold affirmation.
- `inferred`: individual ages were predicted from behavior, attributes, or a model.
- `population_estimate`: aggregate survey or panel evidence; it says nothing about a particular user.

The same product may publish multiple rows when it has distinct claims. Evidence marked `verified` MUST list its verification methods, but even verified product evidence does not establish that the seller can resolve age on an individual impression. Exact execution remains a separate `demographic_targeting` capability.
