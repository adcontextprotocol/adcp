...
This field describes execution and distribution details inside buyer-facing audio inventory, including `radio`, `streaming_audio`, `podcast`, `gaming`, and audio-only `dooh` products. It does not rename channels and it does not change `adagents.json` `property_type` semantics, which remain tied to authorizable inventory surfaces. The field describes the underlying feed, not the delivery environment: a player installed in a physical venue remains `dooh` even when an existing feed value also applies. The field is an array because a sellable product can aggregate multiple audio distribution types. When both product and placement declarations are present, the product-level array should be the union of the audio distribution types the seller may deliver under that product.
