...
After purchase, buyers verify marginals with `get_media_buy_delivery.reporting_dimensions` (`geo`, `device_type`, `device_platform`, `audience`, `placement`, `property`, `collection`, or `installment`) and reconcile measurement expectations through the package's `committed_metrics`, `performance_standards`, and reported `viewability` / `vendor_metric_values`. The standardized `collection_property` and `placement_property` dimensions prove which property carried a content program or aggregate placement. Other arbitrary intersections, such as placement × country or product × signal, are not currently standardized.
