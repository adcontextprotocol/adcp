...
Proposals can be refined using `buying_mode: "refine"` with a `proposal_id` from the initial discovery response. Use `requested_changes` to describe what to adjust — the seller returns an updated proposal with revised allocations, forecasts, and pricing:
...
get_products({
  buying_mode: "refine",
  proposal_id: "swiss_balanced_v1",
  requested_changes: "focus more on German speakers - try 60/40 instead of 40/60"
})
