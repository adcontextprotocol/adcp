...
Forecasts use a `metrics` map keyed by metric name (from the `forecastable-metric` enum), with each value being a range (low/mid/high). Standard metrics include impressions, reach, clicks, spend, views, conversions, leads, and grps. Platforms can include additional metrics beyond the standard set. Each forecast declares its method:
...
For cross-channel planning, forecasts declare a `reach_unit` (individuals, households, devices, accounts, cookies) so buyers can compare reach across publishers. GRP-based forecasts (linear TV, radio) use `demographic_system` and `demographic` to specify the target demo, following the same pattern as CPP pricing. Publishers may also include a `budget_curve` array showing how delivery metrics scale with different budget levels.
