...
- `expires_at` (datetime, conditionally required): Required when `is_custom` is true; the configured product is no longer valid after this time. A recognized expired ID issued to the authenticated account and lineage returns `PRODUCT_EXPIRED`; an ID whose tombstone is no longer retained or belongs to another account/lineage returns `PRODUCT_NOT_FOUND`, preventing cross-tenant existence disclosure.
