...
Forecast points can also carry `dimensions` when a seller needs to expose availability by country, region, placement, device, or audience. Each dimensional row declares one primary `kind` (`geo`, `placement`, `device_type`, `device_platform`, or `audience`) and uses the same canonical identifiers as targeting and delivery reporting, such as `geo_level`/`geo_code` for geography and `placement_ref` for placements. This keeps dimensional availability inside one product or proposal instead of forcing sellers to create separate products for every country or placement. Dimension rows are independent of `pricing_options`; the product's pricing options still describe how the product is bought.

Forecast points may include measurement-aware forecasts alongside standard delivery metrics:
- **`viewability`** mirrors the `get_media_buy_delivery` `viewability` block, but numeric values use ForecastRange objects. Sellers SHOULD only emit forecast viewability when the product can report corresponding delivery viewability; `standard` is required whenever viewability values are present because MRC and GroupM rows are not interchangeable.
...
Sellers can expose availability by country, placement, or other dimensions as forecast points on the same product. Rows with the same `dimensions.kind` are comparable slices of the same forecast family. Rows with different `kind` values are alternative cuts of the inventory, not additive partitions — buyers MUST NOT sum a country row and a placement row unless the seller explicitly documents that intersection.
...
Each point is a row in the forecast, not a separate product. Buyers can compare country or placement avails, viewability expectations, and vendor measurement forecasts while still selecting from the product's normal pricing options when they create the buy.

A buyer agent turns a chosen dimensional row into a buy through the existing buy-time surfaces:
- `kind: "geo"` rows map to `packages[].targeting_overlay.geo_countries`, `geo_regions`, `geo_metros`, or `geo_postal_areas`, depending on `geo_level` and seller support.
...
- `kind: "placement"` rows map to `creative_assignments[].placement_refs` for targetable placements. If the buyer wants to buy only that placement, the product must expose the placement as `mode: "targetable"` or the buyer should request a refined product/proposal.

After purchase, buyers verify the same slices with `get_media_buy_delivery.reporting_dimensions` (`geo`, `device_type`, `device_platform`, `audience`, or `placement`) and reconcile measurement expectations through the package's `committed_metrics`, `performance_standards`, and reported `viewability` / `vendor_metric_values`.
