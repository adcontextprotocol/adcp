...
| `platform_distribution` | `vendor` | The vendor's rails deliver segments to the seller's destination. The buyer initiates distribution in the vendor's system; segment-to-audience binding is coordinated during account setup until [#6540](https://github.com/adcontextprotocol/adcp/issues/6540) lands. Optional `destination_ref` is an opaque account-scoped destination or seat reference; omit it until bilateral setup establishes the destination. Optional `bind_expiry_days` declares when an unfulfilled bind may expire. |
...
**What works today vs. what's coming.** These declarations are the discovery leg. For grant- and distribution-based paths (`dataset_query`, `clean_room`, `platform_distribution`), the runtime step — telling the seller which share or arriving segment maps to which targetable audience — is coordinated during account setup until external audience source references on `sync_audiences` land ([#6540](https://github.com/adcontextprotocol/adcp/issues/6540)). Inline push via `sync_audiences` works end-to-end today.
