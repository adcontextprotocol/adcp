...
| FAST / virtual linear | Programmed channel | Scheduled airing / programming block | "Acme Retro" → "Tuesday 20:00 movie" |
...
| `kind` | string | No | What kind of content program: `series` (TV/podcast), `publication` (print issues), `event_series` (live events), `rotation` (DOOH), `channel` (FAST or virtual linear stream). Defaults to `series`. |
...
## FAST channels and owner-sold carriage

A FAST channel is a `channel` collection, not a property. This preserves the
three independent inventory axes:

- the host CTV app is the **property** where the ad runs;
- the FAST channel is the **collection** whose programming surrounds the ad;
...
```json
{
  ...
  "distribution": [{
    ...
    "identifiers": [
      { "type": "publisher_channel_id", "value": "channel_942" },
...
`publisher_channel_id` is the generic fallback for a host-assigned channel or
EPG identifier. Its namespace is the enclosing `publisher_domain`; the stable
identity is therefore `(publisher_domain, value)`. Use a more specific
identifier type when one exists.
...
For a buyer to treat this supply path as verified, both declarations must
match: the owner asserts carriage on the host property, and the host authorizes
the seller for that property and collection. Missing host authorization fails
closed. A host may add `placement_ids` or `placement_tags` when it needs to
constrain positions, but owner-sold carriage does not require it.
...
```json
{
  ...
  "placements": [
    { "kind": "seller_inline", "placement_id": "pre_roll", "name": "Pre-roll", "mode": "targetable" }
  ],
  ...
  "products": [
    {
      ...
      "placements": [
        { "kind": "seller_inline", "placement_id": "pre_roll", "name": "Pre-roll", "mode": "targetable" },
        { "kind": "seller_inline", "placement_id": "mid_roll", "name": "Mid-roll", "mode": "targetable" }
      ],
      "delivery_type": "guaranteed",
      "pricing_options": [
  ...
  "products": [
    {
      ...
      "placements": [
        { "kind": "seller_inline", "placement_id": "pre_roll", "name": "Pre-roll (15s)", "mode": "targetable" },
        { "kind": "seller_inline", "placement_id": "mid_roll", "name": "Mid-roll (30s)", "mode": "targetable" }
      ],
      "delivery_type": "guaranteed",
      "pricing_options": [
  ...
  "products": [
    {
      ...
      "placements": [
        ...
        { "kind": "seller_inline", "placement_id": "mid_roll", "name": "Mid-roll host read (60s)", "mode": "targetable" }
      ],
      "delivery_type": "guaranteed",
      "pricing_options": [
  ...
  "products": [
    {
      ...
      "placements": [
        { "kind": "seller_inline", "placement_id": "in_game_overlay", "name": "In-game overlay (10s)", "mode": "targetable" },
        { "kind": "seller_inline", "placement_id": "halftime", "name": "Halftime feature (60s)", "mode": "targetable" }
      ],
      "delivery_type": "guaranteed",
...
The seller returns products with `collections` selectors. The buyer resolves full collection objects from each publisher's [adagents.json](/docs/governance/property/adagents). Registry clients can also bootstrap and look up the crawled collection catalog through `/api/registry/catalog/collections/sync` and `/api/registry/catalog/collections/distribution`, which is useful for cross-referencing platform aliases such as YouTube channel IDs back to the publisher-declared collection.
