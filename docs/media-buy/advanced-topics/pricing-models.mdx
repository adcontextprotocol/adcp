...
**Example** (fixed pricing — pay per purchase):
```json
{
...
**Example** (auction pricing — pay per lead):
```json
{
  "pricing_option_id": "cpa_usd_lead_auction",
  "pricing_model": "cpa",
  "event_type": "lead",
  "currency": "USD",
  "floor_price": 2.00,
...
**Billing**: Charged when the specified `event_type` fires. The pricing option declares what event triggers billing — this is independent of `optimization_goal`, which controls delivery optimization.
...
**Note**: CPA replaces the need for separate "CPO" (Cost Per Order) or "CPL" (Cost Per Lead) pricing models. A seller can offer multiple CPA options with different event types and prices on the same product.
