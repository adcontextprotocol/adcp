...
- `list_price` — the fully-loaded rate card (gross) starting point, inclusive of any surcharges or fees. The breakdown only models reductions from this starting point.
- `adjustments` — ordered list of adjustments, each categorized by `kind`
- `fixed_price` (on the parent pricing option) — must equal the result of applying all `discount` adjustments to `list_price`
...
Multiple discounts compound in array order. Rate-based discounts multiply the running price; amount-based discounts subtract from it:

```
list_price: 20.00
  discount, rate: 0.10       → 20.00 × 0.90 = 18.00
  discount, amount: 2.00     → 18.00 − 2.00 = 16.00
fixed_price: 16.00 ✓
```

For the structure example above, only the volume discount (kind `discount`) applies: 14.00 × (1 − 0.15) = 11.90 ✓

### Rate vs. Amount

Each adjustment must include exactly one of `rate` or `amount`:

- `rate` — decimal proportion in the range (0, 1), e.g., 0.15 for 15%
- `amount` — positive number in the pricing option's currency
...
The `price_breakdown` field also appears on the package response (confirmed line item).
...
**Auction-based packages** use `price_breakdown` to disclose commission and settlement terms against the clearing price. The discount invariant does not apply — the clearing price is the price. The breakdown is informational only:
