...
```json
{
  ...
  "cancellation_policy": {
    ...
    "penalty_description": "50% of remaining committed spend"
  }
}
```

Unlike `measurement_terms`, cancellation policy is not a negotiation surface — the seller declares it and the buyer accepts it by creating a media buy against the product. A guaranteed buy canceled without sufficient notice may incur the declared penalty.
