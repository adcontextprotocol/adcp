...
The invariant is: `list_price` with all `discount` adjustments applied sequentially equals `fixed_price`. Commission and settlement adjustments don't participate — they're disclosed for transparency. If no `discount` adjustments are present, `fixed_price` must equal `list_price`.
...
Multiple discounts compound in array order using these formulas:
