...
Beyond declaring a provider, sellers can publish structured `measurement_terms` on products — and buyers can negotiate them at buy creation:
...
- **`ivt_threshold`** — Maximum acceptable Invalid Traffic rate. The `vendor` is identified by domain using the standard brand reference (e.g., `{ "domain": "doubleverify.com" }`). When agreed on a confirmed package, creatives MUST include `tracker_script` or `tracker_pixel` assets from the specified vendor.
- **`performance_guarantees`** — Array of guaranteed minimum rates for performance metrics. Each entry specifies a `metric` (`viewability`, `completion_rate`, `brand_safety`, `attention_score`), a `min_rate` (decimal, e.g., 0.70 for 70%), an optional `standard` (required for viewability: `"mrc"` or `"groupm"`), and a `vendor`. When agreed on a confirmed package, creatives MUST include tracker assets from specified vendors.
- **`makegood_policy`** — Closed menu of remedies available when any threshold is breached. Three remedy types: `additional_delivery` (extend/add impressions, like-for-like), `credit` (toward future buys), `invoice_adjustment` (reduce current invoice). Seller proposes from this menu; buyer accepts or disputes.

**Negotiation flow:**

1. Seller publishes `measurement_terms` on the product (defaults)
2. Buyer proposes terms on the package request at `create_media_buy` (overrides)
...
When the buyer omits `measurement_terms`, the product's defaults apply.
...
```json
{
  ...
  "cancellation_policy": {
    ...
    "penalty": { "type": "percent_remaining", "rate": 0.5 }
  }
}
```
...
- **`none`** — No financial penalty (cancellation with notice is free)

Unlike `measurement_terms`, cancellation policy is not a negotiation surface — the seller declares it and the buyer accepts it by creating a media buy against the product. A guaranteed buy canceled without sufficient notice incurs the declared penalty.
