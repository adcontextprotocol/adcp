...
- `list_price` — the rate card (gross) starting point
- `adjustments` — ordered list of adjustments, each categorized by `kind`
- `fixed_price` — the result of applying all `discount` adjustments to `list_price`

### Adjustment Kinds

Adjustments fall into three kinds based on their economic effect:

| Kind | Effect on buyer price | Effect on publisher revenue | When applied |
|------|----------------------|----------------------------|--------------|
| `discount` | Reduces it | Reduces it | Before quoting — walks `list_price` to `fixed_price` |
...
**Discounts** change what the buyer pays. Volume discounts, negotiated rates, and early booking discounts all reduce both the buyer's cost and the publisher's revenue.

**Commissions** are revenue splits. The buyer's price and budget don't change — commission determines how the payment is divided between the intermediary (e.g., agency) and the publisher. Budgets are always managed inclusive of commissions.

**Settlement adjustments** are applied at invoicing or payment time (e.g., a cash discount for early payment). They don't affect the committed price or budget.

### Invariant

The invariant is: `list_price` with all `discount` adjustments applied sequentially equals `fixed_price`. Commission and settlement adjustments don't participate — they're disclosed for transparency.

Multiple discounts compound in array order. For example, two discounts of 10% and 5% apply as: `list_price × 0.90 × 0.95` (not `list_price × 0.85`).

For the example above, only the volume discount (kind `discount`) applies: 14.00 × (1 − 0.15) = 11.90 ✓

### Rate vs. Amount

Each adjustment uses either `rate` (a decimal proportion, e.g., 0.15 for 15%) or `amount` (a fixed monetary value in the pricing option's currency). These are mutually exclusive.
...
The `price_breakdown` field also appears on the package response (confirmed line item). For fixed-price packages, this echoes the pricing option's breakdown. For auction-based packages, `list_price` represents the clearing price before any commission or settlement adjustments — discount adjustments typically don't apply since the clearing price is the price.
...
```json
{
  ...
  "fixed_price": 14.00,
  "currency": "EUR",
  "eligible_adjustments": ["discount", "commission", "settlement"]
}
```
