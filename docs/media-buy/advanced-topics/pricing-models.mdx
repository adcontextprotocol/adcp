...
### Measurement Terms and Performance Guarantees
...
- **`measurement_terms`** — Who counts the billing metric and what happens when thresholds are breached
- **`performance_guarantees`** — What rates are guaranteed (viewability, IVT, completion, brand safety, attention)
...
- **`makegood_policy`** — Closed menu of remedies available when any performance guarantee or billing variance is breached. Three remedy types: `additional_delivery` (extend/add impressions, like-for-like), `credit` (toward future buys), `invoice_adjustment` (reduce current invoice). Seller proposes from this menu; buyer accepts or disputes.

**Performance guarantees fields:**

Each entry in the `performance_guarantees` array specifies:

- **`metric`** — What is guaranteed: `viewability`, `ivt`, `completion_rate`, `brand_safety`, `attention_score`
...
1. Seller publishes `measurement_terms` and `performance_guarantees` on the product (defaults)
