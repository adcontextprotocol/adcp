...
**Example** (auction pricing):
```json
{
  "pricing_option_id": "cpa_usd_auction",
  "pricing_model": "cpa",
  "currency": "USD",
  "floor_price": 2.00,
  "price_guidance": {
...
**Billing**: Charged when a conversion event fires. The specific event type (purchase, lead, signup, etc.) is defined by the package's `optimization_goal.event_type`, which ties to event sources configured via `sync_event_sources`.

**How CPA connects to conversion tracking:**
1. Buyer configures event sources via `sync_event_sources` (e.g., a pixel tracking purchases)
2. Buyer sets `optimization_goal` on the package with `event_source_id` and `event_type`
3. Events fire via `log_event` as conversions occur
4. Delivery reports include `conversions`, `conversion_value`, `cost_per_acquisition`, and `roas`

**Note**: CPA replaces the need for separate "CPO" (Cost Per Order) or "CPL" (Cost Per Lead) pricing models. Use `optimization_goal.event_type` to specify the conversion type: `purchase` for CPO equivalents, `lead` for CPL equivalents, etc.
...
- Publisher provides `floor_price` (hard constraint) and `price_guidance` (percentile hints)
