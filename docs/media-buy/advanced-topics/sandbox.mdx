...
Sandbox is part of the account's **natural key**. The same brand/operator pair can have both a production and a sandbox account — distinguished by `sandbox: true` in the account reference. No separate provisioning step is needed — just include `sandbox: true` and the seller handles it.
...
Include `sandbox: true` in the account reference. No provisioning or account discovery is required — just add the flag:
