...
Contact the seller to provision a sandbox account for your brand. The seller returns a sandbox `account_id` that you use in place of your production account ID. From the protocol's perspective, a sandbox account behaves identically to a real account — the seller internally flags it as test and suppresses real platform calls and billing.
