...
The vendor's `brand.json` at their domain is the discovery point for their agent capabilities. This follows the same pattern as `brand_agent` (brand data via MCP) and `rights_agent` (rights licensing via MCP) — measurement vendors can advertise a measurement agent that provides live delivery rates for active media buys.

This enables the same data flow pattern used by [content standards](/docs/governance/content-standards): the buyer or seller queries the measurement agent for current rates against agreed performance standards, rather than waiting for post-campaign reporting. The measurement vendor participates as an agent, not a black box.
