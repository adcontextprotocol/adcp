...
That product cannot execute ages 21–35 exactly: `18–24 ∪ 25–34 ∪ 35–44` broadens the request, while `25–34` narrows it. The seller rejects the request or returns explicit alternatives for buyer approval. It never books a widened range silently.
...
`targeting_overlay.demographics` is applied state and matches the resolution's `applied` predicate. Buyers should recompute `equivalent` from `requested` and `applied`. A non-equivalent readback includes `difference_reason` and `buyer_approved: true`; the schema does not permit a stored non-equivalent package to claim that approval was absent. The same core `Package` schema carries this readback in synchronous `CreateMediaBuySuccess` responses and terminal async completion artifacts; `get_media_buys` exposes the corresponding `PackageStatus` field. Updates use the normal full-overlay replacement semantics and refresh the resolution atomically.
