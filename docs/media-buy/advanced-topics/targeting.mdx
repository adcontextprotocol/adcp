...
**Note**: Metro and postal targeting require specifying the classification system, enabling international support. Not all geographic granularities are supported by all publishers. Country and region are most widely supported.
...
Test group targeting uses structured metro areas with explicit classification system:
...
All geographic targeting uses inclusive matching.
...
1. **Support Geographic Targeting**: Handle geographic parameters (`geo_countries`, `geo_regions`, `geo_metros`, `geo_postal_areas`) to the extent your platform supports them. Declare supported metro and postal systems in `get_adcp_capabilities`
