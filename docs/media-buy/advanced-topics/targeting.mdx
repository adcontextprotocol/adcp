---
title: Targeting
description: "AdCP targeting — how natural language briefs replace taxonomy-based audience selection, with geographic overlays and real-time decisioning at impression time."
...
For targeting decisions that must happen at impression time, AdCP uses the **[Trusted Match Protocol (TMP)](/docs/trusted-match)**. TMP is the real-time execution layer that evaluates pre-negotiated packages at serve time across any surface.
...
See the [TMP documentation](/docs/trusted-match) for the full specification and surface-specific integration guides.
...
| — | `url_allowlist` / `url_blocklist` | (no discovery filter — runtime gate only) | Restrict / reject delivery on specific content URLs |

**Filters** tell the sell-side agent what matters to the buyer so it can curate relevant products. Without a proximity filter, a seller might recommend products that don't support geo_proximity — and the buyer wouldn't discover the gap until `create_media_buy`.
...
Use when delivery must be restricted to (`url_allowlist`) or kept off (`url_blocklist`) specific content URLs:

- **Allowlists**: first-party inventory deals scoped to a fixed set of landing pages or article URLs.
- **Blocklists**: brand-safety negative lists supplied by the buyer or a third-party vendor, surgical exclusions for sensitive content, or removal of competitor URLs from a broader buy.

Both fields target **content URLs** (the page the impression appears on), never user-identifying URLs — the same boundary that excludes raw `url` artifact refs from `url_hash` storage on TMP. The receiver canonicalizes raw URLs using the same content-URL rules already defined for the `url_hash` artifact-ref on context-match-request (`static/schemas/source/trusted-match/context-match-request.json`, `ArtifactRef.value`): strip scheme, strip `www.` / `m.` / `amp.` host prefix, lowercase the entire URL (host and path — the reference implementation lowercases before splitting the path), strip query, strip fragment, strip trailing slash. Use these rules — **not** the `/docs/reference/url-canonicalization` page, which describes a different RFC-3986 identifier-canonicalization algorithm scoped to request signing, `adagents.json` lookups, and registry-key comparison (it preserves scheme and query, which is wrong for content matching). Because `url_allowlist` / `url_blocklist` share the canonicalization with the `url_hash` artifact-ref, a buyer's `url` entry and a publisher's `url_hash` classification of the same content dedupe to the same canonical key.

Each list entry is `{ type, value }`:

- `type: "url"` — raw URL the receiver canonicalizes. Example: `{ "type": "url", "value": "https://oakwood.example.com/2026/03/kitchen-trends" }`.
- `type: "url_hash"` — pre-hashed value in the AdCP wire shape (44-char standard-base64 of a 32-byte Blake3-256 digest). Use when a brand-safety vendor or publisher pipeline already supplies hashes and the raw URLs are not available to the buyer. Example: `{ "type": "url_hash", "value": "KwARbnwpUNNChmpEFqJGMAkJMJoqmsZKPZsPBaHbCno=" }`.

Buyers may mix both forms in a single list. Sellers MUST declare URL-allowlist and URL-blocklist support in `get_adcp_capabilities`; sellers that do not support URL allow/block-lists MUST reject media buys carrying these fields rather than silently dropping them.

**Precedence**: `url_blocklist` takes precedence over `url_allowlist`. A URL present in both lists is rejected — a single blocklist entry can prohibit a URL that would otherwise be allowed.

```json
{
  "targeting_overlay": {
    "url_allowlist": [
      { "type": "url", "value": "https://oakwood.example.com/spring-catalog" },
      { "type": "url", "value": "https://oakwood.example.com/dining-tables" }
    ],
    "url_blocklist": [
      { "type": "url_hash", "value": "KwARbnwpUNNChmpEFqJGMAkJMJoqmsZKPZsPBaHbCno=" }
    ]
  }
}
```
...
- **Note**: This field is from the legacy AXE integration. New implementations should use [TMP](/docs/trusted-match), where audience targeting is handled through the Identity Match path.
...
- **Note**: This field is from the legacy AXE integration. New implementations should use [TMP](/docs/trusted-match), where suppression is handled through the Identity Match path.
...
- **Format**: Array of `audience_id` strings from [`sync_audiences`](/docs/media-buy/task-reference/sync_audiences)
...
- **[Trusted Match Protocol (TMP)](/docs/trusted-match)** - Real-time execution layer for impression-time targeting, frequency capping, and brand suitability
