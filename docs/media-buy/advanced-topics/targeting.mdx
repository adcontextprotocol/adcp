...
Both fields target **content URLs** (the page the impression appears on), never user-identifying URLs — the same boundary that excludes raw `url` artifact refs from `url_hash` storage on TMP. The receiver canonicalizes raw URLs using the AdCP content-URL canonicalization rules ([reference](/docs/reference/url-canonicalization)): strip scheme, strip `www.` / `m.` / `amp.` host prefix, lowercase, strip query, strip fragment, strip trailing slash. The same canonicalization underlies the `url_hash` artifact-ref defined on context-match-request — so `url` and `url_hash` entries dedupe correctly when they reference the same content.
