...
AdCP starts with **brief-based targeting** for semantic preferences and uses typed overlays when a buyer needs an exact, auditable constraint. Publishers return products that describe which overlay semantics they can execute.
...
Use a typed `targeting_overlay` for controls whose exact wire semantics must survive booking and readback. In particular, numeric demographic predicates such as ages 25–40 belong in `targeting_overlay.demographics`; prose such as “millennial parents interested in sustainable products” remains in the brief. If both are supplied, the prose guides discovery while the overlay is the auditable package constraint.
...
That product cannot execute ages 21–35 exactly: `18–24 ∪ 25–34 ∪ 35–44` broadens the request, while `25–34` narrows it. The seller rejects the request and may suggest supported predicates. If the buyer chooses one, it submits that predicate as a new request; the seller never stores a widened or narrowed alternative under the original request.
...
`targeting_overlay.demographics` is applied state and matches the resolution's `applied` predicate. `equivalent` is always `true` for stored packages, and buyers should still recompute equality from `requested` and `applied`. A seller rejects any non-equivalent compilation; selecting a suggested predicate requires a new buyer request, which becomes the new auditable intent. The same core `Package` schema carries this readback in synchronous `CreateMediaBuySuccess` responses and terminal async completion artifacts; `get_media_buys` exposes the corresponding `PackageStatus` field. Updates use the normal full-overlay replacement semantics and refresh the resolution atomically.
