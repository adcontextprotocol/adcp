...
- **Per-keyword bid**: The optional `bid_price` overrides the package-level `bid_price` for that keyword. If omitted, the package `bid_price` applies.
- **Use cases**: Search campaigns, retail media sponsored products, keyword-based intent targeting
- **Note**: Seller must declare `execution.targeting.keyword_targets: true` in `get_adcp_capabilities`. Keyword-level delivery data (`by_keyword` in reporting) requires `reporting_capabilities.supports_keyword_breakdown: true` on the product — these are independent capabilities.
...
```json
{
  ...
  "keyword_targets": [
    ...
    { "keyword": "nike pegasus 41", "match_type": "exact", "bid_price": 1.20 }
  ]
}
```
...
- **Note**: Seller must declare support in `get_adcp_capabilities`. Use `negative_keywords_add` and `negative_keywords_remove` in `update_media_buy` to add/remove negatives incrementally after launch.
...
Keyword targeting in `targeting_overlay` uses replacement semantics when sent via `update_media_buy` — the full `keyword_targets` list replaces the previous one. For negative keywords, use the incremental operations `negative_keywords_add` and `negative_keywords_remove` instead, which append to or remove from the existing list without requiring a full replacement:
...
Incremental operations are the right tool for negative keyword management because negative lists grow continuously as agents learn from `by_keyword` search term data in delivery reports.
