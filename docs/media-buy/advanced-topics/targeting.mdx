...
- **Discovery**: Products either return inline `signal_targeting_options`, point buyers to `get_signals` with `signal_targeting_options_source: "get_signals"`, or use `"product_and_get_signals"` for product-specific overrides plus the broader feed. Do not duplicate a large wholesale signal feed inline unless the product needs product-specific pricing, activation handles, defaults, or grouping hints.
