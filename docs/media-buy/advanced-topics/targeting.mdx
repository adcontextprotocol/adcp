...
**Cross-level resolution.** Geographic levels form a hierarchy: country > region > metro > postal. Exclusion at a higher level removes all locations within that geography, regardless of inclusion at a more specific level. For example, `geo_countries_exclude: ["US"]` combined with `geo_regions: ["US-CA"]` results in no US delivery — the country-level exclusion takes precedence.
...
**Capabilities.** Declaring geographic targeting support in `get_adcp_capabilities` implies support for both inclusion and exclusion at that level. Sellers that support `geo_metros` targeting are expected to also support `geo_metros_exclude`.
