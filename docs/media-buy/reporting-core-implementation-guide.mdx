...
The reporting lab requires negotiated `adcp_version: "3.2-beta.12"` or later;
the training agent otherwise serves its default 3.0 surface, where these
experimental reporting fields are unavailable. Put that version on every
