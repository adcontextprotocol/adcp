...
description: "The required Reliable Reporting tier: obligations, immutable closes, seller status, and buyer-to-seller status sync — over the transports you already have."
...
baseline: a seller must retain immutable revision evidence, expose its exact
binding over existing reporting transports, and record what the authenticated
buyer could actually consume. Core requires no destination, manifest, external
...
Five ideas, two complementary status tasks:
...
5. **[`sync_reporting_status`](/docs/media-buy/task-reference/sync_reporting_status)
   closes the loop** — the buyer tells the seller whether an expected period
   was received, omitted, missing, or unreadable. Both attributed histories stay
   visible; neither overwrites the other.
...
- **Buyer:** retain the accepted configuration generation, derive the same
  ...
  than evidence that no activity occurred. Sync the resulting consumer status
  back to the seller, and update it whenever consumption state changes.
...
- **No push requirement.** Polling `get_reporting_status` is the
  ...
  conformant. `sync_reporting_status` is an ordinary inbound task call, not a
  seller push or a buyer-hosted callback.
...
- **No Managed canonicalization or billing receipts.** `reconciliation_mode` is
  ...
  for you. Core's lightweight `sync_reporting_status` instead echoes only the
  fixed revision-binding SHA-256 when reporting was received; it carries no row
  totals, Managed evidence, or billing acceptance. Receipts and Managed external
  materialization machinery belong to the `reconciled_billing` tier.
