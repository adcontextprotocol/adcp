...
description: "Follow a media buyer from brief to delivery across CTV, display, and audio — a visual walkthrough of the AdCP media buy workflow."
---
...
| Flight dates | `start_date` / `end_date` on tactics |
...
Sam wants to narrow down. He sends a follow-up brief — and the agents refine their proposals without starting over:
...
The `context_id` links this to the previous discovery. The agent understands Sam is narrowing, not starting fresh.
