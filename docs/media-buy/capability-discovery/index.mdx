...
### [`list_authorized_properties`](/docs/media-buy/task-reference/list_authorized_properties) <sup>deprecated</sup>
Get all properties a sales agent is authorized to represent. **Deprecated** - use `get_adcp_capabilities` for portfolio information via the `media_buy.portfolio` section.
...
2. **Understand Formats**: Call [`list_creative_formats`](/docs/media-buy/task-reference/list_creative_formats) to learn supported creative types
