...
- **`authorized_agents`** *(required unless `authoritative_file` is present)*: Array of authorized sales agents
...
- **`authoritative_file`** *(optional, alternative to `authorized_agents`)*: Full URL to the authoritative adagents.json file for this publisher. When present, buyer agents should fetch and use the file at this URL instead. Enables delegation to third-party managers who can serve property-specific files (static or dynamically generated)
...
## Authoritative File Redirects

The optional `authoritative_file` field enables publishers to delegate **their entire adagents.json file** to another entity by specifying the exact URL where the complete, authoritative file is hosted. When present, buyer agents should fetch and use that file instead of the local file - replacing **both the property list and the authorized agents**.

**What gets replaced:** The ENTIRE file - properties, tags, authorized_agents, everything. The authoritative file is the complete source of truth for what properties exist and who can sell them.

**Critical distinction:** This is for **ownership/management delegation**, not sales representation. The delegating publisher must control their domain and choose to have someone else host their complete authorization file.

**Why full URLs matter:**
- Managers can serve property-specific files (e.g., `meta.com/.well-known/adagents/instagram.json`)
- Supports both static files and dynamic generation (e.g., `?brand=subsidiary-name`)
- No ambiguity about which file to fetch
- Managers can organize files however they want
- Each property gets a distinct, cacheable URL

**Valid use cases (publisher controls domain):**
- **Multi-brand corporations**: Instagram/Facebook/WhatsApp (owned by Meta) delegate complete file to Meta
- **Subsidiary relationships**: Local news site delegates complete file to parent media company
- **Publisher consortiums**: Member publishers delegate to consortium's centralized management
- **Ad server / platform hosting**: Publisher's properties and authorization managed by their ad server or SSP platform
- **White-label platforms**: Publishers using platform delegate complete file to platform provider

**NOT for (these use standard authorization patterns):**
- **Sales representation**: DOOH network selling venues they don't own - venues don't have domains to delegate from
- **Ad network relationships**: Independent publishers working with ad networks - use authorized_agents instead
- **Reseller relationships**: Use the authorization_type patterns, not delegation

### Static Property-Specific Files

Manager hosts separate files for each property:
...
```json
{
  ...
  "authoritative_file": "https://meta.com/.well-known/adagents/instagram.json",
  "last_updated": "2025-01-10T18:00:00Z"
}
```

**What happens**: Buyer agents fetch `instagram.com/.well-known/adagents.json`, find the `authoritative_file` URL, then fetch `https://meta.com/.well-known/adagents/instagram.json` for Instagram-specific authorization.

**Organizational patterns:**
- By property: `meta.com/.well-known/adagents/instagram.json`
- By domain: `meta.com/.well-known/adagents/instagram.com.json`
- Nested: `meta.com/.well-known/adagents/social/instagram.json`

### Dynamic Generation: Platform/Ad Server Hosting

Publisher delegates to their ad server or SSP platform, which dynamically generates the COMPLETE file (properties + authorization) from their system of record:
...
```json
{
  ...
  "contact": {
    ...
    "email": "ads@publisher.com",
    "domain": "publisher.com"
  },
  "authoritative_file": "https://adserver.example.com/adagents?publisher=publisher.com"
}
```

**What the platform returns** (dynamically generated from ad server database):
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/v1/adagents.json",
  "contact": {
    "name": "Example Ad Server",
    "domain": "adserver.example.com"
  },
  "properties": [
    {
      "property_type": "website",
      "name": "Publisher Main Site",
      "identifiers": [
    ...
    // Properties pulled from ad server's property database
  ],
  "tags": {
    "web": {"name": "Web Properties", "description": "Website inventory"},
    "mobile": {"name": "Mobile Apps", "description": "Mobile app inventory"}
  },
  "authorized_agents": [
    {
      "url": "https://sales.publisher.com",
      "authorized_for": "Direct sales",
      "authorization_type": "property_tags",
      "property_tags": ["web", "mobile"]
    },
    {
      "url": "https://programmatic.adserver.example.com",
      "authorized_for": "Programmatic sales through platform",
      "authorization_type": "property_tags",
      "property_tags": ["web", "mobile"]
    }
  ],
...
**Benefits of platform hosting:**
- **Single source of truth**: Properties and authorization pulled from same database as ad serving
- **Real-time accuracy**: Changes in ad server immediately reflected in adagents.json
- **No manual maintenance**: Publisher doesn't manage static files
- **Property discovery**: Platform knows exactly what properties exist and their configuration
- **Automatic updates**: New properties auto-appear when added to ad server

### Minimal Redirect

Simplest possible redirect:
...
### Validation Flow with authoritative_file

When buyer agents encounter an `authoritative_file` field:

1. **Detect redirect**: Parse the publisher's adagents.json and find `authoritative_file` URL
2. **Follow redirect**: Fetch the file at the specified URL
3. **Validate authorization**: Check if the sales agent is authorized in the fetched file
4. **Prevent loops**: Track redirect chain and reject if loop detected or depth > 3
5. **Cache by URL**: Cache the fetched file with the authoritative URL as key
6. **Preserve contact**: Keep original domain's contact info for property-specific questions

**Security considerations:**
- Authoritative URLs MUST use HTTPS
- Buyer agents MUST validate URL format before fetching
- Buyer agents MUST detect and reject redirect loops (if authoritative file has another redirect)
...
- URLs should be validated against SSRF attacks (no localhost, private IPs, etc.)
...
### Example 1: Meta Network with Property-Specific Files

Meta manages Instagram, Facebook, and WhatsApp, with each property redirecting to its own property-specific authorization file at Meta:
...
```json
{
  ...
  "authoritative_file": "https://meta.com/.well-known/adagents/facebook.json",
  "last_updated": "2025-01-10T18:00:00Z"
}
```

**meta.com/.well-known/adagents/instagram.json** (authoritative file for Instagram):
...
### Example 3: Media Company with Local Subsidiaries

A regional media company owns dozens of local news sites. Each site has its own domain but delegates authorization management to the parent company:

**springfield-news.com/.well-known/adagents.json:**
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/v1/adagents.json",
  "contact": {
    "name": "Springfield News",
    "email": "advertising@springfield-news.com",
    "domain": "springfield-news.com"
  },
  "authoritative_file": "https://regionalmedia.com/.well-known/adagents/springfield-news.json",
  "last_updated": "2025-01-10T14:00:00Z"
}
```

**riverside-herald.com/.well-known/adagents.json:**
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/v1/adagents.json",
  "contact": {
  ...
  "authoritative_file": "https://regionalmedia.com/.well-known/adagents/riverside-herald.json"
}
```

**regionalmedia.com/.well-known/adagents/springfield-news.json** (authoritative for Springfield News):
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/v1/adagents.json",
  "contact": {
    "name": "Regional Media Group",
    "email": "adops@regionalmedia.com",
    "domain": "regionalmedia.com"
  },
  "properties": [
    {
      "property_type": "website",
      "name": "Springfield News",
      "identifiers": [
        {"type": "domain", "value": "springfield-news.com"}
      ],
      "publisher_domain": "springfield-news.com",
      "tags": ["regional_media_network", "local_news", "midwest"]
    }
  ],
  "tags": {
    "regional_media_network": {
      "name": "Regional Media Network",
      "description": "All Regional Media Group properties"
    },
    "local_news": {
      "name": "Local News Sites",
      "description": "Community journalism properties"
    }
  },
  "authorized_agents": [
    {
      "url": "https://sales.regionalmedia.com",
      "authorized_for": "Direct sales for Regional Media properties",
      "authorization_type": "property_tags",
      "property_tags": ["regional_media_network"]
    },
    {
      "url": "https://programmatic-partner.com",
      "authorized_for": "Programmatic sales for all local news properties",
      "authorization_type": "property_tags",
      "property_tags": ["local_news"]
    }
  ],
...
**Why this pattern works**: Regional Media Group owns all the subsidiary domains, so each can legitimately delegate to the parent. The parent can manage authorization centrally while preserving local brand identity and contact info.

### Example 4: DOOH Network (Sales Representation - NO Delegation)

For comparison, here's how a DOOH network like Clear Channel should handle authorization - **without** using `authoritative_file`, because they don't own venue domains:

**clearchannel.com/.well-known/adagents.json:**
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/v1/adagents.json",
  "contact": {
    "name": "Clear Channel Outdoor",
    "email": "adops@clearchannel.com",
  ...
  "properties": [
    {
      ...
      "tags": ["clear_channel_network", "nyc_metro", "high_traffic"]
    },
    {
      "property_type": "dooh",
      "name": "Airport Mall Digital",
      "identifiers": [
        {"type": "venue_id", "value": "APT_MALL_045"},
        {"type": "network_id", "value": "clear_channel_airports"}
      ],
      "tags": ["clear_channel_network", "airports", "high_dwell"]
    }
    // ... hundreds more DOOH properties
  ],
  "tags": {
    "clear_channel_network": {
      "name": "Clear Channel Network",
      "description": "All Clear Channel represented DOOH properties"
    }
  },
  "authorized_agents": [
    {
...
**Why NO delegation**: DOOH venues don't have domains to host adagents.json files. Clear Channel directly lists all properties they represent in their own file. This is **sales representation**, not ownership delegation.
