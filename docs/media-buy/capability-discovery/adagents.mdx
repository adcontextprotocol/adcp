...
The optional `authoritative_file` field enables publishers to delegate their advertising operations to a third-party manager by specifying the exact URL of the authoritative adagents.json file. When present, buyer agents should fetch and use the file at this URL instead of the local file.

**Why full URLs matter:**
- Managers can serve property-specific files (e.g., `meta.com/.well-known/adagents/instagram.json`)
- Supports both static files and dynamic generation (e.g., `?domain=instagram.com`)
...
**Common use cases:**
- **Multi-brand networks**: Instagram/Facebook/WhatsApp redirect to property-specific Meta files
- **DOOH networks**: Individual venue domains redirect to venue-specific network files
- **Publisher consortiums**: Member publishers redirect to consortium's per-publisher files
- **White-label platforms**: Publishers redirect to platform provider with dynamic generation
...
### DOOH Network with Dynamic Generation

Clear Channel manages hundreds of venue domains. Instead of maintaining separate static files, they use dynamic generation:
...
```json
{
  ...
  "authoritative_file": "https://clearchannel.com/adagents?venue=timessquare",
  "last_updated": "2025-01-10T14:00:00Z"
}
```

**airportmall-venue.com/.well-known/adagents.json:**
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/v1/adagents.json",
  "contact": {
    "name": "Airport Mall Venue Operations"
  },
  "authoritative_file": "https://clearchannel.com/adagents?venue=airportmall"
}
```

**clearchannel.com/adagents?venue=timessquare** (dynamically generated):
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/v1/adagents.json",
  "contact": {
  ...
  "tags": {
    ...
    "high_traffic": {
      ...
      "description": "Premium high-traffic venues"
    }
  },
  "authorized_agents": [
    {
...
**Why this pattern**: Clear Channel can manage hundreds of venues without maintaining separate static files for each. The server generates venue-specific authorization on-demand from a database.
