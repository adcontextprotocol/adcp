...
AdCP provides a hosted validation API that performs comprehensive checks including structure validation, URL reference resolution, and agent card verification.
...
```bash CLI
# Validate a domain using the API
curl -X POST https://adcontextprotocol.org/api/adagents/validate \
  -H "Content-Type: application/json" \
  -d '{"domain": "example.com"}' | jq '.data.validation'

# Create and validate a new adagents.json structure
curl -X POST https://adcontextprotocol.org/api/adagents/create \
  -H "Content-Type: application/json" \
  -d '{
    "authorized_agents": [
      {
...
curl -X POST https://adcontextprotocol.org/api/adagents/validate-cards \
  -H "Content-Type: application/json" \
  -d '{"agent_urls": ["https://test-agent.adcontextprotocol.org"]}' | jq '.'
```
...
The validation API returns detailed results including:
...
```json
{
  ...
  "data": {
    ...
    "validation": {
      ...
      "status_code": 200,
      "raw_data": { /* full adagents.json content */ }
    }
  },
...
For direct validation without the API:
