...
The optional `authoritative_file` field enables publishers to delegate **authorization file management** to another entity by specifying the exact URL of the authoritative adagents.json file. When present, buyer agents should fetch and use the file at this URL instead of the local file.

**Critical distinction:** This is for **ownership/management delegation**, not sales representation. The delegating publisher must control their domain and choose to have someone else manage their authorization file.
...
- **Multi-brand corporations**: Instagram/Facebook/WhatsApp (owned by Meta) delegate to Meta
- **Subsidiary relationships**: Local news site delegates to parent media company
- **Publisher consortiums**: Member publishers delegate to consortium's centralized management
- **White-label platforms**: Publishers using platform delegate to platform provider (if publisher owns domain)
...
Manager dynamically generates files based on query parameters:
...
```json
{
  ...
  "authoritative_file": "https://manager.clearchannel.com/adagents?domain=timessquare-venue.com"
}
```

**What happens**: Manager's server receives the request, generates the appropriate authorization file on-the-fly based on the `domain` parameter, and returns it.

**Benefits of dynamic generation:**
- No static file maintenance per property
- Can pull from database or configuration
- Enables real-time authorization updates
- Supports thousands of properties without file proliferation
