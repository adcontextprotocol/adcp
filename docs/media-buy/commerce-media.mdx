...
| Advertiser account with retailer | `account_id` + `list_accounts` | [Accounts & Agents](/docs/building/by-layer/L2/accounts-and-agents) |
...
| On-site display / video | Product with canonical `format_options` | [Media Products](/docs/media-buy/product-discovery/media-products) |
| Retailer-owned shopper signal | Brief-based; optional named segments discovered in `signal_targeting_options` and selected with package-level `signal_targeting_groups` | [Targeting](/docs/media-buy/advanced-topics/targeting) |
...
Before creating media buys, sync the brand's data feeds to the retailer's account via `sync_catalogs`. This builds account state that creatives and targeting reference at serve time. See [Account state](/docs/building/by-layer/L2/account-state) for how this fits the broader setup sequence.
...
- [Account state](/docs/building/by-layer/L2/account-state) — How catalogs, event sources, and campaigns build on the account
- [Accounts & Agents](/docs/building/by-layer/L2/accounts-and-agents) — Account setup and billing relationships
