...
describe('getToolSetDescriptionsForRouter', () => {
  ...
  describe('admin user', () => {
    ...
    it('should include bounded admin domains and hide the legacy set', () => {
      ...
      expect(descriptions).toMatch(/\*\*brand_registry_records\*\*/);
      expect(descriptions).toMatch(/\*\*brand_registry_identity\*\*/);
      expect(descriptions).not.toMatch(/\*\*brand_registry\*\*/);
...
describeWithApi('AddieRouter.route (LLM)', () => {
  ...
  describe('prod: DM conversations — always respond, calibrate confidence', () => {
    ...
    it('should route agent testing requests to agent_quality', async () => {
      const plan = await routeAsMember(
        'test https://david-five-kappa.vercel.app/api/ad-mcp'
      );
      expect(plan.action).toBe('respond');
      if (plan.action === 'respond') {
        expect(plan.tool_sets).toContain('agent_quality');
      }
    }, 15000);
