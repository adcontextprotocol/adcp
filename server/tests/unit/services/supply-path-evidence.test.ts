...
vi.mock('../../../src/utils/url-security.js', async importOriginal => ({ ...await importOriginal<typeof import('../../../src/utils/url-security.js')>(), safeFetchAxiosLike: safeFetch }));
...
describe('registry supply-path IAB fetching', () => {
  ...
  it('fetches only applicable evidence with bounded bytes/time and same-origin redirects', async () => {
    ...
    expect(safeFetch).toHaveBeenCalledWith('https://hoststream.example/app-ads.txt', expect.objectContaining({ maxRedirects: 3, redirectHostPolicy: 'same-origin', maxResponseBytes: 256 * 1024, timeoutMs: 5000 }));
  });
...
it('propagates non-transient network policy refusals instead of returning an IAB verdict', async () => {
  safeFetch.mockReset();
  safeFetch.mockRejectedValue(new TypeError('fetch failed', { cause: new NetworkPolicyRefusedError('URL resolved to a private or internal IP address') }));
  await expect(fetchHostInventoryPartnerDomains(input)).rejects.toThrow('fetch failed');
});
