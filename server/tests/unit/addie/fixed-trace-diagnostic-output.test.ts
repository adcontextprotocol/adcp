...
function scriptedRouter(
  ...
  const response: ModelResponse = {
    ...
    content: [
      {
        type: "text",
        text: JSON.stringify({ action: "ignore", reason: "Synthetic route." }),
      },
    ],
    finishReason: "stop",
  ...
  const provider: ModelProvider = {
    ...
    prepare(request: ModelRequest): PreparedModelInvocation {
      return {
        ...
        providerRequest: structuredClone(request) as unknown as Readonly<
...
function stage(
  provider: ModelProvider,
  pricing: FixedTracePricing = provider.id === "openai"
    ? OPENAI_PRICING
    : PRICING,
...
function twoTurnProvider(afterRouterResponse?: () => void): {
  provider: ModelProvider;
  calls: ModelRequest[];
} {
  const calls: ModelRequest[] = [];
        ...
        : generationTurn++ === 0
          ...
          : {
              ...
              content: [
                {
                  type: "text",
                  text: "A buyer calls a seller task and receives its structured response.",
                },
              ],
              finishReason: "stop",
              providerFinishReason: "stop",
...
describe("fixed-trace diagnostic output reservation", () => {
  it("never overwrites an existing artifact", () => {
    const path = join(
      mkdtempSync(join(tmpdir(), "fixed-trace-output-")),
      "artifact.json",
    );
    writeFileSync(path, "existing");
    expect(() => reserveFixedTraceDiagnosticOutput(path)).toThrow(
      "Cannot exclusively reserve",
    );
  ...
  it("rejects directory and missing-parent targets before dispatch", () => {
    ...
    expect(() => reserveFixedTraceDiagnosticOutput(directory)).toThrow(
      "Cannot exclusively reserve",
    );
    expect(() =>
      reserveFixedTraceDiagnosticOutput(
        join(directory, "missing", "artifact.json"),
      ),
    ).toThrow("Cannot exclusively reserve");
  });
  ...
  it("runs the manual diagnostic candidate path into a complete reserved artifact with scripted providers", async () => {
    ...
    const selectedTrace = FIXED_TRACE_SUITE.find(
      (trace) => trace.id === "surface-channel-chatter",
    );
      ...
      runs: [
        {
          ...
          observations: [
            { traceId: selectedTrace.id, terminalStatus: "ignored" },
          ],
        },
      ],
    });
    ...
    expect(
      artifact.runs[0].observations.every(
        (observation) => observation.metadata.runId === artifact.runs[0].runId,
      ),
    ).toBe(true);
    ...
    expect(
      persisted.runs[0].observations[0].metadata.router.usage,
  ...
  it("rejects a provider-mismatched plan before scripted dispatch", async () => {
    ...
    const invoke = (plans: FixedTraceDiagnosticProviderPlan[]) =>
      runFixedTraceDiagnosticArtifact({
        plans,
        baseConfig,
        budget,
    ...
    await expect(invoke([{ ...plan, name: "not-anthropic" }])).rejects.toThrow(
      "provider plans require unique names",
    );
  ...
  it("rejects a plan identity accessor before it can change validation into execution", async () => {
    ...
    await expect(
      invoke(
        [accessorPlan] as unknown as FixedTraceDiagnosticProviderPlan[],
        failedPath,
      ),
    ...
    const artifact = await invoke(
      [
        {
          name: "anthropic",
          router: accessorPlan.router,
          generation: accessorPlan.generation,
        },
      ],
      completedPath,
    );
  ...
  it("never rereads a delegate identity while cloning an authenticated plan", async () => {
    ...
    const artifact = await runFixedTraceDiagnosticArtifact({
      plans: [
        {
          name: "anthropic",
          router: stage(wrapper),
          generation: stage(wrapper),
        },
      ],
      baseConfig: {
        sourceBundleSha256: "a".repeat(64),
    ...
    await expect(
      invoke(
        [
          {
            name: "anthropic",
            router: { ...trustedRouter, pricing: forgedPricing },
            generation: trustedGeneration,
          },
        ],
        failedPath,
      ),
    ).rejects.toThrow("Fixed trace pricing profile is not evaluator approved");
    expect(router.calls).toHaveLength(0);
    expect(readFileSync(failedPath, "utf8")).toBe("");
    expect(budget.snapshot()).toMatchObject({
      accountedSpendUsd: 0,
    ...
    await expect(
      invoke(
        [
          {
            name: "anthropic",
            router: { ...trustedRouter, pricing: accessorPricing },
            generation: trustedGeneration,
          },
        ],
        failedPath,
      ...
      "provider plan 0.router.pricing.inputUsdPerMillionTokens must be an own data property",
    );
  ...
  it("does not let a final-response mutation alter its snapshotted manual artifact", async () => {
    ...
    const router = scriptedRouter(() => {
      plan.router.model = "mutated-after-final-response";
    });
    const budget = new FixedTraceBudget(1);
    plan = {
      name: "anthropic",
  ...
  it("derives child run IDs internally instead of accepting an unrelated callback result", async () => {
    ...
    expect(artifact.runs[0].observations[0].metadata.runId).toBe(
      "root:anthropic",
    );
  });
  ...
  it("retains an unknown-model response as unknown exposure without inventing a spend equality", async () => {
    ...
    expect(artifact.runs[0].observations[0].metadata.router).toMatchObject({
      source: "provider",
      returnedModel: "unapproved-model",
  ...
  it("rejects a settled ledger for an unpriced dispatched provider response", () => {
    ...
    ).toThrow("unpriced dispatched response lacks unknown budget exposure");
  });
  ...
  it("reconciles a pre-dispatch budget rejection with a local/not-run observation", async () => {
    ...
    expect(artifact.runs[0].observations[0]).toMatchObject({
      ...
      metadata: {
        router: { source: "local", dispatched: false },
  ...
  it("preflights every plan before leasing a pristine budget or dispatching an earlier plan", async () => {
    ...
    await expect(
      invoke([firstPlan, invalidSecondPlan], failedPath),
    ).rejects.toThrow("generation maxIterations must be between");
  ...
  it("prevents post-preflight method and prototype tampering in a two-turn zero-rate run", async () => {
    ...
    const path = join(directory, "artifact.json");
    const selectedTrace = FIXED_TRACE_SUITE.find(
      (trace) => trace.id === "knowledge-task-model",
    );
    ...
    const delegate = twoTurnProvider(() => {
      ...
      replace("own_respond", () =>
        Object.defineProperty(generation, "respond", {
          value: delegate.provider.respond,
        }),
      );
      replace("own_prepare", () =>
        Object.defineProperty(generation, "prepare", {
          value: delegate.provider.prepare,
        }),
      );
      replace("prototype_swap", () => Object.setPrototypeOf(generation, {}));
      replace("prototype_respond", () =>
        Object.defineProperty(BudgetedFixedTraceProvider.prototype, "respond", {
          value: delegate.provider.respond,
        }),
      );
      replace("prototype_prepare", () =>
        Object.defineProperty(BudgetedFixedTraceProvider.prototype, "prepare", {
          value: delegate.provider.prepare,
        }),
      );
    });
    ...
    generation = new BudgetedFixedTraceProvider(
      delegate.provider,
      budget,
      PRICING,
      policy,
    ...
    const artifact = await runFixedTraceDiagnosticArtifact({
      ...
      baseConfig: {
        ...
        traceSuite: [selectedTrace],
        traceSuiteSha256: fixedTraceSuiteSha256([selectedTrace]),
        toolDefinitions: canonicalFixedTraceToolDefinitions().filter((tool) =>
          ["search_docs", "get_doc"].includes(tool.name),
        ),
        toolDefinitionProvenance: "fixture_local",
        architectureArm: "two_stage_llm_router",
      },
    ...
    expect(artifact.runs[0].summary.totalEstimatedCostUsd).toBeCloseTo(
      0.000105,
    );
    expect(artifact.budget).toMatchObject({
      dispatchedCalls: 3,
  ...
  it("rejects a zero-rate ledger with preexisting completed, unknown, or rejected activity", async () => {
      ...
      await expect(
        runFixedTraceDiagnosticArtifact({
          plans: [
            {
              name: "anthropic",
              router: budgetedStage(provider.provider, budget),
              generation: budgetedStage(provider.provider, budget),
            },
          ],
          baseConfig: {
            sourceBundleSha256: "a".repeat(64),
          ...
          budget,
          outputReservation: reserveFixedTraceDiagnosticOutput(
            join(directory, `${suffix}.json`),
          ),
          runRootId: "root",
          runStartedAt: "2026-09-05T00:00:00.000Z",
          sourceBundleFiles: ["synthetic.ts"],
    ...
    const completedReservation = completed.reserve(
      prepared,
      1,
      ZERO_RATE_PRICING,
    );
