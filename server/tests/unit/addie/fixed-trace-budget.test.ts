...
describe("fixed trace provider budget", () => {
  it("exposes only reviewed production pricing and rejects former test profiles before dispatch", () => {
    ...
    for (const profile of liveProfiles) {
      expect(
        `${profile.expectedModel}\n${profile.profileId}\n${profile.source}`,
      ).not.toMatch(/synthetic|test/i);
    }

    const delegate = new BudgetScriptedProvider([RESPONSE]);
    expect(() =>
      fixedTraceResponsePricingPolicy("anthropic", "synthetic-manual-model", {
  ...
  it("fails closed when a nonzero cache bucket has no recorded formula", () => {
    expect(() =>
      fixedTraceEstimatedCostUsd(
        { inputTokens: 10, outputTokens: 0, cacheReadTokens: 1 },
        {
          ...PRICING,
          cacheReadUsdPerMillionTokens: null,
          cacheReadAccounting: "unsupported",
  ...
  it("closes shared admission rather than settling an unapproved returned model at requested rates", async () => {
    ...
    const provider = new BudgetedFixedTraceProvider(
      ...
      budget,
      PRICING,
      RESPONSE_PRICING_POLICY,
    );

    await expect(
      collectModelResponse(provider.respond(REQUEST)),
    ...
    await expect(
      ...
      reason: "budget_exposure_unknown",
    });
  ...
  it("rejects a caller callback as returned-model pricing authority", () => {
    ...
    expect(
      () =>
        new BudgetedFixedTraceProvider(
          delegate,
          budget,
          PRICING,
  ...
  it("halts later calls after a dispatched response has unknown usage", async () => {
    ...
    await expect(
      collectModelResponse(provider.respond(REQUEST)),
    ).rejects.toBeInstanceOf(FixedTraceBudgetAdmissionError);
    expect(delegate.dispatches).toHaveBeenCalledTimes(1);
  ...
  it("treats malformed terminal usage as unknown exposure", async () => {
    ...
    await expect(
      collectModelResponse(provider.respond(REQUEST)),
    ).rejects.toThrow("Fixed trace budget usage is invalid");
    expect(budget.snapshot()).toMatchObject({
      reservedUsd: 0,
      remainingUsd: null,
      dispatchedCalls: 1,
    ...
    await expect(
      collectModelResponse(
        provider.respond(REQUEST, {
          beforeDispatch: () => {
            throw new Error("local policy rejected");
          },
        }),
      ),
