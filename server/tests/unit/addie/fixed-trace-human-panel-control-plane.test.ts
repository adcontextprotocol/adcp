...
describe("fixed-trace human-primary control plane", () => {
  ...
  it("requires signed reservation, dispatch, and reconciliation in one monotonic cost chain", () => {
    const reservation = (eventIndex = 1, previousEventDigest: string | null = null, committedCents = 1, reservationId = "reservation") => { const body = { phase: "reservation" as const, trustRootId: "addie-human-panel-evaluator-v1", reservationId, subjectFingerprint: hash(1), authorizedRateCents: 1, committedCents, eventIndex, previousEventDigest, timestamp: `2026-01-01T00:00:0${eventIndex}.000Z` }; return { ...body, eventFingerprint: digest(body), signature: "YW55dGhpbmc=" }; };
    const assigned = (reservationEvent: ReturnType<typeof reservation>, eventIndex = 2) => { const body = { phase: "assignment" as const, trustRootId: reservationEvent.trustRootId, reservationId: reservationEvent.reservationId, subjectFingerprint: reservationEvent.subjectFingerprint, eventIndex, previousEventDigest: reservationEvent.eventFingerprint, timestamp: `2026-01-01T00:00:0${eventIndex}.000Z` }; return { ...body, eventFingerprint: digest(body), signature: "YW55dGhpbmc=" }; };
    const reconciliation = (assignmentEvent: ReturnType<typeof assigned>, eventIndex = 3, actualCents = 1) => { const body = { phase: "reconciliation" as const, trustRootId: assignmentEvent.trustRootId, reservationId: assignmentEvent.reservationId, subjectFingerprint: assignmentEvent.subjectFingerprint, completedResponseFingerprint: hash(2), actualCents, eventIndex, previousEventDigest: assignmentEvent.eventFingerprint, timestamp: `2026-01-01T00:00:0${eventIndex}.000Z` }; return { ...body, eventFingerprint: digest(body), signature: "YW55dGhpbmc=" }; };
    ...
    const reserved = reservation(); const dispatched = assigned(reserved); const reconciled = reconciliation(dispatched);
    expect(validateFixedTraceHumanCostLedger(ledger([reserved, dispatched, reconciled]) as any)).toBe(1);
    ...
    expect(() => validateFixedTraceHumanCostLedger(ledger([reserved, reconciled]) as any)).toThrow(/precedes|reordered/); // response before a reservation-backed dispatch
    ...
    expect(() => validateFixedTraceHumanCostLedger(ledger([reserved, assigned(reserved, 3)]) as any)).toThrow(/reordered|forked|gap/);
    expect(() => validateFixedTraceHumanCostLedger(ledger([reserved, reservation(2, reserved.eventFingerprint, 1, "duplicate-subject")]) as any)).toThrow(/duplicate/);
    ...
    expect(() => validateFixedTraceHumanCostLedger(ledger([reserved, dispatched, mismatchedReconciliation]) as any, [{ subjectFingerprint: hash(1), completedResponseFingerprint: hash(2) }])).toThrow(/completed response/);
    expect(() => validateFixedTraceHumanCostLedger(ledger([reserved, dispatched, reconciliation(dispatched, 3, 65_001)]) as any)).toThrow(/exceeds \$650/);
  });
