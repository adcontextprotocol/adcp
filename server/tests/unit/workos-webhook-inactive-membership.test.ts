...
describe('organization_membership webhook reconciliation', () => {
  ...
  it('cleans authoritative provider absence without owner succession', () => {
    expect(providerAbsentBlock).toContain('deleteExactOrganizationMembership(');
    expect(providerAbsentBlock).toContain('exactDeletion.matched');
    expect(providerAbsentBlock).toContain('deleteOrganizationMembership(');
