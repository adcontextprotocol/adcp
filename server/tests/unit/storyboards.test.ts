...
describe('wrapper contract', () => {
  ...
  it('caps hosted aliases at registered published compliance bundles', () => {
  ...
  it('loads the released 3.1.23 bundle for canonical hosted 3.1 selection', () => {
  ...
  it.each(['3.1.18', '3.1.20', '3.1.22', '3.1.23'])('keeps exact stable target %s pinned for capability discovery', (version) => {
    ...
    expect(loadComplianceIndex(hostedComplianceOptions(target)).adcp_version).toBe(version);
  });
