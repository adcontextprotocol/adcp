...
describe('POST /api/admin/users/:userId/linked-emails (admin bind)', () => {
  ...
  beforeEach(async () => {
    ...
    await pool.query(`INSERT INTO working_group_memberships (working_group_id, workos_user_id, status)
      SELECT id, 'user_test_admin_bind_credential', 'active' FROM working_groups WHERE slug = 'aao-admin'`);
  ...
  async function cleanup() {
    await pool.query(`DELETE FROM working_group_memberships
      WHERE workos_user_id = 'user_test_admin_bind_credential'`);
