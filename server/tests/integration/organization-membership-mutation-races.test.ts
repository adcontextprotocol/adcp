...
vi.mock('@workos-inc/node', () => {
  ...
  return { WorkOS: class {
    apiKeys = { createValidation: async () => { state.apiKeyValidations++; return { apiKey: { id: 'key_test', name: 'test', owner: { type: 'organization', id: 'org_mutation_race_test' }, permissions: ['admin'] } }; } };
    userManagement = {
...
async function exactRole(userId: string, role: string, actor = A) {
  return request(app).post(`/api/organizations/${org}/members/by-email`)
    .set('Cookie', await cookie(actor)).set('X-CSRF-Token', CSRF_TOKEN).send({ email: `${userId}@membership-race.example.test`, role });
}
...
describe('independent mounted management race and residual-state attacks', () => {
  ...
  it('JWT verification source outage returns authorization_unavailable before provider reads or writes', async () => {
    ...
    try {
      ...
      expect(response.body.error).toBe('Authentication service temporarily unavailable');
      expect(state.reads).toEqual([]);
      await noEffects();
    } finally {
      __setJWKSForTesting(async () => verificationKey);
  ...
  it('an API-key prefix is not reclassified as a malformed JWT and still cannot authorize a human mutation', async () => {
    const response = await request(app).post(`/api/organizations/${org}/invitations`)
      .set('Authorization', 'Bearer sk_.e30.eA').send({ email: 'invitee@membership-race.example.test' });
    expect(response.status).toBe(403);
    expect(state.apiKeyValidations).toBe(1);
    expect(state.reads).toEqual([]);
    await noEffects();
  });

  it('a malformed compact JWT without an API-key prefix remains local 401', async () => {
    const response = await request(app).post(`/api/organizations/${org}/invitations`)
      .set('Authorization', 'Bearer xk_.e30.eA').send({ email: 'invitee@membership-race.example.test' });
    expect(response.status).toBe(401);
  ...
  it('bearer user-source outage returns authorization_unavailable before provider reads or writes', async () => {
    ...
    try {
      const response = await request(app).post(`/api/organizations/${org}/invitations`)
        .set('Authorization', `Bearer ${bearer}`).send({ email: 'invitee@membership-race.example.test' });
      expect(response.status).toBe(503);
      expect(response.body.error).toBe('Authorization service temporarily unavailable');
      expect(state.reads).toEqual([]);
      expect(state.writes).toEqual([]);
    } finally {
  ...
  it('an exact admin cannot demote the sole owner', async () => {
    await member(B, 'admin');
    const response = await exactRole(A, 'member', B);
    expect(response.status).toBe(403);
    await noEffects();
  ...
  for (const source of ['provider', 'local']) it(`remaining owner ${source} revoke during demotion preflight prevents every write`, async () => {
    ...
    state.afterTargetRead = async () => { state.afterTargetRead = gate.hook; };
    const pending = exactRole(target, 'member').then(r => r);
    await gate.arrived;
  ...
  it('concurrent exact owner demotions preserve one owner and honest audit', async () => {
    await member(target, 'owner');
    const gate = barrier(); state.afterWrite = gate.hook;
    const first = exactRole(A, 'member', target).then(r => r);
    await gate.arrived;
    const second = exactRole(target, 'member').then(r => r);
    gate.release();
  ...
  for (const source of ['bans', 'authorization_epochs', 'identity_workos_users', 'organization_memberships']) it(`${source}: required SQL source outage is terminal 503 before every provider call`, async () => {
    ...
    try {
      ...
      expect(response.status, JSON.stringify(response.body)).toBe(503);
      // Current-main distinguishes authentication-snapshot outages from
      // later mutation-fence outages while both remain terminal 503s.
      const error = source === 'authorization_epochs' || source === 'identity_workos_users'
  ...
  for (const lock of ['organization', 'advisory', 'actor']) it(`${lock} lock contention is a defined 409 without mutation effects`, async () => {
    const blocker = await pool.connect();
    try {
      await blocker.query('BEGIN');
      if (lock === 'organization') await blocker.query('SELECT 1 FROM organizations WHERE workos_organization_id=$1 FOR UPDATE', [org]);
      else if (lock === 'advisory') await blocker.query('SELECT pg_advisory_xact_lock(hashtext($1))', [`membership-ownerless-promote:${org}`]);
      else await blocker.query('SELECT 1 FROM users WHERE workos_user_id=$1 FOR UPDATE', [A]);
      const response = await call(families[1], await cookie());
      expect(response.status, JSON.stringify(response.body)).toBe(409);
      expect(response.body).toEqual({ error: 'membership_state_conflict' });
      if (lock !== 'actor') expect(state.reads).toEqual([]);
      await noEffects();
      expect((await pool.query('SELECT status FROM organization_join_requests WHERE id=$1', [joinId])).rows[0].status).toBe('pending');
    } finally { await blocker.query('ROLLBACK'); blocker.release(); }
  });
  it('local lock contention after a provider write returns honest reconciliation 503', async () => {
    const blocker = await pool.connect();
    try {
      await blocker.query('BEGIN');
      await blocker.query('SELECT 1 FROM users WHERE workos_user_id=$1 FOR UPDATE', [A]);
