...
const CollectionEventPayloadSchema = z
  .object({
    ...
    collection: z.record(z.string(), z.unknown()).optional().openapi({
      description:
        "Normalized collection body (core/collection.json shape). On authoritative projections, " +
        "third-party distribution[] entries carry a registry-computed `host_confirmed` boolean: " +
        "whether the host's own adagents.json affirmatively names the owning publisher in a " +
        "collections selector reaching the claimed properties. It corroborates the (owner, " +
        "collection, host, properties) path ONLY — it says nothing about the entry's " +
        "identifiers[]. It is derived from the host manifest cached at the owner's last " +
        "projection — never publisher-authored (the origin registry strips publisher-supplied " +
        "values) — so a host that withdraws its grant leaves a stale true until the owner's next " +
        "crawl, and false covers both 'host does not corroborate' and 'host not yet crawled'. " +
        "Absent on community-contributed projections, self-carriage (host == owner) entries, and " +
        "entries past the per-collection annotation cap. Mirrors consuming this feed re-serve " +
        "whatever the feed carried and MUST NOT present the flag as locally verified. Carriage " +
        "remains discovery data; for enforcement use the supply-path verifier or the " +
        "authoritative files.",
    }),
    changed_fields: ChangedFieldsSchema.optional(),
    ...
    try {
      ...
      if (verdict.legs.host_authorization.ok) {
        ...
        verdict = verifySupplyPath({
          ...
          hostManifest,
          hostInventoryPartnerDomains: null,
          hostInventoryPartnerDomainsByFile: await fetchHostInventoryPartnerDomains({ ownerDomain, hostDomain, agentUrl: parsed.data.agent_url, collectionId: parsed.data.collection_id, ownerManifest, hostManifest, hostInventoryPartnerDomains: null }),
        });
      }
