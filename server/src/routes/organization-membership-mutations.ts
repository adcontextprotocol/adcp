...
function assignRole(tx: Mutation, role: Role): void {
  if (role === 'owner' && tx.role !== 'owner') mutationDenied('Only owners can assign the owner role');
}
...
async function roleChangeAllowed(tx: Mutation, member: OrganizationMembership, role: Role): Promise<void> {
  ...
  assignRole(tx, role);
  if (member.role.slug === 'owner' && tx.role !== 'owner') mutationDenied("Only owners can change another owner's role");
...
async function sendInvitation(tx: Mutation, email: string, role: Role): Promise<Invitation> {
  ...
  const invitation = await tx.providerWrite('send_invitation', { email, role }, () => tx.workos.userManagement.sendInvitation({
    email, organizationId: tx.orgId, roleSlug: role, inviterUserId: tx.actorId,
  }), async created => {
...
function handler(minimumRole: Role, operation: (tx: Mutation, req: Request) => Promise<MutationReply>): RequestHandler {
  return async (req, res) => {
    try {
      const result = await Mutation.run(req, minimumRole, tx => operation(tx, req));
      res.status(result.status ?? 200).json(result.body);
    } catch (error) {
      logger.error({ error, actorId: req.user ? getOrganizationAuthorizationUserId(req.user) : undefined, orgId: req.params.orgId }, 'Membership mutation rejected');
...
export function registerOrganizationMembershipMutations(router: Router): void {
  for (const decision of ['approve', 'reject'] as const) {
    router.post(`/:orgId/join-requests/:requestId/${decision}`, excludeOrganizationAuthorizationObservation, requireAuth, handler('admin', async (tx, req) => {
      const role = decision === 'approve' ? roleInput(req.body?.role, false) : 'member';
  ...
  router.post('/:orgId/invitations', excludeOrganizationAuthorizationObservation, requireAuth, invitationRateLimiter, handler('admin', async (tx, req) => {
  ...
  router.delete('/:orgId/invitations/:invitationId', excludeOrganizationAuthorizationObservation, requireAuth, handler('admin', async (tx, req) => {
  ...
  router.post('/:orgId/invitations/:invitationId/resend', excludeOrganizationAuthorizationObservation, requireAuth, handler('admin', async (tx, req) => {
  ...
  router.post('/:orgId/domain-users/add', excludeOrganizationAuthorizationObservation, requireAuth, handler('admin', async (tx, req) => {
    const email = emailInput(req.body?.email);
  ...
  router.post('/:orgId/members/by-email', excludeOrganizationAuthorizationObservation, requireAuth, handler('admin', async (tx, req) => {
    const email = emailInput(req.body?.email);
    const role = roleInput(req.body?.role);
    const seat = seatInput(req.body?.seat_type);
    assignRole(tx, role);
    ...
    if (!members.length) {
      if (user.id === tx.actorId) mutationDenied();
  ...
  router.patch('/:orgId/members/:membershipId', excludeOrganizationAuthorizationObservation, requireAuth, handler('admin', async (tx, req) => {
  ...
  router.delete('/:orgId/members/:membershipId', excludeOrganizationAuthorizationObservation, requireAuth, handler('admin', async (tx, req) => {
    const member = await targetMember(tx, req.params.membershipId);
  ...
  router.post('/:orgId/seat-requests', excludeOrganizationAuthorizationObservation, requireAuth, invitationRateLimiter, handler('member', async (tx, req) => {
  ...
  for (const decision of ['approve', 'deny'] as const) {
    router.post(`/:orgId/seat-requests/:requestId/${decision}`, excludeOrganizationAuthorizationObservation, requireAuth, handler('admin', async (tx, req) => {
