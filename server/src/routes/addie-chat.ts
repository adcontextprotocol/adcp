...
export function createAddieChatRouter(): { pageRouter: Router; apiRouter: Router } {
  ...
  apiRouter.post("/", optionalAuth, chatRateLimiter, anonymousDailyLimiter, async (req, res) => {
    ...
    try {
      ...
      const { messageToProcess, requestTools: memberTools } = await prepareRequestWithMemberTools(
      ...
      let response;
      try {
        response = await claudeClient.processMessage(messageToProcess, contextMessages, requestTools, undefined, processOptions);
      } catch (error) {
  ...
  apiRouter.post("/stream", optionalAuth, chatRateLimiter, anonymousDailyLimiter, async (req, res) => {
    ...
    try {
      ...
      // Prepare message with member context and per-request tools
      const { messageToProcess, requestTools: memberTools, siAgents } = await prepareRequestWithMemberTools(
        inputValidation.sanitized,
        req.user?.id,
        externalId,
        isAuth
      ...
      for await (const event of claudeClient.processMessageStream(messageToProcess, contextMessages, requestTools, processOptions)) {
