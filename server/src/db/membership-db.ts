...
export interface ExactOrganizationMembershipDeletion {
  matched: boolean;
  role: string | null;
}
...
export async function deleteExactOrganizationMembership(
  ...
  membershipId: string,
  client: PoolClient,
): Promise<ExactOrganizationMembershipDeletion> {
  const result = await client.query<{ role: string }>(
    `DELETE FROM organization_memberships
  ...
  return { matched: result.rowCount === 1, role: result.rows[0]?.role ?? null };
}
