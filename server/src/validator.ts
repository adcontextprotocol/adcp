...
export class AgentValidator {
  ...
  private matchesCollections(
    ...
    return selectors.some((selector) =>
      requested.some((candidate) =>
        canonicalizePublisherDomain(selector.publisher_domain) === candidate.publisher_domain &&
  ...
  private normalizeCollectionSelectors(
    ...
    for (const value of selectors) {
      ...
      if (typeof selector.publisher_domain !== "string" || !Array.isArray(selector.collection_ids)) {
        continue;
      }
      ...
      const ids = idsByDomain.get(domain) || new Set<string>();
      for (const collectionId of selector.collection_ids) {
