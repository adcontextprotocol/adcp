...
// Membership page - serve membership.html at /membership
    ...
    this.app.get('/api/admin/stats', requireAuth, requireAdmin, async (req, res) => {
      try {
        ...
        // Run additional stats queries in parallel for better performance
        const [slackStats, addieStats, addieRatings, userStats, orgStats, recentBookings] = await Promise.all([
          // Slack stats (consolidated into single query)
          pool.query(`
            SELECT
        ...
        res.json({
          ...
          monthly_revenue: formatCurrency(parseInt(revenue.current_month_revenue)), // Alias for dashboard
          ...
          bookings_30d_revenue: formatCurrency(parseInt(bookings.booking_revenue)),
          ...
          addie_avg_rating: parseFloat(ratings.avg_rating).toFixed(1),
          ...
          avg_engagement_score: parseFloat(users.avg_engagement).toFixed(0),
