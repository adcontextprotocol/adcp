...
export const MEMBER_TOOLS: AddieTool[] = [
  ...
  // ============================================
  {
    name: 'save_agent',
    description:
      'Register an agent in the AAO registry on behalf of the current organization. The agent record is added to the org\'s member profile and surfaces in `/dashboard/agents` and (when authorized) the public registry. New agents land with `members_only` visibility — discoverable to fellow Professional-tier (or higher) AAO members, but not publicly listed in the directory or brand.json. To list publicly, the caller promotes the agent via the dashboard publish flow; that flow gates on an API-access subscription tier. The agent\'s type (`brand`, `sales`, `buying`, `measurement`, `creative`, `signals`, etc.) is auto-detected by probing `get_adcp_capabilities` at save time — never ask the user for type. Optionally store credentials securely (encrypted, never shown in conversations). Auth modes: (1) none — public agent, no credentials needed; (2) static bearer or basic via `auth_token` + `auth_type`; (3) OAuth 2.0 client credentials (RFC 6749 §4.4, machine-to-machine) via `oauth_client_credentials`; (4) OAuth user authorization — interactive sign-in advertised by the agent\'s `/.well-known/oauth-protected-resource`, no fields collected here. Once saved, the registry can run storyboards based on the agent\'s declared capabilities — but compliance monitoring is a downstream benefit, not the reason to register.',
    usage_hints: 'use for "register an agent in the AAO registry", "register my agent", "add an agent", "connect my agent", "save my agent", "remember this agent URL", "store my auth token", "configure client credentials", "save OAuth client credentials". When the user opens the conversation asking to register an agent without providing details, do not call save_agent immediately — first ask for the agent URL, then ask which auth method they need (none / static bearer / static basic / OAuth client credentials / OAuth user authorization), then collect the matching fields. Tell them the agent type is auto-detected from a capability probe at save time, so they do not need to declare it.',
    input_schema: {
      type: 'object',
      properties: {
        agent_url: { type: 'string', description: 'Agent URL' },
