...
interface CreateMessageInputBase {
  ...
  tools_used?: string[];
  tool_calls?: Array<{ name: string; input: unknown; result: unknown; duration_ms?: number; is_error?: boolean; result_status?: string; dispatch_status?: 'not_dispatched'; github_issue_receipt?: unknown }>;
...
export interface ThreadMessage {
  ...
  tool_calls: Array<{ name: string; input: unknown; result: unknown; duration_ms?: number; is_error?: boolean; result_status?: string; dispatch_status?: 'not_dispatched'; github_issue_receipt?: unknown }> | null;
        ...
        const priorUnknownOutcome = await client.query(
          `SELECT 1
           ...
           WHERE reservation.thread_id = $1
             ...
             -- later success or executor-proven non-dispatch settles this
             -- reservation; an ambiguous error, empty result, or missing result
             -- deliberately leaves it unknown and replay-safe.
             AND NOT EXISTS (
               SELECT 1
               ...
               WHERE receipt.thread_id = reservation.thread_id
                 ...
                 AND (
                   -- Only the executor can emit this marker, after a second
                   -- authority check rejects before handler entry.
                   receipt_call->>'dispatch_status' = 'not_dispatched'
                   OR (
                     -- An empty result is not an error but is also not a
                     -- confirmed mutation. Only an explicit ok receipt retires
