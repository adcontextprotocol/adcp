...
export const SYNTHETIC_ROUTER_CORPUS: ReadonlyArray<RouterEvalCase> = Object.freeze([
  ...
  { id: 'community-groups', context: dm('Show the working groups I belong to and list the measurement committee documents.'), expected: { action: 'respond', toolSets: ['community_groups'], confidence: 'high', requiresDepth: false } },
  ...
  { id: 'brand-canonical', context: dm('Check whether this leaf brand and its corporate house have reciprocal brand.json assertions.'), expected: { action: 'respond', toolSets: ['brand_registry'], confidence: 'high', requiresDepth: false } },
  { id: 'agent-validation', context: dm('Validate my AdCP agent implementation.'), expected: { action: 'respond', toolSets: ['agent_validation'], confidence: 'high', requiresDepth: false } },
  { id: 'property-catalog', context: dm('Audit this property list and add legitimate missing domains to the catalog.'), expected: { action: 'respond', toolSets: ['property_catalog'], confidence: 'high', requiresDepth: false } },
  { id: 'publisher-property-diagnosis', context: dm('My adagents.json is valid but my agent still cannot see publisher properties. Diagnose the whole setup.'), expected: { action: 'respond', toolSets: ['agent_validation', 'property_catalog'], confidence: 'high', requiresDepth: true } },
  ...
  { id: 'publishing-submission', context: dm('Publish the article in this Google Doc for editorial review.'), expected: { action: 'respond', toolSets: ['publishing'], confidence: 'high', requiresDepth: false } },
  { id: 'publishing-social-draft', context: dm('Draft LinkedIn posts about my published perspective.'), expected: { action: 'respond', toolSets: ['publishing'], confidence: 'high', requiresDepth: false } },
  ...
  { id: 'publishing-cover-regeneration', context: dm('Regenerate the cover illustration for my published post.'), expected: { action: 'respond', toolSets: ['publishing'], confidence: 'high', requiresDepth: false } },
  { id: 'publishing-confirmation', context: { ...dm('Approve it.'), isAAOAdmin: true, isThread: true, threadMessages: ['User: Show me the pending editorial review.', 'Addie: The requested article is pending. Would you like me to approve it?'] }, expected: { action: 'respond', toolSets: ['publishing'], confidence: 'high', requiresDepth: false } },
  ...
  { id: 'multi-intent', context: dm('Inspect the schema fields and then validate my implementation against them.'), expected: { action: 'respond', toolSets: ['schema_reference', 'agent_validation'], confidence: 'high', requiresDepth: true } },
