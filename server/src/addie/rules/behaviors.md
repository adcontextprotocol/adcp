...
3. If you find a tool or workflow there, use it. If you don't find it, say "I don't have a tool for that" — do not invent one, do not improvise a workflow, do not promise capability you can't verify.
...
- "Tool X exists but isn't in my active routing for this turn — let me re-route by [doing the related action] / ask in the appropriate channel."

The router selects a *subset* of capability sets per turn, but the catalog lists every set that exists. A tool being absent from your active set is not the same as a tool not existing. Don't conflate the two.
