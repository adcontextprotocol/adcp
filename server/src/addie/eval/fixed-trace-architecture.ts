...
export const FIXED_TRACE_ARCHITECTURE_ARMS = Object.freeze({
  ...
  direct_generation: Object.freeze({
    ...
    admission: "not_admitted_architecture",
    rolloutEligible: false,
    diagnosticOnly: true,
  }),
  deterministic_policy_llm_fallback_hybrid: Object.freeze({
    ...
    routeSource:
      "reviewed_safe_subset_of_production_quick_match_with_unchanged_llm_fallback",
    rolloutEligible: false,
    diagnosticOnly: true,
  }),
...
export type FixedTraceArchitectureArmId =
  keyof typeof FIXED_TRACE_ARCHITECTURE_ARMS;
 ...
 */
export const FIXED_TRACE_HYBRID_POLICY_VERSION =
  "fixed-trace-hybrid-safe-subset-v2";
export const FIXED_TRACE_HYBRID_SAFETY_GATE_VERSION =
  "exact-harmless-terminal-admission-v1";
...
export function validateFixedTraceHybridPolicy(
  policy: FixedTraceHybridPolicy,
): void {
  if (!policy.version.trim())
    throw new Error("Fixed trace hybrid policy version is required");
  if (
    !Array.isArray(policy.localTerminalActions) ||
    policy.localTerminalActions.length === 0 ||
    policy.localTerminalActions.some(
      (action) => action !== "ignore" && action !== "react",
    ) ||
    new Set(policy.localTerminalActions).size !==
      policy.localTerminalActions.length ||
    policy.safetyGateVersion !== FIXED_TRACE_HYBRID_SAFETY_GATE_VERSION ||
    policy.safetyGateStatus !== "reviewed_safe_subset" ||
    policy.localAdmissionSource !==
      "reviewed_safe_subset_of_production_quick_match" ||
...
function ownDataProperty(
  ...
  try {
    if (typeof source !== "object" || source === null) {
      throw new FixedTraceHybridAdmissionSnapshotError(
        `Hybrid admission ${owner} must be an object`,
      );
    }
    const descriptor = Object.getOwnPropertyDescriptor(source, name);
    if (!descriptor || !("value" in descriptor)) {
      throw new FixedTraceHybridAdmissionSnapshotError(
        `Hybrid admission ${owner}.${name} must be an own data property`,
      );
    }
    ...
    throw new FixedTraceHybridAdmissionSnapshotError(
      `Hybrid admission ${owner}.${name} could not be snapshotted`,
      { cause: error },
    );
  }
}
...
function snapshotHybridAdmissionInput(input: unknown): HybridAdmissionSnapshot {
  ...
  try {
    ...
    if (descriptor && !("value" in descriptor)) {
      throw new FixedTraceHybridAdmissionSnapshotError(
        "Hybrid admission input.channelPrivacy must be an own data property",
      );
    }
    ...
    if (error instanceof FixedTraceHybridAdmissionSnapshotError) throw error;
    throw new FixedTraceHybridAdmissionSnapshotError(
      "Hybrid admission input.channelPrivacy could not be snapshotted",
      { cause: error },
    );
  }
  if (
    ...
    throw new FixedTraceHybridAdmissionSnapshotError(
      "Hybrid admission input has invalid request facts",
    );
...
function snapshotHybridPolicy(input: unknown): FixedTraceHybridPolicy {
  ...
  if (!Array.isArray(localTerminalActions)) {
    throw new FixedTraceHybridAdmissionSnapshotError(
      "Hybrid admission policy.localTerminalActions must be an array",
    );
  }
  const actionLength = ownDataProperty(
    localTerminalActions,
    "length",
    "policy.localTerminalActions",
  );
  if (
    typeof actionLength !== "number" ||
    !Number.isSafeInteger(actionLength) ||
    actionLength < 0 ||
    actionLength > 2
  ) {
    throw new FixedTraceHybridAdmissionSnapshotError(
      "Hybrid admission policy.localTerminalActions has invalid length",
    );
  }
  ...
  const snapshot = Object.freeze({
    ...
    localAdmissionSource: ownDataProperty(
      policy,
    ...
    requirePrivateChannelForChannelOutcome: ownDataProperty(
      policy,
      "requirePrivateChannelForChannelOutcome",
      "policy",
    ),
  ...
  try {
    ...
    throw new FixedTraceHybridAdmissionSnapshotError(
      "Hybrid admission policy is invalid",
      { cause: error },
    );
  }
  return snapshot;
...
type SafeTerminalForm = Readonly<{
  action: "ignore" | "react";
  emoji?: string;
}>;
...
const UNSAFE_OR_AMBIGUOUS_LANGUAGE =
  /\b(?:no|not|never|don't|do\s+not|delete|remove|ship|send|invoice|billing|payment|account|admin|tool|generate|create|update|change|cancel|refund|user)\b/i;
 ...
 */
function safeTerminalForm(
  snapshot: HybridAdmissionSnapshot,
): SafeTerminalForm | null {
  if (Buffer.byteLength(snapshot.message, "utf8") > 128) return null;
  if (
    CONTROL_OR_LINE_SEPARATOR.test(snapshot.message) ||
    UNSAFE_DELIMITER_OR_QUOTE.test(snapshot.message)
  )
    return null;
  ...
  if (
    SAFE_IGNORE_FORMS.has(normalized) ||
    (normalized.endsWith(".") && SAFE_IGNORE_FORMS.has(normalized.slice(0, -1)))
  ) {
...
export function decideFixedTraceHybridRoute(input: {
  ...
  if (snapshot.isAdmin)
    return {
      mode: "llm_router_fallback",
      reason: "admin_requires_router",
      plan: null,
    };
  if (snapshot.isThread)
    return {
      mode: "llm_router_fallback",
      reason: "thread_context_requires_router",
      plan: null,
    };
  if (snapshot.source === "channel" && snapshot.channelPrivacy !== "private") {
    return {
      mode: "llm_router_fallback",
      reason: "channel_privacy_not_captured",
      plan: null,
    };
  }
  ...
  if (!safeForm)
    return {
      mode: "llm_router_fallback",
      reason: "unsafe_or_ambiguous_message",
      plan: null,
    };
  let matcher: HybridQuickMatcher;
  try {
    const suppliedMatcher = Object.getOwnPropertyDescriptor(
      input,
      "quickMatcher",
    );
    if (suppliedMatcher && !("value" in suppliedMatcher))
      throw new Error("quickMatcher accessor");
    if (
      suppliedMatcher?.value !== undefined &&
      typeof suppliedMatcher.value !== "function"
    ) {
  ...
  if (!matchedAction)
    return {
      mode: "llm_router_fallback",
      reason: "no_production_quick_match",
      plan: null,
    };
  if (matchedAction === "respond") {
    return {
      mode: "llm_router_fallback",
      reason: "tool_or_mutation_capability_requires_router",
      plan: null,
    };
  }
  if (
    matchedAction !== safeForm.action ||
    !policy.localTerminalActions.includes(matchedAction)
  ) {
    return {
      mode: "llm_router_fallback",
      reason: "policy_disallows_terminal_action",
      plan: null,
    };
  }
...
export type FixedTraceToolDefinitionProvenance =
  "fixture_local" | "evaluator_owned_production_definitions_simulated_receipts";
...
export interface FixedTraceToolUniverseProvenance {
  ...
  intentNarrowing:
    | "llm_router"
    | "production_quick_match_or_llm_router"
    | "not_applied"
    | "fixture_oracle";
  bounded: boolean;
  deployable: boolean;
...
export function fixedTraceDirectRequestThreadFacts(
...
export function fixedTraceRequestThreadFactsProvenance(
  ...
  if (arm !== "direct_generation")
    return Object.freeze({ source: "not_applicable", traceFacts: [] });
  return Object.freeze({
    source: "fixture_case_request_not_authenticated",
    traceFacts: Object.freeze(
      traceSuite
        .map((trace) =>
          Object.freeze({
            traceId: trace.id,
            requestThreadFactsSha256: sha256(
              fixedTraceDirectRequestThreadFacts(trace),
            ),
...
export interface FixedTraceExecutionEnvelopeProvenance {
  source:
    | "fixture_expectation"
    | "request_thread_facts_not_captured"
    | "evaluator_owned_shared_request_thread_envelope"
    | "fixture_oracle";
  deployable: boolean;
}
  ...
  if (arm === "direct_generation") {
    return Object.freeze({
      ...
      definitionHandlerSha256:
        FIXED_TRACE_DIRECT_TOOL_UNIVERSE.definitionHandlerSha256,
    });
  }
  if (arm === "oracle_route_diagnostic") {
    return Object.freeze({
...
function freezeAdmission(
  ...
  return Object.freeze({
    ...
    universe: Object.freeze({
      ...universe,
      toolNames:
        universe.toolNames === null
          ? null
          : Object.freeze([...universe.toolNames]),
    }),
  });
}
 ...
 */
export function deriveFixedTraceDirectToolUniverse(
  trace: FixedTraceCase,
): FixedTraceDirectToolUniverse {
