  ...
  if (!proposalLifecycleWrite && !concreteTargetingWrite) {
    ...
    try {
      ...
      let staleDirective = session.complyExtensions.forcedUpstreamUnavailable?.tool === 'get_products'
        ...
        : undefined;
      if (ctx.principal?.startsWith('static:')) {
  ...
  return {
    ...
    creative: {
      ...
      ...(includeThreeOneFields(ctx) ? {
        ...
        preview: {
          routes: SUPPORTED_CANONICAL_BUILD_CAPABILITIES
            .filter(capability => CANONICAL_PREVIEW_CAPABILITY_IDS.has(capability.capabilityId))
            .map(capability => ({
