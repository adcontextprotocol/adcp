...
export function toPublicMembershipMutationError(error: unknown): PublicMembershipMutationError {
  ...
  if (code === '55P03' || code === '40P01' || code === '40001') {
    return { status: 409, body: { error: 'membership_state_conflict' } };
  }
  if (code === '57014') {
    return { status: 503, body: { error: 'authorization_unavailable' } };
  }
...
async function connectForMembershipMutation(): Promise<PoolClient> {
  try { return await getPool().connect(); }
  catch { throw new MembershipMutationError(503, 'Authorization state unavailable'); }
}
 ...
 * elevates a write role. Platform-admin affiliation, static/API keys and dev
 * sessions cannot substitute for a credential's organization membership.
 * The local active-membership mirror AND fresh WorkOS
...
export class OrganizationMembershipMutation {
  ...
  private async readSnapshot(): Promise<Snapshot> {
    ...
    try {
      rows = (await this.db.query<Snapshot>(
        `SELECT EXISTS (SELECT 1 FROM users WHERE workos_user_id = $1) AS credential_exists,
              ...
              COALESCE(ae.epoch, 0)::text AS epoch, iwu.xmin::text AS binding_version,
    ...
    if (!row.credential_exists) throw new MembershipMutationError(401, 'Invalid credential');
    if (!row.identity_id || !row.binding_version) mutationDenied('Identity binding changed');
  ...
  async checkProviderAuthority(): Promise<void> {
    ...
    await this.recheck();
    if (!Array.isArray(response?.data) || response.data.some(row => !row || typeof row !== 'object')) {
      throw new MembershipMutationError(503, 'Membership authority unavailable');
    }
  ...
  async readProvider<T extends object>(read: () => Promise<T>): Promise<T> {
    await this.checkProviderAuthority();
    let result: T;
    try {
      result = await read();
      // SDK reads used here return an ID-bearing resource or a data page.
      // A malformed/missing envelope is source uncertainty, never a denial
      // inferred from incomplete membership or owner inventory.
      if (!result || typeof result !== 'object'
        || ('data' in result
          ? !Array.isArray(result.data) || result.data.some(row => !row || typeof row !== 'object')
          : !('id' in result) || typeof result.id !== 'string')) {
        throw new Error('Invalid membership provider response');
      }
    }
  ...
  async checkTargets(): Promise<void> {
    try {
      ...
      throw new MembershipMutationError(503, 'Membership target state unavailable');
    }
  ...
  async audit(action: string, resourceType: string, resourceId: string, details: Record<string, unknown> = {}): Promise<void> {
    await this.write(
      ...
      [this.orgId, this.actorId, action, resourceType, resourceId, JSON.stringify({
        ...
        canonical_workos_user_id: this.snapshot.canonical_user_id,
        identity_id: this.snapshot.identity_id, operation_id: this.operationId,
        authority: 'exact_membership',
      })],
    );
  }
  ...
  static async run(req: Request, minimumRole: Role, operation: (tx: OrganizationMembershipMutation) => Promise<MutationReply>): Promise<MutationReply> {
    ...
    // Dev sessions are synthetic even when ALLOW_DEV_MODE_IN_PROD is enabled.
    if (dev) mutationDenied('Development sessions cannot manage organization membership');
    ...
    const db = await connectForMembershipMutation();
    ...
    try {
      ...
      tx = new OrganizationMembershipMutation(db, getAuthorizationEnforcementWorkos(), actorId, orgId, minimumRole);
      tx.expiresAt = verified?.expiresAt;
      tx.snapshot = await tx.readSnapshot();
      const authentication = req.user.authorizationSnapshot;
      if (!authentication) throw new MembershipMutationError(503, 'Authentication state unavailable');
      if (authentication.authenticatedUserId !== actorId
        || authentication.canonicalUserId !== tx.snapshot.canonical_user_id
        || authentication.identityId !== tx.snapshot.identity_id
        || authentication.bindingVersion !== tx.snapshot.binding_version
        || authentication.authorizationEpoch !== tx.snapshot.epoch
        || authentication.selectedOrganizationId !== orgId) {
        invalidateSessionsForUsers([actorId, req.user.id]);
        mutationDenied('Authentication state changed; retry the request');
      }
      if (req.user.identityId !== undefined && req.user.identityId !== tx.snapshot.identity_id) mutationDenied('Identity binding changed');
      if (req.user.id !== tx.snapshot.canonical_user_id) mutationDenied('Identity binding changed');
      ...
      for (const notify of tx.afterCommit) void Promise.resolve().then(notify).catch(error => logger.warn({ error, operationId: tx?.operationId, orgId, actorId }, 'Membership notification failed'));
      return result;
    } catch (error) {
      ...
      if (!commitAttempted) {
        for (const effect of [...effects].reverse()) {
          if (effect.outcome === 'succeeded' && effect.compensate) {
            ...
            catch (compensationError) { logger.error({ compensationError, operationId: tx?.operationId, orgId, actorId, action: effect.action }, 'Membership compensation not confirmed'); }
          }
        }
      }
...
export async function cancelJoinRequestForExactCredential(req: Request, requestId: string): Promise<boolean> {
  ...
  const actorId = getOrganizationAuthorizationUserId(req.user);
  const dev = isDevModeEnabled() && Object.values(DEV_USERS).some(user => user.id === actorId);
  if (dev) mutationDenied('Development sessions cannot cancel organization join requests');
  let verified: Awaited<ReturnType<typeof verifyWorkOSJWT>>;
  const header = req.headers.authorization;
  if (header !== undefined) {
    const match = /^Bearer[\t ]+([^\t ]+)[\t ]*$/i.exec(header);
  ...
  try {
    ...
    if (!snapshot.identity_id || !snapshot.binding_version) mutationDenied('Identity binding changed');
    const authentication = req.user.authorizationSnapshot;
    if (!authentication) throw new MembershipMutationError(503, 'authorization_unavailable');
    if (authentication.authenticatedUserId !== actorId
      || authentication.canonicalUserId !== snapshot.canonical_user_id
      || authentication.identityId !== snapshot.identity_id
      || authentication.bindingVersion !== snapshot.binding_version
      || authentication.authorizationEpoch !== snapshot.epoch
      || (authentication.selectedOrganizationId !== null
        && authentication.selectedOrganizationId !== orgId)
      || req.user.id !== snapshot.canonical_user_id
