import { safeFetchAxiosLike, isNetworkPolicyRefusal } from '../utils/url-security.js';
...
/** Bounded IAB evidence from the applicable host surfaces; redirects remain within the exact publisher origin. */
export async function fetchHostInventoryPartnerDomains(input: SupplyPathInput): Promise<NonNullable<SupplyPathInput['hostInventoryPartnerDomainsByFile']>> {
  ...
  const contents = await Promise.all(policy.files.map(async file => {
    try {
      ...
      if (isNetworkPolicyRefusal(error)) throw error;
      return null;
    }
  }));
