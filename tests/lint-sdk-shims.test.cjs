...
test('scanner finds the expected high-risk SDK reach-ins', () => {
  ...
  assert.ok(findings.some((f) => f.file === 'scripts/patch-sdk-rc15.mjs' && f.term.includes('task-map')));
  const patchEntry = ledger.find(entry => entry.id === 'sdk-storyboard-product-wire');
  assert.ok(patchEntry, 'missing storyboard product-wire patch ledger entry');
  for (const term of patchEntry.terms) {
    assert.ok(
      findings.some(finding => finding.file === 'scripts/patch-sdk-rc15.mjs' && finding.term.includes(term)),
      `SDK patch no longer references required artifact: ${term}`,
    );
  }
});
