...
describe("canonical inventory delivery reporting", () => {
  ...
  let validateInstallmentPropertyRow;
  ...
  before(async () => {
    ...
    [
      ...
      validateInstallmentPropertyRow,
      ...
      compile(extension.properties.by_installment_property.items),
  ...
  it("accepts property, collection, installment, and property-intersection rows", () => {
    ...
    const installment_ref = { collection_ref, installment_id: "episode_42" };
    ...
    assert.equal(validateInstallmentPropertyRow({ installment_ref, publisher_domain: "streaming.example", identifier, property_ref, impressions: 60, spend: 12 }), true, JSON.stringify(validateInstallmentPropertyRow.errors));
    assert.equal(validateInstallmentPropertyRow({ installment_ref, publisher_domain: "streaming.example", property_ref, impressions: 60, spend: 12 }), false);
    const contradictoryInstallmentProperty = { installment_ref, publisher_domain: "other.example", identifier, property_ref, impressions: 60, spend: 12 };
    assert.equal(validateInstallmentPropertyRow(contradictoryInstallmentProperty), true, "JSON Schema cannot compare sibling values");
    assert.equal(hasConsistentInventoryIdentity(contradictoryInstallmentProperty), false);
    assert.match(
      readSchema("/schemas/core/installment-property-delivery-metrics.json")["x-adcp-validation"]
        .verifier_constraints.property_domain_consistency,
      /MUST equal/
    );
  ...
  it("declares all inventory dimensions and validates their controls", () => {
    for (const dimension of ["placement", "property", "collection", "installment", "collection_property", "installment_property", "placement_property"]) {
  ...
  it("discloses suppression separately for every property-grain breakdown", () => {
    ...
    for (const dimension of ["property", "collection_property", "installment_property", "placement_property"]) {
