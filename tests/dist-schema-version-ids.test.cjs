...
test('generated latest schemas publish absolute AdCP $id and external $ref URIs', (t) => {
  const latestDir = path.join(DIST_SCHEMAS_DIR, 'latest');
  if (!fs.existsSync(latestDir)) {
    t.skip('dist/schemas/latest is generated by npm run build:schemas');
    return;
  }

  const failures = [];
  for (const file of walkJsonFiles(latestDir)) {
    ...
    const visit = (node) => {
      ...
      for (const [key, value] of Object.entries(node)) {
        if ((key === '$id' || key === '$ref') && typeof value === 'string') {
          const isLocalFragment = value.startsWith('#');
          const isCanonicalAdcpUri = /^https:\/\/adcontextprotocol\.org\/schemas\/(?:latest|\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?)\//.test(value);
          if (!isLocalFragment && !isCanonicalAdcpUri) {
            failures.push(`${path.relative(path.join(__dirname, '..'), file)}: ${key}=${value}`);
          }
          continue;
        }
        visit(value);
...
test('dist schema root discovery marks prereleases but points canonical aliases at stable releases', () => {
  ...
  assert.equal(discovery.$id, 'https://adcontextprotocol.org/schemas/index.json');
