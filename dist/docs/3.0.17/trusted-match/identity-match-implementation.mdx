...
Steps 1, 2, and 6 cross the wire and are normatively defined in the [TMP specification](/dist/docs/3.0.17/trusted-match/specification). Steps 3 and 5 cross the impression-tracker → cap-state-store boundary and are defined on this page. Step 4 is buyer-internal — the protocol does not constrain it.
...
- [TMP Specification](/dist/docs/3.0.17/trusted-match/specification) — wire spec, TMPX format, conformance invariants
- [Buyer Guide](/dist/docs/3.0.17/trusted-match/buyer-guide) — buyer agent integration, Context Match + Identity Match flows
- [Migration from AXE](/dist/docs/3.0.17/trusted-match/migration-from-axe) — for buyers transitioning from AXE-shaped pipelines, including the OpenRTB User.eids cross-walk
- [Privacy architecture](/dist/docs/3.0.17/trusted-match/privacy-architecture) — what each party learns
- [Router architecture](/dist/docs/3.0.17/trusted-match/router-architecture) — provider registration, fan-out, latency
