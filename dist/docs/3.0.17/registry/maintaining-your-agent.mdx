...
There is one path into the AAO registry: **an AAO member explicitly enrolls the agent on their member profile**. See [Registering an agent](/dist/docs/3.0.17/registry/registering-an-agent) for the full enrollment flow — dashboard or `PUT /api/me/member-profile`, end-to-end in under five minutes.
...
See [AAO Verified](/dist/docs/3.0.17/building/verification/aao-verified) for complete eligibility and attestation details.
...
2. Find the storyboard definition in the [Compliance Catalog](/dist/docs/3.0.17/building/verification/compliance-catalog).
3. Reproduce locally:
   ```bash
...
- [Registering an agent](/dist/docs/3.0.17/registry/registering-an-agent) — the enrollment path, fields, and programmatic registration.
- [Registry API overview](/dist/docs/3.0.17/registry) — `POST /api/registry/crawl-request` and the full endpoint catalog.
- [AAO Verified](/dist/docs/3.0.17/building/verification/aao-verified) — full lifecycle states, axis semantics, and badge embedding.
- [Compliance Catalog](/dist/docs/3.0.17/building/verification/compliance-catalog) — the storyboard index, per-specialism coverage, and how to run storyboards locally.
