...
The [Conformance Specification](/dist/docs/3.2.0-rc.1/building/verification/conformance#conformance-is-layered) defines three obligation layers: Universal, Protocol, and Specialism. This page explains what happens inside the Specialism layer: how a specialism manifest resolves to a set of graded scenarios, and how per-scenario capability gates can narrow or expand that set.

## From declaration to graded scenarios

When your agent declares a specialism in [`get_adcp_capabilities`](/dist/docs/3.2.0-rc.1/protocol/get_adcp_capabilities), the runner:
...
**Runner behavior:** all eight `requires_scenarios` run, including `proposal_finalize`. Priya's platform is graded on the full proposal lifecycle — brief with proposals, refine, finalize, and execute via [`create_media_buy`](/dist/docs/3.2.0-rc.1/media-buy/task-reference/create_media_buy).
...
In a raw `StoryboardResult`, `overall_passed` combines executed-step results with [storyboard invariants](/dist/docs/3.2.0-rc.1/building/verification/compliance-catalog#cross-resource-invariants). A storyboard-scoped assertion can set `overall_passed: false` even when `failed_count` — and the aggregate `summary.steps_failed` count — is zero. Inspect `assertions[]` for its `assertion_id`, `scope`, optional `step_id`, and `error` or structured `hint`. Step-scoped assertion failures also appear in that step's `validations[]` with `check: "assertion"`.
...
| Capabilities schema | `/schemas/3.2.0-rc.1/protocol/get-adcp-capabilities-response.json` | `static/schemas/source/protocol/get-adcp-capabilities-response.json` |

The full specialism-to-scenario index is at [Compliance Catalog](/dist/docs/3.2.0-rc.1/building/verification/compliance-catalog). The runner output contract defining every skip reason and verdict shape is at `static/compliance/source/universal/runner-output-contract.yaml`.

## Related

- [Conformance Specification](/dist/docs/3.2.0-rc.1/building/verification/conformance) — the three-layer obligation model and the normative storyboard index
- [Compliance Catalog](/dist/docs/3.2.0-rc.1/building/verification/compliance-catalog) — full taxonomy of protocols, specialisms, and universal storyboards
- [Validate Your Agent](/dist/docs/3.2.0-rc.1/building/verification/validate-your-agent) — running the suite locally with `@adcp/sdk`
