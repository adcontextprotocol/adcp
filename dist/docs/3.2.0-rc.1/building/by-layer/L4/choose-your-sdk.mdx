...
For the layered model behind this — what each layer contains and what an SDK at each layer should provide — see the [SDK stack reference](/dist/docs/3.2.0-rc.1/building/cross-cutting/sdk-stack).

## Coverage matrix

What "shipped" means at each layer is the [L0–L3 checklist](/dist/docs/3.2.0-rc.1/building/cross-cutting/sdk-stack#what-an-sdk-at-each-layer-should-provide).
...
3.2 support statement yet. See the [3.2 prerelease program](/dist/docs/3.2.0-rc.1/reference/3-2-beta).
</Warning>
...
For the status of every published protocol version, see [Versions & Compatibility](/dist/docs/3.2.0-rc.1/reference/versions). Migration and archived-version guidance lives under [Release notes & migration](/dist/docs/3.2.0-rc.1/reference/migration/index).
...
community-maintained ports are welcome — see the [Builders Working Group](/dist/docs/3.2.0-rc.1/community/working-group)
and the [Slack community](/dist/docs/3.2.0-rc.1/community/joining-slack).
...
- `@adcp/sdk/testing` — buyer-side storyboard runner (`runStoryboard`, `comply`, `testAgent`) and seller-side controller scaffold (`createComplyController` — see [Get Test-Ready](/dist/docs/3.2.0-rc.1/building/verification/get-test-ready))
...
```bash
npx @adcp/sdk@14.0.0-beta.31 --help
npx @adcp/sdk@14.0.0-beta.31 --save-auth my-agent https://sales.example.com/mcp
npx @adcp/sdk@14.0.0-beta.31 my-agent list_products '{"adcp_version":"3.2-rc.0","account":{"account_id":"account_123"}}'
# or against the built-in public test agent:
npx @adcp/sdk@14.0.0-beta.31 test-mcp list_products '{"adcp_version":"3.2-rc.0"}'
```
...
```bash
uvx --from adcp==8.0.0b13 adcp --help
uvx --from adcp==8.0.0b13 adcp --save-auth my-agent https://sales.example.com/mcp
uvx --from adcp==8.0.0b13 adcp my-agent get_products '{"adcp_version":"3.2-rc.0","idempotency_key":"550e8400-e29b-41d4-a716-446655442064","buying_mode":"brief","brief":"CTV campaign"}'
```

## What's next

- **[Build an agent](/dist/docs/3.2.0-rc.1/building/by-layer/L4/build-an-agent)** — server-side L4 path. Skill files + coding agent.
- **[Build a caller](/dist/docs/3.2.0-rc.1/building/by-layer/L4/build-a-caller)** — client-side L4 path. Install, call, handle responses, ingest reporting.
- **[Schemas](/dist/docs/3.2.0-rc.1/building/by-layer/L0/schemas)** — schema bundle, type generation, version pinning.
- **[Migrate from hand-rolled](/dist/docs/3.2.0-rc.1/building/by-layer/L4/migrate-from-hand-rolled)** — already running an AdCP agent built before the SDKs covered much? Swap one layer at a time.
