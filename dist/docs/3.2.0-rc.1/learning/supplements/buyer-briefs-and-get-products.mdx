...
training seller supports the exact `3.2-rc.0` wire bundle with
`@adcp/sdk@14.0.0-beta.31`. Verify those advertised capabilities before the lab;
...
- Read `get_adcp_capabilities.adcp.supported_versions` and `media_buy.lifecycle_tools`, pin `adcp_version: "3.2-rc.0"` for this lab, and validate the echoed served release before using targeting-aware discovery.
