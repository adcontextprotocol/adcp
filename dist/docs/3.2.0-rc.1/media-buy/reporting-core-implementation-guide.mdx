...
binding over its existing reporting transports. Core requires no destination,
manifest, external materialization, receipt, or push code. It does require the
...
4. **[`get_reporting_status`](/dist/docs/3.2.0-rc.1/media-buy/task-reference/get_reporting_status)
...
- **No Managed canonicalization or receipts.** `reconciliation_mode` is
  ...
  for you. Core still commits its fixed revision-binding JCS/SHA-256 digest;
  receipts and Managed external materialization machinery belong to the
...
`destination | manifest | receipt | readiness | webhook` (the Core binding
...
3. **Generate obligations.** When an account installs a delivery
   configuration through [`sync_accounts`](/dist/docs/3.2.0-rc.1/accounts/tasks/sync_accounts),
...
- **`managed_delivery: true`** adds offerings with a `method` —
  ...
  principal-layer destinations ([`sync_principal`](/dist/docs/3.2.0-rc.1/protocol/sync_principal)).
- **`reconciled_billing: true`** adds `receipt_task`
  ([`sync_reporting_receipts`](/dist/docs/3.2.0-rc.1/media-buy/task-reference/sync_reporting_receipts)),
