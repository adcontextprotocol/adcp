...
usable. It does not replace [`get_media_buy_delivery`](/dist/docs/3.2.0-rc.1/media-buy/task-reference/get_media_buy_delivery)
...
| `action_required` | An SLA or retry boundary was crossed, **or** readable revision and verified materialization evidence crossed the immediate consumer-reconciliation boundary with a missing or rejected current receipt; `issues[]` supplies the human action. Reliable Reporting defines no separate receipt SLA. |
...
[`sync_accounts`](/dist/docs/3.2.0-rc.1/accounts/tasks/sync_accounts) `notification_configs[]`. It is
...
[`sync_accounts`](/dist/docs/3.2.0-rc.1/accounts/tasks/sync_accounts) and
[`list_accounts`](/dist/docs/3.2.0-rc.1/accounts/tasks/list_accounts) return
