...
Before a seller can make seller-side checks, the buyer account must sync governance agent endpoints with the seller via [`sync_governance`](/dist/docs/3.2.0-rc.1/accounts/tasks/sync_governance). This is account setup, not per-buy negotiation.
...
The buyer-side orchestrator calls [`check_governance`](/dist/docs/3.2.0-rc.1/governance/campaign/tasks/check_governance) before every spend-commit action on a governed plan.
...
After a seller responds, the buyer-side orchestrator calls [`report_plan_outcome`](/dist/docs/3.2.0-rc.1/governance/campaign/tasks/report_plan_outcome). This lets the governance agent reconcile the approved action with the seller's actual response and update budget state from confirmed outcomes.
...
See the [`check_governance` task reference](/dist/docs/3.2.0-rc.1/governance/campaign/tasks/check_governance) for request and response fields, and the [Campaign Governance specification](/dist/docs/3.2.0-rc.1/governance/campaign/specification) for lifecycle and token validation details.
