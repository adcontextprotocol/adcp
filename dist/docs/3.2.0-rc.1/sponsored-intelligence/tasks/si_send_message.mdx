...
<Note>
**Experimental.** Sponsored Intelligence ([`si_get_offering`](/dist/docs/3.2.0-rc.1/sponsored-intelligence/tasks/si_get_offering), [`si_initiate_session`](/dist/docs/3.2.0-rc.1/sponsored-intelligence/tasks/si_initiate_session), `si_send_message`, [`si_terminate_session`](/dist/docs/3.2.0-rc.1/sponsored-intelligence/tasks/si_terminate_session)) is part of AdCP 3.0 as an experimental surface — it may change between 3.x releases with at least 6 weeks' notice. Sellers implementing any of these tasks MUST declare `sponsored_intelligence.core` in `experimental_features`. See [experimental status](/dist/docs/3.2.0-rc.1/reference/experimental-status) for the full contract.
</Note>
...
|-------|------|----------|-------------|
| `session_id` | string | Yes | Session ID from [`si_initiate_session`](/dist/docs/3.2.0-rc.1/sponsored-intelligence/tasks/si_initiate_session) |
...
If a prior [`si_initiate_session`](/dist/docs/3.2.0-rc.1/sponsored-intelligence/tasks/si_initiate_session) or `si_send_message` response included `sponsored_context`, the host decides whether it can honor the declared `context_use` and `disclosure_obligation` before presenting, comparing, or otherwise using that context. On a subsequent `si_send_message` request, the host MAY include `sponsored_context_receipt` alongside a normal `message` or `action_response` to make that decision visible to the brand/seller.
...
The host should call [`si_terminate_session`](/dist/docs/3.2.0-rc.1/sponsored-intelligence/tasks/si_terminate_session) after handling the handoff to properly close the session.
