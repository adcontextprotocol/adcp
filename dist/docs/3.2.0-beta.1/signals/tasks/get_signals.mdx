...
**Request Schema**: [`https://adcontextprotocol.org/schemas/3.2.0-beta.1/signals/get-signals-request.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.1/signals/get-signals-request.json)
**Response Schema**: [`https://adcontextprotocol.org/schemas/3.2.0-beta.1/signals/get-signals-response.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.1/signals/get-signals-response.json)
...
| `account` | [AccountRef](/dist/docs/3.2.0-beta.1/building/by-layer/L2/accounts-and-agents#account-references) | No | Account for this request. When provided, the signals agent returns per-account pricing options if configured. In wholesale mode, this is the rate-card scope; when omitted the agent returns default rate-card pricing or omits `pricing_options` entirely. |
...
should first call [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.1/protocol/get_adcp_capabilities)
...
- **signals**: Array of matching signals
  ...
  - **signal_agent_segment_id**: Opaque signal handle issued by this signal source. Use it verbatim for [`activate_signal`](/dist/docs/3.2.0-beta.1/signals/tasks/activate_signal); do not treat it as a globally portable signal ID. For package-level `signal_targeting_groups`, `signal_ref` is the buy-time identity and this handle is echoed only when the selected product option exposes it as a separate execution handle.
  ...
  - **pricing_options**: Array of pricing options for this signal when it has an incremental price. Pass the selected `pricing_option_id` in [`report_usage`](/dist/docs/3.2.0-beta.1/accounts/tasks/report_usage) or package-level `signal_targeting_groups` for billing verification. Omitted when pricing is unavailable to the caller, bundled into the destination product, or has no incremental cost.
...
```json
{
  "$schema": "/schemas/3.2.0-beta.1/signals/get-signals-response.json",
  "status": "completed",
...
Some signals offer multiple pricing models. The buyer selects one and passes its `pricing_option_id` in [`report_usage`](/dist/docs/3.2.0-beta.1/accounts/tasks/report_usage) for direct signal usage or in package-level `signal_targeting_groups` when the signal is selected on a media buy:
...
```json
{
  "$schema": "/schemas/3.2.0-beta.1/signals/get-signals-request.json",
...
Use `coverage_forecast` as the authoritative field for signal-first discovery: "how much inventory has this signal or its values?" Use product or proposal forecasts with `kind: "signal"` dimensions as the authoritative surface for product-specific planning: "how does this signal restrict this product's baseline availability?" Bucket-specific filtering is client-side for now; request filters such as `min_coverage_percentage` still use the legacy scalar.
  ...
  - **pricing_options** (array): Array of pricing options available for this signal when it has an incremental price. Select one and pass its `pricing_option_id` in [`report_usage`](/dist/docs/3.2.0-beta.1/accounts/tasks/report_usage) or package-level `signal_targeting_groups`.
...
### Discovery Errors
- [`REFERENCE_NOT_FOUND`](/dist/docs/3.2.0-beta.1/building/verification/compliance-catalog#error-code-reference-not-found): Referenced `signal_agent_segment_id` doesn't exist,
...
5. **Activation Required**: If `is_live` is false, use the [`activate_signal`](/dist/docs/3.2.0-beta.1/signals/tasks/activate_signal) task
6. **The message field** provides a quick summary of the most relevant findings

## Seller-offered signals on media products

A sales agent that owns or is authorized to apply targeting signals exposes buy-time product eligibility through [`get_products`](/dist/docs/3.2.0-beta.1/media-buy/task-reference/get_products). It can also expose a broader cross-product signal feed through `get_signals` when buyers need discovery or activation before package selection:
...
- [`create_media_buy`](/dist/docs/3.2.0-beta.1/media-buy/task-reference/create_media_buy) selects the signal for a package in `packages[].targeting_overlay.signal_targeting_groups`, carrying the selected signal `pricing_option_id`, `signal_ref`, and any separate seller execution handle when required.
...
Signals agents declare wholesale support in [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.1/protocol/get_adcp_capabilities):
...
This is the seller-side wholesale signals feed returned by `get_signals`. It is not a [`sync_catalogs`](/dist/docs/3.2.0-beta.1/media-buy/task-reference/sync_catalogs) feed; `sync_catalogs` manages buyer-provided campaign input feeds on a seller account.
