...
| `ads_txt_managerdomain` | Discovered via the publisher's `ads.txt` `MANAGERDOMAIN=` directive pointing to the manager file. | Weakest — the [`managerdomain` fallback safety rule](/dist/docs/3.2.0-beta.1/governance/property/adagents#safety-rules-for-this-fallback) is the only positive cross-check. |
...
- [adagents.json Tech Spec](/dist/docs/3.2.0-beta.1/governance/property/adagents) — the trust root.
- [Managed Network Deployment](/dist/docs/3.2.0-beta.1/governance/property/managed-networks) — the canonical multi-publisher pattern this endpoint indexes.
