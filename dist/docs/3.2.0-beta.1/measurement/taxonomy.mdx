...
- [`get_media_buy_delivery`](/dist/docs/3.2.0-beta.1/media-buy/task-reference/get_media_buy_delivery) — current delivery state
- [`provide_performance_feedback`](/dist/docs/3.2.0-beta.1/media-buy/task-reference/provide_performance_feedback) — buyer-side observed performance
- [Optimization & Reporting](/dist/docs/3.2.0-beta.1/media-buy/media-buys/optimization-reporting) — how reporting connects to optimization goals
...
- [`sync_event_sources`](/dist/docs/3.2.0-beta.1/media-buy/task-reference/sync_event_sources) — buyer pushes conversion event sources into seller platforms so the platform can optimize toward real outcomes
- [`log_event`](/dist/docs/3.2.0-beta.1/media-buy/task-reference/log_event) — buyer-attested event delivery
- [Conversion Tracking](/dist/docs/3.2.0-beta.1/media-buy/conversion-tracking/) — patterns that connect delivery to outcomes
- [Trusted Match](/dist/docs/3.2.0-beta.1/trusted-match/) — identity resolution that makes the join possible without leaking PII
...
- **Identity loss** (cookieless, IDFA deprecation, walled-garden signal collapse) shows up in _attribution_, not metrics. The seller still serves and counts impressions; the buyer's join to outcomes degrades. Fixes belong at the attribution boundary (clean rooms, [Trusted Match](/dist/docs/3.2.0-beta.1/trusted-match/)), not in delivery payloads.
- **AI-content provenance** is a _verification_ concern (was this ad what the brand approved?), not an attribution concern. It belongs alongside other verification capabilities — see [Provenance Verification](/dist/docs/3.2.0-beta.1/governance/creative/provenance-verification) — not bolted onto outcome reporting.
...
The buyer orchestrator is the hub. In the first experimental tier it exposes buyer-approved delivery to the provider through `get_media_buy_delivery`; the provider returns a compact [`provide_performance_feedback`](/dist/docs/3.2.0-beta.1/media-buy/task-reference/provide_performance_feedback) assertion to that gateway.
