...
> **Technical Reference**: This guide shows how to use the [`build_creative` task](/dist/docs/3.0.3/creative/task-reference/build_creative). For complete API specifications, see the task reference documentation.
...
Assets are provided via [brand identity](/dist/docs/3.0.3/brand-protocol/brand-json) - no separate asset library management needed.
...
The response includes a preview URL you can embed in an iframe. See [`preview_creative`](/dist/docs/3.0.3/creative/task-reference/preview_creative) for device variants, batch preview, and output format options.
...
See [brand.json reference](/dist/docs/3.0.3/brand-protocol/brand-json) for comprehensive examples.
...
Once you've built and previewed your creative, attach it to a media buy. Pass the manifest from `build_creative` as a creative in `create_media_buy` (see [`create_media_buy`](/dist/docs/3.0.3/media-buy/task-reference/create_media_buy) for the full request structure):
...
For sellers with [inline creative management](/dist/docs/3.0.3/creative/sales-agent-creative-capabilities), creatives travel with the media buy. For other workflows, use [`sync_creatives`](/dist/docs/3.0.3/creative/task-reference/sync_creatives) to upload creatives to the sales agent's library before referencing them in a media buy.
...
Preview doesn't require the media buy to be active — you can preview with just the brief manifest before calling `create_media_buy`. See [Previewing generative creative](/dist/docs/3.0.3/creative/task-reference/preview_creative#previewing-generative-creative) for the full mental model.

4. **Seller generates at serve time** — The seller uses the brief plus the buyer's [brand identity](/dist/docs/3.0.3/brand-protocol/brand-json) to generate page-tailored creatives per impression or per page context. The brief and brand identity act as guardrails — the agent generates within these constraints. No buyer action is required for ongoing generation.
...
See [Creative capabilities on sales agents](/dist/docs/3.0.3/creative/sales-agent-creative-capabilities) for the full pattern.
...
- **Brand identity** — colors, logos, voice guidelines, approved assets via [brand.json](/dist/docs/3.0.3/brand-protocol/brand-json)
...
These constraints are expressed in natural language in the brief. The protocol does not enforce them at runtime — verify compliance through post-flight variant review via [`get_creative_delivery`](/dist/docs/3.0.3/creative/task-reference/get_creative_delivery).

See [Previewing generative creative](/dist/docs/3.0.3/creative/task-reference/preview_creative#conversational-and-interactive-formats) for the full mental model on previewing stateful formats.

## Next steps

- **Multi-format generation**: Generate creatives for multiple sizes in one call with `target_format_ids` — see [Multi-format generation](/dist/docs/3.0.3/creative/task-reference/build_creative#multi-format-generation)
- **Browse examples**: See [Task reference](/dist/docs/3.0.3/creative/task-reference/build_creative) for `build_creative` examples
- **Sales agent creative flow**: See [Creative capabilities on sales agents](/dist/docs/3.0.3/creative/sales-agent-creative-capabilities) for seller-side generation
- **Delivery reporting**: See [get_creative_delivery](/dist/docs/3.0.3/creative/task-reference/get_creative_delivery) for variant-level analytics
...
4. Iterate using [refinement](/dist/docs/3.0.3/creative/task-reference/build_creative#iterative-refinement) — pass back the manifest with a new message, or update the brief asset

### Asset Management
Assets are provided via [brand identity](/dist/docs/3.0.3/brand-protocol/brand-json):
