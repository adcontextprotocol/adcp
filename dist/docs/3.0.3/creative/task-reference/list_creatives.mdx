...
**Schema**: [`creative/list-creatives-request.json`](https://adcontextprotocol.org/schemas/3.0.3/creative/list-creatives-request.json)
...
| `include_snapshot` | boolean | No | Include a lightweight delivery snapshot — lifetime impressions and last-served date (default: false). For detailed analytics, use [`get_creative_delivery`](/dist/docs/3.0.3/creative/task-reference/get_creative_delivery). |
...
| `account` | [AccountRef](/dist/docs/3.0.3/building/integration/accounts-and-agents#account-references) | No | Account reference for pricing. When provided with `include_pricing`, the agent returns `pricing_options` from this account's rate card on each creative. |
...
{/* Using latest because these schemas are not yet released in any version.
     Update to correct version alias after the next release. */}
**Schema**: [`creative/list-creatives-response.json`](https://adcontextprotocol.org/schemas/3.0.3/creative/list-creatives-response.json)
...
| `pricing_options` | [VendorPricingOption](/dist/docs/3.0.3/creative/specification#pricing)[] | Pricing options for this creative (when `include_pricing=true` and `account` provided). Vendors may offer multiple options (volume tiers, context-specific rates, different models per product line). Same pattern as `get_signals` and `list_content_standards`. |
...
- [`get_creative_delivery`](/dist/docs/3.0.3/creative/task-reference/get_creative_delivery) - Detailed performance analytics with date ranges, variant breakdowns, and full delivery metrics
- [`build_creative`](/dist/docs/3.0.3/creative/task-reference/build_creative) - Build manifests from library creatives or generate from scratch
- [`sync_creatives`](/dist/docs/3.0.3/creative/task-reference/sync_creatives) - Upload and manage creative assets on any agent hosting a creative library
- [`list_creative_formats`](/dist/docs/3.0.3/creative/task-reference/list_creative_formats) - Discover supported creative formats
- [`preview_creative`](/dist/docs/3.0.3/creative/task-reference/preview_creative) - Generate previews of creative manifests
