...
<Note>
**Experimental.** Sponsored Intelligence ([`si_get_offering`](/dist/docs/3.2.0-beta.5/sponsored-intelligence/tasks/si_get_offering), `si_initiate_session`, [`si_send_message`](/dist/docs/3.2.0-beta.5/sponsored-intelligence/tasks/si_send_message), [`si_terminate_session`](/dist/docs/3.2.0-beta.5/sponsored-intelligence/tasks/si_terminate_session)) is part of AdCP 3.0 as an experimental surface — it may change between 3.x releases with at least 6 weeks' notice. Sellers implementing any of these tasks MUST declare `sponsored_intelligence.core` in `experimental_features`. See [experimental status](/dist/docs/3.2.0-beta.5/reference/experimental-status) for the full contract.
</Note>
...
| `offering_token` | string | No | Token from [`si_get_offering`](/dist/docs/3.2.0-beta.5/sponsored-intelligence/tasks/si_get_offering) for correlation |
| `sponsored_context_receipt` | object | No | Host receipt for sponsored context accepted before session initiation |
| `context` | object | No | Opaque correlation data echoed unchanged in the response (see [application context](/dist/docs/3.2.0-beta.5/building/by-layer/L2/context-sessions#application-context-context)) |

### Offering Token

If a host performed a [`si_get_offering`](/dist/docs/3.2.0-beta.5/sponsored-intelligence/tasks/si_get_offering) lookup before initiating, include the token for **session continuity**:
...
```json
{
  "$schema": "/schemas/3.2.0-beta.5/sponsored-intelligence/si-initiate-session-request.json",
...
```json
{
  "$schema": "/schemas/3.2.0-beta.5/sponsored-intelligence/si-initiate-session-response.json",
  "status": "completed",
