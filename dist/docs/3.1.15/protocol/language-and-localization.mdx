...
**`targeting.language` is a known vocabulary divergence.** The language filter
in [`get_products`](/dist/docs/3.1.15/media-buy/task-reference/get_products) and related tasks uses ISO 639-1 two-letter codes only
(`^[a-z]{2}$`), not the shared BCP 47 tag profile. It is the one legacy field
that can appear in the same request as creative locale eligibility: a buyer
targeting `language: ["fr"]` against a creative with
`accepted_language_ranges: ["fr-CA"]` carries two distinct vocabularies in one
request — `fr` is valid for targeting and is a valid Basic Filtering range, but
the creative policy is constrained to `fr-CA` and its more-specific descendants.
This is a documented divergence; neither side is wrong. The `^[a-z]{2}$`
pattern also cannot express ISO 639-3-only languages, which is an additional
pre-existing limitation on that field.
