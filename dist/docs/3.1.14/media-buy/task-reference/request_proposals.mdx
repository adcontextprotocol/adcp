...
**Request schema:** [`/schemas/3.1.14/media-buy/request-proposals-request.json`](https://adcontextprotocol.org/schemas/3.1.14/media-buy/request-proposals-request.json)
...
Every returned purchase references the exact `product_id` and `pricing_option_id` whose pricing and forecast the seller used. The returned `proposal_id` is the only proposal linkage needed by [`refine_proposals`](/dist/docs/3.1.14/media-buy/task-reference/refine_proposals) and [`decline_proposals`](/dist/docs/3.1.14/media-buy/task-reference/decline_proposals). Each ID identifies one immutable commercial snapshot; refinement and finalization mint new IDs instead of adding a second version field. A draft is indicative and does not reserve inventory. Finalize it through `refine_proposals` before passing the resulting committed proposal to [`accept_proposal`](/dist/docs/3.1.14/media-buy/task-reference/accept_proposal). The deprecated [`create_media_buy`](/dist/docs/3.1.14/media-buy/task-reference/create_media_buy) proposal mode remains the 3.x compatibility adapter.
