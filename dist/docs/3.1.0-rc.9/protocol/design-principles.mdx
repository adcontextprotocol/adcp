...
→ Use the [Capabilities explorer](/dist/docs/3.1.0-rc.9/protocol/capabilities-explorer) before proposing a new flag. It walks the actual schema tree.
...
Targeting is what the buyer wants. It's not a downstream filter on a generic catalog — it's what the seller curates *against*. Buyers send a brief (or `wholesale + filters`) to [`get_products`](/dist/docs/3.1.0-rc.9/media-buy/task-reference/get_products); the seller returns products already shaped for that intent, with pricing already scoped to the buyer's account via the `account` parameter. Iteration happens through `buying_mode: "refine"` with a typed change array — the round-trip is folded into discovery, not bolted on afterward.
...
→ See [Targeting](/dist/docs/3.1.0-rc.9/media-buy/advanced-topics/targeting) for the brief-first model.
...
→ The [Capabilities explorer](/dist/docs/3.1.0-rc.9/protocol/capabilities-explorer) renders the existing tree.

---

### 5. Trust is bilateral and `/.well-known`-rooted

Trust in AdCP is bilateral and verifiable, not blessed by a registry. Operators declare organization identity, agents, property relationships, and signing-key discovery via [`brand.json`](/dist/docs/3.1.0-rc.9/brand-protocol/brand-json) at `/.well-known/brand.json`; publishers declare which agents are authorized for which properties via [`adagents.json`](/dist/docs/3.1.0-rc.9/governance/property/adagents). Either party can resolve and verify the other's declarations. The [Registry](/dist/docs/3.1.0-rc.9/registry) helps with discovery and resolution; it does not gate participation. Every discovery hop is an HTTP GET against a deterministic path — the model ads.txt and sellers.json got right, deliberately reused.
...
→ See [Architecture — Privacy posture across domains](/dist/docs/3.1.0-rc.9/protocol/architecture#privacy-posture-across-domains) for the per-domain mechanism table.
...
→ See [Trust & Security — Governance](/dist/docs/3.1.0-rc.9/trust#governance) for what AdCP provides versus what it explicitly does not, and [Governance — Embedded human judgment](/dist/docs/3.1.0-rc.9/governance/embedded-human-judgment) for how HITL surfaces in the protocol.
