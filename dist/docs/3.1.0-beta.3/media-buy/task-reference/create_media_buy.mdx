...
**Request Schema**: [`/schemas/3.1.0-beta.3/media-buy/create-media-buy-request.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.3/media-buy/create-media-buy-request.json)
**Response Schema**: [`/schemas/3.1.0-beta.3/media-buy/create-media-buy-response.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.3/media-buy/create-media-buy-response.json)
...
|-----------|------|----------|-------------|
| `account` | [account-ref](/dist/docs/3.1.0-beta.3/building/integration/accounts-and-agents#account-references) | Yes | Account reference. Pass `{ "account_id": "..." }` or `{ "brand": {...}, "operator": "..." }` if the seller supports implicit resolution. Required for billing and policy evaluation. |
...
| `brand` | BrandRef | Yes | Brand reference — resolved to full identity at execution time. See [brand.json](/dist/docs/3.1.0-beta.3/brand-protocol/brand-json) |
| `start_time` | string | Yes | `"asap"` or ISO 8601 date-time |
| `end_time` | string | Yes | ISO 8601 date-time (UTC unless timezone specified) |
| `invoice_recipient` | [BusinessEntity](/dist/docs/3.1.0-beta.3/building/integration/accounts-and-agents#billing-entity-and-invoice-recipient) | No | Override the account's default billing entity for this buy. The seller MUST validate the recipient is authorized and include it in `check_governance` when governance agents are configured. |
...
| `format_ids` | FormatID[] | No | v1 path. Format IDs that will be used — must be supported by product. When omitted (and `capability_ids` is also omitted), defaults to all formats supported by the product. V2-native buyer SDKs targeting a heterogeneous seller population SHOULD dual-emit this alongside `capability_ids` so v1-only sellers receive an explicit format set rather than defaulting to all-formats. |
| `capability_ids` | string[] | No | v2 path. References to entries in the product's `format_options[i].capability_id` (see [canonical formats](/dist/docs/3.1.0-beta.3/creative/canonical-formats)). Lets V2-mental-model buyers author a `create_media_buy` without translating back through `v1_format_ref[]`. When both `capability_ids` and `format_ids` are sent, `capability_ids` wins (the seller routes by it; `format_ids` becomes a v1-compat hint the resolving seller ignores). Sellers MUST reject with `UNSUPPORTED_FEATURE` (field path `packages[i].capability_ids[j]`) when an entry doesn't match a `format_options[]` entry, when the product is v1-only (carries `format_ids` but no `format_options[]`), or when the product's `format_options[]` entries don't publish `capability_id` values (the seller SHOULD set `error.details.reason: "capability_ids_not_published"` so buyers can fall back to `format_ids[]`). |
...
| `measurement_terms` | [MeasurementTerms](/dist/docs/3.1.0-beta.3/media-buy/advanced-topics/pricing-models#measurement-terms-and-performance-standards) | No | Buyer's proposed billing measurement and makegood terms. Overrides product defaults. Seller accepts (echoed on confirmed package), rejects with `TERMS_REJECTED`, or adjusts. When omitted, product's `measurement_terms` apply. |
| `performance_standards` | [PerformanceStandard[]](/dist/docs/3.1.0-beta.3/media-buy/advanced-topics/pricing-models#measurement-terms-and-performance-standards) | No | Buyer's proposed performance standards (viewability, IVT, completion rate, brand safety, attention score). Overrides product defaults. Seller accepts, rejects with `TERMS_REJECTED`, or adjusts. When omitted, product's `performance_standards` apply. |
...
Include the `catalogs` field in a package to make it catalog-driven. Each catalog should have a distinct type (e.g., one product catalog, one store catalog). The referenced catalogs must already be synced via [`sync_catalogs`](/dist/docs/3.1.0-beta.3/media-buy/task-reference/sync_catalogs).
...
The platform distributes budget across catalog items based on performance. For per-item reporting, use [`get_media_buy_delivery`](/dist/docs/3.1.0-beta.3/media-buy/task-reference/get_media_buy_delivery) which returns `by_catalog_item` breakdowns. Creative variants for catalog-driven packages represent individual catalog items rendered as ads.
...
See [Proposals](/dist/docs/3.1.0-beta.3/media-buy/product-discovery/media-products#proposals) for the complete workflow.
...
Each package SHOULD specify the formats it will use, via either `format_ids[]` (v1 path — structured `{agent_url, id}` references) or `capability_ids[]` (v2 path — references into the product's `format_options[]`). Omitting both defaults to all formats supported by the product. Specifying lets the system:
...
Use `capability_ids[]` when the buyer's codebase is V2-native (reads `Product.format_options[]`); use `format_ids[]` when integrating with v1-only sellers or libraries. Dual-emission is allowed and recommended for buyer SDKs targeting a mixed seller population — see the `format_ids` row above.

V2-path example (buyer authoring against `Product.format_options[i].capability_id` directly):
  ...
  "packages": [
    {
      "product_id": "prod_d979b543",
      "pricing_option_id": "cpm_usd_auction",
      "capability_ids": ["nytimes_homepage_image"],
      "budget": 2500,
      "bid_price": 5.00
    }
  ]
...
Full brand identity data (colors, fonts, product catalog) is resolved from brand.json at execution time. See [brand.json](/dist/docs/3.1.0-beta.3/brand-protocol/brand-json).
...
Packages can use different currencies when sellers support it. See [Pricing Models](/dist/docs/3.1.0-beta.3/media-buy/advanced-topics/pricing-models).
...
- Frequency capping
- AXE segment inclusion/exclusion (legacy — new integrations use [TMP](/dist/docs/3.1.0-beta.3/trusted-match))

See [Targeting](/dist/docs/3.1.0-beta.3/media-buy/advanced-topics/targeting) for details.
...
4. create_media_buy → Specify formats via format_ids[] (v1) or
                       capability_ids[] (v2); omit for all-formats default
...
Invalid format example (v1 path):
...
Invalid capability example (v2 path):

```json
{
  "errors": [{
    "code": "UNSUPPORTED_FEATURE",
    "message": "Product 'ctv_sports_premium' has no format_options[] entry with capability_id 'audio_standard'",
    "field": "packages[0].capability_ids[0]",
    "supported_capability_ids": ["ctv_video_30s_premium", "ctv_video_15s_premium"]
  }]
}
```
...
**Note:** For the complete status list see [Task Lifecycle](/dist/docs/3.1.0-beta.3/building/implementation/task-lifecycle).
...
For complete async handling patterns, see [Async Operations](/dist/docs/3.1.0-beta.3/building/implementation/async-operations).
...
- Impression-time targeting (audience, frequency, suitability) is handled by [TMP](/dist/docs/3.1.0-beta.3/trusted-match)
...
<Note>
Content standards are created by calling [`create_content_standards`](/dist/docs/3.1.0-beta.3/governance/content-standards/tasks/create_content_standards) on a verification agent (e.g., IAS, DoubleVerify). Standards MUST be [calibrated](/dist/docs/3.1.0-beta.3/governance/content-standards/tasks/calibrate_content) with each seller before use in production to ensure the seller's local evaluation model aligns with the verification agent's interpretation. See the [Content Standards overview](/dist/docs/3.1.0-beta.3/governance/content-standards/index) for the full setup workflow: create → calibrate → activate → validate.
</Note>
...
1. **Upload Creatives**: Use [`sync_creatives`](/dist/docs/3.1.0-beta.3/creative/task-reference/sync_creatives) before deadline
2. **Monitor Status**: Use [`get_media_buy_delivery`](/dist/docs/3.1.0-beta.3/media-buy/task-reference/get_media_buy_delivery)
3. **Optimize**: Use [`provide_performance_feedback`](/dist/docs/3.1.0-beta.3/media-buy/task-reference/provide_performance_feedback)
4. **Update**: Use [`update_media_buy`](/dist/docs/3.1.0-beta.3/media-buy/task-reference/update_media_buy) to modify campaign
...
Full migration: [Migration › `media_buy_status`](/dist/docs/3.1.0-beta.3/reference/migration/media-buy-status).

## Learn More

- [Media Buy Lifecycle](/dist/docs/3.1.0-beta.3/media-buy/media-buys/) - Complete campaign workflow
- [get_products](/dist/docs/3.1.0-beta.3/media-buy/task-reference/get_products) - Discover inventory
- [Targeting](/dist/docs/3.1.0-beta.3/media-buy/advanced-topics/targeting) - Targeting strategies
- [Pricing Models](/dist/docs/3.1.0-beta.3/media-buy/advanced-topics/pricing-models) - Currency and pricing
