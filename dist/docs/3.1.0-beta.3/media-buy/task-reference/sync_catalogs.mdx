...
This task sits between format discovery and creative submission in the [account state setup sequence](/dist/docs/3.1.0-beta.3/building/integration/account-state):
...
|-----------|------|----------|-------------|
| `account` | [account-ref](/dist/docs/3.1.0-beta.3/building/integration/accounts-and-agents#account-references) | Conditional | Account reference. Pass `{ "account_id": "..." }` or `{ "brand": {...}, "operator": "..." }` if the seller supports implicit resolution. Required when the agent has multiple accounts. |
...
- [Account state](/dist/docs/3.1.0-beta.3/building/integration/account-state) — How catalogs fit into the account setup sequence
