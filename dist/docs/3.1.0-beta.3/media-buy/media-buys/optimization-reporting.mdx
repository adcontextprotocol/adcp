...
- See [Security](/dist/docs/3.1.0-beta.3/building/by-layer/L1/security) for implementation details
...
Webhook delivery status is tracked through **AdCP's global task management system** (see [Task Lifecycle](/dist/docs/3.1.0-beta.3/building/by-layer/L3/task-lifecycle)).
...
Offline files sit at rest for `file_retention_days`, so a misconfigured IAM policy leaks historical reporting across tenants. The [general security controls](/dist/docs/3.1.0-beta.3/building/by-layer/L1/security) apply; the offline-specific requirements:
...
See [Webhooks](/dist/docs/3.1.0-beta.3/building/by-layer/L3/webhooks) for detailed implementation guidance.
