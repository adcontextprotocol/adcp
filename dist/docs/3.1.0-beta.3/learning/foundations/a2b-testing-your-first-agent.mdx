...
Poll every 2–5 seconds. When `task.status` is `completed`, the `result` field contains the full `create_media_buy` response with `media_buy_id`. See the [Task lifecycle](/dist/docs/3.1.0-beta.3/building/by-layer/L3/task-lifecycle) doc for all MCP task status values.
