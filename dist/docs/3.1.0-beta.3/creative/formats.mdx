...
description: "Creative formats in AdCP define asset requirements, technical constraints, and rendering behavior that shape how ads are built and delivered."
"og:title": "AdCP — Creative Formats"
---

> **Canonical-formats preview (3.1)**: starting in 3.1, formats can be declared inline on products via `format_options` (an array of `ProductFormatDeclaration`s narrowing one of 12 canonical formats). This page describes the v1 format-registry model, which remains a first-class path through 4.x. For the canonical-formats model, see [canonical-formats](/dist/docs/3.1.0-beta.3/creative/canonical-formats) and the [migration guide](/dist/docs/3.1.0-beta.3/creative/canonical-formats-migration).
...
For an overview of how formats, manifests, and creative agents work together, see the [Creative Protocol Overview](/dist/docs/3.1.0-beta.3/creative).
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.1.0-beta.3/creative/list-creative-formats-response.json",
  "status": "completed",
  "formats": [
    {
      "format_id": {
...
**For sales agents implementing format support:** See [Implementing Standard Format Support](/dist/docs/3.1.0-beta.3/media-buy/capability-discovery/implementing-standard-formats).
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.1.0-beta.3/core/format.json",
  "format_id": {
    "agent_url": "https://creative.adcontextprotocol.org",
    "id": "video_30s_hosted"
  },
...
<Note>
  For a normative contrast between `format_id` (structured reference object) and `format` (full definition object) — including the two most common validation errors — see [Format References](/dist/docs/3.1.0-beta.3/protocol/format-references).
</Note>
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.1.0-beta.3/core/format-id.json",
  "agent_url": "https://creative.adcontextprotocol.org",
  "id": "display_300x250"
}
```
...
- **accessibility**: Optional WCAG conformance declaration — see [Accessibility](/dist/docs/3.1.0-beta.3/creative/accessibility)
...
A buyer agent that requires DoubleVerify viewability or IAS brand safety measurement should filter formats by tracker support. If no tracker slot exists, those vendors cannot instrument the creative — the buyer must rely on the seller's declared measurement vendor instead.
...
See [Display Ads - HTML Display Formats](/dist/docs/3.1.0-beta.3/creative/channels/display#html-display-formats-with-execution-context) for complete examples.
...
See [Video Channel Guide](/dist/docs/3.1.0-beta.3/creative/channels/video) for complete specifications.
...
See [Display Channel Guide](/dist/docs/3.1.0-beta.3/creative/channels/display) for complete specifications.
...
See [Audio Channel Guide](/dist/docs/3.1.0-beta.3/creative/channels/audio) for complete specifications.
...
See [DOOH Channel Guide](/dist/docs/3.1.0-beta.3/creative/channels/dooh) for complete specifications.
...
See [Carousel Channel Guide](/dist/docs/3.1.0-beta.3/creative/channels/carousels) for complete specifications.

## Multi-Asset & Frame-Based Formats

Some formats like carousels, slideshows, and stories use repeatable asset groups where each frame contains a collection of assets. See the [Carousel & Multi-Asset Formats guide](/dist/docs/3.1.0-beta.3/creative/channels/carousels) for complete documentation on frame-based format patterns.
...
- [Creative Protocol Overview](/dist/docs/3.1.0-beta.3/creative) - How formats, manifests, and agents work together
- [Creative Manifests](/dist/docs/3.1.0-beta.3/creative/creative-manifests) - Pairing assets with formats
- [Asset Types](/dist/docs/3.1.0-beta.3/creative/asset-types) - Understanding asset specifications
- [Channel Guides](/dist/docs/3.1.0-beta.3/creative/channels/video) - Detailed format documentation by media type
- [Implementing Standard Format Support](/dist/docs/3.1.0-beta.3/media-buy/capability-discovery/implementing-standard-formats) - For sales agents
- [list_creative_formats Task](/dist/docs/3.1.0-beta.3/creative/task-reference/list_creative_formats) - API reference for format discovery
