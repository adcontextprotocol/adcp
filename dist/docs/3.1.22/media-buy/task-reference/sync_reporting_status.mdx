...
**Request schema:** [`sync-reporting-status-request.json`](https://adcontextprotocol.org/schemas/3.1.22/media-buy/sync-reporting-status-request.json)

{/* Using latest because sync_reporting_status is not released in an immutable version yet.
    Update to the next 3.2 release-candidate URL after publication. */}
**Response schema:** [`sync-reporting-status-response.json`](https://adcontextprotocol.org/schemas/3.1.22/media-buy/sync-reporting-status-response.json)
...
5. exposes the caller's history through
   [`get_reporting_status`](/dist/docs/3.1.22/media-buy/task-reference/get_reporting_status);
   and
