...
**Request Schema**: [`/schemas/3.1.22/media-buy/get-products-request.json`](https://adcontextprotocol.org/schemas/3.1.22/media-buy/get-products-request.json)
**Response Schema**: [`/schemas/3.1.22/media-buy/get-products-response.json`](https://adcontextprotocol.org/schemas/3.1.22/media-buy/get-products-response.json)
...
| `catalog` | [Catalog](/dist/docs/3.1.22/creative/catalogs) | No | Catalog of items the buyer wants to promote. The seller matches catalog items against its inventory and returns products where matches exist. Requires `brand`. See [Catalog discovery](#catalog-discovery) below. |
...
| `property_list` | PropertyListRef | No | [AdCP 3.0] Reference to a property list for filtering. See [Property Lists](/dist/docs/3.1.22/governance/property/tasks/property_lists) |
...
The seller must declare `features.property_list_filtering: true` in [`get_adcp_capabilities`](/dist/docs/3.1.22/protocol/get_adcp_capabilities) to support this filter. In AdCP 3.1 this legacy flag also declares support for `targeting_overlay.property_list` on media-buy create and update requests. See the [Property Governance overview](/dist/docs/3.1.22/governance/property/index) for the full workflow.
</Note>
...
| `channels` | string[] | Filter by advertising channels (e.g., `["display", "ctv", "social", "streaming_audio"]`). See [Media Channel Taxonomy](/dist/docs/3.1.22/reference/media-channel-taxonomy) |
...
```json
{
  "$schema": "/schemas/3.1.22/media-buy/get-products-request.json",
  "buying_mode": "wholesale",
  "filters": {
    "signal_targeting": [
      {
...
The `catalog` field uses the same [Catalog](/dist/docs/3.1.22/creative/catalogs) object used throughout AdCP. You can reference a synced catalog by `catalog_id`, provide inline items, or use selectors to filter:
...
| `measurement_readiness` | [MeasurementReadiness](/dist/docs/3.1.22/media-buy/conversion-tracking/#measurement-readiness) | (Optional) Whether the buyer's event setup is sufficient for this product's optimization. Only present when the seller can evaluate the buyer's account context. |
| `measurement_terms` | [MeasurementTerms](/dist/docs/3.1.22/media-buy/advanced-topics/pricing-models#measurement-terms-and-performance-standards) | (Optional) Seller's default billing measurement and makegood terms. Buyers may propose different terms at `create_media_buy`. |
| `performance_standards` | [PerformanceStandard[]](/dist/docs/3.1.22/media-buy/advanced-topics/pricing-models#measurement-terms-and-performance-standards) | (Optional) Seller's default performance standards (viewability, IVT, completion rate, brand safety, attention score). Buyers may propose different standards at `create_media_buy`. |
| `cancellation_policy` | [CancellationPolicy](/dist/docs/3.1.22/media-buy/advanced-topics/pricing-models#cancellation-policy) | (Optional) Cancellation notice period and penalties for guaranteed products. Buyers accept these terms by creating a media buy against the product. |
...
Publishers may return proposals alongside products - structured media plans with budget allocations. See [Proposals](/dist/docs/3.1.22/media-buy/product-discovery/media-products#proposals) for details.
...
| `forecast` | DeliveryForecast | Aggregate delivery forecast for the proposal. Contains forecast points with metric ranges. See [Delivery Forecasts](/dist/docs/3.1.22/media-buy/product-discovery/media-products#delivery-forecasts) |
...
```json
{
  "$schema": "/schemas/3.1.22/media-buy/get-products-response.json",
  "status": "completed",
...
**See schema for complete field list**: [`get-products-response.json`](https://adcontextprotocol.org/schemas/3.1.22/media-buy/get-products-response.json)
...
See [Property Targeting](/dist/docs/3.1.22/media-buy/product-discovery/media-products#property-targeting) for more details and [Property Governance](/dist/docs/3.1.22/governance/property/specification) for more on property lists.
...
See the [Refinement guide](/dist/docs/3.1.22/media-buy/product-discovery/refinement) for the full walkthrough: scope types, action semantics, seller responses, and common patterns. The parameter shape is defined in the [Refine array](#refine-array) section above.
...
See [Authentication Guide](/dist/docs/3.1.22/building/by-layer/L2/authentication) for details.
...
**Note:** For the complete status list see [Task Lifecycle](/dist/docs/3.1.22/building/by-layer/L3/task-lifecycle).
...
2. **Create Media Buy**: Use [`create_media_buy`](/dist/docs/3.1.22/media-buy/task-reference/create_media_buy) to execute campaign
3. **Prepare Creatives**: Use [`list_creative_formats`](/dist/docs/3.1.22/creative/task-reference/list_creative_formats) to see format requirements
4. **Supply Assets**: Use [`sync_creatives`](/dist/docs/3.1.22/creative/task-reference/sync_creatives) for library-backed sellers, or inline `packages[].creatives` for inline-only sellers

## Learn More

- [Product Discovery Guide](/dist/docs/3.1.22/media-buy/product-discovery/) - Understanding briefs and products
- [Pricing Models](/dist/docs/3.1.22/media-buy/advanced-topics/pricing-models) - CPM, CPCV, CPP explained
- [Brief Expectations](/dist/docs/3.1.22/media-buy/product-discovery/brief-expectations) - How to write effective briefs
- [Media Products](/dist/docs/3.1.22/media-buy/product-discovery/media-products) - Product structure and fields
