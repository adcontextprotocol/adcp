...
4. **[`get_reporting_status`](/dist/docs/3.1.22/media-buy/task-reference/get_reporting_status)
...
3. **Generate obligations.** When an account installs a delivery
   configuration through [`sync_accounts`](/dist/docs/3.1.22/accounts/tasks/sync_accounts),
...
[controller reference](/dist/docs/3.1.22/building/by-layer/L3/comply-test-controller#reporting_core_lifecycle_probe)
...
- **`managed_delivery: true`** adds offerings with a `method` —
  ...
  principal-layer destinations ([`sync_principal`](/dist/docs/3.1.22/protocol/sync_principal)).
