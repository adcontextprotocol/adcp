...
For the feature narrative, start with [What's New in AdCP 3.2](/dist/docs/3.1.22/reference/whats-new-in-3-2).
For prerelease artifacts and SDK timing, use the [3.2 prerelease program](/dist/docs/3.1.22/reference/3-2-beta).
...
Use a maintained [AdCP SDK](/dist/docs/3.1.22/building/by-layer/L4/choose-your-sdk) for
...
| 2 | SDK and codegen maintainers | Generate from the signed `3.2.0-rc.1` protocol tarball, not `main` or `/schemas/3.1.22/`. |
...
until 4.0. See [Migrating to `media_buy_status`](/dist/docs/3.1.22/reference/migration/media-buy-status)
...
webhook receivers. See [Signed requests](/dist/docs/3.1.22/building/by-layer/L1/security#signed-requests-transport-layer).
...
Follow the complete field map in [Targeting-aware product discovery](/dist/docs/3.1.22/reference/migration/targeting-aware-discovery).
...
automatically. See [Migrating secured asset access](/dist/docs/3.1.22/reference/migration/asset-access).
...
See [Cross-role governance enforcement](/dist/docs/3.1.22/reference/migration/cross-role-governance-enforcement).
