...
[3.2 prerelease program](/dist/docs/3.1.22/reference/3-2-beta).
</Warning>
...
<CardGroup cols={3}>
  ...
  <Card title="Build it" icon="code" href="/dist/docs/3.1.22/building/by-layer/L4/choose-your-sdk">
    Choose an SDK-backed RC.1 path or use the signed RC.1 bundle directly.
  </Card>
  <Card title="Adopt it" icon="route" href="/dist/docs/3.1.22/reference/migration/3-1-to-3-2">
...
| A developer or platform team | Seven capability-gated tools replace overloaded modes, with immutable proposal state, revision checks, exact version negotiation, and MCP validation-schema pairs that are 76–95% smaller for comparable operations. | [Choose an SDK](/dist/docs/3.1.22/building/by-layer/L4/choose-your-sdk) |
| An existing 3.1 adopter | There is no forced migration deadline: keep production pinned to `"3.1"`, add compact tools incrementally, and validate an exact release candidate in staging. | [Follow the migration guide](/dist/docs/3.1.22/reference/migration/3-1-to-3-2) |
...
facades. See [Proposal negotiation](/dist/docs/3.1.22/media-buy/product-discovery/proposal-negotiation)
...
[Implementing Reliable Reporting Core](/dist/docs/3.1.22/media-buy/reporting-core-implementation-guide).
...
After authentication, [`sync_principal`](/dist/docs/3.1.22/protocol/sync_principal) lets a
...
[`get_principal`](/dist/docs/3.1.22/protocol/get_principal) is the side-effect-free repair
...
`overlay_support.daypart_targets`. See [Targeting-aware product discovery](/dist/docs/3.1.22/reference/migration/targeting-aware-discovery).
...
See [programmed channels and owner-sold carriage](/dist/docs/3.1.22/media-buy/product-discovery/collections-and-installments#programmed-channels-and-owner-sold-carriage).
...
asset-scoped bearer tokens. See [Migrating secured asset access](/dist/docs/3.1.22/reference/migration/asset-access).
...
ratcheted floors only as regression guards. See [Known Limitations](/dist/docs/3.1.22/reference/known-limitations#conformance-and-testing)
...
1. Read the [3.2 prerelease program](/dist/docs/3.1.22/reference/3-2-beta).
2. Follow [Migrating from 3.1 to 3.2](/dist/docs/3.1.22/reference/migration/3-1-to-3-2).
3. Choose the exact package and support level in [Choose your SDK](/dist/docs/3.1.22/building/by-layer/L4/choose-your-sdk).
4. Use the [Release Notes](/dist/docs/3.1.22/reference/release-notes#version-3-2-0) for the cumulative record.
