...
document URL, not a JSON Schema URL, so it does not live under `/schemas/3.1.22/`.
...
schemas—for example, 3.0, 3.1, or 3.2 schemas under `/schemas/3.1.22/`—without
...
A2A 1.0 models `Part.data` as `google.protobuf.Value`. Its numeric arm is an IEEE 754 double, so an integer-valued JSON number can reserialize with a decimal point (`3` → `3.0`). AdCP treats those forms as the same JSON numeric value: both satisfy a JSON Schema `integer` type because they have no fractional value. This lexical change does not violate the [context echo contract](/dist/docs/3.1.22/building/by-layer/L2/context-sessions#normative-echo-contract) and does not make an otherwise valid integer-typed task field invalid.
...
contract](/dist/docs/3.1.22/building/by-layer/L3/async-identity-and-convergence).
...
[A2A response extraction algorithm](/dist/docs/3.1.22/building/by-layer/L0/a2a-response-extraction)
...
Machine-readable advertisement, activation, invocation, Submitted mapping, and polling fixtures are published in [`a2a-profile-extension-v3.json`](https://adcontextprotocol.org/test-vectors/a2a-profile-extension-v3.json). See [Reference test vectors](/dist/docs/3.1.22/reference/test-vectors) for versioning guidance.
...
- [A2A Guide](/dist/docs/3.1.22/building/by-layer/L0/a2a-guide)
- [`get_adcp_capabilities`](/dist/docs/3.1.22/protocol/get_adcp_capabilities)
- [Calling an AdCP agent](/dist/docs/3.1.22/protocol/calling-an-agent)
