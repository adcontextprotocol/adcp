...
|---------|--------------------------|----------------|
| [`get_adcp_capabilities`](/dist/docs/3.0.18/protocol/get_adcp_capabilities) | Which protocols and specialisms you claim, and that you support sandbox | Your agent's capability response |
| [`sync_accounts`](/dist/docs/3.0.18/media-buy/advanced-topics/sandbox) (or `list_accounts`) | How to obtain a sandbox account to run tests against | Your agent's account tool |
| [`comply_test_controller`](/dist/docs/3.0.18/building/by-layer/L3/comply-test-controller) | How to seed fixtures and force seller-side transitions deterministically | Your agent, sandbox-only |
...
`get_adcp_capabilities` is how the runner picks which storyboards apply to you. It is also the [conformance](/dist/docs/3.0.18/building/verification/conformance) contract: you are promising to pass every storyboard that matches what you declare.
...
- **`specialisms`** — Pulls in opt-in specialism storyboards (see the [Compliance Catalog](/dist/docs/3.0.18/building/verification/compliance-catalog) for the full enumeration).
...
<Warning>
Claiming `sales-guaranteed` when you only run RTB ships you into storyboards you will fail on record. Conformance status is part of the [Verified](/dist/docs/3.0.18/building/verification/conformance) badge buyer agents use to filter sellers — overclaim once, lose inclusion everywhere.
</Warning>
...
See the [Compliance test controller reference](/dist/docs/3.0.18/building/by-layer/L3/comply-test-controller) for scenario-by-scenario parameters and response shapes, and the [Compliance Catalog](/dist/docs/3.0.18/building/verification/compliance-catalog) for which scenarios each specialism requires.
...
<Note>
The scaffold is TypeScript/JavaScript. Python, Go, and Java sellers implement the tool directly against the [schema](https://adcontextprotocol.org/schemas/3.0.18/compliance/comply-test-controller-request.json) — the contract below (adapters, error codes, idempotency semantics) applies the same way. SDKs for other languages are tracked in [Schemas and SDKs](/dist/docs/3.0.18/building/by-layer/L0/schemas).
</Note>
...
The runner discovers your capabilities, obtains a sandbox account, seeds fixtures via the controller, and walks each matching storyboard. See [Validate Your Agent](/dist/docs/3.0.18/building/verification/validate-your-agent) for the full CLI, debug flags, and Addie workflows.
...
- **[Validate Your Agent](/dist/docs/3.0.18/building/verification/validate-your-agent)** — CLI, Addie workflows, and multi-instance verification
- **[Compliance test controller reference](/dist/docs/3.0.18/building/by-layer/L3/comply-test-controller)** — Full scenario-by-scenario spec
- **[Sandbox mode](/dist/docs/3.0.18/media-buy/advanced-topics/sandbox)** — The two account model paths in depth
- **[Conformance](/dist/docs/3.0.18/building/verification/conformance)** — What "conformant" and "verified" mean once your runs pass
