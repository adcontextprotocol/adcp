...
<Note>
This shows a simplified structure. The full adagents.json schema uses separate `properties` and `authorized_agents` arrays with a `delegation_type` field. See the [adagents.json tech spec](/dist/docs/3.0.18/governance/property/adagents) for the complete schema.
</Note>
...
- **Walled gardens** (social platforms, AI platforms, retail media networks) typically use explicit accounts — set `require_operator_auth: true` in [`get_adcp_capabilities`](/dist/docs/3.0.18/protocol/get_adcp_capabilities) so each operator authenticates independently.
