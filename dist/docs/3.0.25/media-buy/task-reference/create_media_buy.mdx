...
**Request Schema**: [`/schemas/3.0.25/media-buy/create-media-buy-request.json`](https://adcontextprotocol.org/schemas/3.0.25/media-buy/create-media-buy-request.json)
**Response Schema**: [`/schemas/3.0.25/media-buy/create-media-buy-response.json`](https://adcontextprotocol.org/schemas/3.0.25/media-buy/create-media-buy-response.json)
...
|-----------|------|----------|-------------|
| `account` | [account-ref](/dist/docs/3.0.25/building/by-layer/L2/accounts-and-agents#account-references) | Yes | Account reference. Pass `{ "account_id": "..." }` or `{ "brand": {...}, "operator": "..." }` if the seller supports implicit resolution. Required for billing and policy evaluation. |
...
| `brand` | BrandRef | Yes | Brand reference — resolved to full identity at execution time. See [brand.json](/dist/docs/3.0.25/brand-protocol/brand-json) |
| `start_time` | string | Yes | `"asap"` or ISO 8601 date-time |
| `end_time` | string | Yes | ISO 8601 date-time (UTC unless timezone specified) |
| `invoice_recipient` | [BusinessEntity](/dist/docs/3.0.25/building/by-layer/L2/accounts-and-agents#billing-entity-and-invoice-recipient) | No | Override the account's default billing entity for this buy. The seller MUST validate the recipient is authorized and include it in `check_governance` when governance agents are configured. |
...
| `measurement_terms` | [MeasurementTerms](/dist/docs/3.0.25/media-buy/advanced-topics/pricing-models#measurement-terms-and-performance-standards) | No | Buyer's proposed billing measurement and makegood terms. Overrides product defaults. Seller accepts (echoed on confirmed package), rejects with `TERMS_REJECTED`, or adjusts. When omitted, product's `measurement_terms` apply. |
| `performance_standards` | [PerformanceStandard[]](/dist/docs/3.0.25/media-buy/advanced-topics/pricing-models#measurement-terms-and-performance-standards) | No | Buyer's proposed performance standards (viewability, IVT, completion rate, brand safety, attention score). Overrides product defaults. Seller accepts, rejects with `TERMS_REJECTED`, or adjusts. When omitted, product's `performance_standards` apply. |
...
Include the `catalogs` field in a package to make it catalog-driven. Each catalog should have a distinct type (e.g., one product catalog, one store catalog). The referenced catalogs must already be synced via [`sync_catalogs`](/dist/docs/3.0.25/media-buy/task-reference/sync_catalogs).
...
The platform distributes budget across catalog items based on performance. For per-item reporting, use [`get_media_buy_delivery`](/dist/docs/3.0.25/media-buy/task-reference/get_media_buy_delivery) which returns `by_catalog_item` breakdowns. Creative variants for catalog-driven packages represent individual catalog items rendered as ads.
...
See [Proposals](/dist/docs/3.0.25/media-buy/product-discovery/media-products#proposals) for the complete workflow.
...
Full brand identity data (colors, fonts, product catalog) is resolved from brand.json at execution time. See [brand.json](/dist/docs/3.0.25/brand-protocol/brand-json).
...
Packages can use different currencies when sellers support it. See [Pricing Models](/dist/docs/3.0.25/media-buy/advanced-topics/pricing-models).
...
- Frequency capping
- AXE segment inclusion/exclusion (legacy — new integrations use [TMP](/dist/docs/3.0.25/trusted-match))

See [Targeting](/dist/docs/3.0.25/media-buy/advanced-topics/targeting) for details.
...
**Note:** For the complete status list see [Task Lifecycle](/dist/docs/3.0.25/building/by-layer/L3/task-lifecycle).
...
For complete async handling patterns, see [Async Operations](/dist/docs/3.0.25/building/by-layer/L3/async-operations).
...
- Impression-time targeting (audience, frequency, suitability) is handled by [TMP](/dist/docs/3.0.25/trusted-match)
...
<Note>
Content standards are created by calling [`create_content_standards`](/dist/docs/3.0.25/governance/content-standards/tasks/create_content_standards) on a verification agent (e.g., IAS, DoubleVerify). Standards MUST be [calibrated](/dist/docs/3.0.25/governance/content-standards/tasks/calibrate_content) with each seller before use in production to ensure the seller's local evaluation model aligns with the verification agent's interpretation. See the [Content Standards overview](/dist/docs/3.0.25/governance/content-standards/index) for the full setup workflow: create → calibrate → activate → validate.
</Note>
...
1. **Upload Creatives**: Use [`sync_creatives`](/dist/docs/3.0.25/creative/task-reference/sync_creatives) before deadline
2. **Monitor Status**: Use [`get_media_buy_delivery`](/dist/docs/3.0.25/media-buy/task-reference/get_media_buy_delivery)
3. **Optimize**: Use [`provide_performance_feedback`](/dist/docs/3.0.25/media-buy/task-reference/provide_performance_feedback)
4. **Update**: Use [`update_media_buy`](/dist/docs/3.0.25/media-buy/task-reference/update_media_buy) to modify campaign

## Learn More

- [Media Buy Lifecycle](/dist/docs/3.0.25/media-buy/media-buys/) - Complete campaign workflow
- [get_products](/dist/docs/3.0.25/media-buy/task-reference/get_products) - Discover inventory
- [Targeting](/dist/docs/3.0.25/media-buy/advanced-topics/targeting) - Targeting strategies
- [Pricing Models](/dist/docs/3.0.25/media-buy/advanced-topics/pricing-models) - Currency and pricing
