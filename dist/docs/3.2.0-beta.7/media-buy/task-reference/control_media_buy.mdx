...
[`sync_creatives`](/dist/docs/3.2.0-beta.7/creative/task-reference/sync_creatives) has a separate idempotency and atomicity boundary, does not
...
without a race window. Use the [`update_media_buy`](/dist/docs/3.2.0-beta.7/media-buy/task-reference/update_media_buy) compatibility facade to send
...
transaction. If [`accept_proposal`](/dist/docs/3.2.0-beta.7/media-buy/task-reference/accept_proposal) applies the amendment and a subsequent
...
Catalog controls accept only IDs previously managed through [`sync_catalogs`](/dist/docs/3.2.0-beta.7/media-buy/task-reference/sync_catalogs).
...
**Request schema:** [`/schemas/3.2.0-beta.7/media-buy/control-media-buy-request.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.7/media-buy/control-media-buy-request.json)
...
When an otherwise valid control would exceed the accepted budget, targeting, or delivery envelope, the seller returns [`REQUOTE_REQUIRED`](/dist/docs/3.2.0-beta.7/building/verification/compliance-catalog#error-code-requote-required). The buyer reads `accepted_proposal_id` from [`get_media_buys`](/dist/docs/3.2.0-beta.7/media-buy/task-reference/get_media_buys), passes it to [`refine_proposals`](/dist/docs/3.2.0-beta.7/media-buy/task-reference/refine_proposals) to create a draft amendment, finalizes that draft into a committed hold, and applies it through [`accept_proposal`](/dist/docs/3.2.0-beta.7/media-buy/task-reference/accept_proposal). This pointer survives SDK restart and moves atomically after each accepted amendment.
