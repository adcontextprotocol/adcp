...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.7/core/creative-representation-set.json",
...
5. Normalize tracker defaults, bind one exact VAST/DAAST execution version, and match every first-class tracker against the destination's [tracker execution contract](/dist/docs/3.2.0-beta.7/creative/tracker-execution-contracts).
...
When no candidate works, return `CREATIVE_REPRESENTATION_UNRESOLVED`. `error.details` MUST conform to [`error-details/creative-representation-unresolved.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.7/error-details/creative-representation-unresolved.json), with exactly one `representation_rejections` entry for every candidate. Unsupported representations remain in the immutable set; failure does not delete or rewrite them.
...
- [Display ads](/dist/docs/3.2.0-beta.7/creative/channels/display)
- [Video ads](/dist/docs/3.2.0-beta.7/creative/channels/video)
- [Universal macros](/dist/docs/3.2.0-beta.7/creative/universal-macros)
- [Creative manifests](/dist/docs/3.2.0-beta.7/creative/creative-manifests)
