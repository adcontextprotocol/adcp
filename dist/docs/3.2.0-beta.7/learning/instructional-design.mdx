...
- **Learner escalation** — learners who need human assistance can contact certification@agenticadvertising.org. Assessment disputes are handled through the [complaints process](/dist/docs/3.2.0-beta.7/learning/policies/complaints)
...
Recertification is targeted, not blanket. If a protocol update adds a new governance task but doesn't affect media buy workflows, only credentials that cover governance are flagged. Credential holders receive a notification through Addie with context on what changed and what they need to review. The decision process, delta-assessment rules, and recorded AdCP 3.1 and 3.2 decisions are documented in the [recertification policy](/dist/docs/3.2.0-beta.7/learning/policies/recertification).
...
Modules B4, C4, and D4 use a five-phase approach built on the same tools developers use in practice — [skill files](/dist/docs/3.2.0-beta.7/building/by-layer/L4/build-an-agent) to generate agents and [storyboards](/dist/docs/3.2.0-beta.7/building/verification/validate-your-agent) to validate them:
...
Organizational policies supporting this framework are documented in the [policies section](/dist/docs/3.2.0-beta.7/learning/policies/nondiscrimination), including the [recertification policy](/dist/docs/3.2.0-beta.7/learning/policies/recertification).
