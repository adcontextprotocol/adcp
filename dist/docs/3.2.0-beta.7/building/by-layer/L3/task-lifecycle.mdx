...
Human approval at the task layer is modelled as `input-required` (when the buyer must respond, e.g. confirm a budget) or `submitted` (when the seller is waiting on an internal human, e.g. IO signing). These implement the [Embedded Human Judgment](/dist/docs/3.2.0-beta.7/governance/embedded-human-judgment) principle that judgment cannot be delegated to software — when an action exceeds autonomous authority, the system halts for human review rather than proceeding.
...
- **All transports**: Use [push notifications](/dist/docs/3.2.0-beta.7/building/by-layer/L3/webhooks) (recommended) or poll the AdCP task with `get_task_status` / legacy `tasks/get`.
...
Notifications](/dist/docs/3.2.0-beta.7/building/by-layer/L3/webhooks)); `include_result` is the
...
- **Async Operations**: See [Async Operations](/dist/docs/3.2.0-beta.7/building/by-layer/L3/async-operations) for handling different operation types
- **Webhooks**: See [Webhooks](/dist/docs/3.2.0-beta.7/building/by-layer/L3/webhooks) for push notification patterns
- **Error Handling**: See [Error Handling](/dist/docs/3.2.0-beta.7/building/by-layer/L3/error-handling) for error categories and recovery
