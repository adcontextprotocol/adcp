...
This task sits between format discovery and creative submission in the [account state setup sequence](/dist/docs/3.0.2/building/by-layer/L2/account-state):
...
|-----------|------|----------|-------------|
| `account` | [account-ref](/dist/docs/3.0.2/building/by-layer/L2/accounts-and-agents#account-references) | Conditional | Account reference. Pass `{ "account_id": "..." }` or `{ "brand": {...}, "operator": "..." }` if the seller supports implicit resolution. Required when the agent has multiple accounts. |
...
- [Account state](/dist/docs/3.0.2/building/by-layer/L2/account-state) — How catalogs fit into the account setup sequence
