...
- See [Security](/dist/docs/3.0.1/building/implementation/security) for implementation details
...
Webhook delivery status is tracked through **AdCP's global task management system** (see [Task Lifecycle](/dist/docs/3.0.1/building/implementation/task-lifecycle)).
...
Offline files sit at rest for `file_retention_days`, so a misconfigured IAM policy leaks historical reporting across tenants. The [general security controls](/dist/docs/3.0.1/building/implementation/security) apply; the offline-specific requirements:
...
See [Webhooks](/dist/docs/3.0.1/building/implementation/webhooks) for detailed implementation guidance.
