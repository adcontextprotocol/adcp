...
This is the same dependency order documented in [Account state](/dist/docs/3.0.1/building/by-layer/L2/account-state): `sync_catalogs` → `sync_creatives` → `create_media_buy`.
...
- **[Account state](/dist/docs/3.0.1/building/by-layer/L2/account-state)** — How catalogs fit into account state and the setup sequence
