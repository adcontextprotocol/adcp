...
Create a working sales agent that responds to real buyer queries. Point your AI coding assistant (Claude Code, Cursor, Copilot) at the [`build-seller-agent`](/dist/docs/3.1.0-beta.0/building/build-an-agent) skill file — it generates an agent validated against AdCP storyboards, with optional [`comply_test_controller`](/dist/docs/3.1.0-beta.0/building/implementation/comply-test-controller) scaffolding for deterministic lifecycle testing in sandbox mode. The skill tested is specifying correct AdCP behavior.
...
The storyboard exercises the complete buyer workflow — discovery, account sync, media buy, creatives, delivery — and validates every response. See [Validate Your Agent](/dist/docs/3.1.0-beta.0/building/validate-your-agent) for setup and debugging.
...
See the [Compliance Catalog](/dist/docs/3.1.0-beta.0/building/compliance-catalog) for the full list.
