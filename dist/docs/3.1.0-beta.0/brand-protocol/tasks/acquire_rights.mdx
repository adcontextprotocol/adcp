...
| `push_notification_config` | push-notification-config | No | Webhook for async status updates if the acquisition requires approval. See [push notifications](/dist/docs/3.1.0-beta.0/building/implementation/webhooks). |
...
1. **Inline path** — the request carries an intent-phase `governance_context` token on the [protocol envelope](/dist/docs/3.1.0-beta.0/building/implementation/security). The buyer threads the token explicitly per request.
...
- Deduplicating by `idempotency_key` — the same revocation may be delivered multiple times; see [Push Notifications — Reliability](/dist/docs/3.1.0-beta.0/building/implementation/webhooks#reliability) for the canonical dedup contract
