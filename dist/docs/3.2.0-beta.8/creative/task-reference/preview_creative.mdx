...
`preview_creative` renders an existing creative manifest into viewable output. It does not generate or modify the input manifest — use [`build_creative`](/dist/docs/3.2.0-beta.8/creative/task-reference/build_creative) for that. Supports both single creative preview and batch preview (5-10x faster for multiple creatives).

**Request Schema**: [`/schemas/3.2.0-beta.8/creative/preview-creative-request.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.8/creative/preview-creative-request.json)
**Response Schema**: [`/schemas/3.2.0-beta.8/creative/preview-creative-response.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.8/creative/preview-creative-response.json)
...
Preview what a specific variant looked like when served. Use `variant_id` from [`get_creative_delivery`](/dist/docs/3.2.0-beta.8/creative/task-reference/get_creative_delivery) response:
...
```json
{
  "$schema": "/schemas/3.2.0-beta.8/creative/preview-creative-request.json",
  "request_type": "single",
  "quality": "draft",
  "creative_manifest": {
...
After the campaign runs, use variant mode to see exactly what was served. Pass a `variant_id` from [`get_creative_delivery`](/dist/docs/3.2.0-beta.8/creative/task-reference/get_creative_delivery):
...
- [Advanced Preview Patterns](/dist/docs/3.2.0-beta.8/creative/task-reference/preview_creative-advanced) - Caching, workflows, implementation notes
- [Creative Manifests](/dist/docs/3.2.0-beta.8/creative/creative-manifests) - Manifest structure
- [Creative Formats](/dist/docs/3.2.0-beta.8/creative/formats) - Format specifications
