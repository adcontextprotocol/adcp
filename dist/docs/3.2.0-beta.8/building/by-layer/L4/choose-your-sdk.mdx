...
For the layered model behind this — what each layer contains and what an SDK at each layer should provide — see the [SDK stack reference](/dist/docs/3.2.0-beta.8/building/cross-cutting/sdk-stack).

## Coverage matrix

What "shipped" means at each layer is the [L0–L3 checklist](/dist/docs/3.2.0-beta.8/building/cross-cutting/sdk-stack#what-an-sdk-at-each-layer-should-provide).

*Last updated: 2026-08-24.*

| SDK | Current package | 3.2 beta support | L0 | L1 | L2 | L3 |
|---|---|---|:-:|:-:|:-:|:-:|
| **`@adcp/sdk`** (TypeScript) | `14.0.0-beta.8` | Exact `3.2.0-beta.6` support | ✅ | ✅ | ✅ | ✅ |
...
<Warning>
**3.2 packages are prereleases.** The TypeScript beta supports the exact beta.6
protocol checkpoint, while the Python beta supports exact beta.5; neither
implies compatibility with a later checkpoint. Go has not published an exact
3.2 support statement yet. See the [3.2 beta program](/dist/docs/3.2.0-beta.8/reference/3-2-beta).
</Warning>
...
For the status of every published protocol version, see [Versions & Compatibility](/dist/docs/3.2.0-beta.8/reference/versions). Migration and archived-version guidance lives under [Release notes & migration](/dist/docs/3.2.0-beta.8/reference/migration/index).
...
community-maintained ports are welcome — see the [Builders Working Group](/dist/docs/3.2.0-beta.8/community/working-group)
and the [Slack community](/dist/docs/3.2.0-beta.8/community/joining-slack).
...
- `@adcp/sdk/testing` — buyer-side storyboard runner (`runStoryboard`, `comply`, `testAgent`) and seller-side controller scaffold (`createComplyController` — see [Get Test-Ready](/dist/docs/3.2.0-beta.8/building/verification/get-test-ready))
...
```bash
npx @adcp/sdk@14.0.0-beta.8 --help
npx @adcp/sdk@14.0.0-beta.8 --save-auth my-agent https://sales.example.com/mcp
npx @adcp/sdk@14.0.0-beta.8 my-agent list_products '{"adcp_version":"3.2-beta.6","account":{"account_id":"account_123"}}'
# or against the built-in public test agent:
npx @adcp/sdk@14.0.0-beta.8 test-mcp list_products '{"adcp_version":"3.2-beta.6"}'
```
...
- **[Build an agent](/dist/docs/3.2.0-beta.8/building/by-layer/L4/build-an-agent)** — server-side L4 path. Skill files + coding agent.
- **[Build a caller](/dist/docs/3.2.0-beta.8/building/by-layer/L4/build-a-caller)** — client-side L4 path. Install, call, handle responses, ingest reporting.
- **[Schemas](/dist/docs/3.2.0-beta.8/building/by-layer/L0/schemas)** — schema bundle, type generation, version pinning.
- **[Migrate from hand-rolled](/dist/docs/3.2.0-beta.8/building/by-layer/L4/migrate-from-hand-rolled)** — already running an AdCP agent built before the SDKs covered much? Swap one layer at a time.
