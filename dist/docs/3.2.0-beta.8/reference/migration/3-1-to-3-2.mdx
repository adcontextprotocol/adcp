...
<Warning>
**3.2 is in beta.** Beta.6 is the current protocol checkpoint. TypeScript
`@adcp/sdk@14.0.0-beta.8` embeds beta.6; Python `adcp==8.0.0b7` remains on
beta.5, so use the signed beta.6 artifact directly unless your SDK explicitly names beta.6.
...
For the feature narrative, start with [What's New in AdCP 3.2](/dist/docs/3.2.0-beta.8/reference/whats-new-in-3-2).
For prerelease artifacts and SDK timing, use the [3.2 beta program](/dist/docs/3.2.0-beta.8/reference/3-2-beta).
...
| 2 | SDK and codegen maintainers | Generate from the signed `3.2.0-beta.6` protocol tarball, not `main` or `/schemas/3.2.0-beta.8/`. |
...
| 8 | Compliance operators | Test beta.6 from its signed assets or pair `@adcp/sdk@14.0.0-beta.8` with exact beta.6; pair Python `adcp==8.0.0b7` with exact beta.5; require an explicit beta.6 support statement before using any other SDK with the new checkpoint. |

## Required to claim 3.2 behavior

### Serve and echo the exact release

During beta, prerelease matching is exact. A seller serving beta.6 advertises
`"3.2-beta.6"`. Do not advertise stable `"3.2"` before GA, silently negotiate
...
until 4.0. See [Migrating to `media_buy_status`](/dist/docs/3.2.0-beta.8/reference/migration/media-buy-status)
...
webhook receivers. See [Signed requests](/dist/docs/3.2.0-beta.8/building/by-layer/L1/security#signed-requests-transport-layer).
...
Follow the complete field map in [Targeting-aware product discovery](/dist/docs/3.2.0-beta.8/reference/migration/targeting-aware-discovery).
...
automatically. See [Migrating secured asset access](/dist/docs/3.2.0-beta.8/reference/migration/asset-access).
...
See [Cross-role governance enforcement](/dist/docs/3.2.0-beta.8/reference/migration/cross-role-governance-enforcement).
...
| SDK maintainer | Signed beta.6 bundle ingestion, generated-type/validator tests, exact support matrix entry, and protocol feedback destined for a later beta. |
| Compliance operator | Matching beta.6 assets end to end, or an exact beta.5 SDK pairing; no 3.2 badge from a beta checkpoint. |
...
- [Version Adaptation](/dist/docs/3.2.0-beta.8/building/cross-cutting/version-adaptation)
