...
The 3.2 beta cycle started with a protocol-first checkpoint. Beta.8 is the
current protocol checkpoint. No SDK release names exact beta.8 support yet: the
current TypeScript SDK beta consumes the signed beta.6 bundle, and the Python
SDK beta remains on beta.5:
...
| `3.2.0-beta.6` | Current TypeScript SDK-backed protocol checkpoint for reporting, CTV, canonical display, and request-signing integration | Buyer and seller teams running end-to-end TypeScript SDK integrations |
...
| `3.2.0-beta.8` | Current protocol checkpoint for metric-container subsumption conformance and corrected versioned documentation snapshots | Raw-wire adopters, code-generator maintainers, SDK maintainers, and agent teams validating the signed beta.8 artifact |
| SDK wave | `@adcp/sdk@14.0.0-beta.8` embeds `3.2.0-beta.6`; Python `adcp==8.0.0b7` remains on `3.2.0-beta.5`; Go remains pending | TypeScript and Python adopters, Go maintainers, and early integration teams |

Earlier betas remain useful as pinned implementation checkpoints. For
SDK-backed TypeScript testing, pair SDK beta.8 with protocol beta.6. Python
testing remains paired with beta.5 until a Python SDK release explicitly names
another checkpoint. Test beta.8 from its signed artifact with raw-wire or
code-generation workflows. Do not combine a newer wire pin with an SDK generated
from an older bundle unless the SDK maintainer documents forward compatibility.
...
| Surface | beta.0 | beta.1 | beta.2 | beta.3 | beta.4 | beta.5 | beta.6 | beta.7 | beta.8 |
|---|---|---|---|---|---|---|---|---|---|
| Protocol artifact | `3.2.0-beta.0` | `3.2.0-beta.1` | `3.2.0-beta.2` | `3.2.0-beta.3` | `3.2.0-beta.4` | `3.2.0-beta.5` | `3.2.0-beta.6` | `3.2.0-beta.7` | `3.2.0-beta.8` |
| Git tag | `v3.2.0-beta.0` | `v3.2.0-beta.1` | `v3.2.0-beta.2` | `v3.2.0-beta.3` | `v3.2.0-beta.4` | `v3.2.0-beta.5` | `v3.2.0-beta.6` | `v3.2.0-beta.7` | `v3.2.0-beta.8` |
| `adcp_version` wire pin | `"3.2-beta.0"` | `"3.2-beta.1"` | `"3.2-beta.2"` | `"3.2-beta.3"` | `"3.2-beta.4"` | `"3.2-beta.5"` | `"3.2-beta.6"` | `"3.2-beta.7"` | `"3.2-beta.8"` |
| Documentation selector | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` |
...
buyer pinned to `"3.2-beta.8"` must not silently negotiate to another beta, and
a stable `"3.2"` pin must not silently negotiate to a beta.

## Use matched SDK betas

The TypeScript SDK has exact beta.6 support. The npm
`latest` tag remains on SDK 13, so opt into the preview explicitly:

```bash
npm install @adcp/sdk@14.0.0-beta.8
```

SDK 14 includes typed caller and server support for the compact media-buy
lifecycle, the beta.6 schemas and compliance assets, version-aware request
...
See [Choose your SDK](/dist/docs/3.2.0-beta.8/building/by-layer/L4/choose-your-sdk) for the support
matrix and install guidance.

Python `adcp==8.0.0b7` still carries beta.5 schemas and generated types. Its
...
## Use the beta.8 artifact directly

Download the signed beta.8 protocol bundle rather
than copying schemas from `main` or from the moving `/schemas/3.2.0-beta.8/` path:

```bash
curl -fLO https://adcontextprotocol.org/protocol/3.2.0-beta.8.tgz
curl -fLO https://adcontextprotocol.org/protocol/3.2.0-beta.8.tgz.sha256
shasum -a 256 -c 3.2.0-beta.8.tgz.sha256
tar -xzf 3.2.0-beta.8.tgz
```

The extracted `adcp-3.2.0-beta.8/` directory contains:
...
[Verifying protocol tarballs](/dist/docs/3.2.0-beta.8/reference/verifying-protocol-tarballs) for
...
4. Exercise raw MCP or A2A calls with `adcp_version: "3.2-beta.8"` against a
...
<Note>
**No 3.2 verification badge is currently issued for beta.8.** Treat it as an
...
The SDK compatibility matrix lives in [Choose your SDK](/dist/docs/3.2.0-beta.8/building/by-layer/L4/choose-your-sdk).
The TypeScript row names exact beta.6 support; the Python row names exact
beta.5 support with different higher-layer coverage. Neither implies support
...
## Current beta.8 checkpoint

Protocol beta.1 through beta.8 are published. No SDK release names exact beta.8
support yet. TypeScript SDK beta.8 embeds the exact beta.6 bundle, while Python
SDK beta.7 embeds the exact beta.5 bundle. Before treating a test result as
beta.8 protocol evidence, confirm that:

- the test uses the signed beta.8 artifact directly;
- both peers advertise `"3.2-beta.8"` before calls use that pin;
- the integration scenarios below run against beta.8 and retain the exact
  bundle version;
...
- Experimental surfaces retain their [experimental-status contract](/dist/docs/3.2.0-beta.8/reference/experimental-status).
...
- whether the problem blocks beta.8 interoperability.
...
- [What's New in AdCP 3.2](/dist/docs/3.2.0-beta.8/reference/whats-new-in-3-2)
- [Migrating from 3.1 to 3.2](/dist/docs/3.2.0-beta.8/reference/migration/3-1-to-3-2)
- [Release Notes](/dist/docs/3.2.0-beta.8/reference/release-notes#version-3-2-0)
- [Versions & Compatibility](/dist/docs/3.2.0-beta.8/reference/versions)
- [Choose your SDK](/dist/docs/3.2.0-beta.8/building/by-layer/L4/choose-your-sdk)
