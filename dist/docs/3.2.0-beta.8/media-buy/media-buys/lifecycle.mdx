...
This page is the canonical sequence reference for media buy lifecycle. For conceptual background on the full lifecycle — campaign structure, package model, property targeting, and async operations — see [Media Buy Lifecycle](/dist/docs/3.2.0-beta.8/media-buy/media-buys/).
...
state is then read through `get_media_buys`. See [task status and MediaBuy state](/dist/docs/3.2.0-beta.8/media-buy/media-buys#state-machine).
</Note>
...
signal. See [Accountability](/dist/docs/3.2.0-beta.8/media-buy/advanced-topics/accountability) for
...
Buyer agents triaging a non-empty `impairments[]` SHOULD sort entries by `observed_at` ascending — the oldest open impairment is the most likely to have already eaten into delivery. Webhook arrival time is not a reliable proxy: under the coalescence rule (see [webhooks § Coalescence](/dist/docs/3.2.0-beta.8/building/by-layer/L3/webhooks#coalescence)) the seller MAY batch multiple state changes into one fire, and re-emission under a fresh `idempotency_key` resets the transport timestamp without changing `observed_at`.
...
**Snapshot is one of several propagation surfaces.** Sellers declare which surfaces they use via [`capabilities.media_buy.propagation_surfaces`](https://adcontextprotocol.org/schemas/3.2.0-beta.8/protocol/get-adcp-capabilities-response.json) on `get_adcp_capabilities` — a non-exclusive array, so a seller mirroring impairments on both the buy snapshot AND firing webhooks declares `["snapshot", "webhook"]` (the common case for premium guaranteed sellers). The three surface values:
...
See the [Snapshot and log contract](/dist/docs/3.2.0-beta.8/protocol/snapshot-and-log) for the read-side rules that tie `impairments[]` (snapshot) to the `impairment` push (log).
...
- [Media Buy Lifecycle](/dist/docs/3.2.0-beta.8/media-buy/media-buys/) — full lifecycle reference: campaign structure, package model, async operations
- [`buy_products`](/dist/docs/3.2.0-beta.8/media-buy/task-reference/buy_products) — purchase versioned published offers directly
- [`accept_proposal`](/dist/docs/3.2.0-beta.8/media-buy/task-reference/accept_proposal) — accept committed new-buy, amendment, or cancellation terms
- [`control_media_buy`](/dist/docs/3.2.0-beta.8/media-buy/task-reference/control_media_buy) — apply revision-checked operational controls
- [`sync_creatives`](/dist/docs/3.2.0-beta.8/creative/task-reference/sync_creatives) — upload and update library creatives when the seller advertises `creative.has_creative_library: true`
- [Accountability](/dist/docs/3.2.0-beta.8/media-buy/advanced-topics/accountability) — performance standards, measurement terms, makegood resolution
- [Optimization & Reporting](/dist/docs/3.2.0-beta.8/media-buy/media-buys/optimization-reporting) — delivery monitoring, dimensional reporting, campaign updates
