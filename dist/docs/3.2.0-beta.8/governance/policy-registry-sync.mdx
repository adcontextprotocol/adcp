...
**Use a different `policy_id` from the registry one.** If the buyer reuses the canonical ID `alcohol_advertising` in `custom_policies`, governance-agent behavior is undefined by the spec — the [policy-entry schema](https://adcontextprotocol.org/schemas/3.2.0-beta.8/governance/policy-entry.json)'s additive-only rule pins registry text as authoritative for that ID. A pinning ID like `alcohol_advertising_pinned_2026Q4` (or your internal versioning convention) sidesteps the conflict.
...
- [Policy Registry](/dist/docs/3.2.0-beta.8/governance/policy-registry) — registry concepts, policy structure, seeded policies, restricted attributes
- [Campaign Governance specification](/dist/docs/3.2.0-beta.8/governance/campaign/specification) — plan binding, `plan_hash`, governance context lifecycle
- [audit trail: internal vs shareable views](/dist/docs/3.2.0-beta.8/governance/campaign/audit-trail) — how to surface evaluation history to counterparties
- [`sync_plans`](/dist/docs/3.2.0-beta.8/governance/campaign/tasks/sync_plans) — `policy_ids`, `policy_categories`, `custom_policies`
- [Annex III & Art 22 obligations](/dist/docs/3.2.0-beta.8/governance/annex-iii-obligations) — when human review is required
