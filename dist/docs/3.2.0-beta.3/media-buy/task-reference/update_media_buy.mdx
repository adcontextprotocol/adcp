...
<Warning>
`update_media_buy` is the AdCP 3.x compatibility facade as of 3.2. New integrations use [`control_media_buy`](/dist/docs/3.2.0-beta.3/media-buy/task-reference/control_media_buy) for operational controls and [`refine_proposals`](/dist/docs/3.2.0-beta.3/media-buy/task-reference/refine_proposals) for commercial amendments or negotiated cancellation. Existing payloads remain supported throughout 3.x.
</Warning>
...
The coarse legacy actions (`update_budget`, `update_dates`, `update_packages`, [`sync_creatives`](/dist/docs/3.2.0-beta.3/creative/task-reference/sync_creatives)) cover the finer-grained vocabulary in a single value for sellers still emitting the 3.0 enum surface. Sellers SHOULD migrate to the finer set; the legacy values are removed in 4.0.
...
Buyer SDKs MUST branch on `mode` to decide whether to expect a synchronous response, conditional handling, or an asynchronous approval callback. Requotes are not modeled as an action mode in 3.1; sellers return [`REQUOTE_REQUIRED`](/dist/docs/3.2.0-beta.3/building/verification/compliance-catalog#error-code-requote-required) when a requested update exceeds the current quoted envelope.

**PATCH Semantics**: Only specified fields are updated; omitted fields remain unchanged.

**Request Schema**: [`/schemas/3.2.0-beta.3/media-buy/update-media-buy-request.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.3/media-buy/update-media-buy-request.json)
**Response Schema**: [`/schemas/3.2.0-beta.3/media-buy/update-media-buy-response.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.3/media-buy/update-media-buy-response.json)
...
|-----------|------|----------|-------------|
| `account` | [account-ref](/dist/docs/3.2.0-beta.3/building/by-layer/L2/accounts-and-agents#account-references) | Yes | Account that owns this media buy. Pass `{ "account_id": "..." }` or `{ "brand": {...}, "operator": "..." }`. Required for governance checks and account resolution. |
...
| `revision` | integer | No | Expected current revision for optimistic concurrency. Seller rejects with [`CONFLICT`](/dist/docs/3.2.0-beta.3/building/verification/compliance-catalog#error-code-conflict) on mismatch. Obtain from [`get_media_buys`](/dist/docs/3.2.0-beta.3/media-buy/task-reference/get_media_buys) or the most recent response. |
...
| `canceled` | boolean | No | Cancel the entire media buy (irreversible). Must be `true` when present. Seller may reject with [`NOT_CANCELLABLE`](/dist/docs/3.2.0-beta.3/building/verification/compliance-catalog#error-code-not-cancellable). |
...
| `invoice_recipient` | [BusinessEntity](/dist/docs/3.2.0-beta.3/building/by-layer/L2/accounts-and-agents#billing-entity-and-invoice-recipient) | No | Override who receives the invoice for this buy. The seller MUST validate authorization and include in [`check_governance`](/dist/docs/3.2.0-beta.3/governance/campaign/tasks/check_governance) when governance agents are configured. |
| `new_packages` | PackageRequest[] | No | New packages to add to this media buy. Same shape as [`create_media_buy`](/dist/docs/3.2.0-beta.3/media-buy/task-reference/create_media_buy) packages. When `budget_allocation` is omitted or fixed, each new package requires `budget` and cannot use `min_spend_target`. To add an uncapped package to an existing seller-optimized buy, include the resulting seller-optimized `budget_allocation` block so the allocation context is explicit. Only supported by sellers that advertise `add_packages` in `valid_actions`. |
...
For budget/pacing interaction and bidding inheritance, see [Budget & Pacing Controls](/dist/docs/3.2.0-beta.3/media-buy/task-reference/create_media_buy#budget--pacing-controls) and [BiddingPolicy Object](/dist/docs/3.2.0-beta.3/media-buy/task-reference/create_media_buy#biddingpolicy-object) on the `create_media_buy` reference.
...
| `media_buy_status` | Media buy lifecycle status after the update (present when status changes, e.g., cancellation). See [Migration › `media_buy_status`](/dist/docs/3.2.0-beta.3/reference/migration/media-buy-status). |
...
| `valid_actions` | Flat-vocabulary actions the buyer can perform after this update. Saves a round-trip to [`get_media_buys`](/dist/docs/3.2.0-beta.3/media-buy/task-reference/get_media_buys). Deprecated in favor of `available_actions[]` and removed in 4.0. |
...
**Note**: Responses use discriminated unions. Terminal errors never carry `warnings`; warnings occur only on successful updates. See [Indicators and Warnings](/dist/docs/3.2.0-beta.3/media-buy/media-buys/indicators).
...
The body-level `media_buy_status` is the canonical field for the buy's lifecycle state. The AdCP 3.2 source schema no longer permits the legacy top-level `status: MediaBuyStatus` form (e.g., `"status": "canceled"`) because it collides with the envelope task-status at the same root key. That legacy lifecycle field existed only during the 3.1 deprecation window; in 3.2, root `status` is TaskStatus only. See [Migration › `media_buy_status`](/dist/docs/3.2.0-beta.3/reference/migration/media-buy-status) for migration guidance.

**[`NOT_CANCELLABLE`](/dist/docs/3.2.0-beta.3/building/verification/compliance-catalog#error-code-not-cancellable) error response:**
```json
{
  "errors": [{
...
**[`INVALID_STATE`](/dist/docs/3.2.0-beta.3/building/verification/compliance-catalog#error-code-invalid-state) error response** (e.g., trying to update a completed media buy):
```json
{
  "errors": [{
    "code": "INVALID_STATE",
...
**[`REQUOTE_REQUIRED`](/dist/docs/3.2.0-beta.3/building/verification/compliance-catalog#error-code-requote-required) error response** (update changes the parameter envelope the quote was priced against):
```json
{
  "errors": [{
...
|------------|-------------|------------|
| [`MEDIA_BUY_NOT_FOUND`](/dist/docs/3.2.0-beta.3/building/verification/compliance-catalog#error-code-media-buy-not-found) | Media buy doesn't exist | Verify `media_buy_id`; for legacy correlation use [`get_media_buys`](/dist/docs/3.2.0-beta.3/media-buy/task-reference/get_media_buys) + `context.internal_campaign_id` |
| [`PACKAGE_NOT_FOUND`](/dist/docs/3.2.0-beta.3/building/verification/compliance-catalog#error-code-package-not-found) | Package doesn't exist | Verify `package_id`; for legacy package correlation use `get_media_buys` + package `context.buyer_ref` |
| [`ACTION_NOT_ALLOWED`](/dist/docs/3.2.0-beta.3/building/verification/compliance-catalog#error-code-action-not-allowed) | Field cannot be changed | See "What Can Be Updated" above |
| [`BUDGET_TOO_LOW`](/dist/docs/3.2.0-beta.3/building/verification/compliance-catalog#error-code-budget-too-low) | New budget below minimum | Increase budget amount |
| [`POLICY_VIOLATION`](/dist/docs/3.2.0-beta.3/building/verification/compliance-catalog#error-code-policy-violation) | Update violates content policy | Review policy requirements |
| [`INVALID_STATE`](/dist/docs/3.2.0-beta.3/building/verification/compliance-catalog#error-code-invalid-state) | Operation not allowed in current state (e.g., updating completed/canceled media buy) | Check campaign status via `get_media_buys` |
| [`NOT_CANCELLABLE`](/dist/docs/3.2.0-beta.3/building/verification/compliance-catalog#error-code-not-cancellable) | Media buy or package cannot be canceled | Check seller's cancellation policy or contact seller |
| [`CREATIVE_REJECTED`](/dist/docs/3.2.0-beta.3/building/verification/compliance-catalog#error-code-creative-rejected) | Creative failed content-policy, brand-safety, or accessibility review | Revise according to the applicable policy or validated accessibility criteria |
...
| [`CREATIVE_DEADLINE_EXCEEDED`](/dist/docs/3.2.0-beta.3/building/verification/compliance-catalog#error-code-creative-deadline-exceeded) | Creative change submitted past the package's `creative_deadline` | Check package `creative_deadline` before submitting creative changes |
| [`BUDGET_EXCEEDED`](/dist/docs/3.2.0-beta.3/building/verification/compliance-catalog#error-code-budget-exceeded) | Operation would exceed allocated budget | Reduce the amount or increase media buy total budget |
| [`CONFLICT`](/dist/docs/3.2.0-beta.3/building/verification/compliance-catalog#error-code-conflict) | Revision mismatch — another update was applied since you last read | Re-read via `get_media_buys` and retry with current `revision` |
...
See [Async Operations](/dist/docs/3.2.0-beta.3/building/by-layer/L3/async-operations) for protocol-specific async patterns and examples.
...
When a buyer's account has governance agents configured, sellers MUST call [`check_governance`](/dist/docs/3.2.0-beta.3/governance/campaign/tasks/check_governance) with `media_buy_id`, `planned_delivery`, and `phase: "modification"` before confirming an update. The governance agent validates change magnitude, budget reallocation, and new parameters against the campaign plan.

See the [seller integration guide](/dist/docs/3.2.0-beta.3/building/operating/seller-integration#execution-checks) for the full execution check workflow and code example.
</Note>
...
1. **Verify Changes**: Use [`get_media_buy_delivery`](/dist/docs/3.2.0-beta.3/media-buy/task-reference/get_media_buy_delivery) to confirm updates
2. **Supply New Creatives**: Use [`sync_creatives`](/dist/docs/3.2.0-beta.3/creative/task-reference/sync_creatives) for library-backed sellers, or `packages[].creatives` in this task for inline-only sellers
3. **Monitor Performance**: Track impact of changes on campaign metrics
4. **Optimize Further**: Use [`provide_performance_feedback`](/dist/docs/3.2.0-beta.3/media-buy/task-reference/provide_performance_feedback) for ongoing optimization

## Learn More

- [Media Buy Lifecycle](/dist/docs/3.2.0-beta.3/media-buy/media-buys/) - Complete campaign workflow
- [Targeting](/dist/docs/3.2.0-beta.3/media-buy/advanced-topics/targeting) - Targeting overlays and restrictions
- [Async Operations](/dist/docs/3.2.0-beta.3/building/by-layer/L3/async-operations) - Async patterns and status checking
- [create_media_buy](/dist/docs/3.2.0-beta.3/media-buy/task-reference/create_media_buy) - Initial campaign creation
