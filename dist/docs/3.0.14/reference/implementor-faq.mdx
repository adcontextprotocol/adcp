...
See [Policy Compliance](/dist/docs/3.0.14/media-buy/media-buys/policy-compliance) for complete implementation guidance.
...
Buyers can discover the agent's portfolio via [`get_adcp_capabilities`](/dist/docs/3.0.14/protocol/get_adcp_capabilities), which returns the publisher domains and primary channels the agent represents. This keeps responses lightweight while maintaining full validation capability.
...
See [Authorization Validation](/dist/docs/3.0.14/governance/property/adagents#buyer-agent-validation) for complete requirements.
...
**A:** The **[Trusted Match Protocol (TMP)](/dist/docs/3.0.14/trusted-match)** is AdCP's real-time execution layer. It determines which pre-negotiated packages activate at impression time using two structurally separated operations: **Context Match** (content relevance, no user identity) and **Identity Match** (user eligibility, no page context). The publisher joins both responses locally. TMP enables cross-publisher frequency capping, brand suitability, and audience targeting across web, mobile, CTV, AI assistants, and retail media.

You may also see references to **AXE** (Agentic eXecution Engine) — that was TMP's predecessor. Existing AXE integrations still work, but new implementations should use TMP. See [AXE documentation](/dist/docs/3.0.14/media-buy/advanced-topics/agentic-execution-engine) for legacy reference.

## Need More Help?

If your question isn't answered here:

1. Check the [Task Reference](/dist/docs/3.0.14/media-buy/task-reference/) for detailed API documentation
2. Review [Brief Expectations](/dist/docs/3.0.14/media-buy/product-discovery/brief-expectations) for discovery guidance
3. See [Media Products](/dist/docs/3.0.14/media-buy/product-discovery/media-products) for product model details
