...
Sync governance agent endpoints to specific accounts. The seller persists these agents and calls them via `check_governance` during media buy lifecycle events. Each account entry pairs an [account reference](/dist/docs/3.0.14/building/by-layer/L2/accounts-and-agents#account-references) with the governance agents for that account, supporting both explicit accounts (`account_id`) and implicit accounts (`brand` + `operator`).
...
**Request Schema**: [`/schemas/3.0.14/account/sync-governance-request.json`](https://adcontextprotocol.org/schemas/3.0.14/account/sync-governance-request.json)
**Response Schema**: [`/schemas/3.0.14/account/sync-governance-response.json`](https://adcontextprotocol.org/schemas/3.0.14/account/sync-governance-response.json)
...
|-------|------|----------|-------------|
| `account` | object | Yes | [Account reference](/dist/docs/3.0.14/building/by-layer/L2/accounts-and-agents#account-references): `{account_id}` for explicit accounts or `{brand, operator}` for implicit accounts. |
...
```json Request
{
  "$schema": "https://adcontextprotocol.org/schemas/3.0.14/account/sync-governance-request.json",
...
- [list_accounts](/dist/docs/3.0.14/accounts/tasks/list_accounts) — Discover accounts and their current governance agents
- [sync_accounts](/dist/docs/3.0.14/accounts/tasks/sync_accounts) — Provision or link advertiser accounts
- [check_governance](/dist/docs/3.0.14/governance/campaign/tasks/check_governance) — How sellers call governance agents during media buy events
- [Accounts and agents](/dist/docs/3.0.14/building/by-layer/L2/accounts-and-agents) — Account models, billing, and trust
