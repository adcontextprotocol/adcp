...
The agent-facing version of this content lives at [`skills/call-adcp-agent/SKILL.md`](https://github.com/adcontextprotocol/adcp/blob/main/skills/call-adcp-agent/SKILL.md) — bundled into the [protocol tarball](/dist/docs/3.0.16/building/by-layer/L0/schemas) so SDKs can ship it to coding agents.
...
2. **`get_adcp_capabilities`**: returns supported protocols, AdCP major versions, and feature flags. Tells you *which* tools this agent supports, not how to call them. See [`get_adcp_capabilities`](/dist/docs/3.0.16/protocol/get_adcp_capabilities).
...
- [Protocol architecture](/dist/docs/3.0.16/protocol/architecture) — how the protocol domains fit together.
- [Required tasks](/dist/docs/3.0.16/protocol/required-tasks) — which tasks an agent must implement to claim a specialism.
- [`get_adcp_capabilities`](/dist/docs/3.0.16/protocol/get_adcp_capabilities) — first call against any new agent.
- [Schemas and SDKs](/dist/docs/3.0.16/building/by-layer/L0/schemas) — how SDKs consume the protocol tarball (which now bundles `skills/`).
