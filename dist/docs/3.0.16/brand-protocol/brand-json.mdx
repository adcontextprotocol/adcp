...
| `relationship` | enum | No | How this brand relates to the property: `owned` (default), `direct`, `delegated`, `ad_network`. Matches `delegation_type` in adagents.json for bilateral verification. See [ad networks](/dist/docs/3.0.16/sponsored-intelligence/networks). |
...
This is the AdCP equivalent of `sellers.json` — the operator's public declaration of which publishers they work with. The publisher confirms by setting the matching `delegation_type` on the agent's authorization in their [adagents.json](/dist/docs/3.0.16/governance/property/adagents). See [ad networks](/dist/docs/3.0.16/sponsored-intelligence/networks) for the bilateral verification pattern.
