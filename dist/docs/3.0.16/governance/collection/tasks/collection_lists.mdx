...
Collection lists are managed, cacheable artifacts that express "these collections, filtered by these criteria." They parallel [property lists](/dist/docs/3.0.16/governance/property/tasks/property_lists) but operate on content programs (shows, series, podcasts) rather than technical surfaces (domains, apps).
...
**Content ratings are metadata filters, not content evaluation.** `content_ratings_exclude: [{ system: "tv_parental", rating: "TV-MA" }]` excludes all collections *declared* as TV-MA. It doesn't evaluate individual episodes — that's [content standards](/dist/docs/3.0.16/governance/content-standards/index).
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.0.16/collection/create-collection-list-request.json",
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.0.16/collection/create-collection-list-response.json",
  "list": {
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.0.16/collection/get-collection-list-request.json",
  "list_id": "cl_novamotors_dna_2026",
  "resolve": true,
  "pagination": {
    "max_results": 1000
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.0.16/collection/get-collection-list-response.json",
  "list": {
    "list_id": "cl_novamotors_dna_2026",
    "name": "Nova Motors CTV Do Not Air — 2026",
    "collection_count": 247
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.0.16/collection/update-collection-list-request.json",
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.0.16/collection/list-collection-lists-request.json",
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.0.16/collection/delete-collection-list-request.json",
...
Collection lists gate delivery decisions, so the `auth_token` and webhook callbacks need explicit lifecycle rules. General controls in [Security](/dist/docs/3.0.16/building/by-layer/L1/security) apply; the collection-list-specific rules:
...
**Webhook URL validation.** The `webhook_url` on `update_collection_list` is SSRF-equivalent to any other buyer-provided callback URL. Apply the canonical [Webhook URL validation (SSRF)](/dist/docs/3.0.16/building/by-layer/L1/security#webhook-url-validation-ssrf) rules — HTTPS only, validated IP ranges (IPv4 and IPv6 including `::ffff:0:0/96`), connection pinning (not just DNS re-resolution), no redirect following, size and timeout caps.

**Webhook signature algorithm.** The webhook signature MUST follow the [standard webhook signing rules](/dist/docs/3.0.16/building/by-layer/L1/security#webhook-security). By default, the RFC 9421 [webhook callbacks profile](/dist/docs/3.0.16/building/by-layer/L1/security#webhook-callbacks) applies: the governance agent signs with its `adcp_use: "webhook-signing"` key published at the `jwks_uri` of its `agents[]` entry in its own brand.json; the subscribing seller verifies covered components `@method`, `@target-uri`, `@authority`, `content-type`, `content-digest`, with `tag="adcp/webhook-signing/v1"`. The deprecated HMAC-SHA256 fallback applies only when the subscribing seller populates `authentication.credentials` on the webhook registration; that path follows the [Legacy HMAC-SHA256 fallback](/dist/docs/3.0.16/building/by-layer/L1/security#legacy-hmac-sha256-fallback-deprecated-removed-in-40) rules, and any body `signature` field under that path is a convenience copy — recipients MUST verify against the headers and MUST NOT trust the body value.
...
The pattern matches [property list sharing](/dist/docs/3.0.16/governance/property/index#sharing-property-lists-with-sellers):
