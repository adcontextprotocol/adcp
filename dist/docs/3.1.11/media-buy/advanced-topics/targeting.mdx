...
For targeting decisions that must happen at impression time, AdCP uses the **[Trusted Match Protocol (TMP)](/dist/docs/3.1.11/trusted-match)**. TMP is the real-time execution layer that evaluates pre-negotiated packages at serve time across any surface.
...
See the [TMP documentation](/dist/docs/3.1.11/trusted-match) for the full specification and surface-specific integration guides.
...
```json
{
  "$schema": "/schemas/3.1.11/core/targeting.json",
  "age_restriction": {
    "min": 21,
...
**Verification methods** (defined in [`age-verification-method.json`](https://adcontextprotocol.org/schemas/3.1.11/enums/age-verification-method.json), based on ISO/IEC 27566-1 age assurance standards):
...
**Available platforms** (defined in [`device-platform.json`](https://adcontextprotocol.org/schemas/3.1.11/enums/device-platform.json), based on Sec-CH-UA-Platform standard extended for CTV):
...
**Available types** (defined in [`device-type.json`](https://adcontextprotocol.org/schemas/3.1.11/enums/device-type.json)):
...
- **Note**: This field is from the legacy AXE integration. New implementations should use [TMP](/dist/docs/3.1.11/trusted-match), where audience targeting is handled through the Identity Match path.
...
- **Note**: This field is from the legacy AXE integration. New implementations should use [TMP](/dist/docs/3.1.11/trusted-match), where suppression is handled through the Identity Match path.
...
- **Format**: Array of `audience_id` strings from [`sync_audiences`](/dist/docs/3.1.11/media-buy/task-reference/sync_audiences)
...
Real-time signals are provided through the [AdCP Signals Protocol](/dist/docs/3.1.11/signals/overview), which allows orchestrators to supply targeting data at impression time.
...
- **[Trusted Match Protocol (TMP)](/dist/docs/3.1.11/trusted-match)** - Real-time execution layer for impression-time targeting, frequency capping, and brand suitability
- **[Signals Protocol](/dist/docs/3.1.11/signals/overview)** - Real-time targeting signals for brand suitability and contextual targeting
- **[Product Discovery](/dist/docs/3.1.11/media-buy/product-discovery/)** - How briefs lead to targeted product recommendations
- **[Example Briefs](/dist/docs/3.1.11/media-buy/product-discovery/example-briefs)** - Real examples of effective targeting briefs
- **[Policy Compliance](/dist/docs/3.1.11/media-buy/media-buys/policy-compliance)** - Automated compliance checking and enforcement
