...
- **Request**: [`get-brand-identity-request.json`](https://adcontextprotocol.org/schemas/3.1.11/brand/get-brand-identity-request.json)
- **Response**: [`get-brand-identity-response.json`](https://adcontextprotocol.org/schemas/3.1.11/brand/get-brand-identity-response.json)
...
Authorized callers (linked via [`sync_accounts`](/dist/docs/3.1.11/accounts/overview)) get deeper data on top of the public baseline.

Account linking is a one-time setup: a buyer agent calls [`sync_accounts`](/dist/docs/3.1.11/accounts/tasks/sync_accounts) on the brand agent, providing its brand reference. After that, the buyer's `get_brand_identity` requests are recognized as authorized.
...
<CardGroup cols={2}>
  <Card title="get_rights" icon="scale-balanced" href="/dist/docs/3.1.11/brand-protocol/tasks/get_rights">
