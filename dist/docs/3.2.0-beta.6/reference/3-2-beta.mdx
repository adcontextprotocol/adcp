...
The 3.2 beta cycle started with a protocol-first checkpoint. Beta.6 is the
current protocol checkpoint. The current TypeScript and Python SDK betas still
consume the exact signed beta.5 bundle:
...
| `3.2.0-beta.5` | Latest SDK-backed protocol checkpoint | Buyer and seller teams running end-to-end SDK integrations |
| `3.2.0-beta.6` | Current protocol checkpoint for reporting, CTV, canonical display, and request-signing integration | Raw-wire and code-generation adopters, SDK maintainers, and teams validating the new contracts directly |
...
SDK-backed TypeScript or Python testing, pair the packages above with protocol
beta.5. Test beta.6 from its signed artifact with raw-wire, code-generation, or
an SDK that explicitly names beta.6. Do not combine a newer wire pin with an SDK
generated from an older bundle unless the SDK maintainer documents forward
compatibility.
...
| Surface | beta.0 | beta.1 | beta.2 | beta.3 | beta.4 | beta.5 | beta.6 |
|---|---|---|---|---|---|---|---|
| Protocol artifact | `3.2.0-beta.0` | `3.2.0-beta.1` | `3.2.0-beta.2` | `3.2.0-beta.3` | `3.2.0-beta.4` | `3.2.0-beta.5` | `3.2.0-beta.6` |
| Git tag | `v3.2.0-beta.0` | `v3.2.0-beta.1` | `v3.2.0-beta.2` | `v3.2.0-beta.3` | `v3.2.0-beta.4` | `v3.2.0-beta.5` | `v3.2.0-beta.6` |
| `adcp_version` wire pin | `"3.2-beta.0"` | `"3.2-beta.1"` | `"3.2-beta.2"` | `"3.2-beta.3"` | `"3.2-beta.4"` | `"3.2-beta.5"` | `"3.2-beta.6"` |
| Documentation selector | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` |
...
buyer pinned to `"3.2-beta.6"` must not silently negotiate to another beta, and
a stable `"3.2"` pin must not silently negotiate to a beta.

## Use matched SDK betas

The TypeScript SDK has exact beta.5 support, not beta.6 support. The npm
...
See [Choose your SDK](/dist/docs/3.2.0-beta.6/building/by-layer/L4/choose-your-sdk) for the support
...
## Use the beta.6 artifact directly

Download the signed beta.6 protocol bundle rather
than copying schemas from `main` or from the moving `/schemas/3.2.0-beta.6/` path:

```bash
curl -fLO https://adcontextprotocol.org/protocol/3.2.0-beta.6.tgz
curl -fLO https://adcontextprotocol.org/protocol/3.2.0-beta.6.tgz.sha256
shasum -a 256 -c 3.2.0-beta.6.tgz.sha256
tar -xzf 3.2.0-beta.6.tgz
```

The extracted `adcp-3.2.0-beta.6/` directory contains:
...
[Verifying protocol tarballs](/dist/docs/3.2.0-beta.6/reference/verifying-protocol-tarballs) for
...
4. Exercise raw MCP or A2A calls with `adcp_version: "3.2-beta.6"` against a
...
<Note>
**No 3.2 verification badge is currently issued for beta.6.** Treat it as an
...
The SDK compatibility matrix lives in [Choose your SDK](/dist/docs/3.2.0-beta.6/building/by-layer/L4/choose-your-sdk).
The TypeScript and Python rows name exact beta.5 support, with different
higher-layer coverage. They do not imply beta.6 support. Treat generated 3.2
types and validators in every other SDK as unavailable until its row names an
exact package and protocol checkpoint. Raw JSON calls may still be possible,
...
## Current beta.6 checkpoint

Protocol beta.1 through beta.6 are published. TypeScript SDK beta.7 and Python
SDK beta.7 embed the exact beta.5 bundle. Before treating a test result as
beta.6 integration evidence, confirm that:

- the test uses the signed beta.6 artifact directly, or the support matrix names
  an exact SDK package that embeds beta.6;
- both peers advertise `"3.2-beta.6"` before calls use that pin;
- the integration scenarios below run against beta.6 and retain the exact
...
- Experimental surfaces retain their [experimental-status contract](/dist/docs/3.2.0-beta.6/reference/experimental-status).
...
- whether the problem blocks beta.6 interoperability.
...
- [What's New in AdCP 3.2](/dist/docs/3.2.0-beta.6/reference/whats-new-in-3-2)
- [Migrating from 3.1 to 3.2](/dist/docs/3.2.0-beta.6/reference/migration/3-1-to-3-2)
- [Release Notes](/dist/docs/3.2.0-beta.6/reference/release-notes#version-3-2-0)
- [Versions & Compatibility](/dist/docs/3.2.0-beta.6/reference/versions)
- [Choose your SDK](/dist/docs/3.2.0-beta.6/building/by-layer/L4/choose-your-sdk)
