...
`@adcp/sdk@14.0.0-beta.7` and Python `adcp==8.0.0b7` still embed beta.5, so use
the signed beta.6 artifact directly unless an SDK explicitly names beta.6.
...
For the feature narrative, start with [What's New in AdCP 3.2](/dist/docs/3.2.0-beta.6/reference/whats-new-in-3-2).
For prerelease artifacts and SDK timing, use the [3.2 beta program](/dist/docs/3.2.0-beta.6/reference/3-2-beta).
...
| 2 | SDK and codegen maintainers | Generate from the signed `3.2.0-beta.6` protocol tarball, not `main` or `/schemas/3.2.0-beta.6/`. |
...
| 8 | Compliance operators | Test beta.6 from its signed assets, or pair `@adcp/sdk@14.0.0-beta.7` or Python `adcp==8.0.0b7` with exact beta.5; require an explicit beta.6 support statement before using an SDK with the new checkpoint. |
...
until 4.0. See [Migrating to `media_buy_status`](/dist/docs/3.2.0-beta.6/reference/migration/media-buy-status)
...
webhook receivers. See [Signed requests](/dist/docs/3.2.0-beta.6/building/by-layer/L1/security#signed-requests-transport-layer).
...
Follow the complete field map in [Targeting-aware product discovery](/dist/docs/3.2.0-beta.6/reference/migration/targeting-aware-discovery).
...
automatically. See [Migrating secured asset access](/dist/docs/3.2.0-beta.6/reference/migration/asset-access).
...
See [Cross-role governance enforcement](/dist/docs/3.2.0-beta.6/reference/migration/cross-role-governance-enforcement).
...
- [Version Adaptation](/dist/docs/3.2.0-beta.6/building/cross-cutting/version-adaptation)
