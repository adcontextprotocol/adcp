...
<Warning>
`get_products` is deprecated for new integrations in AdCP 3.2, but remains fully supported throughout 3.x. Use [`list_products`](/dist/docs/3.2.0-beta.6/media-buy/task-reference/list_products), [`request_proposals`](/dist/docs/3.2.0-beta.6/media-buy/task-reference/request_proposals), [`refine_proposals`](/dist/docs/3.2.0-beta.6/media-buy/task-reference/refine_proposals), or [`decline_proposals`](/dist/docs/3.2.0-beta.6/media-buy/task-reference/decline_proposals). Its `idempotency_key` remains optional. Split-task retries keep the same tool name; they are not cross-name replays of this compatibility facade.
</Warning>

<Info>
**Why this shape.** Targeting, pricing, and curation are folded into one round-trip — the brief drives discovery, the publisher curates against it, and `pricing_options` carry firm prices the buyer commits against via `pricing_option_id`. We rejected a separate `get_price_quote` step between products and buy creation: it splits one expert decision into two underspecified ones and breaks the brief→curation contract. Iteration is `buying_mode: "refine"` with a typed change array — not a new task. → [Design principle: the brief drives discovery](/dist/docs/3.2.0-beta.6/protocol/design-principles#3-the-brief-drives-discovery-targeting-is-an-input-not-a-step).
</Info>
...
**Request Schema**: [`/schemas/3.2.0-beta.6/media-buy/get-products-request.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.6/media-buy/get-products-request.json)
**Response Schema**: [`/schemas/3.2.0-beta.6/media-buy/get-products-response.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.6/media-buy/get-products-response.json)
...
| `catalog` | [Catalog](/dist/docs/3.2.0-beta.6/creative/catalogs) | No | Catalog of items the buyer wants to promote. The seller matches catalog items against its inventory and returns products where matches exist. Requires `brand`. See [Catalog discovery](#catalog-discovery) below. |
...
| `targeting_overlay` | TargetingOverlay | No | Concrete delivery constraints the buyer expects to carry into [`create_media_buy`](/dist/docs/3.2.0-beta.6/media-buy/task-reference/create_media_buy). Prefer this over equivalent brief prose: it is compact, deterministic, and avoids lossy extraction. Returned products, pricing, and forecasts MUST account for the effective targeting. A product that cannot honor the request exactly is omitted or returned with sparse Product `targeting_resolution.modifications`; absence of Product resolution confirms exact acceptance of the structured overlay for that product. |
...
exceeds the limit fails atomically with [`UNSUPPORTED_FEATURE`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-unsupported-feature) and identifies
...
[`PRODUCT_UNAVAILABLE`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-product-unavailable)
...
1. Call [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.6/protocol/get_adcp_capabilities) on a property governance agent to discover available `property_features`
...
top-level discovery filter. See the [Property Governance overview](/dist/docs/3.2.0-beta.6/governance/property/index) for the full workflow.
</Note>
...
the request with [`INVALID_REQUEST`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-invalid-request). This rule applies to every deprecated
...
| `channels` | string[] | Filter by advertising channels (e.g., `["display", "ctv", "social", "streaming_audio"]`). See [Media Channel Taxonomy](/dist/docs/3.2.0-beta.6/reference/media-channel-taxonomy) |
...
```json
{
  "$schema": "/schemas/3.2.0-beta.6/media-buy/get-products-request.json",
  "buying_mode": "brief",
...
The `catalog` field uses the same [Catalog](/dist/docs/3.2.0-beta.6/creative/catalogs) object used throughout AdCP. You can reference a synced catalog by `catalog_id`, provide inline items, or use selectors to filter:
...
| `expires_at` | string | Expiration of this request-scoped configured product or quote. A recognized expired ID issued to the authenticated account and lineage returns [`PRODUCT_EXPIRED`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-product-expired); after the seller no longer retains its tombstone, or when the ID belongs to another account/lineage, [`PRODUCT_NOT_FOUND`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-product-not-found) applies. The error choice never reveals cross-tenant existence. Buyers rediscover in either case. |
...
| `measurement_readiness` | [MeasurementReadiness](/dist/docs/3.2.0-beta.6/media-buy/conversion-tracking/#measurement-readiness) | (Optional) Whether the buyer's event setup is sufficient for this product's optimization. Only present when the seller can evaluate the buyer's account context. |
| `measurement_terms` | [MeasurementTerms](/dist/docs/3.2.0-beta.6/media-buy/advanced-topics/pricing-models#measurement-terms-and-performance-standards) | (Optional) Seller's default billing measurement and makegood terms. Buyers may propose different terms at [`create_media_buy`](/dist/docs/3.2.0-beta.6/media-buy/task-reference/create_media_buy). |
| `performance_standards` | [PerformanceStandard[]](/dist/docs/3.2.0-beta.6/media-buy/advanced-topics/pricing-models#measurement-terms-and-performance-standards) | (Optional) Seller's default performance standards (viewability, IVT, completion rate, brand safety, attention score). Buyers may propose different standards at `create_media_buy`. |
| `cancellation_policy` | [CancellationPolicy](/dist/docs/3.2.0-beta.6/media-buy/advanced-topics/pricing-models#cancellation-policy) | (Optional) Cancellation notice period and penalties for guaranteed products. Buyers accept these terms by creating a media buy against the product. |
...
Publishers may return proposals alongside products - structured media plans with budget allocations. See [Proposals](/dist/docs/3.2.0-beta.6/media-buy/product-discovery/media-products#proposals) for details.
...
| `forecast` | DeliveryForecast | Aggregate delivery forecast for the proposal. Contains forecast points with metric ranges. See [Delivery Forecasts](/dist/docs/3.2.0-beta.6/media-buy/product-discovery/media-products#delivery-forecasts) |
...
This is the seller-side wholesale product feed returned by `get_products`. It is not a [`sync_catalogs`](/dist/docs/3.2.0-beta.6/media-buy/task-reference/sync_catalogs) feed; `sync_catalogs` manages buyer-provided campaign input feeds on the seller account.
...
```json
{
  "$schema": "/schemas/3.2.0-beta.6/media-buy/get-products-response.json",
  "status": "completed",
...
**See schema for complete field list**: [`get-products-response.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.6/media-buy/get-products-response.json)
...
See [Property Targeting](/dist/docs/3.2.0-beta.6/media-buy/product-discovery/media-products#property-targeting) for more details and [Property Governance](/dist/docs/3.2.0-beta.6/governance/property/specification) for more on property lists.
...
See the [Refinement guide](/dist/docs/3.2.0-beta.6/media-buy/product-discovery/refinement) for the full walkthrough: scope types, action semantics, seller responses, and common patterns. The parameter shape is defined in the [Refine array](#refine-array) section above.
...
- **`refine` is only valid in `refine` mode.** Requests that include this field in `brief` or `wholesale` mode are rejected with [`INVALID_REQUEST`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-invalid-request).
...
- **Proposals are ephemeral.** Proposals typically include an `expires_at` timestamp. After expiration, the seller returns [`PROPOSAL_EXPIRED`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-proposal-expired).
...
| Seller could not produce a usable answer because a dependency timed out or was unavailable | [`SERVICE_UNAVAILABLE`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-service-unavailable), normally with `recovery: "transient"` and optional `retry_after` |
...
| [`AUTH_INVALID`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-auth-invalid) | Credentials rejected (expired / revoked) | Human credential rotation required; do not auto-retry |
| [`INVALID_REQUEST`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-invalid-request) | Brief too long or malformed filters | Check request parameters |
| [`PRODUCT_NOT_FOUND`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-product-not-found) | One or more referenced product IDs are unknown | Remove invalid IDs and retry, or re-discover with a `brief` request |
| [`PRODUCT_EXPIRED`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-product-expired) | One or more recognized configured products passed `expires_at` | Re-discover with a new `brief` or `wholesale` request |
| [`PROPOSAL_EXPIRED`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-proposal-expired) | A referenced proposal ID has passed its `expires_at` timestamp | Re-discover with a new `brief` or `wholesale` request |
| [`PROPOSAL_NOT_FOUND`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-proposal-not-found) | The referenced `proposal_id` is unknown to the seller (never issued, wrong tenant, or evicted from cache) | Re-issue `get_products` in `brief` or `wholesale` mode to obtain a fresh draft; finalize that recognized proposal before creating the media buy |
| [`MULTI_FINALIZE_UNSUPPORTED`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-multi-finalize-unsupported) | `refine[]` carried multiple `action: 'finalize'` entries but the seller cannot guarantee atomic multi-proposal commit | Sequence single-proposal finalize calls — one finalize entry per `get_products` call |
| [`POLICY_VIOLATION`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-policy-violation) | The request itself violates an applicable policy and must be corrected | Review the structured policy details and revise the request. A seller making a deliberate business decision to decline an otherwise well-formed brief uses `status: "rejected"` instead. |
...
See [Authentication Guide](/dist/docs/3.2.0-beta.6/building/by-layer/L2/authentication) for details.

## Asynchronous Operations

Most product searches complete immediately, but some scenarios require asynchronous processing. When this happens, you'll receive a status other than `completed`. A `submitted` response with `task_id` is always pollable through [`get_task_status`](https://adcontextprotocol.org/schemas/3.2.0-beta.6/protocol/get-task-status-request.json) (legacy `tasks/get`); `push_notification_config` adds webhook notification for background workflows.
...
Activate the [AdCP A2A Profile Extension v3](/dist/docs/3.2.0-beta.6/building/by-layer/L0/a2a-profile-extension) on the request:
...
2. **Create Media Buy**: Use [`create_media_buy`](/dist/docs/3.2.0-beta.6/media-buy/task-reference/create_media_buy) to execute campaign
3. **Prepare Creatives**: Use each returned product's canonical `format_options[]` to see format requirements
4. **Supply Assets**: Use [`sync_creatives`](/dist/docs/3.2.0-beta.6/creative/task-reference/sync_creatives) for library-backed sellers, or inline `packages[].creatives` for inline-only sellers

## Learn More

- [Product Discovery Guide](/dist/docs/3.2.0-beta.6/media-buy/product-discovery/) - Understanding briefs and products
- [Pricing Models](/dist/docs/3.2.0-beta.6/media-buy/advanced-topics/pricing-models) - CPM, CPCV, CPP explained
- [Brief Expectations](/dist/docs/3.2.0-beta.6/media-buy/product-discovery/brief-expectations) - How to write effective briefs
- [Media Products](/dist/docs/3.2.0-beta.6/media-buy/product-discovery/media-products) - Product structure and fields
