...
Discover the **transformers** a creative agent offers for your account. A transformer is the creative analog of a media-buy product: an agent-offered, account-scoped, selectable unit of build capability (a voice, a model, a style, a director) with a typed configuration surface and per-account pricing. You discover transformers here, then select one with `transformer_id` in [`build_creative`](/dist/docs/3.2.0-beta.6/creative/task-reference/build_creative).
...
**Request Schema**: [`/schemas/3.2.0-beta.6/creative/list-transformers-request.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.6/creative/list-transformers-request.json)
**Response Schema**: [`/schemas/3.2.0-beta.6/creative/list-transformers-response.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.6/creative/list-transformers-response.json)

Offered only by agents that set `creative.supports_transformers: true` in [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.6/protocol/get_adcp_capabilities).

## Why transformers

The set of render knobs a creative agent exposes — and their legal values — is **account-specific and dynamic**. Your configured voices aren't a global enum or a list you hold; the *agent* knows them, and the set changes when you add one. So discovery flows agent → buyer, the same way [`get_products`](/dist/docs/3.2.0-beta.6/media-buy/task-reference/get_products) surfaces account-scoped inventory. `list_transformers` is that discovery surface for creative build capability.
...
| Field | Description |
|-------|-------------|
| `transformer_id` | Stable id; pass to [`build_creative`](/dist/docs/3.2.0-beta.6/creative/task-reference/build_creative) `transformer_id` |
...
See [Transformer schema](https://adcontextprotocol.org/schemas/3.2.0-beta.6/core/transformer.json) for the complete object.
...
Pass the chosen `transformer_id` and a typed `config` (keyed by each param's `field`) to [`build_creative`](/dist/docs/3.2.0-beta.6/creative/task-reference/build_creative):
...
|------|---------|----------|
| [`AUTH_MISSING`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-auth-missing) | `include_pricing` requested without a resolvable account | Supply `account` / authenticate |
| [`INVALID_REQUEST`](/dist/docs/3.2.0-beta.6/building/verification/compliance-catalog#error-code-invalid-request) | Malformed filter or pagination cursor | Correct the request |

Unknown `expand_params` (a `field` no transformer exposes) are ignored, not an error — the param simply returns no `options`.

## Learn More

- [build_creative](/dist/docs/3.2.0-beta.6/creative/task-reference/build_creative) — select a transformer and produce creatives (incl. variants)
- [get_adcp_capabilities](/dist/docs/3.2.0-beta.6/protocol/get_adcp_capabilities) — `creative.supports_transformers` discriminator
- [Vendor pricing](https://adcontextprotocol.org/schemas/3.2.0-beta.6/core/vendor-pricing-option.json) — the `per_unit` rate model
