...
AdCP errors are **application-layer** errors. They belong in the tool/task response, not in the transport error channel. This page defines how the [`error.json`](https://adcontextprotocol.org/schemas/3.1.6/core/error.json) schema maps to MCP and A2A response envelopes.

For the error schema itself, standard codes, and recovery strategies, see [Error Handling](/dist/docs/3.1.6/building/by-layer/L3/error-handling).
...
This follows the [A2A Response Format](/dist/docs/3.1.6/building/by-layer/L0/a2a-response-format) conventions: final states use `.artifacts` for data.
...
AdCP exposes errors in two distinct places. This page covers the **transport envelope** (`adcp_error`); the **payload errors array** (`errors[]`) is covered in [Error Handling — Envelope vs. payload errors](/dist/docs/3.1.6/building/by-layer/L3/error-handling#envelope-vs-payload-errors-the-two-layer-model).
...
- Storyboard authors SHOULD NOT pin assertions to a specific path (e.g., `check: field_present, path: "errors"`) — that couples the test to one layer and fails against agents that surface errors on the other. See [Storyboard authoring — Asserting on errors](/dist/docs/3.1.6/contributing/storyboard-authoring#asserting-on-errors).
...
- [`/schemas/3.1.6/error-details/rate-limited.json`](https://adcontextprotocol.org/schemas/3.1.6/error-details/rate-limited.json)
- [`/schemas/3.1.6/error-details/budget-too-low.json`](https://adcontextprotocol.org/schemas/3.1.6/error-details/budget-too-low.json)
- [`/schemas/3.1.6/error-details/audience-too-small.json`](https://adcontextprotocol.org/schemas/3.1.6/error-details/audience-too-small.json)
- [`/schemas/3.1.6/error-details/account-setup-required.json`](https://adcontextprotocol.org/schemas/3.1.6/error-details/account-setup-required.json)
- [`/schemas/3.1.6/error-details/creative-rejected.json`](https://adcontextprotocol.org/schemas/3.1.6/error-details/creative-rejected.json)
- [`/schemas/3.1.6/error-details/policy-violation.json`](https://adcontextprotocol.org/schemas/3.1.6/error-details/policy-violation.json)
- [`/schemas/3.1.6/error-details/conflict.json`](https://adcontextprotocol.org/schemas/3.1.6/error-details/conflict.json)
...
Sellers MAY use error codes not in the [standard vocabulary](https://adcontextprotocol.org/schemas/3.1.6/enums/error-code.json). To distinguish seller-specific codes from standard codes and avoid collisions between sellers:
...
- Sellers SHOULD register their vendor prefix and codes in the [vendor error code registry](https://adcontextprotocol.org/schemas/3.1.6/error-details/vendor-error-codes.json) by submitting a PR
...
- [Error Handling](/dist/docs/3.1.6/building/by-layer/L3/error-handling) — error schema, standard codes, recovery strategies
- [MCP Guide](/dist/docs/3.1.6/building/by-layer/L0/mcp-guide) — MCP transport integration
- [A2A Guide](/dist/docs/3.1.6/building/by-layer/L0/a2a-guide) — A2A transport integration
- [A2A Response Format](/dist/docs/3.1.6/building/by-layer/L0/a2a-response-format) — canonical A2A response structure
