...
If your agent does not wrap an upstream — for example, a pure decisioning service that owns its own data — the storyboard runner alone is sufficient. See [Validate your agent](/dist/docs/3.1.6/building/verification/validate-your-agent).
...
- **Mock seed data may not match storyboard fixture inputs.** If you see 404s on `_lookup/<resource>`, the storyboard's payloads may reference IDs the mock doesn't seed. Either widen the mock's seed or seed scenario state at runtime via the [compliance test controller](/dist/docs/3.1.6/building/by-layer/L3/comply-test-controller).
...
- **[Validate your agent](/dist/docs/3.1.6/building/verification/validate-your-agent)** — the broader storyboard-runner-driven validation checklist (fuzz, multi-instance, request-signing, webhook conformance).
- **[Compliance test controller](/dist/docs/3.1.6/building/by-layer/L3/comply-test-controller)** — seed scenario state at runtime when storyboards need fixtures the mock doesn't provide.
- **[Build an agent](/dist/docs/3.1.6/building/by-layer/L4/build-an-agent)** — the language-agnostic guide to building an AdCP agent.
- **[Compliance catalog](/dist/docs/3.1.6/building/verification/compliance-catalog)** — the full taxonomy of universal / protocol / specialism storyboards.
