...
|---|---|---|
| One-off verification (single page, single creative, single subsidiary check) | [`verify_brand_claim`](/dist/docs/3.2.0-rc.6/brand-protocol/tasks/verify_brand_claim) | No batching benefit; simpler error model. |
...
- **Request**: [`verify-brand-claims-request.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.6/brand/verify-brand-claims-request.json)
- **Response**: [`verify-brand-claims-response.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.6/brand/verify-brand-claims-response.json)
...
Per-claim failures (an [`UNSUPPORTED_FEATURE`](/dist/docs/3.2.0-rc.6/building/verification/compliance-catalog#error-code-unsupported-feature) for one of many claims, a [`VALIDATION_ERROR`](/dist/docs/3.2.0-rc.6/building/verification/compliance-catalog#error-code-validation-error) for an ambiguous trademark query in a portfolio of properties) DO NOT fail the batch. The corresponding entry in `results[]` carries an `error` field instead of `claim_type`/`status`, and the rest of the batch is unaffected:
...
|---|---|---|
| Per-result | `results[i].error` | `UNSUPPORTED_FEATURE`, [`INVALID_REQUEST`](/dist/docs/3.2.0-rc.6/building/verification/compliance-catalog#error-code-invalid-request) for malformed input in one item, `VALIDATION_ERROR` for an ambiguous match |
| Batch-level | top-level `errors[]` | [`AUTH_INVALID`](/dist/docs/3.2.0-rc.6/building/verification/compliance-catalog#error-code-auth-invalid), [`RATE_LIMITED`](/dist/docs/3.2.0-rc.6/building/verification/compliance-catalog#error-code-rate-limited), malformed request, `claims[]` over the cap |

Full error-code semantics are documented on the [single-target task page § Error handling](/dist/docs/3.2.0-rc.6/brand-protocol/tasks/verify_brand_claim#error-handling).
...
- A [`RATE_LIMITED`](/dist/docs/3.2.0-rc.6/building/verification/compliance-catalog#error-code-rate-limited) response on a bulk call is a batch-level error; the whole batch is rejected. Retry with `Retry-After` honoured.
...
See [`brand.json` § Agent-augmented verification](/dist/docs/3.2.0-rc.6/brand-protocol/brand-json#agent-augmented-verification) for the full trust table.
