...
This page is the canonical sequence reference for media buy lifecycle. For conceptual background on the full lifecycle — campaign structure, package model, property targeting, and async operations — see [Media Buy Lifecycle](/dist/docs/3.2.0-rc.6/media-buy/media-buys/).
...
2. **Form the commercial commitment** — purchase a versioned published offer
   ...
   `accept_proposal`. Either request may set the buyer's MediaBuy `name`; the
   completed commitment echoes it outside `accepted_proposal` and its digest.
...
When `error.details.decline_reason` is present, use the coarse seller-policy
dimension to inform recovery. The reason may select a typed refinement, but
some policies still require the buyer-safe message, a free-text `ask`, or human
negotiation. Begin the amendment from the snapshot's `accepted_proposal_id`,
then revise, finalize, and accept the successor proposal. Sellers retain
disclosure control and may omit `decline_reason` when even the coarse
classification would expose private thresholds or enforcement controls.
...
proposal being accepted. The MediaBuy `name` is not lossy: both compact
commitment requests accept the buyer's display label and return the persisted
value outside the accepted commercial terms.
...
state is then read through `get_media_buys`. See [task status and MediaBuy state](/dist/docs/3.2.0-rc.6/media-buy/media-buys#state-machine).
</Note>
...
A display name may be set on `buy_products` or `accept_proposal` when the
MediaBuy is committed. A later display-name-only change routes to
`control_media_buy`. `name` is revision-checked non-commercial metadata:
changing it increments the MediaBuy revision without changing the accepted
terms or creating an accepted-proposal successor.
...
signal. See [Accountability](/dist/docs/3.2.0-rc.6/media-buy/advanced-topics/accountability) for
...
Buyer agents triaging a non-empty `impairments[]` SHOULD sort entries by `observed_at` ascending — the oldest open impairment is the most likely to have already eaten into delivery. Webhook arrival time is not a reliable proxy: under the coalescence rule (see [webhooks § Coalescence](/dist/docs/3.2.0-rc.6/building/by-layer/L3/webhooks#coalescence)) the seller MAY batch multiple state changes into one fire, and re-emission under a fresh `idempotency_key` resets the transport timestamp without changing `observed_at`.
...
**Snapshot is one of several propagation surfaces.** Sellers declare which surfaces they use via [`capabilities.media_buy.propagation_surfaces`](https://adcontextprotocol.org/schemas/3.2.0-rc.6/protocol/get-adcp-capabilities-response.json) on `get_adcp_capabilities` — a non-exclusive array, so a seller mirroring impairments on both the buy snapshot AND firing webhooks declares `["snapshot", "webhook"]` (the common case for premium guaranteed sellers). The three surface values:
...
See the [Snapshot and log contract](/dist/docs/3.2.0-rc.6/protocol/snapshot-and-log) for the read-side rules that tie `impairments[]` (snapshot) to the `impairment` push (log).
...
- [Media Buy Lifecycle](/dist/docs/3.2.0-rc.6/media-buy/media-buys/) — full lifecycle reference: campaign structure, package model, async operations
- [`buy_products`](/dist/docs/3.2.0-rc.6/media-buy/task-reference/buy_products) — purchase versioned published offers directly
- [`accept_proposal`](/dist/docs/3.2.0-rc.6/media-buy/task-reference/accept_proposal) — accept committed new-buy, amendment, or cancellation terms
- [`control_media_buy`](/dist/docs/3.2.0-rc.6/media-buy/task-reference/control_media_buy) — apply revision-checked operational controls
- [`sync_creatives`](/dist/docs/3.2.0-rc.6/creative/task-reference/sync_creatives) — upload and update library creatives when the seller advertises `creative.has_creative_library: true`
- [Accountability](/dist/docs/3.2.0-rc.6/media-buy/advanced-topics/accountability) — performance standards, measurement terms, makegood resolution
- [Optimization & Reporting](/dist/docs/3.2.0-rc.6/media-buy/media-buys/optimization-reporting) — delivery monitoring, dimensional reporting, campaign updates
