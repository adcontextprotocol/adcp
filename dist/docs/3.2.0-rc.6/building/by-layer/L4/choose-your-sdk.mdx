...
For the layered model behind this — what each layer contains and what an SDK at each layer should provide — see the [SDK stack reference](/dist/docs/3.2.0-rc.6/building/cross-cutting/sdk-stack).

## Coverage matrix

What "shipped" means at each layer is the [L0–L3 checklist](/dist/docs/3.2.0-rc.6/building/cross-cutting/sdk-stack#what-an-sdk-at-each-layer-should-provide).
...
workflows can also use RC.3's [signed bundle](/dist/docs/3.2.0-rc.6/reference/3-2-beta#use-the-rc-3-artifact-directly).
This is not a GA signal: production integrations should remain on 3.1 until
3.2 is released. See the [3.2 prerelease program](/dist/docs/3.2.0-rc.6/reference/3-2-beta).
</Warning>
...
For the status of every published protocol version, see [Versions & Compatibility](/dist/docs/3.2.0-rc.6/reference/versions). Migration and archived-version guidance lives under [Release notes & migration](/dist/docs/3.2.0-rc.6/reference/migration/index).
...
community-maintained ports are welcome — see the [Builders Working Group](/dist/docs/3.2.0-rc.6/community/working-group)
and the [Slack community](/dist/docs/3.2.0-rc.6/community/joining-slack).
...
- `@adcp/sdk/testing` — buyer-side storyboard runner (`runStoryboard`, `comply`, `testAgent`) and seller-side controller scaffold (`createComplyController` — see [Get Test-Ready](/dist/docs/3.2.0-rc.6/building/verification/get-test-ready))
...
- **[Build an agent](/dist/docs/3.2.0-rc.6/building/by-layer/L4/build-an-agent)** — server-side L4 path. Skill files + coding agent.
- **[Build a caller](/dist/docs/3.2.0-rc.6/building/by-layer/L4/build-a-caller)** — client-side L4 path. Install, call, handle responses, ingest reporting.
- **[Schemas](/dist/docs/3.2.0-rc.6/building/by-layer/L0/schemas)** — schema bundle, type generation, version pinning.
- **[Migrate from hand-rolled](/dist/docs/3.2.0-rc.6/building/by-layer/L4/migrate-from-hand-rolled)** — already running an AdCP agent built before the SDKs covered much? Swap one layer at a time.
