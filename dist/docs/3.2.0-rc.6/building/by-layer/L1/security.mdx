...
<Note>
**Looking for the *why*?** This page is the normative implementation reference — the rules a compliant agent follows. For the threat model, the layered defense narrative, and a checklist for brand IT and CISOs, see the [Security Model](/dist/docs/3.2.0-rc.6/building/concepts/security-model).
</Note>
...
Buyers who need to interoperate with receivers that have not yet adopted the 9421 profile MAY opt in by populating `push_notification_config.authentication.credentials`, `accounts[].notification_configs[].authentication.credentials`, `sync_principal.configuration.notification_configs[].authentication.credentials`, or `sync_agent_notification_configs.notification_configs[].authentication.credentials`. When `authentication` is present on the buyer's request, the seller signs with HMAC-SHA256 using the semantics defined in [Push Notifications](/dist/docs/3.2.0-rc.6/building/by-layer/L3/webhooks#legacy-hmac-sha256-fallback). The legacy scheme is a 3.x-only compatibility affordance; sellers MAY decline to support it, and it is removed in AdCP 4.0.
...
- [Offline reporting buckets](/dist/docs/3.2.0-rc.6/media-buy/media-buys/optimization-reporting#security-considerations-for-offline-delivery) — IAM-layer prefix scoping, credential revocation on account status change.
- [Collection lists](/dist/docs/3.2.0-rc.6/governance/collection/tasks/collection_lists#security-considerations) — `auth_token` scope and revocation, distribution-ID validation, webhook signature normative rules.
- [Managed networks `authoritative_location`](/dist/docs/3.2.0-rc.6/governance/property/managed-networks#security-considerations) — validator fetch semantics, change detection, relationship termination.
- [TMP provider registration](/dist/docs/3.2.0-rc.6/trusted-match/specification#provider-registration-security) — dynamic registration authentication, router-to-provider auth, `/health` info-leakage rules.
...
The shared schemas are [`attestation-reference.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.6/core/attestation-reference.json), [`attestation-issuer.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.6/core/attestation-issuer.json), [`attestation-subject.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.6/core/attestation-subject.json), and [`attestation-evaluation.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.6/core/attestation-evaluation.json).
...
The presenter MAY nominate `verify_agent.agent_url`, but it MUST match `accepted_verifiers[]` after [URL canonicalization](/dist/docs/3.2.0-rc.6/reference/url-canonicalization). The evaluator MAY use that verifier, another accepted verifier, or local verification. It MUST NOT call an off-list presenter-supplied endpoint.
...
Rights constraints compose the shared portable-attestation primitive; they do not define a rights-only JWS or trust store. The buyer carries `rights_constraint.attestation_refs[]`, the serving seller remains verifier-of-record, and only the seller produces [`RightsAttestationEvaluation`](https://adcontextprotocol.org/schemas/3.2.0-rc.6/core/rights-attestation-evaluation.json) readback. A buyer-authored evaluation or the mere presence of a reference has no authorization value.
...
Every piece of state — media buys, creatives, idempotency cache entries, session IDs, governance tokens — is scoped to the [account](/dist/docs/3.2.0-rc.6/reference/glossary#a) that owns it. Cross-account reads MUST return a generic "not found" rather than leak existence. The authenticated [agent](/dist/docs/3.2.0-rc.6/reference/glossary#a) is how the seller knows *who is calling*; the `account` on the request is *what billing relationship the call is acting on*. Isolation requires both checks.
...
See [Accounts & Security — Data Isolation](/dist/docs/3.2.0-rc.6/media-buy/advanced-topics/accounts-and-security#data-isolation) for the billing-relationship model these rules enforce, and the glossary for the formal definitions of [Account](/dist/docs/3.2.0-rc.6/reference/glossary#a) and [Agent](/dist/docs/3.2.0-rc.6/reference/glossary#a).
...
1. **Schema validation runs first.** Sellers MUST validate the request against its schema (including presence and format of `idempotency_key` when declared) BEFORE consulting the idempotency cache. A malformed request returns `INVALID_REQUEST` without ever touching the cache — otherwise cache misses become a timing side channel that leaks whether schema validation accepted the key format. Validation errors are never cached (per rule 3).
2. **First call is canonical.** On **task success** (`status: completed` or `status: submitted` for async operations), the seller stores the inner response payload (not the protocol envelope) keyed by `(authenticated_agent, account_id, idempotency_key)` along with a hash of the canonical request payload. **The cache entry is immutable** — replays within the TTL MUST return the originally-cached payload (with `replayed: true`), and state-tracking fields in that payload MUST NOT be refreshed to reflect the resource's current state. This rule applies across both success branches:
    ...
    **Resource deletion does not end the replay window.** The idempotency key record, canonical request hash, and cached response MUST remain available independently of every resource the request created for the full `replay_ttl_seconds`, measured from the successful mutation's durable commit time (`committed_at`). Eviction MUST occur no earlier than `committed_at + replay_ttl_seconds`, even when an operator or later task independently deletes or purges that resource. A `UNIQUE` constraint on an `idempotency_key` column of the resource row alone does not satisfy this requirement: deleting the row would free the key and permit a retry to create a second resource. A replay after resource deletion still returns the historical cached response under rule 4; the buyer then uses the appropriate read task to observe that the resource is gone.
3. **Only unsuccessful operations release their keys.** On any error before a mutation commits — validation, governance denial, transport failure, internal error — the key is **not** stored. A retry re-executes. This matches buyer intent: a retry after a 5xx should try again, not replay a failure. It also prevents a buyer's malformed request from being locked into a key for its full TTL.

    `COMMITTED_RESOURCE_PURGED` is the narrow exception to the ordinary error-response rule because it reports a successful write followed by an independent deletion, not an unsuccessful operation or a requested delete operation. When a mutation commits but its affected resource is independently deleted or purged before the seller can durably record the canonical success response, the seller MUST retain a committed-outcome tombstone and canonical request hash until at least `committed_at + replay_ttl_seconds`, return `COMMITTED_RESOURCE_PURGED` with `recovery: terminal`, and return the same terminal outcome with `replayed: true` on equivalent replays without re-executing the mutation. If the canonical success response was already durably recorded, rule 4 takes precedence and the seller returns that historical response even though a current-state read shows the resource is gone. The tombstone MUST be stored independently of the deleted resource. Buyers MUST NOT automatically retry that logical operation with either the same or a fresh key; they MUST first reconcile through the resource read task and seller audit or operator records, and may initiate a new operation only when reconciliation confirms that is intended. Sellers MUST NOT use `CONFLICT` for this case: its transient recovery would instruct the buyer to retry a write that already committed.
4. **Replay returns the cached response.** A subsequent request with the same `idempotency_key` AND an equivalent canonical-form payload (see "Payload equivalence" below) MUST return the stored inner response without re-executing side effects. The seller injects `replayed: true` onto the outgoing protocol envelope at response time — including when replaying a committed-outcome tombstone — and `replayed` is NOT part of the stored inner response or tombstone. Injection at replay time keeps the stored payload byte-stable across replays regardless of envelope changes (new `timestamp`, rotated `governance_context`, etc.). Transport-specific note for MCP: MCP tool responses do not have a separate envelope slot; servers MAY expose `replayed` inside the tool result object itself (e.g., at the top of the structured return) or via a response metadata field. REST and A2A responses use the envelope field directly.
...
6. **Expired keys are rejected explicitly.** After `committed_at + replay_ttl_seconds` the seller MAY evict a completed cache entry or committed-outcome tombstone. An unresolved in-flight or reconciliation-required claim is not a completed entry and MUST NOT be evicted merely because this interval elapsed. A request arriving after eviction with a key the seller has seen SHOULD be rejected with `IDEMPOTENCY_EXPIRED` rather than silently treated as new — silent re-execution is exactly the double-booking footgun the key was meant to prevent. Sellers SHOULD allow a ±60s clock-skew window at the TTL boundary (the same tolerance applied to JWS `exp` elsewhere in this document) so that a retry arriving seconds after nominal expiry is still replayed from cache rather than treated as fresh.
...
8. **Cache-growth defense.** Sellers MUST apply per-`(authenticated_agent, account)` rate limits on idempotency cache inserts separately from request rate limits, and MUST return `RATE_LIMITED` (see [error taxonomy](/dist/docs/3.2.0-rc.6/building/by-layer/L3/error-handling#rate-limit-handling)) when the per-agent insert rate exceeds the configured ceiling rather than let the cache grow unbounded. A buyer submitting N fresh keys per second on a cheap success-path operation (e.g., `log_event`) would otherwise force unbounded storage, with amplification proportional to `replay_ttl_seconds` at the 3600 s floor. The natural bound is `inserts_per_hour × replay_ttl_hours ≤ max_cache_rows_per_agent`.
...
9. **Concurrent retries — first-insert-wins.** A second request carrying the same `(authenticated_agent, account_id, idempotency_key)` MAY arrive while the first request is still executing — most commonly when the buyer's transport timeout fires before the seller's downstream call returns, and the buyer retries. Sellers MUST resolve the race deterministically; they MUST NOT execute the side effect twice and MUST NOT silently drop the second request. Resolution is a `(unique constraint, INSERT … ON CONFLICT DO NOTHING)` pattern on the scope tuple: the first row to land owns execution and stores the canonical payload hash on the in-flight row (NOT a sentinel); subsequent requests observe an existing row whose response slot is not yet populated but whose payload hash IS populated.
    ...
    Per rule 3, the seller may release the `(in_flight)` claim only after it has definitive evidence that no local or downstream mutation committed. Schema rejection and errors raised before any side-effecting call are definitive pre-commit failures. A downstream timeout, handler timeout, lost response, process crash, or any other unknown outcome is not proof of failure: the seller MUST retain the durable claim, transition it from active execution to a reconciliation-required state, and MUST NOT invoke the mutation again until reconciliation establishes the outcome. Equivalent retries MAY wait, receive `IDEMPOTENCY_IN_FLIGHT`, or drive the rule-10 reconciliation path; a different canonical hash still returns `IDEMPOTENCY_CONFLICT`. When reconciliation proves non-commit, the seller releases the claim; when it proves commit, the seller records the canonical success response or, after an independent purge, the `COMMITTED_RESOURCE_PURGED` tombstone. Whenever the resource write and idempotency ledger share a transactional boundary, the seller MUST atomically commit the mutation and completed ledger state. Implementations that cannot make that transition atomic MUST use the same fail-closed reconciliation discipline.

    Sellers MUST bound the active execution lease to the declared per-task handler timeout so a hung worker can be stopped or replaced. Lease expiry changes the claim to reconciliation-required; it never releases or evicts the claim. Sellers using reject-and-redirect MUST set top-level `error.retry_after` to a value no greater than `replay_ttl_seconds` (declared in `capabilities.idempotency`). Sellers SHOULD also declare `capabilities.idempotency.in_flight_max_seconds` — the maximum active execution lease before reconciliation takes over, scoped to the seller's per-task handler timeout. Buyers SHOULD use that declared value as the primary bound for an individual retry wait when present; when absent, fall back to the order-of-magnitude heuristic (a value derived from the seller's typical handler latency, an order of magnitude below the replay TTL, never the TTL ceiling itself). This value is not permission for a buyer to mint a fresh key or for a seller to release an unresolved claim.
...
10. **Crossing service boundaries — downstream reconciliation.** Sellers commonly invoke downstream systems during request handling — SSP/ad-server calls on `create_media_buy`, payment-provider calls on billing operations, governance-agent calls on `check_governance`. These calls have their own failure modes that can leave the seller in a "downstream unknown" state: the network connection dropped after the downstream accepted the request but before its response arrived; the seller process crashed mid-call; a region failover swapped the worker before the response was persisted. Rule 3 (only proven pre-commit failures release their claims; completed outcomes become cached successes or committed-outcome tombstones) is necessary but not sufficient: a seller that releases an ambiguous claim and re-executes on retry will double-invoke the downstream and create duplicate side effects there.
...
| `plan_hash` | Yes | Audit-layer binding of the attestation to the evaluated plan state. Not part of the seller verification checklist — sellers treat it as opaque cargo. Semantics, canonicalization, and verification paths are defined in [Plan binding and audit](/dist/docs/3.2.0-rc.6/governance/campaign/specification#plan-binding-and-audit). |
...
**`plan_hash` is audit-layer, not wire-layer**: the `plan_hash` claim is cryptographic cargo the token carries for off-wire verification by the governance agent, auditors, and buyer-side compliance. It is not part of this profile's seller verification contract and is never listed in `crit`. Canonicalization, excluded fields, retention rules, and test vectors are specified in [Plan binding and audit](/dist/docs/3.2.0-rc.6/governance/campaign/specification#plan-binding-and-audit) (governance spec). Sellers persist and forward `governance_context` verbatim and perform the 15-step verification checklist below — authenticity, authorization scope, freshness — without inspecting `plan_hash`.
...
- **3.0**: governance agents MUST emit compact JWS per this profile, including the required `plan_hash` audit-layer claim (see [Plan binding and audit](/dist/docs/3.2.0-rc.6/governance/campaign/specification#plan-binding-and-audit) for semantics). Sellers MAY verify the 15-step checklist; sellers that do not verify MUST persist and forward the token unchanged. Values that are not JWS are deprecated and SHOULD only appear from pre-3.0 governance agents during the transition; governance agents that emit non-JWS values in 3.0 MUST declare this in their capabilities so sellers can detect unverifiable deployments.
...
**MCP transport sessions are non-authoritative.** `Mcp-Session-Id` is not a
covered component in this profile and MUST NOT carry AdCP-relevant authority.
A verifier MUST NOT attribute identity, authorization, account selection,
resource access, or task ownership on the basis of `Mcp-Session-Id` or other
unsigned session-continuity or routing metadata. This prohibition does not
include recognized per-request credential channels such as `Authorization`.
For an operation that requires authentication or authorization, the decision
MUST derive from those per-request credentials and, when request signing
applies, the covered components and digest-bound body. Public and bootstrap
operations retain their documented authentication policy. A stateful MCP
implementation MAY use a session for transport continuity or task waiting.
It MAY also use session-local caches only after per-request authentication and
only when every entry is additionally keyed by the authenticated principal and
account. Rebinding an otherwise valid request to another session MUST NOT
change the principal, authorization decision, resource namespace, returned
cached data, or ownership decision applied to that request.
...
5. Canonicalize `A` and every candidate `agents[].url` with the shared [AdCP URL canonicalization algorithm](/dist/docs/3.2.0-rc.6/reference/url-canonicalization), then find entries whose canonical URL equals canonical `A`. If none matches, reject with `request_signature_agent_not_in_brand_json`. If multiple entries match after canonicalization, reject with `request_signature_brand_json_ambiguous`; never select the first. Retain the matched canonical URL as the verified Agent identity if the remaining key checks succeed. A trailing slash remains significant under the shared algorithm, while scheme/host case, default ports, dot segments, and percent-encoded unreserved characters normalize as specified there.
...
5. **Find the `agents[]` entry.** Canonicalize `A` and every `agents[].url` with the shared [AdCP URL canonicalization algorithm](/dist/docs/3.2.0-rc.6/reference/url-canonicalization). Reject `request_signature_agent_not_in_brand_json` on no canonical match and `request_signature_brand_json_ambiguous` on multiple canonical matches. Retain the one matched canonical URL as the Agent identity after signature verification succeeds.
...
The machine-readable authority is [`request-signing-error-code.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.6/enums/request-signing-error-code.json). SDKs MUST consume that schema's `enumMetadata` recovery classifications instead of inferring behavior from a code's spelling or parsing this prose table. These lowercase transport-profile codes are deliberately separate from the uppercase task error vocabulary in `error-code.json` because they are carried in `WWW-Authenticate`, not task `errors[]` arrays.
...
**HMAC→9421 migration.** A buyer transitioning from HMAC to 9421 MUST disable its HMAC verifier once the seller has acknowledged the cutover. Running both verifiers concurrently leaves the HMAC path exploitable for the original 5-minute replay window plus however long the buyer forgets to turn it off; "just in case" operational posture keeps the deprecated path live past the intended deprecation. Sellers SHOULD reject `authentication` blocks from a counterparty that has previously been migrated to 9421, logging the rejection. During the cutover window, buyers MAY run both verifiers but SHOULD maintain a single dedup keyspace so that the same logical event under either scheme maps to the same `(sender identity, idempotency_key)` tuple — see the [Reliability](/dist/docs/3.2.0-rc.6/building/by-layer/L3/webhooks#reliability) section for dedup scope under mixed-mode delivery.
...
When [mTLS](/dist/docs/3.2.0-rc.6/building/by-layer/L2/authentication#mtls) is the authentication mechanism:
...
- **Security Model**: See [Security Model](/dist/docs/3.2.0-rc.6/building/concepts/security-model) for the threat model and the five-layer defense narrative this reference implements
- **Webhooks**: See [Webhooks](/dist/docs/3.2.0-rc.6/building/by-layer/L3/webhooks) for webhook security patterns
- **Error Handling**: See [Error Handling](/dist/docs/3.2.0-rc.6/building/by-layer/L3/error-handling) for authentication errors
- **Orchestrator Design**: See [Orchestrator Design](/dist/docs/3.2.0-rc.6/building/operating/orchestrator-design) for multi-tenant security
