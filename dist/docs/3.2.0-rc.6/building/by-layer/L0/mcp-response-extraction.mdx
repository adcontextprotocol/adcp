...
This page defines the normative algorithm for extracting AdCP success response data from MCP tool results. For error extraction, see [Transport Error Mapping](/dist/docs/3.2.0-rc.6/building/operating/transport-errors).
...
| Error extraction ([transport-errors](/dist/docs/3.2.0-rc.6/building/operating/transport-errors)) | `isError: true` | `structuredContent.adcp_error` or text fallback |
...
All data in `structuredContent` and `content[].text` is seller-controlled. The same prompt injection and data boundary requirements from [Transport Error Mapping](/dist/docs/3.2.0-rc.6/building/operating/transport-errors#security-considerations) apply.

### Size Limits

For pageable reads and other read-only tasks that support field projection, producers SHOULD target at most 128 KiB per serialized MCP response; see [Managing response size](/dist/docs/3.2.0-rc.6/building/concepts/managing-response-size#interoperability-size-target). Clients SHOULD independently enforce a streamed limit on the complete serialized MCP message before buffering or JSON decoding. The recommended client safety limit is 1 MiB. For text fallback, also check the `content[].text` size before the inner `JSON.parse`. The larger safety limit is not a producer target: it bounds hostile or buggy input after normal interoperability guidance has failed.
...
- [A2A Response Extraction](/dist/docs/3.2.0-rc.6/building/by-layer/L0/a2a-response-extraction) — equivalent spec for A2A
