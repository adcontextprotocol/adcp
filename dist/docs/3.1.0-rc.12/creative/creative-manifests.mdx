...
**Note**: Each asset carries an `asset_type` discriminator (`image`, `text`, `url`, etc. — see [asset types](/dist/docs/3.1.0-rc.12/creative/asset-types)). This lets validators select the matching asset schema and report errors against only the chosen branch instead of all 14. The format specification also declares the expected `asset_type` for each `asset_id`; manifest and format should agree.
...
**DOOH-Specific Macros** (see [Universal Macros](/dist/docs/3.1.0-rc.12/creative/universal-macros) for complete list):
...
Use `build_creative` on any agent implementing the Creative Protocol to generate manifests from natural language briefs. See [Generative Creative](/dist/docs/3.1.0-rc.12/creative/generative-creative) for details.
...
3. **For each asset key in `manifest.assets`:**
   ...
   - Fetch the asset type schema (e.g., `/schemas/3.1.0-rc.12/core/assets/image-asset.json`)
...
See [Universal Macros](/dist/docs/3.1.0-rc.12/creative/universal-macros) for complete documentation on available macros, macro substitution process, and format-specific macro support.
...
Manifests for catalog-driven formats (product carousels, dynamic product ads) include their catalogs as assets within the `assets` map. See [Catalogs in Creatives](/dist/docs/3.1.0-rc.12/creative/catalogs#catalogs-in-creatives) for the workflow.

### Repeatable Asset Groups

For carousel, slideshow, and multi-asset formats, see the [Carousel & Multi-Asset Formats](/dist/docs/3.1.0-rc.12/creative/channels/carousels) guide for complete documentation on repeatable asset groups.

## Schema Reference

- [Creative Manifest Schema](https://adcontextprotocol.org/schemas/3.1.0-rc.12/core/creative-manifest.json)
- [Preview Creative Request](https://adcontextprotocol.org/schemas/3.1.0-rc.12/creative/preview-creative-request.json)
- [Preview Creative Response](https://adcontextprotocol.org/schemas/3.1.0-rc.12/creative/preview-creative-response.json)

## Related Documentation

- [Creative Formats](/dist/docs/3.1.0-rc.12/creative/formats) - Understanding format specifications and discovery
- [Channel Guides](/dist/docs/3.1.0-rc.12/creative/channels/video) - Format examples across video, display, audio, DOOH, and carousels
- [build_creative Task](/dist/docs/3.1.0-rc.12/creative/task-reference/build_creative)
- [preview_creative Task](/dist/docs/3.1.0-rc.12/creative/task-reference/preview_creative)
