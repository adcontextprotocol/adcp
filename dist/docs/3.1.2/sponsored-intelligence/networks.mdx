...
A network declares its properties in its [`brand.json`](/dist/docs/3.1.2/brand-protocol/brand-json) using the `relationship` field. This uses the same vocabulary as adagents.json's `delegation_type`, creating a bilateral verification chain — the AdCP equivalent of `sellers.json` in programmatic advertising.
...
Each publisher in the network authorizes the network's agent in their [`adagents.json`](/dist/docs/3.1.2/governance/property/adagents):
...
<Note>
For the SI Chat Protocol session lifecycle and capability negotiation details, see the [SI Chat Protocol](/dist/docs/3.1.2/sponsored-intelligence/si-chat-protocol) and [implementing SI hosts](/dist/docs/3.1.2/sponsored-intelligence/implementing-si-hosts).
</Note>
