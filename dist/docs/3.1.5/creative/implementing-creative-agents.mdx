...
An ad server (CM360, Flashtalking), creative management platform, creative agency, publisher, or sales agent can implement a creative agent. Sales agents that implement the Creative Protocol alongside the Media Buy Protocol serve both roles from a single endpoint — see [Creative capabilities on sales agents](/dist/docs/3.1.5/creative/sales-agent-creative-capabilities).
...
1. **Implement the [Accounts Protocol](/dist/docs/3.1.5/accounts/overview)** — buyers establish accounts with rate cards
...
After the campaign delivers, the buyer reports usage via [`report_usage`](/dist/docs/3.1.5/accounts/tasks/report_usage). This example shows CPM reporting, where cost accrued during delivery rather than at build time. The `pricing_option_id` and `creative_id` flow through for reconciliation:
...
See [list_creative_formats task reference](/dist/docs/3.1.5/creative/task-reference/list_creative_formats) for complete API specification.
...
See [preview_creative task reference](/dist/docs/3.1.5/creative/task-reference/preview_creative) for complete API specification.
...
See [build_creative task reference](/dist/docs/3.1.5/creative/task-reference/build_creative) for complete API specification.
...
See [list_creatives task reference](/dist/docs/3.1.5/creative/task-reference/list_creatives) for complete API specification.
...
See [sync_creatives task reference](/dist/docs/3.1.5/creative/task-reference/sync_creatives) for complete API specification.
...
To verify a remote creative agent is reachable and advertising its format surface, call `get_adcp_capabilities` on the agent endpoint and confirm that the response includes a non-empty `creative.supported_formats` array. Each entry follows the [canonical format declaration](/dist/docs/3.1.5/creative/canonical-formats) shape.
...
|---|---|---|
| Advertiser / account | Account (via [accounts protocol](/dist/docs/3.1.5/accounts/overview)) | Buyer establishes access before querying the library |
...
Declare these capabilities in `get_adcp_capabilities` so buyer agents can determine the correct interaction model without trial and error. See [Interaction models](/dist/docs/3.1.5/creative/specification#interaction-models) in the spec.

Platforms with a creative library should also implement the [accounts protocol](/dist/docs/3.1.5/accounts/overview) so buyers can establish access before querying. This is the same accounts protocol used by sales agents for media buys.

## Related

- [Creative Formats](/dist/docs/3.1.5/creative/formats) - Understanding format structure
- [Creative Manifests](/dist/docs/3.1.5/creative/creative-manifests) - How manifests work
- [Asset Types](/dist/docs/3.1.5/creative/asset-types) - Asset specifications
- [list_creative_formats task](/dist/docs/3.1.5/creative/task-reference/list_creative_formats) - Format discovery API reference
- [list_creatives task](/dist/docs/3.1.5/creative/task-reference/list_creatives) - Creative library API reference
- [build_creative task](/dist/docs/3.1.5/creative/task-reference/build_creative) - Manifest generation API reference
- [preview_creative task](/dist/docs/3.1.5/creative/task-reference/preview_creative) - Preview rendering API reference
