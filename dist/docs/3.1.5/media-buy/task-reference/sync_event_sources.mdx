...
**Request Schema**: [`/schemas/3.1.5/media-buy/sync-event-sources-request.json`](https://adcontextprotocol.org/schemas/3.1.5/media-buy/sync-event-sources-request.json)
**Response Schema**: [`/schemas/3.1.5/media-buy/sync-event-sources-response.json`](https://adcontextprotocol.org/schemas/3.1.5/media-buy/sync-event-sources-response.json)
...
|-----------|------|----------|-------------|
| `account` | [account-ref](/dist/docs/3.1.5/building/by-layer/L2/accounts-and-agents#account-references) | Yes | Account reference. Pass `{ "account_id": "..." }` or `{ "brand": {...}, "operator": "..." }` if the seller supports implicit resolution. |
| `event_sources` | [EventSource](/dist/docs/3.1.5/media-buy/conversion-tracking/#event-source)[] | No | Event sources to sync. When omitted, the call is discovery-only and returns all existing event sources without modification. |
...
| `event_types` | [EventType](/dist/docs/3.1.5/media-buy/conversion-tracking/#event-types)[] | No | Event types this source handles. If omitted, accepts all event types. |
| `action_source` | [ActionSource](/dist/docs/3.1.5/media-buy/conversion-tracking/#action-sources) | No | Flat source category for compatibility (`website`, `app`, `system_generated`, etc.). Pair with `surface` when the flat value is too coarse. |
| `allowed_domains` | string[] | No | Domains authorized to send events for this source |
| `surface` | [EventSurface](/dist/docs/3.1.5/media-buy/conversion-tracking/#event-surfaces) | No | Structured surface this source represents, such as an owned channel, profile, feed, podcast, newsletter list, website, app, or store |
...
**See schema for complete field list**: [sync-event-sources-response.json](https://adcontextprotocol.org/schemas/3.1.5/media-buy/sync-event-sources-response.json)
...
- [Conversion Tracking](/dist/docs/3.1.5/media-buy/conversion-tracking/) - Data model, optimization goals, and the end-to-end flow
- [log_event](/dist/docs/3.1.5/media-buy/task-reference/log_event) - Send marketing events to configured event sources
- [create_media_buy](/dist/docs/3.1.5/media-buy/task-reference/create_media_buy#campaign-with-conversion-optimization) - Set optimization goals on packages referencing event sources
- [get_media_buy_delivery](/dist/docs/3.1.5/media-buy/task-reference/get_media_buy_delivery) - Monitor conversion metrics in delivery reports
