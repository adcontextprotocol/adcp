...
`runtime_status` lives on the product (not the creative); see [`canonical-formats.mdx` §runtime status](/dist/docs/3.0.13/creative/canonical-formats) for the field-level contract.
...
- **Not a promotion gate.** The canonical's promotion to `stable` is gated on `format_schema` evidence across at least two adopters, per [canonical-formats.mdx](/dist/docs/3.0.13/creative/canonical-formats). This page documents what `format_schema` evidence will need to cover for each family.
...
- [`canonical-formats.mdx`](/dist/docs/3.0.13/creative/canonical-formats) — overall canonical-formats vocabulary
