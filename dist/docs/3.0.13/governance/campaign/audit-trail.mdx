...
Both are produced from [`get_plan_audit_logs`](/dist/docs/3.0.13/governance/campaign/tasks/get_plan_audit_logs). The split is a matter of filtering and field selection, not a separate protocol.
...
The table below classifies every field in the [`get_plan_audit_logs`](/dist/docs/3.0.13/governance/campaign/tasks/get_plan_audit_logs#response) response by who can safely see it.
...
```json
{
  "$schema": "/schemas/3.0.13/governance/get-plan-audit-logs-response.json",
  "plans": [
    {
...
```json
{
  "$schema": "/schemas/3.0.13/governance/check-governance-response.json",
...
`mode` is set on the plan via [`sync_plans`](/dist/docs/3.0.13/governance/campaign/tasks/sync_plans) and is the operator's primary lever. The same caller, same payload, same finding produces three different outcomes depending on mode.
...
- **`plan_hash` is verifiable.** It is `base64url_no_pad(SHA-256(JCS(plan_payload)))` over the plan revision the check was evaluated against. Any party with the plan revision can recompute and byte-compare the digest. See [Plan binding and audit](/dist/docs/3.0.13/governance/campaign/specification#plan-binding-and-audit).
...
- [`get_plan_audit_logs`](/dist/docs/3.0.13/governance/campaign/tasks/get_plan_audit_logs) — request and response schema
- [Campaign governance specification](/dist/docs/3.0.13/governance/campaign/specification) — plan binding, drift detection, governance context lifecycle
- [Policy Registry](/dist/docs/3.0.13/governance/policy-registry) — policy versioning, `effective_date`, registry vs inline policies
- [Annex III & Art 22 obligations](/dist/docs/3.0.13/governance/annex-iii-obligations) — when human review is required
