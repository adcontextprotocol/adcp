...
This is a Creative Protocol task. Call it on any agent that declares `"creative"` in `supported_protocols` and `"delivery"` in its [creative agent capabilities](#capability-declaration) — whether that's a dedicated creative service or a [sales agent implementing the Creative Protocol](/dist/docs/3.0.26/creative/sales-agent-creative-capabilities).

**Request Schema**: [`/schemas/3.0.26/creative/get-creative-delivery-request.json`](https://adcontextprotocol.org/schemas/3.0.26/creative/get-creative-delivery-request.json)
**Response Schema**: [`/schemas/3.0.26/creative/get-creative-delivery-response.json`](https://adcontextprotocol.org/schemas/3.0.26/creative/get-creative-delivery-response.json)
...
|-----------|------|----------|-------------|
| `account` | [account-ref](/dist/docs/3.0.26/building/by-layer/L2/accounts-and-agents#account-references) | No | Account reference. Pass `{ "account_id": "..." }` or `{ "brand": {...}, "operator": "..." }` if the seller supports implicit resolution. Limits results to creatives within this account. |
...
When a sales agent implements both protocols, both tasks are available on the same agent URL. See [Creative capabilities on sales agents](/dist/docs/3.0.26/creative/sales-agent-creative-capabilities) for the full pattern.
