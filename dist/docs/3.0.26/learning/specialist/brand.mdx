...
<CardGroup cols={2}>
  <Card title="Brand Protocol overview" icon="fingerprint" href="/dist/docs/3.0.26/brand-protocol">
    The brand domain: identity, distributed publishing, hierarchy, verification, and rights.
  </Card>
  <Card title="brand.json" icon="file-code" href="/dist/docs/3.0.26/brand-protocol/brand-json">
    The self-published identity document — house architecture, `brand_refs[]`, agents, and the mutual-assertion trust model.
  </Card>
  <Card title="verify_brand_claim" icon="circle-check" href="/dist/docs/3.0.26/brand-protocol/tasks/verify_brand_claim">
    The four claim types, the signed-response envelope, and the direction-asymmetric trust rule.
  </Card>
  <Card title="Mutual-assertion trust model" icon="scale-balanced" href="/dist/docs/3.0.26/brand-protocol/brand-json#mutual-assertion-trust-model">
    The two trust layers, the reciprocation table, and why consistency is not standing — the knowability framework.
  </Card>
  <Card title="Seller verification" icon="shield-check" href="/dist/docs/3.0.26/verification/overview">
    The bilateral `adagents.json` + `brand.json` trust chain — Northwind / StreamHaus / Sportshaus Holdings.
  </Card>
  <Card title="Accounts Protocol" icon="building" href="/dist/docs/3.0.26/accounts/overview">
    The operator axis at runtime: `authorized_operators`, the `{brand, operator}` account model, `require_operator_auth`, and billing.
  </Card>
  <Card title="Brand identity → creative" icon="palette" href="/dist/docs/3.0.26/brand-protocol/key-concepts#creative-generation">
  ...
  <Card title="Building a brand agent" icon="server" href="/dist/docs/3.0.26/brand-protocol/building-a-brand-agent">
    Implementing `get_brand_identity`, `verify_brand_claim`, and the response-signing key.
  </Card>
  <Card title="Experimental status" icon="flask" href="/dist/docs/3.0.26/reference/experimental-status">
...
- `…/fixtures/walkthrough/sportshaus-holdings/.well-known/brand.json` — the parent house's identity, with `brand_refs[]` **and** `authorized_operators[]` (a portfolio-wide agency plus a brand-scoped, US-only in-house operator)
  ...
  These are the multi-tier trust-chain fixtures from the [seller-verification walkthrough](/dist/docs/3.0.26/verification/overview).
