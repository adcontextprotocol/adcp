...
3. **Agent Discovery**: Sales agents declare their portfolio via [`get_adcp_capabilities`](/dist/docs/3.1.20/protocol/get_adcp_capabilities) in the `media_buy.portfolio` section

The first two create bilateral verification — like `ads.txt` + `sellers.json` in programmatic. For delegated or network paths, both sides must agree for the supply path to be trusted. See [ad networks](/dist/docs/3.1.20/sponsored-intelligence/networks) for the full pattern.
...
Sales agents use the [`get_adcp_capabilities`](/dist/docs/3.1.20/protocol/get_adcp_capabilities) task to declare their portfolio information in the `media_buy.portfolio` section. This serves multiple purposes:
...
- **[`get_adcp_capabilities`](/dist/docs/3.1.20/protocol/get_adcp_capabilities)** - Discover agent capabilities and portfolio information
- **[Product Discovery](../../media-buy/product-discovery/)** - How authorization integrates with product discovery
- **[Properties Schema](https://adcontextprotocol.org/schemas/3.1.20/core/property.json)** - Technical property data model
