...
See [Signed Governance Context](/dist/docs/3.1.8/building/by-layer/L1/security#signed-governance-context) for the implementer-facing profile (claim set, key discovery, revocation, verification rules).
...
- Compliance coverage from the [policy registry](/dist/docs/3.1.8/governance/policy-registry) -- community-maintained, no per-brand configuration required
- Seller-side verification via governance checks
- Full audit trail via `get_plan_audit_logs`

Set a [`reallocation_threshold`](/dist/docs/3.1.8/governance/campaign/specification#budget-reallocation) on the budget to define guardrails. The governance agent handles the rest.
