...
| Source | URL | Best For |
|--------|-----|----------|
| Website | `https://adcontextprotocol.org/schemas/3.0.4/` | Runtime fetching, version aliases |
...
Declare `supported_protocols` (for protocol baselines) and `specialisms` (for narrow capability claims) in `get_adcp_capabilities` — the compliance runner executes the matching bundles to verify. See the full [Compliance Catalog](/dist/docs/3.0.4/building/compliance-catalog) for every protocol and specialism an agent can claim.

### Common Schemas

| Schema | URL |
|--------|-----|
| Product | `https://adcontextprotocol.org/schemas/3.0.4/core/product.json` |
| Media Buy | `https://adcontextprotocol.org/schemas/3.0.4/core/media-buy.json` |
| Creative Format | `https://adcontextprotocol.org/schemas/3.0.4/core/format.json` |
| Schema Registry | `https://adcontextprotocol.org/schemas/3.0.4/index.json` |
...
All three official SDKs ship AdCP 3.0. Pin to (or above) the minimum listed below and validate with the [storyboard test suite](/dist/docs/3.0.4/building/validate-your-agent).
...
If you are on an earlier major version, upgrade before working through the [3.0 migration guide](/dist/docs/3.0.4/reference/migration/index).
...
- `@adcp/client/testing` — Buyer-side storyboard runner (`runStoryboard`, `comply`, `testAgent`) and seller-side controller scaffold (`createComplyController` — see [Get Test-Ready](/dist/docs/3.0.4/building/get-test-ready))
...
| Major version | `/schemas/3.0.4/`, `/compliance/v3/` | Development, documentation |
...
```javascript
const schema = await fetch(
  'https://adcontextprotocol.org/schemas/3.0.4/core/product.json'
);
```
...
See the [schema registry](https://adcontextprotocol.org/schemas/3.0.4/index.json) for all available schemas.

## Version Discovery

```bash
# Get current version
curl https://adcontextprotocol.org/schemas/3.0.4/index.json | jq '.adcp_version'
```

Check [Release Notes](/dist/docs/3.0.4/reference/release-notes) for version history and migration guides.
...
<Card title="Registry API Reference" icon="server" href="/dist/docs/3.0.4/registry/index">
