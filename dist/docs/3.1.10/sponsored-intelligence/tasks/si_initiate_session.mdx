...
<Note>
**Experimental.** Sponsored Intelligence (`si_get_offering`, `si_initiate_session`, `si_send_message`, `si_terminate_session`) is part of AdCP 3.0 as an experimental surface — it may change between 3.x releases with at least 6 weeks' notice. Sellers implementing any of these tasks MUST declare `sponsored_intelligence.core` in `experimental_features`. See [experimental status](/dist/docs/3.1.10/reference/experimental-status) for the full contract.
</Note>
...
| `context` | object | No | Opaque correlation data echoed unchanged in the response (see [application context](/dist/docs/3.1.10/building/by-layer/L2/context-sessions#application-context-context)) |
...
```json
{
  "$schema": "/schemas/3.1.10/sponsored-intelligence/si-initiate-session-request.json",
...
```json
{
  "$schema": "/schemas/3.1.10/sponsored-intelligence/si-initiate-session-response.json",
  "status": "completed",
