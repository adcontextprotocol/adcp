...
The v2 sunset is not tied to 4.0. v2 reaches end of life on its own schedule because 2.5 was an early, preliminary version — adoption remained small, and the architecture committee identified protections missing from v2 that cannot be backported without a major redesign. v2's shorter window is the documented exception to the [12-month previous-major support commitment](/dist/docs/3.1.10/reference/versioning#support-window-for-previous-major); future majors will not invoke this exception.
...
1. Read [What's new in v3](/dist/docs/3.1.10/reference/whats-new-in-v3).
2. Work through the [v2→v3 migration guide](/dist/docs/3.1.10/reference/migration).
3. Validate with [storyboard testing](/dist/docs/3.1.10/reference/migration/v3-readiness).
...
2. Notify them of the August 1, 2026 (UTC) deprecation and share the [migration guide](/dist/docs/3.1.10/reference/migration).
...
Sellers in mid-migration can temporarily support both versions using `get_adcp_capabilities` and the `major_versions` array — see [Running v2 and v3 side by side](/dist/docs/3.1.10/reference/migration#running-v2-and-v3-side-by-side). Dual-support is a migration tool, not a long-term posture. After August 1, 2026 (UTC), v3-only is the required configuration.

## Related

- **[What's new in v3](/dist/docs/3.1.10/reference/whats-new-in-v3)** — feature-by-feature summary of what 3.0 adds
- **[Migration guide](/dist/docs/3.1.10/reference/migration)** — breaking changes and deep-dive pages for every protocol area
- **[v3 readiness checklist](/dist/docs/3.1.10/reference/migration/v3-readiness)** — 8 minimum requirements for storyboard testing
- **[Versioning & governance](/dist/docs/3.1.10/reference/versioning)** — 3.x stability guarantees and release cadence
