...
| Field | Meaning |
|---|---|
| `vendor` (required, [BrandRef](/dist/docs/3.1.10/brand-protocol/brand-json)) | The party whose count of the billing metric governs invoicing. Same field shape covers seller's ad server, buyer's 3PAS, or a third-party vendor — the BrandRef domain disambiguates. |
...
- [`measurement-terms`](https://adcontextprotocol.org/schemas/3.1.10/core/measurement-terms.json) — schema
- [`get_media_buy_delivery`](/dist/docs/3.1.10/media-buy/task-reference/get_media_buy_delivery) — seller-side finality flags
- [`report_usage`](/dist/docs/3.1.10/accounts/tasks/report_usage) — buyer-side / third-party finality flags
- [Accountability](/dist/docs/3.1.10/media-buy/advanced-topics/accountability) — performance standards, makegood remedies, cancellation
- [Reporting capabilities and measurement windows](/dist/docs/3.1.10/media-buy/media-buys/optimization-reporting)
