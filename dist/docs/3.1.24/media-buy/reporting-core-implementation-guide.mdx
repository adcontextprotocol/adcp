...
4. **[`get_reporting_status`](/dist/docs/3.1.24/media-buy/task-reference/get_reporting_status)
...
[future expectations on complete summaries](/dist/docs/3.1.24/media-buy/task-reference/get_reporting_status#future-expectation-on-a-complete-summary)
...
3. **Generate obligations.** When an account installs a delivery
   configuration through [`sync_accounts`](/dist/docs/3.1.24/accounts/tasks/sync_accounts),
...
7. **Escalate honestly.** A due obligation stays `delayed` while automated
   ...
   elapses; that boundary wins when it overlaps the grace window. Resolve a
   mismatch only when the underlying conflict ends. Waive one only after an
   explicit bilateral agreement covering that exact caller/account issue and
   causing status. A valid waiver retires the issue, restores the underlying
   seller health while retaining the immutable consumer statement, and triggers
   an offered `reporting.status_changed` recovery fire when health changes.
...
[controller reference](/dist/docs/3.1.24/building/by-layer/L3/comply-test-controller#reporting_core_lifecycle_probe)
...
- **`managed_delivery: true`** adds offerings with a `method` —
  ...
  principal-layer destinations ([`sync_principal`](/dist/docs/3.1.24/protocol/sync_principal)).
...
buy — see [Billing Authority](/dist/docs/3.1.24/media-buy/advanced-topics/billing-authority)
