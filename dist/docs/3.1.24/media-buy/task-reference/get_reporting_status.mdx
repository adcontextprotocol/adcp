...
usable. It does not replace [`get_media_buy_delivery`](/dist/docs/3.1.24/media-buy/task-reference/get_media_buy_delivery)
...
[`get_media_buy_delivery`](/dist/docs/3.1.24/media-buy/task-reference/get_media_buy_delivery)
...
[`sync_reporting_status`](/dist/docs/3.1.24/media-buy/task-reference/sync_reporting_status).
...
`resolved` means the underlying disagreement ended: the consumer superseded the
statement with one that agrees, or the seller's projection changed so the two no
longer conflict. `waived` is different. It records an explicit off-protocol
agreement by the authenticated consumer and seller to disregard that exact
caller/account issue; a seller cannot create the agreement unilaterally. The
waiver is bound to the `issue_id`, causing `reporting_status_id`, and diagnosed
conflict. It retires the issue and restores the underlying seller health in
both summary counts and the period projection, even while that immutable
consumer statement remains the current leaf in `consumer_statuses[]`. It does
not turn the statement into seller evidence. A later statement or materially
different conflict is evaluated independently and receives a new issue ID.

If the waiver removes the last impairment, sellers offering
`reporting.status_changed` emit the recovery transition only after the recovered
projection is readable; `issue_ids` is empty or absent. If another impairment
keeps health unchanged, no health-transition notification is required. Polling
...
[`sync_accounts`](/dist/docs/3.1.24/accounts/tasks/sync_accounts) `notification_configs[]`. It is
...
[`sync_reporting_receipts`](/dist/docs/3.1.24/media-buy/task-reference/sync_reporting_receipts).
...
[`sync_accounts`](/dist/docs/3.1.24/accounts/tasks/sync_accounts) and
[`list_accounts`](/dist/docs/3.1.24/accounts/tasks/list_accounts) return
