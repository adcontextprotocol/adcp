...
This is the specialized compatibility surface. When the seller advertises [`sync_principal`](/dist/docs/3.1.24/protocol/sync_principal), use its `notification_configs` section so webhooks and reusable connection resources share one versioned caller-scoped configuration. Sellers exposing both tasks maintain one underlying subscriber set.

The initial event type is `capabilities.changed`, an invalidation signal for cached [`get_adcp_capabilities`](/dist/docs/3.1.24/protocol/get_adcp_capabilities) responses.

**Request Schema**: [`/schemas/3.1.24/protocol/sync-agent-notification-configs-request.json`](https://adcontextprotocol.org/schemas/3.1.24/protocol/sync-agent-notification-configs-request.json)
**Response Schema**: [`/schemas/3.1.24/protocol/sync-agent-notification-configs-response.json`](https://adcontextprotocol.org/schemas/3.1.24/protocol/sync-agent-notification-configs-response.json)

## When to call

First call [`get_adcp_capabilities`](/dist/docs/3.1.24/protocol/get_adcp_capabilities). Only call this task when the seller advertises it as the registration task:
...
When `registration_task` is [`sync_principal`](/dist/docs/3.1.24/protocol/sync_principal), call that task instead.
...
When the seller's advertised capability document materially changes, it publishes the new [`get_adcp_capabilities`](/dist/docs/3.1.24/protocol/get_adcp_capabilities) snapshot first, then fires `capabilities.changed` to active subscribers:
