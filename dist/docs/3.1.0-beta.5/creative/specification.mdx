...
Sales agents that implement both the Media Buy Protocol and the Creative Protocol serve both roles from a single endpoint. See [Creative capabilities on sales agents](/dist/docs/3.1.0-beta.5/creative/sales-agent-creative-capabilities).
...
Agents that host a creative library should also implement the [accounts protocol](/dist/docs/3.1.0-beta.5/accounts/overview) so buyers can establish access before querying. Sales agents that already implement accounts for media buys do not need to do anything additional.
...
**Schema**: [`enums/creative-status.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/enums/creative-status.json)
...
- `approved` → `archived` (seller-initiated): seller archives an unassigned creative for inactivity, post-flight expiry, or storage policy. Sellers MUST NOT seller-archive a creative that has active package assignments — the `approved` → `rejected` (revocation) path with an `impairment` on the affected buy is the only conformant route when active serving is involved. State-change observability for seller-initiated archive follows the [creative retention contract](/dist/docs/3.1.0-beta.5/creative/creative-libraries#creatives-outlast-campaigns) — minimally, the new `status` MUST be visible on the next `list_creatives` read.
...
Seller-initiated and system-initiated transitions fire push notifications against the account's [`notification_configs[]`](/dist/docs/3.1.0-beta.5/accounts/tasks/sync_accounts#account-level-webhook-subscriptions) subscribers — each entry whose `event_types[]` includes the fired type receives an independent fire. Two event types cover the surface:

- **`creative.status_changed`** — fires on every seller-initiated or system-initiated transition: `pending_review → approved`/`rejected`, `approved → pending_review` (re-review), `approved → rejected` (post-approval revocation), `approved → archived` (seller-initiated). Payload: [`creative-status-changed-webhook.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/creative/creative-status-changed-webhook.json).
- **`creative.purged`** — fires when a creative is destroyed (retention sweep, takedown, legal erasure). `soft` purges retain a tombstone on `list_creatives` (with `include_purged: true`) for 30 days; `hard` purges retain no record — the webhook is the buyer's only signal. Payload: [`creative-purged-webhook.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/creative/creative-purged-webhook.json).
...
Both surfaces use the same `pricing_options[]` array of [`vendor-pricing-option`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/core/vendor-pricing-option.json) objects. Both require `account` and `include_pricing: true` on the request.
...
Creative agents reuse the vendor pricing models defined in [`vendor-pricing-option.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/core/vendor-pricing-option.json):
...
**Schema**: [`core/creative-consumption.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/core/creative-consumption.json)
...
Creative agents that charge for their services MUST implement the [Accounts Protocol](/dist/docs/3.1.0-beta.5/accounts/overview). This applies to any creative agent with pricing — ad servers, generation platforms, and transformation agents that bill for usage.
...
**Reference**: [`build_creative` task](/dist/docs/3.1.0-beta.5/creative/task-reference/build_creative)
...
**Reference**: [`preview_creative` task](/dist/docs/3.1.0-beta.5/creative/task-reference/preview_creative)
...
**Schema**: [`creative/list-creatives-request.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/creative/list-creatives-request.json) / [`creative/list-creatives-response.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/creative/list-creatives-response.json)

**Reference**: [`list_creatives` task](/dist/docs/3.1.0-beta.5/creative/task-reference/list_creatives)
...
- When `account` and `include_pricing=true` are provided, agents that charge MUST include `pricing_options` on each creative — an array of [`vendor-pricing-option`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/core/vendor-pricing-option.json) objects. Vendors may offer multiple options per creative (volume tiers, context-specific rates, different pricing models).
...
**Schema**: [`creative/sync-creatives-request.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/creative/sync-creatives-request.json) / [`creative/sync-creatives-response.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/creative/sync-creatives-response.json)

**Reference**: [`sync_creatives` task](/dist/docs/3.1.0-beta.5/creative/task-reference/sync_creatives)
...
**Reference**: [`get_creative_delivery` task](/dist/docs/3.1.0-beta.5/creative/task-reference/get_creative_delivery)
...
Creative agents MUST return errors using the [standard AdCP error schema](/dist/docs/3.1.0-beta.5/building/by-layer/L3/error-handling).
...
| Schema | Description |
|--------|-------------|
| [`core/format.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/core/format.json) | Format definition |
| [`core/creative-manifest.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/core/creative-manifest.json) | Creative manifest |
| [`core/creative-asset.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/core/creative-asset.json) | Asset definition |
...
| [`creative/list-creatives-request.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/creative/list-creatives-request.json) | list_creatives request |
| [`creative/list-creatives-response.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/creative/list-creatives-response.json) | list_creatives response |
| [`creative/sync-creatives-request.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/creative/sync-creatives-request.json) | sync_creatives request |
| [`creative/sync-creatives-response.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/creative/sync-creatives-response.json) | sync_creatives response |
