...
Transform, generate, or retrieve a creative manifest for a specific format. To render a visual preview of the resulting manifest, pass it to [`preview_creative`](/dist/docs/3.1.0-beta.5/creative/task-reference/preview_creative). Supports three modes:
...
For generation and transformation, `build_creative` takes a creative manifest as input and produces a creative manifest as output. For library retrieval, provide a `creative_id` (from [`list_creatives`](/dist/docs/3.1.0-beta.5/creative/task-reference/list_creatives)) and the agent resolves it from its library.

For information about format IDs and how to reference formats, see [Creative Formats - Referencing Formats](/dist/docs/3.1.0-beta.5/creative/formats#referencing-formats).
...
| `consumption` | object | Structured consumption details — `tokens`, `images_generated`, `renders`, `duration_seconds`. See [`creative-consumption.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.5/core/creative-consumption.json). Informational for cost verification; `vendor_cost` is the billing source of truth. |
...
```json
{
  "$schema": "/schemas/3.1.0-beta.5/media-buy/build-creative-request.json",
...
Retrieve a creative from the agent's library and resolve it into a manifest with ad-serving assets. Use this when you know the `creative_id` from [`list_creatives`](/dist/docs/3.1.0-beta.5/creative/task-reference/list_creatives) and want the creative agent to produce a delivery-ready manifest with tags (HTML, JavaScript, or VAST):
...
```json
{
  "$schema": "/schemas/3.1.0-beta.5/media-buy/build-creative-response.json",
  "status": "completed",
  "creative_manifest": {
    "format_id": {
      "agent_url": "https://creative.example.com",
...
<Note>
**Cross-agent workflow**: When creative generation and media buying are handled by different agents, use `build_creative` on the creative agent to produce a manifest with tags, then [`sync_creatives`](/dist/docs/3.1.0-beta.5/creative/task-reference/sync_creatives) on the sales agent to upload it. When the sales agent implements both protocols, this happens on a single endpoint — see [Creative capabilities on sales agents](/dist/docs/3.1.0-beta.5/creative/sales-agent-creative-capabilities).
</Note>
...
The `pricing_option_id` corresponds to one of the options from [`list_creatives`](/dist/docs/3.1.0-beta.5/creative/task-reference/list_creatives#pricing). The buyer passes it in [`report_usage`](/dist/docs/3.1.0-beta.5/accounts/tasks/report_usage) for reconciliation.
...
For these cases, configure a webhook via `push_notification_config` to receive the result. See [Async Operations](/dist/docs/3.1.0-beta.5/building/by-layer/L3/async-operations) and [Push Notifications](/dist/docs/3.1.0-beta.5/building/by-layer/L3/webhooks).
...
2. **Preview**: Use `preview_creative` to see how it renders (see [preview_creative](/dist/docs/3.1.0-beta.5/creative/task-reference/preview_creative))
...
2. **preview_creative** → Renders the manifest separately (see [preview_creative](/dist/docs/3.1.0-beta.5/creative/task-reference/preview_creative))
