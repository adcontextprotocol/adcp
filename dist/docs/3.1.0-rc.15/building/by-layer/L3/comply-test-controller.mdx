...
**Schemas**: [`comply-test-controller-request.json`](https://adcontextprotocol.org/schemas/3.1.0-rc.15/compliance/comply-test-controller-request.json) | [`comply-test-controller-response.json`](https://adcontextprotocol.org/schemas/3.1.0-rc.15/compliance/comply-test-controller-response.json)
...
Transitions a creative to the specified status. The seller MUST enforce valid transitions per the [creative lifecycle state machine](/dist/docs/3.1.0-rc.15/creative/specification#creative-status-lifecycle).
...
Transitions an account to the specified status. The seller MUST enforce the [account lifecycle rules](/dist/docs/3.1.0-rc.15/accounts/overview#account-status-lifecycle) — terminal states (`rejected`, `closed`) cannot be exited.
...
account (account + principal pair) into a submitted task envelope.
`force_get_products_arm` applies only to `get_products` with
`buying_mode: "brief"` or `"refine"`.
`force_get_signals_arm` applies only to `get_signals` with
...
buyers cannot reliably trigger async discovery from request shape alone, but
conformance needs a deterministic way to prove clients and sellers honor the
task-result path. The submitted envelope carries only `status` and `task_id`
(plus optional advisory fields such as `message`); terminal `products[]`,
`proposals[]`, or `signals[]` land on task completion through `get_task_status`
(legacy `tasks/get`) and any registered push notification.

**Params:**

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `arm` | `submitted` | Yes | Target response arm for the next matching discovery call. |
...
`force_create_media_buy_arm`, carrying `forced.arm` and `forced.task_id`.
...
same operation before consumption overwrites the prior directive. Request
idempotency replay semantics are unchanged: if the discovery request that
consumed the directive is replayed, the seller returns the cached submitted
envelope and does not consume a fresh directive.
...
| `fixture` | object | No | Pricing option shape per the [`PricingOption`](https://adcontextprotocol.org/schemas/3.1.0-rc.15/core/pricing-option.json) schema (`pricing_model`, `currency`, `floor_price` for auction-based, `fixed_price` for fixed, etc.) |
...
The cross-page framing for both implementations of this pattern, the SDK's `_bridge` advisory marker, and the runtime-signals disambiguation table all live in the Conformance Specification → [Test surfaces and the storyboard loop](/dist/docs/3.1.0-rc.15/building/verification/conformance#test-surfaces-and-the-storyboard-loop).
...
Sellers that implement `list_scenarios` MUST respond with scenario names that appear verbatim in the `scenario` enum of [`comply-test-controller-request.json`](https://adcontextprotocol.org/schemas/3.1.0-rc.15/compliance/comply-test-controller-request.json). Custom seller-specific scenario names are not part of the compliance contract; storyboard runners will not dispatch to scenarios outside the canonical enum, so listing them serves no purpose. A seller that supports `seed_product` MUST respond with the string `"seed_product"` — not `"create_test_product"` or any other variant.
...
1. **Sellers validate transition ordering.** The controller enforces the same state machine rules as production. Calling `force_creative_status(approved)` on a creative that was never `processing` is an error — the controller rejects it just as production would. The lifecycle state machines referenced here are defined in the respective protocol specifications (see [creative lifecycle](/dist/docs/3.1.0-rc.15/creative/specification#creative-status-lifecycle), [account lifecycle](/dist/docs/3.1.0-rc.15/accounts/overview#account-status-lifecycle), [media buy lifecycle](/dist/docs/3.1.0-rc.15/media-buy/specification), [SI session lifecycle](/dist/docs/3.1.0-rc.15/sponsored-intelligence/specification#session-states)).
