...
Governance constraints are evaluated after generation via [`get_creative_features`](/dist/docs/3.0.5/governance/creative/get_creative_features), which scores a specific creative manifest — but knowing the feature set before generation avoids a reject-regenerate cycle. Creative agents that are aware of governance requirements produce compliant output on the first pass.

See the [Creative Governance overview](/dist/docs/3.0.5/governance/creative/index) for the feature-based evaluation model.
...
Different agents price differently. The CTV ad server charges CPM (cost per thousand impressions served). The transformation agent charges per format adapted. The AI generation platform charges per image generated. The protocol handles all of these through the same `pricing_options` pattern — see the [pricing specification](/dist/docs/3.0.5/creative/specification#pricing) for details.
...
- **Key concepts**: [Assets, formats, manifests, and creative agents](/dist/docs/3.0.5/creative/key-concepts) — the building blocks behind this walkthrough
- **AI creative**: [AI creative for campaign teams](/dist/docs/3.0.5/creative/ai-creative-overview) — the strategic view for non-engineers
- **Generative creative**: [Generative creative](/dist/docs/3.0.5/creative/generative-creative) — Tier 1, 2, and 3 in depth
- **Build an agent**: [Implementing creative agents](/dist/docs/3.0.5/creative/implementing-creative-agents) — make your platform speak AdCP
- **Orchestrate**: [Multi-agent orchestration](/dist/docs/3.0.5/creative/multi-agent-orchestration) — the engineering patterns behind Maya's platform
- **Get certified**: The [Buyer track](/dist/docs/3.0.5/learning/tracks/buyer) teaches the full creative workflow through interactive modules
