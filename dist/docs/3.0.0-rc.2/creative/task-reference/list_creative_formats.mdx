...
| `wcag_level` | string | No | Filter to formats meeting at least this WCAG level: `A`, `AA`, `AAA`. See [Accessibility](/dist/docs/3.0.0-rc.2/creative/accessibility). |
...
When catalog assets are present, buyers should sync the required catalogs via [`sync_catalogs`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/sync_catalogs) before submitting creatives. See [Catalogs](/dist/docs/3.0.0-rc.2/creative/catalogs) for the full lifecycle.
...
1. **Build Creatives**: Use [`build_creative`](/dist/docs/3.0.0-rc.2/creative/task-reference/build_creative) to assemble assets into format
2. **Preview**: Use [`preview_creative`](/dist/docs/3.0.0-rc.2/creative/task-reference/preview_creative) to see visual output
3. **Validate**: Use [`sync_creatives`](/dist/docs/3.0.0-rc.2/creative/task-reference/sync_creatives) with `dry_run: true`
4. **Upload**: Use [`sync_creatives`](/dist/docs/3.0.0-rc.2/creative/task-reference/sync_creatives) to upload to media buy

## Learn More

- [Format Schema](https://adcontextprotocol.org/schemas/v3/core/format.json) - Complete format structure
- [Asset Types](/dist/docs/3.0.0-rc.2/creative/asset-types) - Asset specification details
- [Standard Formats](/dist/docs/3.0.0-rc.2/media-buy/capability-discovery/implementing-standard-formats) - IAB-compatible reference formats
