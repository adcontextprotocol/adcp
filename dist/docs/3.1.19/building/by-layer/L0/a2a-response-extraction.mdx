...
This page defines the normative algorithm for extracting AdCP response data from A2A Task objects and TaskStatusUpdateEvents. For the canonical response structure that sellers must produce, see [A2A Response Format](/dist/docs/3.1.19/building/by-layer/L0/a2a-response-format). For error-specific extraction, see [Transport Error Mapping](/dist/docs/3.1.19/building/operating/transport-errors).
...
This algorithm extracts *any* AdCP data from A2A responses, including error payloads (`adcp_error`). Error-specific extraction ([Transport Error Mapping](/dist/docs/3.1.19/building/operating/transport-errors)) is a specialization that checks for the `adcp_error` key in the extracted data.
...
All data in `.artifacts[].parts[].data` and `status.message.parts[].data` is seller-controlled. The prompt injection, data boundary, and size limit requirements from [Transport Error Mapping](/dist/docs/3.1.19/building/operating/transport-errors#security-considerations) apply.
...
- [A2A Response Format](/dist/docs/3.1.19/building/by-layer/L0/a2a-response-format) — canonical response structure for sellers
- [Transport Error Mapping](/dist/docs/3.1.19/building/operating/transport-errors) — error extraction from MCP and A2A
- [MCP Response Extraction](/dist/docs/3.1.19/building/by-layer/L0/mcp-response-extraction) — equivalent spec for MCP
- [A2A Guide](/dist/docs/3.1.19/building/by-layer/L0/a2a-guide) — A2A transport integration
