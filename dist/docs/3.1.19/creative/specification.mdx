...
Sales agents that implement both the Media Buy Protocol and the Creative Protocol serve both roles from a single endpoint. See [Creative capabilities on sales agents](/dist/docs/3.1.19/creative/sales-agent-creative-capabilities).
...
Agents that host a creative library should also implement the [accounts protocol](/dist/docs/3.1.19/accounts/overview) so buyers can establish access before querying. Sales agents that already implement accounts for media buys do not need to do anything additional.
...
**Schema**: [`enums/creative-status.json`](https://adcontextprotocol.org/schemas/3.1.19/enums/creative-status.json)
...
- `approved` → `archived` (seller-initiated): seller archives an unassigned creative for inactivity, post-flight expiry, or storage policy. Sellers MUST NOT seller-archive a creative that has active package assignments — the `approved` → `rejected` (revocation) path with an `impairment` on the affected buy is the only conformant route when active serving is involved. State-change observability for seller-initiated archive follows the [creative retention contract](/dist/docs/3.1.19/creative/creative-libraries#creatives-outlast-campaigns) — minimally, the new `status` MUST be visible on the next `list_creatives` read.
...
Both surfaces use the same `pricing_options[]` array of [`vendor-pricing-option`](https://adcontextprotocol.org/schemas/3.1.19/core/vendor-pricing-option.json) objects. Both require `account` and `include_pricing: true` on the request.
...
Creative agents reuse the vendor pricing models defined in [`vendor-pricing-option.json`](https://adcontextprotocol.org/schemas/3.1.19/core/vendor-pricing-option.json):
...
**Schema**: [`core/creative-consumption.json`](https://adcontextprotocol.org/schemas/3.1.19/core/creative-consumption.json)
...
Creative agents that charge for their services MUST implement the [Accounts Protocol](/dist/docs/3.1.19/accounts/overview). This applies to any creative agent with pricing — ad servers, generation platforms, and transformation agents that bill for usage.
...
**Reference**: [`list_transformers` task](/dist/docs/3.1.19/creative/task-reference/list_transformers)
...
**Reference**: [`build_creative` task](/dist/docs/3.1.19/creative/task-reference/build_creative)
...
**Reference**: [`preview_creative` task](/dist/docs/3.1.19/creative/task-reference/preview_creative)
...
**Schema**: [`creative/list-creatives-request.json`](https://adcontextprotocol.org/schemas/3.1.19/creative/list-creatives-request.json) / [`creative/list-creatives-response.json`](https://adcontextprotocol.org/schemas/3.1.19/creative/list-creatives-response.json)

**Reference**: [`list_creatives` task](/dist/docs/3.1.19/creative/task-reference/list_creatives)
...
- When `account` and `include_pricing=true` are provided, agents that charge MUST include `pricing_options` on each creative — an array of [`vendor-pricing-option`](https://adcontextprotocol.org/schemas/3.1.19/core/vendor-pricing-option.json) objects. Vendors may offer multiple options per creative (volume tiers, context-specific rates, different pricing models).
...
**Schema**: [`creative/sync-creatives-request.json`](https://adcontextprotocol.org/schemas/3.1.19/creative/sync-creatives-request.json) / [`creative/sync-creatives-response.json`](https://adcontextprotocol.org/schemas/3.1.19/creative/sync-creatives-response.json)

**Reference**: [`sync_creatives` task](/dist/docs/3.1.19/creative/task-reference/sync_creatives)
...
**Reference**: [`get_creative_delivery` task](/dist/docs/3.1.19/creative/task-reference/get_creative_delivery)
...
Creative agents MUST return errors using the [standard AdCP error schema](/dist/docs/3.1.19/building/by-layer/L3/error-handling).
...
| Schema | Description |
|--------|-------------|
| [`core/format.json`](https://adcontextprotocol.org/schemas/3.1.19/core/format.json) | Format definition |
| [`core/creative-manifest.json`](https://adcontextprotocol.org/schemas/3.1.19/core/creative-manifest.json) | Creative manifest |
| [`core/creative-asset.json`](https://adcontextprotocol.org/schemas/3.1.19/core/creative-asset.json) | Asset definition |
...
| [`creative/list-creatives-request.json`](https://adcontextprotocol.org/schemas/3.1.19/creative/list-creatives-request.json) | list_creatives request |
| [`creative/list-creatives-response.json`](https://adcontextprotocol.org/schemas/3.1.19/creative/list-creatives-response.json) | list_creatives response |
| [`creative/sync-creatives-request.json`](https://adcontextprotocol.org/schemas/3.1.19/creative/sync-creatives-request.json) | sync_creatives request |
| [`creative/sync-creatives-response.json`](https://adcontextprotocol.org/schemas/3.1.19/creative/sync-creatives-response.json) | sync_creatives response |
