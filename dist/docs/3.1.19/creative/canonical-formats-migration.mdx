...
description: "Concrete migration paths for sellers, creative agents, buyers, and tooling moving from v1 named formats to canonical-format product-bound declarations."
...
For the architecture, read [`canonical-formats`](/dist/docs/3.1.19/creative/canonical-formats) first. This page is just the migration mechanics.
...
[`/schemas/registries/v1-canonical-mapping.json`](https://adcontextprotocol.org/schemas/3.1.19/registries/v1-canonical-mapping.json) is the authoritative AAO-published registry. SDKs consume it to project v1 formats to v2 canonicals during dual-emission and v1↔v2 translation. Two match modes per entry:

- **`format_id_glob`** — exact / glob match against v1 `format_id.id`. Published entries currently use non-wildcarded literals for observed legacy video/audio ids whose duration, dimensions, or VAST delivery cannot be recovered structurally.
...
The registry currently carries 22 literal entries and 7 structural fallbacks covering hosted video/audio, VAST, DAAST, HTML5 bundles, display tags, and AAO-namespace static-display ids. Subsequent PRs expand coverage as adopter feedback surfaces patterns. Governance follows the same rules as `asset-group-vocabulary.json`: PR with rationale + ≥1 reference adopter + AgenticAdvertising.org maintainer review; entries are additive and digest-pinned.
...
Resolution order when an SDK projects a v1 format to its v2 canonical:
...
Reverse direction (v2 → v1 demotion for v1-only buyers reading a v2 seller) relies on the seller-authored `v1_format_ref`. SDKs MUST NOT invert structural matches. They MAY use a non-wildcarded literal registry entry as a non-normative best-effort fallback only when it does not contradict a seller-authored reference; downstream consumers MUST NOT depend on that fallback.
...
If a v1 format slot uses an author-invented name that the canonical vocabulary covers, the format declaration carries an optional `asset_group_id` field on the slot pointing at the canonical entry. Same as the existing `asset_role` field, but referencing the [canonical vocabulary](https://adcontextprotocol.org/schemas/3.1.19/core/asset-group-vocabulary.json) rather than free text.
...
| `sync_creatives` | Submit creative manifest | Unchanged. Sales agents accept manifests with `assets` keyed by slot name per the format's `slots` declaration. Use `dry_run: true` to rehearse the actual upload/update operation without mutating the library. |
...
| `validate_input` | (didn't exist) | New buyer manifest-preflight primitive. Validates a manifest against canonical formats and/or specific products without committing to a render. It does not rehearse `sync_creatives` library mutations, IDs, assignments, or account gates. Cheap; `predicted` field carries pre-flight estimates. |
...
2. Use `validate_input` for cheap manifest preflight before committing to renders or comparing product targets. Use `sync_creatives` with `dry_run: true` when the question is whether the seller would accept the actual upload/update request.
3. Use the canonical [`asset_group_id` vocabulary](https://adcontextprotocol.org/schemas/3.1.19/core/asset-group-vocabulary.json) when constructing manifests; the registry's `aliases` field maps v1-era slot names to canonical equivalents.
...
- [Canonical Formats overview](/dist/docs/3.1.19/creative/canonical-formats)
- [RFC #3305](https://github.com/adcontextprotocol/adcp/issues/3305) — architectural decisions and rationale
- [Asset group vocabulary](https://adcontextprotocol.org/schemas/3.1.19/core/asset-group-vocabulary.json)
- [BrandRef schema](https://adcontextprotocol.org/schemas/3.1.19/core/brand-ref.json)
