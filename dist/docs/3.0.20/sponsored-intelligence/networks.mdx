...
A network declares its properties in its [`brand.json`](/dist/docs/3.0.20/brand-protocol/brand-json) using the `relationship` field. This uses the same vocabulary as adagents.json's `delegation_type`, creating a bilateral verification chain — the AdCP equivalent of `sellers.json` in programmatic advertising.
...
Each publisher in the network authorizes the network's agent in their [`adagents.json`](/dist/docs/3.0.20/governance/property/adagents):
...
When an ad network sells [SI Chat Protocol](/dist/docs/3.0.20/sponsored-intelligence/si-chat-protocol) sessions on behalf of brands, it acts as an intermediary in the session flow. The brand syncs offerings to the network via `sync_catalogs` with `type: "offering"`, and the network forwards them to underlying platforms. When a platform triggers a session, the network routes it to the correct brand agent.
...
<Note>
For the SI Chat Protocol session lifecycle and capability negotiation details, see the [SI Chat Protocol](/dist/docs/3.0.20/sponsored-intelligence/si-chat-protocol) and [implementing SI hosts](/dist/docs/3.0.20/sponsored-intelligence/implementing-si-hosts).
</Note>
