...
**Request Schema**: [`/schemas/3.2.0-rc.0/media-buy/sync-catalogs-request.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.0/media-buy/sync-catalogs-request.json)
**Response Schema**: [`/schemas/3.2.0-rc.0/media-buy/sync-catalogs-response.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.0/media-buy/sync-catalogs-response.json)
...
This task sits between format discovery and creative submission in the [account state setup sequence](/dist/docs/3.2.0-rc.0/building/by-layer/L2/account-state):

1. [`get_products`](/dist/docs/3.2.0-rc.0/media-buy/task-reference/get_products) — check canonical `format_options[].params.slots[]` for catalog requirements
2. **`sync_catalogs`** — push the required feeds to the account
3. [`sync_creatives`](/dist/docs/3.2.0-rc.0/creative/task-reference/sync_creatives) — submit creatives that reference synced catalogs by `catalog_id`
4. [`create_media_buy`](/dist/docs/3.2.0-rc.0/media-buy/task-reference/create_media_buy) — launch the campaign
...
|-----------|------|----------|-------------|
| `account` | [account-ref](/dist/docs/3.2.0-rc.0/building/by-layer/L2/accounts-and-agents#account-references) | Yes | Account containing the catalogs. Pass `{ "account_id": "..." }` or an implicit-resolution reference supported by the seller. |
...
Before sending updates, check [`get_adcp_capabilities`](/dist/docs/3.2.0-rc.0/protocol/get_adcp_capabilities) for:
...
Each catalog in the `catalogs` array is a [Catalog](/dist/docs/3.2.0-rc.0/creative/catalogs#the-catalog-object) object. Key fields:
...
|-------|-------------|------------|
| [`REFERENCE_NOT_FOUND`](/dist/docs/3.2.0-rc.0/building/verification/compliance-catalog#error-code-reference-not-found) | Referenced `catalog_id` doesn't exist or is not accessible (when using `catalog_ids` filter; `error.field` = `catalog_ids`) | Verify catalog IDs from a previous sync or discovery call |
| [`UNSUPPORTED_FEATURE`](/dist/docs/3.2.0-rc.0/building/verification/compliance-catalog#error-code-unsupported-feature) | Seller does not declare `catalog_item_availability_updates` | Remove the updates or route to a seller that declares the capability |
| [`INVALID_REQUEST`](/dist/docs/3.2.0-rc.0/building/verification/compliance-catalog#error-code-invalid-request) | Duplicate item update, seller-managed catalog, `dry_run` conflict, or mixed request whose catalog work would be asynchronous | Correct the request or split catalog ingestion from availability changes |
| [`FEED_FETCH_FAILED`](/dist/docs/3.2.0-rc.0/building/verification/compliance-catalog#error-code-feed-fetch-failed) | Platform couldn't fetch the feed URL | Check URL accessibility, authentication, and feed format |
| [`INVALID_FEED_FORMAT`](/dist/docs/3.2.0-rc.0/building/verification/compliance-catalog#error-code-invalid-feed-format) | Feed doesn't match declared `feed_format` | Verify feed content matches the format (XML for google_merchant_center, etc.) |
| [`ITEM_VALIDATION_FAILED`](/dist/docs/3.2.0-rc.0/building/verification/compliance-catalog#error-code-item-validation-failed) | Items failed schema validation | Check `item_issues` for per-item rejection reasons |
| [`CATALOG_LIMIT_EXCEEDED`](/dist/docs/3.2.0-rc.0/building/verification/compliance-catalog#error-code-catalog-limit-exceeded) | Account has reached maximum catalog count | Remove unused catalogs or contact seller |
...
- [Catalogs](/dist/docs/3.2.0-rc.0/creative/catalogs) — Complete documentation on catalog types, sourcing, and format requirements
- [Account state](/dist/docs/3.2.0-rc.0/building/by-layer/L2/account-state) — How catalogs fit into the account setup sequence
- [sync_creatives](/dist/docs/3.2.0-rc.0/creative/task-reference/sync_creatives) — Submit creatives that reference synced catalogs
- [Canonical formats](/dist/docs/3.2.0-rc.0/creative/canonical-formats) — Discover format catalog requirements
