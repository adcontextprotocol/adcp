...
already serves [`get_media_buy_delivery`](/dist/docs/3.2.0-rc.0/media-buy/task-reference/get_media_buy_delivery)
...
4. **[`get_reporting_status`](/dist/docs/3.2.0-rc.0/media-buy/task-reference/get_reporting_status)
...
3. **Generate obligations.** When an account installs a delivery
   configuration through [`sync_accounts`](/dist/docs/3.2.0-rc.0/accounts/tasks/sync_accounts),
...
4. **Close the history.** Keep one `ledger_snapshot_id` across every page,
   ...
   a zero-row revision from no revision. Apply managed-resource and receipt
   checks only when the selected tier requires them.
5. **Treat push as a wake-up.** A doorbell starts a status read; it never
   substitutes for one. Poll periodically even when notifications are enabled.

## How to verify

- The **`reporting-core-declaration` universal storyboard** grades capability
  and tier boundaries without a controller. The lifecycle-focused
...
`get_reporting_status` (the current training surface is `3.2-beta.11`); the
...
- **`managed_delivery: true`** adds offerings with a `method` —
  ...
  principal-layer destinations ([`sync_principal`](/dist/docs/3.2.0-rc.0/protocol/sync_principal)).
- **`reconciled_billing: true`** adds `receipt_task`
  ([`sync_reporting_receipts`](/dist/docs/3.2.0-rc.0/media-buy/task-reference/sync_reporting_receipts)),
