...
|---|---|---|---|
| **Beta** | 3.2 (current prerelease: `beta.11`) | `"3.2-beta.11"` | Development and staging only. Beta.11 is the current protocol checkpoint; TypeScript SDK beta.29 and Python SDK beta.12 embed it exactly. Follow the [3.2 beta program](/dist/docs/3.2.0-rc.0/reference/3-2-beta). |
| **Active** | 3.1 (current stable: 3.1.20) | `"3.1"` | Current production minor release. See [What's New in AdCP 3.1](/dist/docs/3.2.0-rc.0/reference/whats-new-in-3-1) for the feature set. |
...
| **End of life** | 2.0.0, 2.1.0, 2.5.0–2.5.3 | — | No patches. Migrate to 3.0. See [v2 sunset](/dist/docs/3.2.0-rc.0/reference/v2-sunset). |
| **Planned** | 4.0 | — | Target in [planned releases](/dist/docs/3.2.0-rc.0/reference/versioning#planned-releases). |
...
Supported patches don't change the wire contract. Upgrading within 3.0.x is always safe, and 3.1 remains additive over 3.0. The withdrawn 3.1.3 release is not a supported patch or implementation target. For per-version detail, see [Release Notes](/dist/docs/3.2.0-rc.0/reference/release-notes) (curated narrative) and the [Changelog](/dist/docs/3.2.0-rc.0/reference/changelog) (full technical history).

### Prereleases

| Version | Released | Notes |
|---|---|---|
| **3.2.0-beta.11** | 2026-09-03 | Current protocol checkpoint. Adds `get_principal` to the MCP media-buy role profile alongside `sync_principal`, and aligns training-agent and compliance guidance with the beta.10 bundle. TypeScript SDK beta.29 and Python SDK beta.12 embed it exactly; use `"3.2-beta.11"` only with peers that explicitly advertise it. |
...
| 3.0.0-rc.3 | 2026-04-01 | Superseded by GA. See [prerelease upgrade notes](/dist/docs/3.2.0-rc.0/reference/migration/prerelease-upgrades) if you adopted rc.3. |
...
The entire 2.x line has been fully deprecated since August 1, 2026 (UTC). The 2.x schema URLs continue to resolve so existing integrations don't break silently, but those schemas receive no further updates. See [v2 Sunset](/dist/docs/3.2.0-rc.0/reference/v2-sunset) for migration paths.

## FAQ

### What's "current"?
For new production integrations, **3.1.20** is current; pin your integration to `"3.1"` (not `"3.1.20"`). Existing 3.0 integrations can remain pinned to `"3.0"` while they migrate. Use 3.2 only for beta testing with an exact advertised prerelease pin. See [Version negotiation](/dist/docs/3.2.0-rc.0/reference/versioning#version-negotiation).
...
Yes — v2 agents can be registered in the AgenticAdvertising.org registry but cannot be verified. Certification and verified-default discovery require v3.x. See [v2 Sunset → If you use the AgenticAdvertising.org registry](/dist/docs/3.2.0-rc.0/reference/v2-sunset#if-you-use-the-agenticadvertisingorg-registry).

### How long will 3.0 be supported?
At least 12 months after 4.0 GA. See [Versioning → Support window for previous major](/dist/docs/3.2.0-rc.0/reference/versioning#support-window-for-previous-major).

### Where is 4.0?
Targeted for early 2027. 3.x is mid-cycle — see the [planned releases table](/dist/docs/3.2.0-rc.0/reference/versioning#planned-releases).
...
| Implementing or testing 3.2 without an SDK | Use the signed **3.2 beta.11** artifacts with raw-wire or code-generation workflows. Follow the [beta program](/dist/docs/3.2.0-rc.0/reference/3-2-beta). |
| Running an SDK-backed 3.2 integration test | Use `@adcp/sdk@14.0.0-beta.29` or Python `adcp==8.0.0b12` with exact wire pin `"3.2-beta.11"`; use Go only after its support matrix row names an exact checkpoint. |
| Running an existing 3.0 integration | Stay on **3.0** while you migrate, or move to **3.1** when your SDK and validation are ready. |
| Running a 2.5.x integration | Upgrade now — out of support. Start with the [migration guide](/dist/docs/3.2.0-rc.0/reference/migration). |
| Running a 2.0 or 2.1 integration | Upgrade now — out of support. |
| Trying a post-3.1 feature | Use the [3.2 beta program](/dist/docs/3.2.0-rc.0/reference/3-2-beta), [Release Notes](/dist/docs/3.2.0-rc.0/reference/release-notes), and [experimental status](/dist/docs/3.2.0-rc.0/reference/experimental-status). |

## Related

- **[Versioning & Governance](/dist/docs/3.2.0-rc.0/reference/versioning)** — release tiers, schema-change scope, cadence policy
- **[v2 Sunset](/dist/docs/3.2.0-rc.0/reference/v2-sunset)** — full v2 end-of-life timeline and migration paths
- **[What's new in v3](/dist/docs/3.2.0-rc.0/reference/whats-new-in-v3)** — feature-by-feature summary of what 3.0 adds
- **[What's New in AdCP 3.1](/dist/docs/3.2.0-rc.0/reference/whats-new-in-3-1)** — final 3.1 feature set and adopter actions
- **[What's new in AdCP 3.2](/dist/docs/3.2.0-rc.0/reference/whats-new-in-3-2)** — beta feature set and adopter actions
- **[3.2 beta program](/dist/docs/3.2.0-rc.0/reference/3-2-beta)** — current artifacts, exact SDK pairing, and beta testing gates
- **[Release Notes](/dist/docs/3.2.0-rc.0/reference/release-notes)** — curated narrative for each version
- **[Changelog](/dist/docs/3.2.0-rc.0/reference/changelog)** — full technical change history
