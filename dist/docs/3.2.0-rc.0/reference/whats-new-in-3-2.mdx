...
<Warning>
**3.2 is in beta.** Beta.11 is the current protocol checkpoint. TypeScript
`@adcp/sdk@14.0.0-beta.29` and Python `adcp==8.0.0b12` embed beta.11. Python
higher-level helpers remain partial, and Go exact support is
still pending. See the [3.2 beta program](/dist/docs/3.2.0-rc.0/reference/3-2-beta) before testing.
</Warning>
...
<CardGroup cols={3}>
  ...
  <Card title="Build it" icon="code" href="/dist/docs/3.2.0-rc.0/building/by-layer/L4/choose-your-sdk">
    Choose an SDK that explicitly embeds the current 3.2 protocol checkpoint.
  </Card>
  <Card title="Adopt it" icon="route" href="/dist/docs/3.2.0-rc.0/reference/migration/3-1-to-3-2">
...
facades. See [Proposal negotiation](/dist/docs/3.2.0-rc.0/media-buy/product-discovery/proposal-negotiation)
...
After authentication, [`sync_principal`](/dist/docs/3.2.0-rc.0/protocol/sync_principal) lets a
...
[`get_principal`](/dist/docs/3.2.0-rc.0/protocol/get_principal) is the side-effect-free repair
...
`overlay_support.daypart_targets`. See [Targeting-aware product discovery](/dist/docs/3.2.0-rc.0/reference/migration/targeting-aware-discovery).
...
See [programmed channels and owner-sold carriage](/dist/docs/3.2.0-rc.0/media-buy/product-discovery/collections-and-installments#programmed-channels-and-owner-sold-carriage).
...
asset-scoped bearer tokens. See [Migrating secured asset access](/dist/docs/3.2.0-rc.0/reference/migration/asset-access).
...
ratcheted floors only as regression guards. See [Known Limitations](/dist/docs/3.2.0-rc.0/reference/known-limitations#conformance-and-testing)
...
| An SDK maintainer | Generate from the signed beta.11 tarball, name the exact supported bundle, and feed accepted protocol corrections into the RC. |
| A compliance operator | Test beta.11 from its signed assets or an SDK that explicitly lists beta.11, such as TypeScript SDK 14 beta.29 or Python SDK 8 beta.12; retain end-to-end integration evidence and do not issue a 3.2 badge from a beta. |

## Start here

1. Read the [3.2 beta program](/dist/docs/3.2.0-rc.0/reference/3-2-beta).
2. Follow [Migrating from 3.1 to 3.2](/dist/docs/3.2.0-rc.0/reference/migration/3-1-to-3-2).
3. Choose the exact package and support level in [Choose your SDK](/dist/docs/3.2.0-rc.0/building/by-layer/L4/choose-your-sdk).
4. Use the [Release Notes](/dist/docs/3.2.0-rc.0/reference/release-notes#version-3-2-0) for the cumulative record.
