...
The 3.2 beta cycle started with a protocol-first checkpoint. Beta.11 is the
current protocol checkpoint. TypeScript SDK `14.0.0-beta.29` and Python SDK
`8.0.0b12` consume the signed beta.11 bundle:
...
| `3.2.0-beta.11` | Current protocol checkpoint | SDK-backed, raw-wire, code-generation, and integration testing |
| SDK wave | `@adcp/sdk@14.0.0-beta.29` and Python `adcp==8.0.0b12` embed `3.2.0-beta.11`; Go remains pending | TypeScript and Python adopters, Go maintainers, and early integration teams |

Earlier betas remain useful as pinned implementation checkpoints. For
SDK-backed testing of the current checkpoint, pair TypeScript SDK beta.29 or
Python SDK beta.12 with protocol beta.11. TypeScript SDK beta.28 and Python SDK
beta.11 remain historical pairings for beta.10. Do not combine a newer wire pin with an
...
| Surface | beta.0 | beta.1 | beta.2 | beta.3 | beta.4 | beta.5 | beta.6 | beta.7 | beta.8 | beta.9 | beta.10 | beta.11 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Protocol artifact | `3.2.0-beta.0` | `3.2.0-beta.1` | `3.2.0-beta.2` | `3.2.0-beta.3` | `3.2.0-beta.4` | `3.2.0-beta.5` | `3.2.0-beta.6` | `3.2.0-beta.7` | `3.2.0-beta.8` | `3.2.0-beta.9` | `3.2.0-beta.10` | `3.2.0-beta.11` |
| Git tag | `v3.2.0-beta.0` | `v3.2.0-beta.1` | `v3.2.0-beta.2` | `v3.2.0-beta.3` | `v3.2.0-beta.4` | `v3.2.0-beta.5` | `v3.2.0-beta.6` | `v3.2.0-beta.7` | `v3.2.0-beta.8` | `v3.2.0-beta.9` | `v3.2.0-beta.10` | `v3.2.0-beta.11` |
| `adcp_version` wire pin | `"3.2-beta.0"` | `"3.2-beta.1"` | `"3.2-beta.2"` | `"3.2-beta.3"` | `"3.2-beta.4"` | `"3.2-beta.5"` | `"3.2-beta.6"` | `"3.2-beta.7"` | `"3.2-beta.8"` | `"3.2-beta.9"` | `"3.2-beta.10"` | `"3.2-beta.11"` |
| Documentation selector | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` | `3.2-beta` |
...
buyer pinned to `"3.2-beta.10"` must not silently negotiate to another beta, and
a stable `"3.2"` pin must not silently negotiate to a beta.

## Use matched SDK betas

The TypeScript SDK has exact beta.11 support. The npm
`latest` tag remains on SDK 13, so opt into the preview explicitly:

```bash
npm install @adcp/sdk@14.0.0-beta.29
```

SDK 14 includes typed caller and server support for the compact media-buy
lifecycle, the beta.11 schemas and compliance assets, version-aware request
...
See [Choose your SDK](/dist/docs/3.2.0-rc.0/building/by-layer/L4/choose-your-sdk) for the support
matrix and install guidance.

Python `adcp==8.0.0b12` carries beta.11 schemas and generated types. Its
...
## Use the beta.11 artifact directly

Download the signed beta.11 protocol bundle rather
than copying schemas from `main` or from the moving `/schemas/3.2.0-rc.0/` path:

```bash
curl -fLO https://adcontextprotocol.org/protocol/3.2.0-beta.11.tgz
curl -fLO https://adcontextprotocol.org/protocol/3.2.0-beta.11.tgz.sha256
shasum -a 256 -c 3.2.0-beta.11.tgz.sha256
tar -xzf 3.2.0-beta.11.tgz
```

The extracted `adcp-3.2.0-beta.11/` directory contains:
...
[Verifying protocol tarballs](/dist/docs/3.2.0-rc.0/reference/verifying-protocol-tarballs) for
...
4. Exercise raw MCP or A2A calls with `adcp_version: "3.2-beta.11"` against a
...
<Note>
**No 3.2 verification badge is currently issued for beta.11.** Treat it as an
...
The SDK compatibility matrix lives in [Choose your SDK](/dist/docs/3.2.0-rc.0/building/by-layer/L4/choose-your-sdk).
The TypeScript and Python rows name exact beta.11 support with different
...
## Current beta.11 checkpoint

Protocol beta.1 through beta.11 are published. TypeScript SDK beta.29 and
Python SDK beta.12 embed the exact beta.11 bundle. Before treating
a test result as beta.11 protocol evidence, confirm that:

- the test uses the signed beta.11 artifact directly;
- both peers advertise `"3.2-beta.11"` before calls use that pin;
- the integration scenarios below run against beta.11 and retain the exact
  bundle version;
...
- Experimental surfaces retain their [experimental-status contract](/dist/docs/3.2.0-rc.0/reference/experimental-status).
...
- whether the problem blocks beta.10 interoperability.
...
- [What's New in AdCP 3.2](/dist/docs/3.2.0-rc.0/reference/whats-new-in-3-2)
- [Migrating from 3.1 to 3.2](/dist/docs/3.2.0-rc.0/reference/migration/3-1-to-3-2)
- [Release Notes](/dist/docs/3.2.0-rc.0/reference/release-notes#version-3-2-0)
- [Versions & Compatibility](/dist/docs/3.2.0-rc.0/reference/versions)
- [Choose your SDK](/dist/docs/3.2.0-rc.0/building/by-layer/L4/choose-your-sdk)
