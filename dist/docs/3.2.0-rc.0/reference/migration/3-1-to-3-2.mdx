...
`@adcp/sdk@14.0.0-beta.29` and Python `adcp==8.0.0b12` embed beta.11 exactly.
...
For the feature narrative, start with [What's New in AdCP 3.2](/dist/docs/3.2.0-rc.0/reference/whats-new-in-3-2).
For prerelease artifacts and SDK timing, use the [3.2 beta program](/dist/docs/3.2.0-rc.0/reference/3-2-beta).
...
Use a maintained [AdCP SDK](/dist/docs/3.2.0-rc.0/building/by-layer/L4/choose-your-sdk) for
...
| 2 | SDK and codegen maintainers | Generate from the signed `3.2.0-beta.11` protocol tarball, not `main` or `/schemas/3.2.0-rc.0/`. |
...
| 8 | Compliance operators | Test beta.11 from its signed assets or pair `@adcp/sdk@14.0.0-beta.29` or Python `adcp==8.0.0b12` with exact beta.11. Retain the exact package and wire versions. |

## Required to claim 3.2 behavior

### Serve and echo the exact release

During beta, prerelease matching is exact. A seller serving beta.11 advertises
`"3.2-beta.11"`. Do not advertise stable `"3.2"` before GA, silently negotiate
...
until 4.0. See [Migrating to `media_buy_status`](/dist/docs/3.2.0-rc.0/reference/migration/media-buy-status)
...
webhook receivers. See [Signed requests](/dist/docs/3.2.0-rc.0/building/by-layer/L1/security#signed-requests-transport-layer).
...
Follow the complete field map in [Targeting-aware product discovery](/dist/docs/3.2.0-rc.0/reference/migration/targeting-aware-discovery).
...
automatically. See [Migrating secured asset access](/dist/docs/3.2.0-rc.0/reference/migration/asset-access).
...
See [Cross-role governance enforcement](/dist/docs/3.2.0-rc.0/reference/migration/cross-role-governance-enforcement).
...
| SDK maintainer | Signed beta.10 bundle ingestion, generated-type/validator tests, exact support matrix entry, and protocol feedback destined for the release candidate. |
| Compliance operator | Matching beta.10 assets and SDK support end to end; no 3.2 badge from a beta checkpoint. |
