...
For the layered model behind this — what each layer contains and what an SDK at each layer should provide — see the [SDK stack reference](/dist/docs/3.2.0-rc.0/building/cross-cutting/sdk-stack).

## Coverage matrix

What "shipped" means at each layer is the [L0–L3 checklist](/dist/docs/3.2.0-rc.0/building/cross-cutting/sdk-stack#what-an-sdk-at-each-layer-should-provide).

*Last updated: 2026-09-03.*

| SDK | Current package | 3.2 beta support | L0 | L1 | L2 | L3 |
|---|---|---|:-:|:-:|:-:|:-:|
| **`@adcp/sdk`** (TypeScript) | `14.0.0-beta.29` | Exact `3.2.0-beta.11` support | ✅ | ✅ | ✅ | ✅ |
| **`adcp`** (Python) | `8.0.0b12` | Exact `3.2.0-beta.11` schemas and types; higher-level helpers remain partial | ✅ | ⚠️ | ⚠️ | ⚠️ |
...
<Warning>
**3.2 packages are prereleases.** TypeScript beta.29 and Python beta.12 support
the exact beta.11 protocol checkpoint.
Neither implies compatibility with a later checkpoint. Go has not published an exact
3.2 support statement yet. See the [3.2 beta program](/dist/docs/3.2.0-rc.0/reference/3-2-beta).
</Warning>
...
For the status of every published protocol version, see [Versions & Compatibility](/dist/docs/3.2.0-rc.0/reference/versions). Migration and archived-version guidance lives under [Release notes & migration](/dist/docs/3.2.0-rc.0/reference/migration/index).
...
community-maintained ports are welcome — see the [Builders Working Group](/dist/docs/3.2.0-rc.0/community/working-group)
and the [Slack community](/dist/docs/3.2.0-rc.0/community/joining-slack).
...
- `@adcp/sdk/testing` — buyer-side storyboard runner (`runStoryboard`, `comply`, `testAgent`) and seller-side controller scaffold (`createComplyController` — see [Get Test-Ready](/dist/docs/3.2.0-rc.0/building/verification/get-test-ready))
...
```bash
npx @adcp/sdk@14.0.0-beta.28 --help
npx @adcp/sdk@14.0.0-beta.28 --save-auth my-agent https://sales.example.com/mcp
npx @adcp/sdk@14.0.0-beta.28 my-agent list_products '{"adcp_version":"3.2-beta.10","account":{"account_id":"account_123"}}'
# or against the built-in public test agent:
npx @adcp/sdk@14.0.0-beta.28 test-mcp list_products '{"adcp_version":"3.2-beta.10"}'
```
...
- **[Build an agent](/dist/docs/3.2.0-rc.0/building/by-layer/L4/build-an-agent)** — server-side L4 path. Skill files + coding agent.
- **[Build a caller](/dist/docs/3.2.0-rc.0/building/by-layer/L4/build-a-caller)** — client-side L4 path. Install, call, handle responses, ingest reporting.
- **[Schemas](/dist/docs/3.2.0-rc.0/building/by-layer/L0/schemas)** — schema bundle, type generation, version pinning.
- **[Migrate from hand-rolled](/dist/docs/3.2.0-rc.0/building/by-layer/L4/migrate-from-hand-rolled)** — already running an AdCP agent built before the SDKs covered much? Swap one layer at a time.
