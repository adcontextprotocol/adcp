...
|---|---|---|
| Retailer / retail media network | Sales agent (MCP server) | [`adagents.json`](/dist/docs/3.0.0-beta.3/media-buy/advanced-topics/accounts-and-security) |
| Advertiser account with retailer | `account_id` + `list_accounts` | [Accounts & Agents](/dist/docs/3.0.0-beta.3/building/integration/accounts-and-agents) |
| Brand's product catalog | `brand_manifest.product_catalog` | [Brand Manifest](/dist/docs/3.0.0-beta.3/creative/brand-manifest) |
| GTIN / SKU selection for promotion | `product_selectors` (`manifest_gtins`, `manifest_skus`) | [Promoted Offerings](/dist/docs/3.0.0-beta.3/creative/promoted-offerings) |
| Sponsored product listing | Product with catalog-rendered creative | [Creative Formats](/dist/docs/3.0.0-beta.3/creative/formats) |
| On-site display / video | Product with standard `format_ids` | [Media Products](/dist/docs/3.0.0-beta.3/media-buy/product-discovery/media-products) |
| Retailer first-party audience | Brief-based; named segments via `data_provider_signals` | [Targeting](/dist/docs/3.0.0-beta.3/media-buy/advanced-topics/targeting) |
| ROAS target / max conversions | `optimization_goal` on package | [Optimization & Reporting](/dist/docs/3.0.0-beta.3/media-buy/media-buys/optimization-reporting) |
| Closed-loop measurement | `measurement` + `conversion_tracking` on product | [Delivery Reporting](/dist/docs/3.0.0-beta.3/media-buy/task-reference/get_media_buy_delivery) |
| Attribution window (14d click, 1d view) | `attribution_window` in delivery response | [Delivery Reporting](/dist/docs/3.0.0-beta.3/media-buy/task-reference/get_media_buy_delivery) |
| ROAS / attributed revenue | `roas`, `conversion_value` in delivery metrics | [Delivery Reporting](/dist/docs/3.0.0-beta.3/media-buy/task-reference/get_media_buy_delivery) |
| In-store attribution | `by_action_source` with `in_store` | [Delivery Reporting](/dist/docs/3.0.0-beta.3/media-buy/task-reference/get_media_buy_delivery) |

Each retailer is a separate sales agent. Their media offerings are modeled as products. The buyer's brand manifest carries the product catalog for SKU-level creative rendering. Account relationships between brands and retailers are managed via `list_accounts` and `account_id` on media buys — see [Accounts & Agents](/dist/docs/3.0.0-beta.3/building/integration/accounts-and-agents).
...
The `catalog_match` field tells the buyer which of their GTINs are eligible on this retailer. Buyers use these values in `product_selectors.manifest_gtins` when creating media buys — the same way `publisher_properties` lists available properties for refinement. Sellers can include `matched_gtins`, `matched_skus`, or both.
...
Digital screens in physical retail locations — checkout lanes, end-caps, and entrance displays. These bridge the digital-physical gap, with measurement tied to in-store purchases.
...
### Brand Manifest with Product Catalog

The buyer provides a brand manifest that includes their product catalog feed. This enables SKU-level targeting and catalog-rendered creatives:
...
Query `get_products` with `channels: ["retail_media"]` to find commerce media products. The brand manifest enables the seller to filter by catalog eligibility:
...
This tells the buyer which GTINs are eligible on this retailer and how many of their total catalog items were evaluated. Buyers use `matched_gtins` values in `product_selectors.manifest_gtins` when creating media buys — or use broader selectors like `manifest_tags` and `manifest_category` knowing the catalog coverage.

<Tip>
Sellers can also return [proposals](/dist/docs/3.0.0-beta.3/media-buy/product-discovery/media-products#proposals) with recommended budget allocations across product types — encoding media planning expertise that traditionally required human sales reps.
</Tip>
  ...
  "packages": [
    {
      ...
      "pacing": "even",
      "promoted_offerings": {
        "brand_manifest": "https://kraftheinz.com/manifest.json",
        "product_selectors": {
          "manifest_gtins": ["00013000006408", "00013000006415", "00013000006422"],
...
The `account_id` identifies the brand's billing relationship with this retailer. The sponsored products package uses `product_selectors` to specify which items to promote and `optimization_goal` to tell the retailer to optimize for purchases at a 3x ROAS target. The display and premium packages use standard creative workflows.
...
2. **Keep your product catalog current** — catalog freshness directly affects sponsored product eligibility and performance
3. **Use `optimization_goal`** — set `target_roas` or `target_cpa` on packages to let the retailer optimize delivery against their purchase data
4. **Use `product_selectors` strategically** — `manifest_gtins` for specific products, `manifest_tags` or `manifest_category` for broad promotions
5. **Budget across the funnel** — sponsored products for conversion, on-site display for awareness, off-site for reach extension
6. **Compare attribution windows** — use `attribution_window` in delivery reports to normalize ROAS across retailers with different lookback windows
...
2. **Declare `conversion_tracking`** with `platform_managed: true` and list `supported_optimization_strategies` so buyers know they can set `optimization_goal` on packages
3. **Include `attribution_window`** in delivery responses — buyers comparing across retailers need to know your lookback windows and model
4. **Report `by_action_source`** to show omnichannel impact — website, app, and in-store conversions
5. **Return `catalog_match`** on sponsored product listings so buyers can see which GTINs/SKUs are eligible
6. **Set `channels: ["retail_media"]`** on all commerce media products so buyers can filter by channel

## Related Documentation

- [Accounts & Agents](/dist/docs/3.0.0-beta.3/building/integration/accounts-and-agents) — Account setup and billing relationships
- [Media Channel Taxonomy](/dist/docs/3.0.0-beta.3/reference/media-channel-taxonomy) — `retail_media` channel definition
- [Media Products](/dist/docs/3.0.0-beta.3/media-buy/product-discovery/media-products) — Product model reference
- [Promoted Offerings](/dist/docs/3.0.0-beta.3/creative/promoted-offerings) — SKU selection and catalog-rendered creatives
- [Brand Manifest](/dist/docs/3.0.0-beta.3/creative/brand-manifest) — Product catalog and brand identity
- [Pricing Models](/dist/docs/3.0.0-beta.3/media-buy/advanced-topics/pricing-models) — CPC, CPM, flat rate details
- [Optimization & Reporting](/dist/docs/3.0.0-beta.3/media-buy/media-buys/optimization-reporting) — `optimization_goal` and conversion optimization
- [Delivery Reporting](/dist/docs/3.0.0-beta.3/media-buy/task-reference/get_media_buy_delivery) — Commerce attribution metrics
- [Targeting](/dist/docs/3.0.0-beta.3/media-buy/advanced-topics/targeting) — Brief-based targeting approach
