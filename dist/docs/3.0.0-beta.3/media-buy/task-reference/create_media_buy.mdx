...
npx adcp \
  ...
  create_media_buy \
  '{"buyer_ref":"summer_campaign_2025","brand_manifest":{"name":"Nike","url":"https://nike.com"},"packages":[{"buyer_ref":"display_package_1","product_id":"prod_d979b543","pricing_option_id":"cpm_usd_fixed","format_ids":[{"agent_url":"https://creative.adcontextprotocol.org","id":"display_300x250_image"}],"budget":30000,"bid_price":5.00},{"buyer_ref":"display_package_2","product_id":"prod_e8fd6012","pricing_option_id":"cpm_usd_fixed","format_ids":[{"agent_url":"https://creative.adcontextprotocol.org","id":"display_300x250_html"}],"budget":20000,"bid_price":4.50}],"start_time":"2025-06-01T00:00:00Z","end_time":"2025-08-31T23:59:59Z"}' \
...
| `brand_manifest` | BrandManifestRef | Yes | Brand identity - minimal (name/URL) acceptable for sales agents. See [Brand Manifest](/dist/docs/3.0.0-beta.3/creative/brand-manifest) |
...
| `bid_price` | number | No | Bid price for auction pricing (required when pricing option has no `fixed_price`) |
| `optimization_goal` | OptimizationGoal | No | Conversion optimization goal - event source, event type, and optional target ROAS or CPA. Product must declare support in `conversion_tracking.supported_optimization_strategies`. See [Pricing Models](/dist/docs/3.0.0-beta.3/media-buy/advanced-topics/pricing-models). |
| `targeting_overlay` | TargetingOverlay | No | Additional targeting criteria (see [Targeting](/dist/docs/3.0.0-beta.3/media-buy/advanced-topics/targeting)) |
...
Set a ROAS target for conversion-optimized delivery. The product must declare support in `conversion_tracking.supported_optimization_strategies`, and you must have an event source configured via `sync_event_sources`:
...
const result = await testAgent.createMediaBuy({
  ...
  packages: [{
    ...
    bid_price: 1.20,
    optimization_goal: {
      event_source_id: 'retailer_sales',
      event_type: 'purchase',
      target_roas: 4.0
    }
  }],
...
if ('media_buy_id' in validated) {
  console.log(`Campaign ${validated.media_buy_id} created with ROAS target`);
}
```
...
async def create_optimized_campaign():
    ...
    print(f"Campaign {result.media_buy_id} created with ROAS target")
...
See [Proposals](/dist/docs/3.0.0-beta.3/media-buy/product-discovery/media-products#proposals) for the complete workflow including conversational refinement.
...
Full manifests with colors, fonts, and product catalogs are only needed for creative generation. See [Brand Manifest](/dist/docs/3.0.0-beta.3/creative/brand-manifest).
...
Packages can use different currencies when sellers support it. See [Pricing Models](/dist/docs/3.0.0-beta.3/media-buy/advanced-topics/pricing-models).
...
See [Targeting](/dist/docs/3.0.0-beta.3/media-buy/advanced-topics/targeting) for details.
...
**Note:** For the complete status list see [Task Lifecycle](/dist/docs/3.0.0-beta.3/building/implementation/task-lifecycle).
...
For complete async handling patterns, see [Async Operations](/dist/docs/3.0.0-beta.3/building/implementation/async-operations).
...
- **Inline creatives**: The `creatives` array creates NEW creatives only. To update existing creatives, use [`sync_creatives`](/dist/docs/3.0.0-beta.3/media-buy/task-reference/sync_creatives). To assign existing library creatives, use `creative_assignments` instead.
...
1. **Upload Creatives**: Use [`sync_creatives`](/dist/docs/3.0.0-beta.3/media-buy/task-reference/sync_creatives) before deadline
2. **Monitor Status**: Use [`get_media_buy_delivery`](/dist/docs/3.0.0-beta.3/media-buy/task-reference/get_media_buy_delivery)
3. **Optimize**: Use [`provide_performance_feedback`](/dist/docs/3.0.0-beta.3/media-buy/task-reference/provide_performance_feedback)
4. **Update**: Use [`update_media_buy`](/dist/docs/3.0.0-beta.3/media-buy/task-reference/update_media_buy) to modify campaign

## Learn More

- [Media Buy Lifecycle](/dist/docs/3.0.0-beta.3/media-buy/media-buys/) - Complete campaign workflow
- [get_products](/dist/docs/3.0.0-beta.3/media-buy/task-reference/get_products) - Discover inventory
- [Targeting](/dist/docs/3.0.0-beta.3/media-buy/advanced-topics/targeting) - Targeting strategies
- [Pricing Models](/dist/docs/3.0.0-beta.3/media-buy/advanced-topics/pricing-models) - Currency and pricing
