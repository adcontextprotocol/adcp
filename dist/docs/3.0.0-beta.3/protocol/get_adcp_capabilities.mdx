...
```json
{
  "$schema": "/schemas/3.0.0-beta.3/protocol/get-adcp-capabilities-response.json",
...
Extension schemas are published in the [AdCP extension registry](/dist/docs/3.0.0-beta.3/building/integration/context-sessions#extensions). When an agent declares support for an extension, buyers know to look for and process `ext.{namespace}` data in responses.
...
1. **Filter products**: Use [`get_products`](/dist/docs/3.0.0-beta.3/media-buy/task-reference/get_products) with capability-aware filters
2. **Validate properties**: Fetch publisher `adagents.json` files for property definitions
3. **Create buys**: Use [`create_media_buy`](/dist/docs/3.0.0-beta.3/media-buy/task-reference/create_media_buy) with supported features

## Learn More

- [adagents.json Specification](/dist/docs/3.0.0-beta.3/governance/property/adagents) - Publisher authorization files
- [Product Filters](/dist/docs/3.0.0-beta.3/media-buy/task-reference/get_products#filters) - Capability-aware filtering
- [Content Standards](/dist/docs/3.0.0-beta.3/governance/content-standards) - Brand safety configuration
