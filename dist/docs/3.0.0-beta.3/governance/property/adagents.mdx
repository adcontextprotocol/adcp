...
**`properties`** *(optional)*: Array of properties covered by this file (canonical property definitions)
  - **`supported_channels`** *(optional)*: Advertising channels this property supports (e.g., `["display", "olv", "social"]`). See [Media Channel Taxonomy](/dist/docs/3.0.0-beta.3/reference/media-channel-taxonomy).
...
sequenceDiagram
    ...
    Buyer->>SustAgent: create_property_list(filters, brand_manifest)
    Buyer->>QualAgent: create_property_list(filters, brand_manifest)
```
...
Governance agents can include vendor-specific data in feature definitions via an `ext` block. See [get_adcp_capabilities](/dist/docs/3.0.0-beta.3/protocol/get_adcp_capabilities) for details.
...
1. **Integrate with Product Discovery**: Use [`get_products`](/dist/docs/3.0.0-beta.3/media-buy/task-reference/get_products) to discover inventory
2. **Validate at Purchase**: Check authorization before calling [`create_media_buy`](/dist/docs/3.0.0-beta.3/media-buy/task-reference/create_media_buy)
...
- [get_adcp_capabilities](/dist/docs/3.0.0-beta.3/protocol/get_adcp_capabilities) - Discover agent capabilities and portfolio
