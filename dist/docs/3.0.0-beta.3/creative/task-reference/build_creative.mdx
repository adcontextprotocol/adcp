...
**Precedence**: When `brand` appears both at the top level and inside `creative_manifest.assets.promoted_offerings`, the top-level `brand` is the authoritative source for creative rendering context. The one inside `promoted_offerings` identifies the brand for the offering.
