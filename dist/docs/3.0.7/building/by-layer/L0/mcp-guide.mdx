...
Transport-specific guide for integrating AdCP using the Model Context Protocol. For task handling, status management, and workflow patterns, see [Task Lifecycle](/dist/docs/3.0.7/building/by-layer/L3/task-lifecycle).

## Testing AdCP via MCP

You can test AdCP tasks using the [CLI tools](/dist/docs/3.0.7/building/by-layer/L4/choose-your-sdk#cli-tools) or by chatting with [Addie](https://agenticadvertising.org), the AgenticAdvertising.org assistant.
...
The MCP Task tracks status within the session. If the session ends before the task completes, the webhook delivers the result independently. See [Push Notifications](/dist/docs/3.0.7/building/by-layer/L3/webhooks) for webhook payload formats and authentication.
...
See [Push Notifications](/dist/docs/3.0.7/building/by-layer/L3/webhooks) for payload formats and authentication.
...
- Declare which typed extensions you support (see [Context & Sessions](/dist/docs/3.0.7/building/by-layer/L2/context-sessions#extension-fields-ext))
...
AdCP errors are returned as tool-level responses with `isError: true` and the error in `structuredContent.adcp_error`. For the full extraction logic and JSON-RPC transport codes, see [Transport Error Mapping](/dist/docs/3.0.7/building/operating/transport-errors).
...
- **Protocol Comparison**: Compare with [A2A integration](/dist/docs/3.0.7/building/by-layer/L0/a2a-guide)
- **Examples**: Find complete workflow examples in Core Concepts

**For status handling, async operations, and clarification patterns, see [Task Lifecycle](/dist/docs/3.0.7/building/by-layer/L3/task-lifecycle) - this guide focuses on MCP transport specifics only.**