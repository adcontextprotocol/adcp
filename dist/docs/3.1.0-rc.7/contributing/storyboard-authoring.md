...
See the full list of seed scenarios and their params in [Compliance test controller — Scenarios](/dist/docs/3.1.0-rc.7/building/by-layer/L3/comply-test-controller#scenarios).
...
See [Context and sessions — Normative echo contract](/dist/docs/3.1.0-rc.7/building/by-layer/L2/context-sessions#normative-echo-contract) for the agent-side rules.

## Asserting on errors

AdCP surfaces errors in two layers (see [Error handling — envelope vs. payload](/dist/docs/3.1.0-rc.7/building/by-layer/L3/error-handling#envelope-vs-payload-errors-the-two-layer-model)). Storyboards MUST assert error shape in a way that works regardless of which layer a conformant agent surfaced the error on.
