...
```javascript
const schema = await fetch(
  'https://adcontextprotocol.org/schemas/3.1.0-rc.7/core/product.json'
);
```
...
See the [schema registry](https://adcontextprotocol.org/schemas/3.1.0-rc.7/index.json) for all available schemas.
...
curl https://adcontextprotocol.org/schemas/3.1.0-rc.7/index.json | jq '.published_version'
```

Check [Release Notes](/dist/docs/3.1.0-rc.7/reference/release-notes) for version history and migration guides.
...
<Card title="Registry API Reference" icon="server" href="/dist/docs/3.1.0-rc.7/registry/index">
