...
- **Request**: [`update-rights-request.json`](https://adcontextprotocol.org/schemas/3.1.9/brand/update-rights-request.json)
- **Response**: [`update-rights-response.json`](https://adcontextprotocol.org/schemas/3.1.9/brand/update-rights-response.json)
...
| `account` | account-ref | Conditional | Account context. Pass when the brand agent has a governance agent bound to this brand+operator via [`sync_governance`](/dist/docs/3.1.9/accounts/tasks/sync_governance) — `update_rights` is a [modification-phase governance trigger](/dist/docs/3.1.9/governance/campaign/specification#spend-commit-invocation), and the brand agent uses `account` to resolve the bound agent when computing the incremental commit delta. When both an inline `governance_context` token and a bound agent are present, the inline token wins. |
...
<CardGroup cols={2}>
  <Card title="acquire_rights" icon="handshake" href="/dist/docs/3.1.9/brand-protocol/tasks/acquire_rights">
