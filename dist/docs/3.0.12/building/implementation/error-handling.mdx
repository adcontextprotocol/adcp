...
| **Level 3** | Use [transport bindings](/dist/docs/3.0.12/building/implementation/transport-errors) to put errors in `structuredContent` (MCP) or artifact `DataPart` (A2A) | Programmatic clients get typed errors without parsing text |
...
- **Transport Bindings**: See [Transport Errors](/dist/docs/3.0.12/building/implementation/transport-errors) for how errors travel over MCP and A2A
- **Task Lifecycle**: See [Task Lifecycle](/dist/docs/3.0.12/building/implementation/task-lifecycle) for status handling
- **Webhooks**: See [Webhooks](/dist/docs/3.0.12/building/implementation/webhooks) for webhook error handling
- **Security**: See [Security](/dist/docs/3.0.12/building/implementation/security) for authentication errors
