...
**Request Schema**: [`https://adcontextprotocol.org/schemas/3.1.0-rc.8/signals/get-signals-request.json`](https://adcontextprotocol.org/schemas/3.1.0-rc.8/signals/get-signals-request.json)
**Response Schema**: [`https://adcontextprotocol.org/schemas/3.1.0-rc.8/signals/get-signals-response.json`](https://adcontextprotocol.org/schemas/3.1.0-rc.8/signals/get-signals-response.json)
...
| `account` | [AccountRef](/dist/docs/3.1.0-rc.8/building/by-layer/L2/accounts-and-agents#account-references) | No | Account for this request. When provided, the signals agent returns per-account pricing options if configured. In wholesale mode, this is the rate-card scope; when omitted the agent returns default rate-card pricing or omits `pricing_options` entirely. |
...
```json
{
  "$schema": "/schemas/3.1.0-rc.8/signals/get-signals-response.json",
  "status": "completed",
...
```json
{
  "$schema": "/schemas/3.1.0-rc.8/signals/get-signals-request.json",
...
When a consumer (storefront, federated marketplace, registry) needs to mirror a signals agent's full priced signals feed, set `discovery_mode: "wholesale"` and omit `signal_spec` / `signal_refs` / deprecated `signal_ids`. Wholesale enumeration is symmetric with [`get_products` `buying_mode: "wholesale"`](/dist/docs/3.1.0-rc.8/media-buy/task-reference/get_products): synchronous, paginated, firm-priced, with partial completion declared via `incomplete[]`.
...
Signals agents declare wholesale support in [`get_adcp_capabilities`](/dist/docs/3.1.0-rc.8/protocol/get_adcp_capabilities):
