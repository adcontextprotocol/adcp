...
**Request Schema**: [`/schemas/3.1.0-rc.8/media-buy/update-media-buy-request.json`](https://adcontextprotocol.org/schemas/3.1.0-rc.8/media-buy/update-media-buy-request.json)
**Response Schema**: [`/schemas/3.1.0-rc.8/media-buy/update-media-buy-response.json`](https://adcontextprotocol.org/schemas/3.1.0-rc.8/media-buy/update-media-buy-response.json)
...
|-----------|------|----------|-------------|
| `account` | [account-ref](/dist/docs/3.1.0-rc.8/building/by-layer/L2/accounts-and-agents#account-references) | Yes | Account that owns this media buy. Pass `{ "account_id": "..." }` or `{ "brand": {...}, "operator": "..." }`. Required for governance checks and account resolution. |
...
| `invoice_recipient` | [BusinessEntity](/dist/docs/3.1.0-rc.8/building/by-layer/L2/accounts-and-agents#billing-entity-and-invoice-recipient) | No | Override who receives the invoice for this buy. The seller MUST validate authorization and include in `check_governance` when governance agents are configured. |
...
| `media_buy_status` | Media buy lifecycle status after the update (present when status changes, e.g., cancellation). Canonical 3.1 field. See [Migration › `media_buy_status`](/dist/docs/3.1.0-rc.8/reference/migration/media-buy-status). |
...
See [Async Operations](/dist/docs/3.1.0-rc.8/building/by-layer/L3/async-operations) for protocol-specific async patterns and examples.
...
- **Inline creatives**: The `creatives` array creates NEW creatives only. To update existing creatives, use [`sync_creatives`](/dist/docs/3.1.0-rc.8/creative/task-reference/sync_creatives). To assign existing library creatives, use `creative_assignments` in the Package Update object.

<Note>
**Campaign Governance — Modification Phase**

When a buyer's account has governance agents configured, sellers MUST call [`check_governance`](/dist/docs/3.1.0-rc.8/governance/campaign/tasks/check_governance) with `media_buy_id`, `planned_delivery`, and `phase: "modification"` before confirming an update. The governance agent validates change magnitude, budget reallocation, and new parameters against the campaign plan.

See the [seller integration guide](/dist/docs/3.1.0-rc.8/building/operating/seller-integration#execution-checks) for the full execution check workflow and code example.
</Note>
...
1. **Verify Changes**: Use [`get_media_buy_delivery`](/dist/docs/3.1.0-rc.8/media-buy/task-reference/get_media_buy_delivery) to confirm updates
2. **Upload New Creatives**: Use [`sync_creatives`](/dist/docs/3.1.0-rc.8/creative/task-reference/sync_creatives) if creative assignments changed
3. **Monitor Performance**: Track impact of changes on campaign metrics
4. **Optimize Further**: Use [`provide_performance_feedback`](/dist/docs/3.1.0-rc.8/media-buy/task-reference/provide_performance_feedback) for ongoing optimization

## Learn More

- [Media Buy Lifecycle](/dist/docs/3.1.0-rc.8/media-buy/media-buys/) - Complete campaign workflow
- [Targeting](/dist/docs/3.1.0-rc.8/media-buy/advanced-topics/targeting) - Targeting overlays and restrictions
- [Async Operations](/dist/docs/3.1.0-rc.8/building/by-layer/L3/async-operations) - Async patterns and status checking
- [create_media_buy](/dist/docs/3.1.0-rc.8/media-buy/task-reference/create_media_buy) - Initial campaign creation
