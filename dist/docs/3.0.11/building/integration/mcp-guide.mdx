...
Transport-specific guide for integrating AdCP using the Model Context Protocol. For task handling, status management, and workflow patterns, see [Task Lifecycle](/dist/docs/3.0.11/building/implementation/task-lifecycle).

## Testing AdCP via MCP

You can test AdCP tasks using the [CLI tools](/dist/docs/3.0.11/building/schemas-and-sdks#cli-tools) or by chatting with [Addie](https://agenticadvertising.org), the AgenticAdvertising.org assistant.
...
**Task Parameters**: See individual task documentation in [Media Buy](/dist/docs/3.0.11/media-buy) and [Signals](/dist/docs/3.0.11/signals/overview) sections.
...
Most chat-based MCP clients (Claude Desktop, Cursor) do not yet support MCP Tasks. If your client doesn't support task-augmented tool calls, use **webhooks** or **polling via `tasks/get`** instead — these work with any MCP client. See [Async Operations](/dist/docs/3.0.11/building/implementation/async-operations) and [Push Notifications](/dist/docs/3.0.11/building/implementation/webhooks) for transport-independent patterns.
...
The MCP Task tracks status within the session. If the session ends before the task completes, the webhook delivers the result independently. See [Push Notifications](/dist/docs/3.0.11/building/implementation/webhooks) for webhook payload formats and authentication.
...
The [`mcp-webhook-payload.json`](https://adcontextprotocol.org/schemas/3.0.11/core/mcp-webhook-payload.json) envelope includes:
...
See [Push Notifications](/dist/docs/3.0.11/building/implementation/webhooks) for payload formats and authentication.
...
<Note>
**Recommended**: Use [`get_adcp_capabilities`](/dist/docs/3.0.11/protocol/get_adcp_capabilities) for runtime capability discovery. The server card extension provides static metadata for tool catalogs and registries.
</Note>
...
- Declare which typed extensions you support (see [Context & Sessions](/dist/docs/3.0.11/building/integration/context-sessions#extension-fields-ext))
- Enable compatibility checks based on version

:::note
The `adcp_version` field in server card metadata is a v2 convention and is not part of the v3 spec. For version negotiation, use [`get_adcp_capabilities`](/dist/docs/3.0.11/protocol/get_adcp_capabilities#version-negotiation) with the `adcp_major_version` field.
:::
...
AdCP errors are returned as tool-level responses with `isError: true` and the error in `structuredContent.adcp_error`. For the full extraction logic and JSON-RPC transport codes, see [Transport Error Mapping](/dist/docs/3.0.11/building/implementation/transport-errors).
...
- **Protocol Comparison**: Compare with [A2A integration](/dist/docs/3.0.11/building/integration/a2a-guide)
- **Examples**: Find complete workflow examples in Core Concepts

**For status handling, async operations, and clarification patterns, see [Task Lifecycle](/dist/docs/3.0.11/building/implementation/task-lifecycle) - this guide focuses on MCP transport specifics only.**