...
**Webhooks** are the recommended approach for `submitted` operations — they work with any transport (MCP, A2A, REST) and handle operations that outlive a single session. See [Push Notifications](/dist/docs/3.0.11/building/implementation/webhooks).
...
The human-in-the-loop patterns in async operations embody the [Embedded Human Judgment](/dist/docs/3.0.11/governance/embedded-human-judgment) framework — human judgment is embedded in system design, not bolted on afterward.
...
- **Webhooks**: See [Webhooks](/dist/docs/3.0.11/building/implementation/webhooks) for push notifications instead of polling
- **Task Lifecycle**: See [Task Lifecycle](/dist/docs/3.0.11/building/implementation/task-lifecycle) for status handling details
- **Orchestrator Design**: See [Orchestrator Design](/dist/docs/3.0.11/building/implementation/orchestrator-design) for production patterns
