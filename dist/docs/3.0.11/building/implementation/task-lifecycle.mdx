...
- **MCP**: Use [MCP Tasks](https://modelcontextprotocol.io/specification/2025-11-25/basic/utilities/tasks) — the client polls via `tasks/get` and retrieves results via `tasks/result` at the protocol level. See [MCP Guide](/dist/docs/3.0.11/building/integration/mcp-guide#async-operations-via-mcp-tasks).
- **A2A**: Use native A2A task lifecycle with SSE streaming. See [A2A Guide](/dist/docs/3.0.11/building/integration/a2a-guide).
- **REST**: Use AdCP's `task_id` with polling or [push notifications](/dist/docs/3.0.11/building/implementation/webhooks).
:::
...
- **MCP**: Use [MCP Tasks](/dist/docs/3.0.11/building/integration/mcp-guide#async-operations-via-mcp-tasks) or poll via `tasks/get`
- **A2A**: Subscribe to SSE stream for real-time updates
- **REST**: Use [push notifications](/dist/docs/3.0.11/building/implementation/webhooks) (recommended) or poll with `task_id`
...
- **Async Operations**: See [Async Operations](/dist/docs/3.0.11/building/implementation/async-operations) for handling different operation types
- **Webhooks**: See [Webhooks](/dist/docs/3.0.11/building/implementation/webhooks) for push notification patterns
- **Error Handling**: See [Error Handling](/dist/docs/3.0.11/building/implementation/error-handling) for error categories and recovery
