...
See [`get_adcp_capabilities`](/dist/docs/3.0.11/protocol/get_adcp_capabilities) for the full capability reference.
...
[Experimental surfaces](/dist/docs/3.0.11/reference/experimental-status) operate under a separate, faster notice window of 6 weeks. The deprecation policy above applies only to stable surfaces.
...
Security-relevant fixes are documented in release notes with a `security` label and land in the current release. Implementations SHOULD upgrade promptly after a security advisory. Older releases within 3.x do not receive routine backports; upgrading to the current release is the expected remediation path. The same posture applies to v2 during its security-only window — see the [v2 sunset page](/dist/docs/3.0.11/reference/v2-sunset) for that timeline.
...
- An entry in the [release notes](/dist/docs/3.0.11/reference/release-notes) with a migration note
- An entry in the [changelog](/dist/docs/3.0.11/reference/changelog)
- A section in the [migration guide](/dist/docs/3.0.11/reference/migration) or a dedicated deep-dive page
...
The v2 sunset is the documented exception to this commitment: v2 predates this policy and lacks account and governance protections that cannot be backported. Its security-only window runs through August 1, 2026 — see the [v2 sunset page](/dist/docs/3.0.11/reference/v2-sunset). Future majors will not invoke this exception.
...
| **`ext.{namespace}`** on any request or response object | Anyone, by declaring a namespace in `extensions_supported` | Namespaced, out-of-band, does not affect baseline interop. Namespaces SHOULD be registered in the [extension registry](/dist/docs/3.0.11/building/integration/context-sessions#extensions). |
...
2. **Experimental addition.** A surface the working group wants to ship for feedback but is not ready to freeze enters the protocol as experimental — see [experimental status](/dist/docs/3.0.11/reference/experimental-status). Experimental surfaces graduate to stable or are removed; there is no permanent experimental state.
...
- **[Roadmap](/dist/docs/3.0.11/reference/roadmap)** — Planned features and milestones
- **[Release Notes](/dist/docs/3.0.11/reference/release-notes)** — What shipped, with migration guides
- **[Changelog](/dist/docs/3.0.11/reference/changelog)** — Technical change history
