...
**Request schema:** [`/schemas/3.1.18/media-buy/control-media-buy-request.json`](https://adcontextprotocol.org/schemas/3.1.18/media-buy/control-media-buy-request.json)
...
When an otherwise valid control would exceed the accepted budget, targeting, or delivery envelope, the seller returns [`REQUOTE_REQUIRED`](/dist/docs/3.1.18/building/verification/compliance-catalog#error-code-requote-required). The buyer reads `accepted_proposal_id` from [`get_media_buys`](/dist/docs/3.1.18/media-buy/task-reference/get_media_buys), passes it to [`refine_proposals`](/dist/docs/3.1.18/media-buy/task-reference/refine_proposals) to create a draft amendment, finalizes that draft into a committed hold, and applies it through [`accept_proposal`](/dist/docs/3.1.18/media-buy/task-reference/accept_proposal). This pointer survives SDK restart and moves atomically after each accepted amendment.
