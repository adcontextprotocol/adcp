...
```json
{
  "$schema": "/schemas/3.1.0-rc.4/creative/get-creative-features-request.json",
  "creative_manifest": {
    "format_id": {
...
{
  "$schema": "/schemas/3.1.0-rc.4/creative/get-creative-features-response.json",
  "status": "completed",
  "results": [
...
Some evaluations (e.g., sandboxed malware scanning) take time. The agent returns `status: "working"` and delivers results via webhook when complete. This uses the standard [async task pattern](/dist/docs/3.1.0-rc.4/building/by-layer/L3/async-operations) — no custom status values needed.
