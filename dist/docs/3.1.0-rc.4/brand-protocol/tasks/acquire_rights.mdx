...
<Note>
**Experimental.** Brand rights lifecycle (`get_rights`, `acquire_rights`, `update_rights`) is part of AdCP 3.0 as an experimental surface — it may change between 3.x releases with at least 6 weeks' notice. Sellers implementing any of these tasks MUST declare `brand.rights_lifecycle` in `experimental_features`. See [experimental status](/dist/docs/3.1.0-rc.4/reference/experimental-status) for the full contract.
</Note>
...
- **Request**: [`acquire-rights-request.json`](https://adcontextprotocol.org/schemas/3.1.0-rc.4/brand/acquire-rights-request.json)
- **Response**: [`acquire-rights-response.json`](https://adcontextprotocol.org/schemas/3.1.0-rc.4/brand/acquire-rights-response.json)
...
| `revocation_webhook` | push-notification-config | Yes | Webhook for revocation notifications. If the rights holder needs to revoke rights, they POST a [revocation-notification](https://adcontextprotocol.org/schemas/3.1.0-rc.4/brand/revocation-notification.json) to this URL. |
...
| `push_notification_config` | push-notification-config | No | Webhook for async status updates if the acquisition requires approval. See [push notifications](/dist/docs/3.1.0-rc.4/building/by-layer/L3/webhooks). |
...
1. **Inline path** — the request carries an intent-phase `governance_context` token on the [protocol envelope](/dist/docs/3.1.0-rc.4/building/by-layer/L1/security). The buyer threads the token explicitly per request.
2. **Bound path** — the request carries `account` (a seller-assigned `account_id`, or `account.brand` + `account.operator` resolving to a bound account), and the brand agent has a governance agent previously bound to that account via [`sync_governance`](/dist/docs/3.1.0-rc.4/accounts/tasks/sync_governance). The brand agent looks up the bound agent without the buyer threading anything per request.
...
Ungoverned requests — neither `governance_context` token nor an account with a bound governance agent — are unaffected by this validation. `estimated_impressions` remains optional in that case (sellers MAY refuse to transact ungoverned plans as a matter of commercial policy, per [Spend-commit invocation](/dist/docs/3.1.0-rc.4/governance/campaign/specification#spend-commit-invocation), but that refusal is independent of this projection rule).
...
3. **Approve**: POST a [`creative-approval-request`](https://adcontextprotocol.org/schemas/3.1.0-rc.4/brand/creative-approval-request.json) to the `approval_webhook` URL, authenticating with the provided credentials. The response is a [`creative-approval-response`](https://adcontextprotocol.org/schemas/3.1.0-rc.4/brand/creative-approval-response.json) with status `approved`, `rejected`, or `pending_review`. If `pending_review`, poll the returned `status_url` for updates (suggested: every 5 minutes, back off to every 30 minutes after 1 hour).
4. **Serve**: Place approved creative via `create_media_buy`, respecting country and date restrictions
5. **Report**: Use [`report_usage`](/dist/docs/3.1.0-rc.4/accounts/tasks/report_usage) with `rights_id` for cap tracking and billing
...
If the rights holder needs to revoke rights (talent controversy, contract violation, etc.), they POST a [`revocation-notification`](https://adcontextprotocol.org/schemas/3.1.0-rc.4/brand/revocation-notification.json) to the buyer's `revocation_webhook`, authenticating with the credentials provided at acquisition time. The notification contains an `idempotency_key` (required, used for deduplication across retries), `rights_id`, `brand_id`, `reason`, and `effective_at` timestamp.

The buyer is responsible for:
- Deduplicating by `idempotency_key` — the same revocation may be delivered multiple times; see [Push Notifications — Reliability](/dist/docs/3.1.0-rc.4/building/by-layer/L3/webhooks#reliability) for the canonical dedup contract
...
The `usage_reporting_url` in the acquired response is a convenience endpoint provided by the rights agent for HTTP-based impression reporting. It accepts the same payload as the [`report_usage`](/dist/docs/3.1.0-rc.4/accounts/tasks/report_usage) MCP task. Use whichever integration is simpler for your pipeline — the MCP tool for agent-to-agent workflows, or the URL for direct HTTP calls from ad servers.

## Renewal and updates

To extend a rights grant, adjust impression caps, change pricing, or pause/resume, use [`update_rights`](/dist/docs/3.1.0-rc.4/brand-protocol/tasks/update_rights). Extended grants receive re-issued generation credentials and an updated `rights_constraint` for re-embedding in manifests.

## Next steps

<CardGroup cols={2}>
  <Card title="update_rights" icon="rotate" href="/dist/docs/3.1.0-rc.4/brand-protocol/tasks/update_rights">
    Extend, adjust, or pause an existing rights grant.
  </Card>
  <Card title="report_usage" icon="chart-line" href="/dist/docs/3.1.0-rc.4/accounts/tasks/report_usage">
    Report impressions against rights grants for billing and cap tracking.
  </Card>
  <Card title="brand.json spec" icon="code" href="/dist/docs/3.1.0-rc.4/brand-protocol/brand-json">
