...
**Response Schema**: [`/schemas/3.1.0-rc.4/account/list-accounts-response.json`](https://adcontextprotocol.org/schemas/3.1.0-rc.4/account/list-accounts-response.json)
...
| `brand` | Brand reference object: `domain` (the [brand registry](/dist/docs/3.1.0-rc.4/brand-protocol/brand-json) house domain) and optional `brand_id` (sub-brand within the house) |
...
| `account_scope` | How the seller scoped this account: `operator`, `brand`, `operator_brand`, or `agent`. See [account scope](/dist/docs/3.1.0-rc.4/building/by-layer/L2/accounts-and-agents#account-scope). |
...
- [sync_accounts](/dist/docs/3.1.0-rc.4/accounts/tasks/sync_accounts) — Sync advertiser accounts with a seller
...
- [Brand protocol](/dist/docs/3.1.0-rc.4/brand-protocol/brand-json) — How vendor agents resolve brand identity from the brand's `domain`
