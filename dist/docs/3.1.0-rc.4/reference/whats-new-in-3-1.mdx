...
description: "Adopter overview of AdCP 3.1 — distributed brand.json, dependency-impact webhooks, wholesale feed mirroring, release-precision version negotiation, brand response signing, canonical creative formats, vendor-attested measurement, action discovery, and more. Additive over 3.0; use exact RC pins during validation and plain 3.1 at GA."
"og:title": "AdCP — What's New in 3.1"
---

<Warning>
**Status: 3.1 release-candidate validation.** The spec is feature-complete for 3.1 and validating through RC artifacts. Pre-release adopters should pin an exact value advertised by the seller's `supported_versions` list, such as `adcp_version: "3.1-rc.4"` for the current release candidate. Use `"3.1"` only after GA.
</Warning>
...
→ Normative spec: [`brand.json` § Distributed publishing](/dist/docs/3.1.0-rc.4/brand-protocol/brand-json#distributed-publishing) · PR [#4505](https://github.com/adcontextprotocol/adcp/pull/4505)
...
→ Spec: [Media Buy Lifecycle § Health & impairments](/dist/docs/3.1.0-rc.4/media-buy/media-buys/lifecycle#health-impairments) · [Snapshot and log contract](/dist/docs/3.1.0-rc.4/protocol/snapshot-and-log) · RFC #2853 · PRs #4588, #4601, #4677, #4685, #4690
...
→ Spec: [Snapshot and log contract](/dist/docs/3.1.0-rc.4/protocol/snapshot-and-log) · [Webhooks § Persistent channel contract](/dist/docs/3.1.0-rc.4/building/by-layer/L3/webhooks#persistent-channel-contract) · RFC #4582 · PRs #4601, #4701, #4730
...
→ Spec: [Wholesale feed webhooks](https://github.com/adcontextprotocol/adcp/blob/main/specs/wholesale-feed-webhooks.md) · [`get_products` § Wholesale feed versioning](/dist/docs/3.1.0-rc.4/media-buy/task-reference/get_products#wholesale-feed-versioning) · [`get_products` § Cache layering](/dist/docs/3.1.0-rc.4/media-buy/task-reference/get_products#cache-layering) · [`get_signals` § Wholesale signals feed](/dist/docs/3.1.0-rc.4/signals/tasks/get_signals#wholesale-signals-feed) · PRs [#4761](https://github.com/adcontextprotocol/adcp/pull/4761) (conditional fetch), [#4762](https://github.com/adcontextprotocol/adcp/pull/4762) (wholesale signals), [#4763](https://github.com/adcontextprotocol/adcp/pull/4763) (feed webhooks), [#4767](https://github.com/adcontextprotocol/adcp/pull/4767) (cluster implementation)
...
→ Spec: [Product discovery § Signal targeting](/dist/docs/3.1.0-rc.4/media-buy/product-discovery/media-products#signal-targeting) · [Targeting § signal_targeting_groups](/dist/docs/3.1.0-rc.4/media-buy/advanced-topics/targeting#signal_targeting_groups) · [`get_signals`](/dist/docs/3.1.0-rc.4/signals/tasks/get_signals) · PR [#5009](https://github.com/adcontextprotocol/adcp/pull/5009)
...
→ Spec: [Canonical formats](/dist/docs/3.1.0-rc.4/creative/canonical-formats) · PRs [#3307](https://github.com/adcontextprotocol/adcp/pull/3307), [#4770](https://github.com/adcontextprotocol/adcp/pull/4770)
...
→ Spec: [Versioning § Version negotiation](/dist/docs/3.1.0-rc.4/reference/versioning#version-negotiation) · PR [#3493](https://github.com/adcontextprotocol/adcp/pull/3493)
...
→ Spec: [Optimization goals § `vendor_metric` kind](/dist/docs/3.1.0-rc.4/media-buy/media-buys/optimization-reporting#vendor-metric-goals) · PRs [#4668](https://github.com/adcontextprotocol/adcp/pull/4668), [#4669](https://github.com/adcontextprotocol/adcp/pull/4669), [#4649](https://github.com/adcontextprotocol/adcp/pull/4649)
...
→ Spec: [Delivery metrics reference](/dist/docs/3.1.0-rc.4/media-buy/task-reference/get_media_buy_delivery) · PRs [#4618](https://github.com/adcontextprotocol/adcp/pull/4618), [#4601](https://github.com/adcontextprotocol/adcp/pull/4601)
...
→ Spec: [Billing measurement](/dist/docs/3.1.0-rc.4/media-buy/advanced-topics/accountability#billing-measurement) · [`report_usage`](/dist/docs/3.1.0-rc.4/accounts/tasks/report_usage) · PRs [#4735](https://github.com/adcontextprotocol/adcp/pull/4735), [#4561](https://github.com/adcontextprotocol/adcp/pull/4561)
...
→ Spec: [Calling an agent § Idempotency](/dist/docs/3.1.0-rc.4/protocol/calling-an-agent) · PRs [#4402](https://github.com/adcontextprotocol/adcp/pull/4402), [#4409](https://github.com/adcontextprotocol/adcp/pull/4409)
...
→ Spec: [TMP IdentityMatch implementation](/dist/docs/3.1.0-rc.4/trusted-match/identity-match-implementation) · PRs [#4070](https://github.com/adcontextprotocol/adcp/pull/4070), [#3687](https://github.com/adcontextprotocol/adcp/pull/3687)
...
→ Spec: [`adagents.json` reference](/dist/docs/3.1.0-rc.4/governance/property/adagents) · PRs [#4504](https://github.com/adcontextprotocol/adcp/pull/4504), [#4173](https://github.com/adcontextprotocol/adcp/pull/4173), [#4536](https://github.com/adcontextprotocol/adcp/pull/4536)

### Compliance suite — capability-gated scenarios

Capability-gated storyboard scenarios let sellers run *only* what they claim. New scenarios for `frequency_cap_enforcement`, `per_creative_attribution`, `metric_mode` + ROAS (using a `contains:` matcher), `audience_buy_flow`, `event_dedup_flow`, and `performance_buy_flow` (capability-gated CPA buys). Plus a new `requires` runtime gate on Storyboards that conditions execution on declared capabilities — no more all-or-nothing scenarios. The full set is enumerated on the [Compliance catalog](/dist/docs/3.1.0-rc.4/building/verification/compliance-catalog).
...
→ See [Release Notes § Version 3.1.0](/dist/docs/3.1.0-rc.4/reference/release-notes#version-3-1-0-unreleased) for the per-PR detail.
...
Pre-GA 3.1 adopters should also update RC-specific integrations: brand verification success responses now require `signed_response`; proposal/action code must remove the temporary `requires_proposal` action mode and use `REQUOTE_REQUIRED` for update repricing; `signal-owned` conformance is discovery-only while `signal-marketplace` remains the activation claim; and measurement catalog discovery remains experimental behind `measurement.core`. These are RC cleanup items before GA, not 3.0 breaking changes.

**Validator obligation for 3.1 SDKs.** The wholesale feed mirroring work tightens one shape within 3.1: `cache_scope` is schema-required on every `get_products` / `get_signals` response (the safety property of the two-layer cache depends on it — see [Cache layering](/dist/docs/3.1.0-rc.4/media-buy/task-reference/get_products#cache-layering)). Pre-3.1 sellers correctly omit the field and remain conformant to their declared version. SDKs that validate strictly against the 3.1 schema MUST select the validator based on the server-declared `adcp_version` (the same release-precision mechanism 3.1 ships in version negotiation): for responses with `adcp_version` starting `3.0`, the 3.1 `cache_scope`-required constraint MUST be relaxed. This is a tightening within 3.1, not a 3.0 break — but SDKs that hardcode the 3.1 schema without version-pinned validation will reject correct 3.0 traffic. Version-pinned validation is the right pattern for every 3.x→3.(x+1) tightening; cache_scope is the first time it's load-bearing.

For the per-PR detail, see [Release Notes § Version 3.1.0](/dist/docs/3.1.0-rc.4/reference/release-notes#version-3-1-0-unreleased). For the version-negotiation cadence and the 3.1 → 3.2 → 4.0 timeline, see [Versioning & Governance § Migration timeline](/dist/docs/3.1.0-rc.4/reference/versioning#migration-timeline).
