...
**Request Schema**: [`/schemas/3.1.0-rc.14/creative/list-creative-formats-request.json`](https://adcontextprotocol.org/schemas/3.1.0-rc.14/creative/list-creative-formats-request.json)
**Response Schema**: [`/schemas/3.1.0-rc.14/creative/list-creative-formats-response.json`](https://adcontextprotocol.org/schemas/3.1.0-rc.14/creative/list-creative-formats-response.json)
...
**Sales agents implementing both protocols** — return their own self-hosted format definitions alongside referenced formats. See [Creative capabilities on sales agents](/dist/docs/3.1.0-rc.14/creative/sales-agent-creative-capabilities).

See [list_creative_formats (Sales Agent)](/dist/docs/3.1.0-rc.14/creative/task-reference/list_creative_formats) for sales agent-specific behavior.
...
| `wcag_level` | string | No | Filter to formats meeting at least this WCAG level: `A`, `AA`, `AAA`. See [Accessibility](/dist/docs/3.1.0-rc.14/creative/accessibility). |
...
See [Format schema](https://adcontextprotocol.org/schemas/3.1.0-rc.14/core/format.json) for complete format object structure.
...
When catalog assets are present, buyers should sync the required catalogs via [`sync_catalogs`](/dist/docs/3.1.0-rc.14/media-buy/task-reference/sync_catalogs) before submitting creatives. See [Catalogs](/dist/docs/3.1.0-rc.14/creative/catalogs) for the full lifecycle.
...
| `input_format_ids` | *(deprecated in 3.1)* Creative formats this format accepts as input manifests (omitted for formats that work from raw assets). Carried on the transformer instead — see [`list_transformers`](/dist/docs/3.1.0-rc.14/creative/task-reference/list_transformers). |
| `output_format_ids` | *(deprecated in 3.1)* Output formats this format can produce (omitted for formats that produce a single fixed output). Carried on the transformer instead — see [`list_transformers`](/dist/docs/3.1.0-rc.14/creative/task-reference/list_transformers). |
...
1. **Build Creatives**: Use [`build_creative`](/dist/docs/3.1.0-rc.14/creative/task-reference/build_creative) to assemble assets into format
2. **Preview**: Use [`preview_creative`](/dist/docs/3.1.0-rc.14/creative/task-reference/preview_creative) to see visual output
3. **Validate**: Use [`sync_creatives`](/dist/docs/3.1.0-rc.14/creative/task-reference/sync_creatives) with `dry_run: true`
4. **Upload**: Use [`sync_creatives`](/dist/docs/3.1.0-rc.14/creative/task-reference/sync_creatives) to upload to an agent-hosted creative library

## Learn More

- [Format Schema](https://adcontextprotocol.org/schemas/3.1.0-rc.14/core/format.json) - Complete format structure
- [Asset Types](/dist/docs/3.1.0-rc.14/creative/asset-types) - Asset specification details
- [Standard Formats](/dist/docs/3.1.0-rc.14/media-buy/capability-discovery/implementing-standard-formats) - IAB-compatible reference formats
