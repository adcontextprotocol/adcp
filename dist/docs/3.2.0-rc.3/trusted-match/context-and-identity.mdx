...
Resolve the provider-attributed targeting tuples through the publisher's [`targeting_kv_mapping`](/dist/docs/3.2.0-rc.3/trusted-match/specification#publisher-targeting-kv-mapping), then set the mapped ad-server keys. Drop any tuple without a mapping.
```
