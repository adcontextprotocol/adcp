...
Transform, generate, or retrieve a creative manifest for a specific format. To render a visual preview of the resulting manifest, pass it to [`preview_creative`](/dist/docs/3.2.0-rc.3/creative/task-reference/preview_creative). Supports three modes:
...
For generation and transformation, `build_creative` takes a creative manifest as input and produces a creative manifest as output. For library retrieval, provide a `creative_id` (from [`list_creatives`](/dist/docs/3.2.0-rc.3/creative/task-reference/list_creatives)) and the agent resolves it from its library.
...
| `transformer_id` | string | No | Select one transformer (discovered via [`list_transformers`](/dist/docs/3.2.0-rc.3/creative/task-reference/list_transformers)) to perform the build. Requested target capabilities MUST be a subset of its `output_capability_ids`. |
...
| `evaluator` | object | No | **Experimental** ([status](/dist/docs/3.2.0-rc.3/reference/experimental-status); feature id `creative.evaluator`). Advisory evaluator ([Evaluator Spec](https://adcontextprotocol.org/schemas/3.2.0-rc.3/core/evaluator-spec.json)) driving a **gate-then-rank** pipeline over the agent's best-of-N: one source form (`exemplars` / account-arranged `evaluator_id` / `agent_url`), an optional hard `feature_requirement[]` **gate** (drop fails — internal pruning of recommended leaves, never a block on an already-produced billable leaf), an explicit `rank_by` ordering (`[{feature_id, direction: maximize\|minimize}]`), and an allowlisted `feature_agent` pointer. Feature discovery uses [`get_adcp_capabilities`](/dist/docs/3.2.0-rc.3/protocol/get_adcp_capabilities) `governance.creative_features`; the evaluator returns those same feature IDs in `variants[].eval.features[]`. `evaluator_id` is not discovered from that catalog; it is a pre-provisioned account preset. External agents (`feature_agent.agent_url` / `agent_url`) MUST be in the seller's `creative_policy.accepted_verifiers[]` — off-list → [`EVALUATOR_AGENT_NOT_ACCEPTED`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-evaluator-agent-not-accepted). Requires `creative.supports_evaluator`; otherwise ignored. Populates the per-leaf `variants[].eval` block. See [`variants[].eval`](#field-descriptions) and [Evaluator authentication](#evaluator-authentication). |
| `refine_from_build_variant_id` | string | No | Refine a prior produced variant: re-build from its `build_variant_id` applying the NL instruction in `message` plus any `config` delta, returning **new** lineage-linked variants (never a mutation). `transformer_id` and target format(s) are inherited from the parent. Composes with `max_variants`/`variant_axis`; mutually exclusive with `max_creatives`. Requires `creative.supports_refinement` — otherwise [`UNSUPPORTED_FEATURE`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-unsupported-feature); an unknown/no-longer-retained ref is [`REFERENCE_NOT_FOUND`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-reference-not-found) (`error.field` = `refine_from_build_variant_id`). See [Refinement](#refinement). |
...
| `consumption` | object | Structured consumption details — `tokens`, `images_generated`, `renders`, `duration_seconds`. See [`creative-consumption.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.3/core/creative-consumption.json). Informational for cost verification; `vendor_cost` is the billing source of truth. |
...
`build_creative.evaluator` selects or calibrates the evaluator; it does not authenticate the evaluator call. Do not put evaluator API keys, bearer tokens, client secrets, `Authorization` values, JWKs, JWKS documents, or JWKS URIs in `evaluator`, `context`, `ext`, or any other payload field. Credential- or trust-material payload keys are non-conforming and should be rejected with [`CREDENTIAL_IN_ARGS`](/dist/docs/3.2.0-rc.3/building/by-layer/L3/error-handling#authentication-and-access).

When `evaluator.agent_url` or `evaluator.feature_agent.agent_url` names an external evaluator, the producing creative agent calls that evaluator's [`get_creative_features`](/dist/docs/3.2.0-rc.3/governance/creative/get_creative_features) endpoint using the normal AdCP transport authentication channel. The evaluator authenticates the creative/seller agent as caller via [RFC 9421 request signing](/dist/docs/3.2.0-rc.3/building/by-layer/L1/security#request-signing) and JWKS discovery, mTLS, or a pre-provisioned Bearer/API-key credential. Payload fields such as `agent_url`, `account`, `context`, and `ext` are not identity assertions and must not be treated as credentials.
...
```json
{
  "$schema": "/schemas/3.2.0-rc.3/media-buy/build-creative-request.json",
...
Retrieve a creative from the agent's library and resolve it into a manifest with ad-serving assets. Use this when you know the `creative_id` from [`list_creatives`](/dist/docs/3.2.0-rc.3/creative/task-reference/list_creatives) and want the creative agent to produce a delivery-ready manifest with tags (HTML, JavaScript, or VAST):
...
```json
{
  "$schema": "/schemas/3.2.0-rc.3/media-buy/build-creative-response.json",
  "status": "completed",
...
<Note>
**Cross-agent workflow**: When creative generation and media buying are handled by different agents, use `build_creative` on the creative agent to produce a manifest with tags, then [`sync_creatives`](/dist/docs/3.2.0-rc.3/creative/task-reference/sync_creatives) on the sales agent to upload it. When the sales agent implements both protocols, this happens on a single endpoint — see [Creative capabilities on sales agents](/dist/docs/3.2.0-rc.3/creative/sales-agent-creative-capabilities).
</Note>
...
The response uses `creative_manifests` instead of singular `creative_manifest`. Each result is a complete canonical manifest, ready for `sync_creatives` or [`preview_creative`](/dist/docs/3.2.0-rc.3/creative/task-reference/preview_creative):
...
You pay for all three takes (`per_unit` × 3); `keep_mode` is advisory. Keeping is a separate trafficking step on the chosen `build_variant_id` — use that id as the `creative_id` when promoting the produced manifest through [`sync_creatives`](/dist/docs/3.2.0-rc.3/creative/task-reference/sync_creatives). The kept variant then lazily earns a creative record whose `creative_id` is the build leaf id and flows to [`report_usage`](/dist/docs/3.2.0-rc.3/accounts/tasks/report_usage) and delivery reporting.
...
The `pricing_option_id` corresponds to one of the options from [`list_creatives`](/dist/docs/3.2.0-rc.3/creative/task-reference/list_creatives#pricing). The buyer passes it in [`report_usage`](/dist/docs/3.2.0-rc.3/accounts/tasks/report_usage) for reconciliation.
...
- **variants[].build_variant_id**: Identifies a single variant — the leaf-level **lineage anchor**. This is its **own namespace** — never reuse a `preview_id` (a [`preview_creative`](/dist/docs/3.2.0-rc.3/creative/task-reference/preview_creative) render) or a served `variant_id` (a delivery-time identifier) here. You traffic a chosen build by passing its `build_variant_id`.
...
- **errors[]**: Advisory (non-terminal) entries on an otherwise-successful build — e.g. a [`BUDGET_CAP_REACHED`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-budget-cap-reached) notice when `budget_status` is `capped`.

<Warning>
You pay for **every produced variant** (`per_unit` × N), not just the ones you keep. Keeping is a separate client act of trafficking the chosen `build_variant_id`(s), typically by using each kept leaf as `creative_id` on [`sync_creatives`](/dist/docs/3.2.0-rc.3/creative/task-reference/sync_creatives). A kept variant lazily earns a creative record when it is added to the library or first trafficked; that `creative_id` is what flows to [`report_usage`](/dist/docs/3.2.0-rc.3/accounts/tasks/report_usage) and delivery reporting. An unkept variant is still billed but never earns a `creative_id`.
</Warning>
...
- Requires the agent to advertise `creative.supports_refinement: true` (it retains produced leaves for an agent-defined window). Agents that retain nothing return [`UNSUPPORTED_FEATURE`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-unsupported-feature); refine a buyer-held manifest instead via the transform path (`creative_manifest` + `message`). An unknown or no-longer-retained ref returns [`REFERENCE_NOT_FOUND`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-reference-not-found) with `error.field` set to `refine_from_build_variant_id`.
...
For these cases, configure a webhook via `push_notification_config` to receive the result. See [Async Operations](/dist/docs/3.2.0-rc.3/building/by-layer/L3/async-operations) and [Push Notifications](/dist/docs/3.2.0-rc.3/building/by-layer/L3/webhooks).
...
2. **Preview**: Use [`preview_creative`](/dist/docs/3.2.0-rc.3/creative/task-reference/preview_creative) to see how it renders (see [preview_creative](/dist/docs/3.2.0-rc.3/creative/task-reference/preview_creative))
3. **Sync**: Use [`sync_creatives`](/dist/docs/3.2.0-rc.3/creative/task-reference/sync_creatives) to traffic the finalized creative
...
| [`VALIDATION_ERROR`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-validation-error) | The `creative_manifest` is malformed or missing required assets for the target format |
| [`CREATIVE_NOT_FOUND`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-creative-not-found) | The `creative_id` does not exist in the agent's library (or in the specified `concept_id`) |
| [`CREATIVE_REJECTED`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-creative-rejected) | The creative failed content-policy, brand-safety, or accessibility review. Accessibility failures SHOULD use `error-details/accessibility-violation.json`. |
| [`COMPLIANCE_UNSATISFIED`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-compliance-unsatisfied) | A required disclosure from the brief cannot be rendered in the target format (e.g., format does not support the required disclosure position) |
