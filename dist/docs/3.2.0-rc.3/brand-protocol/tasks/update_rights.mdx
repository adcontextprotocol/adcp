...
<Note>
**Experimental.** Brand rights lifecycle ([`get_rights`](/dist/docs/3.2.0-rc.3/brand-protocol/tasks/get_rights), [`acquire_rights`](/dist/docs/3.2.0-rc.3/brand-protocol/tasks/acquire_rights), `update_rights`) is part of AdCP 3.0 as an experimental surface — it may change between 3.x releases with at least 6 weeks' notice. Sellers implementing any of these tasks MUST declare `brand.rights_lifecycle` in `experimental_features`. See [experimental status](/dist/docs/3.2.0-rc.3/reference/experimental-status) for the full contract.
</Note>

Modify an existing rights grant — extend dates, adjust impression caps, change pricing, or pause/resume. Parallels [`update_media_buy`](/dist/docs/3.2.0-rc.3/media-buy/task-reference/update_media_buy). Only the fields you provide are updated; omitted fields remain unchanged.

## Schema

- **Request**: [`update-rights-request.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.3/brand/update-rights-request.json)
- **Response**: [`update-rights-response.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.3/brand/update-rights-response.json)
...
|-------|------|----------|-------------|
| `rights_id` | string | Yes | Rights grant identifier from [`acquire_rights`](/dist/docs/3.2.0-rc.3/brand-protocol/tasks/acquire_rights) |
| `account` | account-ref | Conditional | Account context. Pass when the brand agent has a governance agent bound to this brand+operator via [`sync_governance`](/dist/docs/3.2.0-rc.3/accounts/tasks/sync_governance) — `update_rights` is a [modification-phase governance trigger](/dist/docs/3.2.0-rc.3/governance/campaign/specification#spend-commit-invocation), and the brand agent uses `account` to resolve the bound agent when computing the incremental commit delta. When both an inline `governance_context` token and a bound agent are present, the inline token wins. |
...
| `pricing_option_id` | string | No | Switch to a different pricing option from the original [`get_rights`](/dist/docs/3.2.0-rc.3/brand-protocol/tasks/get_rights) offering |
...
<CardGroup cols={2}>
  <Card title="acquire_rights" icon="handshake" href="/dist/docs/3.2.0-rc.3/brand-protocol/tasks/acquire_rights">
