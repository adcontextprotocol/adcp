...
<Warning>
**Reliable Reporting 1.0 is published through `3.2.0-rc.2` and remains
experimental.** RC.2 publishes the opt-in consumer-status sync and readback.
Sellers implementing that extension advertise `consumer_status_task`; it becomes
...
usable. It does not replace [`get_media_buy_delivery`](/dist/docs/3.2.0-rc.3/media-buy/task-reference/get_media_buy_delivery)
as the reporting-data API.

{/* The stable /schemas/v3 alias does not expose this prerelease-only task yet. */}
**Request schema:** [`get-reporting-status-request.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.2/media-buy/get-reporting-status-request.json)

**Response schema:** [`get-reporting-status-response.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.2/media-buy/get-reporting-status-response.json)
...
in flat pagination and `changes_after` checkpoints. Do not infer a later
tier's additions from an earlier baseline.

Not in any published bundle yet — these ship in the next prerelease:
`obligation_counts.consumer_status_pending`, issue `opened_at` / `issue_state` /
`external_ref`, and the `consumer_mismatch_escalation_seconds` and
`operations_contact` capability fields. Validate those against the development
bundle served from `/schemas/3.2.0-rc.3/` rather than the rc.2 pin above.
</Note>
...
[`sync_accounts`](/dist/docs/3.2.0-rc.3/accounts/tasks/sync_accounts) `notification_configs[]`. It is
...
[`sync_reporting_receipts`](/dist/docs/3.2.0-rc.3/media-buy/task-reference/sync_reporting_receipts).
...
[`sync_accounts`](/dist/docs/3.2.0-rc.3/accounts/tasks/sync_accounts) and
[`list_accounts`](/dist/docs/3.2.0-rc.3/accounts/tasks/list_accounts) return
