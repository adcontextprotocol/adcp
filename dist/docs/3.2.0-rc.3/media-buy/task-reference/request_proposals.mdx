...
Sellers declaring `media_buy.outcome_target` in [`get_adcp_capabilities`](/dist/docs/3.2.0-rc.3/protocol/get_adcp_capabilities) accept the structured form in `criteria`:
...
Every returned purchase references the exact `product_id` and `pricing_option_id` whose pricing and forecast the seller used. The returned `proposal_id` is the only proposal linkage needed by [`refine_proposals`](/dist/docs/3.2.0-rc.3/media-buy/task-reference/refine_proposals) and [`decline_proposals`](/dist/docs/3.2.0-rc.3/media-buy/task-reference/decline_proposals). Each ID identifies one immutable commercial snapshot; refinement and finalization mint new IDs instead of adding a second version field. A draft is indicative and does not reserve inventory. Finalize it through `refine_proposals` before passing the resulting committed proposal to [`accept_proposal`](/dist/docs/3.2.0-rc.3/media-buy/task-reference/accept_proposal). The deprecated [`create_media_buy`](/dist/docs/3.2.0-rc.3/media-buy/task-reference/create_media_buy) proposal mode remains the 3.x compatibility adapter.
...
exception for older peers: a valid 2.5, 3.0, or 3.1 [`get_products`](/dist/docs/3.2.0-rc.3/media-buy/task-reference/get_products) brief may
...
- `listed_purchase` means the coordinator successfully re-read the exact
  ...
  [`buy_products`](/dist/docs/3.2.0-rc.3/media-buy/task-reference/buy_products) with those
...
- `legacy_create` means no truthful fence is available. The coordinator can
  ...
  [`create_media_buy`](/dist/docs/3.2.0-rc.3/media-buy/task-reference/create_media_buy), but
