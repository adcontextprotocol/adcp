...
published in `3.2.0-rc.2`. RC.1 implementations do not expose it. Sellers that
...
{/* The stable /schemas/v3 alias does not expose this RC.2-only task yet. */}
...
<Note>
`3.2.0-rc.2` publishes the task itself, not the hardening described on this
page. The `content_mismatch` status and its `mismatch_code`, the
stale-`received` grace window, `obligation_counts.consumer_status_pending`,
...
land in the next prerelease. Until then, validate those against the development
...
[billing authority](/dist/docs/3.2.0-rc.3/media-buy/advanced-topics/billing-authority), not
...
seller MUST reject `consumer` with [`UNSUPPORTED_FEATURE`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-unsupported-feature). Tracked in
