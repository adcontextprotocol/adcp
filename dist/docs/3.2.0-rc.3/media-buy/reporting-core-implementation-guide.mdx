...
4. **[`get_reporting_status`](/dist/docs/3.2.0-rc.3/media-buy/task-reference/get_reporting_status)
...
3. **Generate obligations.** When an account installs a delivery
   configuration through [`sync_accounts`](/dist/docs/3.2.0-rc.3/accounts/tasks/sync_accounts),
...
loop lands in `3.2.0-rc.2`. The public training sales agent serves whichever
...
- **`managed_delivery: true`** adds offerings with a `method` —
  ...
  principal-layer destinations ([`sync_principal`](/dist/docs/3.2.0-rc.3/protocol/sync_principal)).
...
buy — see [Billing Authority](/dist/docs/3.2.0-rc.3/media-buy/advanced-topics/billing-authority)
