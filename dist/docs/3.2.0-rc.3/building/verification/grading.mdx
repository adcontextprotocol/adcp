...
**When to use it:** before flipping any operation to `required_for` in [`get_adcp_capabilities`](/dist/docs/3.2.0-rc.3/protocol/get_adcp_capabilities); when a counterparty reports signature verification failures; when upgrading key algorithms (Ed25519 → P-256 or the reverse).
...
**When to use it:** when [`AUTH_REQUIRED`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-auth-required) errors persist after bearer token configuration; when dynamic client registration returns unexpected responses; when a new seller's OAuth setup fails silently.
...
- [Validate Your Agent](/dist/docs/3.2.0-rc.3/building/verification/validate-your-agent) — storyboard-based protocol compliance testing
- [Authentication](/dist/docs/3.2.0-rc.3/building/by-layer/L2/authentication) — auth model overview, bearer tokens, RFC 9421 introduction
- [Security implementation reference](/dist/docs/3.2.0-rc.3/building/by-layer/L1/security#signed-requests-transport-layer) — full RFC 9421 profile, verifier checklist, key publication rules
