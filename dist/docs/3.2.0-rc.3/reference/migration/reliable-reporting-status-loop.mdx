...
2026. `3.2.0-rc.2` publishes the task as opt-in; making it *required* is what
the notice window governs. Published `3.2.0-rc.1` remains unchanged.
</Warning>
...
6. Use `content_mismatch` only for contract facts the accepted configuration
   ...
   counts belongs to
   [billing authority](/dist/docs/3.2.0-rc.3/media-buy/advanced-topics/billing-authority)
...
uses [`sync_reporting_receipts`](/dist/docs/3.2.0-rc.3/media-buy/task-reference/sync_reporting_receipts)
...
[Billing Authority](/dist/docs/3.2.0-rc.3/media-buy/advanced-topics/billing-authority). Current
...
on re-emission — before validating against current source. No other issue code
is affected.
</Note>
...
[`reporting_core_lifecycle_probe`](/dist/docs/3.2.0-rc.3/building/by-layer/L3/comply-test-controller#reporting_core_lifecycle_probe)
controller fixture, including the `restate_after_received` operation that
restates a revision the caller already reported as `received` and returns the
`stale_received_grace_deadline`, so both sides of the grace boundary are graded
live rather than only in a seller's own integration tests. Cross-principal
non-disclosure still requires an integration test — see
[known limitations](/dist/docs/3.2.0-rc.3/reference/known-limitations#conformance-and-testing).
