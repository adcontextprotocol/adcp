...
| Tier | Entity | Surface |
|---|---|---|
| 1 | **Principal** — who you are, resolved from authenticated transport | `sync_principal` / [`get_principal`](/dist/docs/3.2.0-rc.3/protocol/get_principal) |
| 2 | The principal's standing configuration with this seller | carried on the principal record |
| 3 | **Accounts** — principal × advertiser authority, feeds, schedules | [`sync_accounts`](/dist/docs/3.2.0-rc.3/accounts/tasks/sync_accounts) and domain tasks |

[`get_adcp_capabilities`](/dist/docs/3.2.0-rc.3/protocol/get_adcp_capabilities) states what the seller can support; `sync_principal` states which of it this principal uses; account-level tasks authorize work for particular advertisers.
...
2. **Read [`get_adcp_capabilities`](/dist/docs/3.2.0-rc.3/protocol/get_adcp_capabilities).** Discover `adcp.principal`, its supported sections, event and delivery offerings, and limits.
3. **Verify the resolved identity.** Call [`get_principal`](/dist/docs/3.2.0-rc.3/protocol/get_principal). A `recognized` result returns the existing durable `principal_id` and seller-resolved `principal_kind` without standing configuration. `unconfigured` means stable authentication succeeded but no durable record exists. The read creates neither records nor identifiers.
...
5. **Authorize advertiser accounts separately** through [`sync_accounts`](/dist/docs/3.2.0-rc.3/accounts/tasks/sync_accounts) and account-level tasks. Nothing in steps 1–4 grants account authority.
6. **Bind each account's feeds** to the reusable `destination_ref`s.
7. **Observe readiness.** [`get_principal`](/dist/docs/3.2.0-rc.3/protocol/get_principal) shows destination and declaration state; `principal.changed` keeps it fresh.
...
Only call this task when [`get_adcp_capabilities`](/dist/docs/3.2.0-rc.3/protocol/get_adcp_capabilities) includes both:
...
- Declarations are facts about consumption ability — never preferences or policy. [`get_adcp_capabilities`](/dist/docs/3.2.0-rc.3/protocol/get_adcp_capabilities) stays objective seller facts; the intersection is the negotiated contract.
...
- An empty `webhook_signing_algorithms` intersection while any webhook subscriber is active fails the request with [`UNSUPPORTED_FEATURE`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-unsupported-feature) — fires would be unverifiable.
...
For `notification_configs`, the endpoint proof and webhook-signing rules are identical to [`sync_agent_notification_configs`](/dist/docs/3.2.0-rc.3/protocol/sync_agent_notification_configs). Implementations exposing both tasks maintain one underlying caller-scoped subscriber set.
...
| Code | When | Recovery |
|---|---|---|
| [`AUTH_REQUIRED`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-auth-required) | Anonymous or unstable caller identity. | Authenticate as a stable principal, then retry. |
| [`INVALID_REQUEST`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-invalid-request) | Schema violation, duplicate `destination_id`, credential-shaped `location` or recipient, or oversize section. | Fix the request; do not retry unchanged. |
...
| [`CONFLICT`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-conflict) | `expected_configuration_version` is stale. Nothing was mutated. | Read the current version with [`get_principal`](/dist/docs/3.2.0-rc.3/protocol/get_principal), reconcile, and resubmit under a new `idempotency_key`. |
| [`IDEMPOTENCY_CONFLICT`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-idempotency-conflict) | An `idempotency_key` was reused with a different body. | Use a fresh key for the new logical operation. |
| [`IDEMPOTENCY_IN_FLIGHT`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-idempotency-in-flight) | The original operation for this key is still executing. | Retry the identical request after a short delay. |
| [`IDEMPOTENCY_EXPIRED`](/dist/docs/3.2.0-rc.3/building/verification/compliance-catalog#error-code-idempotency-expired) | The retention window for this key elapsed. | Read current state, then resubmit as a new operation if still needed. |
...
[`sync_agent_notification_configs`](/dist/docs/3.2.0-rc.3/protocol/sync_agent_notification_configs) remains the specialized compatibility task for agents that only support capability-change subscribers. When `capability_changes.notifications.registration_task` is `sync_principal`, clients manage those subscribers through the `notification_configs` section here.

The broader task does not turn [`get_adcp_capabilities`](/dist/docs/3.2.0-rc.3/protocol/get_adcp_capabilities) into caller-specific state. Capabilities remain cacheable seller facts; principal configuration remains authenticated caller state.
