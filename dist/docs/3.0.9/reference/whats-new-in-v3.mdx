...
<Warning>
**v2 is unsupported as of 3.0 GA and fully deprecated on August 1, 2026 (UTC).** See the [v2 sunset page](/dist/docs/3.0.9/reference/v2-sunset) for the timeline and AAO registry policy.
</Warning>
...
| **Brand rights** | No licensing protocol | `get_rights`, `acquire_rights`, `update_rights` with HMAC-authenticated webhooks — shipping [experimental](/dist/docs/3.0.9/reference/experimental-status) in 3.0 |
...
See the [Security implementation guide](/dist/docs/3.0.9/building/implementation/security) for the full threat model, principal roles (brand / operator / agent), and step-by-step verification paths.

<Card title="Security implementation" icon="arrow-right" href="/dist/docs/3.0.9/building/implementation/security">
...
<Card title="Compliance Catalog" icon="arrow-right" href="/dist/docs/3.0.9/building/compliance-catalog">
...
<Card title="Brand Protocol" icon="arrow-right" href="/dist/docs/3.0.9/brand-protocol">
...
<Card title="Brand rights" icon="arrow-right" href="/dist/docs/3.0.9/brand-protocol/tasks/get_rights">
...
<Card title="Collections and installments" icon="arrow-right" href="/dist/docs/3.0.9/media-buy/product-discovery/collections-and-installments">
...
<Card title="Registry API" icon="arrow-right" href="/dist/docs/3.0.9/registry">
...
<Card title="Proposals and forecasting" icon="arrow-right" href="/dist/docs/3.0.9/media-buy/product-discovery/media-products#proposals">
...
<Tip>
**Migrating from v2?** Accounts are entirely new in v3 — there is no v2 equivalent to migrate from. Start with [Accounts and Agents](/dist/docs/3.0.9/building/integration/accounts-and-agents) for the setup guide.
</Tip>
...
| **Account** | Who gets billed? | [Account reference](/dist/docs/3.0.9/building/integration/accounts-and-agents#account-references) — `account_id` from `list_accounts` or natural key |
...
<Card title="Accounts Protocol" icon="arrow-right" href="/dist/docs/3.0.9/accounts/overview">
...
<Card title="Catalogs" icon="arrow-right" href="/dist/docs/3.0.9/creative/catalogs">
...
<Card title="get_adcp_capabilities" icon="arrow-right" href="/dist/docs/3.0.9/protocol/get_adcp_capabilities">
...
<Note>
**Experimental.** Sponsored Intelligence is part of AdCP 3.0 as an experimental surface (feature id `sponsored_intelligence.core`) — session lifecycle, UI components, identity/consent, and capability negotiation may change between 3.x releases with at least 6 weeks' notice. Sellers implementing SI MUST declare `sponsored_intelligence.core` in `experimental_features`. See [experimental status](/dist/docs/3.0.9/reference/experimental-status) for the full contract and the [3.1.0 roadmap](https://github.com/adcontextprotocol/adcp/issues/2201) for planned changes.
</Note>

<Card title="SI Chat Protocol" icon="arrow-right" href="/dist/docs/3.0.9/sponsored-intelligence/si-chat-protocol">
...
<Card title="Data provider guide" icon="arrow-right" href="/dist/docs/3.0.9/signals/data-providers">
...
For the exhaustive rc.2 change list, see the [Release Notes](/dist/docs/3.0.9/reference/release-notes#version-300-rc2) and [CHANGELOG.md](https://github.com/adcontextprotocol/adcp/blob/main/CHANGELOG.md#300-rc2).
...
<Info>
**Upgrading from rc.3?** See [rc.3 → 3.0 prerelease upgrade notes](/dist/docs/3.0.9/reference/migration/prerelease-upgrades) for the breaking changes table, before/after examples, and migration steps.
</Info>

### Specialisms and compliance catalog

Storyboards move into the protocol at `/compliance/{version}/` with a new `specialisms` field on `get_adcp_capabilities`. 21 specialisms roll up to 6 domains; four 3.1 archetypes ship with `status: preview`. See the [Compliance Catalog](/dist/docs/3.0.9/building/compliance-catalog). Renames: `broadcast-platform` → `sales-broadcast-tv`, `social-platform` → `sales-social`. Merge: `property-governance` + `collection-governance` → `inventory-lists`. Promotion: `sponsored_intelligence` specialism → full protocol.
...
See the [prerelease upgrade notes](/dist/docs/3.0.9/reference/migration/prerelease-upgrades#capabilities-model-simplification) for the full list of removed fields and migration steps.
...
<Card title="Collection lists" icon="arrow-right" href="/dist/docs/3.0.9/governance/collection/tasks/collection_lists">
...
<Card title="Broadcast TV" icon="arrow-right" href="/dist/docs/3.0.9/creative/channels/broadcast">
...
A new [Required tasks by protocol](/dist/docs/3.0.9/protocol/required-tasks) reference page consolidates required, conditional, and optional tasks across all AdCP protocols by agent role — a single page to verify your implementation covers the minimum surface.

### Experimental surfaces

AdCP 3.0 ships a core of stable surfaces under the [3.x stability guarantees](/dist/docs/3.0.9/reference/versioning#3x-stability-guarantees), plus four surfaces that are part of the core protocol but not yet frozen. These surfaces are being co-developed with design partners — **OpenAds, Scope3, Yahoo, ONX, and Triton Digital** — running them in production engagements that shape the graduation path. Experimental surfaces carry `x-status: experimental` in their schemas, and sellers implementing them declare the feature id in `experimental_features` on `get_adcp_capabilities`. They may change between 3.x releases with at least 6 weeks' notice.

| Surface | Feature id | Why experimental |
|---|---|---|
| [Brand rights lifecycle](/dist/docs/3.0.9/brand-protocol/tasks/get_rights) | `brand.rights_lifecycle` | Legal-construct surface added late in the 3.0 cycle. First enterprise deployments will expose edge cases in partial rights, sublicensing, revocation, and dispute resolution. |
| [Campaign governance](/dist/docs/3.0.9/governance/campaign/specification) | `governance.campaign` | Multi-party governance semantics (approval conflicts, audit provenance, tie-breaking under Embedded Human Judgment) are not yet settled. |
| [Trusted Match Protocol](/dist/docs/3.0.9/trusted-match/) | `trusted_match.core` | Privacy architecture will evolve with regulator engagement; TMPX exposure tokens, country-partitioned identity, and Offer macros are expected to change. |
| [Sponsored Intelligence](/dist/docs/3.0.9/sponsored-intelligence/overview) | `sponsored_intelligence.core` | Conversational brand experiences are a new advertising model. Session lifecycle, UI components, identity/consent object shape, and capability negotiation are expected to evolve as first-party AI hosts and brand agents integrate. |

The experimental label is deliberately scoped — everything else in 3.0 follows the normal 6-month deprecation notice. See [experimental status](/dist/docs/3.0.9/reference/experimental-status) for the full contract, graduation criteria, and client guidance.
...
<Card title="Channels deep dive" icon="arrow-right" href="/dist/docs/3.0.9/reference/migration/channels">
...
<Card title="Pricing deep dive" icon="arrow-right" href="/dist/docs/3.0.9/reference/migration/pricing">
...
<Card title="Creatives deep dive" icon="arrow-right" href="/dist/docs/3.0.9/reference/migration/creatives">
...
<Card title="Geo targeting deep dive" icon="arrow-right" href="/dist/docs/3.0.9/reference/migration/geo-targeting">
...
Formats now use an `assets` array with `required` boolean instead of `assets_required`. See the [creatives deep dive](/dist/docs/3.0.9/reference/migration/creatives#asset-discovery).
...
<Card title="Catalogs deep dive" icon="arrow-right" href="/dist/docs/3.0.9/reference/migration/catalogs">
...
<Card title="Brand identity deep dive" icon="arrow-right" href="/dist/docs/3.0.9/reference/migration/brand-identity">
...
<Card title="Optimization goals deep dive" icon="arrow-right" href="/dist/docs/3.0.9/reference/migration/optimization-goals">
...
<Card title="Signals deep dive" icon="arrow-right" href="/dist/docs/3.0.9/reference/migration/signals">
...
<Card title="Audiences deep dive" icon="arrow-right" href="/dist/docs/3.0.9/reference/migration/audiences">
...
<Card title="Refinement deep dive" icon="arrow-right" href="/dist/docs/3.0.9/media-buy/product-discovery/refinement">
...
<AccordionGroup>
  <Accordion title="All implementations" defaultOpen>
    ...
    - [ ] Update channel enum values to [new taxonomy](/dist/docs/3.0.9/reference/migration/channels)
    - [ ] Rename `fixed_rate` -> `fixed_price` in [pricing options](/dist/docs/3.0.9/reference/migration/pricing)
    - [ ] Move `price_guidance.floor` -> `floor_price` ([pricing details](/dist/docs/3.0.9/reference/migration/pricing))
    - [ ] Replace `creative_ids` with [`creative_assignments`](/dist/docs/3.0.9/reference/migration/creatives)
    - [ ] Add system specification to [metro/postal targeting](/dist/docs/3.0.9/reference/migration/geo-targeting)
    - [ ] Rename `geo_postal_codes` -> `geo_postal_areas`
    - [ ] Handle new `geo_metros_exclude` and `geo_postal_areas_exclude` fields
    - [ ] Update format parsing to use [`assets` array](/dist/docs/3.0.9/reference/migration/creatives#asset-discovery)
    - [ ] Replace `preview_image` reads with [`format_card`](/dist/docs/3.0.9/reference/migration/creatives#format-cards-replacing-preview_image) rendering
    - [ ] Replace `list_authorized_properties` calls with `get_adcp_capabilities` portfolio
    - [ ] Remove `promoted_offerings` from creative manifest assets and replace with [`catalogs` field](/dist/docs/3.0.9/reference/migration/catalogs)
    - [ ] Remove `promoted_offering` string from media buy and creative manifest objects
    - [ ] Update `optimization_goal` to [`optimization_goals`](/dist/docs/3.0.9/media-buy/media-buys/optimization-reporting) (array of discriminated union)
  ...
  <Accordion title="Seller agents (publishers, SSPs, networks)">
    ...
    - [ ] Handle catalogs synced via [`sync_catalogs`](/dist/docs/3.0.9/reference/migration/catalogs) with approval workflow
  ...
  <Accordion title="Buyer agents and orchestrators (DSPs, agencies, brands)">
    ...
    - [ ] Resolve brands via [`brand.json`](/dist/docs/3.0.9/brand-protocol) before placing buys
    ...
    - [ ] Use the [Registry API](/dist/docs/3.0.9/registry) for brand/property resolution and agent discovery
    ...
    - [ ] Sync catalogs via [`sync_catalogs`](/dist/docs/3.0.9/reference/migration/catalogs) before submitting creatives
  ...
  <Accordion title="Data providers (new in v3)">
    Publish signal catalogs via `adagents.json`. See [Data Provider Guide](/dist/docs/3.0.9/signals/data-providers).
  ...
  <Accordion title="Creative agents (creative management, DCO providers)">
    ...
    - [ ] Support `catalog` field on creative manifests (replaces [`promoted_offerings`](/dist/docs/3.0.9/reference/migration/catalogs) asset)
  ...
  <Accordion title="Brands (new in v3)">
    Establish buy-side identity. See [brand.json specification](/dist/docs/3.0.9/brand-protocol/brand-json).
    ...
    - [ ] Register in the [community brand registry](/dist/docs/3.0.9/registry) if not hosting `brand.json`
  </Accordion>

  <Accordion title="Governance agents (new in v3)">
    Implement brand suitability capabilities. See [Governance Protocol](/dist/docs/3.0.9/governance).
  ...
  <Accordion title="Sponsored Intelligence agents (new in v3 — experimental)">
    Implement conversational brand experiences. SI ships in 3.0 as an [experimental surface](/dist/docs/3.0.9/reference/experimental-status) (`sponsored_intelligence.core`) — session lifecycle, UI components, and capability negotiation may change between 3.x releases with 6 weeks' notice. Sellers MUST declare `sponsored_intelligence.core` in `experimental_features`. See [SI Chat Protocol](/dist/docs/3.0.9/sponsored-intelligence/si-chat-protocol).
