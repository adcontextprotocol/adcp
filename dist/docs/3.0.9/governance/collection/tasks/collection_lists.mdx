...
Collection lists are managed, cacheable artifacts that express "these collections, filtered by these criteria." They parallel [property lists](/dist/docs/3.0.9/governance/property/tasks/property_lists) but operate on content programs (shows, series, podcasts) rather than technical surfaces (domains, apps).
...
**Content ratings are metadata filters, not content evaluation.** `content_ratings_exclude: [{ system: "tv_parental", rating: "TV-MA" }]` excludes all collections *declared* as TV-MA. It doesn't evaluate individual episodes — that's [content standards](/dist/docs/3.0.9/governance/content-standards/index).
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.0.9/collection/create-collection-list-request.json",
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.0.9/collection/create-collection-list-response.json",
  "list": {
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.0.9/collection/get-collection-list-request.json",
  "list_id": "cl_novamotors_dna_2026",
  "resolve": true,
  "pagination": {
    "max_results": 1000
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.0.9/collection/get-collection-list-response.json",
  "list": {
    "list_id": "cl_novamotors_dna_2026",
    "name": "Nova Motors CTV Do Not Air — 2026",
    "collection_count": 247
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.0.9/collection/update-collection-list-request.json",
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.0.9/collection/list-collection-lists-request.json",
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.0.9/collection/delete-collection-list-request.json",
...
Collection lists gate delivery decisions, so the `auth_token` and webhook callbacks need explicit lifecycle rules. General controls in [Security](/dist/docs/3.0.9/building/implementation/security) apply; the collection-list-specific rules:
...
The pattern matches [property list sharing](/dist/docs/3.0.9/governance/property/index#sharing-property-lists-with-sellers):
