...
**Webhooks** are the recommended approach for `submitted` operations — they work with any transport (MCP, A2A, REST) and handle operations that outlive a single session. See [Push Notifications](/dist/docs/3.0.9/building/by-layer/L3/webhooks).
...
- **Webhooks**: See [Webhooks](/dist/docs/3.0.9/building/by-layer/L3/webhooks) for push notifications instead of polling
- **Task Lifecycle**: See [Task Lifecycle](/dist/docs/3.0.9/building/by-layer/L3/task-lifecycle) for status handling details
- **Orchestrator Design**: See [Orchestrator Design](/dist/docs/3.0.9/building/operating/orchestrator-design) for production patterns
