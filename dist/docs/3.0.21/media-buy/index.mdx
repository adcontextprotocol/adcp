...
| Viewability / IVT / completion SLAs | `performance_standards` on packages — see [Accountability](/dist/docs/3.0.21/media-buy/advanced-topics/accountability) |
| Makegoods | `makegood_policy` in `measurement_terms` — see [Accountability](/dist/docs/3.0.21/media-buy/advanced-topics/accountability) |
...
Campaign governance requires the orchestrator to register the campaign plan via [`sync_plans`](/dist/docs/3.0.21/governance/campaign/tasks/sync_plans) before any governance checks. The plan defines authorized parameters — budget limits, channels, flight dates, and compliance policies — against which all subsequent actions are validated. The full governance sequence is `sync_plans` → `check_governance` (proposed) → `create_media_buy` → `check_governance` (committed by seller). See [Campaign Governance](/dist/docs/3.0.21/governance/campaign/index) for the complete specification.
...
See the [Trusted Match Protocol](/dist/docs/3.0.21/trusted-match) for the full specification.
...
- **Product discovery**: [How `get_products` works](/dist/docs/3.0.21/media-buy/product-discovery/media-products) — briefs, wholesale mode, proposals, and refinement
- **Campaign lifecycle**: [Managing media buys](/dist/docs/3.0.21/media-buy/media-buys/index) — status transitions, updates, and approvals
- **Optimization**: [Delivery and reporting](/dist/docs/3.0.21/media-buy/media-buys/optimization-reporting) — metrics, dimensional breakdowns, and feedback loops
- **Governance**: [Campaign governance](/dist/docs/3.0.21/governance/overview) — how the three-party trust model protects Sam's spend
- **Creative**: [Creative walkthrough](/dist/docs/3.0.21/creative/index) — how Maya builds the creatives Sam uses
- **Real-time matching**: [Trusted Match Protocol](/dist/docs/3.0.21/trusted-match) — how packages activate at serve time via Context Match and Identity Match
- **Get certified**: The [Buyer track](/dist/docs/3.0.21/learning/tracks/buyer) teaches the full media buy workflow through interactive modules
