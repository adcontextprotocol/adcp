...
Beyond linear video, the IAB CTV Ad Portfolio's home-screen and on-glass experiences — Menu, Pause, Screensaver, Overlay, Squeezeback, and In-Scene — layer a `ctv_ad_experience` declaration onto `native_in_feed`, `image`, `video_hosted`, `video_vast`, and `sponsored_placement`. See [CTV experience profiles](/dist/docs/3.2.0-beta.11/creative/ctv-experiences) for the experience-to-canonical matrix, per-experience duration and interactivity constraints, and the OpenRTB/AdCOM signaling bridge.

See [Video](/dist/docs/3.2.0-beta.11/creative/channels/video), [Multi-agent orchestration](/dist/docs/3.2.0-beta.11/creative/multi-agent-orchestration), and [Creative delivery](/dist/docs/3.2.0-beta.11/creative/task-reference/get_creative_delivery).
