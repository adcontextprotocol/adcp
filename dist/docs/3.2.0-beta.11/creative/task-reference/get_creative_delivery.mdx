...
This is a Creative Protocol task. Call it on any agent that declares `"creative"` in `supported_protocols` and exposes the `get_creative_delivery` tool — whether that's a dedicated creative service or a [sales agent implementing the Creative Protocol](/dist/docs/3.2.0-beta.11/creative/sales-agent-creative-capabilities).

**Request Schema**: [`/schemas/3.2.0-beta.11/creative/get-creative-delivery-request.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.11/creative/get-creative-delivery-request.json)
**Response Schema**: [`/schemas/3.2.0-beta.11/creative/get-creative-delivery-response.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.11/creative/get-creative-delivery-response.json)
...
|-----------|------|----------|-------------|
| `account` | [account-ref](/dist/docs/3.2.0-beta.11/building/by-layer/L2/accounts-and-agents#account-references) | No | Account reference. Pass `{ "account_id": "..." }` or `{ "brand": {...}, "operator": "..." }` if the seller supports implicit resolution. Limits results to creatives within this account. |
...
Fields available on both `creative.totals` and each `variant` entry. Commonly-used subset — see the [delivery-metrics schema](https://adcontextprotocol.org/schemas/3.2.0-beta.11/core/delivery-metrics.json) for the full list including incrementality, brand lift, and broadcast metrics.
...
| `revision_id` | Buyer-authored input revision from which this execution was derived. Historical rows may name an older revision than the creative currently returned by [`list_creatives`](/dist/docs/3.2.0-beta.11/creative/task-reference/list_creatives). |
...
[`preview_creative`](/dist/docs/3.2.0-beta.11/creative/task-reference/preview_creative) replay by
...
Use [`preview_creative`](/dist/docs/3.2.0-beta.11/creative/task-reference/preview_creative) with `request_type: "variant"` to see what a specific variant looked like when served:
...
|---|---|---|
| [`get_media_buy_delivery`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/get_media_buy_delivery) | Media Buy | WHERE and HOW MUCH: impressions, spend, placement data, optional `by_creative` breakdown |
...
When a sales agent implements both protocols, both tasks are available on the same agent URL. See [Creative capabilities on sales agents](/dist/docs/3.2.0-beta.11/creative/sales-agent-creative-capabilities) for the full pattern.
...
5. **Group by `concept_id`**: For campaign-level roll-ups, use `concept_id` to group related creatives across sizes and sellers. Pull the concept-to-creative mapping from [`list_creatives`](/dist/docs/3.2.0-beta.11/creative/task-reference/list_creatives) on each agent.
...
Agents that support this task expose `get_creative_delivery` in their tool surface and declare Creative Protocol support through [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.11/protocol/get_adcp_capabilities):
