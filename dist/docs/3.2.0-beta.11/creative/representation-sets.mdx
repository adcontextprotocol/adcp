...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.9/core/creative-representation-set.json",
...
5. Normalize tracker defaults, bind one exact VAST/DAAST execution version, and match every first-class tracker against the destination's [tracker execution contract](/dist/docs/3.2.0-beta.11/creative/tracker-execution-contracts).
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.11/core/creative-manifest.json",
  "format_kind": "display_tag",
  "format_option_ref": {
    "scope": "product",
    "format_option_id": "acme_internal_redirect_300x250"
...
When no candidate works, return `CREATIVE_REPRESENTATION_UNRESOLVED`. `error.details` MUST conform to [`error-details/creative-representation-unresolved.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.9/error-details/creative-representation-unresolved.json), with exactly one `representation_rejections` entry for every candidate. Unsupported representations remain in the immutable set; failure does not delete or rewrite them.
...
- [Display ads](/dist/docs/3.2.0-beta.11/creative/channels/display)
- [Video ads](/dist/docs/3.2.0-beta.11/creative/channels/video)
- [Universal macros](/dist/docs/3.2.0-beta.11/creative/universal-macros)
- [Creative manifests](/dist/docs/3.2.0-beta.11/creative/creative-manifests)
