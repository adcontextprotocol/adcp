...
This flow applies only when [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.11/protocol/get_adcp_capabilities) returns `account.require_operator_auth: false`. In that model, the seller trusts the authenticated buyer agent to declare a brand and operator through [`sync_accounts`](/dist/docs/3.2.0-beta.11/accounts/tasks/sync_accounts).

If `require_operator_auth` is `true`, stop here. The operator authenticates directly and the buyer discovers a seller-assigned `account_id` through [`list_accounts`](/dist/docs/3.2.0-beta.11/accounts/tasks/list_accounts) or receives it during out-of-band onboarding. Do not attempt natural-key provisioning unless a future capability explicitly declares it.
...
**Schema:** [`sync-accounts-request.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.11/account/sync-accounts-request.json)
...
**Schema:** [`sync-accounts-response.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.11/account/sync-accounts-response.json)

```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.11/account/sync-accounts-response.json",
  "status": "completed",
  "accounts": [
    {
...
**Schema:** [`account-status-changed-webhook.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.11/core/account-status-changed-webhook.json)

```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.11/core/account-status-changed-webhook.json",
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.11/account/list-accounts-request.json",
  "account": {
    "account_id": "acct_streamhaus_nova_pinnacle"
  }
}
...
**Schema:** [`list-accounts-response.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.11/account/list-accounts-response.json)

```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.11/account/list-accounts-response.json",
  "status": "completed",
  "accounts": [
    {
      "account_id": "acct_streamhaus_nova_pinnacle",
...
Once the authoritative snapshot is `active`, Sam can call [`create_media_buy`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/create_media_buy). Because this is a buyer-declared account, the request uses the durable natural-key reference:
...
An optional `invoice_recipient` can override the account's default billing entity for this media buy. The seller validates that the caller is authorized to use the recipient. When governance agents are configured, the seller includes the override in [`check_governance`](/dist/docs/3.2.0-beta.11/governance/campaign/tasks/check_governance) so the billing redirect can be approved or rejected. See [Billing entity and invoice recipient](/dist/docs/3.2.0-beta.11/building/by-layer/L2/accounts-and-agents#billing-entity-and-invoice-recipient).
...
- [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.11/protocol/get_adcp_capabilities) — account model, supported billing, and durable-notification discovery
- [`sync_accounts`](/dist/docs/3.2.0-beta.11/accounts/tasks/sync_accounts) — provisioning fields, setup results, and account-level subscriptions
- [`list_accounts`](/dist/docs/3.2.0-beta.11/accounts/tasks/list_accounts) — authoritative state and webhook repair
- [Accounts and agents](/dist/docs/3.2.0-beta.11/building/by-layer/L2/accounts-and-agents) — account references, billing roles, and authorization models
- [`create_media_buy`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/create_media_buy) — the first spend operation after activation
- [`check_governance`](/dist/docs/3.2.0-beta.11/governance/campaign/tasks/check_governance) — governance review of campaign-specific invoice recipients
