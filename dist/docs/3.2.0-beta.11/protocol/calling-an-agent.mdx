...
The agent-facing version of this content lives at [`skills/call-adcp-agent/SKILL.md`](https://github.com/adcontextprotocol/adcp/blob/main/skills/call-adcp-agent/SKILL.md) — bundled into the [protocol tarball](/dist/docs/3.2.0-beta.11/building/by-layer/L0/schemas#one-shot-protocol-bundle) so SDKs can ship it to coding agents.
...
2. **[`get_adcp_capabilities`](/dist/docs/3.2.0-beta.11/protocol/get_adcp_capabilities)**: returns supported protocols, AdCP major versions, and feature flags. Tells you *which* tools this agent supports, not how to call them. See [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.11/protocol/get_adcp_capabilities).
3. **[`sync_principal`](/dist/docs/3.2.0-beta.11/protocol/sync_principal)** *(when advertised)*: declares this authenticated caller's durable connection resources, such as agent-level webhooks and reusable reporting destinations. This is caller state, not capability discovery and not advertiser-account authorization.
...
- **Same key, different canonical body** → [`IDEMPOTENCY_CONFLICT`](/dist/docs/3.2.0-beta.11/building/verification/compliance-catalog#error-code-idempotency-conflict). Sellers MUST reject (rule 5 in [security.mdx#idempotency](/dist/docs/3.2.0-beta.11/building/by-layer/L1/security#idempotency)) — do not silently apply the second body, do not silently replay the first response.
- **Same key while first request still running** → [`IDEMPOTENCY_IN_FLIGHT`](/dist/docs/3.2.0-beta.11/building/verification/compliance-catalog#error-code-idempotency-in-flight) (rule 9 in [security.mdx#idempotency](/dist/docs/3.2.0-beta.11/building/by-layer/L1/security#idempotency)). The seller MAY return this code with top-level `error.retry_after` instead of blocking. Wait and retry with the **same key** — minting a fresh key on this code turns a safe retry into a double-execution race.
...
The compact 3.2 lifecycle requires `idempotency_key` on [`request_proposals`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/request_proposals), [`refine_proposals`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/refine_proposals), [`decline_proposals`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/decline_proposals), [`buy_products`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/buy_products), [`accept_proposal`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/accept_proposal), and [`control_media_buy`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/control_media_buy). [`list_products`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/list_products) remains a synchronous key-optional read.

`idempotency_key` is required on: [`create_media_buy`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/create_media_buy), [`update_media_buy`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/update_media_buy), [`sync_creatives`](/dist/docs/3.2.0-beta.11/creative/task-reference/sync_creatives), [`sync_audiences`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/sync_audiences), [`sync_accounts`](/dist/docs/3.2.0-beta.11/accounts/tasks/sync_accounts), [`sync_principal`](/dist/docs/3.2.0-beta.11/protocol/sync_principal), [`sync_agent_notification_configs`](/dist/docs/3.2.0-beta.11/protocol/sync_agent_notification_configs), [`sync_catalogs`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/sync_catalogs), [`sync_event_sources`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/sync_event_sources), [`sync_plans`](/dist/docs/3.2.0-beta.11/governance/campaign/tasks/sync_plans), [`sync_governance`](/dist/docs/3.2.0-beta.11/accounts/tasks/sync_governance), [`activate_signal`](/dist/docs/3.2.0-beta.11/signals/tasks/activate_signal), [`acquire_rights`](/dist/docs/3.2.0-beta.11/brand-protocol/tasks/acquire_rights), [`log_event`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/log_event), [`report_usage`](/dist/docs/3.2.0-beta.11/accounts/tasks/report_usage), [`provide_performance_feedback`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/provide_performance_feedback), [`report_plan_outcome`](/dist/docs/3.2.0-beta.11/governance/campaign/tasks/report_plan_outcome), [`create_property_list`](/dist/docs/3.2.0-beta.11/governance/property/tasks/property_lists#create_property_list), [`update_property_list`](/dist/docs/3.2.0-beta.11/governance/property/tasks/property_lists#update_property_list), [`delete_property_list`](/dist/docs/3.2.0-beta.11/governance/property/tasks/property_lists#delete_property_list), [`create_collection_list`](/dist/docs/3.2.0-beta.11/governance/collection/tasks/collection_lists#create_collection_list), [`update_collection_list`](/dist/docs/3.2.0-beta.11/governance/collection/tasks/collection_lists#update_collection_list), [`delete_collection_list`](/dist/docs/3.2.0-beta.11/governance/collection/tasks/collection_lists#delete_collection_list), [`create_content_standards`](/dist/docs/3.2.0-beta.11/governance/content-standards/tasks/create_content_standards), [`update_content_standards`](/dist/docs/3.2.0-beta.11/governance/content-standards/tasks/update_content_standards), [`calibrate_content`](/dist/docs/3.2.0-beta.11/governance/content-standards/tasks/calibrate_content), [`si_initiate_session`](/dist/docs/3.2.0-beta.11/sponsored-intelligence/tasks/si_initiate_session), [`si_send_message`](/dist/docs/3.2.0-beta.11/sponsored-intelligence/tasks/si_send_message).

Missing the key → `adcp_error.code: 'VALIDATION_ERROR'` with `/idempotency_key` in `issues`.

## `account` is `oneOf` — pick exactly one variant

`account` is a discriminated union. On [`create_media_buy`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/create_media_buy) and [`update_media_buy`](/dist/docs/3.2.0-beta.11/media-buy/task-reference/update_media_buy), two variants:
...
Other tools (e.g. [`sync_creatives`](/dist/docs/3.2.0-beta.11/creative/task-reference/sync_creatives)) may accept a superset — always check the specific tool's schema.
...
| Response carries `status: 'submitted'` and `task_id` | Async — work is queued, NOT done | On the AdCP v3 A2A profile, invoke [`get_task_status`](https://adcontextprotocol.org/schemas/3.2.0-beta.11/protocol/get-task-status-request.json). On MCP, use the advertised AdCP polling task. |
...
A common trap: **A2A `Task.status.state: 'completed'` is not the same as AdCP completion.** A2A task state describes the transport invocation; AdCP-level completion is in the artifact DataPart. A `completed` A2A task can carry a `submitted` AdCP response, whose `task_id` remains only in that DataPart and is polled through [`get_task_status`](https://adcontextprotocol.org/schemas/3.2.0-beta.11/protocol/get-task-status-request.json). It also carries structured business outcomes that are not transport failures, such as `GetProductsRejected`: A2A remains `completed`, while the artifact payload contains `status: "rejected"`, `reason`, and optional `suggestions[]`.

## Related

- Per-task request/response shapes: see the protocol-specific reference (`/docs/media-buy/`, `/docs/creative/`, `/docs/signals/`, etc.).
- [Protocol architecture](/dist/docs/3.2.0-beta.11/protocol/architecture) — how the protocol domains fit together.
- [Required tasks](/dist/docs/3.2.0-beta.11/protocol/required-tasks) — which tasks an agent must implement to claim a specialism.
- [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.11/protocol/get_adcp_capabilities) — first call against any new agent.
- [Schemas](/dist/docs/3.2.0-beta.11/building/by-layer/L0/schemas) — how SDKs consume the protocol tarball (which now bundles `skills/`).
- [Build a caller](/dist/docs/3.2.0-beta.11/building/by-layer/L4/build-a-caller) — build-shaped guide for the caller side: install, call, handle responses, ingest reporting.
