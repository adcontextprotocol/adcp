...
The agent declares `supported_protocols: ["brand"]` in [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.0/protocol/get_adcp_capabilities). The specific tasks it implements define its role:
...
Authorized callers — linked via [`sync_accounts`](/dist/docs/3.2.0-beta.0/accounts/tasks/sync_accounts) — get deeper data on top of that baseline: high-res assets, voice synthesis configs, tone guidelines, and rights availability.
...
[`verify_brand_claim`](/dist/docs/3.2.0-beta.0/brand-protocol/tasks/verify_brand_claim) lets partners ask the brand-agent an authoritative yes/no question about its identity — "is this subsidiary yours", "is this property yours", "is this trademark yours". It's a layered capability on top of the identity tier: same brand data, plus the richer states (`pending_review`, `transferring`, `disputed`, `licensed_in`) the static `brand.json` can't express.

The trust model is asymmetric by direction only after the [brand-authorization cross-check](/dist/docs/3.2.0-beta.0/building/by-layer/L1/security#designated-task-response-signing) returns verifier-local `trust: "trusted"`. Trusted rejections (`disputed` / `not_ours`) are authoritative unilaterally — a trusted brand has standing to refuse association without reciprocation. Trusted assertions (`owned` / `pending_review` / `transferring` / `licensed_*`) are informative but NOT trust-extending alone; the reciprocating side must still confirm. A skipped or failed cross-check cannot extend or revoke trust. This is the load-bearing concept — see [`brand.json` § Agent-augmented verification](/dist/docs/3.2.0-beta.0/brand-protocol/brand-json#agent-augmented-verification) for the full normative table.
...
The signature is a **JWS payload envelope** carried inside the response body (not RFC 9421 §2.2.9 transport response signing — that primitive is undefined in 3.x). `verify_brand_claim` and `verify_brand_claims` are the only tasks on the spec's [designated-task response-signing list](/dist/docs/3.2.0-beta.0/building/by-layer/L1/security#designated-task-response-signing) — the closed-list rule and admission criterion live there.

The envelope is carried in the response's `signed_response` field and follows [`response-payload-jws-envelope.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.0/core/response-payload-jws-envelope.json). Populate `brand_domain` from server-side tenant resolution, not from caller input. Shared multi-brand fleets also need distinct response-signing key material and distinct `kid` values per `brand_domain`; key reuse across brands defeats tenant-bound replay analysis and is non-conformant for `adcp_use: "response-signing"`.
...
See [request-signing](/dist/docs/3.2.0-beta.0/building/by-layer/L1/request-signing) for the keypair-generation and JWKS-publication pattern; the response-signing key follows the same shape with a different `adcp_use` tag.
...
When your agent returns `disputed` or `not_ours`, consumers render the rejection in their own UIs (DSP inventory shopping, portfolio explorer, creative clearance). The agent owes a clear `context_note` — that string ends up in front of humans. See [UI guidance for rejected claims](/dist/docs/3.2.0-beta.0/brand-protocol/ui-guidance) for the consumer-side conventions to keep in mind when writing your `context_note` text.
...
`acquire_rights` follows the same pattern — accept a `rights_id` and `pricing_option_id` from `get_rights`, clear against existing contracts, and return terms with generation credentials. The response includes an authenticated `approval_webhook` (using [`push-notification-config`](https://adcontextprotocol.org/schemas/3.2.0-beta.0/core/push-notification-config.json)) so buyers can submit creatives for review. See the [acquire_rights task reference](/dist/docs/3.2.0-beta.0/brand-protocol/tasks/acquire_rights) for the full schema.
...
Buyers establish authorization by calling [`sync_accounts`](/dist/docs/3.2.0-beta.0/accounts/tasks/sync_accounts) on your agent. After linking, their subsequent `get_brand_identity` requests are recognized as authorized.

Implement the [accounts protocol](/dist/docs/3.2.0-beta.0/accounts/overview) to support this. The linked account is identified by the caller's credentials in the MCP transport — you do not need to pass account IDs in brand protocol requests.
...
How you identify callers depends on your authentication setup. The MCP transport provides session information; your auth middleware maps that to a linked account. See the [authentication guide](/dist/docs/3.2.0-beta.0/building/by-layer/L2/authentication) for patterns.
...
For the full creative manifest specification, see [creative manifests](/dist/docs/3.2.0-beta.0/creative/creative-manifests).
...
- [Brand protocol overview](/dist/docs/3.2.0-beta.0/brand-protocol/index) — How brand discovery works
- [brand.json spec](/dist/docs/3.2.0-beta.0/brand-protocol/brand-json) — File format for brand declaration
- [get_brand_identity](/dist/docs/3.2.0-beta.0/brand-protocol/tasks/get_brand_identity) — Identity task reference
- [verify_brand_claim](/dist/docs/3.2.0-beta.0/brand-protocol/tasks/verify_brand_claim) — Verification task reference
- [UI guidance for rejected claims](/dist/docs/3.2.0-beta.0/brand-protocol/ui-guidance) — Consumer-side rendering of `disputed` / `not_ours`
- [get_rights](/dist/docs/3.2.0-beta.0/brand-protocol/tasks/get_rights) — Rights discovery task reference
- [acquire_rights](/dist/docs/3.2.0-beta.0/brand-protocol/tasks/acquire_rights) — Rights acquisition task reference
- [Accounts overview](/dist/docs/3.2.0-beta.0/accounts/overview) — How account linking works
