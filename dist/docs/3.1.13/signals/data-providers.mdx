...
Both use `/.well-known/adagents.json` as the publishing mechanism. A single `adagents.json` file can declare both `properties` and `signals` simultaneously — see [Unified declaration model](/dist/docs/3.1.13/governance/property/adagents#unified-declaration-model).
...
Identity companies often also provide cross-device identity graphs, but identity resolution as a service (matching Device A to Person B) is not yet part of the AdCP protocol. See the [signals ecosystem guide](/dist/docs/3.1.13/signals/ecosystem#identity-companies) for more on this boundary.
...
Retail signals are especially valuable because they're deterministic — based on actual purchases, not modeled behavior. See the [signals ecosystem guide](/dist/docs/3.1.13/signals/ecosystem#retail-media-networks) for the dual-role pattern (publisher + data provider).
...
Signal definitions declare `policy_categories` and `restricted_attributes` using the same vocabulary as the [Policy Registry](/dist/docs/3.1.13/governance/policy-registry). These fields enable governance agents to match signal metadata against policy entries during campaign validation.

| Signal field | Registry equivalent | Purpose |
|-------------|-------------------|---------|
| `policy_categories` | `policy_categories` on [policy entries](/dist/docs/3.1.13/governance/policy-registry#policy-category-definitions) | Declares which regulatory regimes the signal touches (e.g., `children_directed`, `health_wellness`) |
| `restricted_attributes` | `restricted_attributes` on [policy categories](/dist/docs/3.1.13/governance/policy-registry#restricted-attribute-definitions) | Declares which GDPR Article 9 special categories the signal touches (e.g., `health_data`, `racial_ethnic_origin`) |

Values MUST match the canonical definitions in the Policy Registry. See [policy category definitions](/dist/docs/3.1.13/governance/policy-registry#policy-category-definitions) for the full list of valid `policy_categories` values and [restricted attribute definitions](/dist/docs/3.1.13/governance/policy-registry#restricted-attribute-definitions) for valid `restricted_attributes` values.
...
- [Signals Protocol Overview](/dist/docs/3.1.13/signals/overview) - How signals work in AdCP
- [get_signals Task](/dist/docs/3.1.13/signals/tasks/get_signals) - Signal discovery API
- [activate_signal Task](/dist/docs/3.1.13/signals/tasks/activate_signal) - Signal activation API
- [adagents.json Tech Spec](/dist/docs/3.1.13/governance/property/adagents) - Full adagents.json reference (property-focused)
