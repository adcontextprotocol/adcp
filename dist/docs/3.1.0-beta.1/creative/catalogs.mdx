...
This is the same dependency order documented in [Account state](/dist/docs/3.1.0-beta.1/building/integration/account-state): `sync_catalogs` → `sync_creatives` → `create_media_buy`.
...
- **[Account state](/dist/docs/3.1.0-beta.1/building/integration/account-state)** — How catalogs fit into account state and the setup sequence
