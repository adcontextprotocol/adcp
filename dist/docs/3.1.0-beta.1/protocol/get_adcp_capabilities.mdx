...
See the [Compliance Catalog](/dist/docs/3.1.0-beta.1/building/compliance-catalog) for every protocol's scope. Support for the [compliance test controller](/dist/docs/3.1.0-beta.1/building/implementation/comply-test-controller) is declared via the separate `compliance_testing` capability block (below), not as a protocol value.
...
See the full [Compliance Catalog](/dist/docs/3.1.0-beta.1/building/compliance-catalog) for every specialism and the [enum schema](https://adcontextprotocol.org/schemas/3.1.0-beta.1/enums/specialism.json) for the authoritative list.
...
See [Accounts and Agents](/dist/docs/3.1.0-beta.1/building/integration/accounts-and-agents#what-sellers-declare) for full workflows and [seller patterns](/dist/docs/3.1.0-beta.1/building/integration/accounts-and-agents#seller-patterns) for common combinations of auth model and billing support.
...
Compliance testing capabilities. The presence of this block declares that the agent supports deterministic testing via [`comply_test_controller`](/dist/docs/3.1.0-beta.1/building/implementation/comply-test-controller). Omit the block if the agent does not support compliance testing.

**Production deployments MUST NOT include this block.** `comply_test_controller` is sandbox-only at the deployment level; advertising the capability on a production endpoint is non-conformant even if dispatch is gated. See [Compliance test controller § Sandbox gating](/dist/docs/3.1.0-beta.1/building/implementation/comply-test-controller#sandbox-gating).
...
Extension schemas are published in the [AdCP extension registry](/dist/docs/3.1.0-beta.1/building/integration/context-sessions#extensions). When an agent declares support for an extension, buyers know to look for and process `ext.{namespace}` data in responses.
...
1. **Set up accounts**: Follow the auth model from `account.require_operator_auth` — see [Accounts and Agents](/dist/docs/3.1.0-beta.1/building/integration/accounts-and-agents#what-sellers-declare)
...
- [Accounts and Agents](/dist/docs/3.1.0-beta.1/building/integration/accounts-and-agents) - Auth models, account setup, billing
