...
<Note>
**Per-caller authorization is NOT reported here.** `get_adcp_capabilities` returns the seller's surface — everything it *could* do for any authorized caller. To discover what *you* are allowed to do on a specific account (which tasks are callable for your identity, which request fields are modifiable, any named scope like `attestation_verifier`), read the `authorization` object on per-account entries in [`sync_accounts`](/dist/docs/3.1.0-beta.1/accounts/tasks/sync_accounts) and [`list_accounts`](/dist/docs/3.1.0-beta.1/accounts/tasks/list_accounts) responses. See [Caller authorization](/dist/docs/3.1.0-beta.1/accounts/overview#caller-authorization) for the full shape and semantics.
</Note>

**Request Schema**: [`/schemas/3.1.0-beta.1/protocol/get-adcp-capabilities-request.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.1/protocol/get-adcp-capabilities-request.json)
**Response Schema**: [`/schemas/3.1.0-beta.1/protocol/get-adcp-capabilities-response.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.1/protocol/get-adcp-capabilities-response.json)
...
See the [Compliance Catalog](/dist/docs/3.1.0-beta.1/building/verification/compliance-catalog) for every protocol's scope. Support for the [compliance test controller](/dist/docs/3.1.0-beta.1/building/by-layer/L3/comply-test-controller) is declared via the separate `compliance_testing` capability block (below), not as a protocol value.
...
See the full [Compliance Catalog](/dist/docs/3.1.0-beta.1/building/verification/compliance-catalog) for every specialism and the [enum schema](https://adcontextprotocol.org/schemas/3.1.0-beta.1/enums/specialism.json) for the authoritative list.
...
| `account_financials` | boolean | Default: `false`. When `true`, the seller supports [`get_account_financials`](/dist/docs/3.1.0-beta.1/accounts/tasks/get_account_financials) for querying spend, credit, and invoice status. Only applicable to operator-billed accounts. |
| `sandbox` | boolean | Default: `false`. Strongly recommended for production sales agents. When `true`, the seller supports sandbox accounts for testing. For sandbox, the path follows the account model: explicit accounts discover pre-existing test accounts via `list_accounts`; implicit accounts declare sandbox via `sync_accounts` with `sandbox: true` — no real platform calls or spend. See [Sandbox mode](/dist/docs/3.1.0-beta.1/media-buy/advanced-topics/sandbox). |
...
See [Accounts and Agents](/dist/docs/3.1.0-beta.1/building/by-layer/L2/accounts-and-agents#what-sellers-declare) for full workflows and [seller patterns](/dist/docs/3.1.0-beta.1/building/by-layer/L2/accounts-and-agents#seller-patterns) for common combinations of auth model and billing support.
...
| Field | Type | Description |
|-------|------|-------------|
| `trusted_match` | object | [TMP](/dist/docs/3.1.0-beta.1/trusted-match) support. When present, this seller supports real-time contextual and/or identity matching. Check individual products for per-product TMP capabilities. |
...
Buyers discover AXE support via `get_adcp_capabilities` and filter products to AXE-enabled sellers using `required_axe_integrations` on [`get_products`](/dist/docs/3.1.0-beta.1/media-buy/task-reference/get_products).
...
| Field | Type | Description |
|-------|------|-------------|
| `data_provider_domains` | string[] | Data provider domains this signals agent is authorized to resell. Buyers should fetch each provider's `adagents.json` for signal catalog definitions and to verify authorization. |
| `discovery_modes` | string[] | Discovery modes the agent supports on `get_signals`. `"brief"` (semantic discovery via `signal_spec` / `signal_ids`) is implicit and always supported. Declare `"wholesale"` to advertise that callers can omit `signal_spec` / `signal_ids` and enumerate the full priced catalog. Agents not declaring `"wholesale"` MAY return `INVALID_REQUEST` for wholesale calls. Absent declaration is treated as `["brief"]`. |
...
Array of property features this governance agent can evaluate. See [Property Governance](/dist/docs/3.1.0-beta.1/governance/property/index).

| Field | Type | Description |
|-------|------|-------------|
| `feature_id` | string | **Required.** Unique identifier (e.g., `mfa_score`, `coppa_certified`). Use `registry:{policy_id}` prefix for features mapped to [Policy Registry](/dist/docs/3.1.0-beta.1/governance/policy-registry) entries. |
...
Array of creative features this governance agent can evaluate. Same field schema as `property_features`. See [Creative Governance](/dist/docs/3.1.0-beta.1/governance/creative/index).
...
Content standards verification capabilities. See [Content Standards](/dist/docs/3.1.0-beta.1/governance/content-standards/index).
...
Policy registry integration capabilities. See [Policy Registry](/dist/docs/3.1.0-beta.1/governance/policy-registry).
...
```json
{
  "$schema": "/schemas/3.1.0-beta.1/protocol/get-adcp-capabilities-response.json",
  "adcp": {
    "major_versions": [3],
    "idempotency": { "supported": true, "replay_ttl_seconds": 86400 }
  },
...
Measurement protocol capabilities. Only present if `measurement` is in `supported_protocols`. The `measurement` protocol is in development — currently scoped to `get_adcp_capabilities` for catalog discovery (this block); additional measurement tasks (reporting, attribution, panel queries) and a baseline compliance storyboard land in subsequent minors. The capability block is shipping now so measurement vendors can publish their per-metric catalogs and AAO can crawl them, before the broader task surface is settled.
...
Measurement agents publish a per-metric catalog so buyers know which metrics each vendor offers. This is the canonical source of truth — AAO crawls it to populate the federated [measurement-vendor index](/dist/docs/3.1.0-beta.1/registry/index#measurement-vendor-discovery).
...
Compliance testing capabilities. The presence of this block declares that the agent supports deterministic testing via [`comply_test_controller`](/dist/docs/3.1.0-beta.1/building/by-layer/L3/comply-test-controller). Omit the block if the agent does not support compliance testing.

**Production deployments MUST NOT include this block.** `comply_test_controller` is sandbox-only at the deployment level; advertising the capability on a production endpoint is non-conformant even if dispatch is gated. See [Compliance test controller § Sandbox gating](/dist/docs/3.1.0-beta.1/building/by-layer/L3/comply-test-controller#sandbox-gating).
...
| `algorithms` | string[] | **Required when `supported: true`.** Subset of `["ed25519", "ecdsa-p256-sha256"]` — the algorithms this seller will sign webhooks with. Matches the webhook-signing verifier allowlist (see [Verifier checklist for webhooks](/dist/docs/3.1.0-beta.1/building/by-layer/L1/security#webhook-callbacks), step 4). Buyers MUST be prepared to verify any algorithm listed AND MUST reject onboarding with a user-actionable error if an advertised algorithm is outside this enumerated set — a seller advertising an out-of-set algorithm (e.g., `hs256`) is either misconfigured or signalling a non-conforming profile. |
...
Extension schemas are published in the [AdCP extension registry](/dist/docs/3.1.0-beta.1/building/by-layer/L2/context-sessions#extensions). When an agent declares support for an extension, buyers know to look for and process `ext.{namespace}` data in responses.
...
See [experimental status](/dist/docs/3.1.0-beta.1/reference/experimental-status) for the full stability contract, graduation criteria, and client guidance.

### catalog_versioning

Conditional-fetch token capabilities for [`get_products`](/dist/docs/3.1.0-beta.1/media-buy/task-reference/get_products#catalog-versioning) and [`get_signals`](/dist/docs/3.1.0-beta.1/signals/tasks/get_signals#catalog-versioning). Independent of the change feed — an agent MAY support cheap version probes without implementing the full feed (and vice versa).

| Field | Type | Description |
|-------|------|-------------|
| `supported` | boolean | **Required.** Whether the agent returns `catalog_version` on responses and honors `if_catalog_version` on requests. When absent or `false`, buyers can still probe by reading the response (the field-presence detection path) but can't pre-flight-decide which agents to cache versions for. |
| `pricing_version_separate` | boolean | Whether the agent tracks `pricing_version` independently of `catalog_version`. When `true`, callers can send `if_pricing_version` alongside `if_catalog_version` for finer-grained "did prices change?" probes. When `false` or absent, the agent collapses both into `catalog_version` — sending `if_pricing_version` is wasted bytes. |
...
Per-agent inventory change-feed capabilities. Declared by sales agents (products) and signals agents (signals). When `supported` is `true`, consumers can poll `GET /catalog/events` on this agent to maintain a near-real-time mirror of its product and/or signal catalog without re-fetching unchanged inventory. See `specs/catalog-change-feed.md` for the full spec.

Complementary to (and independent of) [`catalog_versioning`](#catalog-versioning) — the change feed gives a continuous event stream; the version tokens give a cheap "anything changed?" probe. Adopters MAY ship either, both, or neither.

**Consumer precedence.** When more than one mechanism is declared, consumers SHOULD prefer them in this order: (1) `catalog_change_feed` when subscribing to a long-lived mirror (lowest latency, lowest seller cost); (2) `catalog_versioning` conditional fetch when the consumer only needs occasional polling (cheaper than full re-enumeration); (3) wholesale enumeration via `buying_modes` / `discovery_modes` when neither is declared, or as the recovery path after `RETENTION_EXPIRED` / `catalog.bulk_change` / cold start.

| Field | Type | Description |
|-------|------|-------------|
| `supported` | boolean | **Required.** Whether this agent publishes a change feed. When `false` or this stanza is absent, consumers fall back to wholesale polling (optionally with `if_catalog_version` probes). |
| `retention_window_days` | integer | Minimum age of events the agent guarantees to serve. Consumers whose cursor is older than this window get `RETENTION_EXPIRED` and MUST resync via wholesale enumeration. SHOULD be 30 days; MUST NOT be less than 7. |
| `webhooks_supported` | boolean | Whether the agent supports webhook subscriptions on `/catalog/subscriptions`. Polling the feed is always the source of truth — webhooks are best-effort notifications. |
| `event_types` | string[] | Event types this agent emits. Sales agents emit `product.*` events; signals agents emit `signal.*` events; agents that are both publish both event families on one feed. `catalog.bulk_change` is the fast-forward event recommended for rate-card sweeps that would otherwise produce thousands of individual events. |
...
- AXE URL in `media_buy.execution.axe_integrations` → Seller can execute through that exchange (legacy — new integrations use [TMP](/dist/docs/3.1.0-beta.1/trusted-match))
...
When `supported_protocols` includes `"creative"`, the buyer can call Creative Protocol tasks (`list_creative_formats`, `sync_creatives`, `get_creative_delivery`, etc.) on this agent. See [Creative capabilities on sales agents](/dist/docs/3.1.0-beta.1/creative/sales-agent-creative-capabilities).
...
1. **Set up accounts**: Follow the auth model from `account.require_operator_auth` — see [Accounts and Agents](/dist/docs/3.1.0-beta.1/building/by-layer/L2/accounts-and-agents#what-sellers-declare)
2. **Filter products**: Use [`get_products`](/dist/docs/3.1.0-beta.1/media-buy/task-reference/get_products) with capability-aware filters
3. **Validate properties**: Fetch publisher `adagents.json` files for property definitions
4. **Create buys**: Use [`create_media_buy`](/dist/docs/3.1.0-beta.1/media-buy/task-reference/create_media_buy) with supported features

## Learn More

- [Accounts and Agents](/dist/docs/3.1.0-beta.1/building/by-layer/L2/accounts-and-agents) - Auth models, account setup, billing
- [adagents.json Specification](/dist/docs/3.1.0-beta.1/governance/property/adagents) - Publisher authorization files
- [Product Filters](/dist/docs/3.1.0-beta.1/media-buy/task-reference/get_products#filters) - Capability-aware filtering
- [Content Standards](/dist/docs/3.1.0-beta.1/governance/content-standards) - Brand safety configuration
