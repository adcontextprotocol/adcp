---
title: get_signals
description: "get_signals is the AdCP task for discovering audience and contextual signals. Search by natural language or signal ID, filter by platform and CPM, and get real-time deployment status with activation keys."
...
**Request Schema**: [`https://adcontextprotocol.org/schemas/3.1.0-beta.1/signals/get-signals-request.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.1/signals/get-signals-request.json)
**Response Schema**: [`https://adcontextprotocol.org/schemas/3.1.0-beta.1/signals/get-signals-response.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.1/signals/get-signals-response.json)
...
|-----------|------|----------|-------------|
| `discovery_mode` | string | No (v3.1+ SHOULD include) | `"brief"` (default) or `"wholesale"`. `"brief"`: existing behavior — `signal_spec` or `signal_ids` is required, agent performs inference/RAG. `"wholesale"`: raw catalog enumeration — `signal_spec` and `signal_ids` MUST NOT be provided; agent returns its full priced catalog, paginated, scoped by `filters` / `account` / `destinations` / `countries` when present. **Timing semantics:** `"wholesale"` is a catalog read — agents SHOULD respond synchronously and MUST NOT route through the async/Submitted arm; partial completion uses [`incomplete[]`](#incomplete-array). Agents receiving requests from pre-v3.1 clients without `discovery_mode` MUST default to `"brief"`. Probe support via [`get_adcp_capabilities`](/dist/docs/3.1.0-beta.1/protocol/get_adcp_capabilities) (`signals.discovery_modes`). |
...
| `account` | [AccountRef](/dist/docs/3.1.0-beta.1/building/by-layer/L2/accounts-and-agents#account-references) | No | Account for this request. When provided, the signals agent returns per-account pricing options if configured. In wholesale mode, this is the rate-card scope; when omitted the agent returns default rate-card pricing or omits `pricing_options` entirely. |
...
| `filters` | Filters | No | Filters to refine results (see Filters Object below). In wholesale mode, filters constrain the enumerated catalog (e.g., `filters.data_providers: ["acme-data"]` returns only that provider's signals). |
| `if_catalog_version` | string | No | Opaque `catalog_version` token from a prior `get_signals` response. When provided, the agent compares against its current catalog version for the caller's `cache_scope` and MAY return `unchanged: true` (with `signals` omitted) if nothing has changed. See [Catalog versioning](#catalog-versioning) and [Cache layering](#cache-layering). |
| `if_pricing_version` | string | No | Opaque `pricing_version` token from a prior response. MUST only be sent together with `if_catalog_version`. Evaluation order: `if_catalog_version` mismatch → full payload; `if_catalog_version` matches but `if_pricing_version` mismatches → full payload (so the caller sees updated `pricing_options`); both match → agent MAY return `unchanged: true`. Agents that don't track pricing separately ignore this. |
...
| `pagination` | object | No | Pagination envelope. `pagination.max_results` (max: 100, default: 50) controls page size; `pagination.cursor` (opaque token from previous response) advances pages. Required for wholesale (catalogs may be large). |
...
|-------|------|-------------|
| `catalog_version` | string | Opaque token representing the version of the catalog state used to compose this response. Treat as opaque — no format, no ordering, no inspection. Returned on every response by agents implementing conditional-fetch. See [Catalog versioning](#catalog-versioning). |
| `pricing_version` | string | Optional finer-grained token. Changes when prices move while `catalog_version` only changes for structure/metadata. Agents not separating these MAY omit `pricing_version`. |
...
```json
{
  "$schema": "/schemas/3.1.0-beta.1/signals/get-signals-response.json",
...
```json
{
  "$schema": "/schemas/3.1.0-beta.1/signals/get-signals-request.json",
...
- **signals** (array): Array of matching signals
  ...
  - **signal_type** (string): `marketplace` (resold third-party), `owned` (agent's first-party data), or `custom` (agent-native segment built on demand)
...
## Wholesale catalog enumeration

When a consumer (storefront, federated marketplace, registry) needs to mirror a signals agent's full priced catalog, set `discovery_mode: "wholesale"` and omit `signal_spec` / `signal_ids`. Wholesale enumeration is symmetric with [`get_products` `buying_mode: "wholesale"`](/dist/docs/3.1.0-beta.1/media-buy/task-reference/get_products): synchronous, paginated, firm-priced, with partial completion declared via `incomplete[]`.
...
- Storefronts and registries MAY use wholesale enumeration plus `adagents.json` cross-reference to materialize a "data publisher catalog" view: for each known data provider, the set of signals available via authorized signals agents, with prices.

### Pricing

`pricing_options[]` MUST be populated for authenticated callers when the agent has firm prices to declare. When `account` is omitted, the agent returns default rate-card pricing or omits `pricing_options[]` (in which case the caller MUST re-query with `account` before composing). Unauthenticated callers MAY receive catalog metadata without pricing.

### Capability declaration

Signals agents declare wholesale support in [`get_adcp_capabilities`](/dist/docs/3.1.0-beta.1/protocol/get_adcp_capabilities):
...
Even with wholesale enumeration, a consumer mirroring an agent's catalog would re-fetch every paginated page on each poll just to detect changes. To avoid that, `get_signals` supports an opaque `catalog_version` token returned on every response. Pass it back via `if_catalog_version` on a subsequent call and the agent MAY short-circuit with `unchanged: true` — no catalog payload, no per-page diff.
...
When `unchanged: true`, `signals[]` MUST be omitted and consumers MUST NOT mutate their local catalog mirror.

### Catalog changed — full payload returned (abbreviated)

```json test=false
{
  "message": "Returning 312 signals (catalog version advanced).",
...
- A returned `catalog_version` is scope-keyed via `cache_scope`. Callers cache `(cache_scope, catalog_version)` pairs alongside the `(account, filters, discovery_mode, destinations, countries)` tuple used. See [Cache layering](#cache-layering) for the two-layer model.
- `pricing_version` is an optional finer-grained token: when present, it changes when prices move but `catalog_version` changes only when structure/metadata moves. Common for rate-card sweeps that don't change segment metadata.
- **`if_pricing_version` requires `if_catalog_version`.** Pricing has no structural baseline of its own. Sending `if_pricing_version` without `if_catalog_version` is a schema-level error. The agent's evaluation is two-stage: catalog mismatch returns the full payload; catalog match with pricing mismatch also returns the full payload (so the caller sees updated `pricing_options`); both match → `unchanged: true`.
- **`filters` canonicalization.** Agents MUST treat the `filters` object as canonicalized before hashing into the `catalog_version` keyspace: keys sorted lexicographically, omitted-and-default values treated identically, array values sorted where the filter has set semantics (e.g., `catalog_types`, `data_providers`). Callers that pass equivalent-but-differently-shaped filter objects MUST receive the same `catalog_version`. Prevents silent stale-mirror bugs from key-order or default-elision differences. **Forward-compat default:** new filter fields added in 3.x minor versions MUST declare set-vs-sequence semantics; absent an explicit declaration, the rule defaults to **set-semantics**.
- **Pagination interaction.** Agents MUST return `catalog_version` on every paginated page (not only the first) when they declare `catalog_change_feed.supported: true`; agents that do not declare the feed SHOULD do the same. If the catalog mutates between pages, the new version surfaces on the next page and the caller MUST restart pagination from `cursor: null` — the partial pages already received describe a stale version.
- **`unchanged: true` and in-progress pagination.** A caller mid-pagination MAY send `if_catalog_version` matching the version their pages so far were drawn from. If the agent confirms `unchanged: true`, the response omits `signals[]` and pagination envelope entirely; the caller abandons their in-progress walk under that version. Agents MAY NOT use the conditional-fetch short-circuit to skip individual pages within an active pagination — `unchanged` is whole-catalog-versus-cached-version, not per-page.
- Pre-v3.1 agents that ignore `if_catalog_version` simply return the full payload — semantically correct, just inefficient.

For ongoing change tracking beyond conditional fetch, see `specs/catalog-change-feed.md` — a per-agent event stream that emits `signal.created` / `signal.updated` / `signal.priced` / `signal.removed` events for incremental sync.
...
|---|---|---|
| Public | `(agent, discovery_mode, filters, destinations, countries)` | `catalog_version_public`, the catalog payload as seen without an account ref |
| Account overlay | `(agent, discovery_mode, filters, destinations, countries, account_id)` | `catalog_version_account`, the catalog payload when `cache_scope: "account"` was returned |
...
**Conditional fetch with `if_catalog_version`.** Send the token paired with whichever scope it was returned in. The agent compares against the current version for that scope. If the caller's token belongs to an `"account"` scope but the agent responds with `cache_scope: "public"`, that's the downgrade signal.
...
|-------|------|----------|-------------|
| `scope` | string | Yes | `"signals"`: not all matching signals were returned. `"pricing"`: signals returned but pricing is absent or unconfirmed. `"catalog"`: in wholesale mode, whole-catalog enumeration could not complete. |
...
| `discovery_mode: "wholesale"` (neither `signal_spec` nor `signal_ids`) | Wholesale — enumerate the agent's full priced catalog. See [Wholesale catalog enumeration](#wholesale-catalog-enumeration). |
