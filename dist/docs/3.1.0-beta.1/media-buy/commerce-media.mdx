...
| Advertiser account with retailer | `account_id` + `list_accounts` | [Accounts & Agents](/dist/docs/3.1.0-beta.1/building/by-layer/L2/accounts-and-agents) |
...
Before creating media buys, sync the brand's data feeds to the retailer's account via `sync_catalogs`. This builds account state that creatives and targeting reference at serve time. See [Account state](/dist/docs/3.1.0-beta.1/building/by-layer/L2/account-state) for how this fits the broader setup sequence.
...
- [Account state](/dist/docs/3.1.0-beta.1/building/by-layer/L2/account-state) — How catalogs, event sources, and campaigns build on the account
- [Accounts & Agents](/dist/docs/3.1.0-beta.1/building/by-layer/L2/accounts-and-agents) — Account setup and billing relationships
