...
**Request Schema**: [`/schemas/3.1.0-beta.1/media-buy/get-products-request.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.1/media-buy/get-products-request.json)
**Response Schema**: [`/schemas/3.1.0-beta.1/media-buy/get-products-response.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.1/media-buy/get-products-response.json)
...
| `catalog` | [Catalog](/dist/docs/3.1.0-beta.1/creative/catalogs) | No | Catalog of items the buyer wants to promote. The seller matches catalog items against its inventory and returns products where matches exist. Requires `brand`. See [Catalog discovery](#catalog-discovery) below. |
| `filters` | Filters | No | Structured filters (see below) |
| `property_list` | PropertyListRef | No | [AdCP 3.0] Reference to a property list for filtering. See [Property Lists](/dist/docs/3.1.0-beta.1/governance/property/tasks/property_lists) |
| `pagination` | PaginationRequest | No | Cursor-based pagination for large product catalogs (see below) |
| `if_catalog_version` | string | No | Opaque `catalog_version` token from a prior `get_products` response. When provided, the seller compares against its current catalog version and MAY return `unchanged: true` (with `products` omitted) if nothing has changed. Versions are scoped to the (agent, account, filters, `buying_mode`, `property_list`, `catalog`) tuple. See [Catalog versioning](#catalog-versioning). |
| `if_pricing_version` | string | No | Opaque `pricing_version` token from a prior response. MUST only be sent together with `if_catalog_version`. Evaluation order: `if_catalog_version` mismatch → full payload; `if_catalog_version` matches but `if_pricing_version` mismatches → full payload (so the buyer sees updated `pricing_options`); both match → seller MAY return `unchanged: true`. Sellers that don't track pricing separately ignore this. |
...
The seller must declare `features.property_list_filtering: true` in [`get_adcp_capabilities`](/dist/docs/3.1.0-beta.1/protocol/get_adcp_capabilities) to support this filter. See the [Property Governance overview](/dist/docs/3.1.0-beta.1/governance/property/index) for the full workflow.
</Note>
...
| `channels` | string[] | Filter by advertising channels (e.g., `["display", "ctv", "social", "streaming_audio"]`). See [Media Channel Taxonomy](/dist/docs/3.1.0-beta.1/reference/media-channel-taxonomy) |
...
The `catalog` field uses the same [Catalog](/dist/docs/3.1.0-beta.1/creative/catalogs) object used throughout AdCP. You can reference a synced catalog by `catalog_id`, provide inline items, or use selectors to filter:
...
| `measurement_readiness` | [MeasurementReadiness](/dist/docs/3.1.0-beta.1/media-buy/conversion-tracking/#measurement-readiness) | (Optional) Whether the buyer's event setup is sufficient for this product's optimization. Only present when the seller can evaluate the buyer's account context. |
| `measurement_terms` | [MeasurementTerms](/dist/docs/3.1.0-beta.1/media-buy/advanced-topics/pricing-models#measurement-terms-and-performance-standards) | (Optional) Seller's default billing measurement and makegood terms. Buyers may propose different terms at `create_media_buy`. |
| `performance_standards` | [PerformanceStandard[]](/dist/docs/3.1.0-beta.1/media-buy/advanced-topics/pricing-models#measurement-terms-and-performance-standards) | (Optional) Seller's default performance standards (viewability, IVT, completion rate, brand safety, attention score). Buyers may propose different standards at `create_media_buy`. |
| `cancellation_policy` | [CancellationPolicy](/dist/docs/3.1.0-beta.1/media-buy/advanced-topics/pricing-models#cancellation-policy) | (Optional) Cancellation notice period and penalties for guaranteed products. Buyers accept these terms by creating a media buy against the product. |

### Proposals Array (Optional)

Publishers may return proposals alongside products - structured media plans with budget allocations. See [Proposals](/dist/docs/3.1.0-beta.1/media-buy/product-discovery/media-products#proposals) for details.
...
| `forecast` | DeliveryForecast | Aggregate delivery forecast for the proposal. Contains forecast points with metric ranges. See [Delivery Forecasts](/dist/docs/3.1.0-beta.1/media-buy/product-discovery/media-products#delivery-forecasts) |
...
| `catalog_version` | string | Opaque token representing the version of the catalog state used to compose this response. Sellers implementing conditional-fetch (`if_catalog_version`) MUST return this on every response so buyers can cache and probe later. Treat as opaque — no format, no ordering, no inspection. See [Catalog versioning](#catalog-versioning). |
| `pricing_version` | string | Optional opaque token representing the version of the pricing layer. When the seller supports independent pricing versioning, `pricing_version` changes when prices move but `catalog_version` changes only when structure/metadata moves. Sellers not separating these MAY omit `pricing_version` and use `catalog_version` for both. |
...
A buyer that just synced a seller's catalog can ask "has anything changed since version X?" in one cheap call, regardless of catalog size. Sellers return an opaque `catalog_version` on every response; buyers pass it back via `if_catalog_version` on the next call and the seller MAY short-circuit with `unchanged: true` — no products payload, no per-page diff. Patterned on HTTP `ETag` / `If-None-Match`.
...
```json
{
  "$schema": "/schemas/3.1.0-beta.1/media-buy/get-products-request.json",
  "buying_mode": "wholesale",
...
```json
{
  "$schema": "/schemas/3.1.0-beta.1/media-buy/get-products-response.json",
...
Response (catalog changed — full payload returned, abbreviated):

```json test=false
{
  "message": "Returning 50 of 312 products (catalog version advanced).",
...
- A returned `catalog_version` is scoped to the request parameters that produced it. Buyers MUST cache the version alongside the `(account, filters, buying_mode, property_list, catalog)` tuple used.
- `pricing_version` is an optional finer-grained token: when present, it changes when prices move but `catalog_version` changes only when structure/metadata moves. Common for rate-card sweeps that don't change product metadata.
- **`if_pricing_version` requires `if_catalog_version`.** Pricing has no structural baseline of its own. Sending `if_pricing_version` without `if_catalog_version` is a schema-level error. The seller's evaluation is two-stage: catalog mismatch returns the full payload (pricing is implicitly stale); catalog match with pricing mismatch also returns the full payload (so the buyer sees updated `pricing_options`); both match → `unchanged: true`.
- **`filters` canonicalization.** Sellers MUST treat the `filters` object as canonicalized before hashing into the `catalog_version` keyspace: keys MUST be sorted lexicographically, omitted-and-default values MUST be treated identically (a missing `delivery_type` key is the same scope as `delivery_type: null`), array values MUST be sorted where the filter has set semantics (e.g., `channels`, `format_ids`, `required_metrics`) and preserved-order where the filter has sequence semantics (e.g., `preferred_delivery_types`). Buyers that pass equivalent-but-differently-shaped filter objects MUST receive the same `catalog_version` from the seller. This rule prevents silent stale-mirror bugs from key-order or default-elision differences between buyer SDKs. **Forward-compat default:** new filter fields added in 3.x minor versions MUST declare set-vs-sequence semantics in their schema (via `x-canonicalization: set | sequence` or equivalent prose); absent an explicit declaration, the rule defaults to **set-semantics** (sort before hashing). Sellers and SDKs that drift on this default produce cache misses that consumers can't explain.
- **Pagination interaction.** `catalog_version` describes the catalog as a whole, not individual pages. Sellers MUST return `catalog_version` on every paginated page (not only the first) when they declare `catalog_change_feed.supported: true`; sellers that do not declare the feed SHOULD do the same. When the catalog mutates between pages, the new version surfaces on the next page and the buyer MUST restart pagination from `cursor: null` — the partial pages they've already received describe a stale version. Sellers MAY alternatively snapshot the catalog at the start of pagination and serve all pages from that snapshot under the original version; either implementation is conformant as long as `catalog_version` on a given page is the version that page belongs to.
- **`unchanged: true` and in-progress pagination.** A buyer that is mid-pagination on `cursor: X` MAY send `if_catalog_version` matching the version their pages so far were drawn from. If the seller confirms `unchanged: true`, the response omits `products[]` and pagination envelope entirely; the buyer abandons their in-progress walk under that version with confidence that no further pages would have produced new data. Sellers MAY NOT use the conditional-fetch short-circuit to skip individual pages within an active pagination — `unchanged` is whole-catalog-versus-cached-version, not per-page.
- Pre-v3.1 sellers that ignore `if_catalog_version` simply return the full payload — semantically correct, just inefficient (same as the unchanged-server path in HTTP).

For ongoing change tracking beyond conditional fetch, see `specs/catalog-change-feed.md` — a per-agent event stream that emits `product.created` / `product.updated` / `product.priced` / `product.removed` events for incremental sync.
...
**Why this matters.** A buyer mirroring catalogs across N accounts at one seller doesn't want to hold N copies of inventory that's actually identical for every buyer. The public layer is the seller's published rate card; most accounts at most sellers price off it directly. Premium custom deals are the exception.

**Two-layer cache.**

| Layer | Cache key | What's stored |
|---|---|---|
| Public | `(agent, buying_mode, filters, property_list, catalog)` | `catalog_version_public`, the catalog payload as seen without an account ref |
| Account overlay | `(agent, buying_mode, filters, property_list, catalog, account_id)` | `catalog_version_account`, the catalog payload as seen WITH this account ref, when `cache_scope: "account"` was returned |
...
**Conditional fetch with `if_catalog_version`.** Send the token paired with whichever scope it was returned in. The seller compares against the current version for that scope. If the buyer's token belongs to an `"account"` scope but the seller responds with `cache_scope: "public"`, that's the downgrade signal — buyer drops the overlay.

**Event invalidation.** The change feed declares `applies_to.scope` on `*.priced` and `*.updated` events:
...
See `specs/catalog-change-feed.md` §"Cache layering and event scoping" for the full event-side spec.

**See schema for complete field list**: [`get-products-response.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.1/media-buy/get-products-response.json)
...
See [Property Targeting](/dist/docs/3.1.0-beta.1/media-buy/product-discovery/media-products#property-targeting) for more details and [Property Governance](/dist/docs/3.1.0-beta.1/governance/property/specification) for more on property lists.
...
See the [Refinement guide](/dist/docs/3.1.0-beta.1/media-buy/product-discovery/refinement) for the full walkthrough: scope types, action semantics, seller responses, and common patterns. The parameter shape is defined in the [Refine array](#refine-array) section above.
...
See [Authentication Guide](/dist/docs/3.1.0-beta.1/building/integration/authentication) for details.
...
**Note:** For the complete status list see [Task Lifecycle](/dist/docs/3.1.0-beta.1/building/implementation/task-lifecycle).
...
2. **Create Media Buy**: Use [`create_media_buy`](/dist/docs/3.1.0-beta.1/media-buy/task-reference/create_media_buy) to execute campaign
3. **Prepare Creatives**: Use [`list_creative_formats`](/dist/docs/3.1.0-beta.1/creative/task-reference/list_creative_formats) to see format requirements
4. **Upload Assets**: Use [`sync_creatives`](/dist/docs/3.1.0-beta.1/creative/task-reference/sync_creatives) to provide creative assets

## Learn More

- [Product Discovery Guide](/dist/docs/3.1.0-beta.1/media-buy/product-discovery/) - Understanding briefs and products
- [Pricing Models](/dist/docs/3.1.0-beta.1/media-buy/advanced-topics/pricing-models) - CPM, CPCV, CPP explained
- [Brief Expectations](/dist/docs/3.1.0-beta.1/media-buy/product-discovery/brief-expectations) - How to write effective briefs
- [Media Products](/dist/docs/3.1.0-beta.1/media-buy/product-discovery/media-products) - Product structure and fields
