...
High-level summaries of major AdCP releases with migration guidance. For detailed technical changelogs, see [CHANGELOG.md](https://github.com/adcontextprotocol/adcp/blob/main/CHANGELOG.md). For version stability, schema-change scope, and the 3.x guarantees see [Versioning & Governance](/dist/docs/3.0.8/reference/versioning). For v2 end-of-life see the [v2 sunset page](/dist/docs/3.0.8/reference/v2-sunset).
...
| Implementing a buyer agent | Read the new `AUTH_REQUIRED` sub-cases in [error-handling.mdx](/dist/docs/3.0.8/building/implementation/error-handling) — the wire code stays the same but you SHOULD NOT auto-retry when credentials were attached and rejected (terminal case). 3.1 will split this into separate enum values via #3739. |
...
| Implementing governance or TMP | Review the experimental-surface additions below. Per the [experimental-status contract](/dist/docs/3.0.8/reference/experimental-status), additive changes are permitted within a patch release. |
...
- **URL canonicalization** — Now applies uniformly to brand.json agent URLs (`brand_agent_entry.url`, `brand_agent.url`, `rights_agent.url`). Two URLs differing only in case, default port, or percent-encoded unreserved characters compare equal during agent resolution. New reference page at [URL canonicalization](/dist/docs/3.0.8/reference/url-canonicalization).
...
Per the [experimental-surface contract](/dist/docs/3.0.8/reference/experimental-status), these surfaces accept additive changes within a patch release. Changes here do not affect agents that don't implement these protocols.
...
**Status:** General Availability | [AdCP 3.0 overview](/dist/docs/3.0.8/reference/whats-new-in-v3)
...
- **Request signing (optional in 3.0, required for Verified)** — If you plan to claim AdCP Verified, implement RFC 9421 Ed25519 signing per the [signing profile](/dist/docs/3.0.8/building/implementation/security#request-signing) and declare your signing key via the account surface. Test against `static/compliance/source/test-vectors/request-signing/` and the runner's `signed_requests` harness.
...
- **Capabilities** — Remove all boolean capability checks (`features.content_standards`, `brand.identity`, `trusted_match.supported`, etc.). Test for object presence instead. See the [prerelease upgrade notes](/dist/docs/3.0.8/reference/migration/prerelease-upgrades#capabilities-model-simplification) for the full field list.
- **Products** — Ensure all products include `reporting_capabilities`. This field is now required.
- **Media buy status** — Replace `pending_activation` with `pending_creatives` and `pending_start`. See [lifecycle states](/dist/docs/3.0.8/media-buy/media-buys#lifecycle-states).
...
- **Upgrading from v2?** Start with the [migration guide](/dist/docs/3.0.8/reference/migration).
- **Upgrading from a prerelease?** Go to the [prerelease upgrade notes](/dist/docs/3.0.8/reference/migration/prerelease-upgrades).
- **New to AdCP?** Read the [3.0 overview](/dist/docs/3.0.8/reference/whats-new-in-v3) and then [get started](/dist/docs/3.0.8/quickstart).

---

## Version 3.0.0-rc.3

**Status:** Release Candidate (superseded by 3.0.0) | [AdCP 3.0 overview](/dist/docs/3.0.8/reference/whats-new-in-v3) | [SDK support](/dist/docs/3.0.8/building/schemas-and-sdks#adcp-3-0-support)
...
| Governance context | Structured `governance-context.json` schema | Signed-JWS `governance_context` string (opaque to forwarders, cryptographically verifiable by auditors — see [security.mdx](/dist/docs/3.0.8/building/implementation/security#signed-governance-context)) |
...
**Status:** Release Candidate | [AdCP 3.0 overview](/dist/docs/3.0.8/reference/whats-new-in-v3)
...
**Status:** Beta | [AdCP 3.0 overview](/dist/docs/3.0.8/reference/whats-new-in-v3)
...
4. **Commerce Media** - Catalog-driven product discovery (`catalog` on `get_products`), catalog-driven packages, per-catalog-item delivery reporting (`by_catalog_item`), store catchment targeting, and `catalog_types` on products. Cross-retailer GTIN matching via catalog selectors. Commerce attribution metrics (`roas`, `new_to_brand_rate`) in delivery response. See the [Commerce Media Guide](/dist/docs/3.0.8/media-buy/commerce-media) and [Catalogs](/dist/docs/3.0.8/creative/catalogs).
...
**Status:** Beta | [Full Changelog](https://github.com/adcontextprotocol/adcp/blob/main/CHANGELOG.md#300-beta2) | [AdCP 3.0 overview](/dist/docs/3.0.8/reference/whats-new-in-v3)
...
**Status:** Beta | [Full Changelog](https://github.com/adcontextprotocol/adcp/blob/main/CHANGELOG.md#300-beta1) | [AdCP 3.0 overview](/dist/docs/3.0.8/reference/whats-new-in-v3)
...
Version 3.0.0 is a **major release** that expands AdCP beyond media buying into governance, brand suitability, and conversational commerce. See the [AdCP 3.0 overview](/dist/docs/3.0.8/reference/whats-new-in-v3) for detailed upgrade instructions.

**🎯 Key Themes:**

1. **Media Channel Taxonomy** - Complete overhaul from 9 format-oriented channels to 19 planning-oriented channels that reflect how buyers allocate budgets. See [Media Channel Taxonomy](/dist/docs/3.0.8/reference/media-channel-taxonomy).
...
3. **Sponsored Intelligence Protocol** - Conversational brand experiences in AI assistants. Defines how hosts invoke brand agent endpoints for rich engagement without breaking conversational flow. See [Sponsored Intelligence](/dist/docs/3.0.8/sponsored-intelligence/si-chat-protocol).
...
See [AdCP 3.0 overview](/dist/docs/3.0.8/reference/whats-new-in-v3) for detailed before/after examples and migration steps.
...
See [SI Chat Protocol](/dist/docs/3.0.8/sponsored-intelligence/si-chat-protocol) for complete documentation.
...
- [ ] Update channel enum values ([taxonomy guide](/dist/docs/3.0.8/reference/media-channel-taxonomy))
...
**[View AdCP 3.0 overview →](/dist/docs/3.0.8/reference/whats-new-in-v3)**
...
- `/schemas/3.0.8/` - Backward compat alias (same as v2)
...
See [Versioning & Governance](/dist/docs/3.0.8/reference/versioning) for the canonical version tiers, release cadence, 3.x stability guarantees, and deprecation policy. See the [v2 sunset page](/dist/docs/3.0.8/reference/v2-sunset) for the v2 end-of-life timeline.
