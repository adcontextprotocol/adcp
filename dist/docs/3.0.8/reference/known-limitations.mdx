...
Surfaces shipped in 3.x but not yet frozen are listed separately — see [Experimental Status](/dist/docs/3.0.8/reference/experimental-status).
...
- **No protocol guarantee about LLM prompt-injection defense.** That is an operator concern on every LLM-powered agent in the loop. See the [Security Model — threats specific to agentic advertising](/dist/docs/3.0.8/building/understanding/security-model#threats-specific-to-agentic-advertising).
- **Structural privacy applies only to TMP.** The Trusted Match Protocol enforces privacy structurally (separated code paths, schema prohibitions). Every other domain relies on contractual confidentiality or per-session consent. See [Privacy posture across domains](/dist/docs/3.0.8/protocol/architecture#privacy-posture-across-domains).
...
- **No latency or response-time SLA.** The protocol has no normative expectations for how quickly an agent must respond (unlike OpenRTB's per-auction `tmax`). Buyers and sellers negotiate timing through the commercial relationship or through task-specific [accountability terms](/dist/docs/3.0.8/media-buy/advanced-topics/accountability). A structured SLA declaration is deferred.
...
- [Security Model](/dist/docs/3.0.8/building/understanding/security-model) — the threat model and layered defense
- [Privacy Considerations](/dist/docs/3.0.8/reference/privacy-considerations) — cross-protocol privacy entry point
- [Experimental Status](/dist/docs/3.0.8/reference/experimental-status) — surfaces shipped in 3.x but not yet frozen
- [Versioning & Governance](/dist/docs/3.0.8/reference/versioning) — cadence, support windows, and how limitations become follow-ups
- [Roadmap](/dist/docs/3.0.8/reference/roadmap) — what's coming next
