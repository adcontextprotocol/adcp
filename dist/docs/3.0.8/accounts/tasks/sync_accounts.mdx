...
`sync_accounts` is used across all seller protocols: media buy agents, signals agents, governance agents, and creative agents. It declares the buyer's intent — the seller provisions or links accounts internally. For implicit accounts (`require_operator_auth: false`), use natural keys (`brand` + `operator`) on subsequent requests. For explicit accounts (`require_operator_auth: true`), discover seller-assigned account IDs via [`list_accounts`](/dist/docs/3.0.8/accounts/tasks/list_accounts). For sandbox on implicit accounts, include `sandbox: true` in the account entry — the seller provisions a test account with no real spend. For explicit accounts, sandbox accounts are pre-existing test accounts discovered via [`list_accounts`](/dist/docs/3.0.8/accounts/tasks/list_accounts).
...
**Request Schema**: [`/schemas/3.0.8/account/sync-accounts-request.json`](https://adcontextprotocol.org/schemas/3.0.8/account/sync-accounts-request.json)
**Response Schema**: [`/schemas/3.0.8/account/sync-accounts-response.json`](https://adcontextprotocol.org/schemas/3.0.8/account/sync-accounts-response.json)
...
|-------|------|----------|-------------|
| `brand` | object | Yes | Brand reference identifying the advertiser. Contains `domain` (house domain where brand.json is hosted) and optional `brand_id` (for multi-brand houses). See [brand-ref](/dist/docs/3.0.8/brand-protocol/brand-json). |
...
|--------|---------|-----------|
| `active` | Ready to use | Use [account reference](/dist/docs/3.0.8/building/integration/accounts-and-agents#account-references) in protocol operations |
...
If the buyer did not provide `push_notification_config`, poll [`list_accounts`](/dist/docs/3.0.8/accounts/tasks/list_accounts) to check for status changes.
...
- [list_accounts](/dist/docs/3.0.8/accounts/tasks/list_accounts) -- Poll for status changes on pending accounts
- [sync_governance](/dist/docs/3.0.8/accounts/tasks/sync_governance) -- Sync governance agents to accounts
- [Accounts and agents](/dist/docs/3.0.8/building/integration/accounts-and-agents) -- Billing models, trust models, and authorized operators
- [Brand protocol](/dist/docs/3.0.8/brand-protocol/brand-json) -- How seller agents resolve brand identity from the `brand.domain`
- [get_adcp_capabilities](/dist/docs/3.0.8/protocol/get_adcp_capabilities) -- Discover `supported_billing` and `require_operator_auth` before syncing accounts
