...
**Verification** follows the 14-step [request verifier checklist](/dist/docs/3.0.8/building/implementation/security#verifier-checklist-requests) with three webhook substitutions:

- Error codes use the `webhook_signature_*` prefix (see [Webhook error taxonomy](/dist/docs/3.0.8/building/implementation/security#webhook-error-taxonomy)).
...
The `created`/`expires`/`nonce` sig-params enforce a 5-minute max validity window and `(keyid, nonce)` replay dedup. See [Transport replay dedup](/dist/docs/3.0.8/building/implementation/security#transport-replay-dedup) for the per-keyid cap and memory-bounding rules.
:::
...
Normative rules for the legacy scheme are in [Webhook Security](/dist/docs/3.0.8/building/implementation/security#legacy-hmac-sha256-fallback-deprecated-removed-in-40).
...
**A2A webhooks:** Use the [A2A response extraction](/dist/docs/3.0.8/building/implementation/a2a-response-extraction) algorithm — final states extract from `.artifacts[0].parts[]` (last DataPart), interim states from `status.message.parts[]` (first DataPart).
...
See [Task Reference](/dist/docs/3.0.8/media-buy/task-reference) for details on `reporting_webhook`.

## Next steps

- [Task Lifecycle](/dist/docs/3.0.8/building/implementation/task-lifecycle) — status values and transitions
- [Async Operations](/dist/docs/3.0.8/building/implementation/async-operations) — handling long-running tasks
- [Error Handling](/dist/docs/3.0.8/building/implementation/error-handling) — webhook error patterns
