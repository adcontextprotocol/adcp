...
For the error schema itself, standard codes, and recovery strategies, see [Error Handling](/dist/docs/3.0.8/building/by-layer/L3/error-handling).
...
This follows the [A2A Response Format](/dist/docs/3.0.8/building/by-layer/L0/a2a-response-format) conventions: final states use `.artifacts` for data.
...
AdCP exposes errors in two distinct places. This page covers the **transport envelope** (`adcp_error`); the **payload errors array** (`errors[]`) is covered in [Error Handling — Envelope vs. payload errors](/dist/docs/3.0.8/building/by-layer/L3/error-handling#envelope-vs-payload-errors-the-two-layer-model).
...
- [Error Handling](/dist/docs/3.0.8/building/by-layer/L3/error-handling) — error schema, standard codes, recovery strategies
- [MCP Guide](/dist/docs/3.0.8/building/by-layer/L0/mcp-guide) — MCP transport integration
- [A2A Guide](/dist/docs/3.0.8/building/by-layer/L0/a2a-guide) — A2A transport integration
- [A2A Response Format](/dist/docs/3.0.8/building/by-layer/L0/a2a-response-format) — canonical A2A response structure
