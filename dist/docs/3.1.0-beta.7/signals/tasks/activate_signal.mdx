...
**Request Schema**: [`https://adcontextprotocol.org/schemas/3.1.0-beta.7/signals/activate-signal-request.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.7/signals/activate-signal-request.json)
**Response Schema**: [`https://adcontextprotocol.org/schemas/3.1.0-beta.7/signals/activate-signal-response.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.7/signals/activate-signal-response.json)
...
|-----------|------|----------|-------------|
| `idempotency_key` | string | Yes | Client-generated unique key for this request. Prevents duplicate activations on retries. MUST be unique per (seller, request) pair. Min 16 chars. See [Idempotency](/dist/docs/3.1.0-beta.7/building/by-layer/L1/security#idempotency) for normative semantics. |
...
| `account` | [AccountRef](/dist/docs/3.1.0-beta.7/building/by-layer/L2/accounts-and-agents#account-references) | No | Account for this activation. Associates with a commercial relationship established via `sync_accounts`. |
| `pricing_option_id` | string | Yes (if signal has pricing options) | The pricing option selected from `pricing_options` in the [`get_signals`](/dist/docs/3.1.0-beta.7/signals/tasks/get_signals) response. Records the buyer's pricing commitment at activation time. Pass this same value in subsequent `report_usage` calls. |
...
```json
{
  "$schema": "/schemas/3.1.0-beta.7/signals/activate-signal-response.json",
  "status": "completed",
...
See **[Webhooks](/dist/docs/3.1.0-beta.7/building/by-layer/L3/webhooks)** for complete details on webhook configuration and reliability.
