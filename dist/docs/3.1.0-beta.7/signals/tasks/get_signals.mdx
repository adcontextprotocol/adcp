...
**Request Schema**: [`https://adcontextprotocol.org/schemas/3.1.0-beta.7/signals/get-signals-request.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.7/signals/get-signals-request.json)
**Response Schema**: [`https://adcontextprotocol.org/schemas/3.1.0-beta.7/signals/get-signals-response.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.7/signals/get-signals-response.json)
...
```json
{
  "$schema": "/schemas/3.1.0-beta.7/signals/get-signals-response.json",
  "status": "completed",
...
```json
{
  "$schema": "/schemas/3.1.0-beta.7/signals/get-signals-request.json",
...
When a consumer (storefront, federated marketplace, registry) needs to mirror a signals agent's full priced signals feed, set `discovery_mode: "wholesale"` and omit `signal_spec` / `signal_refs` / deprecated `signal_ids`. Wholesale enumeration is symmetric with [`get_products` `buying_mode: "wholesale"`](/dist/docs/3.1.0-beta.7/media-buy/task-reference/get_products): synchronous, paginated, firm-priced, with partial completion declared via `incomplete[]`.
...
Signals agents declare wholesale support in [`get_adcp_capabilities`](/dist/docs/3.1.0-beta.7/protocol/get_adcp_capabilities):
