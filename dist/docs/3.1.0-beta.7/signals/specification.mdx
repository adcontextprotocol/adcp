...
- **Account**: The commercial relationship on whose behalf the request is made. See [Accounts Protocol](/dist/docs/3.1.0-beta.7/accounts/overview).
...
- Signal agents MUST return `REFERENCE_NOT_FOUND` for unauthorized accounts — the
  ...
  [error-handling.mdx](/dist/docs/3.1.0-beta.7/building/by-layer/L3/error-handling) for the full
...
See [Declaring governance metadata](/dist/docs/3.1.0-beta.7/signals/data-providers#declaring-governance-metadata) for implementation details.
...
**Schema**: [`get-signals-request.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.7/signals/get-signals-request.json) / [`get-signals-response.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.7/signals/get-signals-response.json)

**Reference**: [`get_signals` task](/dist/docs/3.1.0-beta.7/signals/tasks/get_signals)
...
**Schema**: [`activate-signal-request.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.7/signals/activate-signal-request.json) / [`activate-signal-response.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.7/signals/activate-signal-response.json)

**Reference**: [`activate_signal` task](/dist/docs/3.1.0-beta.7/signals/tasks/activate_signal)
...
Signal agents MUST return errors using the [standard AdCP error schema](/dist/docs/3.1.0-beta.7/building/by-layer/L3/error-handling).

Signal agents MUST use Signals Protocol error codes as defined in the [Error Handling Reference](/dist/docs/3.1.0-beta.7/building/by-layer/L3/error-handling).
...
When a seller-offered signal should be applied only to specific packages on a sales-agent buy, orchestrators SHOULD carry that selection in `create_media_buy.packages[].targeting_overlay.signal_targeting_groups`, including the selected signal `pricing_option_id` when required and the selected `signal_ref`. `get_signals` is the broad wholesale discovery surface, while the selected product's `signal_targeting_allowed`, optional inline `signal_targeting_options`, and `signal_targeting_rules` govern buy-time eligibility and pricing. `included_signals` is descriptive only: it identifies signals already bundled into or planned into the product, but does not make them selectable. If a wholesale product allows signal targeting but omits inline options, buyers use `get_signals` as the selectable signal feed. If the product option or `get_signals` result exposes a separate `signal_agent_segment_id` required by the seller, buyers echo it as an execution handle; otherwise `signal_ref` is sufficient. This is the package-level binding; `audience_include` and `audience_exclude` remain scoped to buyer-managed audiences from `sync_audiences`.
...
| Schema | Description |
|--------|-------------|
| [`signals/get-signals-request.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.7/signals/get-signals-request.json) | get_signals request |
| [`signals/get-signals-response.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.7/signals/get-signals-response.json) | get_signals response |
| [`signals/activate-signal-request.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.7/signals/activate-signal-request.json) | activate_signal request |
| [`signals/activate-signal-response.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.7/signals/activate-signal-response.json) | activate_signal response |
| [`core/deployment.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.7/core/deployment.json) | Deployment target |
| [`core/activation-key.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.7/core/activation-key.json) | Activation key |
