...
For the layered model behind this — what each layer contains and what an SDK at each layer should provide — see the [SDK stack reference](/dist/docs/3.1.0-beta.7/building/cross-cutting/sdk-stack).

## Coverage matrix

What "shipped" means at each layer is the [L0–L3 checklist](/dist/docs/3.1.0-beta.7/building/cross-cutting/sdk-stack#what-an-sdk-at-each-layer-should-provide).
...
Pre-3.0 callers should work through the [3.0 migration guide](/dist/docs/3.1.0-beta.7/reference/migration/index) before upgrading. For the at-a-glance status of every published protocol version, see [Versions & Compatibility](/dist/docs/3.1.0-beta.7/reference/versions). For procurement-grade context on the support window, see the [v2 sunset timeline](/dist/docs/3.1.0-beta.7/reference/v2-sunset) and [versioning & governance](/dist/docs/3.1.0-beta.7/reference/versioning).

**Python and TypeScript carry full L0–L4 coverage as the first-class supported languages.** **Go** is moving in the same direction. **Other languages** are not on the official roadmap; community-maintained ports are welcome — see the [Builders Working Group](/dist/docs/3.1.0-beta.7/community/working-group) and the [Slack community](https://join.slack.com/t/agenticads/shared_invite/zt-3c5sxvdjk-x0rVmLB3OFHVUp~WutVWZg).
...
- `@adcp/sdk/testing` — buyer-side storyboard runner (`runStoryboard`, `comply`, `testAgent`) and seller-side controller scaffold (`createComplyController` — see [Get Test-Ready](/dist/docs/3.1.0-beta.7/building/verification/get-test-ready))
...
- **[Build an agent](/dist/docs/3.1.0-beta.7/building/by-layer/L4/build-an-agent)** — server-side L4 path. Skill files + coding agent.
- **[Build a caller](/dist/docs/3.1.0-beta.7/building/by-layer/L4/build-a-caller)** — client-side L4 path. Install, call, handle responses, ingest reporting.
- **[Schemas](/dist/docs/3.1.0-beta.7/building/by-layer/L0/schemas)** — schema bundle, type generation, version pinning.
- **[Migrate from hand-rolled](/dist/docs/3.1.0-beta.7/building/by-layer/L4/migrate-from-hand-rolled)** — already running an AdCP agent built before the SDKs covered much? Swap one layer at a time.
