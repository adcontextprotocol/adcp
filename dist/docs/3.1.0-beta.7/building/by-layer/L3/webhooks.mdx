...
**Verification** follows the 14-step [request verifier checklist](/dist/docs/3.1.0-beta.7/building/by-layer/L1/security#verifier-checklist-requests) with three webhook substitutions:

- Error codes use the `webhook_signature_*` prefix (see [Webhook error taxonomy](/dist/docs/3.1.0-beta.7/building/by-layer/L1/security#webhook-error-taxonomy)).
...
The `created`/`expires`/`nonce` sig-params enforce a 5-minute max validity window and `(keyid, nonce)` replay dedup. See [Transport replay dedup](/dist/docs/3.1.0-beta.7/building/by-layer/L1/security#transport-replay-dedup) for the per-keyid cap and memory-bounding rules.
:::
...
Normative rules for the legacy scheme are in [Webhook Security](/dist/docs/3.1.0-beta.7/building/by-layer/L1/security#legacy-hmac-sha256-fallback-deprecated-removed-in-40).
...
**A2A webhooks:** Use the [A2A response extraction](/dist/docs/3.1.0-beta.7/building/by-layer/L0/a2a-response-extraction) algorithm — final states extract from `.artifacts[0].parts[]` (last DataPart), interim states from `status.message.parts[]` (first DataPart).
...
See [Task Reference](/dist/docs/3.1.0-beta.7/media-buy/task-reference) for details on `reporting_webhook`.
...
This section is the transport half of the [Snapshot and log contract](/dist/docs/3.1.0-beta.7/protocol/snapshot-and-log). For the read-side rules (snapshot is authoritative, replay = re-read), see that page.

### Delivery semantics

- **At-least-once delivery.** Sellers MAY re-fire the same logical event under retry. Receivers MUST dedupe transport retries by `idempotency_key`. For state-shaped events that also carry a typed `notification_id` (see [`mcp-webhook-payload.json`](https://adcontextprotocol.org/schemas/3.1.0-beta.7/core/mcp-webhook-payload.json) and [snapshot-and-log Rule 1](/dist/docs/3.1.0-beta.7/protocol/snapshot-and-log#1-two-distinct-ids-per-fire-and-per-state)), receivers MUST also track `notification_id` to correlate fires to current snapshot state — seeing the same `notification_id` under two different `idempotency_key` values is a re-emission signal, not a transport retry.
...
- [Task Lifecycle](/dist/docs/3.1.0-beta.7/building/by-layer/L3/task-lifecycle) — status values and transitions
- [Async Operations](/dist/docs/3.1.0-beta.7/building/by-layer/L3/async-operations) — handling long-running tasks
- [Error Handling](/dist/docs/3.1.0-beta.7/building/by-layer/L3/error-handling) — webhook error patterns
