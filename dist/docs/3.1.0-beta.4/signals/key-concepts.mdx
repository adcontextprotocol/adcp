...
|--------|--------|--------------|
| `data_provider` | `data_provider_domain` + `signal_id` | Verifiable via the publisher domain's adagents.json signal catalog |
...
**Data-provider catalog signals** come from external data providers who publish their offerings at `/.well-known/adagents.json`. Buyers can independently verify that a seller or signal source is authorized to resell them.

**Signal-source signals** are proprietary to a signal source and use `scope: "signal_source"` with that source's `signal_source_url` when they are not published in an adagents.json signal catalog.
...
| Task | Purpose |
|------|---------|
| [`get_signals`](/dist/docs/3.1.0-beta.4/signals/tasks/get_signals) | Discover signals matching campaign criteria |
| [`activate_signal`](/dist/docs/3.1.0-beta.4/signals/tasks/activate_signal) | Activate a signal for use in campaigns |
...
The Signals Protocol operates within the broader [AdCP ecosystem](/dist/docs/3.1.0-beta.4/intro#the-adcp-ecosystem-layers). Signal agents integrate directly with decisioning platforms (DSPs, orchestration platforms), eliminating intermediary reporting and usage tracking. Signal agents advertise their available data providers via [`get_adcp_capabilities`](/dist/docs/3.1.0-beta.4/protocol/get_adcp_capabilities).
...
- [Data provider guide](/dist/docs/3.1.0-beta.4/signals/data-providers) — how to publish a signal catalog
- [Signals ecosystem](/dist/docs/3.1.0-beta.4/signals/ecosystem) — how each type of company participates
- [Protocol specification](/dist/docs/3.1.0-beta.4/signals/specification) — formal conformance requirements
- [get_signals task reference](/dist/docs/3.1.0-beta.4/signals/tasks/get_signals) — discovery API details
- [activate_signal task reference](/dist/docs/3.1.0-beta.4/signals/tasks/activate_signal) — activation API details
