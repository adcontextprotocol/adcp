...
Buyers integrating with receivers that have not yet adopted the RFC 9421 webhook profile MAY opt into the legacy HMAC-SHA256 scheme by populating `push_notification_config.authentication.credentials`. That path is deprecated and removed in AdCP 4.0 — see [Legacy HMAC-SHA256 fallback](#legacy-hmac-sha256-fallback-deprecated) below. Because the inbound request that registers the webhook is typically not 9421-signed in 3.0, the `authentication` block is susceptible to on-path strip/inject — see [Downgrade and injection resistance](/dist/docs/3.2.0-beta.10/building/by-layer/L1/security#webhook-callbacks) for the operational mitigations.
:::
...
**Verification** follows the 14-step [request verifier checklist](/dist/docs/3.2.0-beta.10/building/by-layer/L1/security#verifier-checklist-requests) with three webhook substitutions:

- Error codes use the `webhook_signature_*` prefix (see [Webhook error taxonomy](/dist/docs/3.2.0-beta.10/building/by-layer/L1/security#webhook-error-taxonomy)).
...
The `created`/`expires`/`nonce` sig-params enforce a 5-minute max validity window and `(keyid, nonce)` replay dedup. See [Transport replay dedup](/dist/docs/3.2.0-beta.10/building/by-layer/L1/security#transport-replay-dedup) for the per-keyid cap and memory-bounding rules.
:::
...
Normative rules for the legacy scheme are in [Webhook Security](/dist/docs/3.2.0-beta.10/building/by-layer/L1/security#legacy-hmac-sha256-fallback-deprecated-removed-in-40).
...
contract](/dist/docs/3.2.0-beta.10/building/by-layer/L3/async-identity-and-convergence). Delivery
...
**A2A webhooks:** Use the [A2A response extraction](/dist/docs/3.2.0-beta.10/building/by-layer/L0/a2a-response-extraction) algorithm — final states extract from `.artifacts[0].parts[]` (last DataPart), interim states from `status.message.parts[]` (first DataPart).
...
See [Task Reference](/dist/docs/3.2.0-beta.10/media-buy/task-reference) for details on `reporting_webhook`.

## Agent-level webhooks

Agent-level webhooks outlive any account or media buy. They are registered with [`sync_principal`](/dist/docs/3.2.0-beta.10/protocol/sync_principal), or with the specialized [`sync_agent_notification_configs`](/dist/docs/3.2.0-beta.10/protocol/sync_agent_notification_configs) compatibility task, not with account-level `notification_configs[]` or per-task `push_notification_config`.

The initial agent-level event type is `capabilities.changed`. It invalidates cached [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.10/protocol/get_adcp_capabilities) responses. The webhook payload is deliberately small and does not contain the full capability document; receivers re-read `get_adcp_capabilities` and replace their cached snapshot from the fresh response. Sellers MUST publish the new capability snapshot before firing so the webhook's `capabilities_version` is observable on read. Receivers SHOULD retry the read with normal transient-error backoff if the webhook revision is not yet observable.
...
This section is the transport half of the [Snapshot and log contract](/dist/docs/3.2.0-beta.10/protocol/snapshot-and-log). For the read-side rules (snapshot is authoritative, replay = re-read), see that page.

### Delivery semantics

- **At-least-once delivery.** Sellers MAY re-fire the same logical event under retry. Receivers MUST dedupe transport retries by `idempotency_key`. For state-shaped events that also carry a typed `notification_id` (see [`mcp-webhook-payload.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.10/core/mcp-webhook-payload.json) and [snapshot-and-log Rule 1](/dist/docs/3.2.0-beta.10/protocol/snapshot-and-log#1-separate-transport-event-and-resource-identity)), receivers MUST also track `notification_id` to correlate fires to current snapshot state — seeing the same `notification_id` under two different `idempotency_key` values is a re-emission signal, not a transport retry.
...
- [Task Lifecycle](/dist/docs/3.2.0-beta.10/building/by-layer/L3/task-lifecycle) — status values and transitions
- [Async Operations](/dist/docs/3.2.0-beta.10/building/by-layer/L3/async-operations) — handling long-running tasks
- [Error Handling](/dist/docs/3.2.0-beta.10/building/by-layer/L3/error-handling) — webhook error patterns
