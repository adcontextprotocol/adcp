...
already serves [`get_media_buy_delivery`](/dist/docs/3.2.0-beta.10/media-buy/task-reference/get_media_buy_delivery)
...
4. **[`get_reporting_status`](/dist/docs/3.2.0-beta.10/media-buy/task-reference/get_reporting_status)
...
3. **Generate obligations.** When an account installs a delivery
   configuration through [`sync_accounts`](/dist/docs/3.2.0-beta.10/accounts/tasks/sync_accounts),
...
- **`managed_delivery: true`** adds offerings with a `method` —
  ...
  principal-layer destinations ([`sync_principal`](/dist/docs/3.2.0-beta.10/protocol/sync_principal)).
