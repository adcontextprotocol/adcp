...
<Tip>
**Pricing Models**
Products declare which pricing models they support. Buyers select a specific pricing option when creating media buys. See the complete [Pricing Models Guide](/dist/docs/3.2.0-beta.10/media-buy/advanced-topics/pricing-models) for details on CPM, CPCV, CPP, CPC, CPA, vCPM, flat rate, and time-based pricing.
</Tip>
...
- `installments` (list[Installment], optional): Specific installments available within this product. See [Collections and installments](/dist/docs/3.2.0-beta.10/media-buy/product-discovery/collections-and-installments).
...
- `catalog_types` (list[string], optional): Catalog types this product supports for catalog-driven campaigns. A sponsored product listing declares `["product"]`, a job board declares `["job", "offering"]`. Buyers match synced catalogs to products via this field. See [Catalogs](/dist/docs/3.2.0-beta.10/creative/catalogs).
...
Products that support `optimization_goals` with `kind: "event"` declare their capabilities in `conversion_tracking`. Seller-level capabilities (supported event types, UID types, attribution windows) are declared in [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.10/protocol/get_adcp_capabilities).
...
See [Conversion Tracking & Optimization Goals](/dist/docs/3.2.0-beta.10/media-buy/conversion-tracking) for the full optimization goals reference.
...
Schema: [`core/audience-activation-method.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.10/core/audience-activation-method.json)

#### Activation method patterns

Each entry declares a `pattern` plus pattern-specific fields. Vendor identity is a [BrandRef](/dist/docs/3.2.0-beta.10/brand-protocol/brand-json) domain — new platforms declare a domain, no enum change required.

| Pattern | Required fields | Meaning |
|---------|----------------|---------|
| `sync_audiences` | — | Inline audience member pushes via the AdCP [`sync_audiences`](/dist/docs/3.2.0-beta.10/media-buy/task-reference/sync_audiences) task. Any AdCP-speaking buyer can use it. |
...
| `platform_distribution` | `vendor` | The vendor's rails deliver segments to the seller's destination. The buyer initiates distribution in the vendor's system, then binds the arriving segment with a [`platform_segment` source reference](/dist/docs/3.2.0-beta.10/media-buy/task-reference/sync_audiences#external-audience-sources-experimental) on `sync_audiences`. Optional `destination_ref` is an opaque account-scoped destination or seat reference; omit it until bilateral setup establishes the destination. Optional `bind_expiry_days` declares when an unfulfilled bind may expire. |
...
Buyers filter discovery with [`filters.audience_activation_methods`](/dist/docs/3.2.0-beta.10/media-buy/task-reference/get_products): OR across requested entries, AND within an entry, omitted optional fields are wildcards, `directions` matches on non-empty intersection (a method that omits `directions` matches any requested directions), and `vendor` matches on `domain` plus `brand_id` when specified.

**The runtime leg.** These declarations are the discovery half. The runtime step — telling the seller which share or arriving segment maps to which targetable audience — is an [external source reference on `sync_audiences`](/dist/docs/3.2.0-beta.10/media-buy/task-reference/sync_audiences#external-audience-sources-experimental) (`dataset` for `dataset_query` grants, `platform_segment` for `platform_distribution` deliveries), under the same experimental feature id. Inline push works on any seller declaring `sync_audiences`; source references work on sellers declaring the corresponding grant/distribution patterns.
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.10/pricing-options/cpcv-option.json",
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.10/pricing-options/cpm-option.json",
...
Guaranteed products can also declare `performance_standards`, `measurement_terms`, and `cancellation_policy` that define accountability obligations at the buy level. See [Accountability](/dist/docs/3.2.0-beta.10/media-buy/advanced-topics/accountability).
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.10/core/creative-policy.json",
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.10/core/placement.json",
  "kind": "publisher_ref",
...
See [Creative Assignment and Placement Targeting](/dist/docs/3.2.0-beta.10/media-buy/media-buys/index.mdx#creative-assignment-and-placement-targeting) for more details.
...
See [Collections and installments](/dist/docs/3.2.0-beta.10/media-buy/product-discovery/collections-and-installments) for the full model, examples, and targeting details.
...
See [`get_products` refinement](/dist/docs/3.2.0-beta.10/media-buy/task-reference/get_products#refinement) for the full workflow and examples.
...
The split 3.2 [`request_proposals`](/dist/docs/3.2.0-beta.10/media-buy/task-reference/request_proposals) task returns immutable drafts. [`refine_proposals`](/dist/docs/3.2.0-beta.10/media-buy/task-reference/refine_proposals) either creates another draft revision or, with `action: "finalize"`, returns a committed snapshot with inventory held until `expires_at`. [`accept_proposal`](/dist/docs/3.2.0-beta.10/media-buy/task-reference/accept_proposal) is the acceptance/execution step. Legacy `get_products` retains its equivalent finalize form throughout 3.x.
...
Products are discovered through the [Product Discovery](/dist/docs/3.2.0-beta.10/media-buy/product-discovery) process, which uses natural language to match campaign briefs with available inventory. Once products are identified, they can be purchased via `create_media_buy`.

## See Also

- [Product Discovery](/dist/docs/3.2.0-beta.10/media-buy/product-discovery) - How to discover products using natural language
- [Media Buys](/dist/docs/3.2.0-beta.10/media-buy/media-buys) - How to purchase products
- [Targeting](/dist/docs/3.2.0-beta.10/media-buy/advanced-topics/targeting) - Detailed targeting options
- [Creative Formats](/dist/docs/3.2.0-beta.10/creative/formats) - Understanding format specifications and discovery
