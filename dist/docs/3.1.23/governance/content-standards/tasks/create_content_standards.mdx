...
**Schema**: [create-content-standards-request.json](https://adcontextprotocol.org/schemas/3.1.23/content-standards/create-content-standards-request.json)
...
```json
{
  "$schema": "/schemas/3.1.23/content-standards/create-content-standards-request.json",
...
**Schema**: [create-content-standards-response.json](https://adcontextprotocol.org/schemas/3.1.23/content-standards/create-content-standards-response.json)

### Success Response

```json
{
  "$schema": "/schemas/3.1.23/content-standards/create-content-standards-response.json",
  "status": "completed",
...
<Warning>
**Calibrate before production use.** After creating content standards, sellers MUST calibrate their local evaluation models via [`calibrate_content`](/dist/docs/3.1.23/governance/content-standards/tasks/calibrate_content) before using the standards in production. Uncalibrated standards produce inconsistent evaluations — the verification agent and the seller's local model may interpret the same policy differently without alignment. See the [Calibration section](/dist/docs/3.1.23/governance/content-standards/index#calibration) for the full workflow.
</Warning>
