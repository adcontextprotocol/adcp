...
Sales agents that already implement the [accounts protocol](/dist/docs/3.1.23/accounts/overview) for media buys do not need additional account setup for creative tasks — the same account covers both protocols.
...
- **Creative library**: For sellers that advertise `creative.has_creative_library: true`, the `status` field on each creative in [`list_creatives`](/dist/docs/3.1.23/creative/task-reference/list_creatives) uses the creative status enum: `processing`, `pending_review`, `approved`, `rejected`, or `archived`.
- **Package level**: The `approval_status` field on each creative in [`get_media_buys`](/dist/docs/3.1.23/media-buy/task-reference/get_media_buys) responses uses the creative approval status enum: `pending_review`, `approved`, or `rejected`. Rejected creatives include a `rejection_reason` with a human-readable explanation.
...
See [Previewing generative creative](/dist/docs/3.1.23/creative/task-reference/preview_creative#previewing-generative-creative) for the full mental model and expectations table.
...
- [Creative Specification](/dist/docs/3.1.23/creative/specification) — Protocol requirements for creative agents
- [Implementing Creative Agents](/dist/docs/3.1.23/creative/implementing-creative-agents) — Implementation guide for standalone creative agents
- [Generative Creative](/dist/docs/3.1.23/creative/generative-creative) — Using `build_creative` for AI-powered generation
- [get_creative_delivery](/dist/docs/3.1.23/creative/task-reference/get_creative_delivery) — Variant-level delivery reporting
- [get_media_buy_delivery](/dist/docs/3.1.23/media-buy/task-reference/get_media_buy_delivery) — Media buy delivery reporting
- [Media Buy Lifecycle](/dist/docs/3.1.23/media-buy/media-buys/lifecycle) — State machine, sequenced flow, guaranteed deal IO path, creative sync timing
