...
[billing authority](/dist/docs/3.1.23/media-buy/advanced-topics/billing-authority), not
...
5. exposes the caller's history through
   [`get_reporting_status`](/dist/docs/3.1.23/media-buy/task-reference/get_reporting_status);
   and
...
seller MUST reject `consumer` with [`UNSUPPORTED_FEATURE`](/dist/docs/3.1.23/building/verification/compliance-catalog#error-code-unsupported-feature). Tracked in
