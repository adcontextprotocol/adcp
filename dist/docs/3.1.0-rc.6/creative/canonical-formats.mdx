...
description: "Canonical formats live in AdCP 3.1; sellers' products narrow them inline, with experimental flags on immature canonicals and product declarations."
...
> **Status:** 3.1 surface in release-candidate preparation. The canonical-formats framework ships with 3.1; until GA, adopters should use exact prerelease pins advertised by `supported_versions` (for example `"3.1-beta.7"` today and `"3.1-rc.1"` once an RC artifact exists). Three canonicals plus `custom` remain marked `experimental` until adopter evidence supports promotion. Historical design context lives in [RFC #3305](https://github.com/adcontextprotocol/adcp/issues/3305) and [#3307](https://github.com/adcontextprotocol/adcp/pull/3307).
>
...
For hands-on authoring practice, use the [S2 creative specialist module](/dist/docs/3.1.0-rc.6/learning/specialist/creative), which links to this reference instead of duplicating it.
...
| **`format_shape`** | Recognized global pattern from the [format-shape vocabulary registry](https://adcontextprotocol.org/schemas/3.1.0-rc.6/core/format-shape-vocabulary.json). Required when `format_kind: "custom"`. |
...
| Brand identity | Sometimes redeclared as format slots | Implicit via `brand` (a [`BrandRef`](https://adcontextprotocol.org/schemas/3.1.0-rc.6/core/brand-ref.json) — `domain` plus optional `brand_id` for house-of-brands) resolving brand.json; explicit override via `brand_kit_override` inline on the BrandRef itself |
...
|---|---|---|
| `sponsored_placement` | Four meaningfully different retail-media adapter contracts (Amazon SP, Criteo SP / CitrusAd SP, Pinterest Collection, generative-per-SKU) under one canonical — see [Sponsored Placement adapter contracts](/dist/docs/3.1.0-rc.6/creative/sponsored-placement-adapter-contracts) | Two adopters with `format_schema` evidence |
...
Format `slots` reference canonical `asset_group_id` values from the [vocabulary registry](https://adcontextprotocol.org/schemas/3.1.0-rc.6/core/asset-group-vocabulary.json). The current canonical entries:
...
See [`build_creative` task reference](/dist/docs/3.1.0-rc.6/creative/task-reference/build_creative) for the full request/response shape.
...
- **Product satisfaction** answers whether a submitted or requested creative is specific enough for a product's accepted format declaration. When the product declares a fixed `width`, `height`, `duration_ms_exact`, or equivalent duration range, the requested creative or package selector MUST declare and match that constraint. An under-specified request is not a wildcard for product gating.

Range constraints use containment, not overlap. A range-based request satisfies a product only when every value the request permits falls within the product's accepted range; overlap alone is insufficient. An exact value, such as `duration_ms_exact`, satisfies a range when that exact value falls inside the accepted range.
...
- **Universal macros as a new schema** — already documented at [`/docs/creative/universal-macros`](/dist/docs/3.1.0-rc.6/creative/universal-macros). Canonical formats reference them by name.
...
- [S2 creative specialist module](/dist/docs/3.1.0-rc.6/learning/specialist/creative) — training lab for reading product `format_options[]`, selecting `format_kind`, and mapping `asset_source`
- [v1 → canonical-formats migration guide](/dist/docs/3.1.0-rc.6/creative/canonical-formats-migration) — concrete migration paths for sellers, creative agents, buyers, and publisher-direct integrations
...
- [Asset group vocabulary](https://adcontextprotocol.org/schemas/3.1.0-rc.6/core/asset-group-vocabulary.json) — canonical slot-name registry
- [Video Brief schema](https://adcontextprotocol.org/schemas/3.1.0-rc.6/creative/video-brief.json) — typed per-segment generation brief for build_creative (renamed from earlier `scenes`)
- [Universal macros](/dist/docs/3.1.0-rc.6/creative/universal-macros) — substitution patterns referenced from canonical tracking
