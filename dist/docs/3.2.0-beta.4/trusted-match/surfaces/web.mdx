...
- `signals_by_provider` preserves which provider emitted each targeting pair. The Prebid module resolves each `(provider_id, key)` through the publisher's [`targeting_kv_mapping`](/dist/docs/3.2.0-beta.4/trusted-match/specification#publisher-targeting-kv-mapping) before setting the resulting GAM key. Unmapped tuples are dropped.
...
The Prebid module resolves the context match `signals_by_provider` tuples through the publisher's [`targeting_kv_mapping`](/dist/docs/3.2.0-beta.4/trusted-match/specification#publisher-targeting-kv-mapping):
