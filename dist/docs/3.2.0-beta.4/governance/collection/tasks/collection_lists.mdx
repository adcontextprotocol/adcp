...
Collection lists are managed, cacheable artifacts that express "these collections, filtered by these criteria." They parallel [property lists](/dist/docs/3.2.0-beta.4/governance/property/tasks/property_lists) but operate on content programs (shows, series, podcasts) rather than technical surfaces (domains, apps).
...
|------|---------|---------------|
| [`create_collection_list`](/dist/docs/3.2.0-beta.4/governance/collection/tasks/collection_lists#create_collection_list) | Create a new collection list | Seconds |
| [`get_collection_list`](/dist/docs/3.2.0-beta.4/governance/collection/tasks/collection_lists#get_collection_list) | Fetch list with resolved collections | Seconds (cached) |
| [`update_collection_list`](/dist/docs/3.2.0-beta.4/governance/collection/tasks/collection_lists#update_collection_list) | Modify base collections or filters | Seconds |
| [`list_collection_lists`](/dist/docs/3.2.0-beta.4/governance/collection/tasks/collection_lists#list_collection_lists) | List collection lists for an account | Seconds |
| [`delete_collection_list`](/dist/docs/3.2.0-beta.4/governance/collection/tasks/collection_lists#delete_collection_list) | Remove a collection list | Seconds |
...
**Content ratings are metadata filters, not content evaluation.** `content_ratings_exclude: [{ system: "tv_parental", rating: "TV-MA" }]` excludes all collections *declared* as TV-MA. It doesn't evaluate individual episodes — that's [content standards](/dist/docs/3.2.0-beta.4/governance/content-standards/index).
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.4/collection/create-collection-list-request.json",
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.4/collection/create-collection-list-response.json",
  "status": "completed",
  "list": {
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.4/collection/get-collection-list-request.json",
  "list_id": "cl_novamotors_dna_2026",
  "resolve": true,
  "pagination": {
    "max_results": 1000
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.4/collection/get-collection-list-response.json",
  "status": "completed",
  "list": {
    "list_id": "cl_novamotors_dna_2026",
    "name": "Nova Motors CTV Do Not Air — 2026",
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.4/collection/update-collection-list-request.json",
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.4/collection/list-collection-lists-request.json",
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-beta.4/collection/delete-collection-list-request.json",
...
Webhooks contain a summary only — recipients must call [`get_collection_list`](/dist/docs/3.2.0-beta.4/governance/collection/tasks/collection_lists#get_collection_list) for the updated entries. Recipients MUST verify the `signature` before processing and MUST dedupe by `idempotency_key` so retried deliveries of the same change event are ignored.
...
Collection lists gate delivery decisions, so the `auth_token` and webhook callbacks need explicit lifecycle rules. General controls in [Security](/dist/docs/3.2.0-beta.4/building/by-layer/L1/security) apply; the collection-list-specific rules:

**`auth_token` scope, revocation, and log hygiene.** Each token authorizes exactly one `list_id`; do not reuse a token across lists. Governance agents MUST issue a distinct token per seller that receives the list — shared tokens cannot be revoked per relationship and make list-wide rotation the only response to a single compromise. Tokens MUST NOT be written to logs, cache keys, or metric labels, and error responses from [`get_collection_list`](/dist/docs/3.2.0-beta.4/governance/collection/tasks/collection_lists#get_collection_list) MUST NOT echo the presented token.

[`delete_collection_list`](/dist/docs/3.2.0-beta.4/governance/collection/tasks/collection_lists#delete_collection_list) and per-seller revocation differ:
...
The pattern matches [property list sharing](/dist/docs/3.2.0-beta.4/governance/property/index#sharing-property-lists-with-sellers):
