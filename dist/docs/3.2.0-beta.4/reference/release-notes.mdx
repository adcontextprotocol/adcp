...
Authoritative version-by-version release record for AdCP, with cumulative change detail, migration guidance, and per-version adoption notes. For the 3.2 adopter overview, see [What's New in AdCP 3.2](/dist/docs/3.2.0-beta.4/reference/whats-new-in-3-2); for the upgrade checklist, see [Migrating from 3.1 to 3.2](/dist/docs/3.2.0-beta.4/reference/migration/3-1-to-3-2); and for prerelease artifact and SDK timing, see the [3.2 beta program](/dist/docs/3.2.0-beta.4/reference/3-2-beta). For the at-a-glance status of every published version, see [Versions & Compatibility](/dist/docs/3.2.0-beta.4/reference/versions). For source-level changelogs, see [CHANGELOG.md](https://github.com/adcontextprotocol/adcp/blob/main/CHANGELOG.md). For version stability, schema-change scope, and the 3.x guarantees see [Versioning & Governance](/dist/docs/3.2.0-beta.4/reference/versioning). For v2 end-of-life see the [v2 sunset page](/dist/docs/3.2.0-beta.4/reference/v2-sunset).

---

## Version 3.2.0

**Status:** Beta cycle — minor release targeting the 3.2.0 milestone. Beta.0 was the protocol-first cut used as the canonical input for SDK work. Beta.3 is the current checkpoint, with exact TypeScript support in `@adcp/sdk@14.0.0-beta.4`. Stable wire shapes remain backward compatible except for the ratified `media_buy_status` cleanup and request-signature encoding migration described below; explicitly experimental surfaces may carry noticed changes under the [experimental-status contract](/dist/docs/3.2.0-beta.4/reference/experimental-status).
...
Each beta is immutable. The `3.2-beta` documentation selector advances to the latest beta, while pinned protocol, schema, compliance, and documentation artifacts remain available. See the [3.2 beta program](/dist/docs/3.2.0-beta.4/reference/3-2-beta).
...
- `create_media_buy` and `update_media_buy` success bodies use `media_buy_status`; root `status` is the task-envelope state. See [Migrating to `media_buy_status`](/dist/docs/3.2.0-beta.4/reference/migration/media-buy-status).
...
The legacy field remains schema-valid for 3.x wire compatibility. Consumers must not activate received credentials automatically: they may process the legacy form only for an explicitly allowlisted peer and otherwise must reject or quarantine it. The field is eligible for removal in 4.0 or later only after both deprecation-policy gates are satisfied: at least six months from publication of this notice and at least one full release cycle. See [Migrating secured asset access](/dist/docs/3.2.0-beta.4/reference/migration/asset-access) and [#5699](https://github.com/adcontextprotocol/adcp/issues/5699) for removal tracking.
...
- [`list_creatives`](/dist/docs/3.2.0-beta.4/creative/task-reference/list_creatives) remains the creative-library query task.
...
See [`list_creative_formats`](/dist/docs/3.2.0-beta.4/creative/task-reference/list_creative_formats), [Creative Protocol specification § Conformance](/dist/docs/3.2.0-beta.4/creative/specification#conformance), and [Implementing creative agents](/dist/docs/3.2.0-beta.4/creative/implementing-creative-agents).
...
For adoption-oriented framing before the full change list, start with [What's New in AdCP 3.1](/dist/docs/3.2.0-beta.4/reference/whats-new-in-3-1). For a role-based upgrade checklist, use [Migrating from 3.0 to 3.1](/dist/docs/3.2.0-beta.4/reference/migration/3-0-to-3-1).
...
| A house running the AAO crawler today (or any consumer that walks brand hierarchy) | Read [`brand.json` § Mutual-assertion trust model](/dist/docs/3.2.0-beta.4/brand-protocol/brand-json#mutual-assertion-trust-model). The 3-tier read — identity-trusted vs relationship-trusted vs unverified — replaces the binary "is this in my portfolio" check for `brand_refs[]` pointer children. The 180-day TTL is already AAO's reference behavior. |
...
See [`brand.json` § Distributed publishing](/dist/docs/3.2.0-beta.4/brand-protocol/brand-json) for the full normative spec including resolution algorithm, Conformance MUSTs, adoption path, and prior art.
...
**Creative-agent canonical build capabilities.** Creative agents declare what they can produce via `creative.supported_formats[]` on `get_adcp_capabilities`. Every targetable entry carries an agent-local stable `capability_id`; buyers pass that ID as `target_capability_id` in `build_creative`. This is intentionally separate from media-buy `format_option_id`, which selects a product-local or publisher-catalog format option. The training agent and storyboards verify advertised capability IDs, canonical `format_kind` manifests, unsupported target rejection, and 3.0 compatibility gating. See [Canonical formats](/dist/docs/3.2.0-beta.4/creative/canonical-formats) and [`build_creative`](/dist/docs/3.2.0-beta.4/creative/task-reference/build_creative).

**Canonical format satisfaction at create time.** Media-buy create/update/read surfaces now preserve package-level canonical selectors, including direct `format_kind`/`params` requests. The conformance suite tests that under-specified selectors fail closed instead of matching products by loose overlap. See [Product discovery](/dist/docs/3.2.0-beta.4/media-buy/product-discovery/media-products).

**Per-version verification badge issuance.** Public verification runs issue badges for the selected public AdCP line. Badges are version-scoped, so 3.0 badges remain active independently of 3.1 badges. Buyers pinned to a specific AdCP release should read versioned badge data instead of flattening to a single current badge. See [Compliance catalog](/dist/docs/3.2.0-beta.4/building/verification/compliance-catalog) and [Versioning](/dist/docs/3.2.0-beta.4/reference/versioning).
...
The cache holds read responses too from 3.1 onward — sellers MUST encrypt the cache tier at rest with the same controls applied to the underlying resource store. Operators with read-heavy buyer mixes SHOULD raise the rule-8 insert ceiling (sized originally against write-heavy traffic). See [security.mdx § Idempotency](/dist/docs/3.2.0-beta.4/building/by-layer/L1/security#idempotency) for the full contract.
...
**`refine[]` finalize-exclusivity (#4107).** If any `refine[]` entry on `get_products` has `action: 'finalize'`, ALL entries MUST be proposal-scoped finalize entries. Mixing finalize with non-finalize entries is rejected with `INVALID_REQUEST`. Multi-finalize is atomic across proposal_ids; sellers that can't guarantee atomicity reject with `INVALID_REQUEST` (or `MULTI_FINALIZE_UNSUPPORTED` for a more specific signal). See [refinement guide § Finalize is exclusive](/dist/docs/3.2.0-beta.4/media-buy/product-discovery/refinement#finalize-is-exclusive-within-refine).
...
See [Metric lifecycle](/dist/docs/3.2.0-beta.4/media-buy/media-buys/optimization-reporting#metric-lifecycle) for the full capability → commitment → optimization → delivery picture and [`kind: vendor_metric`](/dist/docs/3.2.0-beta.4/media-buy/conversion-tracking/#kind-vendor_metric) for the goal-shape reference.
...
`adcp_major_version` (the 3.0 integer field) and `adcp.major_versions` remain functional through all of 3.x — additive ship, no required changes for 3.0-conformant agents. The full migration table lives in [Versioning & Governance § Migration timeline](/dist/docs/3.2.0-beta.4/reference/versioning#migration-timeline).
...
| Implementing governance or TMP | Review the experimental-surface additions below. Per the [experimental-status contract](/dist/docs/3.2.0-beta.4/reference/experimental-status), additive changes are permitted within a patch release. |
...
- **URL canonicalization** — Now applies uniformly to brand.json agent URLs (`brand_agent_entry.url`, `brand_agent.url`, `rights_agent.url`). Two URLs differing only in case, default port, or percent-encoded unreserved characters compare equal during agent resolution. New reference page at [URL canonicalization](/dist/docs/3.2.0-beta.4/reference/url-canonicalization).
...
Per the [experimental-surface contract](/dist/docs/3.2.0-beta.4/reference/experimental-status), these surfaces accept additive changes within a patch release. Changes here do not affect agents that don't implement these protocols.
...
**Status:** General Availability | [AdCP 3.0 overview](/dist/docs/3.2.0-beta.4/reference/whats-new-in-v3)
...
<Info>
**Upgrading from rc.3?** See [rc.3 → 3.0 prerelease upgrade notes](/dist/docs/3.2.0-beta.4/reference/migration/prerelease-upgrades) for the breaking changes table, before/after examples, and migration steps.
</Info>
...
- **Capabilities** — Remove all boolean capability checks (`features.content_standards`, `brand.identity`, `trusted_match.supported`, etc.). Test for object presence instead. See the [prerelease upgrade notes](/dist/docs/3.2.0-beta.4/reference/migration/prerelease-upgrades#capabilities-model-simplification) for the full field list.
- **Products** — Ensure all products include `reporting_capabilities`. This field is now required.
- **Media buy status** — Replace `pending_activation` with `pending_creatives` and `pending_start`. See the [state machine](/dist/docs/3.2.0-beta.4/media-buy/media-buys#state-machine).
...
- **Upgrading from v2?** Start with the [migration guide](/dist/docs/3.2.0-beta.4/reference/migration).
- **Upgrading from a prerelease?** Go to the [prerelease upgrade notes](/dist/docs/3.2.0-beta.4/reference/migration/prerelease-upgrades).
- **New to AdCP?** Read the [3.0 overview](/dist/docs/3.2.0-beta.4/reference/whats-new-in-v3) and then [get started](/dist/docs/3.2.0-beta.4/quickstart).

---

## Version 3.0.0-rc.3

**Status:** Release Candidate (superseded by 3.0.0) | [AdCP 3.0 overview](/dist/docs/3.2.0-beta.4/reference/whats-new-in-v3) | [SDK support](/dist/docs/3.2.0-beta.4/building/schemas-and-sdks#adcp-3-0-support)
...
| Governance context | Structured `governance-context.json` schema | Signed-JWS `governance_context` string (opaque to forwarders, cryptographically verifiable by auditors — see [security.mdx](/dist/docs/3.2.0-beta.4/building/by-layer/L1/security#signed-governance-context)) |
...
**Status:** Release Candidate | [AdCP 3.0 overview](/dist/docs/3.2.0-beta.4/reference/whats-new-in-v3)
...
**Status:** Beta | [AdCP 3.0 overview](/dist/docs/3.2.0-beta.4/reference/whats-new-in-v3)
...
4. **Commerce Media** - Catalog-driven product discovery (`catalog` on `get_products`), catalog-driven packages, per-catalog-item delivery reporting (`by_catalog_item`), store catchment targeting, and `catalog_types` on products. Cross-retailer GTIN matching via catalog selectors. Commerce attribution metrics (`roas`, `new_to_brand_rate`) in delivery response. See the [Commerce Media Guide](/dist/docs/3.2.0-beta.4/media-buy/commerce-media) and [Catalogs](/dist/docs/3.2.0-beta.4/creative/catalogs).
...
**Status:** Beta | [Full Changelog](https://github.com/adcontextprotocol/adcp/blob/main/CHANGELOG.md#300-beta2) | [AdCP 3.0 overview](/dist/docs/3.2.0-beta.4/reference/whats-new-in-v3)
...
**Status:** Beta | [Full Changelog](https://github.com/adcontextprotocol/adcp/blob/main/CHANGELOG.md#300-beta1) | [AdCP 3.0 overview](/dist/docs/3.2.0-beta.4/reference/whats-new-in-v3)
...
Version 3.0.0 is a **major release** that expands AdCP beyond media buying into governance, brand suitability, and conversational commerce. See the [AdCP 3.0 overview](/dist/docs/3.2.0-beta.4/reference/whats-new-in-v3) for detailed upgrade instructions.

**🎯 Key Themes:**

1. **Media Channel Taxonomy** - Complete overhaul from 9 format-oriented channels to 19 planning-oriented channels that reflect how buyers allocate budgets. See [Media Channel Taxonomy](/dist/docs/3.2.0-beta.4/reference/media-channel-taxonomy).
...
3. **Sponsored Intelligence Protocol** - Conversational brand experiences in AI assistants. Defines how hosts invoke brand agent endpoints for rich engagement without breaking conversational flow. See [Sponsored Intelligence](/dist/docs/3.2.0-beta.4/sponsored-intelligence/si-chat-protocol).
...
See [AdCP 3.0 overview](/dist/docs/3.2.0-beta.4/reference/whats-new-in-v3) for detailed before/after examples and migration steps.
...
See [SI Chat Protocol](/dist/docs/3.2.0-beta.4/sponsored-intelligence/si-chat-protocol) for complete documentation.
...
- [ ] Update channel enum values ([taxonomy guide](/dist/docs/3.2.0-beta.4/reference/media-channel-taxonomy))
...
**[View AdCP 3.0 overview →](/dist/docs/3.2.0-beta.4/reference/whats-new-in-v3)**
...
- `/schemas/3.2.0-beta.4/` - Backward compat alias (same as v2)
...
See [Versioning & Governance](/dist/docs/3.2.0-beta.4/reference/versioning) for the canonical version tiers, release cadence, 3.x stability guarantees, and deprecation policy. See the [v2 sunset page](/dist/docs/3.2.0-beta.4/reference/v2-sunset) for the v2 end-of-life timeline.
...
- **Community** - [Slack](/dist/docs/3.2.0-beta.4/community/joining-slack)
