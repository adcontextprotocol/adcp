...
| `offering_token` | string | No | Token from [`si_get_offering`](/dist/docs/3.2.0-beta.4/sponsored-intelligence/tasks/si_get_offering) for correlation |
| `sponsored_context_receipt` | object | No | Host receipt for sponsored context accepted before session initiation |
| `context` | object | No | Opaque correlation data echoed unchanged in the response (see [application context](/dist/docs/3.2.0-beta.4/building/by-layer/L2/context-sessions#application-context-context)) |

### Offering Token

If a host performed a [`si_get_offering`](/dist/docs/3.2.0-beta.4/sponsored-intelligence/tasks/si_get_offering) lookup before initiating, include the token for **session continuity**:
...
```json
{
  "$schema": "/schemas/3.2.0-beta.4/sponsored-intelligence/si-initiate-session-request.json",
...
```json
{
  "$schema": "/schemas/3.2.0-beta.4/sponsored-intelligence/si-initiate-session-response.json",
  "status": "completed",
