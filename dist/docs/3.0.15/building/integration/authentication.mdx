...
`Authorization` is the canonical header for both MCP and A2A. Sellers MAY accept a legacy alias during migration, but MUST also accept the standard `Authorization: Bearer ...` or `Authorization: Basic ...` form on the same endpoint.
