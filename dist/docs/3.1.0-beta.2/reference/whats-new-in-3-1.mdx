...
For per-PR detail and migration tables, see [Release Notes § Version 3.1.0](/dist/docs/3.1.0-beta.2/reference/release-notes#version-3-1-0-unreleased). For long-form normative reference, follow the link on each headline below.

<Note>
**Looking for the major v2 → v3 changes instead?** See [What's New in AdCP 3](/dist/docs/3.1.0-beta.2/reference/whats-new-in-v3). This page covers the 3.0 → 3.1 minor delta only.
</Note>
...
| **Catalog mirroring** | Re-fetch wholesale on every poll to detect changes | ETag-style `catalog_version` / `if_catalog_version` conditional fetch on `get_products` / `get_signals`; `cache_scope` (public/account) on every response for two-layer cache layering |
| **Wholesale signals** | `get_signals` required `signal_spec` or `signal_ids` — no protocol-conformant way to enumerate the full priced catalog | `discovery_mode: "wholesale"` symmetric with `get_products buying_mode: "wholesale"`; paginated full-catalog enumeration with `pricing_options[]` populated |
| **Per-agent change feed** | None | `GET /catalog/events` (UUID v7 cursor) emits `product.*` / `signal.*` / `catalog.bulk_change` events with `applies_to.scope` for cache invalidation; optional webhook subscriptions with HMAC anti-replay (signed timestamp + monotonic delivery seq) and SSRF guards |
...
→ Normative spec: [`brand.json` § Distributed publishing](/dist/docs/3.1.0-beta.2/brand-protocol/brand-json#distributed-publishing) · PR [#4505](https://github.com/adcontextprotocol/adcp/pull/4505)
...
→ Spec: [Brand Protocol § verify_brand_claim](/dist/docs/3.1.0-beta.2/brand-protocol/tasks/verify_brand_claim) · PRs [#4540](https://github.com/adcontextprotocol/adcp/pull/4540), [#4603](https://github.com/adcontextprotocol/adcp/pull/4603)
...
→ Spec: [Media Buy Lifecycle § Health & impairments](/dist/docs/3.1.0-beta.2/media-buy/media-buys/lifecycle#health-impairments) · [Snapshot and log contract](/dist/docs/3.1.0-beta.2/protocol/snapshot-and-log) · RFC #2853 · PRs #4588, #4601, #4677, #4685, #4690
...
→ Spec: [Snapshot and log contract](/dist/docs/3.1.0-beta.2/protocol/snapshot-and-log) · [Webhooks § Persistent channel contract](/dist/docs/3.1.0-beta.2/building/by-layer/L3/webhooks#persistent-channel-contract) · RFC #4582 · PRs #4601, #4701, #4730
...
Capability declarations: `catalog_versioning` (conditional fetch + `pricing_version_separate` + `cache_scope_account`), `catalog_change_feed` (events / webhooks / retention window / event types), `media_buy.buying_modes` and `signals.discovery_modes` (wholesale support). JSON Schema for the event payload at `core/catalog-event.json` (discriminated on event_type with 9 branches + `appliesTo` / `removalReason` `$defs`) and the feed-poll response wrapper at `core/catalog-events-response.json`.

→ Spec: [Catalog change feed](https://github.com/adcontextprotocol/adcp/blob/main/specs/catalog-change-feed.md) · [`get_products` § Catalog versioning](/dist/docs/3.1.0-beta.2/media-buy/task-reference/get_products#catalog-versioning) · [`get_products` § Cache layering](/dist/docs/3.1.0-beta.2/media-buy/task-reference/get_products#cache-layering) · [`get_signals` § Wholesale enumeration](/dist/docs/3.1.0-beta.2/signals/tasks/get_signals#wholesale-catalog-enumeration) · PRs [#4761](https://github.com/adcontextprotocol/adcp/pull/4761) (conditional fetch), [#4762](https://github.com/adcontextprotocol/adcp/pull/4762) (wholesale signals), [#4763](https://github.com/adcontextprotocol/adcp/pull/4763) (change feed), [#4767](https://github.com/adcontextprotocol/adcp/pull/4767) (cluster implementation)
...
→ Spec: [Canonical formats](/dist/docs/3.1.0-beta.2/creative/canonical-formats) · PRs [#3307](https://github.com/adcontextprotocol/adcp/pull/3307), [#4770](https://github.com/adcontextprotocol/adcp/pull/4770)
...
→ Spec: [Versioning § Version negotiation](/dist/docs/3.1.0-beta.2/reference/versioning#version-negotiation) · PR [#3493](https://github.com/adcontextprotocol/adcp/pull/3493)
...
→ Spec: [Optimization goals § `vendor_metric` kind](/dist/docs/3.1.0-beta.2/media-buy/media-buys/optimization-reporting#vendor-metric-goals) · PRs [#4668](https://github.com/adcontextprotocol/adcp/pull/4668), [#4669](https://github.com/adcontextprotocol/adcp/pull/4669), [#4649](https://github.com/adcontextprotocol/adcp/pull/4649)
...
→ Spec: [Delivery metrics reference](/dist/docs/3.1.0-beta.2/media-buy/task-reference/get_media_buy_delivery) · PRs [#4618](https://github.com/adcontextprotocol/adcp/pull/4618), [#4601](https://github.com/adcontextprotocol/adcp/pull/4601)
...
→ Spec: [Billing measurement](/dist/docs/3.1.0-beta.2/media-buy/advanced-topics/accountability#billing-measurement) · [`report_usage`](/dist/docs/3.1.0-beta.2/accounts/tasks/report_usage) · PRs [#4735](https://github.com/adcontextprotocol/adcp/pull/4735), [#4561](https://github.com/adcontextprotocol/adcp/pull/4561)
...
→ Spec: [Media Buy Lifecycle § Action discovery](/dist/docs/3.1.0-beta.2/media-buy/media-buys/lifecycle#action-discovery) · PR [#4514](https://github.com/adcontextprotocol/adcp/pull/4514)
...
→ Spec: [Calling an agent § Idempotency](/dist/docs/3.1.0-beta.2/protocol/calling-an-agent) · PRs [#4402](https://github.com/adcontextprotocol/adcp/pull/4402), [#4409](https://github.com/adcontextprotocol/adcp/pull/4409)
...
→ Spec: [TMP IdentityMatch implementation](/dist/docs/3.1.0-beta.2/trusted-match/identity-match-implementation) · PRs [#4070](https://github.com/adcontextprotocol/adcp/pull/4070), [#3687](https://github.com/adcontextprotocol/adcp/pull/3687)
...
→ Spec: [`adagents.json` reference](/dist/docs/3.1.0-beta.2/governance/property/adagents) · PRs [#4504](https://github.com/adcontextprotocol/adcp/pull/4504), [#4173](https://github.com/adcontextprotocol/adcp/pull/4173), [#4536](https://github.com/adcontextprotocol/adcp/pull/4536)

### Compliance suite — capability-gated scenarios

Capability-gated storyboard scenarios let sellers run *only* what they claim. New scenarios for `frequency_cap_enforcement`, `per_creative_attribution`, `metric_mode` + ROAS (using a `contains:` matcher), `audience_buy_flow`, `event_dedup_flow`, and `performance_buy_flow` (capability-gated CPA buys). Plus a new `requires` runtime gate on Storyboards that conditions execution on declared capabilities — no more all-or-nothing scenarios. The full set is enumerated on the [Compliance catalog](/dist/docs/3.1.0-beta.2/building/verification/compliance-catalog).

→ Spec: [Compliance catalog](/dist/docs/3.1.0-beta.2/building/verification/compliance-catalog) · PRs [#4312](https://github.com/adcontextprotocol/adcp/pull/4312), [#4642](https://github.com/adcontextprotocol/adcp/pull/4642), [#4664](https://github.com/adcontextprotocol/adcp/pull/4664), [#4722](https://github.com/adcontextprotocol/adcp/pull/4722), [#4727](https://github.com/adcontextprotocol/adcp/pull/4727), [#4731](https://github.com/adcontextprotocol/adcp/pull/4731)
...
→ See [Release Notes § Version 3.1.0](/dist/docs/3.1.0-beta.2/reference/release-notes#version-3-1-0-unreleased) for the per-PR detail.
...
**Validator obligation for 3.1 SDKs.** The catalog-sync cluster tightens one shape within 3.1: `cache_scope` is schema-required on every `get_products` / `get_signals` response (the safety property of the two-layer cache depends on it — see [Cache layering](/dist/docs/3.1.0-beta.2/media-buy/task-reference/get_products#cache-layering)). Pre-3.1 sellers correctly omit the field and remain conformant to their declared version. SDKs that validate strictly against the 3.1 schema MUST select the validator based on the server-declared `adcp_version` (the same release-precision mechanism 3.1 ships in version negotiation): for responses with `adcp_version` starting `3.0`, the 3.1 `cache_scope`-required constraint MUST be relaxed. This is a tightening within 3.1, not a 3.0 break — but SDKs that hardcode the 3.1 schema without version-pinned validation will reject correct 3.0 traffic. Version-pinned validation is the right pattern for every 3.x→3.(x+1) tightening; cache_scope is the first time it's load-bearing.

For the per-PR detail, see [Release Notes § Version 3.1.0](/dist/docs/3.1.0-beta.2/reference/release-notes#version-3-1-0-unreleased). For the version-negotiation cadence and the 3.1 → 3.2 → 4.0 timeline, see [Versioning & Governance § Migration timeline](/dist/docs/3.1.0-beta.2/reference/versioning#migration-timeline).
