...
For the layered model behind this — what each layer contains and what an SDK at each layer should provide — see the [SDK stack reference](/dist/docs/3.2.0-beta.9/building/cross-cutting/sdk-stack).

## Coverage matrix

What "shipped" means at each layer is the [L0–L3 checklist](/dist/docs/3.2.0-beta.9/building/cross-cutting/sdk-stack#what-an-sdk-at-each-layer-should-provide).

*Last updated: 2026-08-28.*

| SDK | Current package | 3.2 beta support | L0 | L1 | L2 | L3 |
|---|---|---|:-:|:-:|:-:|:-:|
| **`@adcp/sdk`** (TypeScript) | `14.0.0-beta.13` | Exact `3.2.0-beta.6` support | ✅ | ✅ | ✅ | ✅ |
| **`adcp`** (Python) | `8.0.0b9` | Exact `3.2.0-beta.6` schemas and types; higher-level helpers remain partial | ✅ | ⚠️ | ⚠️ | ⚠️ |
...
exact beta.6 protocol checkpoint; neither implies compatibility with a later
checkpoint. Go has not published an exact
3.2 support statement yet. See the [3.2 beta program](/dist/docs/3.2.0-beta.9/reference/3-2-beta).
</Warning>
...
For the status of every published protocol version, see [Versions & Compatibility](/dist/docs/3.2.0-beta.9/reference/versions). Migration and archived-version guidance lives under [Release notes & migration](/dist/docs/3.2.0-beta.9/reference/migration/index).
...
community-maintained ports are welcome — see the [Builders Working Group](/dist/docs/3.2.0-beta.9/community/working-group)
and the [Slack community](/dist/docs/3.2.0-beta.9/community/joining-slack).
...
- `@adcp/sdk/testing` — buyer-side storyboard runner (`runStoryboard`, `comply`, `testAgent`) and seller-side controller scaffold (`createComplyController` — see [Get Test-Ready](/dist/docs/3.2.0-beta.9/building/verification/get-test-ready))
...
```bash
npx @adcp/sdk@14.0.0-beta.13 --help
npx @adcp/sdk@14.0.0-beta.13 --save-auth my-agent https://sales.example.com/mcp
npx @adcp/sdk@14.0.0-beta.13 my-agent list_products '{"adcp_version":"3.2-beta.6","account":{"account_id":"account_123"}}'
# or against the built-in public test agent:
npx @adcp/sdk@14.0.0-beta.13 test-mcp list_products '{"adcp_version":"3.2-beta.6"}'
```
...
- **[Build an agent](/dist/docs/3.2.0-beta.9/building/by-layer/L4/build-an-agent)** — server-side L4 path. Skill files + coding agent.
- **[Build a caller](/dist/docs/3.2.0-beta.9/building/by-layer/L4/build-a-caller)** — client-side L4 path. Install, call, handle responses, ingest reporting.
- **[Schemas](/dist/docs/3.2.0-beta.9/building/by-layer/L0/schemas)** — schema bundle, type generation, version pinning.
- **[Migrate from hand-rolled](/dist/docs/3.2.0-beta.9/building/by-layer/L4/migrate-from-hand-rolled)** — already running an AdCP agent built before the SDKs covered much? Swap one layer at a time.
