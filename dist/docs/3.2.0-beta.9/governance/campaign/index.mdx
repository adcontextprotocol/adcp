...
The orchestrator pushes campaign plans via [`sync_plans`](/dist/docs/3.2.0-beta.9/governance/campaign/tasks/sync_plans), then calls [`check_governance`](/dist/docs/3.2.0-beta.9/governance/campaign/tasks/check_governance) before commitment-bearing operations. Every enforcing service verifies the approved `governance_context`; media-buy services that claim online execution checking additionally validate `planned_delivery` before commit. After the response, the orchestrator calls [`report_plan_outcome`](/dist/docs/3.2.0-beta.9/governance/campaign/tasks/report_plan_outcome) to close the loop.
...
2. The governance agent resolves the brand via the [Brand Protocol](/dist/docs/3.2.0-beta.9/brand-protocol/index) and retrieves its compliance configuration
...
Service-side governance checks solve this. The buyer syncs governance agents (URLs with credentials) via [`sync_governance`](/dist/docs/3.2.0-beta.9/accounts/tasks/sync_governance). When a service receives a governed commitment, it prepares what it plans to deliver and calls the governance agent before committing. The execution decision is binary: `approved` or `denied`.
...
- Compliance coverage from the [policy registry](/dist/docs/3.2.0-beta.9/governance/policy-registry) -- registry policies are community-maintained, no per-brand configuration required
...
| Domain | Relationship |
|--------|-------------|
| **[Property Governance](/dist/docs/3.2.0-beta.9/governance/property/index)** | Campaign Governance validates that the orchestrator *uses* property lists correctly (e.g., not bypassing them). Property Governance provides the lists. |
| **[Content Standards](/dist/docs/3.2.0-beta.9/governance/content-standards/index)** | Campaign Governance validates that content standards references are included in media buys. Content Standards handles the actual content evaluation. |
| **[Creative Governance](/dist/docs/3.2.0-beta.9/governance/creative/index)** | Campaign Governance validates that creative assignments match format requirements. Creative Governance scans the creatives themselves. |
| **[Brand Protocol](/dist/docs/3.2.0-beta.9/brand-protocol/index)** | Campaign Governance resolves the brand's compliance configuration via the Brand Protocol. The brand's policy team configures compliance policies in brand.json; the governance agent applies them automatically. |

## Next steps

<CardGroup cols={2}>
  <Card title="Responsibilities" icon="users" href="/dist/docs/3.2.0-beta.9/governance/campaign/responsibilities">
    What buyer-side orchestrators, sellers, and governance agents each check and carry.
  </Card>
  <Card title="Safety model" icon="shield-check" href="/dist/docs/3.2.0-beta.9/governance/campaign/safety-model">
    How the three-party trust model, separation of duties, and structural controls make agentic advertising safe.
  </Card>
  <Card title="Specification" icon="file-lines" href="/dist/docs/3.2.0-beta.9/governance/campaign/specification">
    Data models, validation logic, capability declaration, and orchestrator integration patterns.
  </Card>
  <Card title="Tasks" icon="list-check" href="/dist/docs/3.2.0-beta.9/governance/campaign/tasks/index">
