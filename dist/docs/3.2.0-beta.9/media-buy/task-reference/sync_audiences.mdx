...
Manage first-party CRM audiences on a seller account. Upload hashed customer lists, check matching status, and reference the resulting audiences in [`create_media_buy`](/dist/docs/3.2.0-beta.9/media-buy/task-reference/create_media_buy) targeting overlays for explicit retargeting or suppression.

Audiences are distinct from [signals](/dist/docs/3.2.0-beta.9/signals/): signals are named targeting dimensions discovered through product signal options, provider-published signal definitions, or [`get_signals`](/dist/docs/3.2.0-beta.9/signals/tasks/get_signals); audiences are data you own and upload. Use `audience_include` to target only members of an uploaded list. `audience_include` is a hard constraint — only users on the list are eligible. A signal returned by `get_signals` is not a synced audience ID and instead belongs in [`signal_targeting_groups`](/dist/docs/3.2.0-beta.9/media-buy/advanced-topics/targeting#signal_targeting_groups). To find new users *similar to* an audience (lookalike expansion), describe that intent in your campaign brief — the seller handles expansion strategy. Note: lookalike intent expressed in the brief cannot be verified through the protocol; confirm via seller-side reporting.
...
**Request Schema**: [`/schemas/3.2.0-beta.9/media-buy/sync-audiences-request.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.9/media-buy/sync-audiences-request.json)
**Response Schema**: [`/schemas/3.2.0-beta.9/media-buy/sync-audiences-response.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.9/media-buy/sync-audiences-response.json)
...
|-----------|------|----------|-------------|
| `account` | [account-ref](/dist/docs/3.2.0-beta.9/building/by-layer/L2/accounts-and-agents#account-references) | Yes | Account reference. Pass `{ "account_id": "..." }` or `{ "brand": {...}, "operator": "..." }` if the seller supports implicit resolution. |
...
**Identifier support varies by seller**: Check [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.9/protocol/get_adcp_capabilities) → `media_buy.audience_targeting.supported_identifier_types` and `media_buy.audience_targeting.supported_uid_types` before sending. MAID support is not universal (LinkedIn does not accept MAIDs; iOS IDFA requires App Tracking Transparency consent). The `media_buy.audience_targeting.matching_latency_hours` range and `media_buy.audience_targeting.minimum_audience_size` in capabilities are also seller-specific.

**Activation paths beyond inline push**: Inline members are one activation method. Sellers declare which paths they support — AdCP push, dataset sharing, clean rooms, platform distribution — via [`audience_activation`](/dist/docs/3.2.0-beta.9/media-buy/product-discovery/media-products#audience-activation) on products and `media_buy.audience_targeting.supported_activation_methods` in [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.9/protocol/get_adcp_capabilities). Filter discovery with `filters.audience_activation_methods` on [`get_products`](/dist/docs/3.2.0-beta.9/media-buy/task-reference/get_products) to find inventory compatible with your data pipeline. [External source references](#external-audience-sources-experimental) — pointing this task at a shared dataset or vendor-distributed segment instead of pushing members — are available experimentally on sellers that declare the corresponding activation patterns.
...
- `task_id` — Handle for polling via [`get_task_status`](https://adcontextprotocol.org/schemas/3.2.0-beta.9/protocol/get-task-status-request.json) on the A2A profile (or the advertised AdCP polling task on MCP), or receiving a webhook on completion
...
Once an audience is `ready`, reference it by `audience_id` in [`create_media_buy`](/dist/docs/3.2.0-beta.9/media-buy/task-reference/create_media_buy) targeting overlays. Audience IDs are scoped to the seller account — they cannot be used across sellers.
...
1. Read the seller's declared `consumer_identities` from its [`audience_activation`](/dist/docs/3.2.0-beta.9/media-buy/product-discovery/media-products#audience-activation) declaration.
2. Grant access in the vendor's system (e.g. `ALTER SHARE audience_share ADD ACCOUNTS = SELLERORG.INGEST_US`). Out of band — AdCP never carries credentials. The grant MUST name a principal without conveying a secret through AdCP: Databricks D2D and open sharing with OIDC token federation qualify; the classic bearer-token recipient profile does not. A request carrying credential material is rejected with [`CREDENTIAL_IN_ARGS`](/dist/docs/3.2.0-beta.9/building/verification/compliance-catalog#error-code-credential-in-args).
...
4. The seller reads, matches, and the audience moves `processing` → `ready` / `too_small` exactly as for pushed audiences. A missing or revoked grant is a retryable per-audience failure (`action: "failed"` with [`SOURCE_ACCESS_FAILED`](/dist/docs/3.2.0-beta.9/building/verification/compliance-catalog#error-code-source-access-failed)) — grant and retry.
...
- **Transport is fixed at creation.** An audience is either buyer-pushed or externally sourced for its lifetime. Cross-transport upserts (member deltas against a sourced audience, or `source` against a pushed one) are rejected with [`CONFLICT`](/dist/docs/3.2.0-beta.9/building/verification/compliance-catalog#error-code-conflict) (`error.field: "audience_id"`); convert by delete-and-recreate.
...
**Webhook (recommended)**: Configure `push_notification_config` at the protocol level before uploading. The task stays active while the seller's platform matches members. When matching completes, the task completes and the webhook fires with the final result — `status: "ready"` or `status: "too_small"`. Check [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.9/protocol/get_adcp_capabilities) → `audience_targeting.matching_latency_hours` to set realistic expectations (typically 1–48 hours).

**Polling fallback**: If not using webhooks, poll with discovery-only calls (omit `audiences`) no more frequently than every 15 minutes. A2A profile callers use [`get_task_status`](https://adcontextprotocol.org/schemas/3.2.0-beta.9/protocol/get-task-status-request.json) with the `task_id`; MCP callers use the advertised AdCP polling task. The task will be `submitted` while matching is in progress and `completed` when the audience is ready or too small.

**Agent workflow**: Upload with `push_notification_config` set. Externalize the `audience_id` and `account_id` before the session ends. When the webhook fires with `status: "ready"`, resume and proceed to [`create_media_buy`](/dist/docs/3.2.0-beta.9/media-buy/task-reference/create_media_buy).
...
**Hashed identifiers are pseudonymous PII, not anonymous.** Unsalted SHA-256 of an email or phone number is recoverable via precomputed dictionaries of the email and E.164 namespaces, so `hashed_email` and `hashed_phone` MUST be treated as PII for retention, consent, access control, and data-subject-request purposes. Do not describe them as "privacy-preserving" in operator documentation or DPAs. See [Privacy Considerations](/dist/docs/3.2.0-beta.9/reference/privacy-considerations#unsalted-hashed-identifiers-are-pseudonymous-not-anonymous).
...
|------------|-------------|------------|
| [`ACCOUNT_NOT_FOUND`](/dist/docs/3.2.0-beta.9/building/verification/compliance-catalog#error-code-account-not-found) | Account does not exist | Verify `account_id` |
| [`REFERENCE_NOT_FOUND`](/dist/docs/3.2.0-beta.9/building/verification/compliance-catalog#error-code-reference-not-found) | Audience to remove from doesn't exist or is not accessible (`error.field` = `audience_id`) | Check `audience_id` or omit `remove` |
| [`INVALID_REQUEST`](/dist/docs/3.2.0-beta.9/building/verification/compliance-catalog#error-code-invalid-request) | Identifier format or payload size is invalid | Verify SHA-256 hex encoding and keep the payload within the documented limit |
| [`RATE_LIMITED`](/dist/docs/3.2.0-beta.9/building/verification/compliance-catalog#error-code-rate-limited) | Too many sync requests | Retry with exponential backoff; poll no more than every 15 minutes |
| [`SOURCE_ACCESS_FAILED`](/dist/docs/3.2.0-beta.9/building/verification/compliance-catalog#error-code-source-access-failed) | Seller could not read an external audience source (per-audience, experimental) | `error.field` `source.locator`/`source.segment_ref`: fix the reference; `source`: grant the seller's declared consumer identity and retry; no `error.field`: vendor outage — retry with backoff |
| [`CONFLICT`](/dist/docs/3.2.0-beta.9/building/verification/compliance-catalog#error-code-conflict) | Cross-transport upsert: member deltas against a sourced audience or `source` against a pushed one (`error.field` = `audience_id`) | Transport is fixed at creation — delete and recreate to convert |
| [`UNSUPPORTED_FEATURE`](/dist/docs/3.2.0-beta.9/building/verification/compliance-catalog#error-code-unsupported-feature) | `source` kind whose activation pattern the seller has not declared | Check the seller's `audience_activation` declarations before sending source references |

## Next Steps

- [Targeting](/dist/docs/3.2.0-beta.9/media-buy/advanced-topics/targeting) — Reference audiences in `targeting_overlay.audience_include` and `audience_exclude`
- [create_media_buy](/dist/docs/3.2.0-beta.9/media-buy/task-reference/create_media_buy) — Apply audience targeting to packages
- [Conversion Tracking](/dist/docs/3.2.0-beta.9/media-buy/conversion-tracking/) — Track outcomes from audience-targeted campaigns
