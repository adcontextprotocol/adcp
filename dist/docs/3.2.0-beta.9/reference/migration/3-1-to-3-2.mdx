...
`@adcp/sdk@14.0.0-beta.13` and Python `adcp==8.0.0b9` embed beta.6, so use the
signed beta.9 artifact directly unless your SDK explicitly names beta.9.
...
For the feature narrative, start with [What's New in AdCP 3.2](/dist/docs/3.2.0-beta.9/reference/whats-new-in-3-2).
For prerelease artifacts and SDK timing, use the [3.2 beta program](/dist/docs/3.2.0-beta.9/reference/3-2-beta).
...
| 2 | SDK and codegen maintainers | Generate from the signed `3.2.0-beta.9` protocol tarball, not `main` or `/schemas/3.2.0-beta.9/`. |
...
| 8 | Compliance operators | Test beta.9 from its signed assets, or pair `@adcp/sdk@14.0.0-beta.13` or Python `adcp==8.0.0b9` with exact beta.6; require an explicit beta.9 support statement before using any SDK with the new checkpoint. |
...
until 4.0. See [Migrating to `media_buy_status`](/dist/docs/3.2.0-beta.9/reference/migration/media-buy-status)
...
webhook receivers. See [Signed requests](/dist/docs/3.2.0-beta.9/building/by-layer/L1/security#signed-requests-transport-layer).
...
Follow the complete field map in [Targeting-aware product discovery](/dist/docs/3.2.0-beta.9/reference/migration/targeting-aware-discovery).
...
automatically. See [Migrating secured asset access](/dist/docs/3.2.0-beta.9/reference/migration/asset-access).
...
See [Cross-role governance enforcement](/dist/docs/3.2.0-beta.9/reference/migration/cross-role-governance-enforcement).
