...
`brand` is an identity-only key. Sellers resolve its canonical manifest from the domain; callers do not send logos, colors, or governance overrides in this request. Catalog ingestion belongs to [`sync_catalogs`](/dist/docs/3.2.0-beta.2/media-buy/task-reference/sync_catalogs); discovery accepts only a compact catalog selection under `criteria.catalog`.
...
2. Register the durable account subscriber through [`sync_accounts`](/dist/docs/3.2.0-beta.2/accounts/tasks/sync_accounts).
