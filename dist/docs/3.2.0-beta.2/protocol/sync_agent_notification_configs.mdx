...
The initial event type is `capabilities.changed`, an invalidation signal for cached [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.2/protocol/get_adcp_capabilities) responses.

**Request Schema**: [`/schemas/3.2.0-beta.2/protocol/sync-agent-notification-configs-request.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.2/protocol/sync-agent-notification-configs-request.json)
**Response Schema**: [`/schemas/3.2.0-beta.2/protocol/sync-agent-notification-configs-response.json`](https://adcontextprotocol.org/schemas/3.2.0-beta.2/protocol/sync-agent-notification-configs-response.json)

## When to call

First call [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.2/protocol/get_adcp_capabilities). Only call this task when the seller advertises:
...
This task MUST only be accepted when the transport request resolves to a stable authenticated principal. The principal can be an OAuth client subject, API-key subject, verified signed-request subject, registry identity, or equivalent deployment identity, but it MUST be stable across retries and future replacements. Sellers MUST reject NO_AUTH, anonymous, or otherwise unbound registrations with [`AUTH_MISSING`](/dist/docs/3.2.0-beta.2/building/verification/compliance-catalog#error-code-auth-missing) when no credentials are present, [`AUTH_INVALID`](/dist/docs/3.2.0-beta.2/building/verification/compliance-catalog#error-code-auth-invalid) when presented credentials fail validation, or [`INVALID_REQUEST`](/dist/docs/3.2.0-beta.2/building/verification/compliance-catalog#error-code-invalid-request) when the transport exposes anonymous registration as a malformed request.
...
When the seller's advertised capability document materially changes, it publishes the new [`get_adcp_capabilities`](/dist/docs/3.2.0-beta.2/protocol/get_adcp_capabilities) snapshot first, then fires `capabilities.changed` to active subscribers:
