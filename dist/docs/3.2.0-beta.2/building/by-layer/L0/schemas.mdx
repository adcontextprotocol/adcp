...
| Source | URL | Best For |
|--------|-----|----------|
| Website | `https://adcontextprotocol.org/schemas/3.2.0-beta.2/` | Runtime fetching, version aliases |
...
Declare `supported_protocols` (for protocol baselines) and `specialisms` (for narrow capability claims) in `get_adcp_capabilities` — the compliance runner executes the matching bundles to verify. See the full [Compliance Catalog](/dist/docs/3.2.0-beta.2/building/verification/compliance-catalog) for every protocol and specialism an agent can claim.

## Common schemas

| Schema | URL |
|--------|-----|
| Product | `https://adcontextprotocol.org/schemas/3.2.0-beta.2/core/product.json` |
| Media Buy | `https://adcontextprotocol.org/schemas/3.2.0-beta.2/core/media-buy.json` |
| Creative Format | `https://adcontextprotocol.org/schemas/3.2.0-beta.2/core/format.json` |
| Schema Registry | `https://adcontextprotocol.org/schemas/3.2.0-beta.2/index.json` |
...
| Major version | `/schemas/3.2.0-beta.2/`, `/compliance/v3/` | Development, documentation |
...
```javascript
const schema = await fetch(
  'https://adcontextprotocol.org/schemas/3.2.0-beta.2/core/product.json'
);
```
...
See the [schema registry](https://adcontextprotocol.org/schemas/3.2.0-beta.2/index.json) for all available schemas.
...
curl https://adcontextprotocol.org/schemas/3.2.0-beta.2/index.json | jq '.published_version'
```
...
Check [Release Notes](/dist/docs/3.2.0-beta.2/reference/release-notes) for version history and migration guides.
...
<Card title="Registry API Reference" icon="server" href="/dist/docs/3.2.0-beta.2/registry/index">
