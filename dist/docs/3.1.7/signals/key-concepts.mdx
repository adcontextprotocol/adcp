...
| Task | Purpose |
|------|---------|
| [`get_signals`](/dist/docs/3.1.7/signals/tasks/get_signals) | Discover signals matching campaign criteria; required for every signals agent |
| [`activate_signal`](/dist/docs/3.1.7/signals/tasks/activate_signal) | Activate a marketplace signal for use in campaigns; required for `signal_marketplace` conformance |
...
The Signals Protocol operates within the broader [AdCP ecosystem](/dist/docs/3.1.7/intro#the-adcp-ecosystem-layers). Signal agents integrate directly with decisioning platforms (DSPs, orchestration platforms), eliminating intermediary reporting and usage tracking. Signal agents advertise their available data providers via [`get_adcp_capabilities`](/dist/docs/3.1.7/protocol/get_adcp_capabilities).
...
- [Data provider guide](/dist/docs/3.1.7/signals/data-providers) — how to publish signal definitions
- [Signals ecosystem](/dist/docs/3.1.7/signals/ecosystem) — how each type of company participates
- [Protocol specification](/dist/docs/3.1.7/signals/specification) — formal conformance requirements
- [get_signals task reference](/dist/docs/3.1.7/signals/tasks/get_signals) — discovery API details
- [activate_signal task reference](/dist/docs/3.1.7/signals/tasks/activate_signal) — activation API details
