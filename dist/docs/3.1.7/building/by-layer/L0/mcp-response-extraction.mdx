...
This page defines the normative algorithm for extracting AdCP success response data from MCP tool results. For error extraction, see [Transport Error Mapping](/dist/docs/3.1.7/building/operating/transport-errors).
...
| Error extraction ([transport-errors](/dist/docs/3.1.7/building/operating/transport-errors)) | `isError: true` | `structuredContent.adcp_error` or text fallback |
...
All data in `structuredContent` and `content[].text` is seller-controlled. The same prompt injection and data boundary requirements from [Transport Error Mapping](/dist/docs/3.1.7/building/operating/transport-errors#security-considerations) apply.
...
- [A2A Response Extraction](/dist/docs/3.1.7/building/by-layer/L0/a2a-response-extraction) — equivalent spec for A2A
