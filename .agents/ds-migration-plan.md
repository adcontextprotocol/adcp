...
- **`--info` and `--chart-3` repoint** — both currently resolve to the same value as `--primary` because the Tailwind `blue-*` ramp now holds the Scope3 brand-blue values (`blue-600` = #3055F8, where `--primary` points). Need to repoint `--info` / `--info-foreground` / `--chart-3` to a distinct ramp (sky or cyan are the natural candidates) so brand emphasis and informational signal don't visually collide. Address before any migrated page starts using `--info` or `--chart-3` in earnest.
...
- **2026-05-06:** Blue ramp consolidation. Tailwind's `blue-*` ramp now holds Scope3 brand-blue values (anchored at `blue-600` = #3055F8, smooth lightness bell from 50 to 1000, hue locked at ~267°), eliminating the parallel `brand-blue-*` namespace. Single source of truth for blue. `--primary` repointed to `blue-600` in both light and dark mode. Added custom `-1000` stops to every primitive ramp (slate, gray, neutral, blue, cyan, green, emerald, red, orange, amber, yellow) for flooding deep backgrounds — `blue-1000` = #071044. Color story gained an edge-to-edge **Primitives — Grid** view for at-a-glance ramp review. **Known follow-up tracked in Open questions:** `--info` and `--chart-3` currently collapse onto `--primary` since they reference Tailwind blue stops; need to repoint to a distinct ramp before pages start using them.
