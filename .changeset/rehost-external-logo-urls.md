...
**Backfill** for existing rows: `server/src/scripts/rehost-external-brand-logos.ts` walks every `brands` row with a manifest, finds external `logos[*].url` values under both `brand_manifest.brands[*].logos` and `brand_manifest.logos`, and rewrites each in place via the same `rehostExternalLogo` path. Default dry-run, `--apply` to write, `--domain <x>` to scope. Run via `fly ssh console -a adcp-docs -C 'node /app/dist/scripts/rehost-external-brand-logos.js --apply'`.
