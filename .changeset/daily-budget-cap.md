...
- **`get-media-buys-response.json`** echoes the accepted `daily_budget_cap` and resolved `budget_cap_timezone` on each package so buyers can verify enforcement and reconcile enforced-vs-requested.
