...
- **Duration type**: Added reusable `core/duration.json` schema (`{value, unit}` where unit is `"hours"`, `"days"`, or `"campaign"`). Replaces compact duration strings throughout the spec. When unit is `"campaign"`, value must be 1 — the window spans the full campaign flight. (#1215)
- **FrequencyCap.window**: Changed from pattern-validated string (`"7d"`) to a duration object (e.g. `{"value": 7, "unit": "days"}` or `{"value": 1, "unit": "campaign"}`). Also applied to `optimization_goal.target_frequency.window`. (#1215)
