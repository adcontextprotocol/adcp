...
- Forced-architecture walk-back per @bokelley: three 3.1 MUSTs that mandate specific operator architectures — HSM/KMS custody for governance keys, multi-region CDN + availability SLA on the revocation list, and origin separation across governance/transport/webhook JWKS — are downgraded from MUST to SHOULD and called out as candidate ISPs. Threat models stay; the forced-architecture bar is deferred to community discussion. The "software custody is rejected by conforming verifiers in 3.1+" cutover is removed — verifiers MAY refuse software-key counterparties as policy, but the protocol does not forbid `"software"` custody at the wire level in 3.x.
