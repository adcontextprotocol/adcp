...
- **`/schemas/tmp/available-package.json`** — `seller_agent` added as
  a required field. Breaking change for the experimental TMP surface:
  sellers syncing `AvailablePackage` payloads must now populate it.
...
TMP remains experimental under AdCP 3.x (see
`experimental_features: trusted_match.core`). The
`SellerAgentRef.id` slot and optional `ext` namespace leave room
to layer signed seller claims or an AAO-assigned opaque identifier
