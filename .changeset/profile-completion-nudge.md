...
- Admin `/admin/members` page adds three status columns — Profile public, brand.json manifest present, Announced — plus an "Announcement" filter (All / Ready but not announced / Not announce-ready / Already announced) so admins can see the announce-ready backlog at a glance and catch up on members who pre-date the workflow. "Announced" is sourced from `org_activities.announcement_published` entries (written by Workflow B once it lands).
