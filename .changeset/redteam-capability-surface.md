...
- **S-6 — Origin separation for JWKS publishing.** Governance signing keys MUST be served from a separate origin than transport and webhook signing keys. Operators advertise the layout via a new doc-only `identity.key_origins` capability map. When the field is present, verifiers MUST check origin separation at onboarding and reject on co-tenancy — the MUST is otherwise unverifiable on the wire.
