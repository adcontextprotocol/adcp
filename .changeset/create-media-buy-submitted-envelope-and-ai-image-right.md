...
- **`create-media-buy-response.json` now has three branches.** Added a `CreateMediaBuySubmitted` discriminator (`status: "submitted"`, required `task_id`, optional `taskId`/`message`) for async create_media_buy flows — e.g., guaranteed buys awaiting IO signing. The completion artifact still carries `media_buy_id` and `packages`; the submitted envelope forbids both. Before this change, sellers that followed the `sales-guaranteed` storyboard narrative emitted a response shape the schema rejected, so the specialism's namesake scenario could not pass. Fixes #2420.
