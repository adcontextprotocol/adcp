...
function stageBundle(bundleParent, version, schemasSource, rootDirName, isDev = false) {
  ...
  // adcp_version: legacy alias kept through 3.x for SDK compatibility.
  // Both carry full semver; distinct from the wire field defined in
  // core/version-envelope.json (release-precision).
  const manifest = {
