...
function lintStoryboardIdempotency(sourceDir, schemasDir) {
  ...
  const duplicateGeneratedKeyAllowedReplayKeys = new Set([
    'universal/agent-configuration.yaml:$generate:uuid_v4#agent_configuration_apply',
      ...
      for (const step of phase.steps) {
        ...
        if (hasKey) {
          ...
          if (!isGeneratedIdempotencyKey(key, generatedContextNames)) {
            ...
            !duplicateGeneratedKeyAllowedFiles.has(rel)
            && !duplicateGeneratedKeyAllowedReplayKeys.has(`${rel}:${key}`)
          ) {
  ...
  if (duplicateGeneratedKeyViolations.length > 0) {
    ...
    throw new Error(
      ...
      `Only explicitly allowlisted replay vectors may intentionally reuse generated keys.`
    );
  }
